...
static UINT8
    used_spr[(NUMSPRITEFREESLOTS / 8) +
             1]; // Bitwise flag for sprite freeslot in use! I would use ceil()
                 // here if I could, but it only saves 1 byte of memory anyway.
...
FUNCPRINTF static void deh_warning(const char *first, ...) {
  ...
  vsnprintf(
      buf, 1000, first,
      argptr); // sizeof only returned 4 here. it didn't like that pointer.
  va_end(argptr);
...
static void deh_strlcpy(char *dst, const char *src, size_t size,
                        const char *warntext) {
...
INT32 flags;         Bits = 3232
MF_SOLID|MF_SHOOTABLE|MF_DROPOFF|MF_PICKUP|MF_NOTDMATCH, INT32 raisestate;
...
static void readAnimTex(MYFILE *f, INT32 num)
{
        ...
        do {
                if (myfgets(s, sizeof s, f) != NULL)
                {
                        ...
                        else if (fastcmp(word, "SPEED")) harddefs[num].speed =
i; else if (fastcmp(word, "ISTEXTURE")) harddefs[num].istexture = i;
    ...
    if (myfgets(s, MAXLINELEN, f)) {
      ...
      if (fastcmp(word, "PLAYERTEXT")) {
        ...
        if (playertext) {
          ...
          strcat(description[num].notes,
                 myhashfgets(playertext, sizeof(description[num].notes), f));
        } else
          strcpy(description[num].notes, "");

        // For some reason, cutting the string did not work above. Most likely
        // due to strcpy or strcat... It works down here, though.
        {
          INT32 numline = 0;
          for (i = 0; i < MAXLINELEN - 1; i++) {
            ...
            if (numline >= 20 || description[num].notes[i] == '\0' ||
                description[num].notes[i] == '#')
              break;
          }
        }
      ...
      /*if (fastcmp(word, "PLAYERNAME"))
      {
              ...
              strlcpy(description[num].text, word2, sizeof
      (description[num].text)); for (word2 = description[num].text; *word2;
      word2++) if (*word2 == '_') *word2 = ' '; PlayerMenu[num].text =
      description[num].text;
      }*/
      /* 			else if (fastcmp(word, "MENUPOSITION"))
                              { // Make sure you make MENUPOSITION the first
      thing under CHARACTER if you're using it!
                                      // This is to manually choose a slot and
      overwrite existing characters! It is NOT necessary for most individual
      character wads!! #ifdef DELFILE if (disableundo) #endif
                                      {
                                              slotfound = true;
                                              num = i;
                                      }
      ...
      /*else*/ if (fastcmp(word, "PICNAME")) {
        ...
        /*
                ...
                But try to enable something that's already enabled and you will
           be sent to a free slot.

                Because of this, you are allowed to edit any previous entrys you
           like, but only if you signal that you are purposely doing so by
           disabling and then reenabling the slot.

                ... Or use MENUPOSITION first, that works too. Hell, you could
           edit multiple character slots in a single section that way, due to
           how SOC editing works.
        */
        if (i != IT_DISABLED && !slotfound &&
            (slotfound = findFreeSlot(&num)) == false)
          goto done;
        ...
        strlcpy(description[num].skinname, word2,
                sizeof description[num].skinname);
...
static int freeslotusage[2][2] = {
    {0, 0}, {0, 0}}; // [S_, MT_][max, previous .wad's max]
    ...
    if (myfgets(s, MAXLINELEN, f)) {
      ...
      if (fastcmp(type, "SFX")) {
        ...
        for (i = SPR_FIRSTFREESLOT; i <= SPR_LASTFREESLOT; i++) {
          ...
          used_spr[(i - SPR_FIRSTFREESLOT) / 8] |=
              1 << (i % 8); // Okay, this sprite slot has been named now.
          break;
        }
        ...
        deh_warning("Freeslots: unknown enum class '%s' for '%s_%s'", type,
                    type, word);
    }
  } while (!myfeof(f)); // finish when the line is empty
  ...
  do {
    if (myfgets(s, MAXLINELEN, f)) {
      ...
      if (fastcmp(word, "MAPTHINGNUM") || fastcmp(word, "DOOMEDNUM")) {
        ...
        DEH_WriteUndoline(word, va("%d", mobjinfo[num].reactiontime),
                          UNDO_NONE);
        ...
        DEH_WriteUndoline(word, va("%d", mobjinfo[num].missilestate),
                          UNDO_NONE);
...
static void readlevelheader(MYFILE *f, INT32 num) {
  ...
  // This call automatically saves all previous information when DELFILE is
  // defined. We don't need to do it ourselves.
    ...
    if (myfgets(s, MAXLINELEN, f)) {
      ...
      if (fastcmp(word, "SUBTITLE")) {
        deh_strlcpy(mapheaderinfo[num - 1]->subttl, word2,
                    sizeof(mapheaderinfo[num - 1]->subttl),
                    va("Level header %d: subtitle", num));
        continue;
      } else if (fastcmp(word, "LEVELNAME")) {
        deh_strlcpy(mapheaderinfo[num - 1]->lvlttl, word2,
                    sizeof(mapheaderinfo[num - 1]->lvlttl),
                    va("Level header %d: levelname", num));
        continue;
      } else if (fastcmp(word, "ZONETITLE")) {
        deh_strlcpy(mapheaderinfo[num - 1]->zonttl, word2,
                    sizeof(mapheaderinfo[num - 1]->zonttl),
                    va("Level header %d: zonetitle", num));
        continue;
      }
      ...
      if (fastncmp(word, "LUA.", 4)) {
        ...
        // Note: we actualy strlwr word here, so things are made a little easier
        // for Lua
        strlwr(word);
        word += 4; // move past "lua."

        // ... and do a simple name sanity check; the name must start with a
        // letter
        if (*word < 'a' || *word > 'z') {
          deh_warning("Level header %d: invalid custom option name \"%s\"", num,
                      word);
          continue;
        }
        ...
        mapheaderinfo[num - 1]->customopts = Z_Realloc(
            mapheaderinfo[num - 1]->customopts,
            sizeof(customoption_t) * mapheaderinfo[num - 1]->numCustomOptions,
            PU_STATIC, NULL);
      ...
      // Strings that can be truncated
        ...
        deh_strlcpy(mapheaderinfo[num - 1]->scriptname, word2,
                    sizeof(mapheaderinfo[num - 1]->scriptname),
                    va("Level header %d: scriptname", num));
      } else if (fastcmp(word, "RUNSOC")) {
        deh_strlcpy(mapheaderinfo[num - 1]->runsoc, word2,
                    sizeof(mapheaderinfo[num - 1]->runsoc),
                    va("Level header %d: runsoc", num));
        ...
        deh_strlcpy(mapheaderinfo[num - 1]->actnum, word2,
                    sizeof(mapheaderinfo[num - 1]->actnum),
                    va("Level header %d: actnum", num));
      } else if (fastcmp(word, "NEXTLEVEL")) {
        if (fastcmp(word2, "TITLE"))
          i = 1100;
        else if (fastcmp(word2, "EVALUATION"))
          i = 1101;
        ...
        if (fastcmp(word2, "NONE"))
          ...
          deh_strlcpy(mapheaderinfo[num - 1]->musname, word2,
                      sizeof(mapheaderinfo[num - 1]->musname),
                      va("Level header %d: music", num));
        }
      }
#ifdef MUSICSLOT_COMPATIBILITY
        ...
        if (i && i <= 1035)
          snprintf(mapheaderinfo[num - 1]->musname, 7, "%sM",
                   G_BuildMapName(i));
        else if (i && i <= 1050)
          strncpy(mapheaderinfo[num - 1]->musname,
                  compat_special_music_slots[i - 1036], 7);
        else
          mapheaderinfo[num - 1]->musname[0] = 0; // becomes empty string
        ...
        deh_strlcpy(mapheaderinfo[num - 1]->musintername, word2,
                    sizeof(mapheaderinfo[num - 1]->musintername),
                    va("Level header %d: intermission music", num));
      else if (fastcmp(word, "FORCECHARACTER")) {
        strlcpy(mapheaderinfo[num - 1]->forcecharacter, word2,
                SKINNAMESIZE + 1);
        strlwr(
            mapheaderinfo[num - 1]->forcecharacter); // skin names are lowercase
        ...
        if (i >= 0 && i <= MAXUNLOCKABLES) // 0 for no unlock required, anything
        ...
        mapheaderinfo[num - 1]->skybox_scalex =
            mapheaderinfo[num - 1]->skybox_scaley =
        ...
        if (fastcmp(word2, "NONE"))
          i = -1;
        else if (fastcmp(word2, "NORMAL"))
          i = 0;
        else if (fastcmp(word2, "BOSS"))
          i = 1;
        else if (fastcmp(word2, "ERZ3"))
          i = 2;
        ...
        if (fastcmp(word2, "DEFAULT"))
          i = SAVE_DEFAULT;
        else if (fastcmp(word2, "ALWAYS"))
          i = SAVE_ALWAYS;
        else if (fastcmp(word2, "NEVER"))
          i = SAVE_NEVER;

        if (i >= SAVE_NEVER && i <= SAVE_ALWAYS)
          mapheaderinfo[num - 1]->saveoverride = (SINT8)i;
        else
          deh_warning("Level header %d: invalid save override number %d", num,
                      i);
      }
      ...
      // Individual triggers for level flags, for ease of use (and 2.0
      // compatibility)
      ...
      // Individual triggers for menu flags
        ...
        if (i || word2[0] == 'T' || word2[0] == 'Y')
          ...
          mapheaderinfo[num - 1]->menuflags &= ~LF2_HIDEINSTATS;
      } else if (fastcmp(word, "RECORDATTACK") ||
                 fastcmp(word,
                         "TIMEATTACK")) { // TIMEATTACK is an accepted alias
        if (i || word2[0] == 'T' || word2[0] == 'Y')
...
static void readcutscenescene(MYFILE *f, INT32 num, INT32 scenenum) {
  ...
  DEH_WriteUndoline("SCENETEXT", cutscenes[num]->scene[scenenum].text,
                    UNDO_ENDTEXT);
  ...
  do {
    if (myfgets(s, MAXLINELEN, f)) {
      ...
      if (fastcmp(word, "SCENETEXT")) {
        ...
        strcat(buffer,
               myhashfgets(scenetext, bufferlen - strlen(buffer) - 1, f));
      ...
      if (fastcmp(word, "NUMBEROFPICS")) {
        DEH_WriteUndoline(
            word, va("%d", cutscenes[num]->scene[scenenum].numpics), UNDO_NONE);
        ...
        if (fastcmp(word + 4, "NAME")) {
          DEH_WriteUndoline(
              word, cutscenes[num]->scene[scenenum].picname[picid], UNDO_NONE);
          strncpy(cutscenes[num]->scene[scenenum].picname[picid], word2, 8);
        } else if (fastcmp(word + 4, "HIRES")) {
          DEH_WriteUndoline(
              word, va("%d", cutscenes[num]->scene[scenenum].pichires[picid]),
              UNDO_NONE);
          cutscenes[num]->scene[scenenum].pichires[picid] =
              (UINT8)(i || word2[0] == 'T' || word2[0] == 'Y');
        } else if (fastcmp(word + 4, "DURATION")) {
          DEH_WriteUndoline(
              word,
              va("%u", cutscenes[num]->scene[scenenum].picduration[picid]),
              UNDO_NONE);
          cutscenes[num]->scene[scenenum].picduration[picid] = usi;
        } else if (fastcmp(word + 4, "XCOORD")) {
          DEH_WriteUndoline(
              word, va("%u", cutscenes[num]->scene[scenenum].xcoord[picid]),
              UNDO_NONE);
          cutscenes[num]->scene[scenenum].xcoord[picid] = usi;
        } else if (fastcmp(word + 4, "YCOORD")) {
          DEH_WriteUndoline(
              word, va("%u", cutscenes[num]->scene[scenenum].ycoord[picid]),
              UNDO_NONE);
          cutscenes[num]->scene[scenenum].ycoord[picid] = usi;
        } else
          deh_warning("CutSceneScene %d: unknown word '%s'", num, word);
      } else if (fastcmp(word, "MUSIC")) {
        DEH_WriteUndoline(word, cutscenes[num]->scene[scenenum].musswitch,
                          UNDO_NONE);
        ...
        i = get_mus(word2, true);
        if (i && i <= 1035)
          snprintf(cutscenes[num]->scene[scenenum].musswitch, 7, "%sM",
                   G_BuildMapName(i));
        else if (i && i <= 1050)
          strncpy(cutscenes[num]->scene[scenenum].musswitch,
                  compat_special_music_slots[i - 1036], 7);
        else
          cutscenes[num]->scene[scenenum].musswitch[0] =
        ...
        DEH_WriteUndoline(
            word, va("%u", cutscenes[num]->scene[scenenum].musswitchflags),
            UNDO_NONE);
        cutscenes[num]->scene[scenenum].musswitchflags =
            ((UINT16)i) & MUSIC_TRACKMASK;
      } else if (fastcmp(word, "MUSICPOS")) {
        DEH_WriteUndoline(
            word, va("%u", cutscenes[num]->scene[scenenum].musswitchposition),
            UNDO_NONE);
        cutscenes[num]->scene[scenenum].musswitchposition =
            (UINT32)get_number(word2);
      } else if (fastcmp(word, "MUSICLOOP")) {
        DEH_WriteUndoline(word,
                          va("%u", cutscenes[num]->scene[scenenum].musicloop),
                          UNDO_NONE);
        cutscenes[num]->scene[scenenum].musicloop =
            (UINT8)(i || word2[0] == 'T' || word2[0] == 'Y');
      } else if (fastcmp(word, "TEXTXPOS")) {
        DEH_WriteUndoline(word,
                          va("%u", cutscenes[num]->scene[scenenum].textxpos),
                          UNDO_NONE);
        cutscenes[num]->scene[scenenum].textxpos = usi;
      } else if (fastcmp(word, "TEXTYPOS")) {
        DEH_WriteUndoline(word,
                          va("%u", cutscenes[num]->scene[scenenum].textypos),
                          UNDO_NONE);
        cutscenes[num]->scene[scenenum].textypos = usi;
      } else if (fastcmp(word, "FADEINID")) {
        DEH_WriteUndoline(
            word, va("%u", cutscenes[num]->scene[scenenum].fadenum), UNDO_NONE);
...
static void readframe(MYFILE *f, INT32 num) {
  ...
  do {
    if (myfgets(s, MAXLINELEN, f)) {
      ...
      } else if (fastcmp(word1, "SPRITESUBNUMBER") ||
                 fastcmp(word1, "SPRITEFRAME")) {
        DEH_WriteUndoline(word1, va("%d", states[num].frame), UNDO_NONE);
        states[num].frame =
            (INT32)get_number(word2); // So the FF_ flags get calculated
      ...
      /*			if (fastcmp(word, "OFFSET"))
                              {
                                      ...
                                      if (value <= 64)
                                              ...
                                              value = (value+8)/8;
                                      if (value >= -1 && value < sfx_freeslot0 -
         1) S_sfx[num].name = savesfxnames[value+1]; else deh_warning("Sound %d:
         offset out of bounds", num);
                              }
                              else */
...
static boolean GoodDataFileName(const char *s) {
  ...
  if (!fasticmp(p, tail))
    return false; // doesn't end in .dat

  if (fasticmp(s, "gamedata.dat"))
    return false; // Vanilla SRB2 gamedata
  if (fasticmp(s, "main.dat"))
    return false; // Vanilla SRB2 time attack replay folder
  if (fasticmp(s, "kartdata.dat"))
    return false; // SRB2Kart gamedata
  if (fasticmp(s, "kart.dat"))
    return false; // SRB2Kart time attack replay folder
  if (fasticmp(s, "online.dat"))
    return false; // SRB2Kart online replay folder
...
static void reademblemdata(MYFILE *f, INT32 num) {
  ...
  DEH_WriteUndoline("MAPNUM", va("%d", emblemlocations[num - 1].level),
                    UNDO_NONE);
  DEH_WriteUndoline("VAR", va("%d", emblemlocations[num - 1].var), UNDO_NONE);
  DEH_WriteUndoline("SPRITE", va("%d", emblemlocations[num - 1].sprite),
                    UNDO_NONE);
  DEH_WriteUndoline("COLOR", va("%d", emblemlocations[num - 1].color),
                    UNDO_NONE);
    ...
    if (myfgets(s, MAXLINELEN, f)) {
      ...
      if (fastcmp(word, "HINT")) {
        while ((tmp = strchr(word2, '\\')))
          *tmp = '\n';
        deh_strlcpy(emblemlocations[num - 1].hint, word2,
                    sizeof(emblemlocations[num - 1].hint),
                    va("Emblem %d: hint", num));
        continue;
      }
      strupr(word2);
  ...
  if (!emblemlocations[num - 1].sprite)
    switch (emblemlocations[num - 1].type) {
    /*case ET_RINGS:
    ...
    case ET_TIME: // case ET_NTIME:
      emblemlocations[num - 1].sprite = 'B';
      break;
    default:
  ...
  if (!emblemlocations[num - 1].color)
    ...
    case ET_TIME: // case ET_NTIME:
      emblemlocations[num - 1].color = SKINCOLOR_GREY;
      break;
    default:
...
static void readextraemblemdata(MYFILE *f, INT32 num) {
  ...
  DEH_WriteUndoline("CONDITIONSET",
                    va("%d", extraemblems[num - 1].conditionset), UNDO_NONE);
  DEH_WriteUndoline("SPRITE", va("%d", extraemblems[num - 1].sprite),
                    UNDO_NONE);
    ...
    if (myfgets(s, MAXLINELEN, f)) {
      ...
      if (fastcmp(word, "NAME"))
        deh_strlcpy(extraemblems[num - 1].name, word2,
                    sizeof(extraemblems[num - 1].name),
                    va("Extra emblem %d: name", num));
      else if (fastcmp(word, "OBJECTIVE"))
        deh_strlcpy(extraemblems[num - 1].description, word2,
                    sizeof(extraemblems[num - 1].description),
                    va("Extra emblem %d: objective", num));
...
static void readunlockable(MYFILE *f, INT32 num) {
  ...
  DEH_WriteUndoline("CONDITIONSET", va("%d", unlockables[num].conditionset),
                    UNDO_NONE);
  ...
  DEH_WriteUndoline("NOCHECKLIST", va("%d", unlockables[num].nochecklist),
                    UNDO_NONE);
  ...
  do {
    if (myfgets(s, MAXLINELEN, f)) {
      ...
      if (fastcmp(word, "NAME"))
        deh_strlcpy(unlockables[num].name, word2, sizeof(unlockables[num].name),
                    va("Unlockable %d: name", num));
      else if (fastcmp(word, "OBJECTIVE"))
        deh_strlcpy(unlockables[num].objective, word2,
                    sizeof(unlockables[num].objective),
                    va("Unlockable %d: objective", num));
        ...
        unlockables[num].nocecho =
            (UINT8)(i || word2[0] == 'T' || word2[0] == 'Y');
      else if (fastcmp(word, "NOCHECKLIST"))
        unlockables[num].nochecklist =
            (UINT8)(i || word2[0] == 'T' || word2[0] == 'Y');
...
#define PARAMCHECK(n)                                                          \
  do {                                                                         \
    if (!params[n]) {                                                          \
      deh_warning("Too few parameters, need %d", n);                           \
      return;                                                                  \
    }                                                                          \
  } while (0)
static void readcondition(UINT8 set, UINT32 id, char *word2) {
  ...
  ||        (++offset && fastcmp(params[0], "NIGHTSGRADE")))
  {
          ...
          if (x1 < 0 || x1 >= NUMMAPS)
          {
                  deh_warning("Level number %d out of range (1 - %d)", re,
  NUMMAPS); return;
          }

          // Mare number (0 for overall)
          if (params[3]) // Only if we actually got 3 params (so the second one
  == mare and not requirement) x2 = (INT16)atoi(params[2]); else x2 = 0;
    ...
    if (re <= 0 || re > MAXCONDITIONSETS) {
      deh_warning("Condition set %d out of range (1 - %d)", re,
                  MAXCONDITIONSETS);
      return;
    }
  } else {
    ...
    if (myfgets(s, MAXLINELEN, f)) {
      ...
      if (fastncmp(word, "CONDITION", 9)) {
        ...
        if (id == 0) {
          ...
          deh_warning("Condition set %d: conditions are out of order, ignoring "
                      "this line",
                      setnum);
          continue;
        }
...
static void readwipes(MYFILE *f) {
  ...
  do {
    if (myfgets(s, MAXLINELEN, f)) {
      ...
      if (value == UINT8_MAX && (wipeoffset <= wipe_level_toblack ||
                                 wipeoffset >= wipe_speclevel_towhite)) {
...
static void DEH_LoadDehackedFile(MYFILE *f, UINT16 wad) {
  ...
  */
    ...
    if (word) {
      ...
      if (word2) {
        ...
        if (fastcmp(word, "PATCH")) {
          ...
          DEH_WriteUndoline(word, word2, UNDO_HEADER);
          // This is not a major mod.
        } else if (fastcmp(word, "THING") || fastcmp(word, "MOBJ") ||
                   fastcmp(word, "OBJECT")) {
          ...
          if (i < NUMMOBJTYPES && i >= 0) {
            if (i < (MT_FIRSTFREESLOT + freeslotusage[1][1]))
              G_SetGameModified(
                  multiplayer,
                  true); // affecting something earlier than the first freeslot
                         // allocated in this .wad? DENIED
            readthing(f, i);
          } else {
        ...
        /*				else if (fastcmp(word, "ANIMTEX"))
          ...
          if (i > 0 && i <= NUMMAPS) {
            if (mapheaderinfo[i - 1])
              G_SetGameModified(
                  multiplayer, true); // only mark as a major mod if it replaces
                                      // an already-existing mapheaderinfo
            readlevelheader(f, i);
          } else {
          ...
          DEH_WriteUndoline(word, word2, UNDO_HEADER);
          // G_SetGameModified(multiplayer, true); -- might have to reconsider
          // in a future update
        ...
        /*				else if (fastcmp(word, "POINTER"))
                                        {
                                                word = strtok(NULL, " "); // get
           frame word = strtok(NULL, ")"); if (word)
                                                {
                                                        i = atoi(word);
                                                        if (i < NUMSTATES && i
           >= 0)
                                                        {
                                                                if (myfgets(s,
           MAXLINELEN, f)) states[i].action = saveactions[searchvalue(s)];
                                                        }
                                                        else
                                                        {
                                                                deh_warning("Pointer:
           Frame %d doesn't exist", i); ignorelines(f);
                                                        }
                                                }
                                                else
                                                        deh_warning("pointer
           (Frame %d) : missing ')'", i); G_SetGameModified(multiplayer, true);
                                        }*/
        ...
        /*				else if (fastcmp(word, "SPRITE"))
           ...
           MAXLINELEN, f))
                                                        {
                                                                INT32 k;
                                                                k =
           (searchvalue(s) - 151328)/8; if (k >= 0 && k < NUMSPRITES)
                                                                        sprnames[i]
           = savesprnames[k]; else
                                                                {
                                                                        deh_warning("Sprite
           %d: offset out of bounds", i); ignorelines(f);
                                                                }
                                                        }
                                                }
                                                else
                                                        deh_warning("Sprite %d
           doesn't exist",i);
                                                // This is not a major mod.
                                        }*/
          ...
          if (i >= 0 && i < NUMHUDITEMS)
            ...
            deh_warning("HUD item number %d out of range (0 - %d)", i,
                        NUMHUDITEMS - 1);
            ignorelines(f);
          }
          DEH_WriteUndoline(word, word2, UNDO_HEADER);
          ...
          if (!(refreshdirmenu & REFRESHDIR_GAMEDATA)) {
            deh_warning("You must define a custom gamedata to use \"%s\"",
                        word);
            ignorelines(f);
            ...
            deh_warning("Emblem number %d out of range (1 - %d)", i,
                        MAXEMBLEMS);
            ignorelines(f);
          }
          DEH_WriteUndoline(word, word2, UNDO_HEADER);
          ...
          if (!(refreshdirmenu & REFRESHDIR_GAMEDATA)) {
            ...
            deh_warning("Extra emblem number %d out of range (1 - %d)", i,
                        MAXEXTRAEMBLEMS);
            ignorelines(f);
          }
          DEH_WriteUndoline(word, word2, UNDO_HEADER);
          ...
          if (!(refreshdirmenu & REFRESHDIR_GAMEDATA)) {
            ...
            deh_warning("Unlockable number %d out of range (1 - %d)", i,
                        MAXUNLOCKABLES);
            ignorelines(f);
          }
          DEH_WriteUndoline(word, word2, UNDO_HEADER);
          ...
          if (!(refreshdirmenu & REFRESHDIR_GAMEDATA)) {
            ...
            deh_warning("Condition set number %d out of range (1 - %d)", i,
                        MAXCONDITIONSETS);
            ignorelines(f);
          }
  ...
  if (deh_num_warning) {
    ...
    if (devparm) {
      I_Error("%s%s",
              va(M_GetText("%d warning%s in the SOC lump\n"), deh_num_warning,
                 deh_num_warning == 1 ? "" : "s"),
              M_GetText("See log.txt for details.\n"));
      // while (!I_GetKey())
      // I_OsPolling();
    }
  }
...
void DEH_LoadDehackedLumpPwad(UINT16 wad, UINT16 lump) {
  ...
  DEH_WriteUndoline(va("# uload for wad: %u, lump: %u", wad, lump), NULL,
                    UNDO_DONE);
  Z_Free(f.data);
}
...
// because sadly no one remembers this place while searching for full state
// names.
static const char *const STATE_LIST[] = {
    // array length left dynamic for sanity testing later.
    "S_NULL",
    "S_UNKNOWN",
    ...
    "S_PLAY_SUPERTRANS7",
    "S_PLAY_SUPERTRANS8",
    "S_PLAY_SUPERTRANS9", // This has special significance in the code. If you
    add more frames, search for it and make the appropriate changes.
    */
    ...
    "S_CYBRAKDEMON_FINISH_ATTACK",  // If just attacked, remove MF2_FRET w/out
                                    // going back to spawnstate
    "S_CYBRAKDEMON_FINISH_ATTACK2", // Force a delay between attacks so you
                                    // don't
    ...
    "S_KLIT1", // Diagonal lightning. No, it not being straight doesn't make it
               // gay.
    "S_KLIT2",
    "S_KLIT3",
    "S_KLIT4",
    ...
    "S_ROAMINGSHADOW", // Roaming Shadow (the one that uses above's wind attack
                       // or
                       // smth)
...
// because sadly no one remembers this place while searching for full state
// names.
static const char *const MOBJTYPE_LIST[] = {
    // array length left dynamic for sanity testing later.
    "MT_NULL",
    "MT_UNKNOWN",
...
static const char *const MOBJFLAG_LIST[] = {
    "SPECIAL",       "SOLID",          "SHOOTABLE",     "NOSECTOR",
    "NOBLOCKMAP",    "PAPERCOLLISION", "PUSHABLE",      "BOSS",
    "SPAWNCEILING",  "NOGRAVITY",      "AMBIENT",       "SLIDEME",
    "NOCLIP",        "FLOAT",          "BOXICON",       "MISSILE",
    "SPRING",        "BOUNCE",         "MONITOR",       "NOTHINK",
    "FIRE",          "NOCLIPHEIGHT",   "ENEMY",         "SCENERY",
    "PAIN",          "STICKY",         "NIGHTSITEM",    "NOCLIPTHING",
    "GRENADEBOUNCE", "RUNSPAWNFUNC",   "DONTENCOREMAP", NULL};
...
static const char *const MOBJFLAG2_LIST[] = {
    ...
    "BEYONDTHEGRAVE", // Source of this missile has died and has since
                      // respawned.
    ...
    "INFLOAT", // Floating to a height for a move, don't auto float to target's
               // height.
    ...
    "FIRING",       // turret fire
    "SUPERFIRE", // Firing something with Super Sonic-stopping properties. Or,
                 // if mobj has MF_MISSILE, this is the actual fire from it.
    ...
    "BOSSDEAD",   // Boss is dead! (Not necessarily fleeing, if a fleeing point
                  // doesn't exist.)
    "AMBUSH",     // Alternate behaviour typically set by MTF_AMBUSH
    NULL};

static const char *const MOBJEFLAG_LIST[] = {
    "ONGROUND",     // The mobj stands on solid floor (not on another mobj or in
                    // air)
    "JUSTHITFLOOR", // The mobj just hit the floor while falling, this is
                    // cleared on next frame
    "TOUCHWATER", // The mobj stands in a sector with water, and touches the
                  // surface
    "UNDERWATER", // The mobj stands in a sector with water, and his waist is
    ...
    "DRAWONLYFORP2", "DRAWONLYFORP3", "DRAWONLYFORP4", NULL};
...
static const char *const PLAYERFLAG_LIST[] = {
    ...
    // Cheats
    "GODMODE", "NOCLIP", "INVIS",

    // True if button down last tic.
    "ATTACKDOWN", "USEDOWN", "JUMPDOWN", "WPNDOWN",

    // Unmoving states
    "STASIS",     // Player is not allowed to move
    "JUMPSTASIS", // and that includes jumping.
    // (we don't include FULLSTASIS here I guess because it's just those two
    // together...?)
    ...
    "JUMPED", "SPINNING", "STARTDASH", "THOKKED",
    ...
    "NIGHTSMODE", "TRANSFERTOCLOSEST",

    // Spill rings after falling
    "NIGHTSFALL", "DRILLING", "SKIDDOWN",

    /*** TAG STUFF ***/
    "TAGGED", // Player has been tagged and awaits the next round in hide and
              // seek.
...
static const char *const ML_LIST[16] = {
    "IMPASSIBLE",    "BLOCKMONSTERS", "TWOSIDED", "DONTPEGTOP",
    "DONTPEGBOTTOM", "EFFECT1",       "NOCLIMB",  "EFFECT2",
    "EFFECT3",       "EFFECT4",       "EFFECT5",  "NOSONIC",
    "NOTAILS",       "NOKNUX",        "BOUNCY",   "TFERLINE"};
...
static const char *const POWERS_LIST[] = {
    "INVULNERABILITY", "SNEAKERS", "FLASHING", "SHIELD",
    ...
    // Weapon ammunition
    "INFINITYRING", "AUTOMATICRING", "BOUNCERING", "SCATTERRING", "GRENADERING",
    "EXPLOSIONRING", "RAILRING",

    // Power Stones
    "EMERALDS", // stored like global 'emeralds' variable

    // NiGHTS powerups
    "NIGHTS_SUPERLOOP", "NIGHTS_HELPER", "NIGHTS_LINKFREEZE",
...
static const char *const KARTSTUFF_LIST[] = {"POSITION",
                                             "OLDPOSITION",
                                             "POSITIONDELAY",
                                             "PREVCHECK",
                                             "NEXTCHECK",
...
static const char *const HUDITEMS_LIST[] = {
    ...
    "RINGS",         "RINGSSPLIT",
    "RINGSNUM",      "RINGSNUMSPLIT",

    "SCORE",         "SCORENUM",

    "TIME",          "TIMESPLIT",
    "MINUTES",       "MINUTESSPLIT",
    "TIMECOLON",     "TIMECOLONSPLIT",
    "SECONDS",       "SECONDSSPLIT",
    "TIMETICCOLON",  "TICS",

    "SS_TOTALRINGS", "SS_TOTALRINGS_SPLIT",

    "GETRINGS",      "GETRINGSNUM",
...
struct {
  ...
  lua_Integer v;
    ...
    // INT64 and UINT64 limits not included, they're too big for most purposes
    // anyway
    // signed
    {"INT8_MIN", INT8_MIN},
    {"INT16_MIN", INT16_MIN},
    ...
    {"RING_DIST", RING_DIST},
    {"PUSHACCEL", PUSHACCEL},
    {"MODID", MODID}, // I don't know, I just thought it would be cool for a wad
                      // to potentially know what mod it was loaded into.
    ...
    {"LE_PINCHPHASE",
     LE_PINCHPHASE}, // A boss entered pinch phase (and, in most cases, is
                     // preparing their pinch phase attack!)
    {"LE_ALLBOSSESDEAD",
     LE_ALLBOSSESDEAD}, // All bosses in the map are dead (Egg capsule raise)
    {"LE_BOSSDEAD",
     LE_BOSSDEAD}, // A boss in the map died (Chaos mode boss tally)
    {"LE_BOSS4DROP", LE_BOSS4DROP}, // CEZ boss dropped its cage
    {"LE_BRAKVILEATACK",
     LE_BRAKVILEATACK}, // Brak's doing his LOS attack, oh noes
    ...
    {"SH_FIREFLOWER", SH_FIREFLOWER}, // Lower bits are a normal shield stacked
                                      // on top of the fire flower
    {"SH_FORCE",
     SH_FORCE}, // Lower bits are how many hits left, 0 is the last hit
    // Stack masks
    {"SH_STACK", SH_STACK},
    ...
    // Used to be MF_ for some stupid reason, now they're GF_ to stop them
    // looking
    // like mobjflags
    ...
    {"FF_INVERTPLANES",
     FF_INVERTPLANES},                  ///< Reverse the plane visibility rules.
    {"FF_ALLSIDES", FF_ALLSIDES},       ///< Render inside and outside sides.
    {"FF_INVERTSIDES", FF_INVERTSIDES}, ///< Only render inside sides.
    {"FF_DOUBLESHADOW",
     FF_DOUBLESHADOW}, ///< Make two lightlist entries to reset light?
    {"FF_FLOATBOB",
     FF_FLOATBOB}, ///< Floats on water and bobs if you step on it.
    {"FF_NORETURN", FF_NORETURN}, ///< Used with ::FF_CRUMBLE. Will not return
                                  ///< to its original position after falling.
    {"FF_CRUMBLE",
     FF_CRUMBLE}, ///< Falls 2 seconds after being stepped on, and randomly
                  ///< brings all touching crumbling 3dfloors down with it,
                  ///< providing their master sectors share the same tag (allows
                  ///< crumble platforms above or below, to also exist).
    {"FF_SHATTERBOTTOM",
     FF_SHATTERBOTTOM}, ///< Used with ::FF_BUSTUP. Like FF_SHATTER, but only
                        ///< breaks from the bottom. Good for springing up
                        ///< through rubble.
    {"FF_MARIO",
     FF_MARIO}, ///< Acts like a question block when hit from underneath. Goodie
                ///< spawned at top is determined by master sector.
    {"FF_BUSTUP",
     FF_BUSTUP}, ///< You can spin through/punch this block and it will crumble!
    {"FF_QUICKSAND", FF_QUICKSAND}, ///< Quicksand!
    {"FF_PLATFORM", FF_PLATFORM}, ///< You can jump up through this to the top.
    {"FF_REVERSEPLATFORM",
     FF_REVERSEPLATFORM}, ///< A fall-through floor in normal gravity, a
                          ///< platform in reverse gravity.
    {"FF_INTANGABLEFLATS",
     FF_INTANGABLEFLATS}, ///< Both flats are intangable, but the sides are
                          ///< still solid.
    {"FF_SHATTER",
     FF_SHATTER}, ///< Used with ::FF_BUSTUP. Thinks everyone's Knuckles.
    {"FF_SPINBUST", FF_SPINBUST}, ///< Used with ::FF_BUSTUP. Jump or fall onto
                                  ///< it while curled in a ball.
    {"FF_ONLYKNUX", FF_ONLYKNUX}, ///< Used with ::FF_BUSTUP. Only Knuckles can
                                  ///< break this rock.
    {"FF_RIPPLE", FF_RIPPLE},     ///< Ripple the flats
    {"FF_COLORMAPONLY",
     FF_COLORMAPONLY}, ///< Only copy the colormap, not the lightlevel
    {"FF_GOOWATER",
     FF_GOOWATER}, ///< Used with ::FF_SWIMMABLE. Makes thick bouncey goop.

    // Slope flags
    {"SL_NOPHYSICS",
     SL_NOPHYSICS}, // Don't do momentum adjustment with this slope
    {"SL_NODYNAMIC",
     SL_NODYNAMIC}, // Slope will never need to move during the level, so don't
                    // fuss with recalculating it
    {"SL_ANCHORVERTEX", SL_ANCHORVERTEX}, // Slope is using a Slope Vertex Thing
                                          // to anchor its position
    {"SL_VERTEXSLOPE",
     SL_VERTEXSLOPE}, // Slope is built from three Slope Vertex Things
    ...
    {"KRITEM_TRIPLESNEAKER",
     KRITEM_TRIPLESNEAKER}, // Additional roulette IDs (not usable for much in
                            // Lua besides K_GetItemPatch)
...
static mobjtype_t
get_mobjtype(const char *word) { // Returns the vlaue of MT_ enumerations
  mobjtype_t i;
  if (*word >= '0' && *word <= '9')
    return atoi(word);
...
static statenum_t
get_state(const char *word) { // Returns the value of S_ enumerations
  statenum_t i;
  if (*word >= '0' && *word <= '9')
    return atoi(word);
...
static sfxenum_t
get_sfx(const char *word) { // Returns the value of SFX_ enumerations
  sfxenum_t i;
  if (*word >= '0' && *word <= '9')
    return atoi(word);
...
#ifdef MUSICSLOT_COMPATIBILITY
static UINT16
get_mus(const char *word,
        UINT8 dehacked_mode) { // Returns the value of MUS_ enumerations
  UINT16 i;
  char lumptmp[4];
...
static hudnum_t
get_huditem(const char *word) { // Returns the value of HUD_ enumerations
  hudnum_t i;
  if (*word >= '0' && *word <= '9')
    return atoi(word);
...
// Loops through every constant and operation in word and performs its
// calculations, returning the final value.
fixed_t get_number(const char *word) { return LUA_EvalMath(word); }
...
void DEH_Check(void) {
  ...
  if (dehstates != S_FIRSTFREESLOT)
    I_Error("You forgot to update the Dehacked states list, you dolt!\n(%d "
            "states defined, versus %s in the Dehacked list)\n",
  ...
  if (dehmobjs != MT_FIRSTFREESLOT)
    I_Error("You forgot to update the Dehacked mobjtype list, you dolt!\n(%d "
            "mobj types defined, versus %s in the Dehacked list)\n",
            MT_FIRSTFREESLOT, sizeu1(dehmobjs));

  if (dehpowers != NUMPOWERS)
    I_Error("You forgot to update the Dehacked powers list, you dolt!\n(%d "
            "powers defined, versus %s in the Dehacked list)\n",
            NUMPOWERS, sizeu1(dehpowers));

  if (dehkartstuff != NUMKARTSTUFF)
    I_Error("You forgot to update the Dehacked powers list, you dolt!\n(%d "
            "kartstuff defined, versus %s in the Dehacked list)\n",
            NUMKARTSTUFF, sizeu1(dehkartstuff));

  if (dehcolors != MAXTRANSLATIONS)
    I_Error("You forgot to update the Dehacked colors list, you dolt!\n(%d "
            "colors defined, versus %s in the Dehacked list)\n",
...
// TODO: Error checking! @.@; There's currently no way to know which ones failed
// and why!
//
    ...
    if (fastcmp(type, "SFX")) {
      ...
      if (sfx != sfx_None) {
        ...
        I_Error("Out of Sfx Freeslots while allocating \"%s\"\nLoad less "
                "addons to fix this.",
                word); // Should never get here since S_AddSoundFx was changed
                       // to throw I_Error when it can't allocate
    } else if (fastcmp(type, "SPR")) {
      char wad;
      spritenum_t j;
      ...
      for (j = SPR_FIRSTFREESLOT; j <= SPR_LASTFREESLOT; j++) {
        ...
        // sprnames[j][4] = 0;
        used_spr[(j - SPR_FIRSTFREESLOT) / 8] |=
            1 << (j % 8); // Okay, this sprite slot has been named now.
        lua_pushinteger(L, j);
        r++;
        break;
      }
...
// Hardcoded A_Action name to call for super() or NULL if super() would be
// invalid. Set in lua_infolib.
const char *superactions[MAXRECURSION];
UINT8 superstack = 0;

static int lib_dummysuper(lua_State *L) {
  return luaL_error(L,
                    "Can't call super() outside of hardcode-replacing A_Action "
                    "functions being "
                    "called by state changes!"); // convoluted, I know. @_@;;
}
...
static inline int lib_getenum(lua_State *L) {
  ...
  if (strlen(word) == 1) { // Assume sprite frame if length 1.
    ...
    if (mathlib)
      return luaL_error(L, "mobjflag '%s' could not be found.\n", word);
    return 0;
    ...
    if (mathlib)
      return luaL_error(L, "mobjflag2 '%s' could not be found.\n", word);
    return 0;
    ...
    if (mathlib)
      return luaL_error(L, "mapthingflag '%s' could not be found.\n", word);
    return 0;
  ...
  if (fastcmp(word, "gamemap")) {
    ...
    if ((!multiplayer || !(netgame || demo.playback)) &&
        !playeringame[serverplayer])
      return 0;
    LUA_PushUserdata(L, &players[serverplayer], META_PLAYER);
    return 1;
  } else if (fastcmp(word,
                     "consoleplayer")) { // Player controlling the console,
                                         // basically our local player
    ...
    lua_pushinteger(
        L, exitcountdown); // This name is pretty dumb. Hence why we'll prefer
                           // more descriptive names at least in Lua...
    return 1;
