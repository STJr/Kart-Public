...
#if NUMSCREENS < 5
#define NOWIPE // do not enable wipe image post processing for ARM, SH and MIPS
               // CPUs
#endif
...
static void F_DoWipe(fademask_t *fademask) {
  ...
  // from and to, so it uses a little extra memory, but again, helps it run
  // faster.
  {
    ...
    do {
      ...
      if (*mask == 0) {
        ...
        while (draw_linestogo--) {
          M_Memcpy(w_base + relativepos, s_base + relativepos,
                   draw_rowend - draw_rowstart);
          relativepos += vid.width;
        }
      } else if (*mask == 10) {
        // shortcut - memcpy target to work
        while (draw_linestogo--) {
          M_Memcpy(w_base + relativepos, e_base + relativepos,
                   draw_rowend - draw_rowstart);
          relativepos += vid.width;
        }
      } else {
...
void F_RunWipe(UINT8 wipetype, boolean drawMenu) {
  ...
  for (;;) {
    ...
    if (rendermode == render_opengl)
      HWR_DoWipe(wipetype,
                 wipeframe - 1); // send in the wipe type and wipeframe because
                                 // we need to cache the graphic
    else if (rendermode != render_none) // this allows F_RunWipe to be called in
                                        // dedicated servers
      F_DoWipe(fmask);
