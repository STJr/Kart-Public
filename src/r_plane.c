...
// the last visplane list is outside of the hash table and is used for fof
// planes
...
static void R_DrawTranslucentWaterSpan_8(void) {
  ...
  // address in the texture with two shifts, an OR and one AND. (see below) for
  // texture sizes > 64 the amount of precision we can allow will decrease, but
  // only by one bit per power of two (obviously) Ok, because I was able to
  // eliminate the variable spot below, this function is now FASTER than the
  // original span renderer. Whodathunkit?
  xposition = ds_xfrac << nflatshiftup;
  yposition = (ds_yfrac + waterofs) << nflatshiftup;
  ...
  while (count >= 8) {
    ...
    // because we don't have the uber complicated math to calculate it now, so
    // that was a memory write we didn't need!
    dest[0] = colormap[*(ds_transmap +
                         (source[((yposition >> nflatyshift) & nflatmask) |
                                 (xposition >> nflatxshift)]
                          << 8) +
                         *dsrc++)];
    xposition += xstep;
    yposition += ystep;

    dest[1] = colormap[*(ds_transmap +
                         (source[((yposition >> nflatyshift) & nflatmask) |
                                 (xposition >> nflatxshift)]
                          << 8) +
                         *dsrc++)];
...
void R_MapPlane(INT32 y, INT32 x1, INT32 x2) {
  ...
  if (itswater) {
    ...
    angle =
        (currentplane->viewangle + currentplane->plangle + xtoviewangle[x1]) >>
        ANGLETOFINESHIFT;
  ...
  if (currentplane->extra_colormap)
    ds_colormap =
        currentplane->extra_colormap->colormap + (ds_colormap - colormaps);
...
static visplane_t *new_visplane(unsigned hash) {
  ...
  if (!check) {
    ...
    if (check == NULL)
      I_Error("%s: Out of memory", "new_visplane"); // FIXME: ugly
  } else {
...
visplane_t *R_FindPlane(fixed_t height, INT32 picnum, INT32 lightlevel,
                        ...
                        extracolormap_t *planecolormap, ffloor_t *pfloor,
  ...
  if (slope)
    ;
  else // Don't mess with this right now if a slope is involved
  {
    xoff += viewx;
    yoff -= viewy;
    if (plangle != 0) {
      ...
      yoff =
          -FixedMul(oldxoff, sinecomponent) + FixedMul(yoff, cosinecomponent);
    }
  }
  ...
  if (polyobj) {
    if (polyobj->angle != 0) {
      ...
      xoff -= FixedMul(FINECOSINE(fineshift), polyobj->centerPt.x) +
              FixedMul(FINESINE(fineshift), polyobj->centerPt.y);
      yoff -= FixedMul(FINESINE(fineshift), polyobj->centerPt.x) -
              FixedMul(FINECOSINE(fineshift), polyobj->centerPt.y);
    } else {
...
void R_ExpandPlane(visplane_t *pl, INT32 start, INT32 stop) {
  ...
  /*
          ...
          if (x <= stop)
                  I_Error("R_ExpandPlane: planes in same subsector
...
static void R_DrawSkyPlane(visplane_t *pl) {
  ...
  for (x = pl->minx; x <= pl->maxx; x++) {
    ...
    if (dc_yl <= dc_yh) {
      ...
      dc_iscale =
          FixedMul(skyscale, FINECOSINE(xtoviewangle[x] >> ANGLETOFINESHIFT));
      dc_x = x;
...
void R_DrawSinglePlane(visplane_t *pl) {
  ...
  if (pl->polyobj && pl->polyobj->translucency != 0) {
    ...
    // (sidenote: ported to polys 10-15-2014, there was no time travel involved
    // -Red)
    if (pl->polyobj->translucency >= 10)
      return; // Don't even draw it
    else if (pl->polyobj->translucency > 0)
      ds_transmap =
          transtables + ((pl->polyobj->translucency - 1) << FF_TRANSSHIFT);
    else // Opaque, but allow transparent flat pixels
      spanfunc = splatfunc;

#ifdef SHITPLANESPARENCY
    if (spanfunc == splatfunc ||
        (pl->extra_colormap && pl->extra_colormap->fog))
#else
    ...
    for (rover = pl->ffloor->target->ffloors; rover; rover = rover->next) {
      if ((pl->ffloor->flags & FF_CUTEXTRA) && (rover->flags & FF_EXTRA)) {
        if (pl->ffloor->flags & FF_EXTRA) {
          ...
          if (((pl->ffloor->flags & (FF_FOG | FF_SWIMMABLE)) ==
               (rover->flags & (FF_FOG | FF_SWIMMABLE))) &&
    ...
    if (pl->ffloor->flags & FF_RIPPLE && !pl->slope) {
      ...
      if (spanfunc == R_DrawTranslucentSpan_8) {
        ...
        if (splitscreen > 2 &&
            viewplayer == &players[displayplayers[3]]) // Only copy the part of
                                                       // the screen we need
          scr = (screens[0] + (top + (viewheight)) * vid.width + viewwidth);
        else if ((splitscreen == 1 &&
                  viewplayer == &players[displayplayers[1]]) ||
        ...
        VID_BlitLinearScreen(scr, screens[1] + ((top)*vid.width), vid.width,
                             bottom - top, vid.width, vid.width);
      }
    }
#endif
  } else
  ...
  if (!pl->slope) // Don't mess with angle on slopes! We'll handle this
                  // ourselves later
  ...
  ds_source = (UINT8 *)W_CacheLumpNum(levelflats[pl->picnum].lumpnum,
  ...
  if (pl->slope) {
    // Potentially override other stuff for now cus we're mean. :< But draw a
    // slope plane! I copied ZDoom's code and adapted it to SRB2... -fickle
    floatv3_t p, m, n;
    float ang;
    float vx, vy, vz;
    ...
    fixed_t temp;
    // Okay, look, don't ask me why this works, but without this setup there's a
    // disgusting-looking misalignment with the textures. -fickle
    ...
    if (hack) {
      /*
      Essentially: We can't & the components along the regular axes when the
      plane is rotated. This is because the distance on each regular axis in
      order to loop is different. We rotate them, & the components, add them
      together, & them again, and then rotate them back. These three seperate &
      operations are done per axis in order to prevent overflows. toast 10/04/17
      ---
      ...of coooourse, this still isn't perfect. but it looks... merely kind of
      grody, rather than completely wrong? idk. i'm just backporting this to
      kart right now. if anyone else wants to ever try dig around: it's drifting
      towards 0,0, and no, multiplying by fudge doesn't fix it. toast 27/09/18
      */
      ...
      fixed_t ox = (FixedMul(pl->slope->o.x, cosinecomponent) & modmask) -
                   (FixedMul(pl->slope->o.y, sinecomponent) & modmask);
      fixed_t oy = (-FixedMul(pl->slope->o.x, sinecomponent) & modmask) -
                   (FixedMul(pl->slope->o.y, cosinecomponent) & modmask);

      temp = ox & modmask;
      oy &= modmask;
      ox = FixedMul(temp, cosinecomponent) +
           FixedMul(oy, -sinecomponent); // negative sine for opposite direction
      oy = -FixedMul(temp, -sinecomponent) + FixedMul(oy, cosinecomponent);

      temp = xoffs;
      xoffs = (FixedMul(temp, cosinecomponent) & modmask) +
              (FixedMul(yoffs, sinecomponent) & modmask);
      yoffs = (-FixedMul(temp, sinecomponent) & modmask) +
              (FixedMul(yoffs, cosinecomponent) & modmask);

      temp = xoffs & modmask;
      yoffs &= modmask;
      xoffs = FixedMul(temp, cosinecomponent) +
              FixedMul(yoffs, -sinecomponent); // ditto
      yoffs =
          -FixedMul(temp, -sinecomponent) + FixedMul(yoffs, cosinecomponent);
      ...
      xoffs -= (pl->slope->o.x + (1 << (31 - nflatshiftup))) &
               ~((1 << (32 - nflatshiftup)) - 1);
      yoffs += (pl->slope->o.y + (1 << (31 - nflatshiftup))) &
               ~((1 << (32 - nflatshiftup)) - 1);
    }
...
#define ANG2RAD(angle) ((float)((angle)*M_PIl) / ANGLE_180)
    ...
    temp = P_GetZAt(pl->slope, pl->viewx + FLOAT_TO_FIXED(sin(ang)),
                    pl->viewy + FLOAT_TO_FIXED(cos(ang)));
    m.y = FIXED_TO_FLOAT(temp) - zeroheight;
    temp = P_GetZAt(pl->slope, pl->viewx + FLOAT_TO_FIXED(cos(ang)),
                    pl->viewy - FLOAT_TO_FIXED(sin(ang)));
    ...
    // Eh. I tried making this stuff fixed-point and it exploded on me. Here's a
    // macro for the only floating-point vector function I recall using.
