...
static boolean con_forcepic =
    true;           // at startup toggle console translucency when first off
boolean con_recalc; // set true when screen size has changed

static tic_t
    con_tick; // console ticker for anim or blinking prompt cursor
              // con_scrollup should use time (currenttime - lasttime)..

static boolean
    consoletoggle;           // true when console key pushed, ticker will handle
...
static INT32
    con_hudtime[MAXHUDLINES]; // remaining time of display for hud msg lines
...
static size_t input_sel; // position of selection marker (I.E.: anything between
                         // this and input_cur is "selected")
static size_t
    input_len; // length of current line, used to bound cursor and such
...
static consvar_t cons_msgtimeout = {
    "con_hudtime", "5", CV_SAVE, CV_Unsigned, NULL, 0, NULL, NULL, 0, 0, NULL};

// number of lines displayed on the HUD
static consvar_t cons_hudlines = {"con_hudlines",
                                  "5",
                                  CV_CALL | CV_SAVE,
                                  CV_Unsigned,
...
static consvar_t cons_speed = {
    "con_speed", "8", CV_SAVE, speed_cons_t, NULL, 0, NULL, NULL, 0, 0, NULL};

// percentage of screen height to use for console
static consvar_t cons_height = {
    "con_height", "50", CV_SAVE, CV_Unsigned, NULL, 0, NULL, NULL, 0, 0, NULL};

static CV_PossibleValue_t backpic_cons_t[] = {
    {0, "translucent"}, {1, "picture"}, {0, NULL}};
// whether to use console background picture, or translucent mode
static consvar_t cons_backpic = {"con_backpic",
                                 "translucent",
                                 CV_SAVE,
                                 backpic_cons_t,
                                 NULL,
...
static CV_PossibleValue_t backcolor_cons_t[] = {
    {0, "White"},  {1, "Black"},     {2, "Sepia"},     {3, "Brown"},
    {4, "Pink"},   {5, "Raspberry"}, {6, "Red"},       {7, "Creamsicle"},
    {8, "Orange"}, {9, "Gold"},      {10, "Yellow"},   {11, "Emerald"},
    {12, "Green"}, {13, "Cyan"},     {14, "Steel"},    {15, "Periwinkle"},
    {16, "Blue"},  {17, "Purple"},   {18, "Lavender"}, {0, NULL}};
consvar_t cons_backcolor = {"con_backcolor",
                            "Black",
                            CV_CALL | CV_SAVE,
                            backcolor_cons_t,
...
static CV_PossibleValue_t menuhighlight_cons_t[] = {
    ...
    {0, NULL}};
consvar_t cons_menuhighlight = {"menuhighlight",
                                "Game type",
                                CV_SAVE,
                                menuhighlight_cons_t,
                                NULL,
                                0,
                                NULL,
                                NULL,
...
static void CONS_backcolor_Change(void) { CON_SetupBackColormap(); }

// Font colormap colors
// TODO: This could probably be improved somehow...
// These colormaps are 99% identical, with just a few changed bytes
UINT8 *yellowmap, *purplemap, *greenmap, *bluemap, *graymap, *redmap,
...
void CON_Init(void) {
  ...
  if (!dedicated) {
    ...
    con_started = true;
    con_startup =
        true; // need explicit screen refresh until we are in Doom loop
    consoletoggle = false;
    ...
    con_started = true;
    con_startup =
        false; // need explicit screen refresh until we are in Doom loop
    consoletoggle = true;
...
static void CON_RecalcSize(void) {
  ...
  string = Z_Malloc(CON_BUFFERSIZE, PU_STATIC,
                    NULL); // BP: it is a line but who know
  ...
  if (oldcon_width) // not the first time
  {
    for (i = oldcon_cy + 1; i < oldcon_cy + oldnumlines; i++) {
      if (tmp_buffer[(i % oldnumlines) * oldcon_width]) {
        M_Memcpy(string, &tmp_buffer[(i % oldnumlines) * oldcon_width],
                 oldcon_width);
        conw = oldcon_width - 1;
...
void CON_Ticker(void) {
  ...
  if (cons_backpic.value) // clip only when using an opaque background
  {
    ...
    // NOTE: BIG HACK::SUBTRACT 10, SO THAT WATER DON'T COPY LINES OF THE
    // CONSOLE
...
static void CON_InputAddString(const char *c) {
  ...
  if (input_cur != input_len)
    memmove(&inputlines[inputline][input_cur + csize],
            &inputlines[inputline][input_cur], input_len - input_cur);
...
static void CON_InputDelSelection(void) {
  ...
  if (end != input_len)
    memmove(&inputlines[inputline][start], &inputlines[inputline][end],
            input_len - end);
...
static void CON_InputAddChar(char c) {
  ...
  if (input_cur != input_len)
    memmove(&inputlines[inputline][input_cur + 1],
            &inputlines[inputline][input_cur], input_len - input_cur);
...
boolean CON_Responder(event_t *ev) {
  static UINT8 consdown =
      false; // console is treated differently due to rare usage
  ...
  if (ev->type != ev_keydown && ev->type != ev_console) {
    if (ev->data1 == gamecontrol[gc_console][0] ||
        ev->data1 == gamecontrol[gc_console][1])
      consdown = false;
    return false;
  }
  ...
  if (key == KEY_LSHIFT || key == KEY_RSHIFT || key == KEY_LCTRL ||
      key == KEY_RCTRL || key == KEY_LALT || key == KEY_RALT)
    return true;
  ...
  if (ctrldown) {
    ...
    if (key == KEY_TAB) {
      ...
      CONS_Printf("\nCommands:\n");
      for (i = 0, cmd = COM_CompleteCommand(completion, i); cmd;
           cmd = COM_CompleteCommand(completion, ++i))
        CONS_Printf("  \x83"
                    "%s"
                    "\x80"
                    "%s\n",
      ...
      for (i = 0, cmd = CV_CompleteVar(completion, i); cmd;
           cmd = CV_CompleteVar(completion, ++i))
        CONS_Printf("  \x83"
                    "%s"
                    "\x80"
                    "%s\n",
    ...
    if (key == 'x' || key == 'X') {
      if (input_sel > input_cur)
        I_ClipboardCopy(&inputlines[inputline][input_cur],
                        input_sel - input_cur);
      else
        I_ClipboardCopy(&inputlines[inputline][input_sel],
                        input_cur - input_sel);
  ...
  // allow people to use keypad in console (good for typing IP addresses) -
  // Calum
  if (key >= KEY_KEYPAD7 && key <= KEY_KPADDEL) {
    XBOXSTATIC char keypad_translation[] = {'7', '8', '9', '-', '4', '5', '6',
                                            '+', '1', '2', '3', '0', '.'};
  ...
  if (key >= 'A' && key <= 'Z' &&
      !(shiftdown ^
        capslock)) // this is only really necessary for dedicated servers
    key = key + 'a' - 'A';
...
static void CON_DrawInput(void) {
  ...
  {
    clen -= 2; // There will always be some extra truncation -- but where is
               // what we'll find out

    if (input_cur <= clen / 2) {
      // Close enough to right edge to show all
      c = 0;
      // Always will truncate right side from this position, so always draw
      // right ellipsis
      rellip = 1;
    } else {
  ...
  if (lellip) {
    x -= charwidth * 3;
    if (input_sel < c)
      V_DrawFill(x, y, charwidth * 3, (10 * con_scalefactor),
                 107 | V_NOSCALESTART);
    for (i = 0; i < 3; ++i, x += charwidth)
      V_DrawCharacter(x, y,
                      '.' | cv_constextsize.value | V_GRAYMAP | V_NOSCALESTART,
                      !cv_allcaps.value);
  } else
    V_DrawCharacter(x - charwidth, y,
                    CON_PROMPTCHAR | cv_constextsize.value | V_GRAYMAP |
                        V_NOSCALESTART,
                    !cv_allcaps.value);

  for (cend = c + clen; c < cend; ++c, x += charwidth) {
    if ((input_sel > c && input_cur <= c) ||
        (input_sel <= c && input_cur > c)) {
      V_DrawFill(x, y, charwidth, (10 * con_scalefactor), 107 | V_NOSCALESTART);
      V_DrawCharacter(
          x, y, p[c] | cv_constextsize.value | V_YELLOWMAP | V_NOSCALESTART,
          !cv_allcaps.value);
    } else
      V_DrawCharacter(x, y, p[c] | cv_constextsize.value | V_NOSCALESTART,
                      !cv_allcaps.value);

    if (c == input_cur && con_tick >= 4)
      V_DrawCharacter(x, y + (con_scalefactor * 2),
                      '_' | cv_constextsize.value | V_NOSCALESTART,
                      !cv_allcaps.value);
  }
...
static void CON_DrawHudlines(void) {
  ...
  if (chat_on && OLDCHAT)
    y = charheight; // leave place for chat input in the first row of text (only
                    // do it if consolechat is on.)
  else
    y = 0;
  ...
  for (i = con_cy - con_hudlines + 1; i <= con_cy; i++) {
    ...
    for (c = 0, x = 0; c < con_width; c++, x += charwidth, p++) {
      ...
      if (*p < HU_FONTSTART)
        ...
        // charwidth = SHORT(hu_font['A'-HU_FONTSTART]->width) *
        // con_scalefactor;
        V_DrawCharacter(x, y,
                        (INT32)(*p) | charflags | cv_constextsize.value |
                            V_NOSCALESTART,
                        !cv_allcaps.value);
      }
    }

    // V_DrawCharacter(x, y, (p[c]&0xff) | cv_constextsize.value |
    // V_NOSCALESTART, !cv_allcaps.value);
    y += charheight;
  }
...
static void CON_DrawConsole(void) {
  ...
  for (y = (con_curlines - minheight) % charheight;
       y <= con_curlines - minheight; y += charheight, i++) {
    INT32 x;
    size_t c;
  ...
  if ((con_curlines == con_destlines) && (con_curlines >= minheight) &&
      !con_startup)
    CON_DrawInput();
}
...
void CON_Drawer(void) {
  ...
  if (con_curlines > 0)
    CON_DrawConsole();
           ...
           gamestate == GS_VOTING || gamestate == GS_EVALUATION ||
