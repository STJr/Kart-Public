...
int curlprogress_callback(void *clientp, double dltotal, double dlnow,
...
UINT8 *PutFileNeeded(UINT16 firstfile) {
  size_t i;
  UINT8 count = 0;
  UINT8 *p_start = netbuffer->packettype == PT_MOREFILESNEEDED
                       ? netbuffer->u.filesneededcfg.files
                       : netbuffer->u.serverinfo.fileneeded;
  UINT8 *p = p_start;
  char wadfilename[MAX_WADPATH] = "";
  UINT8 filestatus;

  for (i = mainwads + 1; i < numwadfiles;
       i++) // mainwads+1, otherwise we start on the first mainwad
  {
    // If it has only music/sound lumps, don't put it in the list
    if (!wadfiles[i]->important)
      continue;

    if (firstfile) { // Skip files until we reach the first file.
      firstfile--;
      continue;
    }
    ...
    if (p + 1 + 4 + min(strlen(wadfilename) + 1, MAX_WADPATH) + 16 >
        p_start + MAXFILENEEDED) {
    ...
    filestatus = 1; // Importance - not really used any more, holds 1 by default
                    // for backwards compat with MS
 ...
 *
 * \param fileneedednum_parm The number of files (sent in this page) needed to
 * join the server \param fileneededstr The memory block containing the list of
 * needed files \param firstfile The first file index to read from
 */
void D_ParseFileneeded(INT32 fileneedednum_parm, UINT8 *fileneededstr,
  ...
  for (i = firstfile; i < fileneedednum; i++) {
    fileneeded[i].status =
        FS_NOTCHECKED; // We haven't even started looking for the file yet
    filestatus = READUINT8(p); // The first byte is the file status
    fileneeded[i].willsend = (UINT8)(filestatus >> 4);
    fileneeded[i].totalsize =
        READUINT32(p);         // The four next bytes are the file size
    fileneeded[i].file = NULL; // The file isn't open yet
    READSTRINGN(p, fileneeded[i].filename,
                MAX_WADPATH); // The next bytes are the file name
    READMEM(p, fileneeded[i].md5sum,
            16); // The last 16 bytes are the file checksum
  }
}
...
boolean CL_CheckDownloadable(void) {
  ...
  CONS_Alert(
      CONS_NOTICE,
      M_GetText("You need additional files to connect to this server:\n"));
  for (i = 0; i < fileneedednum; i++)
    if (fileneeded[i].status != FS_FOUND && fileneeded[i].status != FS_OPEN) {
      CONS_Printf(" * \"%s\" (%dK)", fileneeded[i].filename,
                  fileneeded[i].totalsize >> 10);
  ...
  case 1:
    CONS_Printf(M_GetText(
        "Some files are larger than the server is willing to send.\n"));
    break;
  case 2:
    CONS_Printf(M_GetText("The server is not allowing download requests.\n"));
    break;
  case 3:
    CONS_Printf(M_GetText("All files downloadable, but you have chosen to "
                          "disable downloading locally.\n"));
    break;
  }
  return false;
}
...
boolean CL_SendRequestFile(void) {
  ...
  if (M_CheckParm("-nodownload")) {
    CONS_Printf(
        "Direct download - Attempted to download files in -nodownload mode");
    return false;
  }
#endif
  ...
  for (i = 0; i < fileneedednum; i++) {
    if (fileneeded[i].status != FS_FOUND && fileneeded[i].status != FS_OPEN &&
      ...
      CONS_Printf("Direct download - attempted to download files that were not "
                  "sendable\n");
      return false;
    }
    ...
    if ((fileneeded[i].status == FS_NOTFOUND ||
         ...
         fileneeded[i].status == FS_FALLBACK)) {
  ...
  if (totalfreespaceneeded > availablefreespace) {
    CONS_Printf("Direct download -\n"
                ...
                sizeu1((size_t)(totalfreespaceneeded >> 10)),
                sizeu2((size_t)(availablefreespace >> 10)));
    return false;
  }
...
tryagain:
  ...
  for (i = 0; i < fileneedednum; i++) {
    if ((fileneeded[i].status == FS_NOTFOUND ||
      ...
      checklen = strlen(fileneeded[i].filename) +
                 2; // plus the fileid (and terminator, in case this is last)
      if ((UINT8 *)(p + checklen) >= netbuffer->u.textcmd + MAXTEXTCMD) {
        skippedafile = i;
        // we might have a shorter file that can fit in the remaining space, and
        // file ID permits out-of-order data
        continue;
      }
  ...
  if (p == (char *)netbuffer->u.textcmd) {
    CONS_Printf("Direct download - fileneeded name for %s (fileneeded[%d]) too "
                "long??\n",
                (skippedafile != -1 ? fileneeded[skippedafile].filename : NULL),
                skippedafile);
    return false;
  }
...
boolean Got_RequestFilePak(INT32 node) {
  ...
  UINT8 id;
  while (p < netbuffer->u.textcmd +
                 MAXTEXTCMD) // Don't allow hacked client to overflow
  {
    id = READUINT8(p);
    if (id == 0xFF)
      break;
 ...
 *         2 if some already loaded files are not requested or are in a
 * different order 3 too many files, over WADLIMIT 4 still checking, continuing
 * next tic
 *
 */
INT32
CL_CheckFiles(void) {
  ...
  for (i = 0; i < fileneedednum; i++) {
    ...
    if (fileneeded[i].status !=
        FS_NOTCHECKED) // since we're running this over multiple tics now, its
                       // possible for us to come across files checked in
                       // previous tics
      continue;
    ...
    fileneeded[i].status =
        findfile(fileneeded[i].filename, fileneeded[i].md5sum, true);
    CONS_Debug(DBG_NETPLAY, "found %d\n", fileneeded[i].status);
    return 4;
  }

  // now making it here means we've checked the entire list and no FS_NOTCHECKED
  // files remain
  ...
  for (i = 0; i < fileneedednum; i++) {
    if (fileneeded[i].status == FS_OPEN)
      ...
      I_Error("Try to load file \"%s\" with status of %d (%s)\n",
              fileneeded[i].filename, fileneeded[i].status, s);
    }
  }
  return true;
}
...
static boolean SV_SendFile(INT32 node, const char *filename, UINT8 fileid) {
  ...
  if (cv_noticedownload.value)
    CONS_Printf("Sending file \"%s\" (id %d) to node %d (%s)\n", filename,
                fileid, node, I_GetNodeAddress(node));
  ...
  if (!wadfiles[i]) {
    ...
    // Not found
    // Don't inform client (probably someone who thought they could leak 2.2
    // ACZ)
  ...
  p->ram = SF_FILE; // It's a file, we need to close it and free its name once
...
void SV_SendRam(INT32 node, void *data, size_t size, freemethod_t freemethod,
  ...
  p->ram = freemethod; // Remember how to free the memory block for when we're
                       // done sending it
  p->id.ram = data;
  p->size = (UINT32)size;
  p->fileid = fileid;
 ...
 * either because the file has been fully sent or because the node was
 * disconnected
 *
...
static void SV_EndFileSend(INT32 node) {
  ...
  // Free the file request according to the freemethod parameter used with
  // SV_SendFile/Ram
  switch (p->ram) {
  case SF_FILE: // It's a file, close it and free its filename
    ...
    free(p->id.filename);
    break;
  case SF_Z_RAM: // It's a memory block allocated with Z_Alloc or the likes,
                 // use Z_Free
    Z_Free(p->id.ram);
    break;
...
void SV_FileSendTicker(void) {
  ...
  if (cv_downloadspeed.value) // New (and experimental) behavior
  {
    ...
    maxpacketsent = 1;
#endif
    if (packetsent > maxpacketsent &&
        maxpacketsent > 0) // Send at least one packet
  ...
  found:
    ...
    if (transfer[i].init == false) {
      if (!ram) // Sending a file
      {
        ...
        if (transferFiles[f->fileid].count == 0) {
          // It needs opened.
          transferFiles[f->fileid].file = fopen(f->id.filename, "rb");

          if (!transferFiles[f->fileid].file) {
            I_Error("Can't open file %s: %s", f->id.filename, strerror(errno));
          }
        }
    ...
    if (ram) {
      ...
      I_Error("SV_FileSendTicker: can't read %s byte on %s at %d because %s",
              sizeu1(size), f->id.filename, transfer[i].position,
              M_FileError(transferFiles[f->fileid].file));

      transferFiles[f->fileid].position =
          (UINT32)(transferFiles[f->fileid].position + size);
    }
...
void Got_Filetxpak(void) {
  ...
  if (!(strcmp(filename, "srb2.srb") && strcmp(filename, "srb2.wad") &&
        strcmp(filename, "patch.dta")
        //&& strcmp(filename, "music.dta")
        && strcmp(filename, "gfx.kart") && strcmp(filename, "textures.kart") &&
        strcmp(filename, "chars.kart") && strcmp(filename, "maps.kart") &&
        strcmp(filename, "sounds.kart") && strcmp(filename, "music.kart") &&
        strcmp(filename, "patch.kart")))
    I_Error("Tried to download \"%s\"", filename);

  if (filenum >= fileneedednum) {
    DEBFILE(va("fileframent not needed %d>%d\n", filenum, fileneedednum));
    // I_Error("Received an unneeded file fragment (file id received: %d, file
    // id needed: %d)\n", filenum, fileneedednum);
    return;
  }
  ...
  if (file->status == FS_DOWNLOADING) {
    ...
    // We can receive packet in the wrong order, anyway all os support gaped
    // file
 ...
 *
 */
boolean SV_SendingFile(INT32 node) { return transfer[node].txlist != NULL; }
...
// Note: if completepath is true, "filename" is modified, but only if FS_FOUND
// is going to be returned (Don't worry about WinCE's version of filesearch,
// nobody cares about that OS anymore)
filestatus_t findfile(char *filename, const UINT8 *wantedmd5sum,
  ...
  boolean badmd5 = false; // store whether md5 was bad from either of the first
                          // two searches (if nothing was found in the third)
  ...
  if (homecheck == FS_FOUND) // we found the file, so return that we have :)
    return FS_FOUND;
  else if (homecheck == FS_MD5SUMBAD) // file has a bad md5; move on and look
                                      // for a file with the right md5
    badmd5 = true;
  ...
  if (homecheck != FS_NOTFOUND) // if not found this time, fall back on the
...
int curlprogress_callback(void *clientp, double dltotal, double dlnow,
  ...
  (void)clientp;
  (void)ultotal;
  (void)ulnow; // Function prototype requires these but we won't use, so just
               // discard
  curl_dlnow = dlnow;
  curl_dltotal = dltotal;
  getbytes = curl_dlnow /
             (time(NULL) - curl_starttime); // To-do: Make this more accurate???
  return 0;
}
...
void CURLPrepareFile(const char *url, int dfilenum) {
  ...
  if (http_handle && multi_handle) {
    ...
    curl_easy_setopt(http_handle, CURLOPT_PROTOCOLS,
                     CURLPROTO_HTTP | CURLPROTO_HTTPS);

    curl_easy_setopt(http_handle, CURLOPT_USERAGENT,
                     va("SRB2Kart/v%d.%d", VERSION,
                        SUBVERSION)); // Set user agent as some servers won't
                                      // accept invalid user agents.
    ...
    curl_easy_setopt(http_handle, CURLOPT_PROGRESSFUNCTION,
                     curlprogress_callback);
  ...
  while ((m = curl_multi_info_read(multi_handle, &msgs_left))) {
    if (m && (m->msg == CURLMSG_DONE)) {
      ...
      if (easyres != CURLE_OK) {
        ...
        easy_handle_error = (response_code)
                                ? va("HTTP reponse code %ld", response_code)
                                : curl_easy_strerror(easyres);
        ...
        if (checkfilemd5(curl_curfile->filename, curl_curfile->md5sum) ==
            FS_MD5SUMBAD) {
          CONS_Alert(CONS_ERROR,
                     M_GetText("HTTP Download of %s finished but is corrupt or "
                               "has been modified\n"),
                     filename);
          curl_curfile->status = FS_FALLBACK;
          curl_failedwebdownload = true;
...
HTTP_login *CURLGetLogin(const char *url, HTTP_login ***return_prev_next) {
  ...
  for (prev_next = &curl_logins; (login = (*prev_next));
