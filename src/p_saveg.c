...
static void P_NetUnArchivePlayers(void) {
  ...
  for (i = 0; i < MAXPLAYERS; i++) {
    ...
    players[i].continues =
        READSINT8(save_p); // continues that player has acquired
    ...
    players[i].speed = READFIXED(
        save_p); // Player's speed (distance formula of MOMX and MOMY values)
    ...
    /////////////////////
    players[i].numboxes =
        READINT16(save_p); // Number of item boxes obtained for Race Mode
    players[i].totalring =
        READINT16(save_p); // Total number of rings obtained for Race Mode
    players[i].realtime =
        READUINT32(save_p);              // integer replacement for leveltime
    ...
    players[i].gotflag =
        READUINT16(save_p); // 1 == Red, 2 == Blue Do you have the flag?
...
static void P_NetArchiveWorld(void) {
  ...
  { // HACK: Open wad file rather quickly so we can get the data from the
    // relevant lumps
...
#undef retrieve_mapdata
  ...
  for (i = 0; i < numsectors; i++, ss++, ms++) {
    ...
    if (diff) {
      ...
      // Special case: save the stats of all modified ffloors along with their
      // ffloor "number"s we don't bother with ffloors that haven't changed,
      // that would just add to savegame even more than is really needed
      if (diff & SD_FFLOORS) {
        size_t j = 0; // ss->ffloors is saved as ffloor #0, ss->ffloors->next is
                      // #1, etc
        ffloor_t *rover;
        UINT8 fflr_diff;
        for (rover = ss->ffloors; rover; rover = rover->next) {
  ...
  for (i = 0; i < numlines; i++, mld++, li++) {
    ...
    if (SHORT(mld->special) == 321 ||
        SHORT(mld->special) ==
            322) // only reason li->callcount would be non-zero is if either of
                 // these are involved
      diff |= LD_CLLCOUNT;

    if (li->sidenum[0] != 0xffff) {
      si = &sides[li->sidenum[0]];
      if (si->textureoffset != SHORT(msd[li->sidenum[0]].textureoffset)
                                   << FRACBITS)
        diff |= LD_S1TEXOFF;
      // SoM: 4/1/2000: Some textures are colormaps. Don't worry about invalid
      // textures.
      ...
      if (R_CheckTextureNumForName(msd[li->sidenum[0]].bottomtexture) != -1 &&
          si->bottomtexture !=
              R_TextureNumForName(msd[li->sidenum[0]].bottomtexture))
        diff |= LD_S1BOTTEX;
    ...
    if (li->sidenum[1] != 0xffff) {
      ...
      if (si->textureoffset != SHORT(msd[li->sidenum[1]].textureoffset)
                                   << FRACBITS)
        diff2 |= LD_S2TEXOFF;
      ...
      if (R_CheckTextureNumForName(msd[li->sidenum[1]].bottomtexture) != -1 &&
          si->bottomtexture !=
              R_TextureNumForName(msd[li->sidenum[1]].bottomtexture))
        diff2 |= LD_S2BOTTEX;
...
static void P_NetUnArchiveWorld(void) {
  ...
  for (;;) {
    ...
    if (i > numsectors)
      I_Error("Invalid sector number %u from server (expected end at %s)", i,
              sizeu1(numsectors));
    ...
    if (diff & SD_FFLOORS) {
      ...
      if (!rover) // it is assumed sectors[i].ffloors actually exists, but just
                  // in case...
        I_Error("Sector does not have any ffloors!");

      fflr_i = READUINT16(get); // get first modified ffloor's number ready
      for (;;) // for some reason the usual for (rover = x; ...) thing doesn't
               // work here?
      {
        if (fflr_i ==
            0xffff) // end of modified ffloors list, let's stop already
          break;
...
static void SaveMobjThinker(const thinker_t *th, const UINT8 type) {
  ...
  if (mobj->friction != ORIG_FRICTION)
    diff |= MD_FRICTION;
  if (mobj->movefactor !=
      FRACUNIT) // if (mobj->movefactor != ORIG_FRICTION_FACTOR)
    diff |= MD_MOVEFACTOR;
  if (mobj->fuse)
    diff |= MD_FUSE;
  if (mobj->watertop)
  ...
  // save pointer, at load time we will search this pointer to reinitilize
  // pointers
...
//
static inline void SaveFireflickerThinker(const thinker_t *th,
                                          const UINT8 type) {
...
//
static inline void SavePolyrotatetThinker(const thinker_t *th,
                                          const UINT8 type) {
...
#ifdef PARANOIA
    else if (th->function.acv !=
             P_RemoveThinkerDelayed) // wait garbage collection
...
static void LoadMobjThinker(actionf_p1 thinker) {
  ...
  if (diff & MD_SPAWNPOINT) {
    ...
    if (mapthings[spawnpointnum].type == 1705 ||
        mapthings[spawnpointnum].type == 1713) // NiGHTS Hoop special case
    {
...
static void P_NetUnArchiveThinkers(void) {
  ...
  for (currentthinker = thinkercap.next; currentthinker != &thinkercap;
    ...
    if (currentthinker->function.acp1 == (actionf_p1)P_MobjThinker ||
        currentthinker->function.acp1 == (actionf_p1)P_NullPrecipThinker)
      P_RemoveSavegameMobj(
          (mobj_t *)currentthinker); // item isn't saved, don't remove it
    else {
  ...
  // clear sector thinker pointers so they don't point to non-existant thinkers
  // for all of eternity
  ...
  for (;;) {
    ...
    /// \todo rewrite all the code that uses an elevator_t but isn't an
    /// elevator \note working on it!
...
static void P_RelinkPointers(void) {
  ...
  for (currentthinker = thinkercap.next; currentthinker != &thinkercap;
    ...
    if (currentthinker->function.acp1 == (actionf_p1)P_MobjThinker) {
      ...
      if (mobj->type == MT_HOOP || mobj->type == MT_HOOPCOLLIDE ||
          mobj->type == MT_HOOPCENTER)
        continue;
...
static inline void P_UnArchiveSPGame(INT16 mapoverride) {
  ...
  if (strcmp(testname, timeattackfolder)) {
    if (modifiedgame)
      ...
      I_Error("This save file is for a particular mod, it cannot be used with "
              "the regular game.");
  }
...
static void P_NetArchiveMisc(void) {
  ...
  if (gamestate != GS_LEVEL)
    WRITEINT16(
        save_p,
        GS_WAITINGPLAYERS); // nice hack to put people back into waitingplayers
  else
...
void P_SaveNetGame(void) {
  ...
  mobj_t *mobj;
  INT32 i =
      1; // don't start from 0, it'd be confused with a blank pointer otherwise
...
boolean P_LoadNetGame(void) {
  ...
  // The precipitation would normally be spawned in P_SetupLevel, which is
  // called by P_NetUnArchiveMisc above. However, that would place it up before
  // P_NetUnArchiveThinkers, so the thinkers would be deleted later. Therefore,
  // P_SetupLevel will *not* spawn precipitation when loading a netgame save.
  // Instead, precip has to be spawned here. This is done in
  // P_NetUnArchiveSpecials now.
