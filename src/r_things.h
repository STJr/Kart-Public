...
typedef struct {
  ...
  char hudname[SKINNAMESIZE +
               1]; // HUD name to display (officially exactly 5 characters long)
...
// NOT SKINS STUFF !
// -----------
typedef enum { SC_NONE = 0, SC_TOP = 1, SC_BOTTOM = 2 } spritecut_e;
...
typedef struct vissprite_s {
  ...
  fixed_t scale,
      sortscale;     // sortscale only differs from scale for flat sprites
  ...
  boolean isScaled;
  INT32
  dispoffset; // copy of info->dispoffset, affects ordering but not drawing
...
#if 1 // 2.1 compat
  if (cn == '+')
    return '\\' - 'A'; // PK3 can't use backslash, so use + instead
  return cn - 'A';
#else
  if (cn >= 'A' && cn <= 'Z')
    return cn - 'A';
...
FUNCMATH FUNCINLINE static ATTRINLINE boolean
R_ValidSpriteAngle(UINT8 rotation) {
