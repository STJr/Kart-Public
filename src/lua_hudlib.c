...
#define HUDONLY                                                                \
  if (!hud_running)                                                            \
    return luaL_error(L, "HUD rendering code should not be called outside of " \
                         "rendering hooks!");
...
static const char *const hud_disable_options[] = {
    "stagetitle", "textspectator",

    "time",
    "gametypeinfo", // Bumpers / Karma / Laps depending on gametype
    "minimap", "item", "position",
    "check",                 // "CHECK" f-zero indicator
    "minirankings",          // Gametype rankings to the left
    "battlerankingsbumpers", // bumper drawer for battle. Useful if you want to
                             // make a custom battle gamemode without bumpers
                             // being involved.
    "battlefullscreen", // battlefullscreen func (WAIT, ATTACK OR PROTECT ...)
    "battlecomebacktimer", // come back timer in battlefullscreen
    "wanted", "speedometer", "freeplay", "rankings", NULL};

enum hudinfo { hudinfo_x = 0, hudinfo_y };

static const char *const hudinfo_opt[] = {"x", "y", NULL};
...
static const char *const patch_opt[] = {"valid",      "width",     "height",
                                        "leftoffset", "topoffset", NULL};

enum hudhook { hudhook_game = 0, hudhook_scores };
static const char *const hudhook_opt[] = {"game", "scores", NULL};
...
static const char *const align_opt[] = {"left",  "center",     "right",
                                        "fixed", "small",      "small-right",
                                        "thin",  "thin-right", NULL};

// width types for v.stringWidth
enum widtht { widtht_normal = 0, widtht_small, widtht_thin };
static const char *const widtht_opt[] = {"normal", "small", "thin", NULL};
...
static const char *const camera_opt[] = {
    "chase",     "aiming", "x",        "y",      "z",      "angle",
    "subsector", "floorz", "ceilingz", "radius", "height", "momx",
    "momy",      "momz",   "pnum",     NULL};
...
static int lib_getHudInfo(lua_State *L) {
  ...
  if (i >= NUMHUDITEMS)
    return luaL_error(L, "hudinfo[] index %d out of range (0 - %d)", i,
                      NUMHUDITEMS - 1);
...
static int libd_cachePatch(lua_State *L) {
  HUDONLY
  LUA_PushUserdata(L, W_CachePatchName(luaL_checkstring(L, 1), PU_STATIC),
                   META_PATCH);
  return 1;
}
...
static int libd_draw(lua_State *L) {
  ...
  if (LUA_HUD_IsDrawListValid(list))
    ...
    V_DrawFixedPatch(x << FRACBITS, y << FRACBITS, FRACUNIT, flags, patch,
                     colormap);
  return 0;
}
...
static int libd_drawOnMinimap(lua_State *L) {
  ...
  boolean centered; // the patch is centered and doesn't need readjusting on x/y
                    // coordinates.
  huddrawlist_h list;
  ...
  // replicate exactly what source does for its minimap drawer; AKA hardcoded
  // garbo.

  // first, check what position the mmap is supposed to be in (pasted from
  // k_kart.c):
  ...
  // now that we have replicated this behavior, we can draw an icon from our
  // supplied x, y coordinates by replicating k_kart.c's totally understandable
  // uncommented code!!!
  ...
  if (LUA_HUD_IsDrawListValid(list))
    LUA_HUD_AddDrawScaled(list, amxpos, amypos, scale, patch, splitflags,
                          colormap);
  else
...
static int libd_fadeScreen(lua_State *L) {
  ...
  if (strength > maxstrength)
    return luaL_error(L, "%s fade strength %d out of range (0 - %d)",
                      ((color & 0xFF00) ? "COLORMAP" : "TRANSMAP"), strength,
                      maxstrength);
  ...
  {
    if (LUA_HUD_IsDrawListValid(list))
      LUA_HUD_AddDrawFill(list, 0, 0, BASEVIDWIDTH, BASEVIDHEIGHT,
                          ((color & 0xFF00) ? 31 : color));
    else
...
static int libd_getColormap(lua_State *L) {
  ...
  else if (lua_type(L, 1) == LUA_TNUMBER) // skin number
  {
    ...
    if (skinnum < TC_BLINK || skinnum >= MAXSKINS)
      return luaL_error(L, "skin number %d is out of range (%d - %d)", skinnum,
                        TC_BLINK, MAXSKINS - 1);
  } else // skin name
  {
  ...
  LUA_PushUserdata(L, colormap,
                   META_COLORMAP); // push as META_COLORMAP userdata,
                                   // specifically for patches to use!
  return 1;
}
...
static int libd_renderer(lua_State *L) {
  ...
  case render_opengl:
    ...
    break; // OpenGL renderer
  case render_soft:
    lua_pushliteral(L, "software");
    break; // Software renderer
  default:
    lua_pushliteral(L, "none");
    break; // render_none (for dedicated), in case there's any reason this
           // should be run
  }
  return 1;
}

// 30/10/18 Lat': Get cv_translucenthud's value for HUD rendering as a normal
// V_xxTRANS int Could as well be thrown in global vars for ease of access but I
// guess it makes sense for it to be a HUD fn
static int libd_getlocaltransflag(lua_State *L) {
  HUDONLY
  lua_pushinteger(L,
                  (10 - cv_translucenthud.value) *
                      V_10TRANS); // A bit weird that it's called
                                  // "translucenthud" yet 10 is fully opaque :V
  return 1;
}

static luaL_Reg lib_draw[] = {{"patchExists", libd_patchExists},
...
static luaL_Reg lib_hud[] = {{"enable", lib_hudenable},
