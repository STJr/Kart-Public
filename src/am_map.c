...
static INT32 f_w, f_h; // size of window on screen (always the screen width and
                       // height respectively)
...
static INT32 followplayer =
    true; // specifies whether to follow the player around
...
static inline void AM_drawWalls(void) {
  ...
  fixed_t backf1 = 0, backf2 = 0, backc1 = 0,
          backc2 = 0; // back floor ceiling ends
...
#define SLOPEPARAMS(slope, end1, end2, normalheight)                           \
  if (slope) {                                                                 \
    ...
    SLOPEPARAMS(lines[i].frontsector->f_slope, frontf1, frontf2,
                lines[i].frontsector->floorheight)
    SLOPEPARAMS(lines[i].frontsector->c_slope, frontc1, frontc2,
                lines[i].frontsector->ceilingheight)
    if (lines[i].backsector) {
      SLOPEPARAMS(lines[i].backsector->f_slope, backf1, backf2,
                  lines[i].backsector->floorheight)
      SLOPEPARAMS(lines[i].backsector->c_slope, backc1, backc2,
    ...
    if (!lines[i].backsector) // 1-sided
    {
      if (lines[i].flags & ML_NOCLIMB)
        ...
        AM_drawMline(&l, WALLCOLORS);
               ...
               || (frontf1 == frontc1 &&
                   frontf2 == frontc2)) // Front is thok barrier
    {
      if (backf1 == backc1 && backf2 == backc2 && frontf1 == frontc1 &&
          frontf2 == frontc2) // BOTH are thok barriers
      {
        if (lines[i].flags & ML_NOCLIMB)
...
static void AM_drawLineCharacter(const mline_t *lineguy, size_t lineguylines,
                                 fixed_t scale, angle_t angle, INT32 color,
                                 fixed_t x, fixed_t y) {
  size_t i;
  mline_t l;
...
static inline void AM_drawPlayers(void) {
  ...
  if (!multiplayer) {
    AM_drawLineCharacter(player_arrow, NUMPLYRLINES, 0, plr->mo->angle, DWHITE,
                         plr->mo->x, plr->mo->y);
    return;
  }
  ...
  for (i = 0; i < MAXPLAYERS; i++) {
    ...
    p = &players[i];
    if (p->skincolor > 0)
      color = R_GetTranslationColormap(TC_DEFAULT, p->skincolor,
                                       GTC_CACHE)[GREENS + 8];
