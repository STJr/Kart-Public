...
consvar_t cv_masterserver = {"masterserver",
                             ...
                             0,
                             NULL};
consvar_t cv_rendezvousserver = {"holepunchserver",
                                 "relay.kartkrew.org",
                                 CV_SAVE,
                                 NULL,
                                 NULL,
                                 ...
                                 0,
                                 NULL};
consvar_t cv_servername = {"servername",
                           "SRB2Kart server",
                           ...
                           0,
                           NULL};
consvar_t cv_server_contact = {"server_contact",
                               "",
                               CV_SAVE | CV_CALL | CV_NOINIT,
                               NULL,
                               Update_parameters,
...
consvar_t cv_masterserver_update_rate = {"masterserver_update_rate",
                                         "15",
                                         CV_SAVE | CV_CALL | CV_NOINIT,
...
consvar_t cv_advertise = {"advertise",
                          "No",
...
consvar_t cv_masterserver_nagattempts = {"masterserver_nagattempts",
                                         "5",
                                         CV_SAVE,
                                         CV_Unsigned,
                                         NULL,
...
static void Finish_masterserver_change(char *api) {
  char rules[256];
...
#ifdef MASTERSERVER
  I_spawn_thread("register-server", (I_thread_fn)Register_server_thread,
                 New_server_id());
#endif /*MASTERSERVER*/
}

static void UpdateServer(void) {
  I_spawn_thread("update-server", (I_thread_fn)Update_server_thread,
                 Server_id());
}

void UnregisterServer(void) {
#ifdef MASTERSERVER
  I_spawn_thread("unlist-server", (I_thread_fn)Unlist_server_thread,
                 Server_id());
#endif /*MASTERSERVER*/
}
...
static inline void SendPingToMasterServer(void) {
  ...
  if (Online()) {
    ...
    /*Lock_state();*/
    {
      ready = (MSRegisteredId == MSId && !MSInProgress &&
