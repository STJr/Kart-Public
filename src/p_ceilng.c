  ...
  case 1: // UP
    ...
    if (ceiling->type == bounceCeiling) {
      const fixed_t origspeed =
          FixedDiv(ceiling->origspeed, (ELEVATORSPEED / 2));
      const fixed_t fs =
          abs(ceiling->sector->ceilingheight -
              lines[ceiling->texture].frontsector->ceilingheight);
      const fixed_t bs = abs(ceiling->sector->ceilingheight -
                             lines[ceiling->texture].backsector->ceilingheight);
      if (fs < bs)
    ...
    if (res == pastdest) {
      ...
      case moveCeilingByFrontSector:
        if (ceiling->texture < -1) // chained linedef executing
          P_LinedefExecute((INT16)(ceiling->texture + INT16_MAX + 2), NULL,
                           NULL);
      ...
      case bounceCeilingCrush: {
        ...
        if (dest == lines[ceiling->texture].frontsector->ceilingheight) {
          ...
          ceiling->speed = ceiling->origspeed =
              FixedDiv(abs(lines[ceiling->texture].dy),
                       4 * FRACUNIT); // return trip, use dy
        } else {
          dest = lines[ceiling->texture].frontsector->ceilingheight;
          ceiling->speed = ceiling->origspeed =
              FixedDiv(abs(lines[ceiling->texture].dx),
                       4 * FRACUNIT); // going frontways, use dx
        }
...
void T_CrushCeiling(ceiling_t *ceiling) {
  ...
  case 1: // UP
    if (ceiling->type == crushBothOnce) {
      // Move the floor
      T_MovePlane(ceiling->sector, ceiling->speed,
                  ceiling->bottomheight -
                      (ceiling->topheight - ceiling->bottomheight),
                  false, 0, -ceiling->direction);
    }
    ...
    if (res == pastdest) {
      ...
      if (ceiling->type == crushCeilOnce || ceiling->type == crushBothOnce) {
        // Remove
        switch (ceiling->type) {
        case crushCeilOnce:
          ceiling->sector->ceilspeed = 0;
  ...
  case -1: // DOWN
    if (ceiling->type == crushBothOnce) {
      ...
      T_MovePlane(ceiling->sector, ceiling->speed, ceiling->bottomheight,
                  ceiling->crush, 0, -ceiling->direction);
    }
  ...
  while ((secnum = P_FindSectorFromLineTag(line, secnum)) >= 0) {
    ...
    case moveCeilingByFrontTexture:
      if (line->flags & ML_NOCLIMB)
        ...
        ceiling->speed = FixedDiv(sides[line->sidenum[0]].textureoffset,
      ...
      /*
                              case lowerCeilingByLine:
                                      ceiling->speed =
         FixedDiv(abs(line->dx),8*FRACUNIT); ceiling->direction = -1; // Move
         down ceiling->bottomheight = sec->ceilingheight - abs(line->dy);
                                      break;

                              case raiseCeilingByLine:
                                      ceiling->speed =
         FixedDiv(abs(line->dx),8*FRACUNIT); ceiling->direction = 1; // Move
         up ceiling->topheight = sec->ceilingheight + abs(line->dy); break;
      */

    case bounceCeiling:
      ceiling->speed = P_AproxDistance(
      ...
      ceiling->texture =
          (fixed_t)(line -
                    lines); // hack: use texture to store sourceline number
      break;
...
EV_DoCrush(line_t *line, ceiling_e type) {
  ...
  while ((secnum = P_FindSectorFromLineTag(line, secnum)) >= 0) {
    ...
    if (line->flags & ML_EFFECT4)
      ...
      ceiling->oldspeed =
          (R_PointToDist2(line->v2->x, line->v2->y, line->v1->x, line->v1->y) /
           16);
