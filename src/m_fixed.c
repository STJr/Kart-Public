...
vector2_t *FV2_AddEx(const vector2_t *a_i, const vector2_t *a_c,
                     vector2_t *a_o) {
  a_o->x = a_i->x + a_c->x;
  a_o->y = a_i->y + a_c->y;
  return a_o;
...
vector2_t *FV2_SubEx(const vector2_t *a_i, const vector2_t *a_c,
                     vector2_t *a_o) {
  a_o->x = a_i->x - a_c->x;
  a_o->y = a_i->y - a_c->y;
  return a_o;
...
// Vector Complex Math
vector2_t *FV2_Midpoint(const vector2_t *a_1, const vector2_t *a_2,
                        vector2_t *a_o) {
...
vector2_t *FV2_Negate(vector2_t *a_1) { return FV2_NegateEx(a_1, a_1); }

boolean FV2_Equal(const vector2_t *a_1, const vector2_t *a_2) {
  fixed_t Epsilon = FRACUNIT / FRACUNIT;

  if ((abs(a_2->x - a_1->x) > Epsilon) || (abs(a_2->y - a_1->y) > Epsilon)) {
    return true;
  }
...
//
vector2_t *FV2_Point2Vec(const vector2_t *point1, const vector2_t *point2,
                         vector2_t *a_o) {
...
vector3_t *FV3_AddEx(const vector3_t *a_i, const vector3_t *a_c,
                     vector3_t *a_o) {
  a_o->x = a_i->x + a_c->x;
  a_o->y = a_i->y + a_c->y;
  a_o->z = a_i->z + a_c->z;
...
vector3_t *FV3_SubEx(const vector3_t *a_i, const vector3_t *a_c,
                     vector3_t *a_o) {
  a_o->x = a_i->x - a_c->x;
  a_o->y = a_i->y - a_c->y;
  a_o->z = a_i->z - a_c->z;
...
// Vector Complex Math
vector3_t *FV3_Midpoint(const vector3_t *a_1, const vector3_t *a_2,
                        vector3_t *a_o) {
  a_o->x = FixedDiv(a_2->x - a_1->x, 2 * FRACUNIT);
  a_o->y = FixedDiv(a_2->y - a_1->y, 2 * FRACUNIT);
...
vector3_t *FV3_Negate(vector3_t *a_1) { return FV3_NegateEx(a_1, a_1); }
...
//
void FV3_ClosestPointOnVector(const vector3_t *dir, const vector3_t *p,
                              vector3_t *out) {
...
fixed_t FV3_Dot(const vector3_t *a_1, const vector3_t *a_2) {
  return (FixedMul(a_1->x, a_2->x) + FixedMul(a_1->y, a_2->y) +
          FixedMul(a_1->z, a_2->z));
}

vector3_t *FV3_Cross(const vector3_t *a_1, const vector3_t *a_2,
                     vector3_t *a_o) {
...
//
vector3_t *FV3_ClosestPointOnLine(const vector3_t *Line, const vector3_t *p,
                                  vector3_t *out) {
...
//
void FV3_ClosestPointOnTriangle(const vector3_t *tri, const vector3_t *point,
                                vector3_t *result) {
  UINT8 i;
...
// Given two points, create a vector between them.
//
vector3_t *FV3_Point2Vec(const vector3_t *point1, const vector3_t *point2,
                         vector3_t *a_o) {
  a_o->x = point1->x - point2->x;
  a_o->y = point1->y - point2->y;
...
fixed_t FV3_PlaneDistance(const vector3_t *a_normal, const vector3_t *a_point) {
  return -(FixedMul(a_normal->x, a_point->x) +
           FixedMul(a_normal->y, a_point->y) +
           FixedMul(a_normal->z, a_point->z));
}

boolean FV3_IntersectedPlane(const vector3_t *a_triangle,
                             const vector3_t *a_line, vector3_t *a_normal,
  ...
  *originDistance = FV3_PlaneDistance(a_normal, &a_triangle[0]);
  ...
  distance1 =
      (FixedMul(a_normal->x, a_line[0].x) + FixedMul(a_normal->y, a_line[0].y) +
       FixedMul(a_normal->z, a_line[0].z)) +
      *originDistance;

  distance2 =
      (FixedMul(a_normal->x, a_line[1].x) + FixedMul(a_normal->y, a_line[1].y) +
       FixedMul(a_normal->z, a_line[1].z)) +
      *originDistance;
...
// the plane.
//
fixed_t FV3_PlaneIntersection(const vector3_t *pOrigin,
                              const vector3_t *pNormal,
                              const vector3_t *rOrigin,
                              const vector3_t *rVector) {
...
//
fixed_t FV3_IntersectRaySphere(const vector3_t *rO, const vector3_t *rV,
                               const vector3_t *sO, fixed_t sR) {
  vector3_t Q;
  fixed_t c, v, d;
  FV3_SubEx(sO, rO, &Q);
...
//
vector3_t *FV3_IntersectionPoint(const vector3_t *vNormal,
                                 const vector3_t *vLine, fixed_t distance,
                                 vector3_t *ReturnVec) {
  vector3_t vLineDir; // Variables to hold the point and the line's direction
  fixed_t Numerator = 0, Denominator = 0, dist = 0;

  // Here comes the confusing part.  We need to find the 3D point that is
  // actually on the plane.  Here are some steps to do that:

  // 1)  First we need to get the vector of our line, Then normalize it so it's
  // a length of 1
  ...
  //    You notice we negate that distance.  We negate the distance because we
  //    want to eventually go BACKWARDS from our point to the plane. By doing
  //    this is will basically bring us back to the plane to find our
  //    intersection point.
  Numerator =
      -(FixedMul(
            vNormal->x,
            vLine[0].x) + // Use the plane equation with the normal and the line
        FixedMul(vNormal->y, vLine[0].y) +
        FixedMul(vNormal->z, vLine[0].z) + distance);

  // 3) If we take the dot product between our line vector and the normal of the
  // polygon,
  //    this will give us the cosine of the angle between the 2 (since they are
  //    both normalized - length 1). We will then divide our Numerator by this
  //    value to find the offset towards the plane from our arbitrary point.
  Denominator =
      FV3_Dot(vNormal, &vLineDir); // Get the dot product of the line's vector
                                   // and the normal of the plane

  // Since we are using division, we need to make sure we don't get a divide by
  // zero error If we do get a 0, that means that there are INFINITE points
  // because the the line is on the plane (the normal is perpendicular to the
  // line - (Normal.Vector = 0)). In this case, we should just return any point
  // on the line.
  ...
  // We need to then times this distance (dist) by our line's vector
  // (direction).  When you times a scalar (single number) by a vector you move
  // along that vector.  That is what we are doing.  We are moving from our
  // arbitrary point we chose from the line BACK to the plane along the lines
  // vector.  It seems logical to just get the numerator, which is the distance
  // from the point to the line, and then just move back that much along the
  // line's vector. Well, the distance from the plane means the SHORTEST
  // distance.  What about in the case that the line is almost parallel with the
  // polygon, but doesn't actually intersect it until half way down the line's
  // length.  The distance from the plane is short, but the distance from the
  // actual intersection point is pretty long.  If we divide the distance by the
  // dot product of our line vector and the normal of the plane, we get the
  // correct length.  Cool huh?

  dist = FixedDiv(
      Numerator,
      Denominator); // Divide to get the multiplying (percentage) factor

  // Now, like we said above, we times the dist by the vector, then add our
  // arbitrary point. This essentially moves the point along the vector to a
  // certain distance.  This now gives us the intersection point.  Yay!
...
//
UINT8
FV3_PointOnLineSide(const vector3_t *point, const vector3_t *line) {
...
//
void FM_CreateObjectMatrix(matrix_t *matrix, fixed_t x, fixed_t y, fixed_t z,
                           fixed_t anglex, fixed_t angley, fixed_t anglez,
                           fixed_t upx, fixed_t upy, fixed_t upz,
                           fixed_t radius) {
  vector3_t upcross;
  vector3_t upvec;
  vector3_t basevec;
...
//
void FM_MultMatrixVec3(const matrix_t *matrix, const vector3_t *vec,
                       vector3_t *out) {
#define M(row, col) matrix->m[col * 4 + row]
  out->x = FixedMul(vec->x, M(0, 0)) + FixedMul(vec->y, M(0, 1)) +
           FixedMul(vec->z, M(0, 2)) + M(0, 3);

  out->y = FixedMul(vec->x, M(1, 0)) + FixedMul(vec->y, M(1, 1)) +
           FixedMul(vec->z, M(1, 2)) + M(1, 3);

  out->z = FixedMul(vec->x, M(2, 0)) + FixedMul(vec->y, M(2, 1)) +
           FixedMul(vec->z, M(2, 2)) + M(2, 3);
#undef M
}
...
#define R(row, col) result.m[col * 4 + row]
  ...
  for (i = 0; i < 4; i++) {
    for (j = 0; j < 4; j++)
      R(i, j) = FixedMul(D(i, 0), M(0, j)) + FixedMul(D(i, 1), M(1, j)) +
                FixedMul(D(i, 2), M(2, j)) + FixedMul(D(i, 3), M(3, j));
  }
