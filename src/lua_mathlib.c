...
static int lib_finesine(lua_State *L) {
  lua_pushfixed(
      L, FINESINE((luaL_checkangle(L, 1) >> ANGLETOFINESHIFT) & FINEMASK));
  return 1;
}

static int lib_finecosine(lua_State *L) {
  lua_pushfixed(
      L, FINECOSINE((luaL_checkangle(L, 1) >> ANGLETOFINESHIFT) & FINEMASK));
  return 1;
}
...
static int lib_finetangent(lua_State *L) {
  ...
  if (newtan) {
    ...
    // use & 4095 instead of & FINEMASK (8191), so it doesn't go out of the
    // array's bounds
    lua_pushfixed(L, FINETANGENT(((luaL_checkangle(L, 1) + ANGLE_90) >>
                                  ANGLETOFINESHIFT) &
                                 4095));
  } else {
    LUA_Deprecated(L, "tan(angle)", "tan(angle, true)");
    lua_pushfixed(
        L, FINETANGENT((luaL_checkangle(L, 1) >> ANGLETOFINESHIFT) & 4095));
  }
  return 1;
}
...
static int lib_getsecspecial(lua_State *L) {
  lua_pushinteger(
      L, GETSECSPECIAL(luaL_checkinteger(L, 1), luaL_checkinteger(L, 2)));
  return 1;
}
...
// without needing *2 or +1 Returns both color and frame numbers!
static int lib_coloropposite(lua_State *L) {
  UINT8 colornum = (UINT8)luaL_checkinteger(L, 1);
  if (colornum >= MAXSKINCOLORS)
    return luaL_error(L, "skincolor %d out of range (0 - %d).", colornum,
                      MAXSKINCOLORS - 1);
...
static luaL_Reg lib[] = {{"abs", lib_abs},
                         {"min", lib_min},
                         {"max", lib_max},
                         {"sin", lib_finesine},
                         {"cos", lib_finecosine},
