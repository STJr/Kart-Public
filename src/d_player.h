...
typedef enum {
  ...
  /*** TAG STUFF ***/
  PF_TAGGED = 1 << 27, // Player has been tagged and awaits the next round in
                       // hide and seek.
  PF_TAGIT = 1 << 28,  // The player is it! For Tag Mode

  /*** misc ***/
  PF_FORCESTRAFE =
      1 << 29,             // Turning inputs are translated into strafing inputs
...
typedef enum {
  ...
  SH_THUNDERCOIN,
  SH_FLAMEAURA,
  // Pity shield: the world's most basic shield ever, given to players who suck
  // at Match
  SH_PITY,
...
typedef enum {
  ...
  k_positiondelay, // Used for position number, so it can grow when
  ...
  k_starpostwp,   // Temporarily stores player waypoint for... some reason. Used
                  // when respawning and finishing.
  ...
  k_throwdir, // Held dir of controls; 1 = forward, 0 = none, -1 = backward (was
              // "player->heldDir")
  k_lapanimation, // Used to show the lap start wing logo animation
  k_laphand, // Lap hand gfx to use; 0 = none, 1 = :ok_hand:, 2 = :thumbs_up:, 3
             // = :thumps_down:
  k_cardanimation, // Used to determine the position of some full-screen Battle
                   // Mode graphics
  ...
  k_floorboost,  // Prevents Sneaker sounds for a breif duration when triggered
                 // by a floor panel
  k_spinouttype, // Determines whether to thrust forward or not while spinning
                 // out; 0 = move forwards, 1 = stay still
  ...
  k_offroad, // In Super Mario Kart, going offroad has lee-way of about 1 second
             // before you start losing speed
  k_pogospring, // Pogo spring bounce effect
  k_brakestop,  // Wait until you've made a complete stop for a few tics before
                // letting brake go in reverse.
  k_waterskip,  // Water skipping counter
  k_dashpadcooldown, // Separate the vanilla SA-style dash pads from using
                     // pw_flashing
  ...
  k_boostangle,      // angle set when not spun out OR boosted to determine what
                // direction you should keep going at if you're spun out and
                // boosted.
  k_boostcam,      // Camera push forward on boost
  k_destboostcam,  // Ditto
  k_timeovercam,   // Camera timer for leaving behind or not
  k_aizdriftstrat, // Let go of your drift while boosting? Helper for the SICK
                   // STRATZ you have just unlocked
  k_brakedrift,    // Helper for brake-drift spark spawning

  k_itemroulette, // Used for the roulette when deciding what item to give you
                  // (was "pw_kartitem")
  k_roulettetype, // Used for the roulette, for deciding type (currently only
                  // used for Battle, to give you better items from Karma items)
  ...
  // k_thunderanim,			// Duration of Thunder Shield's use
  // animation
  ...
  k_eggmanheld, // Eggman monitor held, separate from k_itemheld so it doesn't
                // stop you from getting items
  k_eggmanexplode,  // Fake item recieved, explode in a few seconds
  k_eggmanblame,    // Fake item recieved, who set this fake
  k_lastjawztarget, // Last person you target with jawz, for playing the target
                    // switch sfx
  k_bananadrag,   // After a second of holding a banana behind you, you start to
                  // slow down
  k_spinouttimer, // Spin-out from a banana peel or oil slick (was
                  // "pw_bananacam")
  ...
  k_comebackpoints, // Number of times you've bombed or gave an item to someone;
                    // once it's 3 it gets set back to 0 and you're given a
                    // bumper
  k_comebackmode,   // 0 = bomb, 1 = item
  k_wanted, // Timer for determining WANTED status, lowers when hitting people,
            // prevents the game turning into Camp Lazlo
  k_yougotem, // "You Got Em" gfx when hitting someone as a karma player via a
              // method that gets you back in the game instantly

  // v1.0.2+ vars
  k_itemblink, // Item flashing after roulette, prevents Hyudoro stealing AND
               // serves as a mashing indicator
  k_itemblinkmode, // Type of flashing: 0 = white (normal), 1 = red (mashing), 2
                   // = rainbow (enhanced items)
  k_getsparks,     // Disable drift sparks at low speed, JUST enough to give
                   // acceleration the actual headstart above speed
...
typedef struct player_s {
  ...
  angle_t frameangle; // for the player add the ability to have the sprite only
                      // face other angles
  angle_t old_frameangle, old_frameangle2;
  INT16 lturn_max[MAXPREDICTTICS]; // What's the expected turn value for
                                   // full-left for a number of frames back (to
                                   // account for netgame latency)?
  ...
  angle_t starpostangle; // Angle that the starpost is facing - you respawn
                         // facing this way
  ...
  UINT8
  textvar; // which line of NiGHTS text to show -- let's not use cheap hacks
