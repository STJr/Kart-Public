...
void R_DrawColumn_8(void) {
  ...
  frac =
      (dc_texturemid + FixedMul((dc_yl << FRACBITS) - centeryfrac, fracstep)) *
      (!dc_hires);
...
void R_DrawTranslucentColumn_8(void) {
  ...
  // This is as fast as it gets.
  {
    ...
    if (dc_texheight & heightmask) {
      ...
      {
        *dest = *(transmap +
                  (colormap[source[(frac >> FRACBITS) & heightmask]] << 8) +
                  (*dest));
        dest += vid.width;
        frac += fracstep;
        *dest = *(transmap +
...
/**	\brief The R_DrawTranslatedTranslucentColumn_8 function
        Spiffy function. Not only does it colormap a sprite, but does
   translucency as well. Uber-kudos to Cyan Helkaraxe
*/
    ...
    if (dc_texheight & heightmask) {
      ...
      while ((count -= 2) >= 0) // texture height is a power of 2
      {
        *dest =
            *(dc_transmap +
              (dc_colormap
                   [dc_translation[dc_source[(frac >> FRACBITS) & heightmask]]]
               << 8) +
              (*dest));
        dest += vid.width;
        frac += fracstep;
...
void R_DrawSpan_8(void) {
  ...
  while (count >= 8) {
    ...
    dest[0] = colormap[source[((yposition >> nflatyshift) & nflatmask) |
                              (xposition >> nflatxshift)]];
    xposition += xstep;
    yposition += ystep;

    dest[1] = colormap[source[((yposition >> nflatyshift) & nflatmask) |
                              (xposition >> nflatxshift)]];
    xposition += xstep;
    yposition += ystep;

    dest[2] = colormap[source[((yposition >> nflatyshift) & nflatmask) |
                              (xposition >> nflatxshift)]];
    xposition += xstep;
    yposition += ystep;

    dest[3] = colormap[source[((yposition >> nflatyshift) & nflatmask) |
                              (xposition >> nflatxshift)]];
    xposition += xstep;
    yposition += ystep;

    dest[4] = colormap[source[((yposition >> nflatyshift) & nflatmask) |
                              (xposition >> nflatxshift)]];
    xposition += xstep;
    yposition += ystep;

    dest[5] = colormap[source[((yposition >> nflatyshift) & nflatmask) |
                              (xposition >> nflatxshift)]];
    xposition += xstep;
    yposition += ystep;

    dest[6] = colormap[source[((yposition >> nflatyshift) & nflatmask) |
                              (xposition >> nflatxshift)]];
    xposition += xstep;
    yposition += ystep;

    dest[7] = colormap[source[((yposition >> nflatyshift) & nflatmask) |
                              (xposition >> nflatxshift)]];
    xposition += xstep;
    yposition += ystep;
  ...
  while (count-- && dest <= deststop) {
    *dest++ = colormap[source[((yposition >> nflatyshift) & nflatmask) |
                              (xposition >> nflatxshift)]];
    xposition += xstep;
    yposition += ystep;
  }
...
void R_CalcTiltedLighting(fixed_t start, fixed_t end) {
  // ZDoom uses a different lighting setup to us, and I couldn't figure out how
  // to adapt their version of this function. Here's my own.
  INT32 left = ds_x1, right = ds_x2;
  fixed_t step = (end - start) / (ds_x2 - ds_x1 + 1);
  INT32 i;

  // I wanna do some optimizing by checking for out-of-range segments on either
  // side to fill in all at once, but I'm too bad at coding to not crash the
  // game trying to do that. I guess this is fast enough for now...
...
void R_DrawTiltedSpan_8(void) {
  ...
  {
    float planelightfloat = BASEVIDWIDTH * BASEVIDWIDTH / vid.width /
                            (zeroheight - FIXED_TO_FLOAT(viewz)) / 21.0f;
...
#define INVSPAN 0.0625f
  ...
  while (width >= SPANSIZE) {
    ...
    for (i = SPANSIZE - 1; i >= 0; i--) {
      colormap = planezlight[tiltlighting[ds_x1++]] + (ds_colormap - colormaps);
      *dest = colormap[source[((v >> nflatyshift) & nflatmask) |
                              (u >> nflatxshift)]];
      dest++;
      u += stepu;
      v += stepv;
  ...
  if (width > 0) {
    if (width == 1) {
      ...
      *dest = colormap[source[((v >> nflatyshift) & nflatmask) |
      ...
      for (; width != 0; width--) {
        colormap =
            planezlight[tiltlighting[ds_x1++]] + (ds_colormap - colormaps);
        *dest = colormap[source[((v >> nflatyshift) & nflatmask) |
                                (u >> nflatxshift)]];
        dest++;
        u += stepu;
...
#define INVSPAN 0.0625f
  ...
  while (width >= SPANSIZE) {
    ...
    for (i = SPANSIZE - 1; i >= 0; i--) {
      colormap = planezlight[tiltlighting[ds_x1++]] + (ds_colormap - colormaps);
      *dest = *(ds_transmap +
                (colormap[source[((v >> nflatyshift) & nflatmask) |
                                 (u >> nflatxshift)]]
                 << 8) +
                *dest);
      dest++;
...
void R_DrawTranslucentSplat_8(void) {
  ...
  while (count >= 8) {
    ...
    // because we don't have the uber complicated math to calculate it now, so
    // that was a memory write we didn't need!
    val = source[((yposition >> nflatyshift) & nflatmask) |
                 (xposition >> nflatxshift)];
    if (val != TRANSPARENTPIXEL)
...
void R_DrawTranslucentSpan_8(void) {
  ...
  while (count >= 8) {
    ...
    // that was a memory write we didn't need!
    dest[0] = *(ds_transmap +
                (colormap[source[((yposition >> nflatyshift) & nflatmask) |
                                 (xposition >> nflatxshift)]]
                 << 8) +
                dest[0]);
    xposition += xstep;
...
void R_DrawColumnShadowed_8(void) {
  ...
  // This runs through the lightlist from top to bottom and cuts up the column
  // accordingly.
