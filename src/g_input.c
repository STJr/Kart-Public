...
static CV_PossibleValue_t mousesens_cons_t[] = {
    {1, "MIN"}, {MAXMOUSESENSITIVITY, "MAX"}, {0, NULL}};
static CV_PossibleValue_t onecontrolperkey_cons_t[] = {
    {1, "One"}, {2, "Several"}, {0, NULL}};

// mouse values are used once
consvar_t cv_mousesens = {"mousesens", "20", CV_SAVE, mousesens_cons_t,
                          NULL,        0,    NULL,    NULL,
                          0,           0,    NULL};
consvar_t cv_mousesens2 = {
    "mousesens2", "20", CV_SAVE, mousesens_cons_t, NULL, 0, NULL, NULL, 0, 0,
    NULL};
consvar_t cv_mouseysens = {
    "mouseysens", "20", CV_SAVE, mousesens_cons_t, NULL, 0, NULL, NULL, 0, 0,
    NULL};
consvar_t cv_mouseysens2 = {
    "mouseysens2", "20", CV_SAVE, mousesens_cons_t, NULL, 0, NULL, NULL, 0, 0,
    NULL};
consvar_t cv_controlperkey = {"controlperkey",
                              "One",
                              CV_SAVE,
                              onecontrolperkey_cons_t,
                              NULL,
                              0,
                              NULL,
                              NULL,
...
INT32 joyxmove[JOYAXISSET], joyymove[JOYAXISSET], joy2xmove[JOYAXISSET],
    joy2ymove[JOYAXISSET], joy3xmove[JOYAXISSET], joy3ymove[JOYAXISSET],
    joy4xmove[JOYAXISSET], joy4ymove[JOYAXISSET];
...
void G_MapEventsToControls(event_t *ev) {
  ...
  case ev_mouse: // buttons are virtual keys
    ...
    mousex =
        (INT32)(ev->data2 *
                ((cv_mousesens.value * cv_mousesens.value) / 110.0f + 0.1f));
    mousey =
        (INT32)(ev->data3 *
                ((cv_mousesens.value * cv_mousesens.value) / 110.0f + 0.1f));
    mlooky =
        (INT32)(ev->data3 *
                ((cv_mouseysens.value * cv_mousesens.value) / 110.0f + 0.1f));
    break;
  ...
  case ev_joystick3:
    ...
    if (i >= JOYAXISSET)
      break;
    if (ev->data2 != INT32_MAX)
      joy3xmove[i] = ev->data2;
    if (ev->data3 != INT32_MAX)
      joy3ymove[i] = ev->data3;
    break;
  ...
  case ev_joystick4:
    ...
    if (i >= JOYAXISSET)
      break;
    if (ev->data2 != INT32_MAX)
      joy4xmove[i] = ev->data2;
    if (ev->data3 != INT32_MAX)
      joy4ymove[i] = ev->data3;
    break;
  ...
  case ev_mouse2: // buttons are virtual keys
    ...
    mouse2x =
        (INT32)(ev->data2 *
                ((cv_mousesens2.value * cv_mousesens2.value) / 110.0f + 0.1f));
    mouse2y =
        (INT32)(ev->data3 *
                ((cv_mousesens2.value * cv_mousesens2.value) / 110.0f + 0.1f));
    mlook2y =
        (INT32)(ev->data3 *
                ((cv_mouseysens2.value * cv_mousesens2.value) / 110.0f + 0.1f));
    break;
...
static keyname_t keynames[] = {
    ...
    {KEY_MOUSE1 + 7, "MOUSE8"},
    {KEY_2MOUSE1 + 0,
     "SEC_MOUSE2"}, // BP: sorry my mouse handler swap button 1 and 2
...
#define NOMOREJOYBTN_1S
    ...
    {KEY_DBL2MOUSE1 + 0,
     "DBLSEC_MOUSE2"}, // BP: sorry my mouse handler swap button 1 and 2
...
static const char *gamecontrolname[num_gamecontrols] = {
    ...
    "aimforward", "aimbackward", "turnleft",  "turnright", "accelerate",
    "drift",      "brake",       "fire",      "lookback",  "camreset",
    "camtoggle",  "spectate",    "lookup",    "lookdown",  "centerview",
    "talkkey",    "teamtalkkey", "scores",    "console",   "pause",
    "systemmenu", "screenshot",  "recordgif", "viewpoint", "custom1",
    "custom2",    "custom3",
};
...
INT32
G_KeyStringtoNum(const char *keystr) {
  UINT32 j;
...
void G_Controldefault(UINT8 player) {
  if (player == 0 || player == 1) {
    ...
    // gamecontrol[gc_screenshot ][1] = KEY_HAT1+1; // D-Pad Down // absolutely
    // fucking NOT
...
INT32
G_CheckDoubleUsage(INT32 keynum, boolean modify) {
...
static INT32 G_FilterKeyByVersion(INT32 numctrl, INT32 keyidx, INT32 player,
                                  INT32 *keynum1, INT32 *keynum2,
                                  boolean *nestedoverride) {
#if 1 // SRB2Kart filters/migrations
  (void)nestedoverride;

  // Migration: 1.6 (majorexec 10) Joystick Defaults changed to use SDL Game
  // Controllers
  if (GETMAJOREXECVERSION(cv_execversion.value) < 10) {
    ...
    if (keyidx == 1 && numctrl == gc_fire &&
        *keynum2 == joybuttonbase + 4) // Xbox DInput LB
    {
      *keynum2 = joybuttonbase + 9; // SDL LEFTSHOULDER
    }
    if (keyidx == 1 && numctrl == gc_drift &&
        *keynum2 == joybuttonbase + 5) // Xbox DInput RB
    {
      *keynum2 = joybuttonbase + 10; // SDL RIGHTSHOULDER
    }

    // Pause and Systemmenu are only bound for P1
    if (keyidx == 1 && player == 0 && numctrl == gc_pause &&
        *keynum2 == joybuttonbase + 6) // Xbox DInput Back
    {
      *keynum2 = joybuttonbase + 4; // SDL BACK
    }
    if (keyidx == 0 && player == 0 && numctrl == gc_systemmenu &&
        *keynum1 == joybuttonbase + 7) // Xbox DInput Start
    {
    ...
    !defined(WMINPUT) && !defined(_WII)
  if (GETMAJOREXECVERSION(cv_execversion.value) < 27 &&
      ( // v2.1.22
          numctrl == gc_weaponnext || numctrl == gc_weaponprev ||
          numctrl == gc_tossflag || numctrl == gc_use ||
          numctrl == gc_camreset || numctrl == gc_jump || numctrl == gc_pause ||
          numctrl == gc_systemmenu || numctrl == gc_camtoggle ||
          numctrl == gc_screenshot || numctrl == gc_talkkey ||
          numctrl == gc_scores || numctrl == gc_centerview)) {
    ...
    if (player == 0 && numctrl == gc_systemmenu)
      ...
      defaultkey =
          (player == 1 ? gamecontrolbis[numctrl][0] : gamecontrol[numctrl][1]);
      ...
      if (*keynum2) // push keynum2 down; this is an edge case
      {
        ...
        *keynum2 = 0;
        ...
        defaultoverride = true;
               ...
               (!*keynum2 ||
                (!*keynum1 &&
                 *keynum2))) // last one is the same edge case as above
    {
      keynum = defaultkey;
      defaultoverride = true;
    ...
    // pass it through if it's the same numctrl. This is an edge case -- when
    // using LOADCONFIG, gamecontrol is not reset with default.
    //
    ...
    if (keynum && (!existingctrl || existingctrl == numctrl))
      ...
      *keynum2 = 0;
      return G_FilterKeyByVersion(numctrl, keyidx, player, keynum1, keynum2,
                                  nestedoverride);
      // recursion *should* be safe because we only assign keynum2 to a joy
      // default and then clear it if we find that keynum1 already has the joy
      // default.
    } else
      return 0;
  }
...
void Command_Setcontrol_f(void) {
  ...
  if (na != 3 && na != 4) {
    CONS_Printf(M_GetText("setcontrol <controlname> <keyname> [<2nd keyname>]: "
...
void Command_Setcontrol2_f(void) {
  ...
  if (na != 3 && na != 4) {
    CONS_Printf(M_GetText("setcontrol2 <controlname> <keyname> [<2nd "
                          "keyname>]: set controls for player 2\n"));
    return;
  }
...
void Command_Setcontrol3_f(void) {
  ...
  if (na != 3 && na != 4) {
    CONS_Printf(M_GetText("setcontrol3 <controlname> <keyname> [<2nd "
                          "keyname>]: set controls for player 3\n"));
    return;
  }
...
void Command_Setcontrol4_f(void) {
  ...
  if (na != 3 && na != 4) {
    CONS_Printf(M_GetText("setcontrol4 <controlname> <keyname> [<2nd "
                          "keyname>]: set controls for player 4\n"));
    return;
  }
