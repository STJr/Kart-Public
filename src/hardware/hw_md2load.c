...
typedef struct {
  ...
  int offsetSkins; // Offset, in bytes from the start of the file, to the list
                   // of skin names.
  int offsetST;   // Offset, in bytes from the start of the file, to the list of
                  // texture coordinates
  int offsetTris; // Offset, in bytes from the start of the file, to the list of
                  // triangles
  int offsetFrames; // Offset, in bytes from the start of the file, to the list
                    // of frames
  int offsetGLcmds; // Offset, in bytes from the start of the file, to the list
                    // of gl commands
  int offsetEnd; // Offset, in bytes from the start of the file, to the end of
                 // the file (filesize)
} md2header_t;

typedef struct {
  unsigned short
      meshIndex[3]; // indices into the array of vertices in each frames
...
typedef struct {
  unsigned char v[3]; // Scaled vertices. You'll need to multiply them with
                      // scale[x] to make them normal.
...
model_t *MD2_LoadModel(const char *fileName, int ztag, boolean useFloat) {
  ...
  //
  // <SSNTails>
  // the UV coordinates in MD2 are not compatible with glDrawElements like MD3
  // is. So they need to be loaded as full float.
  //
  // MD2 is intended to be draw in triangle strips and fans
  // not very compatible with a modern GL implementation, either
  // so the idea would be to full float expand it, and put it in a vertex buffer
  // object I'm sure there's a way to convert the UVs to 'tinyframes', but maybe
  // that's a job for someone else. You'd have to decompress the model, then
  // recompress, reindexing the triangles and weeding out duplicate coordinates
  ...
  if (fread(buffer, fileLen, 1, f)) {
  } // squash ignored fread error
  fclose(f);
  ...
  retModel->meshes =
      (mesh_t *)Z_Calloc(sizeof(mesh_t) * retModel->numMeshes, ztag, 0);
  ...
  retModel->materials = (material_t *)Z_Calloc(
      sizeof(material_t) * retModel->numMaterials, ztag, 0);
  ...
  for (t = 0; t < retModel->numMaterials; t++) {
    ...
    /*		retModel->materials[t].texture =
       Texture::ReadTexture((char*)texturefilename, ZT_TEXTURE);
                    ...
                    if (!systemSucks)
                    {
                            ...
                            *ptr++ = '\0';
                            ...
                            sprintf(openfilename, "%s/%s", "textures",
       normalMapName);
       ...
       Resource::Open(openfilename); if (res)
                            {
                                    Resource::Close(res);
                                    retModel->materials[t].lightmap =
       Texture::ReadTexture(normalMapName, ZT_TEXTURE);
                            }
                    }*/
  }
  ...
  if (!useFloat) // Decompress to MD3 'tinyframe' space
  {
    ...
    retModel->meshes[0].tinyframes = (tinyframe_t *)Z_Calloc(
        sizeof(tinyframe_t) * header->numFrames, ztag, 0);
    retModel->meshes[0].numVertices = header->numXYZ;
    retModel->meshes[0].uvs = (float *)Z_Malloc(
        sizeof(float) * 2 * retModel->meshes[0].numVertices, ztag, 0);
    ...
    for (i = 0; i < header->numFrames; i++, ptr += header->framesize) {
      ...
      retModel->meshes[0].tinyframes[i].vertices =
          (short *)Z_Malloc(sizeof(short) * 3 * header->numXYZ, ztag, 0);
      retModel->meshes[0].tinyframes[i].normals =
          (char *)Z_Malloc(sizeof(char) * 3 * header->numXYZ, ztag, 0);

      //			if (retModel->materials[0].lightmap)
      //				retModel->meshes[0].tinyframes[i].tangents
      //=
      //(char*)malloc(sizeof(char));//(char*)Z_Malloc(sizeof(char)*3*header->numVerts,
      // ztag);
      retModel->meshes[0].indices = (unsigned short *)Z_Malloc(
          sizeof(unsigned short) * 3 * header->numTris, ztag, 0);
      ...
      for (j = 0; j < header->numXYZ; j++, vertex++) {
        *vertptr = (short)(((vertex->v[0] * framePtr->scale[0]) +
                            framePtr->translate[0]) /
                           dataScale);
        vertptr++;
        *vertptr = (short)(((vertex->v[2] * framePtr->scale[2]) +
                            framePtr->translate[2]) /
                           dataScale);
        vertptr++;
        *vertptr = -1.0f * (short)(((vertex->v[1] * framePtr->scale[1]) +
                                    framePtr->translate[1]) /
                                   dataScale);
        vertptr++;
    ...
    for (j = 0; j < header->numTris; j++, trisPtr++) {
      ...
      *indexptr = trisPtr->meshIndex[2];
      ...
      uvptr[trisPtr->meshIndex[0] * 2] =
          texcoords[trisPtr->stIndex[0]].s / (float)header->skinwidth;
      uvptr[trisPtr->meshIndex[0] * 2 + 1] =
          (texcoords[trisPtr->stIndex[0]].t / (float)header->skinheight);
      uvptr[trisPtr->meshIndex[1] * 2] =
          texcoords[trisPtr->stIndex[1]].s / (float)header->skinwidth;
      uvptr[trisPtr->meshIndex[1] * 2 + 1] =
          (texcoords[trisPtr->stIndex[1]].t / (float)header->skinheight);
      uvptr[trisPtr->meshIndex[2] * 2] =
          texcoords[trisPtr->stIndex[2]].s / (float)header->skinwidth;
      uvptr[trisPtr->meshIndex[2] * 2 + 1] =
          (texcoords[trisPtr->stIndex[2]].t / (float)header->skinheight);
    }
  } else // Full float loading method
  {
    ...
    retModel->meshes[0].frames =
        (mdlframe_t *)Z_Calloc(sizeof(mdlframe_t) * header->numFrames, ztag, 0);
    retModel->meshes[0].uvs = (float *)Z_Malloc(
        sizeof(float) * 2 * retModel->meshes[0].numVertices, ztag, 0);
    ...
    for (i = 0; i < header->numFrames; i++, ptr += header->framesize) {
      ...
      md2frame_t *framePtr = (md2frame_t *)ptr;
      retModel->meshes[0].frames[i].normals =
          (float *)Z_Malloc(sizeof(float) * 3 * header->numTris * 3, ztag, 0);
      retModel->meshes[0].frames[i].vertices =
          (float *)Z_Malloc(sizeof(float) * 3 * header->numTris * 3, ztag, 0);
      //			if (retModel->materials[0].lightmap)
      //				retModel->meshes[0].frames[i].tangents =
      //(float*)malloc(sizeof(float));//(float*)Z_Malloc(sizeof(float)*3*header->numTris*3,
      // ztag); float *vertptr, *normptr;
      ...
      framePtr--;
      for (j = 0; j < header->numTris; j++, trisPtr++) {
        *vertptr = ((vertex[trisPtr->meshIndex[0]].v[0] * framePtr->scale[0]) +
                    framePtr->translate[0]) *
                   WUNITS;
        vertptr++;
        *vertptr = ((vertex[trisPtr->meshIndex[0]].v[2] * framePtr->scale[2]) +
                    framePtr->translate[2]) *
                   WUNITS;
        vertptr++;
        *vertptr = -1.0f *
                   ((vertex[trisPtr->meshIndex[0]].v[1] * framePtr->scale[1]) +
                    framePtr->translate[1]) *
                   WUNITS;
        vertptr++;

        *vertptr = ((vertex[trisPtr->meshIndex[1]].v[0] * framePtr->scale[0]) +
                    framePtr->translate[0]) *
                   WUNITS;
        vertptr++;
        *vertptr = ((vertex[trisPtr->meshIndex[1]].v[2] * framePtr->scale[2]) +
                    framePtr->translate[2]) *
                   WUNITS;
        vertptr++;
        *vertptr = -1.0f *
                   ((vertex[trisPtr->meshIndex[1]].v[1] * framePtr->scale[1]) +
                    framePtr->translate[1]) *
                   WUNITS;
        vertptr++;

        *vertptr = ((vertex[trisPtr->meshIndex[2]].v[0] * framePtr->scale[0]) +
                    framePtr->translate[0]) *
                   WUNITS;
        vertptr++;
        *vertptr = ((vertex[trisPtr->meshIndex[2]].v[2] * framePtr->scale[2]) +
                    framePtr->translate[2]) *
                   WUNITS;
        vertptr++;
        *vertptr = -1.0f *
                   ((vertex[trisPtr->meshIndex[2]].v[1] * framePtr->scale[1]) +
                    framePtr->translate[1]) *
                   WUNITS;
        vertptr++;

        *normptr++ =
            avertexnormals[vertex[trisPtr->meshIndex[0]].lightNormalIndex][0];
        *normptr++ =
            avertexnormals[vertex[trisPtr->meshIndex[0]].lightNormalIndex][2];
        *normptr++ =
            avertexnormals[vertex[trisPtr->meshIndex[0]].lightNormalIndex][1];

        *normptr++ =
            avertexnormals[vertex[trisPtr->meshIndex[1]].lightNormalIndex][0];
        *normptr++ =
            avertexnormals[vertex[trisPtr->meshIndex[1]].lightNormalIndex][2];
        *normptr++ =
            avertexnormals[vertex[trisPtr->meshIndex[1]].lightNormalIndex][1];

        *normptr++ =
            avertexnormals[vertex[trisPtr->meshIndex[2]].lightNormalIndex][0];
        *normptr++ =
            avertexnormals[vertex[trisPtr->meshIndex[2]].lightNormalIndex][2];
        *normptr++ =
            avertexnormals[vertex[trisPtr->meshIndex[2]].lightNormalIndex][1];
      }
    }
  }
