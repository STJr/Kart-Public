...
#define NZCLIP_PLANE                                                           \
  0.9f // Seems to be only used for the HUD and screen textures
...
enum EPolyFlags {
  ...
  PF_Masked = 0x00000001, // Poly is alpha scaled and 0 alpha pels are discarded
                          // (holes in texture)
  PF_Translucent =
      0x00000002, // Poly is transparent, alpha = level of transparency
  ...
  PF_Blending = (PF_Environment | PF_Additive | PF_Translucent | PF_Masked |
                 PF_Substractive | PF_Fog) &
                ~PF_NoAlphaTest,
  ...
  PF_Modulated = 0x00001000, // Modulation (multiply output with constant ARGB)
                             // When set, pass the color constant into the
                             // FSurfaceInfo -> FlatColor
