...
static ALboolean reallocate_stack(void) {
  ...
  if (ALstack)
    new_stack = realloc(ALstack, sizeof(stack_snd_t) *
  ...
  if (new_stack) {
    ...
    for (clean_stack = allocated_sounds; clean_stack < allocated_sounds;
         clean_stack++)
...
static ALvoid kill_sound(stack_snd_t *snd) {
  ...
  if (ALo_GetError() != AL_NO_ERROR) {
    DBG_Printf("%s: alDeleteSources, %s", __FUNCTION__,
...
static ALvoid ALSetPan(ALuint sid, INT32 sep) {
  ...
  if (ALo_GetError() != AL_NO_ERROR) {
    DBG_Printf("%s: alSourcefv, %s", __FUNCTION__,
               GetALErrorString(ALo_Lasterror));
  }
}
...
EXPORT INT32 HWRAPI(Startup)(I_Error_t FatalErrorFunction, snddev_t *snd_dev) {
  ...
  ALCint AlSetup[8] = {ALC_FREQUENCY, 22050,    ALC_REFRESH, 35,
                       ALC_SYNC,      AL_FALSE, ALC_INVALID, ALC_INVALID};
...
EXPORT INT32 HWRAPI(AddSource)(source3D_data_t *src, u_int sfxhandle) {
  ALsizei chan =
      makechan(sfxhandle, (ALboolean)(src ? src->permanent : AL_FALSE));
  ...
  if (src) // 3D
  {
    ...
    if (ALo_GetError() != AL_NO_ERROR) {
      DBG_Printf("%s: AL_SOURCE_RELATIVE, %s", __FUNCTION__,
                 GetALErrorString(ALo_Lasterror));
    }

    alSource3f(sid, AL_POSITION, src->pos.x, src->pos.z, src->pos.y);
    if (ALo_GetError() != AL_NO_ERROR) {
      DBG_Printf("%s: AL_POSITION, %s", __FUNCTION__,
                 GetALErrorString(ALo_Lasterror));
    }
    alSource3f(sid, AL_VELOCITY, src->pos.momx, src->pos.momz, src->pos.momy);
    if (ALo_GetError() != AL_NO_ERROR) {
      DBG_Printf("%s: AL_VELOCITY, %s", __FUNCTION__,
                 GetALErrorString(ALo_Lasterror));
    }
  } else // 2D
  {
  ...
  if (ALo_GetError() != AL_NO_ERROR) {
    DBG_Printf("%s: AL_ROLLOFF_FACTOR, %s", __FUNCTION__,
               GetALErrorString(ALo_Lasterror));
  }

  alSourcef(sid, AL_REFERENCE_DISTANCE, 0.0f);
  if (ALo_GetError() != AL_NO_ERROR) {
    DBG_Printf("%s: AL_REFERENCE_DISTANC, %s", __FUNCTION__,
               GetALErrorString(ALo_Lasterror));
  }
  return chan;
}
...
EXPORT INT32 HWRAPI(StartSource)(INT32 chan) {
  ...
  alSourcePlay(sid);
  if (ALo_GetError() != AL_NO_ERROR) {
    DBG_Printf("%s: alSourcePlay, %s", __FUNCTION__,
               GetALErrorString(ALo_Lasterror));
  }

  alGetSourcei(sid, AL_SOURCE_STATE, &state);
  if (ALo_GetError() != AL_NO_ERROR) {
    DBG_Printf("%s: alGetSourcei, %s", __FUNCTION__,
               GetALErrorString(ALo_Lasterror));
  }
...
EXPORT void HWRAPI(StopSource)(INT32 chan) {
  ...
  if (ALo_GetError() != AL_NO_ERROR) {
    DBG_Printf("%s: alSourceQueueBuffers, %s", __FUNCTION__,
               GetALErrorString(ALo_Lasterror));
  }
#else
  alSourceStop(sid);
  if (ALo_GetError() != AL_NO_ERROR) {
    DBG_Printf("%s: alSourceStop, %s", __FUNCTION__,
               GetALErrorString(ALo_Lasterror));
  }
#endif
}
...
EXPORT void HWRAPI(BeginFrameUpdate)(void) {
  ...
  if (ALo_GetError() != AL_NO_ERROR) {
    DBG_Printf("%s: alcSuspendContext, %s", __FUNCTION__,
               GetALErrorString(ALo_Lasterror));
  }
}
...
EXPORT void HWRAPI(EndFrameUpdate)(void) {
  ...
  if (ALo_GetError() != AL_NO_ERROR) {
    DBG_Printf("%s: alcProcessContext, %s", __FUNCTION__,
               GetALErrorString(ALo_Lasterror));
  }
}
...
EXPORT void HWRAPI(UpdateListener)(listener_data_t *data, INT32 num) {
  ...
  if (data) {
    ...
    if (data->f_angle)
      f_angle = (ALdouble)((data->f_angle) / 180.0l * M_PIl);
    ...
    alListener3f(AL_POSITION, (ALfloat)-data->x, (ALfloat)data->z,
                 (ALfloat)data->y);
    if (ALo_GetError() != AL_NO_ERROR) {
      DBG_Printf("%s: AL_POSITIO, %s", __FUNCTION__,
                 GetALErrorString(ALo_Lasterror));
    }

    alListener3f(AL_VELOCITY, (ALfloat)data->momx, (ALfloat)data->momz,
                 (ALfloat)data->momy);
    if (ALo_GetError() != AL_NO_ERROR) {
      DBG_Printf("%s: AL_VELOCITY, %s", __FUNCTION__,
                 GetALErrorString(ALo_Lasterror));
    ...
    if (ALo_GetError() != AL_NO_ERROR) {
      DBG_Printf("%s: AL_ORIENTATION, %s", __FUNCTION__,
                 GetALErrorString(ALo_Lasterror));
    }
  } else {
...
EXPORT void HWRAPI(UpdateSourceParms)(INT32 chan, INT32 vol, INT32 sep) {
  ...
  if (vol != -1) {
    ...
    if (ALo_GetError() != AL_NO_ERROR) {
      DBG_Printf("%s: alSourcef, %s", __FUNCTION__,
                 GetALErrorString(ALo_Lasterror));
    }
  }
  ...
  if (sep != -1) {
    ...
    if (ALo_GetError() != AL_NO_ERROR) {
      DBG_Printf("%s: ALSetPan, %s", __FUNCTION__,
                 GetALErrorString(ALo_Lasterror));
    }
  }
}
...
EXPORT void HWRAPI(SetGlobalSfxVolume)(INT32 vol) {
  ...
  if (ALo_GetError() != AL_NO_ERROR) {
    DBG_Printf("%s: AL_GAIN, %s", __FUNCTION__,
               GetALErrorString(ALo_Lasterror));
  }
}
...
EXPORT INT32 HWRAPI(SetCone)(INT32 chan, cone_def_t *cone_def) {
  ...
  if (ALo_GetError() != AL_NO_ERROR) {
    DBG_Printf("%s: AL_CONE_INNER_ANGLE, %s", __FUNCTION__,
               GetALErrorString(ALo_Lasterror));
  }

  alSourcef(sid, AL_CONE_OUTER_ANGLE, cone_def->outer);
  if (ALo_GetError() != AL_NO_ERROR) {
    DBG_Printf("%s: AL_CONE_OUTER_ANGL, %s", __FUNCTION__,
               GetALErrorString(ALo_Lasterror));
  }

  alSourcei(sid, AL_CONE_OUTER_GAIN, cone_def->outer_gain);
  if (ALo_GetError() != AL_NO_ERROR) {
    DBG_Printf("%s: AL_CONE_OUTER_GAIN, %s", __FUNCTION__,
               GetALErrorString(ALo_Lasterror));
  }
...
EXPORT INT32 HWRAPI(ReloadSource)(INT32 chan, u_int sfxhandle) {
  ...
  if (ALo_Lasterror != AL_NO_ERROR) {
    DBG_Printf("%s: alSourcei, %s", __FUNCTION__,
               GetALErrorString(ALo_Lasterror));
  }
...
EXPORT void HWRAPI(KillSfx)(u_int sfxhandle) {
  ...
  if (ALo_GetError() == AL_INVALID_OPERATION) {
    ...
    DBG_Printf("%s: alDeleteBuffers, %s", __FUNCTION__,
               GetALErrorString(ALo_Lasterror));
  }
}
