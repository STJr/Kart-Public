...
//-----------------------------------------------------------------------------
/// \file
/// \brief hardware renderer, using the standard HardWareRender driver DLL for
/// SRB2
...
static CV_PossibleValue_t grfiltermode_cons_t[] = {
    ...
    {0, NULL}};
CV_PossibleValue_t granisotropicmode_cons_t[] = {
    {1, "MIN"}, {16, "MAX"}, {0, NULL}};

consvar_t cv_grrounddown = {"gr_rounddown", "Off", 0, CV_OnOff, NULL, 0,
                            NULL,           NULL,  0, 0,        NULL};

consvar_t cv_grfiltermode = {"gr_filtermode",
                             "Nearest",
                             CV_CALL | CV_SAVE,
                             grfiltermode_cons_t,
                             ...
                             0,
                             NULL};
consvar_t cv_granisotropicmode = {"gr_anisotropicmode",
                                  "1",
                                  CV_CALL | CV_SAVE,
                                  ...
                                  0,
                                  NULL};
consvar_t cv_grcorrecttricks = {
    "gr_correcttricks", "Off", 0, CV_OnOff, NULL, 0, NULL, NULL, 0, 0, NULL};
consvar_t cv_grsolvetjoin = {
    "gr_solvetjoin", "On", 0, CV_OnOff, NULL, 0, NULL, NULL, 0, 0, NULL};

consvar_t cv_grbatching = {"gr_batching", "On", 0, CV_OnOff, NULL, 0,
                           NULL,          NULL, 0, 0,        NULL};
...
static void CV_anisotropic_ONChange(void) {
  HWD.pfnSetSpecialState(HWD_SET_TEXTUREANISOTROPICMODE,
                         cv_granisotropicmode.value);
}
...
float gr_viewwidth,
    gr_viewheight; // viewport clipping boundaries (screen coords)
...
void HWR_Lighting(FSurfaceInfo *Surface, INT32 light_level,
  ...
  fade_color.rgba =
      (colormap != NULL) ? (UINT32)colormap->fadergba : GL_DEFAULTFOG;
  ...
  if (!(cv_grshaders.value && gr_shadersavailable)) {
    ...
    // 8 is roughly the brightness of the "close" color in Software, and 16 the
    // brightness of the "far" color. 8 is too bright for dark levels, and 16 is
    // too dark for bright levels. 12 is the compromise value. It doesn't look
    // especially good anywhere, but it's the most balanced. (Also, as far as I
    // can tell, fade_color's alpha is actually not used in Software, so we only
    // use light level.)
...
UINT8
HWR_FogBlockAlpha(INT32 light,
                  extracolormap_t *colormap) // Let's see if this can work
{
  ...
  if (!(cv_grshaders.value && gr_shadersavailable)) {
    ...
    // at 255 brightness, alpha is between 0 and 127, at 0 brightness alpha will
    // always be 255
...
static FUINT HWR_CalcWallLight(FUINT lightnum, fixed_t v1x, fixed_t v1y,
                               fixed_t v2x, fixed_t v2y) {
  ...
  if (cv_grfakecontrast.value != 0) {
    ...
    if (cv_grfakecontrast.value == 2) // Smooth setting
    {
      extralight = (-(contrast << FRACBITS) +
                    FixedDiv(AngleFixed(R_PointToAngle2(0, 0, abs(v1x - v2x),
                                                        abs(v1y - v2y))),
                             90 << FRACBITS) *
                        (contrast * 2)) >>
                   FRACBITS;
...
void HWR_RenderPlane(extrasubsector_t *xsub, boolean isceiling,
                     fixed_t fixedheight, FBITFIELD PolyFlags, INT32 lightlevel,
                     lumpnum_t lumpnum, sector_t *FOFsector, UINT8 alpha,
  ...
  // Set fixedheight to the slope's height from our viewpoint, if we have a
  // slope
  if (slope)
  ...
  if (angle) // Only needs to be done if there's an altered angle
  {
    ...
    // rotation like software
    /*tempxsow = FLOAT_TO_FIXED(scrollx);
    tempytow = FLOAT_TO_FIXED(scrolly);
    scrollx = (FIXED_TO_FLOAT(FixedMul(tempxsow, FINECOSINE(angle)) -
    FixedMul(tempytow, FINESINE(angle)))); scrolly =
    (FIXED_TO_FLOAT(FixedMul(tempxsow, FINESINE(angle)) + FixedMul(tempytow,
    FINECOSINE(angle))));*/

    // This needs to be done so everything aligns after rotation
    // It would be done so that rotation is done, THEN the translation, but I
    // couldn't get it to rotate AND scroll like software does
    tempxsow = FLOAT_TO_FIXED(flatxref);
    tempytow = FLOAT_TO_FIXED(flatyref);
    flatxref = (FIXED_TO_FLOAT(FixedMul(tempxsow, FINECOSINE(angle)) -
                               FixedMul(tempytow, FINESINE(angle))));
    flatyref = (FIXED_TO_FLOAT(FixedMul(tempxsow, FINESINE(angle)) +
  ...
  for (i = 0; i < nrPlaneVerts; i++, v3d++, pv++) {
    ...
    if (angle) // Only needs to be done if there's an altered angle
    {
      ...
      v3d->s = (FIXED_TO_FLOAT(FixedMul(tempxsow, FINECOSINE(angle)) -
                               FixedMul(tempytow, FINESINE(angle))));
      v3d->t = (FIXED_TO_FLOAT(FixedMul(tempxsow, FINESINE(angle)) +
                               FixedMul(tempytow, FINECOSINE(angle))));
    }
    ...
    if (slope) {
      fixedheight =
          P_GetZAt(slope, FLOAT_TO_FIXED(pv->x), FLOAT_TO_FIXED(pv->y));
...
static void HWR_DrawSegsSplats(FSurfaceInfo *pSurf) {
  ...
  M_ClearBox(segbbox);
  M_AddToBox(segbbox, FLOAT_TO_FIXED(((polyvertex_t *)gr_curline->pv1)->x),
             FLOAT_TO_FIXED(((polyvertex_t *)gr_curline->pv1)->y));
  M_AddToBox(segbbox, FLOAT_TO_FIXED(((polyvertex_t *)gr_curline->pv2)->x),
...
FBITFIELD
HWR_TranstableToAlpha(INT32 transtablenum, FSurfaceInfo *pSurf) {
  switch (transtablenum) {
  case tr_trans10:
    pSurf->PolyColor.s.alpha = 0xe6;
    return PF_Translucent;
  case tr_trans20:
    pSurf->PolyColor.s.alpha = 0xcc;
    return PF_Translucent;
  case tr_trans30:
    pSurf->PolyColor.s.alpha = 0xb3;
    return PF_Translucent;
  case tr_trans40:
    pSurf->PolyColor.s.alpha = 0x99;
    return PF_Translucent;
  case tr_trans50:
    pSurf->PolyColor.s.alpha = 0x80;
    return PF_Translucent;
  case tr_trans60:
    pSurf->PolyColor.s.alpha = 0x66;
    return PF_Translucent;
  case tr_trans70:
    pSurf->PolyColor.s.alpha = 0x4c;
    return PF_Translucent;
  case tr_trans80:
    pSurf->PolyColor.s.alpha = 0x33;
    return PF_Translucent;
  case tr_trans90:
    pSurf->PolyColor.s.alpha = 0x19;
    return PF_Translucent;
  }
  return PF_Translucent;
}
...
// HWR_ProjectWall
//
void HWR_ProjectWall(FOutVector *wallVerts, FSurfaceInfo *pSurf,
                     FBITFIELD blendmode, INT32 lightlevel,
                     extracolormap_t *wallcolormap) {
  HWR_Lighting(pSurf, lightlevel, wallcolormap);

  HWD.pfnSetShader(2); // wall shader
  HWD.pfnDrawPolygon(pSurf, wallVerts, 4,
...
// HWR_SplitWall
//
void HWR_SplitWall(sector_t *sector, FOutVector *wallVerts, INT32 texnum,
                   FSurfaceInfo *Surf, INT32 cutflag, ffloor_t *pfloor) {
  ...
  for (i = 0; i < sector->numlights; i++) {
    ...
    if (!(list[i].flags & FF_NOSHADE)) {
      if (pfloor && (pfloor->flags & FF_FOG)) {
        lightnum = HWR_CalcWallLight(pfloor->master->frontsector->lightlevel,
                                     v1x, v1y, v2x, v2y);
    ...
    if ((sector->lightlist[i].flags & FF_CUTSOLIDS) && !(cutflag & FF_EXTRA))
      solid = true;
    else if ((sector->lightlist[i].flags & FF_CUTEXTRA) &&
             (cutflag & FF_EXTRA)) {
      if (sector->lightlist[i].flags & FF_EXTRA) {
        if ((sector->lightlist[i].flags & (FF_FOG | FF_SWIMMABLE)) ==
            (cutflag &
             (FF_FOG | FF_SWIMMABLE))) // Only merge with your own types
          solid = true;
      } else
        solid = true;
    } else
    ...
    if (cutflag & FF_FOG)
      HWR_AddTransparentWall(wallVerts, Surf, texnum, PF_Fog | PF_NoTexture,
                             true, lightnum, colormap);
    else if (cutflag & FF_TRANSLUCENT)
      HWR_AddTransparentWall(wallVerts, Surf, texnum, PF_Translucent, false,
                             lightnum, colormap);
    else
  ...
  if (cutflag & FF_FOG)
    HWR_AddTransparentWall(wallVerts, Surf, texnum, PF_Fog | PF_NoTexture, true,
                           lightnum, colormap);
  else if (cutflag & FF_TRANSLUCENT)
    HWR_AddTransparentWall(wallVerts, Surf, texnum, PF_Translucent, false,
                           lightnum, colormap);
...
#define SLOPEPARAMS(slope, end1, end2, normalheight)                           \
  ...
  SLOPEPARAMS(gr_frontsector->c_slope, worldtop, worldtopslope,
              gr_frontsector->ceilingheight)
  SLOPEPARAMS(gr_frontsector->f_slope, worldbottom, worldbottomslope,
              gr_frontsector->floorheight)
#else
  ...
  lightnum =
      HWR_CalcWallLight(gr_frontsector->lightlevel, vs.x, vs.y, ve.x, ve.y);
...
#ifdef ESLOPE
    SLOPEPARAMS(gr_backsector->c_slope, worldhigh, worldhighslope,
                gr_backsector->ceilingheight)
    SLOPEPARAMS(gr_backsector->f_slope, worldlow, worldlowslope,
                gr_backsector->floorheight)
#undef SLOPEPARAMS
#else
    ...
    if (!gr_curline->polyseg) // Don't do it for polyobjects
    {
      ...
      if (gr_frontsector->ceilingpic == skyflatnum) {
        if (gr_backsector->ceilingpic == skyflatnum) {
          // Both front and back sectors are sky, needs skywall from the
          // frontsector's ceiling, but only if the backsector is lower
          if ((worldhigh <= worldtop &&
               worldhighslope <= worldtopslope) // Assuming ESLOPE is always on
                                                // with my changes
              && (worldhigh != worldtop || worldhighslope != worldtopslope))
          // Removing the second line above will render more rarely visible
          // skywalls. Example: Cave garden ceiling in Dark race
          {
#ifdef ESLOPE
          ...
          // Only the frontsector is sky, just draw a skywall from the front
          // ceiling
#ifdef ESLOPE
        ...
        // Only the backsector is sky, just draw a skywall from the front
        // ceiling
#ifdef ESLOPE
        wallVerts[0].y = FIXED_TO_FLOAT(worldtop);
        wallVerts[1].y = FIXED_TO_FLOAT(worldtopslope);
      ...
      if (gr_frontsector->floorpic == skyflatnum) {
        if (gr_backsector->floorpic == skyflatnum) {
          // Both front and back sectors are sky, needs skywall from the
          // backsector's floor, but only if the it's higher, also needs to
          // check for bottomtexture as the floors don't usually move down when
          // both sides are sky floors
          ...
          // Only the backsector has sky, just draw a skywall from the back
          // floor
#ifdef ESLOPE
      ...
      } else if ((gr_backsector->floorpic == skyflatnum) &&
                 !(gr_sidedef->bottomtexture)) {
        // Only the backsector has sky, just draw a skywall from the back floor
        // if there's no bottomtexture
#ifdef ESLOPE
        wallVerts[3].y = FIXED_TO_FLOAT(worldlow);
        wallVerts[2].y = FIXED_TO_FLOAT(worldlowslope);
#else
    ...
    if ((
        ...
        gr_toptexture) {
      {
        ...
        if (gr_linedef->flags & ML_DONTPEGTOP)
          ...
          texturevpegtop =
              worldhigh + textureheight[gr_sidedef->toptexture] - worldtop;
        else
          texturevpegtop = gr_backsector->ceilingheight +
                           textureheight[gr_sidedef->toptexture] -
                           gr_frontsector->ceilingheight;
#else
        else
          texturevpegtop =
        ...
        // This is so that it doesn't overflow and screw up the wall, it doesn't
        // need to go higher than the texture's height anyway
        texturevpegtop %= SHORT(textures[gr_toptexture]->height) << FRACBITS;

        wallVerts[3].t = wallVerts[2].t = texturevpegtop * grTex->scaleY;
        wallVerts[0].t = wallVerts[1].t =
            (texturevpegtop + gr_frontsector->ceilingheight -
             gr_backsector->ceilingheight) *
            grTex->scaleY;
        ...
        if (!(gr_linedef->flags & ML_EFFECT1)) {
          // Unskewed
          wallVerts[3].t -=
              (worldtop - gr_frontsector->ceilingheight) * grTex->scaleY;
          wallVerts[2].t -=
              (worldtopslope - gr_frontsector->ceilingheight) * grTex->scaleY;
          wallVerts[0].t -=
              (worldhigh - gr_backsector->ceilingheight) * grTex->scaleY;
          wallVerts[1].t -=
              (worldhighslope - gr_backsector->ceilingheight) * grTex->scaleY;
        } else if (gr_linedef->flags & ML_DONTPEGTOP) {
          // Skewed by top
          wallVerts[0].t =
              (texturevpegtop + worldtop - worldhigh) * grTex->scaleY;
          wallVerts[1].t =
              (texturevpegtop + worldtopslope - worldhighslope) * grTex->scaleY;
        } else {
          // Skewed by bottom
          wallVerts[0].t = wallVerts[1].t =
              (texturevpegtop + worldtop - worldhigh) * grTex->scaleY;
          wallVerts[3].t =
              wallVerts[0].t - (worldtop - worldhigh) * grTex->scaleY;
          wallVerts[2].t =
              wallVerts[1].t - (worldtopslope - worldhighslope) * grTex->scaleY;
        }
#endif
      }
      ...
      if (gr_frontsector->numlights)
        HWR_SplitWall(gr_frontsector, wallVerts, gr_toptexture, &Surf,
                      FF_CUTLEVEL, NULL);
      else if (grTex->mipmap.flags & TF_TRANSPARENT)
        HWR_AddTransparentWall(wallVerts, &Surf, gr_toptexture, PF_Environment,
                               false, lightnum, colormap);
      else
        HWR_ProjectWall(wallVerts, &Surf, PF_Masked, lightnum, colormap);
    }
    ...
    if ((
        ...
        gr_bottomtexture) // only if VISIBLE!!!
    {
      {
        ...
        if (!(gr_linedef->flags & ML_DONTPEGBOTTOM))
          ...
          texturevpegbottom =
              gr_frontsector->floorheight - gr_backsector->floorheight;
#else
        if (gr_linedef->flags & ML_DONTPEGBOTTOM)
          texturevpegbottom = worldbottom - worldlow;
        else
        ...
        // This is so that it doesn't overflow and screw up the wall, it doesn't
        // need to go higher than the texture's height anyway
        texturevpegbottom %= SHORT(textures[gr_bottomtexture]->height)
                             << FRACBITS;

        wallVerts[3].t = wallVerts[2].t = texturevpegbottom * grTex->scaleY;
        wallVerts[0].t = wallVerts[1].t =
            (texturevpegbottom + gr_backsector->floorheight -
             gr_frontsector->floorheight) *
            grTex->scaleY;
        wallVerts[0].s = wallVerts[3].s = cliplow * grTex->scaleX;
        wallVerts[2].s = wallVerts[1].s = cliphigh * grTex->scaleX;
        ...
        if (!(gr_linedef->flags & ML_EFFECT1)) {
          // Unskewed
          wallVerts[0].t -=
              (worldbottom - gr_frontsector->floorheight) * grTex->scaleY;
          wallVerts[1].t -=
              (worldbottomslope - gr_frontsector->floorheight) * grTex->scaleY;
          wallVerts[3].t -=
              (worldlow - gr_backsector->floorheight) * grTex->scaleY;
          wallVerts[2].t -=
              (worldlowslope - gr_backsector->floorheight) * grTex->scaleY;
        } else if (gr_linedef->flags & ML_DONTPEGBOTTOM) {
          // Skewed by bottom
          wallVerts[0].t = wallVerts[1].t =
              (texturevpegbottom + worldlow - worldbottom) * grTex->scaleY;
          // wallVerts[3].t = wallVerts[0].t - (worldlow - worldbottom) *
          // grTex->scaleY; // no need, [3] is already this
          wallVerts[2].t = wallVerts[1].t -
                           (worldlowslope - worldbottomslope) * grTex->scaleY;
        } else {
          // Skewed by top
          wallVerts[0].t =
              (texturevpegbottom + worldlow - worldbottom) * grTex->scaleY;
          wallVerts[1].t =
              (texturevpegbottom + worldlowslope - worldbottomslope) *
              grTex->scaleY;
        }
#endif
      }
      ...
      if (gr_frontsector->numlights)
        HWR_SplitWall(gr_frontsector, wallVerts, gr_bottomtexture, &Surf,
                      FF_CUTLEVEL, NULL);
      else if (grTex->mipmap.flags & TF_TRANSPARENT)
        HWR_AddTransparentWall(wallVerts, &Surf, gr_bottomtexture,
                               PF_Environment, false, lightnum, colormap);
      else
        HWR_ProjectWall(wallVerts, &Surf, PF_Masked, lightnum, colormap);
    }
    ...
    if (gr_midtexture) {
...
#ifdef POLYOBJECTS
      // NOTE: With polyobjects, whenever you need to check the properties of
      // the polyobject sector it belongs to, you must use the linedef's
      // backsector to be correct From CB
      ...
      if (gr_linedef->flags & ML_EFFECT2) {
        if (!!(gr_linedef->flags & ML_DONTPEGBOTTOM) ^
            !!(gr_linedef->flags & ML_EFFECT3)) {
          polybottom = max(front->floorheight, back->floorheight) +
                       gr_sidedef->rowoffset;
          polytop = polybottom + textureheight[gr_midtexture] * repeats;
        } else {
          polytop = min(front->ceilingheight, back->ceilingheight) +
                    gr_sidedef->rowoffset;
      ...
      if (gr_curline->polyseg) {
        ...
        // The cut-off values of a linedef can always be constant, since every
        // line has an absoulute front and or back sector
        lowcut = popenbottom;
        highcut = popentop;
      }
...
#endif
          texturevpeg =
              textureheight[gr_sidedef->midtexture] * repeats - h + polybottom;
        else
      ...
      // set top/bottom coords
      // Take the texture peg into account, rather than changing the offsets
      // past where the polygon might not be.
      ...
      // Correct to account for slopes
      {
        ...
        if (gr_linedef->flags & ML_EFFECT2)
          ...
          midtextureslant = worldtop < worldhigh ? worldtopslope - worldtop
                                                 : worldhighslope - worldhigh;

        polytop += midtextureslant;
        polybottom += midtextureslant;

        highcut += worldtop < worldhigh ? worldtopslope - worldtop
                                        : worldhighslope - worldhigh;
        lowcut += worldlow < worldbottom ? worldbottomslope - worldbottom
                                         : worldlowslope - worldlow;
...

        {
          ...
          if (!!(gr_linedef->flags & ML_DONTPEGBOTTOM) ^
              !!(gr_linedef->flags & ML_EFFECT3))
            texturevpeg = textureheight[gr_sidedef->midtexture] * repeats - h +
                          polybottom;
          else
            texturevpeg = polytop - h;
      ...
      // ooops ! this do not work at all because render order we should render
      // it in backtofront order
      ...
      if (gr_curline->polyseg && gr_curline->polyseg->translucency > 0) {
        ...
        {
          Surf.PolyColor.s.alpha =
              0x00; // This shouldn't draw anything regardless of blendmode
          blendmode = PF_Masked;
        } else
          blendmode =
              HWR_TranstableToAlpha(gr_curline->polyseg->translucency, &Surf);
      }
#endif

      if (gr_frontsector->numlights) {
        if (!(blendmode & PF_Masked))
          HWR_SplitWall(gr_frontsector, wallVerts, gr_midtexture, &Surf,
                        FF_TRANSLUCENT, NULL);
        else {
          HWR_SplitWall(gr_frontsector, wallVerts, gr_midtexture, &Surf,
                        FF_CUTLEVEL, NULL);
    ...
    {
      {
        ...
        // PEGGING
#ifdef ESLOPE
        if ((gr_linedef->flags & (ML_DONTPEGBOTTOM | ML_EFFECT2)) ==
            (ML_DONTPEGBOTTOM | ML_EFFECT2))
          texturevpeg = gr_frontsector->floorheight +
                        textureheight[gr_sidedef->midtexture] -
                        gr_frontsector->ceilingheight + gr_sidedef->rowoffset;
        else
#endif
            if (gr_linedef->flags & ML_DONTPEGBOTTOM)
          texturevpeg = worldbottom + textureheight[gr_sidedef->midtexture] -
                        worldtop + gr_sidedef->rowoffset;
        else
        ...
        wallVerts[3].t = wallVerts[2].t = texturevpeg * grTex->scaleY;
        wallVerts[0].t = wallVerts[1].t =
            (texturevpeg + gr_frontsector->ceilingheight -
             gr_frontsector->floorheight) *
            grTex->scaleY;
        wallVerts[0].s = wallVerts[3].s = cliplow * grTex->scaleX;
        wallVerts[2].s = wallVerts[1].s = cliphigh * grTex->scaleX;
        ...
        if (gr_linedef->flags & ML_EFFECT2) {
          wallVerts[3].t +=
              (gr_frontsector->ceilingheight - worldtop) * grTex->scaleY;
          wallVerts[2].t +=
              (gr_frontsector->ceilingheight - worldtopslope) * grTex->scaleY;
          wallVerts[0].t +=
              (gr_frontsector->floorheight - worldbottom) * grTex->scaleY;
          wallVerts[1].t +=
              (gr_frontsector->floorheight - worldbottomslope) * grTex->scaleY;
        } else if (gr_linedef->flags & ML_DONTPEGBOTTOM) {
          wallVerts[3].t =
              wallVerts[0].t + (worldbottom - worldtop) * grTex->scaleY;
          wallVerts[2].t = wallVerts[1].t +
                           (worldbottomslope - worldtopslope) * grTex->scaleY;
        } else {
          wallVerts[0].t =
              wallVerts[3].t - (worldbottom - worldtop) * grTex->scaleY;
          wallVerts[1].t = wallVerts[2].t -
                           (worldbottomslope - worldtopslope) * grTex->scaleY;
        }
#endif
      }
      ...
      // I don't think that solid walls can use translucent linedef types...
        ...
        if (grTex->mipmap.flags & TF_TRANSPARENT)
          HWR_AddTransparentWall(wallVerts, &Surf, gr_midtexture,
                                 PF_Environment, false, lightnum, colormap);
        else
          HWR_ProjectWall(wallVerts, &Surf, PF_Masked, lightnum, colormap);
      }
      ...
      // When there's no midtexture, draw a skywall to prevent rendering behind
      // it
      HWR_DrawSkyWall(wallVerts, &Surf);
    }

    // Single sided lines are simple for skywalls, just need to draw from the
    // top or bottom of the sector if there's a sky flat
    if (!gr_curline->polyseg) {
      if (gr_frontsector->ceilingpic ==
          skyflatnum) // It's a single-sided line with sky for its sector
      {
        wallVerts[3].y = wallVerts[2].y = FIXED_TO_FLOAT(INT32_MAX);
#ifdef ESLOPE
        wallVerts[0].y = FIXED_TO_FLOAT(worldtop);
  ...
  if (gr_frontsector && gr_backsector &&
    ...
    /// TODO add slope support (fixing cutoffs, proper wall clipping) - maybe
    /// just disable highcut/lowcut if either sector or FOF has a slope
    ///      to allow fun plane intersecting in OGL? But then people would abuse
    ///      that and make software look bad. :C
    highcut = gr_frontsector->ceilingheight < gr_backsector->ceilingheight
                  ? gr_frontsector->ceilingheight
                  : gr_backsector->ceilingheight;
    lowcut = gr_frontsector->floorheight > gr_backsector->floorheight
                 ? gr_frontsector->floorheight
                 : gr_backsector->floorheight;
    ...
    if (gr_backsector->ffloors) {
      for (rover = gr_backsector->ffloors; rover; rover = rover->next) {
...
#ifdef ESLOPE
        h = *rover->t_slope ? P_GetZAt(*rover->t_slope, v1x, v1y)
                            : *rover->topheight;
        hS = *rover->t_slope ? P_GetZAt(*rover->t_slope, v2x, v2y)
                             : *rover->topheight;
        l = *rover->b_slope ? P_GetZAt(*rover->b_slope, v1x, v1y)
                            : *rover->bottomheight;
        lS = *rover->b_slope ? P_GetZAt(*rover->b_slope, v2x, v2y)
                             : *rover->bottomheight;
        if (!(*rover->t_slope) && !gr_frontsector->c_slope &&
            !gr_backsector->c_slope && h > highcut)
          h = hS = highcut;
        if (!(*rover->b_slope) && !gr_frontsector->f_slope &&
            !gr_backsector->f_slope && l < lowcut)
          l = lS = lowcut;
        ...
        if (rover->flags & FF_FOG) {
          ...
          // ...Oh well, anyway, Lower Unpegged now changes pegging of FOFs like
          // in software
          ...
          if (!slopeskew) // no skewing
          {
            ...
            wallVerts[3].t =
                (*rover->topheight - h + texturevpeg) * grTex->scaleY;
            wallVerts[2].t =
                (*rover->topheight - hS + texturevpeg) * grTex->scaleY;
            wallVerts[0].t =
                (*rover->topheight - l + texturevpeg) * grTex->scaleY;
            wallVerts[1].t =
                (*rover->topheight - lS + texturevpeg) * grTex->scaleY;
          } else {
        ...
        if (rover->flags & FF_FOG) {
          ...
          lightnum = HWR_CalcWallLight(rover->master->frontsector->lightlevel,
                                       vs.x, vs.y, ve.x, ve.y);
          colormap = rover->master->frontsector->extra_colormap;

          Surf.PolyColor.s.alpha =
              HWR_FogBlockAlpha(rover->master->frontsector->lightlevel,
                                rover->master->frontsector->extra_colormap);

          if (gr_frontsector->numlights)
            HWR_SplitWall(gr_frontsector, wallVerts, 0, &Surf, rover->flags,
                          rover);
          else
            HWR_AddTransparentWall(wallVerts, &Surf, 0, blendmode, true,
                                   lightnum, colormap);
        } else {
          ...
          if (rover->flags & FF_TRANSLUCENT && rover->alpha < 256) {
            blendmode = PF_Translucent;
            Surf.PolyColor.s.alpha =
                (UINT8)rover->alpha - 1 > 255 ? 255 : rover->alpha - 1;
          }

          if (gr_frontsector->numlights)
            HWR_SplitWall(gr_frontsector, wallVerts, texnum, &Surf,
                          rover->flags, rover);
          else {
            if (blendmode != PF_Masked)
              HWR_AddTransparentWall(wallVerts, &Surf, texnum, blendmode, false,
                                     lightnum, colormap);
            else
              HWR_ProjectWall(wallVerts, &Surf, PF_Masked, lightnum, colormap);
          }
    ...
    if (gr_frontsector
            ->ffloors) // Putting this seperate should allow 2 FOF sectors to be
                       // connected without too many errors? I think?
    {
      for (rover = gr_frontsector->ffloors; rover; rover = rover->next) {
        if (!(rover->flags & FF_EXISTS) || !(rover->flags & FF_RENDERSIDES) ||
            !(rover->flags & FF_ALLSIDES))
          continue;
        if (*rover->topheight < lowcut || *rover->bottomheight > highcut)
          continue;
        ...
        if (rover->flags & FF_FOG) {
          ...
          if (newline) {
            wallVerts[3].t = wallVerts[2].t =
                (*rover->topheight - h + sides[newline->sidenum[0]].rowoffset) *
                grTex->scaleY;
            wallVerts[0].t = wallVerts[1].t =
                (h - l +
                 (*rover->topheight - h +
                  sides[newline->sidenum[0]].rowoffset)) *
                grTex->scaleY;
          } else {
            wallVerts[3].t = wallVerts[2].t =
                (*rover->topheight - h +
                 sides[rover->master->sidenum[0]].rowoffset) *
                grTex->scaleY;
            wallVerts[0].t = wallVerts[1].t =
                (h - l +
                 (*rover->topheight - h +
        ...
        if (rover->flags & FF_FOG) {
          ...
          if (gr_backsector->numlights)
            HWR_SplitWall(gr_backsector, wallVerts, 0, &Surf, rover->flags,
                          rover);
          else
            HWR_AddTransparentWall(wallVerts, &Surf, 0, blendmode, true,
                                   lightnum, colormap);
...
// From PrBoom:
//
// e6y: Check whether the player can look beyond this line, rturns true if we
// can't
//
...
#ifdef ESLOPE
  if (afrontsector->f_slope || afrontsector->c_slope || abacksector->f_slope ||
      abacksector->c_slope) {
...
#define SLOPEPARAMS(slope, end1, end2, normalheight)                           \
  if (slope) {                                                                 \
    ...
    SLOPEPARAMS(afrontsector->f_slope, frontf1, frontf2,
                afrontsector->floorheight)
    SLOPEPARAMS(afrontsector->c_slope, frontc1, frontc2,
                afrontsector->ceilingheight)
    SLOPEPARAMS(abacksector->f_slope, backf1, backf2, abacksector->floorheight)
    SLOPEPARAMS(abacksector->c_slope, backc1, backc2,
                abacksector->ceilingheight)
#undef SLOPEPARAMS
  } else
#endif
  {
  ...
  // here we're talking about a CEILING lower than a floor. ...yeah we don't
  // even need to bother.
  ...
  // here we're talking about floors higher than ceilings, don't even bother
  // either.
  ...
  // Lat: Ok, here's what we need to do, we want to draw thok barriers. Let's
  // define what a thok barrier is; -Must have ceilheight <= floorheight
  // -ceilpic must be skyflatnum
  // -an adjacant sector needs to have a ceilingheight or a floor height
  // different than the one we have, otherwise, it's just a huge ass wall, we
  // shouldn't render past it. -said adjacant sector cannot also be a thok
  // barrier, because that's also dumb and we could render far more than we need
  // to as a result :V

  if (backc1 <= backf1 && backc2 <= backf2) {
    checkforemptylines = false;

    // before we do anything, if both sectors are thok barriers, GET ME OUT OF
    // HERE!
  ...
  // Window.
  // We know it's a window when the above isn't true and the back and front
  // sectors don't match
  if (backc1 != frontc1 || backc2 != frontc2 || backf1 != frontf1 ||
      backf2 != frontf2) {
    checkforemptylines = false;
    return false;
  }

  // In this case we just need to check whether there is actually a need to
  // render any lines, so checkforempty lines stays true
  return false;
}
...
void HWR_AddLine(seg_t *line) {
  ...
  if (!line->backsector)
    ...
    if (checkforemptylines &&
        R_IsEmptyLine(line, gr_frontsector, gr_backsector))
      return;
  }

  HWR_ProcessSeg(); // Doesn't need arguments because they're defined globally
                    // :D
  return;
}
...
void HWR_AddPolyObjectSegs(void) {
  ...
  for (i = 0; i < numpolys; ++i) {
    ...
    for (j = 0; j < po_ptrs[i]->segCount; ++j) {
      // Copy the info of a polyobject's seg, then convert it to OpenGL floating
      // point
...
void HWR_RenderPolyObjectPlane(polyobj_t *polysector, boolean isceiling,
                               fixed_t fixedheight, FBITFIELD blendmode,
                               UINT8 lightlevel, lumpnum_t lumpnum,
                               sector_t *FOFsector, UINT8 alpha,
                               extracolormap_t *planecolormap) {
  float height; // constant y for all points on the convex flat polygon
  FOutVector *v3d;
  INT32 i;
  ...
  if (nrPlaneVerts > INT16_MAX) // FIXME: exceeds plVerts size
  {
    CONS_Debug(DBG_RENDER,
               "polygon size of %s exceeds max value of %d vertices\n",
  ...
  // reference point for flat texture coord for each vertex around the polygon
  flatxref = (float)(((fixed_t)FIXED_TO_FLOAT(polysector->origVerts[0].x) &
                      (~flatflag)) /
                     fflatsize);
  flatyref = (float)(((fixed_t)FIXED_TO_FLOAT(polysector->origVerts[0].y) &
                      (~flatflag)) /
                     fflatsize);
  ...
  if (angle) // Only needs to be done if there's an altered angle
  {
    ...
    scrollx = (FIXED_TO_FLOAT(FixedMul(tempxsow, FINECOSINE(angle)) -
                              FixedMul(tempytow, FINESINE(angle))));
    scrolly = (FIXED_TO_FLOAT(FixedMul(tempxsow, FINESINE(angle)) +
                              FixedMul(tempytow, FINECOSINE(angle))));
  ...
  for (i = 0; i < (INT32)nrPlaneVerts; i++, v3d++) {
    // Hurdler: add scrolling texture on floor/ceiling
    v3d->s =
        (float)((FIXED_TO_FLOAT(polysector->origVerts[i].x) / fflatsize) -
                flatxref +
                scrollx); // Go from the polysector's original vertex locations
    v3d->t = (float)(flatyref -
                     (FIXED_TO_FLOAT(polysector->origVerts[i].y) / fflatsize) +
                     scrolly); // Means the flat is offset based on the original
                               // vertex locations
...
void HWR_AddPolyObjectPlanes(void) {
  ...
  // Polyobject Planes need their own function for drawing because they don't
  // have extrasubsectors by themselves It should be okay because polyobjects
  // should always be convex anyway

  for (i = 0; i < numpolys; i++) {
    polyobjsector =
        po_ptrs[i]->lines[0]->backsector; // the in-level polyobject sector

    if (!(po_ptrs[i]->flags &
          POF_RENDERPLANES)) // Only render planes when you should
      continue;
    ...
    if (polyobjsector->floorheight <= gr_frontsector->ceilingheight &&
      ...
      if (po_ptrs[i]->translucency > 0) {
        FSurfaceInfo Surf;
        FBITFIELD blendmode =
            HWR_TranstableToAlpha(po_ptrs[i]->translucency, &Surf);
        HWR_AddTransparentPolyobjectFloor(
            levelflats[polyobjsector->floorpic].lumpnum, po_ptrs[i], false,
            polyobjsector->floorheight, polyobjsector->lightlevel,
            Surf.PolyColor.s.alpha, polyobjsector, blendmode, NULL);
      } else {
        HWR_GetFlat(levelflats[polyobjsector->floorpic].lumpnum,
                    R_NoEncore(polyobjsector, false));
        HWR_RenderPolyObjectPlane(po_ptrs[i], false, polyobjsector->floorheight,
                                  PF_Occlude, polyobjsector->lightlevel,
                                  levelflats[polyobjsector->floorpic].lumpnum,
    ...
    if (polyobjsector->ceilingheight >= gr_frontsector->floorheight &&
      ...
      if (po_ptrs[i]->translucency > 0) {
        ...
        HWR_AddTransparentPolyobjectFloor(
            levelflats[polyobjsector->ceilingpic].lumpnum, po_ptrs[i], true,
            polyobjsector->ceilingheight, polyobjsector->lightlevel,
            Surf.PolyColor.s.alpha, polyobjsector, blendmode, NULL);
      } else {
        HWR_GetFlat(levelflats[polyobjsector->ceilingpic].lumpnum,
                    R_NoEncore(polyobjsector, true));
        HWR_RenderPolyObjectPlane(po_ptrs[i], true,
                                  polyobjsector->ceilingheight, PF_Occlude,
                                  polyobjsector->lightlevel,
                                  levelflats[polyobjsector->floorpic].lumpnum,
                                  polyobjsector, 255, NULL);
      }
    }
...
void HWR_Subsector(size_t num) {
  ...
  if (num >= addsubsector)
    I_Error("HWR_Subsector: ss %s with numss = %s, addss = %s\n", sizeu1(num),
            sizeu2(numsubsectors), sizeu3(addsubsector));
#endif
  ...
  if (gr_frontsector->pseudoSector) {
    ...
    ///@TODO Is this whole virtualFloor mess even useful? I don't think it even
    /// triggers ever.
    cullFloorHeight = locFloorHeight = gr_frontsector->virtualFloorheight;
    if (gr_frontsector->virtualCeiling)
      cullCeilingHeight = locCeilingHeight =
          gr_frontsector->virtualCeilingheight;
    else
    ...
    if (gr_frontsector->f_slope) {
      ...
      locFloorHeight =
          P_GetZAt(gr_frontsector->f_slope, gr_frontsector->soundorg.x,
                   gr_frontsector->soundorg.y);
    }

    if (gr_frontsector->c_slope) {
      cullCeilingHeight = P_GetZAt(gr_frontsector->c_slope, viewx, viewy);
      locCeilingHeight =
          P_GetZAt(gr_frontsector->c_slope, gr_frontsector->soundorg.x,
                   gr_frontsector->soundorg.y);
    }
#endif
  }
    ...
    if (gr_frontsector->floorpic != skyflatnum) {
      if (sub->validcount != validcount) {
        HWR_GetFlat(levelflats[gr_frontsector->floorpic].lumpnum,
                    R_NoEncore(gr_frontsector, false));
        HWR_RenderPlane(
            &extrasubsectors[num], false,
            // Hack to make things continue to work around slopes.
            locFloorHeight == cullFloorHeight ? locFloorHeight
                                              : gr_frontsector->floorheight,
            // We now return you to your regularly scheduled rendering.
            PF_Occlude, floorlightlevel,
            levelflats[gr_frontsector->floorpic].lumpnum, NULL, 255,
            floorcolormap);
      }
    }
  }
    ...
    if (gr_frontsector->ceilingpic != skyflatnum) {
      if (sub->validcount != validcount) {
        HWR_GetFlat(levelflats[gr_frontsector->ceilingpic].lumpnum,
                    R_NoEncore(gr_frontsector, true));
        HWR_RenderPlane(
            &extrasubsectors[num], true,
            // Hack to make things continue to work around slopes.
            locCeilingHeight == cullCeilingHeight
                ? locCeilingHeight
                : gr_frontsector->ceilingheight,
            // We now return you to your regularly scheduled rendering.
            PF_Occlude, ceilinglightlevel,
            levelflats[gr_frontsector->ceilingpic].lumpnum, NULL, 255,
            ceilingcolormap);
      }
    }
  }
  ...
  if (gr_frontsector->ffloors) {
    ...
    for (rover = gr_frontsector->ffloors; rover; rover = rover->next) {
      ...
      if (*rover->b_slope) {
        ...
        centerHeight = P_GetZAt(*rover->b_slope, gr_frontsector->soundorg.x,
                                gr_frontsector->soundorg.y);
      } else
#endif
      ...
      if (centerHeight <= locCeilingHeight && centerHeight >= locFloorHeight &&
          ((viewz < cullHeight && !(rover->flags & FF_INVERTPLANES)) ||
           (viewz > cullHeight && (rover->flags & FF_BOTHPLANES ||
                                   rover->flags & FF_INVERTPLANES)))) {
        if (rover->flags & FF_FOG) {
          UINT8 alpha;

          light = R_GetPlaneLight(gr_frontsector, centerHeight,
                                  viewz < cullHeight ? true : false);

          alpha =
              HWR_FogBlockAlpha(*gr_frontsector->lightlist[light].lightlevel,
                                rover->master->frontsector->extra_colormap);

          HWR_AddTransparentFloor(
              0, &extrasubsectors[num], false, *rover->bottomheight,
              *gr_frontsector->lightlist[light].lightlevel, alpha,
              rover->master->frontsector, PF_Fog | PF_NoTexture, true,
              rover->master->frontsector->extra_colormap);
        } else if (rover->flags & FF_TRANSLUCENT &&
                   rover->alpha < 256) // SoM: Flags are more efficient
        {
          light = R_GetPlaneLight(gr_frontsector, centerHeight,
                                  viewz < cullHeight ? true : false);
          HWR_AddTransparentFloor(
              levelflats[*rover->bottompic].lumpnum, &extrasubsectors[num],
              false, *rover->bottomheight,
              *gr_frontsector->lightlist[light].lightlevel,
              rover->alpha - 1 > 255 ? 255 : rover->alpha - 1,
              rover->master->frontsector,
              (rover->flags & FF_RIPPLE ? PF_Ripple : 0) | PF_Translucent,
          ...
          light = R_GetPlaneLight(gr_frontsector, centerHeight,
                                  viewz < cullHeight ? true : false);
          HWR_RenderPlane(
              &extrasubsectors[num], false, *rover->bottomheight,
              (rover->flags & FF_RIPPLE ? PF_Ripple : 0) | PF_Occlude,
              *gr_frontsector->lightlist[light].lightlevel,
              levelflats[*rover->bottompic].lumpnum, rover->master->frontsector,
              255, gr_frontsector->lightlist[light].extra_colormap);
        }
      }
      ...
      if (*rover->t_slope) {
        ...
        centerHeight = P_GetZAt(*rover->t_slope, gr_frontsector->soundorg.x,
                                gr_frontsector->soundorg.y);
      } else
#endif
      ...
      if (centerHeight >= locFloorHeight && centerHeight <= locCeilingHeight &&
        ...
        if (rover->flags & FF_FOG) {
          ...
          HWR_AddTransparentFloor(
              0, &extrasubsectors[num], true, *rover->topheight,
              *gr_frontsector->lightlist[light].lightlevel, alpha,
              rover->master->frontsector, PF_Fog | PF_NoTexture, true,
              rover->master->frontsector->extra_colormap);
          ...
          light = R_GetPlaneLight(gr_frontsector, centerHeight,
                                  viewz < cullHeight ? true : false);
          HWR_AddTransparentFloor(
              levelflats[*rover->toppic].lumpnum, &extrasubsectors[num], true,
              *rover->topheight, *gr_frontsector->lightlist[light].lightlevel,
              rover->alpha - 1 > 255 ? 255 : rover->alpha - 1,
              rover->master->frontsector,
              (rover->flags & FF_RIPPLE ? PF_Ripple : 0) | PF_Translucent,
              false, gr_frontsector->lightlist[light].extra_colormap);
          ...
          light = R_GetPlaneLight(gr_frontsector, centerHeight,
                                  viewz < cullHeight ? true : false);
          HWR_RenderPlane(
              &extrasubsectors[num], true, *rover->topheight,
              (rover->flags & FF_RIPPLE ? PF_Ripple : 0) | PF_Occlude,
              *gr_frontsector->lightlist[light].lightlevel,
              levelflats[*rover->toppic].lumpnum, rover->master->frontsector,
              255, gr_frontsector->lightlist[light].extra_colormap);
        }
      }
    }
...
#ifdef POLYOBJECTS_PLANES
    if (sub->validcount !=
        validcount) // This validcount situation seems to let us know that the
                    // floors have already been drawn.
    {
...
// sprite translucency effects apply on the rendered view (instead of the
// background sky!!)

static UINT32 gr_visspritecount;
static gr_vissprite_t *gr_visspritechunks[MAXVISSPRITES >> VISSPRITECHUNKBITS] =
    {NULL};
...
// --------------------------------------------------------------------------
static void HWR_ClearSprites(void) { gr_visspritecount = 0; }
...
static gr_vissprite_t *HWR_GetVisSprite(UINT32 num) {
  ...
  if (!gr_visspritechunks[chunk])
    Z_Malloc(sizeof(gr_vissprite_t) * VISSPRITESPERCHUNK, PU_LEVEL,
             &gr_visspritechunks[chunk]);
...
static fixed_t HWR_OpaqueFloorAtPos(fixed_t x, fixed_t y, fixed_t z,
                                    fixed_t height) {
  ...
  if (sec->ffloors) {
    ...
    for (rover = sec->ffloors; rover; rover = rover->next) {
      if (!(rover->flags & FF_EXISTS) || !(rover->flags & FF_RENDERPLANES) ||
          rover->flags & FF_TRANSLUCENT || rover->flags & FF_FOG ||
          rover->flags & FF_INVERTPLANES)
        continue;

      delta1 = z - (*rover->bottomheight +
...
// (see r_main.c)
static boolean HWR_DoCulling(line_t *cullheight, line_t *viewcullheight,
                             float vz, float bottomh, float toph) {
  float cullplane;
...
static void HWR_DrawSpriteShadow(gr_vissprite_t *spr, GLPatch_t *gpatch,
  ...
  mobjfloor = HWR_OpaqueFloorAtPos(spr->mobj->x, spr->mobj->y, spr->mobj->z,
                                   spr->mobj->height);
  if (cv_shadowoffs.value) {
    ...
    floorheight = HWR_OpaqueFloorAtPos(
        spr->mobj->x +
            P_ReturnThrustX(spr->mobj, shadowdir, spr->mobj->z - mobjfloor),
        spr->mobj->y +
            P_ReturnThrustY(spr->mobj, shadowdir, spr->mobj->z - mobjfloor),
    ...
    if (spr->mobj->z < floorheight)
      ...
      floorz = HWR_OpaqueFloorAtPos(
          spr->mobj->x +
              P_ReturnThrustX(spr->mobj, shadowdir, spr->mobj->z - floorheight),
          spr->mobj->y +
              P_ReturnThrustY(spr->mobj, shadowdir, spr->mobj->z - floorheight),
          spr->mobj->z, spr->mobj->height);
  ...
  if (spr->mobj && fabsf(this_scale - 1.0f) > 1.0E-36f) {
    ...
    swallVerts[0].y = swallVerts[1].y = swallVerts[2].y = swallVerts[3].y =
        spr->ty - gpatch->topoffset * this_scale - (floorheight + 3);

    // Now transform the TOP vertices along the floor in the direction of the
    // camera
    swallVerts[3].x =
        spr->x1 + ((gpatch->height * this_scale) + offset) * gr_viewcos;
    swallVerts[2].x =
        spr->x2 + ((gpatch->height * this_scale) + offset) * gr_viewcos;
    swallVerts[3].z =
        spr->z1 + ((gpatch->height * this_scale) + offset) * gr_viewsin;
    swallVerts[2].z =
        spr->z2 + ((gpatch->height * this_scale) + offset) * gr_viewsin;
  } else {
    // Always a pixel above the floor, perfectly flat.
    swallVerts[0].y = swallVerts[1].y = swallVerts[2].y = swallVerts[3].y =
        spr->ty - gpatch->topoffset - (floorheight + 3);
  ...
  if (!cv_translucency.value) // use default translucency (main sprite won't
                              // have any translucency)
  ...
  if (sSurf.PolyColor.s.alpha > floorheight / 4) {
    sSurf.PolyColor.s.alpha =
        (UINT8)(sSurf.PolyColor.s.alpha - floorheight / 4);
    HWD.pfnSetShader(1); // floor shader
...
static void HWR_RotateSpritePolyToAim(gr_vissprite_t *spr,
                                      FOutVector *wallVerts) {
  if (cv_grspritebillboarding.value && spr && spr->mobj &&
      !(spr->mobj->frame & FF_PAPERSPRITE) && wallVerts) {
    ...
    if (R_UsingFrameInterpolation() && !paused) {
      if (spr->precip) {
        R_InterpolatePrecipMobjState((precipmobj_t *)spr->mobj, rendertimefrac,
                                     &interp);
      } else {
      ...
      if (spr->precip) {
        R_InterpolatePrecipMobjState((precipmobj_t *)spr->mobj, FRACUNIT,
                                     &interp);
      } else {
...
static void HWR_SplitSprite(gr_vissprite_t *spr) {
  ...
  GLPatch_t *gpatch;
  FSurfaceInfo Surf;
  const boolean hires = (spr->mobj && spr->mobj->skin &&
                         ((skin_t *)spr->mobj->skin)->flags & SF_HIRES);
  extracolormap_t *colormap;
  FUINT lightlevel;
  FBITFIELD blend = 0;
  UINT8 alpha;
  ...
  if (hires)
    this_scale =
        this_scale * FIXED_TO_FLOAT(((skin_t *)spr->mobj->skin)->highresscale);
  ...
  if (cv_shadow.value // Shadows enabled
      && (spr->mobj->flags & (MF_SCENERY | MF_SPAWNCEILING | MF_NOGRAVITY)) !=
             (MF_SCENERY | MF_SPAWNCEILING |
              MF_NOGRAVITY)                // Ceiling scenery have no shadow.
      && !(spr->mobj->flags2 & MF2_DEBRIS) // Debris have no corona or shadow.
      && (spr->mobj->z >=
          spr->mobj->floorz)) // Without this, your shadow shows on the floor,
                              // even after you die and fall through the ground.
  {
    ////////////////////
    // SHADOW SPRITE! //
    ////////////////////
  ...
  if (spr->mobj && fabsf(this_scale - 1.0f) > 1.0E-36f)
    baseWallVerts[0].y = baseWallVerts[1].y =
        spr->ty - gpatch->height * this_scale;
  else
  ...
  {
    float sprdist = sqrtf((spr->x1 - gr_viewx) * (spr->x1 - gr_viewx) +
                          (spr->z1 - gr_viewy) * (spr->z1 - gr_viewy) +
                          (spr->ty - gr_viewz) * (spr->ty - gr_viewz));
  ...
  if (!cv_translucency.value) // translucency disabled
  {
    ...
    blend = HWR_TranstableToAlpha(
        (spr->mobj->frame & FF_TRANSMASK) >> FF_TRANSSHIFT, &Surf);
  else {
  ...
  for (i = 1; i < sector->numlights; i++) {
    fixed_t h =
        sector->lightlist[i].slope
            ? P_GetZAt(sector->lightlist[i].slope, spr->mobj->x, spr->mobj->y)
            : sector->lightlist[i].height;
    if (h <= temp) {
      if (!(spr->mobj->frame & FF_FULLBRIGHT))
        lightlevel =
            *list[i - 1].lightlevel > 255 ? 255 : *list[i - 1].lightlevel;
  ...
  for (i = 0; i < sector->numlights; i++) {
    ...
    // even if we aren't changing colormap or lightlevel, we still need to
    // continue drawing down the sprite
    ...
    // The x and y only need to be adjusted in the case that it's not a
    // papersprite
    if (cv_grspritebillboarding.value && spr->mobj &&
        !(spr->mobj->frame & FF_PAPERSPRITE)) {
      // Get the x and z of the vertices so billboarding draws correctly
      realheight = realbot - realtop;
      endrealheight = endrealbot - endrealtop;
      heightmult = (realtop - top) / realheight;
      wallVerts[3].x = baseWallVerts[3].x +
                       (baseWallVerts[3].x - baseWallVerts[0].x) * heightmult;
      wallVerts[3].z = baseWallVerts[3].z +
                       (baseWallVerts[3].z - baseWallVerts[0].z) * heightmult;

      heightmult = (endrealtop - endtop) / endrealheight;
      wallVerts[2].x = baseWallVerts[2].x +
                       (baseWallVerts[2].x - baseWallVerts[1].x) * heightmult;
      wallVerts[2].z = baseWallVerts[2].z +
                       (baseWallVerts[2].z - baseWallVerts[1].z) * heightmult;

      heightmult = (realtop - bot) / realheight;
      wallVerts[0].x = baseWallVerts[3].x +
                       (baseWallVerts[3].x - baseWallVerts[0].x) * heightmult;
      wallVerts[0].z = baseWallVerts[3].z +
                       (baseWallVerts[3].z - baseWallVerts[0].z) * heightmult;

      heightmult = (endrealtop - endbot) / endrealheight;
      wallVerts[1].x = baseWallVerts[2].x +
                       (baseWallVerts[2].x - baseWallVerts[1].x) * heightmult;
      wallVerts[1].z = baseWallVerts[2].z +
                       (baseWallVerts[2].z - baseWallVerts[1].z) * heightmult;
    }
#else
  ...
  if (endtop <= endrealbot)
    ...
    if (top <= realbot)
      ...
      // If we're ever down here, somehow the above loop hasn't draw all the
      // light levels of sprite
#ifdef ESLOPE
  wallVerts[3].t = towtop + ((realtop - top) * towmult);
  wallVerts[2].t = towtop + ((endrealtop - endtop) * towmult);
  wallVerts[0].t = towtop + ((realtop - bot) * towmult);
...
static void HWR_DrawSprite(gr_vissprite_t *spr) {
  ...
  // This needs to be AFTER the shadows so that the regular sprites aren't drawn
  // completely black. sprite lighting by modulating the RGB components
  /// \todo coloured
...
static int CompareVisSprites(const void *p1, const void *p2) {
  ...
  int transparency1 =
      (spr1->mobj->flags2 & MF2_SHADOW) || (spr1->mobj->frame & FF_TRANSMASK);
  int transparency2 =
      (spr2->mobj->flags2 & MF2_SHADOW) || (spr2->mobj->frame & FF_TRANSMASK);
  idiff = transparency1 - transparency2;
  if (idiff != 0)
    return idiff;

  fdiff = spr2->tz - spr1->tz; // this order seems correct when checking with
                               // apitrace. Back to front.
  if (fabsf(fdiff) < 1.0E-36f)
    return spr1->dispoffset -
           spr2->dispoffset; // smallest dispoffset first if sprites are at
...
static void HWR_SortVisSprites(void) {
  ...
  qsort(gr_vsprorder, gr_visspritecount, sizeof(gr_vissprite_t *),
        CompareVisSprites);
}
...
typedef struct {
  ...
  boolean fogwall;
  INT32 lightlevel;
  extracolormap_t *wallcolormap; // Doing the lighting in HWR_RenderWall now for
                                 // correct fog after sorting
} wallinfo_t;
...
static size_t numpolyplanes =
    0; // a list of transparent poyobject floors to be drawn
...
void HWR_AddTransparentFloor(lumpnum_t lumpnum, extrasubsector_t *xsub,
                             boolean isceiling, fixed_t fixedheight,
                             INT32 lightlevel, INT32 alpha, sector_t *FOFSector,
                             FBITFIELD blend, boolean fogplane,
                             extracolormap_t *planecolormap) {
  ...
  if (allocedplanes < numplanes + 1) {
    ...
    Z_Realloc(planeinfo, allocedplanes * sizeof(*planeinfo), PU_LEVEL,
              &planeinfo);
  }
...
// When that happens it'll just be done through HWR_AddTransparentFloor and
// HWR_RenderPlane
void HWR_AddTransparentPolyobjectFloor(lumpnum_t lumpnum, polyobj_t *polysector,
                                       boolean isceiling, fixed_t fixedheight,
                                       INT32 lightlevel, INT32 alpha,
                                       sector_t *FOFSector, FBITFIELD blend,
                                       extracolormap_t *planecolormap) {
  ...
  if (allocedpolyplanes < numpolyplanes + 1) {
    ...
    Z_Realloc(polyplaneinfo, allocedpolyplanes * sizeof(*polyplaneinfo),
              PU_LEVEL, &polyplaneinfo);
  }
...
static int CompareDrawNodePlanes(const void *p1, const void *p2) {
  ...
  size_t n2 = *(const size_t *)p2;
  if (!sortnode[n1].plane)
    I_Error("Uh.. This isn't a plane! (n1)");
  if (!sortnode[n2].plane)
    I_Error("Uh.. This isn't a plane! (n2)");
  return ABS(sortnode[n2].plane->fixedheight - viewz) -
         ABS(sortnode[n1].plane->fixedheight - viewz);
}
...
void HWR_RenderDrawNodes(void) {
  ...
  sortnode = Z_Calloc((sizeof(planeinfo_t) * numplanes) +
                      ...
                      PU_STATIC, NULL);
  // todo:
  // However, in reality we shouldn't be re-copying and shifting all this
  // information that is already lying around. This should all be in some sort
  // of linked list or lists.
  sortindex = Z_Calloc(sizeof(size_t) * (numplanes + numpolyplanes + numwalls),
                       PU_STATIC, NULL);
  ...
  // it sorts the list based on the value of the 'drawcount' member of the
  // drawnodes. im thinking the list might already be in that order, but i
  // havent bothered to check yet. anyway doing this sort does not hurt and does
  // not take much time. the while loop after this sort is important however!
  qsort(sortindex, p, sizeof(size_t), CompareDrawNodes);

  // try solving floor order here. for each consecutive run of floors in the
  // list, sort that run.
  while (
      run_start <
      p - 1) // p-1 because a 1 plane run at the end of the list does not count
  {
    // locate run start
    if (sortnode[sortindex[run_start]].plane) {
      // found it, now look for run end
      size_t run_end; // (inclusive)
      for (i = run_start + 1; i < p;
           i++) // size_t and UINT32 being used mixed here... shouldnt break
                // anything though..
      {
      ...
      run_end = i - 1;
      if (run_end >
          run_start) // if there are multiple consecutive planes, not just one
      {
        // consecutive run of planes found, now sort it
        // not sure how long these runs can be in reality...
        qsort(sortindex + run_start, run_end - run_start + 1, sizeof(size_t),
              CompareDrawNodePlanes);
      }
      run_start =
          run_end + 1; // continue looking for runs coming right after this one
    } else {
  ...
  for (i = 0; i < p; i++) {
    if (sortnode[sortindex[i]].plane) {
      // We aren't traversing the BSP tree, so make gr_frontsector null to avoid
      // crashes.
      gr_frontsector = NULL;

      if (!(sortnode[sortindex[i]].plane->blend & PF_NoTexture))
        HWR_GetFlat(sortnode[sortindex[i]].plane->lumpnum,
                    R_NoEncore(sortnode[sortindex[i]].plane->FOFSector,
                               sortnode[sortindex[i]].plane->isceiling));
      HWR_RenderPlane(sortnode[sortindex[i]].plane->xsub,
                      sortnode[sortindex[i]].plane->isceiling,
                      sortnode[sortindex[i]].plane->fixedheight,
                      sortnode[sortindex[i]].plane->blend,
                      sortnode[sortindex[i]].plane->lightlevel,
                      sortnode[sortindex[i]].plane->lumpnum,
                      sortnode[sortindex[i]].plane->FOFSector,
                      sortnode[sortindex[i]].plane->alpha,
                      /*sortnode[sortindex[i]].plane->fogplane,*/
                      sortnode[sortindex[i]].plane->planecolormap);
      ...
      if (!(sortnode[sortindex[i]].polyplane->blend & PF_NoTexture))
        HWR_GetFlat(sortnode[sortindex[i]].polyplane->lumpnum,
                    R_NoEncore(sortnode[sortindex[i]].polyplane->FOFSector,
                               sortnode[sortindex[i]].polyplane->isceiling));
      HWR_RenderPolyObjectPlane(
          sortnode[sortindex[i]].polyplane->polysector,
          sortnode[sortindex[i]].polyplane->isceiling,
          sortnode[sortindex[i]].polyplane->fixedheight,
          sortnode[sortindex[i]].polyplane->blend,
          sortnode[sortindex[i]].polyplane->lightlevel,
          sortnode[sortindex[i]].polyplane->lumpnum,
          sortnode[sortindex[i]].polyplane->FOFSector,
          sortnode[sortindex[i]].polyplane->alpha,
          sortnode[sortindex[i]].polyplane->planecolormap);
    } else if (sortnode[sortindex[i]].wall) {
      if (!(sortnode[sortindex[i]].wall->blend & PF_NoTexture))
        HWR_GetTexture(sortnode[sortindex[i]].wall->texnum);
      HWR_RenderWall(sortnode[sortindex[i]].wall->wallVerts,
                     &sortnode[sortindex[i]].wall->Surf,
                     sortnode[sortindex[i]].wall->blend,
                     sortnode[sortindex[i]].wall->fogwall,
                     sortnode[sortindex[i]].wall->lightlevel,
                     sortnode[sortindex[i]].wall->wallcolormap);
    }
  }
...
void HWR_DrawSprites(void) {
  ...
  for (i = 0; i < gr_visspritecount; i++) {
    ...
    if (spr->precip)
    ...
    else if (spr->mobj && spr->mobj->skin && spr->mobj->sprite == SPR_PLAY) {
      // 8/1/19: Only don't display player models if no default SPR_PLAY is
      // found.
      if (!cv_grmdls.value ||
      ...
      if (!cv_grmdls.value || md2_models[spr->mobj->sprite].notfound ||
...
void HWR_AddSprites(sector_t *sec) {
  ...
  if ((limit_dist =
           (fixed_t)(/*(maptol & TOL_NIGHTS) ? cv_drawdist_nights.value : */
                     cv_drawdist.value)
           << FRACBITS)) {
    for (thing = sec->thinglist; thing; thing = thing->snext) {
      ...
      splitflags = thing->eflags & (MFE_DRAWONLYFORP1 | MFE_DRAWONLYFORP2 |
  ...
  if ((limit_dist = (fixed_t)cv_drawdist_precip.value << FRACBITS)) {
    for (precipthing = sec->preciplist; precipthing;
         precipthing = precipthing->snext) {
      if (precipthing->precipflags & PCF_INVISIBLE)
        continue;

      approx_dist =
          P_AproxDistance(viewx - precipthing->x, viewy - precipthing->y);
...
void HWR_ProjectSprite(mobj_t *thing) {
  ...
  if (tz < ZCLIP_PLANE && !papersprite &&
      (!cv_grmdls.value || md2_models[thing->sprite].notfound ==
                               true)) // Yellow: Only MD2's dont disappear
    return;
  ...
  if (rot >= sprdef->numframes) {
    CONS_Alert(
        CONS_ERROR,
        M_GetText("HWR_ProjectSprite: invalid sprite frame %s/%s for %s\n"),
  ...
  if (sprframe->rotate == SRF_SINGLE) {
    ...
    if ((ang < ANGLE_180) && (sprframe->rotate & SRF_RIGHT)) // See from right
      rot = 6;                                               // F7 slot
             ...
             (sprframe->rotate & SRF_LEFT)) // See from left
      rot = 2;                              // F3 slot
  ...
  if (papersprite) {
    ...
    rightcos =
        FIXED_TO_FLOAT(FINECOSINE((viewangle + ANGLE_90) >> ANGLETOFINESHIFT));
  }

  if (flip) {
    x1 = (FIXED_TO_FLOAT(spritecachedinfo[lumpoff].width -
                         spritecachedinfo[lumpoff].offset) *
          this_scale);
    x2 = (FIXED_TO_FLOAT(spritecachedinfo[lumpoff].offset) * this_scale);
  } else {
    x1 = (FIXED_TO_FLOAT(spritecachedinfo[lumpoff].offset) * this_scale);
    x2 = (FIXED_TO_FLOAT(spritecachedinfo[lumpoff].width -
                         spritecachedinfo[lumpoff].offset) *
          this_scale);
  }
  ...
  if (thing->eflags & MFE_VERTICALFLIP) {
    gz = FIXED_TO_FLOAT(interp.z + thing->height) -
         FIXED_TO_FLOAT(spritecachedinfo[lumpoff].topoffset) * this_scale;
    gzt = gz + FIXED_TO_FLOAT(spritecachedinfo[lumpoff].height) * this_scale;
  } else {
    gzt = FIXED_TO_FLOAT(interp.z) +
          FIXED_TO_FLOAT(spritecachedinfo[lumpoff].topoffset) * this_scale;
    gz = gzt - FIXED_TO_FLOAT(spritecachedinfo[lumpoff].height) * this_scale;
  }

  if (thing->subsector->sector->cullheight) {
    if (HWR_DoCulling(thing->subsector->sector->cullheight,
                      viewsector->cullheight, gr_viewz, gz, gzt))
      return;
  }
  ...
  if (heightsec != -1 &&
      phs != -1) // only clip things which are in special sectors
  {
    if (gr_viewz < FIXED_TO_FLOAT(sectors[phs].floorheight)
            ? FIXED_TO_FLOAT(interp.z) >=
                  FIXED_TO_FLOAT(sectors[heightsec].floorheight)
            : gzt < FIXED_TO_FLOAT(sectors[heightsec].floorheight))
      return;
    if (gr_viewz > FIXED_TO_FLOAT(sectors[phs].ceilingheight)
            ? gzt < FIXED_TO_FLOAT(sectors[heightsec].ceilingheight) &&
                  gr_viewz >= FIXED_TO_FLOAT(sectors[heightsec].ceilingheight)
            : FIXED_TO_FLOAT(interp.z) >=
                  FIXED_TO_FLOAT(sectors[heightsec].ceilingheight))
      return;
  }
  ...
  vis->dispoffset =
      thing->info
          ->dispoffset; // Monster Iestyn: 23/11/15: HARDWARE SUPPORT AT LAST
  vis->patchlumpnum = sprframe->lumppat[rot];
  vis->flip = flip;
  vis->mobj = thing;

  // Hurdler: 25/04/2000: now support colormap in hardware mode
  if ((vis->mobj->flags & MF_BOSS) && (vis->mobj->flags2 & MF2_FRET) &&
      (leveltime & 1)) // Bosses "flash"
  {
    ...
    if (thing->colorized)
      vis->colormap =
          R_GetTranslationColormap(TC_RAINBOW, thing->color, GTC_CACHE);
    else if (thing->skin &&
             thing->sprite == SPR_PLAY) // This thing is a player!
    {
      size_t skinnum = (skin_t *)thing->skin - skins;
      vis->colormap =
          R_GetTranslationColormap((INT32)skinnum, thing->color, GTC_CACHE);
    } else
      vis->colormap =
          R_GetTranslationColormap(TC_DEFAULT, thing->color, GTC_CACHE);
  } else {
  ...
  // CONS_Debug(DBG_RENDER, "------------------\nH: sprite  : %d\nH: frame   :
  // %x\nH: type    : %d\nH: sname   : %s\n\n",
  //             thing->sprite, thing->frame, thing->type,
...
void HWR_ProjectPrecipitationSprite(precipmobj_t *thing) {
...
#ifdef RANGECHECK
    I_Error(
        "HWR_ProjectPrecipitationSprite: invalid sprite frame %i : %i for %s",
  ...
  // okay, we can't return now... this is a hack, but weather isn't networked,
  // so it should be ok
...
void HWR_DrawSkyBackground(float fpov) {
  ...
  if (!atransform.shearing)
    dometransform.anglex =
        (float)(aimingangle >> ANGLETOFINESHIFT) * (360.0f / (float)FINEANGLES);
  dometransform.angley = (float)((viewangle - ANGLE_270) >> ANGLETOFINESHIFT) *
                         (360.0f / (float)FINEANGLES);
  ...
  HWD.pfnRenderSkyDome(skytexture, textures[skytexture]->width,
                       textures[skytexture]->height, dometransform);
  HWD.pfnSetShader(0);
}
...
static inline void HWR_ClearView(void) {
  HWD.pfnGClipRect((INT32)gr_viewwindowx, (INT32)gr_viewwindowy,
                   (INT32)(gr_viewwindowx + gr_viewwidth),
                   (INT32)(gr_viewwindowy + gr_viewheight), ZCLIP_PLANE);
...
void HWR_SetViewSize(void) {
  ...
  gr_pspritexscale =
      ((vid.width * gr_pspriteyscale * BASEVIDHEIGHT) / BASEVIDWIDTH) /
      vid.height;
  gr_pspriteyscale =
      ((vid.height * gr_pspritexscale * BASEVIDWIDTH) / BASEVIDHEIGHT) /
      vid.width;
...
void HWR_RenderFrame(INT32 viewnumber, player_t *player, boolean skybox) {
  ...
  gr_viewludsin =
      FIXED_TO_FLOAT(FINECOSINE(gr_aimingangle >> ANGLETOFINESHIFT));
  gr_viewludcos = FIXED_TO_FLOAT(-FINESINE(gr_aimingangle >> ANGLETOFINESHIFT));

  atransform.anglex = (float)(gr_aimingangle >> ANGLETOFINESHIFT) *
                      (360.0f / (float)FINEANGLES);
  atransform.angley =
      (float)(viewangle >> ANGLETOFINESHIFT) * (360.0f / (float)FINEANGLES);
  ...
  if (cv_grbatching.value) {
    int dummy =
        0; // the vars in RenderBatches are meant for render stats. But we don't
           // have that stuff in this branch so that stuff could be removed...
    HWD.pfnRenderBatches(&dummy, &dummy, &dummy, &dummy, &dummy, &dummy,
                         &dummy);
  }

  // Check for new console commands.
  // this was removed since it caused crashes on leaving record attack with
  // models on since it was removing mobjs that were about to be rendered
  // NetUpdate();
  ...
  if (numplanes || numpolyplanes ||
      numwalls) // Render FOFs and translucent walls after everything
    HWR_RenderDrawNodes();
...
void HWR_RenderPlayerView(INT32 viewnumber, player_t *player) {
  const boolean skybox =
      (skyboxmo[0] &&
       cv_skybox.value); // True if there's a skybox object and skyboxes are on

  // Clear the color buffer, stops HOMs. Also seems to fix the skybox issue on
  // Intel GPUs.
  ...
  R_SetupFrame(player, false); // This can stay false because it is only used to
                               // set viewsky in r_main.c, which isn't used here
...
void HWR_AddTransparentWall(FOutVector *wallVerts, FSurfaceInfo *pSurf,
                            INT32 texnum, FBITFIELD blend, boolean fogwall,
                            INT32 lightlevel, extracolormap_t *wallcolormap) {
  ...
  M_Memcpy(wallinfo[numwalls].wallVerts, wallVerts,
           sizeof(wallinfo[numwalls].wallVerts));
...
void HWR_RenderWall(FOutVector *wallVerts, FSurfaceInfo *pSurf, FBITFIELD blend,
                    boolean fogwall, INT32 lightlevel,
                    extracolormap_t *wallcolormap) {
  FBITFIELD blendmode = blend;
  UINT8 alpha = pSurf->PolyColor.s.alpha; // retain the alpha

  // Lighting is done here instead so that fog isn't drawn incorrectly on
  // transparent walls after sorting
  HWR_Lighting(pSurf, lightlevel, wallcolormap);
  ...
  blendmode |=
      PF_Modulated; // No PF_Occlude means overlapping (incorrect) transparency
...
void HWR_DoPostProcessor(player_t *player) {
  ...
  if (player->flashcount) {
    ...
    v[0].z = v[1].z = v[2].z = v[3].z =
        4.0f; // 4.0 because of the same reason as with the sky, just after the
              // screen is cleared so near clipping plane is 3.99

    // This won't change if the flash palettes are changed unfortunately, but it
    // works for its purpose
    if (player->flashpal == PAL_NUKE) {
      Surf.PolyColor.s.red = 0xff;
      Surf.PolyColor.s.green = Surf.PolyColor.s.blue =
          0x7F; // The nuke palette is kind of pink-ish
    } else
      Surf.PolyColor.s.red = Surf.PolyColor.s.green = Surf.PolyColor.s.blue =
          0xff;

    Surf.PolyColor.s.alpha = 0xc0; // match software mode

    HWD.pfnDrawPolygon(&Surf, v, 4,
                       PF_Modulated | PF_Translucent | PF_NoTexture |
                           PF_NoDepthTest);
  }
  ...
  if (splitscreen) // Not supported in splitscreen - someone want to add
                   // support?
    return;
  ...
  if (*type == postimg_water || *type == postimg_heat) {
    ...
    for (x = 0; x < SCREENVERTS; x++) {
      for (y = 0; y < SCREENVERTS; y++) {
        ...
        v[x][y][0] =
            (x / ((float)(SCREENVERTS - 1.0f) / 9.0f)) - 4.5f +
            (float)sin((disStart + (y * WAVELENGTH)) / FREQUENCY) / AMPLITUDE;
...
void HWR_StartScreenWipe(void) { HWD.pfnStartScreenWipe(); }

void HWR_EndScreenWipe(void) { HWD.pfnEndScreenWipe(); }

void HWR_DrawIntermissionBG(void) { HWD.pfnDrawIntermissionBG(); }
...
void HWR_DoWipe(UINT8 wipenum, UINT8 scrnnum) {
  ...
  if (wipenum > 99 || scrnnum > 99) // not a valid wipe number
    return; // shouldn't end up here really, the loop should've stopped running
            // beforehand
...
void HWR_MakeScreenFinalTexture(void) { HWD.pfnMakeScreenFinalTexture(); }
...
#define SHADER_TYPES 7
  shaderxlat_t shaderxlat[SHADER_TYPES] = {
      {"Flat", 1},        {"WallTexture", 2}, {"Sprite", 3}, {"Model", 4},
      {"WaterRipple", 5}, {"Fog", 6},         {"Sky", 7},
  };
  ...
  while (stoken) {
    ...
    if (!stricmp(stoken, "GLSL")) {
      ...
      if (!value) {
        CONS_Alert(CONS_WARNING,
                   "HWR_LoadShaders: Missing shader type (file %s, line %d)\n",
    ...
    skip_lump:
      ...
      if (!value) {
        CONS_Alert(
            CONS_WARNING,
            "HWR_LoadShaders: Missing shader target (file %s, line %d)\n",
            wadfiles[wadnum]->filename, linenum);
        stoken = strtok(NULL, "\r\n"); // skip end of line
        goto skip_field;
      }
      ...
      for (i = 0; i < SHADER_TYPES; i++) {
        if (!stricmp(shaderxlat[i].type, stoken)) {
          ...
          if (PK3) {
            ...
            shader_lumpnum =
                W_CheckNumForFullNamePK3(shader_lumpname, wadnum, 0);
          } else {
          ...
          if (shader_lumpnum == INT16_MAX) {
            CONS_Alert(CONS_ERROR,
                       "HWR_LoadShaders: Missing shader source %s (file %s, "
                       "line %d)\n",
          ...
          HWD.pfnLoadCustomShader(shaderxlat[i].id, shader_source, shader_size,
                                  (shadertype == 2));
