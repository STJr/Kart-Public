...
// origin      - for object movement tracking. NULL are possible value (no
// movement tracking) source_parm - 3D source parameters (position etc.)
// cone_parm   - 3D source cone parameters. NULL are possible value (default
// cone will be used) channel     - sfx_id      - sfx id vol         - sound
// volume pitch       - sound pitching value Returns:    - sound id
INT32
HW3S_I_StartSound(const void *origin, source3D_data_t *source_parm,
                  channel_type_t channel, sfxenum_t sfx_id, INT32 vol,
...
// Utility functions
INT32
HW3S_SoundIsPlaying(INT32 handle);
...
INT32
S_AdjustSoundParams(const mobj_t *listener, const mobj_t *source, INT32 *vol,
                    INT32 *sep, INT32 *pitch, sfxinfo_t *sfxinfo);
