...
static UINT8 softwaretranstogl[11] = {0,   25,  51,  76,  102, 127,
                                      153, 178, 204, 229, 255};
static UINT8 softwaretranstogl_hi[11] = {0,   51,  102, 153, 204, 255,
                                         255, 255, 255, 255, 255};
static UINT8 softwaretranstogl_lo[11] = {0,  12, 24, 36,  48, 60,
                                         71, 83, 95, 111, 127};
...
void HWR_DrawPatch(GLPatch_t *gpatch, INT32 x, INT32 y, INT32 option) {
  ...
  v[0].x = v[3].x =
      (x * sdupx - SHORT(gpatch->leftoffset) * pdupx) / vid.width - 1;
  v[2].x = v[1].x =
      (x * sdupx + (SHORT(gpatch->width) - SHORT(gpatch->leftoffset)) * pdupx) /
          vid.width -
      1;
  v[0].y = v[1].y =
      1 - (y * sdupy - SHORT(gpatch->topoffset) * pdupy) / vid.height;
  v[2].y = v[3].y =
      1 -
      (y * sdupy + (SHORT(gpatch->height) - SHORT(gpatch->topoffset)) * pdupy) /
          vid.height;
...
void HWR_DrawFixedPatch(GLPatch_t *gpatch, fixed_t x, fixed_t y, fixed_t pscale,
                        INT32 option, const UINT8 *colormap) {
  FOutVector v[4];
  FBITFIELD flags;
  ...
  // -- Monster Iestyn 29/10/18
  {
    ...
    if (option & V_FLIP)
      offsetx =
          (float)(SHORT(gpatch->width) - SHORT(gpatch->leftoffset)) * fscale;
    else
    ...
    if ((option & (V_NOSCALESTART | V_OFFSET)) ==
  ...
  if (!(option & V_NOSCALESTART)) {
    ...
    if (!(option & V_SCALEPATCHMASK)) {
      ...
      // This is done before here compared to software because we directly alter
      // cx and cy to centre
      if (cx >= -0.1f && cx <= 0.1f && SHORT(gpatch->width) == BASEVIDWIDTH &&
          cy >= -0.1f && cy <= 0.1f && SHORT(gpatch->height) == BASEVIDHEIGHT) {
        // Need to temporarily cache the real patch to get the colour of the top
        // left pixel
        patch_t *realpatch =
            W_CacheLumpNumPwad(gpatch->wadnum, gpatch->lumpnum, PU_STATIC);
        const column_t *column =
            (const column_t *)((const UINT8 *)(realpatch) +
                               LONG((realpatch)->columnofs[0]));
        const UINT8 *source = (const UINT8 *)(column) + 3;
        HWR_DrawFill(0, 0, BASEVIDWIDTH, BASEVIDHEIGHT,
                     (column->topdelta == 0xff ? 31 : source[0]));
        Z_Free(realpatch);
      }
      // centre screen
      ...
      if (fabsf((float)vid.height - (float)BASEVIDHEIGHT * dupy) > 1.0E-36f) {
        if ((option & (V_SPLITSCREEN | V_SNAPTOBOTTOM)) ==
  ...
  // positions of the cx, cy, are between 0 and vid.width/vid.height now, we
  // need them to be between -1 and 1
  cx = -1 + (cx / (vid.width / 2));
  ...
  if (alphalevel) {
    ...
    Surf.PolyColor.s.red = Surf.PolyColor.s.green = Surf.PolyColor.s.blue =
        0xff;
    if (alphalevel == 13)
      Surf.PolyColor.s.alpha = softwaretranstogl_lo[hudtrans];
    else if (alphalevel == 14)
      Surf.PolyColor.s.alpha = softwaretranstogl[hudtrans];
    else if (alphalevel == 15)
      Surf.PolyColor.s.alpha = softwaretranstogl_hi[hudtrans];
    else
...
void HWR_DrawCroppedPatch(GLPatch_t *gpatch, fixed_t x, fixed_t y,
                          fixed_t pscale, INT32 option, fixed_t sx, fixed_t sy,
                          fixed_t w, fixed_t h) {
  FOutVector v[4];
  FBITFIELD flags;
  float cx = FIXED_TO_FLOAT(x);
  ...
  if (alphalevel) {
    ...
    Surf.PolyColor.s.red = Surf.PolyColor.s.green = Surf.PolyColor.s.blue =
        0xff;
    if (alphalevel == 13)
      Surf.PolyColor.s.alpha = softwaretranstogl_lo[cv_translucenthud.value];
    else if (alphalevel == 14)
      Surf.PolyColor.s.alpha = softwaretranstogl[cv_translucenthud.value];
    else if (alphalevel == 15)
      Surf.PolyColor.s.alpha = softwaretranstogl_hi[cv_translucenthud.value];
    else
      Surf.PolyColor.s.alpha = softwaretranstogl[10 - alphalevel];
    flags |= PF_Modulated;
    HWD.pfnDrawPolygon(&Surf, v, 4, flags);
...
// --------------------------------------------------------------------------
void HWR_DrawFlatFill(INT32 x, INT32 y, INT32 w, INT32 h,
...
void HWR_FadeScreenMenuBack(UINT16 color, UINT8 strength) {
  ...
  HWD.pfnDrawPolygon(&Surf, v, 4,
                     PF_NoTexture | PF_Modulated | PF_Translucent |
                         PF_NoDepthTest);
}
...
void HWR_DrawViewBorder(INT32 clearlines) {
  ...
  baseviewwidth =
      FixedInt(FixedDiv(FLOAT_TO_FIXED(gr_viewwidth),
                        vid.fdupx)); //(cv_viewsize.value * BASEVIDWIDTH/10)&~7;
  ...
  // top
  HWR_DrawFlatFill(0, 0, BASEVIDWIDTH, (top < clearlines ? top : clearlines),
                   st_borderpatchnum);
  ...
  if (top < clearlines)
    HWR_DrawFlatFill(
        side + baseviewwidth, top, side,
        (clearlines - top < baseviewheight ? clearlines - top : baseviewheight),
        st_borderpatchnum);

  // bottom
  if (top + baseviewheight < clearlines)
    HWR_DrawFlatFill(0, top + baseviewheight, BASEVIDWIDTH, BASEVIDHEIGHT,
                     st_borderpatchnum);
  ...
  if (clearlines > basewindowy + baseviewheight)
    HWR_DrawPatch(W_CachePatchNum(viewborderlump[BRDR_BR], PU_CACHE),
                  basewindowx + baseviewwidth, basewindowy + baseviewheight, 0);
}
...
// -----------------+
// HWR_drawAMline   : draw a line of the automap (the clipping is already done
// in automap code) Arg              : color is a RGB 888 value
// -----------------+
...
// -------------------+
// HWR_DrawConsoleFill     : draw flat coloured transparent rectangle because
// that's cool, and hw sucks less than sw for that.
// -------------------+
...
// --------------------------------------------------------------------------
static inline boolean saveTGA(const char *file_name, void *buffer, INT32 width,
                              INT32 height) {
  INT32 fd;
  TGAHeader tga_hdr;
  INT32 i;
