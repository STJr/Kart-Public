...
static void HWR_FreePoly(poly_t *poly) { Z_Free(poly); }
  ...
  for (; count--; lseg++) {
    ...
    if (ps >= 0) {
      ...
      //  only some times..)
        ...
        skipcut++;
      //    I_Error("CutOutPoly: only one point for split line (%d %d) %d", ps,
      //    pe, debugpos);
    }
  }
  return poly;
...
static void loading_status(void) {
  ...
  V_DrawFill(0, 0, BASEVIDWIDTH, BASEVIDHEIGHT,
             31); // Black background to match fade in effect
  // V_DrawPatchFill(W_CachePatchName("SRB2BACK",PU_CACHE)); // SRB2 background,
  // ehhh too bright.
  ...
  V_DrawCenteredString(BASEVIDWIDTH / 2, 40, V_YELLOWMAP,
                       "OPENGL MODE IS INCOMPLETE AND MAY");
  V_DrawCenteredString(BASEVIDWIDTH / 2, 50, V_YELLOWMAP,
                       "NOT DISPLAY SOME SURFACES.");
  V_DrawCenteredString(BASEVIDWIDTH / 2, 70, V_YELLOWMAP,
                       "USE AT SONIC'S RISK.");
...
static void WalkBSPNode(INT32 bspnum, poly_t *poly, UINT16 *leafnode,
                        fixed_t *bbox) {
  node_t *bsp;
  ...
  if (bspnum & NF_SUBSECTOR) {
    if (bspnum == -1) {
      ...
      // - it can't happens in bsp recursive call since bspnum is a INT32 and
      // children is UINT16
      // - the BSP is complet !! (there just can have subsector without segs) (i
      // am not sure of this point)
  ...
  if (bspnum & NF_SUBSECTOR) {
    if (bspnum != -1) {
      ...
      for (j = 0; j < q->numpts; j++) {
        ...
        if (k == q->numpts)
          k = 0;
        if (!SameVertice(p, &q->pts[j]) && !SameVertice(p, &q->pts[k]) &&
...
// realy but i am soo lasy), the methode discibed is also better for segs
// presition
static INT32 SolveTProblem(void) {
  ...
  CONS_Debug(DBG_RENDER,
             "Solving T-joins. This may take a while. Please wait...\n");
...
void HWR_CreatePlanePolygons(INT32 bspnum) {
  ...
  /*gr_drawsubsectors = (INT16 *)malloc(sizeof (*gr_drawsubsectors) *
  totsubsectors); if (!gr_drawsubsectors) I_Error("couldn't malloc
  gr_drawsubsectors\n");*/
  ...
  // if (skipcut)
  //     CONS_Debug(DBG_RENDER, "%u cuts were skipped because of only one
  //     point\n",skipcut);
