...
static float U_Deg2Rad(float deg) { return deg * ((float)PI / 180.0f); }
...
void VectorRotate(vector_t *rotVec, const vector_t *axisVec, float angle) {
  ...
  rotVec->x = axisVec->x * (ux + vy + wz) +
              (rotVec->x * (axisVec->y * axisVec->y + axisVec->z * axisVec->z) -
               axisVec->x * (vy + wz)) *
                  ca +
              (-wy + vz) * sa;
  rotVec->y = axisVec->y * (ux + vy + wz) +
              (rotVec->y * (axisVec->x * axisVec->x + axisVec->z * axisVec->z) -
               axisVec->y * (ux + wz)) *
                  ca +
              (wx - uz) * sa;
  rotVec->z = axisVec->z * (ux + vy + wz) +
              (rotVec->z * (axisVec->x * axisVec->x + axisVec->y * axisVec->y) -
               axisVec->z * (ux + vy)) *
                  ca +
              (-vx + uy) * sa;
}
...
model_t *LoadModel(const char *filename, int ztag) {
  ...
  if (!strcmp(extension, ".md3")) {
    if (!(model = MD3_LoadModel(filename, ztag, false)))
      return NULL;
  } else if (!strcmp(extension, ".md3s")) // MD3 that will be converted in
                                          // memory to use full floats
  {
...
//
// Creates a new normal for a vertex using the average of all of the polygons it
// belongs to.
//
void GenerateVertexNormals(model_t *model) {
  ...
  for (i = 0; i < model->numMeshes; i++) {
    ...
    for (j = 0; j < mesh->numFrames; j++) {
      ...
      float *newNormals = (float *)Z_Malloc(
          sizeof(float) * 3 * mesh->numTriangles * 3, memTag, 0);
      int k;
      float *vertPtr = frame->vertices;
      float *oldNormals;

      M_Memcpy(newNormals, frame->normals,
               sizeof(float) * 3 * mesh->numTriangles * 3);

      /*			if (!systemSucks)
                              {
                                      memTag = Z_GetTag(frame->tangents);
                                      float *newTangents =
         (float*)Z_Malloc(sizeof(float)*3*mesh->numTriangles*3, memTag);
                                      M_Memcpy(newTangents, frame->tangents,
         sizeof(float)*3*mesh->numTriangles*3);
                              }*/
      ...
      for (k = 0; k < mesh->numVertices; k++) {
        ...
        for (l = 0; l < mesh->numVertices; l++) {
          ...
          if (fabsf(x - testX) > FLT_EPSILON ||
              fabsf(y - testY) > FLT_EPSILON || fabsf(z - testZ) > FLT_EPSILON)
            continue;
        ...
        if (vCount > 1) {
          ...
          /*					if (!systemSucks)
                                                  {
                                                          Vector::vector_t
             tangent; Vector::Tangent(&normal, &tangent); newTangents[3 * k + 0]
             = tangent.x; newTangents[3 * k + 1] = tangent.y; newTangents[3 * k
             + 2] = tangent.z;
                                                  }*/
        }
      }
...
void Optimize(model_t *model) {
  ...
  for (node = matListHead; node; node = node->next) {
    ...
    newMesh->uvs =
        (float *)Z_Malloc(sizeof(float) * 2 * numTriangles * 3, memTag, 0);
    ...
    curFrame->normals =
        (float *)Z_Malloc(sizeof(float) * 3 * numTriangles * 3, memTag, 0);
    //		if (!systemSucks)
    //			curFrame->tangents =
    //(float*)Z_Malloc(sizeof(float)*3*numTriangles*3, memTag, 0);
    curFrame->vertices =
        (float *)Z_Malloc(sizeof(float) * 3 * numTriangles * 3, memTag, 0);
    curFrame->colors =
        (char *)Z_Malloc(sizeof(char) * 4 * numTriangles * 3, memTag, 0);
    ...
    for (j = 0; j < model->numMeshes; j++) {
      ...
      if (curMesh->frames[0].material == curMat) {
        ...
        M_Memcpy(&newMesh->uvs[uvCount], curMesh->uvs,
        ...
        /*				if (!systemSucks)
                                        {
                                                dest =
           (float*)newMesh->frames[0].tangents; src =
           (float*)curMesh->frames[0].tangents; M_Memcpy(&dest[vertCount], src,
                                                        sizeof(float)*3*curMesh->numTriangles*3);
                                        }*/
        ...
        if (srcByte) {
          M_Memcpy(&destByte[colorCount], srcByte,
                   sizeof(char) * 4 * curMesh->numTriangles * 3);
        } else {
          memset(&destByte[colorCount], 255,
                 sizeof(char) * 4 * curMesh->numTriangles * 3);
        }
...
void GeneratePolygonNormals(model_t *model, int ztag) {
  ...
  for (i = 0; i < model->numMeshes; i++) {
    ...
    for (j = 0; j < mesh->numFrames; j++) {
      ...
      frame->polyNormals =
          (vector_t *)Z_Malloc(sizeof(vector_t) * mesh->numTriangles, ztag, 0);
