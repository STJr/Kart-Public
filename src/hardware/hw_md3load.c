...
typedef struct {
  ...
  int offsetTriangles; // Relative offset from start of this struct to where the
                       // list of Triangles start
  int offsetShaders;   // Relative offset from start of this struct to where the
                       // list of Shaders start
  int offsetST; // Relative offset from start of this struct to where the list
                // of tex coords start
  int offsetXYZNormal; // Relative offset from start of this struct to where the
                       // list of vertices start
  int offsetEnd; // Relative offset from start of this struct to where this
                 // surface ends
} md3Surface;
...
typedef struct {
  int index[3]; // List of offset values into the list of Vertex objects that
                // constitute the corners of the Triangle object.
} md3Triangle;
...
static void GetNormalFromLatLong(short latlng, float *out) {
  float *lookup =
      latlnglookup[(unsigned char)(latlng >> 8)][(unsigned char)(latlng & 255)];
...
model_t *MD3_LoadModel(const char *fileName, int ztag, boolean useFloat) {
  ...
  for (i = 0, surfEnd = 0; i < mdh->numSurfaces; i++) {
    ...
    for (j = 0; j < mdS->numShaders; j++, matCount++) {
      ...
      // Load material
      /*			retModel->materials[matCount].texture =
         Texture::ReadTexture(mdShader[j].name, ZT_TEXTURE);
         ...
         Resource::Open(openfilename); if (res)
                                      {
                                              Resource::Close(res);
                                              retModel->materials[matCount].lightmap
    ...
    if (!useFloat) // 'tinyframe' mode with indices
    {
      ...
      retModel->meshes[i].tinyframes = (tinyframe_t *)Z_Calloc(
          sizeof(tinyframe_t) * mdS->numFrames, ztag, 0);
      retModel->meshes[i].numVertices = mdS->numVerts;
      retModel->meshes[i].uvs =
          (float *)Z_Malloc(sizeof(float) * 2 * mdS->numVerts, ztag, 0);
      for (j = 0; j < mdS->numFrames; j++) {
        short *vertptr;
        char *normptr;
        // char *tanptr;
        int k;
        md3Vertex *mdV = (md3Vertex *)((char *)mdS + mdS->offsetXYZNormal +
                                       (mdS->numVerts * j * sizeof(md3Vertex)));
        retModel->meshes[i].tinyframes[j].vertices =
            (short *)Z_Malloc(sizeof(short) * 3 * mdS->numVerts, ztag, 0);
        retModel->meshes[i].tinyframes[j].normals =
            (char *)Z_Malloc(sizeof(char) * 3 * mdS->numVerts, ztag, 0);

        //				if (retModel->materials[0].lightmap)
        //					retModel->meshes[i].tinyframes[j].tangents
        //=
        //(char*)malloc(sizeof(char));//(char*)Z_Malloc(sizeof(char)*3*mdS->numVerts,
        // ztag);
        retModel->meshes[i].indices = (unsigned short *)Z_Malloc(
            sizeof(unsigned short) * 3 * mdS->numTriangles, ztag, 0);
    ...
    } else // Traditional full-float loading method
    {
      ...
      retModel->meshes[i].frames =
          (mdlframe_t *)Z_Calloc(sizeof(mdlframe_t) * mdS->numFrames, ztag, 0);
      retModel->meshes[i].uvs =
          (float *)Z_Malloc(sizeof(float) * 2 * mdS->numTriangles * 3, ztag, 0);
      ...
      for (j = 0; j < mdS->numFrames; j++) {
        ...
        md3Vertex *mdV = (md3Vertex *)((char *)mdS + mdS->offsetXYZNormal +
                                       (mdS->numVerts * j * sizeof(md3Vertex)));
        retModel->meshes[i].frames[j].vertices = (float *)Z_Malloc(
            sizeof(float) * 3 * mdS->numTriangles * 3, ztag, 0);
        retModel->meshes[i].frames[j].normals = (float *)Z_Malloc(
            sizeof(float) * 3 * mdS->numTriangles * 3, ztag, 0);
        //				if (retModel->materials[i].lightmap)
        //					retModel->meshes[i].frames[j].tangents
        //=
        //(float*)malloc(sizeof(float));//(float*)Z_Malloc(sizeof(float)*3*mdS->numTriangles*3,
        // ztag);
  ...
  retModel->tags = (tag_t*)Z_Calloc(sizeof(tag_t) * retModel->numTags *
  mdh->numFrames, ztag); md3Tag *mdTag = (md3Tag*)&buffer[mdh->offsetTags];
  ...
  for (i = 0; i < mdh->numFrames; i++)
  {
          ...
          {
                  strcpys(curTag->name, mdTag->name, sizeof(curTag->name) /
  sizeof(char)); curTag->transform.m[0][0] = mdTag->axis[0];
