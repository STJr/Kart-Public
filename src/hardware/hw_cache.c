...
static void
HWR_DrawPatchInCache(GLMipmap_t *mipmap, INT32 pblockwidth, INT32 pblockheight,
                     INT32 blockmodulo, INT32 ptexturewidth,
                     INT32 ptextureheight, INT32 originx,
                     INT32 originy, // where to draw patch in surface block
  ...
  /*
     ...
     SHORT(realpatch->width), SHORT(realpatch->height), ptexturewidth,
                                                                                                                          textureheight,
  ...
  */
  ...
  for (block += col * bpp; ncols--; block += bpp, xfrac += xfracstep) {
    ...
    patchcol =
        (const column_t *)((const UINT8 *)realpatch +
    ...
    while (patchcol->topdelta != 0xff) {
      ...
      while (count > 0) {
        ...
        if (texel == HWR_PATCHES_CHROMAKEY_COLORINDEX &&
            (mipmap->flags & TF_CHROMAKEYED))
          alpha = 0x00;

        // texel = HWR_CHROMAKEY_EQUIVALENTCOLORINDEX;
        // Lat:  Don't do that, some weirdos still use CYAN on their
        // WALLTEXTURES for translucency :V
      ...
      patchcol =
          (const column_t *)((const UINT8 *)patchcol + patchcol->length + 4);
    }
  }
}
...
static void HWR_ResizeBlock(INT32 originalwidth, INT32 originalheight,
  ...
  static const GLlod_t gr_lods[9] = {
      GR_LOD_LOG2_256, GR_LOD_LOG2_128, GR_LOD_LOG2_64,
      GR_LOD_LOG2_32,  GR_LOD_LOG2_16,  GR_LOD_LOG2_8,
      GR_LOD_LOG2_4,   GR_LOD_LOG2_2,   GR_LOD_LOG2_1};
  ...
  static const booring_aspect_t gr_aspects[8] = {
      {GR_ASPECT_LOG2_1x1, 255, 255}, {GR_ASPECT_LOG2_2x1, 255, 127},
      {GR_ASPECT_LOG2_4x1, 255, 63},  {GR_ASPECT_LOG2_8x1, 255, 31},

      {GR_ASPECT_LOG2_1x1, 255, 255}, {GR_ASPECT_LOG2_1x2, 127, 255},
      {GR_ASPECT_LOG2_1x4, 63, 255},  {GR_ASPECT_LOG2_1x8, 31, 255}};
...
static UINT8 *MakeBlock(GLMipmap_t *grMipmap) {
  ...
  case 2:
    ...
    break;
  case 4:
    memset(block, 0x00, blocksize * sizeof(UINT32));
    break;
  }
...
//
// Create a composite texture from patches, adapt the texture size to a power of
// 2 height and width for the hardware texture cache.
//
static void HWR_GenerateTexture(INT32 texnum, GLTexture_t *grtex) {
  ...
  if (texture->name[0] == 'S' && texture->name[1] == 'K' &&
      texture->name[2] == 'Y' &&
  ...
  if (skyspecial) // Hurdler: not efficient, but better than holes in the sky
                  // (and it's done only at level loading)
  {
    INT32 j;
    RGBA_t col;
  ...
  for (i = 0, patch = texture->patches; i < texture->patchcount; i++, patch++) {
    ...
    HWR_DrawPatchInCache(&grtex->mipmap, blockwidth, blockheight,
                         blockwidth * format2bpp[grtex->mipmap.grInfo.format],
                         texture->width, texture->height, patch->originx,
                         patch->originy, realpatch,
                         format2bpp[grtex->mipmap.grInfo.format]);
    Z_Unlock(realpatch);
  }
  // Hurdler: not efficient at all but I don't remember exactly how
  // HWR_DrawPatchInCache works :(
  if (format2bpp[grtex->mipmap.grInfo.format] == 4) {
    for (i = 3; i < blocksize * 4;
         i += 4) // blocksize*4 because blocksize doesn't include the bpp
    {
      if (block[i] == 0) {
...
// patch may be NULL if grMipmap has been initialised already and makebitmap is
// false
...
void HWR_FreeTextureCache(void) {
  ...
  // our gool is only the textures since user of the texture is the texture
  // cache
...
static void HWR_CacheFlat(GLMipmap_t *grMipmap, lumpnum_t flatlumpnum) {
  ...
  W_ReadLump(flatlumpnum, Z_Malloc(W_LumpLength(flatlumpnum), PU_HWRCACHE,
...
static void HWR_LoadMappedPatch(GLMipmap_t *grmip, GLPatch_t *gpatch) {
  if (!grmip->downloaded && !grmip->grInfo.data) {
    patch_t *patch =
        W_CacheLumpNumPwad(gpatch->wadnum, gpatch->lumpnum, PU_STATIC);
...
// -----------------+
// HWR_GetPatch     : Download a patch to the hardware cache and make it ready
// for use
// -----------------+
void HWR_GetPatch(GLPatch_t *gpatch) {
  // is it in hardware cache
  if (!gpatch->mipmap->downloaded && !gpatch->mipmap->grInfo.data) {
    // load the software patch, PU_STATIC or the Z_Malloc for hardware patch
    // will flush the software patch before the conversion! oh yeah I suffered
    patch_t *patch =
        W_CacheLumpNumPwad(gpatch->wadnum, gpatch->lumpnum, PU_STATIC);
    HWR_MakePatch(patch, gpatch, gpatch->mipmap, true);

    // this is inefficient.. but the hardware patch in heap is purgeable so it
    // should not fragment memory, and besides the REAL cache here is the
    // hardware memory
    Z_Free(patch);
  }
...
void HWR_GetMappedPatch(GLPatch_t *gpatch, const UINT8 *colormap) {
  ...
  if (colormap == colormaps || colormap == NULL) {
    // Load the default (green) color in doom cache (temporary?) AND hardware
    // cache
    HWR_GetPatch(gpatch);
    return;
  }
  ...
  // not found, create it!
  // If we are here, the sprite with the current colormap is not already in
  // hardware memory
...
static void HWR_DrawFadeMaskInCache(GLMipmap_t *mipmap, INT32 pblockwidth,
                                    INT32 pblockheight,
                                    lumpnum_t fademasklumpnum, UINT16 fmwidth,
                                    UINT16 fmheight) {
  INT32 i, j;
  fixed_t posx, posy, stepx, stepy;
  UINT8 *block =
      mipmap->grInfo.data; // places the data directly into here, it already has
                           // the space allocated from HWR_ResizeBlock
  UINT8 *flat;
  UINT8 *dest, *src, texel;
  RGBA_t col;

  // Place the flats data into flat
  W_ReadLump(fademasklumpnum,
             Z_Malloc(W_LumpLength(fademasklumpnum), PU_HWRCACHE, &flat));
  ...
  for (j = 0; j < pblockheight; j++) {
    ...
    for (i = 0; i < pblockwidth; i++) {
      ...
      *dest = col.s.red; // take the red level of the colour and use it for
                         // alpha, as fademasks do
...
static void HWR_CacheFadeMask(GLMipmap_t *grMipmap, lumpnum_t fademasklumpnum) {
  ...
  // setup the texture info
  grMipmap->grInfo.format =
      GR_TEXFMT_ALPHA_8; // put the correct alpha levels straight in so I don't
  ...
  switch (size) {
  // None of these are powers of 2, so I'll need to do what is done for
  // textures and make them powers of 2 before they can be used
  case 256000: // 640x400
    fmwidth = 640;
    fmheight = 400;
    break;
  ...
  default: // Bad lump
    CONS_Alert(
        CONS_WARNING,
        "Fade mask lump of incorrect size, ignored\n"); // I should avoid this
                                                        // by checking the
                                                        // lumpnum in
                                                        // HWR_RunWipe
    break;
  ...
  HWR_DrawFadeMaskInCache(grMipmap, blockwidth, blockheight, fademasklumpnum,
                          fmwidth, fmheight);
