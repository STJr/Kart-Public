...
//  abuse???
// static source3D_data_t  p_attack_sdata; // ?? Now just holds precalculated
// player attack source data static source3D_data_t  p_default_sdata;// ?? ----
// // ---- // ---- // ---- player scream source data

static ambient_sdata_t
    ambient_sdata; // Precalculated datas of an ambient sound sources
...
void HW3S_SetSourcesNum(void) {
  ...
  if (cv_numChannels.value <= STATIC_SOURCES_NUM)
    I_Error(
        "HW3S_SetSourcesNum: Number of sound sources cannot be less than %d\n",
...
void HW3S_StopSoundByID(void *origin, sfxenum_t sfx_id) {
  ...
  for (snum = 0; snum < num_sources; snum++) {
    if (sources[snum].sfxinfo == &S_sfx[sfx_id] &&
        sources[snum].origin == origin) {
      HW3S_KillSource(snum);
      break;
    }
  }
...
//=============================================================================
static INT32 HW3S_GetSource(const void *origin, sfxinfo_t *sfxinfo,
                            boolean splitsound) {
  //
...
//=============================================================================
static void HW3S_FillSourceParameters(const mobj_t *origin,
...
{
        SINT8    *s = (SINT8 *)src + HEADER_SIZE, *d = (SINT8 *)dest +
HEADER_SIZE;
...
// INT32 HW3S_Start3DSound(const void *origin, source3D_data_t *source_parm,
// cone_def_t *cone_parm, channel_type_t channel, INT32 sfx_id, INT32 vol, INT32
// pitch);
//=============================================================================
INT32
HW3S_I_StartSound(const void *origin_p, source3D_data_t *source_parm,
                  channel_type_t c_type, sfxenum_t sfx_id, INT32 volume,
                  INT32 pitch, INT32 sep) {
  sfxinfo_t *sfx;
  ...
  mobj_t *listenmobj =
      players[displayplayers[0]]
          .mo; // TODO: Kart 4P does not support sounds properly here
  ...
  if (splitscreen && listenmobj2) // Copy the sound for the split player
  {
    if (c_type != CT_NORMAL && origin && (origin == listenmobj2)) {
      ...
      if (ambient_source.left.sfxinfo != sfx) {
        ...
        /*
                                        outphased_sfx.data =
           Z_Malloc(sfx_data.length, PU_STATIC, 0);
                                        make_outphase_sfx(outphased_sfx.data,
           sfx_data.data, sfx_data.length); outphased_sfx.length =
           sfx_data.length; outphased_sfx.id = sfx_data.id;
        */
        ambient_source.left.handle = HW3DS.pfnReloadSource(
            ambient_source.left.handle, (u_int)sfx->length);
        // ambient_source.right.handle =
        // HW3DS.pfnReloadSource(ambient_source.right.handle, &outphased_sfx);
        ambient_source.right.handle = HW3DS.pfnReloadSource(
            ambient_source.right.handle, (u_int)sfx->length);
  ...
  if (c_type != CT_NORMAL && origin && (origin == listenmobj)) {
    ...
    if (ambient_source.left.sfxinfo != sfx) {
      ...
      /*
         ...
         sfx_data.length; outphased_sfx.id = sfx_data.id;
      */
      ambient_source.left.handle =
          HW3DS.pfnReloadSource(ambient_source.left.handle, (u_int)sfx->length);
      // ambient_source.right.handle =
      // HW3DS.pfnReloadSource(ambient_source.right.handle, &outphased_sfx);
      ambient_source.right.handle = HW3DS.pfnReloadSource(
          ambient_source.right.handle, (u_int)sfx->length);
...
void HW3S_StartSound(const void *origin, sfxenum_t sfx_id) {
  HW3S_I_StartSound(origin, NULL, CT_NORMAL, sfx_id, 255, NORMAL_PITCH,
                    NORMAL_SEP);
}
...
void S_StartAttackSound(const void *origin, sfxenum_t sfx_id) {
  if (hws_mode != HWS_DEFAULT_MODE)
    HW3S_I_StartSound(origin, NULL, CT_ATTACK, sfx_id, 255, NORMAL_PITCH,
                      NORMAL_SEP);
  else
...
void S_StartScreamSound(const void *origin, sfxenum_t sfx_id) {
  if (hws_mode != HWS_DEFAULT_MODE)
    HW3S_I_StartSound(origin, NULL, CT_SCREAM, sfx_id, 255, NORMAL_PITCH,
                      NORMAL_SEP);
  else
    S_StartSound(origin, sfx_id);
}
...
FUNCMATH static inline float AmbientPos(angle_t an) {
  const fixed_t fm =
      FixedMul(FLOAT_TO_FIXED(MIN_DISTANCE), FINESINE(an >> ANGLETOFINESHIFT));
  return FIXED_TO_FLOAT(fm);
}

//=============================================================================
INT32
HW3S_Init(I_Error_t FatalErrorFunction, snddev_t *snd_dev) {
  INT32 succ;
  ...
  if (HW3DS.pfnStartup(FatalErrorFunction, snd_dev)) {
    ...
    M_Memcpy(&ambient_sdata.right, &ambient_sdata.left,
             sizeof(source3D_data_t));

    ambient_sdata.right.pos.x = -ambient_sdata.left.pos.x;
    ambient_source.left.handle =
        HW3DS.pfnAddSource(&ambient_sdata.left, sfx_None);
    ambient_source.right.handle =
        HW3DS.pfnAddSource(&ambient_sdata.right, sfx_None);
...
//=============================================================================
INT32
HW3S_GetVersion(void) { return HW3DS.pfnGetHW3DSVersion(); }
...
//=============================================================================
INT32
HW3S_SoundIsPlaying(INT32 handle) { return HW3DS.pfnIsPlaying(handle); }

INT32
HW3S_OriginPlaying(void *origin) {
  INT32 snum;
...
INT32
HW3S_SoundPlaying(void *origin, sfxenum_t id) {
  INT32 snum;
...
void HW3S_SetSfxVolume(INT32 volume) { HW3DS.pfnSetGlobalSfxVolume(volume); }
...
void HW3S_UpdateSources(void) {
  ...
  if (splitscreen)
    listener2 = players[displayplayers[1]].mo;
...
#else
      if (src->origin && listener != src->origin &&
          !(listener2 && src->origin == listener2))
        HW3S_Update3DSource(src); // Update positional sources
      (void)pitch;
      (void)sep;
      (void)volume;
...
void HW3S_Shutdown(void) { HW3DS.pfnShutdown(); }
