...
#define RELEASE_3DBUFFER(buf)                                                  \
  {                                                                            \
    if (buf) {                                                                 \
      IDirectSound3DBuffer_Release(buf);                                       \
      (buf) = NULL;                                                            \
    }                                                                          \
  }

// Default flags for buffers
#define _2DSOURCE_FLAGS                                                        \
  (DSBCAPS_CTRLPAN | DSBCAPS_CTRLVOLUME | DSBCAPS_STICKYFOCUS |                \
   DSBCAPS_STATIC | DSBCAPS_CTRLFREQUENCY)
#define _3DSOURCE_FLAGS                                                        \
  (DSBCAPS_CTRL3D | DSBCAPS_STATIC | DSBCAPS_MUTE3DATMAXDISTANCE |             \
...
static BOOL reallocate_stack(void) {
  ...
  if (_stack)
    new_stack = realloc(_stack, sizeof(stack_t) *
                                    (allocated_sounds + SOUND_ALLOCATE_DELTA));
  else
    new_stack =
        malloc(sizeof(stack_t) * (allocated_sounds + SOUND_ALLOCATE_DELTA));
  if (new_stack) {
    _stack = new_stack;
    ZeroMemory(&_stack[allocated_sounds],
               SOUND_ALLOCATE_DELTA * sizeof(stack_t));
    allocated_sounds += SOUND_ALLOCATE_DELTA;
  }
...
static LPDIRECTSOUNDBUFFER create_buffer(void *data, INT32 length, BOOL as3d) {
  ...
  wfm.nChannels = 1;
  wfm.nSamplesPerSec =
      data ? *((UINT16 *)data + 1) : srate; // mostly 11025, but some at 22050.
  ...
  if (as3d)
    dsbdesc.guid3DAlgorithm =
        (virtualization ? DS3DALG_HRTF_FULL : DS3DALG_DEFAULT);
#endif
  ...
  if (data) {
    hr = IDirectSoundBuffer_Lock(dsbuffer, 0, length, &lpvAudio1, &dwBytes1,
                                 &lpvAudio2, &dwBytes2, 0);
    ...
    // copy wave data into the buffer (note: dwBytes1 should equal to
    ...
    hr = IDirectSoundBuffer_Unlock(dsbuffer, lpvAudio1, dwBytes1, lpvAudio2,
                                   dwBytes2);
 ...
 ***************************************************************
 */
static LPDIRECTSOUND3DBUFFER create_3dbuffer(LPDIRECTSOUNDBUFFER dsbuffer,
                                             LPDIRECTSOUND3DBUFFER source3d) {
  HRESULT hr;

  hr = IDirectSoundBuffer_QueryInterface(dsbuffer, &IID_IDirectSound3DBuffer,
                                         (void **)&source3d);
...
static INT32 find_handle(INT32 new_sfx_id, INT32 new_is3d) {
  ...
  for (free_sfx = 0, snd = _stack; free_sfx < allocated_sounds;
       snd++, free_sfx++) {
    ...
    /*
                    if (!is_playing(snd))
                    {
                            if (same_sfx == 0 && snd->sfx_id == new_sfx_id &&
       snd->dsbuffer)
                            {
                                    same_sfx = i;
                                    continue;
  ...
  if (free_sfx == allocated_sounds) {
    DBG_Printf(
        "No free or same sfx found so increase stack (currently %d srcs)\n",
        allocated_sounds);
...
static void Update2DSoundPanning(LPDIRECTSOUNDBUFFER lpSnd, INT32 sep) {
  ...
  // DSBPAN_RANGE)/SEP_RANGE - DSBPAN_RIGHT));
}
...
EXPORT BOOL HWRAPI(Startup)(I_Error_t FatalErrorFunction, snddev_t *snd_dev) {
  ...
  hr =
      IDirectSound_SetCooperativeLevel(DSnd, snd_dev->hWnd, snd_dev->cooplevel);
  if (FAILED(hr)) {
  ...
  DBG_Printf("   Driver is %scertified.\n",
             (dscaps.dwFlags & DSCAPS_CERTIFIED) == DSCAPS_CERTIFIED ? ""
                                                                     : "not ");
  ...
  case DSSPEAKER_STEREO:
    DBG_Printf(
        "stereo with %s geometry ",
        speaker_geometry == DSSPEAKER_GEOMETRY_WIDE ? "wide (arc of 20 deg.)"
        : speaker_geometry == DSSPEAKER_GEOMETRY_NARROW
            ? "narrow (arc of 10 deg.)"
        : speaker_geometry == DSSPEAKER_GEOMETRY_MIN ? "min (arc of 5 deg.)"
        : speaker_geometry == DSSPEAKER_GEOMETRY_MAX ? "max (arc of 180 deg.)"
                                                     : "unknown");
    break;
  default:
  ...
  hr = IDirectSoundBuffer_QueryInterface(
      PrimaryBuffer, &IID_IDirectSound3DListener, (void **)&Listener);
  ...
  IDirectSound3DListener_SetDistanceFactor(Listener, 1.0f / 72.0f,
                                           DS3D_IMMEDIATE);
  IDirectSound3DListener_SetRolloffFactor(Listener, 1.6f, DS3D_IMMEDIATE);
  // IDirectSound3DListener_SetRolloffFactor(Listener, DS3D_MAXROLLOFFFACTOR,
  // DS3D_IMMEDIATE);
...
static stack_t *setup_source(INT32 handle, sfx_data_t *sfx, BOOL is_3dsource) {
  ...
  if (dsbuffer) {
    ...
    if (sfx) {
      ...
      freq = (DWORD)((float)(*((UINT16 *)sfx->data + 1)) *
...
EXPORT INT32 HWRAPI(Add3DSource)(source3D_data_t *src, sfx_data_t *sfx) {
  ...
  if (snd) {
    ...
    IDirectSound3DBuffer_SetVelocity(source, src->pos.momx, src->pos.momz,
    src->pos.momy, update_mode); IDirectSound3DBuffer_SetMode(source,
    src->head_relative?DS3DMODE_HEADRELATIVE:DS3DMODE_NORMAL, update_mode);*/
...
#endif
    snd->parameters.dwMode =
        src->head_relative ? DS3DMODE_HEADRELATIVE : DS3DMODE_NORMAL;
    ...
    IDirectSound3DBuffer_SetAllParameters(snd->dsbuffer3D, &snd->parameters,
                                          DS3D_IMMEDIATE);
...
EXPORT void HWRAPI(BeginFrameUpdate)(void) { update_mode = DS3D_DEFERRED; }
...
EXPORT void HWRAPI(UpdateListener)(listener_data_t *data) {
  ...
  */
  ...
  // IDirectSound3DListener_SetAllParameters(Listener, &listener_parms,
  // DS3D_IMMEDIATE);
  IDirectSound3DListener_SetAllParameters(Listener, &listener_parms,
                                          update_mode);
}
...
EXPORT void HWRAPI(UpdateListener2)(listener_data_t *data) {
  ...
  */
  ...
  CopyMemory(&listener_parms.vVelocity, &velocity, sizeof(D3DVECTOR));
  // IDirectSound3DListener_SetAllParameters(Listener, &listener_parms,
  // DS3D_IMMEDIATE); Alam_GBC: 2 Listeners?
  // IDirectSound3DListener_SetAllParameters(Listener, &listener_parms,
  // update_mode);
}
...
EXPORT INT32 HWRAPI(SetCone)(INT32 handle, cone_def_t *cone_def) {
  ...
  if (snd->dsbuffer3D) {
    ...
    snd->parameters.lConeOutsideVolume =
        recalc_volume(cone_def->outer_gain, 256);

    return IDirectSound3DBuffer_SetAllParameters(snd->dsbuffer3D,
                                                 &snd->parameters, update_mode);
  }
...
EXPORT void HWRAPI(Update3DSource)(INT32 handle, source3D_pos_t *data) {
  ...
  if (snd->dsbuffer3D) {
    ...
    // snd->parameters.dwMode =
    // pos->head_realtive?DS3DMODE_HEADREALTIVE:DS3DMODE_NORMAL;
...
EXPORT INT32 HWRAPI(Reload3DSource)(INT32 handle, sfx_data_t *data) {
  ...
  if (snd) {
    snd->permanent = perm;
    IDirectSound3DBuffer_SetAllParameters(snd->dsbuffer3D, &temp,
                                          DS3D_IMMEDIATE);
