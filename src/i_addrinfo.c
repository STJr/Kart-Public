 ...
 * i686-w64-mingw32-gcc -g -DTESTCASE -Dtest_stub -UUSE_WINSOCK2 -Wall -W
 * i_addrinfo.c -o stub.exe -lwsock32 for 2.2, add -DUSE_WINSOCK2 -lws2_32
 * i686-w64-mingw32-gcc -g -DTESTCASE -Utest_stub -DUSE_WINSOCK2 -Wall -W
 * i_addrinfo.c -o real.exe -lws2_32
 */
...
static HMODULE WS_getfunctions(HMODULE tmp) {
  if (tmp != NULL) {
    ...
    if (WS_getaddrinfo == NULL)
      return NULL;
    WS_freeaddrinfo =
        (p_freeaddrinfo)(LPVOID)GetProcAddress(tmp, "freeaddrinfo");
...
int I_getaddrinfo(const char *node, const char *service,
  ...
  if (node && hostlookup) {
    ...
    if (nodename == NULL) {
      ...
      while (nodename->h_addr_list[addrlen] != NULL)
        addrlen++;
  } else if (node) {
    if (inet_aton(node, &ipv4) != 0)
      addrlen = 1;
...
#define HAVE_NTOP
static const char *inet_ntopA(int af, const void *cp, char *buf,
                              socklen_t len) {
