...
//-----------------------------------------------------------------------------
/// \file  d_clisrv.c
/// \brief SRB2 Network game communication and protocol, all OS independent
/// parts.
...
char motd[254], server_context[8]; // Message of the Day, Unique Context (even
                                   // without Mumble support)
...
static tic_t freezetimeout[MAXNETNODES]; // Until when can this node freeze the
                                         // server before getting a timeout?

UINT16 pingmeasurecount = 1;
UINT32 realpingtable[MAXPLAYERS];   // the base table of ping where an average
                                    // will be sent to everyone.
UINT32 playerpingtable[MAXPLAYERS]; // table of player latency values.
SINT8 nodetoplayer[MAXNETNODES];
SINT8 nodetoplayer2[MAXNETNODES]; // say the numplayer for this node if any
                                  // (splitscreen)
SINT8 nodetoplayer3[MAXNETNODES]; // say the numplayer for this node if any
                                  // (splitscreen == 2)
SINT8 nodetoplayer4[MAXNETNODES]; // say the numplayer for this node if any
                                  // (splitscreen == 3)
...
// Resynching shit!
static UINT32 resynch_score[MAXNETNODES]; // "score" for kicking -- if this gets
                                          // too high then cfail kick
static UINT16 resynch_delay[MAXNETNODES]; // delay time before the player can be
                                          // considered to have desynched
static UINT32 resynch_status[MAXNETNODES]; // 0 bit means synched for that
                                           // player, 1 means possibly desynched
static UINT8 resynch_sent[MAXNETNODES]
                         [MAXPLAYERS]; // what synch packets have we attempted
                                       // to send to the player
static UINT8 resynch_inprogress[MAXNETNODES];
static UINT8 resynch_local_inprogress =
    false; // WE are desynched and getting packets to fix it.
static UINT8 player_joining = false;
UINT8 hu_resynching = 0;

// kart, true when a player is connecting or disconnecting so that the gameplay
...
boolean serverisfull =
    false; // lets us be aware if the server was full after we check files, but
           // before downloading, so we can ask if the user still wants to
           // download or not
...
consvar_t cv_showjoinaddress = {"showjoinaddress",
                                "On",
                                CV_SAVE,
                                CV_OnOff,
                                NULL,
...
static CV_PossibleValue_t playbackspeed_cons_t[] = {
    {1, "MIN"}, {10, "MAX"}, {0, NULL}};
consvar_t cv_playbackspeed = {"playbackspeed",
                              "1",
                              0,
                              playbackspeed_cons_t,
                              NULL,
                              0,
                              NULL,
                              NULL,
...
consvar_t cv_httpsource = {"http_source", "",   CV_SAVE, NULL, NULL, 0,
                           NULL,          NULL, 0,       0,    NULL};

consvar_t cv_kicktime = {"kicktime", "10", CV_SAVE, CV_Unsigned, NULL, 0,
                         NULL,       NULL, 0,       0,           NULL};

static inline void *G_DcpyTiccmd(void *dest, const ticcmd_t *src,
                                 const size_t n) {
...
void SendNetXCmd(netxcmd_t id, const void *param, size_t nparam) {
  if (localtextcmd[0] + 2 + nparam > MAXTEXTCMD) {
    ...
    CONS_Alert(CONS_ERROR,
               M_GetText("NetXCmd buffer full, cannot add netcmd %d! (size: "
                         "%d, needed: %s)\n"),
...
static UINT8 *D_GetExistingTextcmd(tic_t tic, INT32 playernum) {
  ...
  if (textcmdtic) {
    textcmdplayer_t *textcmdplayer =
        textcmdtic->playercmds[playernum & (TEXTCMD_HASH_SIZE - 1)];
...
static UINT8 *D_GetTextcmd(tic_t tic, INT32 playernum) {
  ...
  if (!textcmdplayer) {
    textcmdplayer = *tcpprev =
        Z_Calloc(sizeof(textcmdplayer_t), PU_STATIC, NULL);
...
static void ExtraDataTicker(void) {
        ...
        while (curpos < bufferend) {
          if (*curpos < MAXNETXCMD && listnetxcmd[*curpos]) {
            ...
            CONS_Alert(CONS_WARNING,
                       M_GetText("Got unknown net command [%s]=%d (max %d)\n"),
  ...
  // If you are the server, you need to remember them until every client has
  // been aknowledged, because if you need to resend a PT_SERVERTICS packet, you
  // need to put the commands in it
  if (client)
...
static void resynch_read_player(resynch_pak *rsp) {
  ...
  // At this point, the player should have a body, whether they were respawned
  // or not.
...
static inline void resynch_write_ctf(resynchend_pak *rst) {
  ...
  for (i = 0, mflag = redflag; i < 2; ++i, mflag = blueflag) {
    ...
    if (!mflag) {
      ...
      {
        CONS_Alert(CONS_ERROR,
                   "One of the flags has gone completely missing...\n");
        rst->flagplayer[i] = -2;
      }
      continue;
    }
...
static inline void resynch_read_ctf(resynchend_pak *p) {
  ...
  else if (p->flagplayer[0] != -1) // Held by a player
  {
    if (!playeringame[p->flagplayer[0]])
      I_Error("Invalid red flag player %d who isn't in the game!",
              (INT32)p->flagplayer[0]);
  ...
  else if (p->flagplayer[1] != -1) // Held by a player
  {
    if (!playeringame[p->flagplayer[1]])
      I_Error("Invalid blue flag player %d who isn't in the game!",
              (INT32)p->flagplayer[1]);
...
static inline void resynch_read_others(resynchend_pak *p) {
  ...
  for (i = 0; i < MAXPLAYERS; ++i) {
    ...
    players[i].ctfteam = (INT32)LONG(
        p->ctfteam[i]); // no, 0 does not mean spectator, at least not in Match
...
static void SV_RequireResynch(INT32 node) {
  ...
  resynch_inprogress[node] =
      true; // so we know to send a PT_RESYNCHEND after sync

  // Initial setup
  memset(resynch_sent[node], 0, MAXPLAYERS);
  for (i = 0; i < MAXPLAYERS; ++i) {
    if (!playeringame[i]) // Player not in game so just drop it from required
                          // synch
      resynch_status[node] &= ~(1 << i);
    else if (playernode[i] == node)
      ;  // instantly update THEIR position
...
static inline void CL_DrawConnectionStatus(void) {
  ...
  if (cl_mode != CL_DOWNLOADFILES && cl_mode != CL_LOADFILES &&
    ...
    V_DrawCenteredString(BASEVIDWIDTH / 2, BASEVIDHEIGHT - 24 - 24, V_YELLOWMAP,
                         "Press ESC to abort");

    for (i = 0; i < 16; ++i)
      V_DrawFill((BASEVIDWIDTH / 2 - 128) + (i * 16), BASEVIDHEIGHT - 24, 16, 8,
    ...
    case CL_DOWNLOADSAVEGAME:
      if (lastfilenum != -1) {
        ...
        Net_GetNetStat();
        V_DrawString(BASEVIDWIDTH / 2 - 128, BASEVIDHEIGHT - 24,
                     V_20TRANS | V_MONOSPACE,
                     va(" %4uK", fileneeded[lastfilenum].currentsize >> 10));
        V_DrawRightAlignedString(BASEVIDWIDTH / 2 + 128, BASEVIDHEIGHT - 24,
                                 V_20TRANS | V_MONOSPACE,
    ...
    if (cl_mode == CL_CHECKFILES) {
      ...
      V_DrawCenteredString(BASEVIDWIDTH / 2, BASEVIDHEIGHT - 24 - 24,
                           V_YELLOWMAP, "Press ESC to abort");
      ...
      // Loading progress
      V_DrawCenteredString(BASEVIDWIDTH / 2, BASEVIDHEIGHT - 24 - 32,
                           V_YELLOWMAP, "Checking server addons...");
      totalfileslength = (INT32)((checkednum / (double)(fileneedednum)) * 256);
      M_DrawTextBox(BASEVIDWIDTH / 2 - 128 - 8, BASEVIDHEIGHT - 24 - 8, 32, 1);
      V_DrawFill(BASEVIDWIDTH / 2 - 128, BASEVIDHEIGHT - 24, 256, 8, 175);
      V_DrawFill(BASEVIDWIDTH / 2 - 128, BASEVIDHEIGHT - 24, totalfileslength,
                 8, 160);
      V_DrawCenteredString(BASEVIDWIDTH / 2, BASEVIDHEIGHT - 24,
                           V_20TRANS | V_MONOSPACE,
      ...
      // Loading progress
      V_DrawCenteredString(BASEVIDWIDTH / 2, BASEVIDHEIGHT - 24 - 32,
                           V_YELLOWMAP, "Loading server addons...");
      totalfileslength =
          (INT32)((loadcompletednum / (double)(fileneedednum)) * 256);
      M_DrawTextBox(BASEVIDWIDTH / 2 - 128 - 8, BASEVIDHEIGHT - 24 - 8, 32, 1);
      V_DrawFill(BASEVIDWIDTH / 2 - 128, BASEVIDHEIGHT - 24, 256, 8, 175);
      V_DrawFill(BASEVIDWIDTH / 2 - 128, BASEVIDHEIGHT - 24, totalfileslength,
                 8, 160);
      V_DrawCenteredString(
          BASEVIDWIDTH / 2, BASEVIDHEIGHT - 24, V_20TRANS | V_MONOSPACE,
      ...
      V_DrawCenteredString(BASEVIDWIDTH / 2, BASEVIDHEIGHT - 58 - 14,
                           V_YELLOWMAP, "Press ESC to abort");
      ...
      if (strlen(filename) > sizeof(tempname) - 1) // too long to display fully
      {
        ...
        snprintf(tempname, sizeof(tempname), "%.14s...%.10s", filename,
                 filename + endhalfpos);
      ...
      V_DrawCenteredString(BASEVIDWIDTH / 2, BASEVIDHEIGHT - 58 - 30, 0,
                           va(M_GetText("%s downloading"),
                              ((cl_mode == CL_DOWNLOADFILES) ? "\x82"
                                                               "HTTP"
                                                             : "\x85"
                                                               "Direct")));
      V_DrawCenteredString(BASEVIDWIDTH / 2, BASEVIDHEIGHT - 58 - 22,
                           V_YELLOWMAP, va(M_GetText("\"%s\""), tempname));
      V_DrawString(BASEVIDWIDTH / 2 - 128, BASEVIDHEIGHT - 58,
                   V_20TRANS | V_MONOSPACE,
                   va(" %4uK/%4uK", fileneeded[lastfilenum].currentsize >> 10,
                      file->totalsize >> 10));
      V_DrawRightAlignedString(BASEVIDWIDTH / 2 + 128, BASEVIDHEIGHT - 58,
                               V_20TRANS | V_MONOSPACE,
                               va("%3.1fK/s ", ((double)getbps) / 1024));

      // Download progress

      if (fileneeded[lastfilenum].currentsize !=
          fileneeded[lastfilenum].totalsize)
        totaldldsize =
            downloadcompletedsize +
            fileneeded[lastfilenum].currentsize; // Add in single file progress
                                                 // download if applicable
      else
        totaldldsize = downloadcompletedsize;

      V_DrawCenteredString(BASEVIDWIDTH / 2, BASEVIDHEIGHT - 24 - 14,
                           V_YELLOWMAP, "Overall Download Progress");
      totalfileslength =
          (INT32)((totaldldsize / (double)totalfilesrequestedsize) * 256);
      M_DrawTextBox(BASEVIDWIDTH / 2 - 128 - 8, BASEVIDHEIGHT - 24 - 8, 32, 1);
      V_DrawFill(BASEVIDWIDTH / 2 - 128, BASEVIDHEIGHT - 24, 256, 8, 175);
      V_DrawFill(BASEVIDWIDTH / 2 - 128, BASEVIDHEIGHT - 24, totalfileslength,
                 8, 160);
      ...
      if (totalfilesrequestedsize >> 20 >= 10) // display in MB if over 10MB
        V_DrawString(BASEVIDWIDTH / 2 - 128, BASEVIDHEIGHT - 24,
                     V_20TRANS | V_MONOSPACE,
                     va(" %4uM/%4uM", totaldldsize >> 20,
                        totalfilesrequestedsize >> 20));
      else
        V_DrawString(BASEVIDWIDTH / 2 - 128, BASEVIDHEIGHT - 24,
                     V_20TRANS | V_MONOSPACE,
                     va(" %4uK/%4uK", totaldldsize >> 10,
                        totalfilesrequestedsize >> 10));

      V_DrawRightAlignedString(
          BASEVIDWIDTH / 2 + 128, BASEVIDHEIGHT - 24, V_20TRANS | V_MONOSPACE,
      ...
      for (i = 0; i < 16; ++i)
        V_DrawFill((BASEVIDWIDTH / 2 - 128) + (i * 16), BASEVIDHEIGHT - 24, 16,
                   8, palstart + ((animtime - i) & 15));
...
static void CopyCaretColors(char *p, const char *s, int n) {
  char *t;
  int m;
  int c;
  if (!n)
...
static void SV_SendServerInfo(INT32 node, tic_t servertime) {
  ...
  netbuffer->u.serverinfo.maxplayer = (UINT8)(min(
      (dedicated ? MAXPLAYERS - 1 : MAXPLAYERS), cv_maxplayers.value));

  // SRB2Kart: Vanilla's gametype constants for MS support
  netbuffer->u.serverinfo.gametype =
      (UINT8)((gt == GT_MATCH) ? VANILLA_GT_MATCH : VANILLA_GT_RACE);

  netbuffer->u.serverinfo.modifiedgame = (UINT8)modifiedgame;
  netbuffer->u.serverinfo.cheatsenabled = CV_CheatsEnabled();

  netbuffer->u.serverinfo.kartvars =
      (UINT8)((cv_kartspeed.value & SV_SPEEDMASK) |
  ...
  if (!(mapheaderinfo[gamemap - 1]->levelflags & LF_NOZONE) &&
      !(mapheaderinfo[prevmap]->zonttl[0]))
  ...
  if (!(mapheaderinfo[gamemap - 1]->menuflags & LF2_HIDEINMENU) &&
    ...
    // strncpy(netbuffer->u.serverinfo.maptitle, (char
    // *)mapheaderinfo[gamemap-1]->lvlttl, 33);
    //  set up the levelstring
    if (netbuffer->u.serverinfo.iszone ||
        (mapheaderinfo[gamemap - 1]->levelflags & LF_NOZONE)) {
      if (mapheaderinfo[gamemap - 1]->actnum[0])
        snprintf(netbuffer->u.serverinfo.maptitle, 33, "%s %s",
                 mapheaderinfo[gamemap - 1]->lvlttl,
                 mapheaderinfo[gamemap - 1]->actnum);
      else
        snprintf(netbuffer->u.serverinfo.maptitle, 33, "%s",
                 mapheaderinfo[gamemap - 1]->lvlttl);
    } else {
      if (mapheaderinfo[gamemap - 1]->actnum[0]) {
        if (snprintf(netbuffer->u.serverinfo.maptitle, 33, "%s %s %s",
                     mapheaderinfo[gamemap - 1]->lvlttl,
                     mapheaderinfo[gamemap - 1]->zonttl,
                     mapheaderinfo[gamemap - 1]->actnum) < 0) {
          // If there's an encoding error, send UNKNOWN, we accept that the
          // above may be truncated
          strncpy(netbuffer->u.serverinfo.maptitle, "Unknown", 33);
        }
      } else {
        if (snprintf(netbuffer->u.serverinfo.maptitle, 33, "%s %s",
                     mapheaderinfo[gamemap - 1]->lvlttl,
                     mapheaderinfo[gamemap - 1]->zonttl) < 0) {
          // If there's an encoding error, send UNKNOWN, we accept that the
          // above may be truncated
          strncpy(netbuffer->u.serverinfo.maptitle, "Unknown", 33);
        }
  ...
  if (snprintf(netbuffer->u.serverinfo.httpsource, mirror_length + 1, "%s",
               httpurl) < 0)
    // If there's an encoding error, send nothing, we accept that the above may
    // be truncated
...
static void SV_SendPlayerInfo(INT32 node) {
  ...
  for (i = 0; i < MSCOMPAT_MAXPLAYERS; i++) {
    ...
    strncpy(netbuffer->u.playerinfo[i].name, (const char *)&player_names[i],
            MAXPLAYERNAME + 1);
    ...
    netbuffer->u.playerinfo[i].timeinserver =
        SHORT((UINT16)(players[i].jointime / TICRATE));
    netbuffer->u.playerinfo[i].skin = (UINT8)players[i].skin;

    // Extra data
    // Kart has extra skincolors, so we can't use this
    netbuffer->u.playerinfo[i].data =
        0; // netbuffer->u.playerinfo[i].data = players[i].skincolor;
...
static boolean SV_SendServerConfig(INT32 node) {
  ...
  // we fill these structs with FFs so that any players not in game get sent as
  // 0xFFFF which is nice and easy for us to detect
  memset(netbuffer->u.servercfg.playerskins, 0xFF,
         sizeof(netbuffer->u.servercfg.playerskins));
  memset(netbuffer->u.servercfg.playercolor, 0xFF,
         sizeof(netbuffer->u.servercfg.playercolor));

  memset(netbuffer->u.servercfg.adminplayers, -1,
         sizeof(netbuffer->u.servercfg.adminplayers));
  ...
  netbuffer->u.servercfg.maxplayer = (UINT8)(min(
      (dedicated ? MAXPLAYERS - 1 : MAXPLAYERS), cv_maxplayers.value));
  ...
  CV_SaveNetVars(&p, false);
  {
    ...
    if (debugfile) {
      fprintf(debugfile,
              "ServerConfig Packet about to be sent, size of packet:%s to "
              "node:%d\n",
              sizeu1(len), node);
    }
#endif
...
static void SV_SendSaveGame(INT32 node) {
  ...
  if ((compressedlen = lzf_compress(
           savebuffer + sizeof(UINT32), length - sizeof(UINT32),
           compressedsave + sizeof(UINT32), length - sizeof(UINT32) - 1))) {
  ...
  freezetimeout[node] = I_GetTime() + jointimeout +
                        length / 1024; // 1 extra tic for each kilobyte
}

#ifdef DUMPCONSISTENCY
#define TMPSAVENAME "badmath.sav"
static consvar_t cv_dumpconsistency = {"dumpconsistency",
                                       "Off",
                                       CV_NETVAR,
                                       CV_OnOff,
                                       NULL,
...
static void CL_LoadReceivedSavegame(void) {
  ...
  if (decompressedlen > 0) {
    ...
    lzf_decompress(save_p, length - sizeof(UINT32), decompressedbuffer,
                   decompressedlen);
    Z_Free(savebuffer);
...
static void M_ConfirmConnect(event_t *ev) {
  ...
  if (ev->type == ev_keydown) {
    if (ev->data1 == ' ' || ev->data1 == 'y' || ev->data1 == KEY_ENTER ||
        ev->data1 == gamecontrol[gc_accelerate][0] ||
        ev->data1 == gamecontrol[gc_accelerate][1]) {
      ...
      M_ClearMenus(true);
               ...
               ev->data1 == gamecontrol[gc_brake][0] ||
               ev->data1 == gamecontrol[gc_brake][1]) {
      cl_mode = CL_ABORTED;
      M_ClearMenus(true);
    }
  }
...
static boolean CL_FinishedFileList(void) {
  ...
  } else if (i == 3) // too many files
  {
    ...
    CL_Reset();
    D_StartTitle();
    M_StartMessage(M_GetText("You have too many WAD files loaded\n"
  ...
  } else if (i == 2) // cannot join for some reason
  {
    ...
    CL_Reset();
    D_StartTitle();
    M_StartMessage(M_GetText("You have the wrong addons loaded.\n\n"
...
#endif
    {
      if (!CL_CheckDownloadable()) // nope!
      {
        ...
        CL_Reset();
        D_StartTitle();
        M_StartMessage(M_GetText("An error occured when trying to\n"
      ...
      if (serverisfull)
        M_StartMessage(
            va(M_GetText(
                   ...
                   "\n"
                   "You may download, load server addons, and wait for a "
                   "slot.\n"
                   "\n"
                   "Press ACCEL to continue or BRAKE to cancel.\n\n"),
               downloadsize),
 ...
 *
 * \param asksent The last time we asked the server to join. We re-ask every
 * second in case our request got lost in transmit. \return False if the
 * connection was aborted \sa CL_ServerConnectionTicker \sa CL_ConnectToServer
 *
 */
...
#else
      if (serverlist[i].info.httpsource[0])
        CONS_Printf("We received a http url from the server, however it will "
                    "not be used as this build lacks curl support (%s)\n",
                    serverlist[i].info.httpsource);
#endif
      D_ParseFileneeded(serverlist[i].info.fileneedednum,
                        serverlist[i].info.fileneeded, 0);
 ...
 * \param asksent The last time we asked the server to join. We re-ask every
 * second in case our request got lost in transmit. \return False if the
 * connection was aborted \sa CL_ServerConnectionSearchTicker \sa
 * CL_ConnectToServer
 *
 */
static boolean CL_ServerConnectionTicker(const char *tmpsave, tic_t *oldtic,
                                         tic_t *asksent) {
  boolean waitmore;
  INT32 i;
  ...
  case CL_ASKFULLFILELIST:
    if (cl_lastcheckedfilecount == UINT16_MAX) // All files retrieved
      cl_mode = CL_CHECKFILES;
    else if (fileneedednum != cl_lastcheckedfilecount ||
             I_GetTime() >= *asksent) {
  ...
  case CL_DOWNLOADHTTPFILES:
    ...
    if (curl_failedwebdownload && !curl_transfers) {
      CONS_Printf("One or more files failed to download, falling back to "
                  "internal downloader\n");
      cl_mode = CL_CHECKFILES;
      break;
    }
  ...
  case CL_LEGACYREQUESTFAILED: {
    ...
    CL_Reset();
    D_StartTitle();
    M_StartMessage(M_GetText("The direct download encountered an error.\n"
  ...
  case CL_ASKJOIN:
    if (firstconnectattempttime + NEWTICRATE * 300 < I_GetTime() && !server) {
      ...
      CL_Reset();
      D_StartTitle();
      M_StartMessage(M_GetText("5 minute wait time exceeded.\n"
  ...
  if (*oldtic != I_GetTime()) {
    ...
    if (!modeattacking &&
        (key == KEY_ESCAPE || key >= KEY_MOUSE1 || cl_mode == CL_ABORTED)) {
      CONS_Printf(M_GetText("Network game synchronization aborted.\n"));
      D_QuitNetGame();
      CL_Reset();
      D_StartTitle();
...
static void CL_ConnectToServer(void) {
  ...
  if (i != -1) {
    ...
    CONS_Printf(M_GetText("Version: %d.%d\n"), serverlist[i].info.version,
                serverlist[i].info.subversion);
  }
      ...
      for (i = 0; i < MAXNETNODES; i++)
        if (nodeingame[i])
          pnumnodes++;
             ...
             (client || (server && nodewaited <= pnumnodes))));
...
// If you're a community contributor looking to improve how bans are written,
// please offer your changes back to our Git repository. Kart Krew reserve the
// right to utilise format numbers in use by community builds for different
// layouts.
#define BANFORMAT 1
...
void D_SaveBan(void) {
  ...
  if (!f) {
    CONS_Alert(CONS_WARNING,
               M_GetText("Could not save ban list into ban.txt\n"));
    return;
  }
...
void D_LoadBan(boolean warning) {
  ...
  if (!f) {
    if (warning)
      CONS_Alert(CONS_WARNING,
                 M_GetText("Could not open ban.txt for ban list\n"));
    return;
  }
  ...
  for (i = 0; fgets(buffer, (int)sizeof(buffer), f); i++) {
    ...
    if (i == 0 && !strncmp(address, "BANFORMAT", 9)) {
      ...
      default: {
        fclose(f);
        CONS_Alert(CONS_WARNING,
                   "Could not load unknown ban.txt for ban list (BANFORMAT "
                   "%s, expected %d)\n",
                   mask, BANFORMAT);
        return;
      }
    ...
    {
      CONS_Alert(CONS_WARNING,
                 "\"%s/%s\" is not a valid IP address, discarding...\n",
                 address, mask);
      continue;
    }
    ...
    if (banmode == 0) {
      unbanTime = NO_BAN_TIME;
      username =
          NULL; // not guaranteed to be accurate, but only sane substitute
      reason = strtok(NULL, "\r\n");
  ...
  if (malformed) {
    CONS_Alert(CONS_WARNING,
               "One or more lines of ban.txt are malformed. The game can "
               "correct for this, but some data may be lost.\n");
  }
...
static void Command_connect(void) {
  if (COM_Argc() < 2 || *COM_Argv(1) == 0) {
    CONS_Printf(
        M_GetText("Connect <serveraddress> (port): connect to a server\n"
  ...
  if (Playing() || demo.title) {
    CONS_Printf(M_GetText(
        "You cannot connect while in a game. End this game first.\n"));
    return;
  }
  ...
  if (!stricmp(COM_Argv(1), "self")) {
    ...
    if (netgame && !stricmp(COM_Argv(1), "node")) {
      ...
      if (!stricmp(COM_Argv(1), "any"))
        ...
        CONS_Alert(CONS_ERROR, M_GetText("There is no server identification "
...
void CL_ClearPlayer(INT32 playernum) {
  if (players[playernum].mo) {
    ...
    if ((players[playernum].pflags & PF_NIGHTSMODE) &&
        players[playernum].mo->tracer)
...
void CL_RemovePlayer(INT32 playernum, INT32 reason) {
  ...
  if (server && !demo.playback) {
    ...
    // playerpernode[node] = 0; // It'd be better to remove them all at once,
    // but ghosting happened, so continue to let CL_RemovePlayer do it
    // one-by-one
    playerpernode[node]--;
  ...
  if (gametype == GT_CTF)
    P_PlayerFlagBurst(&players[playernum],
                      false); // Don't take the flag with you!
  ...
  if (playernum == displayplayers[0] && !demo.playback)
    displayplayers[0] =
        consoleplayer; // don't look through someone's view who isn't there
...
static void Command_Nodes(void) {
  ...
  for (i = 0; i < MAXPLAYERS; i++) {
    if (playeringame[i]) {
      ...
      if (I_GetNodeAddress &&
          (address = I_GetNodeAddress(playernode[i])) != NULL)
        CONS_Printf(" - %s", address);
...
static void Command_Ban(void) {
  if (COM_Argc() < 2) {
    CONS_Printf(M_GetText(
        "ban <playername/playernum> <reason>: ban and kick a player\n"));
    return;
  }
...
static void Command_BanIP(void) {
  ...
  if (server) // Only the server can use this, otherwise does nothing.
  {
    ...
    if (I_SetBanAddress && I_SetBanAddress(address, mask)) {
      if (reason) {
        CONS_Printf("Banned IP address %s%s for: %s\n", address,
                    (mask && (strlen(mask) > 0)) ? va("/%s", mask) : "",
                    reason);
      } else {
        CONS_Printf("Banned IP address %s%s\n", address,
                    (mask && (strlen(mask) > 0)) ? va("/%s", mask) : "");
      }
...
static void Command_Kick(void) {
  if (COM_Argc() < 2) {
    CONS_Printf(
        M_GetText("kick <playername/playernum> <reason>: kick a player\n"));
    return;
  }
  ...
  if (server || IsPlayerAdmin(consoleplayer)) {
    ...
    if (server) {
      // Special case if we are trying to kick a player who is downloading the
      // game state: trigger a timeout instead of kicking them, because a kick
      // would only take effect after they have finished downloading
...
static void Got_KickCmd(UINT8 **p, INT32 playernum) {
  ...
  if (playernum != serverplayer && !IsPlayerAdmin(playernum)
    ...
    CONS_Alert(
        CONS_WARNING,
        M_GetText("Illegal kick command received from %s for player %d\n"),
    ...
    /*
                    CONS_Debug(DBG_NETPLAY,
                            "So, you must be asking, why is this an illegal
       ...
       including all of the details above.\n", playernum, pnum,
                                    playernode[playernum],
       playerpernode[playernode[playernum]],
                                    nodetoplayer2[playernode[playernum]],
                                    playernode[pnum],
       playerpernode[playernode[pnum]], nodetoplayer2[playernode[pnum]]);
    */
    pnum = playernum;
  ...
  // Save bans here. Used to be split between here and the actual command,
  // depending on whenever the server did it or a remote admin did it, but it's
  // simply more convenient to keep it all in one place.
  if (server) {
  ...
  case KICK_MSG_GO_AWAY:
    HU_AddChatText(va("\x82*%s has been kicked (Go away)", player_names[pnum]),
                   false);
    kickreason = KR_KICK;
    break;
  case KICK_MSG_PING_HIGH:
    HU_AddChatText(
        va("\x82*%s left the game (Broke delay limit)", player_names[pnum]),
        false);
    kickreason = KR_PINGLIMIT;
    break;
  case KICK_MSG_CON_FAIL:
    HU_AddChatText(
        va("\x82*%s left the game (Synch Failure)", player_names[pnum]), false);
    kickreason = KR_SYNCH;
  ...
  case KICK_MSG_TIMEOUT:
    HU_AddChatText(
        va("\x82*%s left the game (Connection timeout)", player_names[pnum]),
        false);
  ...
  case KICK_MSG_PLAYER_QUIT:
    ...
    break;
  case KICK_MSG_GRIEF:
    HU_AddChatText(va("\x82*%s has been kicked (Automatic grief detection)",
                      player_names[pnum]),
                   false);
    kickreason = KR_KICK;
    break;
  case KICK_MSG_BANNED:
    HU_AddChatText(
        va("\x82*%s has been banned (Don't come back)", player_names[pnum]),
        false);
    kickreason = KR_BAN;
    break;
  case KICK_MSG_CUSTOM_KICK:
    HU_AddChatText(
        va("\x82*%s has been kicked (%s)", player_names[pnum], reason), false);
    kickreason = KR_KICK;
    break;
  case KICK_MSG_CUSTOM_BAN:
    HU_AddChatText(
        va("\x82*%s has been banned (%s)", player_names[pnum], reason), false);
    kickreason = KR_BAN;
    break;
  }
  ...
  if (playernode[pnum] == playernode[consoleplayer]) {
    ...
    // Sal: Because kicks (and a lot of other commands) are player-based, we
    // can't tell which player pnum is on the node from a glance. When we want
    // to remove everyone from a node, we have to get the kicked player's node,
    // then remove everyone on that node manually so we don't miss any. This
    // avoids the bugs with older SRB2 version's online splitscreen kicks,
    // specifically ghosting. On top of this, it can't just be a CL_RemovePlayer
    // call; it has to be a server-sided. Clients don't bother setting any nodes
    // for anything but THE server player (even ignoring the server's extra
    // players!), so it'll often remove everyone because they all have node
    // -1/255, insta-desync! And yes. This is a netxcmd wrap for just
    // CL_RemovePlayer! :V
...
#define removethisplayer(otherp)                                               \
  if (otherp >= 0) {                                                           \
    buf[0] = (UINT8)otherp;                                                    \
    if (otherp != pnum) {                                                      \
      HU_AddChatText(va("\x82*%s left the game (Joined with %s)",              \
...
static void Command_list_http_logins(void) {
  ...
  for (login = curl_logins; login; login = login->next) {
    CONS_Printf("'%s' -> '%s'\n", login->url, login->auth);
  }
}
#endif /*HAVE_CURL*/

static CV_PossibleValue_t netticbuffer_cons_t[] = {
    {0, "MIN"}, {3, "MAX"}, {0, NULL}};
consvar_t cv_netticbuffer = {"netticbuffer",
                             "1",
                             CV_SAVE,
                             netticbuffer_cons_t,
                             NULL,
...
consvar_t cv_allownewplayer = {"allowjoin",
                               "On",
                               CV_SAVE | CV_CALL,
                               CV_OnOff,
                               Joinable_OnChange,
...
consvar_t cv_joinnextround = {"joinnextround",
                              "Off",
                              CV_SAVE | CV_NETVAR,
                              CV_OnOff,
                              NULL,
...
static CV_PossibleValue_t maxplayers_cons_t[] = {
    {2, "MIN"}, {MAXPLAYERS, "MAX"}, {0, NULL}};
consvar_t cv_maxplayers = {"maxplayers",
                           "8",
                           CV_SAVE | CV_CALL,
                           maxplayers_cons_t,
                           Joinable_OnChange,
...
static CV_PossibleValue_t discordinvites_cons_t[] = {
    {0, "Admins Only"}, {1, "Everyone"}, {0, NULL}};
consvar_t cv_discordinvites = {"discordinvites",
                               "Everyone",
                               CV_SAVE | CV_CALL,
                               discordinvites_cons_t,
                               Joinable_OnChange,
                               0,
                               NULL,
                               NULL,
...
static CV_PossibleValue_t resynchattempts_cons_t[] = {
    {0, "MIN"}, {20, "MAX"}, {0, NULL}};
consvar_t cv_resynchattempts = {"resynchattempts",
                                "2",
                                CV_SAVE,
                                ...
                                0,
                                NULL};
consvar_t cv_blamecfail = {"blamecfail", "Off", CV_SAVE, CV_OnOff, NULL, 0,
                           NULL,         NULL,  0,       0,        NULL};

// max file size to send to a player (in kilobytes)
static CV_PossibleValue_t maxsend_cons_t[] = {
    {0, "MIN"}, {51200, "MAX"}, {0, NULL}};
consvar_t cv_maxsend = {
    "maxsend", "MAX", CV_SAVE, maxsend_cons_t, NULL, 0, NULL, NULL, 0, 0, NULL};
consvar_t cv_noticedownload = {"noticedownload",
                               "Off",
                               CV_SAVE,
                               CV_OnOff,
                               NULL,
...
static CV_PossibleValue_t downloadspeed_cons_t[] = {
    {0, "MIN"}, {32, "MAX"}, {0, NULL}};
consvar_t cv_downloadspeed = {"downloadspeed",
                              "MAX",
                              CV_SAVE,
                              downloadspeed_cons_t,
                              NULL,
                              0,
                              NULL,
                              NULL,
...
static void Joinable_OnChange(void) {
  ...
  maxplayer = (UINT8)(min((dedicated ? MAXPLAYERS - 1 : MAXPLAYERS),
                          cv_maxplayers.value));
...
void D_QuitNetGame(void) {
  ...
  if (server) {
    ...
    if (serverrunning && netgame && cv_advertise.value) // see mserv.c Online()
      UnregisterServer();
#endif
  } else if (servernode > 0 && servernode < MAXNETNODES &&
             nodeingame[(UINT8)servernode]) {
...
static void Got_AddPlayer(UINT8 **p, INT32 playernum) {
  ...
  if (playernum != serverplayer && !IsPlayerAdmin(playernum)) {
    ...
    CONS_Alert(CONS_WARNING,
               M_GetText("Illegal add player command received from %s\n"),
  ...
  if (netgame) {
    ...
    if (server && cv_showjoinaddress.value) {
      ...
      if (I_GetNodeAddress && (address = I_GetNodeAddress(node)) != NULL)
        HU_AddChatText(va("\x82*Player %d has joined the game (node %d) (%s)",
                          newplayernum + 1, node, address),
                       false); // merge join notification + IP to avoid clogging
                               // console/chat.
    } else
      HU_AddChatText(va("\x82*Player %d has joined the game (node %d)",
                        newplayernum + 1, node),
                     false); // if you don't wanna see the join address.
  }
...
static void Got_RemovePlayer(UINT8 **p, INT32 playernum) {
  ...
  if (playernum != serverplayer && !IsPlayerAdmin(playernum)) {
    // protect against hacked/buggy client
    CONS_Alert(CONS_WARNING,
               M_GetText("Illegal remove player command received from %s\n"),
               player_names[playernum]);
    if (server) {
      XBOXSTATIC UINT8 buf[2];
...
static boolean SV_AddWaitingPlayers(void) {
  ...
  for (node = 0; node < MAXNETNODES; node++) {
    ...
    for (; nodewaiting[node] > 0; nodewaiting[node]--) {
      ...
      // use the next free slot (we can't put playeringame[newplayernum] = true
      // here)
      newplayernum++;
    }
  }
...
static void HandleConnect(SINT8 node) {
  ...
  // If a server filled out, then it'd overwrite the host and turn everyone into
  // weird husks..... It's too much effort to legimately fix right now. Just
  // prevent it from reaching that state.
  UINT8 maxplayers =
      min((dedicated ? MAXPLAYERS - 1 : MAXPLAYERS), cv_maxplayers.value);
  UINT8 connectedplayers = 0;

  for (UINT8 i = dedicated ? 1 : 0; i < MAXPLAYERS; i++)
    if (playernode[i] !=
        UINT8_MAX) // We use this to count players because it is affected by
                   // SV_AddWaitingPlayers when more than one client joins on
                   // the same tic, unlike playeringame and D_NumPlayers.
  ...
  if (bannednode && bannednode[node].banid != SIZE_MAX) {
    ...
    // Get the reason...
    if (!I_GetBanReason ||
        (reason = I_GetBanReason(bannednode[node].banid)) == NULL)
      reason = "No reason given";

    if (bannednode[node].timeleft != NO_BAN_TIME) {
      // these are fudged a little to allow it to sink in for impatient
      // rejoiners
      ...
      if (days) {
        SV_SendRefuse(node, va("K|%s\n(Time remaining: %d day%s)", reason, days,
                               days > 1 ? "s" : ""));
      } else if (hours) {
        SV_SendRefuse(node, va("K|%s\n(Time remaining: %d hour%s)", reason,
                               hours, hours > 1 ? "s" : ""));
      } else if (minutes) {
        SV_SendRefuse(node, va("K|%s\n(Time remaining: %d minute%s)", reason,
                               minutes, minutes > 1 ? "s" : ""));
      } else {
    ...
    SV_SendRefuse(node, "Incompatible packet formats.");
                     ...
                     sizeof netbuffer->u.clientcfg.application)) {
    SV_SendRefuse(node,
                  "Different SRB2Kart modifications\nare not compatible.");
  } else if (netbuffer->u.clientcfg.version != VERSION ||
             netbuffer->u.clientcfg.subversion != SUBVERSION) {
    SV_SendRefuse(node,
                  va(M_GetText("Different SRB2Kart versions cannot\nplay a "
                               "netgame!\n(server version %d.%d)"),
                     VERSION, SUBVERSION));
  } else if (!cv_allownewplayer.value && node) {
    SV_SendRefuse(
        node, M_GetText("The server is not accepting\njoins for the moment."));
  } else if (connectedplayers >= maxplayers) {
    SV_SendRefuse(node,
                  va(M_GetText("Maximum players reached: %d"), maxplayers));
  } else if (netgame &&
             netbuffer->u.clientcfg.localplayers > 4) // Hacked client?
  {
    SV_SendRefuse(node, M_GetText("Too many players from\nthis node."));
  } else if (netgame && connectedplayers + netbuffer->u.clientcfg.localplayers >
                            maxplayers) {
    SV_SendRefuse(
        node, va(M_GetText("Number of local players\nwould exceed maximum: %d"),
                 maxplayers));
  } else if (netgame && !netbuffer->u.clientcfg.localplayers) // Stealth join?
  {
    ...
    nodewaiting[node] =
        (UINT8)(netbuffer->u.clientcfg.localplayers - playerpernode[node]);
    if (!nodeingame[node]) {
      ...
      /// \note Wait what???
      ///       What if the gamestate takes more than one second to get
      ///       downloaded? Or if a lagspike happens?
      ...
      if (!SV_SendServerConfig(node)) {
        ...
        SV_SendRefuse(node,
                      M_GetText("Server couldn't send info, please try again"));
...
static void HandleServerInfo(SINT8 node) {
  ...
  netbuffer->u.serverinfo
      .application[sizeof netbuffer->u.serverinfo.application - 1] = '\0';
  memcpy(servername, netbuffer->u.serverinfo.servername, MAXSERVERNAME);
  CopyCaretColors(netbuffer->u.serverinfo.servername, servername,
                  MAXSERVERNAME);
  netbuffer->u.serverinfo.gametype =
      (UINT8)((netbuffer->u.serverinfo.gametype == VANILLA_GT_MATCH) ? GT_MATCH
                                                                     : GT_RACE);
 ...
 *
 * \param node The packet sender
 * \todo Choose a better name, as the packet can also come from the server
 * apparently? \sa HandlePacketFromPlayer \sa GetPackets
 *
 */
...
#define SERVERONLY                                                             \
  ...
  case PT_MOREFILESNEEDED:
    if (server && serverrunning) { // But wait I thought I'm the server?
      Net_CloseConnection(node);
      break;
    }
    SERVERONLY
    if (cl_mode == CL_ASKFULLFILELIST &&
        netbuffer->u.filesneededcfg.first == fileneedednum) {
      D_ParseFileneeded(netbuffer->u.filesneededcfg.num,
                        netbuffer->u.filesneededcfg.files,
                        netbuffer->u.filesneededcfg.first);
  ...
  case PT_SERVERREFUSE:            // Negative response of client join request
    ...
    if (cl_mode == CL_WAITJOINRESPONSE) {
      ...
      if (strstr(reason, "Maximum players reached")) {
        serverisfull = true;
        // Special timeout for when refusing due to player cap. The client
        // will wait 3 seconds between join requests when waiting for a slot,
        // so we need this to be much longer We set it back to the value of
        // cv_nettimeout.value in CL_Reset
  ...
  case PT_SERVERCFG: // Positive response of client join request
  {
    ...
    if (server && serverrunning &&
        node != servernode) { // but wait I thought I'm the server?
      Net_CloseConnection(node);
      break;
    }
    SERVERONLY
    /// \note how would this happen? and is it doing the right thing if it
    /// does?
    if (!(cl_mode == CL_WAITJOINRESPONSE || cl_mode == CL_ASKJOIN))
      break;

    if (client) {
      maketic = gametic = neededtic =
          (tic_t)LONG(netbuffer->u.servercfg.gametic);
...
#ifdef JOININGAME
      CONS_Printf(
          M_GetText("Join accepted, waiting for complete game state...\n"));
#else
      CONS_Printf(
          M_GetText("Join accepted, waiting for next level change...\n"));
#endif
    ...
    CV_LoadNetVars(&scp);
#ifdef JOININGAME
    /// \note Wait. What if a Lua script uses some global custom variables
    /// synched with the NetVars hook?
    ///       Shouldn't them be downloaded even at intermission time?
    ///       Also, according to HandleConnect, the server will send the
    ///       savegame even during intermission...
    /// Sryder 2018-07-05: If we don't want to send the player config another
    /// way we need to send the gamestate
    ///                    At almost any gamestate there could be joiners...
    ///                    So just always send gamestate?
  ...
  case PT_FILEFRAGMENT:
    if (server) { // But wait I thought I'm the server?
      Net_CloseConnection(node);
      break;
    }
    SERVERONLY
  ...
  case PT_REQUESTFILE:
    if (server) {
      if (!cv_downloading.value || !Got_RequestFilePak(node))
        Net_CloseConnection(
            node); // close connection if one of the requested files could not
                   // be sent, or you disabled downloading anyway
    } else
...
static boolean CheckForSpeedHacks(UINT8 p) {
  if (netcmds[maketic % TICQUEUE][p].forwardmove > MAXPLMOVE ||
      netcmds[maketic % TICQUEUE][p].forwardmove < -MAXPLMOVE ||
      netcmds[maketic % TICQUEUE][p].sidemove > MAXPLMOVE ||
      netcmds[maketic % TICQUEUE][p].sidemove < -MAXPLMOVE ||
      netcmds[maketic % TICQUEUE][p].driftturn > KART_FULLTURN ||
      netcmds[maketic % TICQUEUE][p].driftturn < -KART_FULLTURN) {
    XBOXSTATIC char buf[2];
    CONS_Alert(CONS_WARNING,
               M_GetText("Illegal movement value received from node %d\n"),
               playernode[p]);
    // D_Clearticcmd(k);
...
static void HandlePacketFromPlayer(SINT8 node) {
  ...
  if (netconsole >= MAXPLAYERS)
    I_Error("bad table nodetoplayer: node %d player %d", doomcom->remotenode,
            netconsole);
#endif
  ...
  case PT_NODEKEEPALIVEMIS:
    ...
    if (netbuffer->packettype == PT_CLIENTMIS ||
        ...
        netbuffer->packettype == PT_CLIENT3MIS ||
        netbuffer->packettype == PT_CLIENT4MIS ||
    ...
    if (nettics[node] > realend) {
      DEBFILE(
          va("out of order ticcmd discarded nettics = %u\n", nettics[node]));
      break;
    }
    ...
    // This should probably still timeout though, as the node should always
    // have a player 1 number
    if (netconsole == -1)
      break;

    // If a client sends a ticcmd it should mean they are done receiving the
    // savegame
    sendingsavegame[node] = false;

    // As long as clients send valid ticcmds, the server can keep running, so
    // reset the timeout
    ...
    // Copy ticcmd
    G_MoveTiccmd(&netcmds[maketic % TICQUEUE][netconsole],
    ...
    if (realstart <= gametic && realstart > gametic - TICQUEUE + 1 &&
        gamestate == GS_LEVEL &&
      ...
      if (cv_resynchattempts.value &&
          resynch_score[node] <= (unsigned)cv_resynchattempts.value * 250) {
        ...
        DEBFILE(va("player %d kicked (synch failure) [%u] %d!=%d\n", netconsole,
                   realstart, consistancy[realstart % TICQUEUE],
                   SHORT(netbuffer->u.clientpak.consistancy)));
        break;
      }
  ...
  case PT_BASICKEEPALIVE:
    ...
    // If a client sends this it should mean they are done receiving the
    // savegame
    sendingsavegame[node] = false;

    // As long as clients send keep alives, the server can keep running, so
    // reset the timeout
    /// \todo Use a separate cvar for that kind of timeout?
    freezetimeout[node] = I_GetTime() + connectiontimeout;
    break;
  ...
  case PT_TEXTCMD4:
    ...
    if (netconsole < 0 || netconsole >= MAXPLAYERS)
      ...
      if (!netbuffer->u.textcmd[0]) {
        DEBFILE(va(
            "GetPacket: Textcmd with size 0 detected! (node %u, player %d)\n",
            node, netconsole));
        Net_UnAcknowledgePacket(node);
        break;
      }
      ...
      if (netbuffer->u.textcmd[0] >
          (size_t)doomcom->datalength - BASEPACKETSIZE - 1) {
        DEBFILE(va("GetPacket: Bad Textcmd packet size! (expected %d, actual "
                   "%s, node %u, player %d)\n",
                   netbuffer->u.textcmd[0],
                   sizeu1((size_t)doomcom->datalength - BASEPACKETSIZE - 1),
                   node, netconsole));
        Net_UnAcknowledgePacket(node);
        break;
      }

      // check if tic that we are making isn't too large else we cannot send
      // it :( doomcom->numslots+1 "+1" since doomcom->numslots can change
      // within this time and sent time
      ...
      while (
          ...
          (TotalTextCmdPerTic(tic) > j ||
           netbuffer->u.textcmd[0] + (textcmd ? textcmd[0] : 0) > MAXTEXTCMD) &&
      ...
      if (tic >= firstticstosend + TICQUEUE) {
        DEBFILE(va("GetPacket: Textcmd too long (max %s, used %s, mak %d, "
                   ...
                   sizeu1(j), sizeu2(TotalTextCmdPerTic(maketic)), maketic,
                   firstticstosend, node, netconsole));
        Net_UnAcknowledgePacket(node);
        break;
      }
      ...
      DEBFILE(
          va("textcmd put in tic %u at position %d (player %d) ftts %u mk %u\n",
             tic, textcmd[0] + 1, netconsole, firstticstosend, maketic));

      M_Memcpy(&textcmd[textcmd[0] + 1], netbuffer->u.textcmd + 1,
               netbuffer->u.textcmd[0]);
  ...
  case PT_CLIENTQUIT:
    ...
    // this allow to send some packets to the quitting client to have their
    // ack back
    nodewaiting[node] = 0;
  ...
  case PT_SERVERTICS:
    ...
    if (!txtpak)
      txtpak = (UINT8 *)&netbuffer->u.serverpak
                   .cmds[netbuffer->u.serverpak.numslots *
    ...
    if (realstart <= neededtic && realend > neededtic) {
      ...
      /*if (realend < neededtic - 2 * TICRATE || neededtic + 2 * TICRATE <
         realstart) I_Error("Received an out of order PT_SERVERTICS packet!\n"
                              "Got tics %d-%d, needed tic %d\n\n"
                              "Please report this crash on the Master
         Board,\n" "IRC or Discord so it can be fixed.\n", (INT32)realstart,
         (INT32)realend, (INT32)neededtic);*/
    }
    break;
  case PT_RESYNCHING:
...
// sends out a packet
//
// no more use random generator, because at very first tic isn't yet
// synchronized Note: It is called consistAncy on purpose.
//
static INT16 Consistancy(void) {
  INT32 i;
  UINT32 ret = 0;
  ...
  if (gamestate == GS_LEVEL) {
    for (th = thinkercap.next; th != &thinkercap; th = th->next) {
      ...
      if (mo->flags &
          (MF_SPECIAL | MF_SOLID | MF_PUSHABLE | MF_BOSS | MF_MISSILE |
           MF_SPRING | MF_MONITOR | MF_FIRE | MF_ENEMY | MF_PAIN | MF_STICKY)) {
        ret -= mo->type;
        ret += mo->x;
        ret -= mo->y;
        ret += mo->z;
...
static void CL_SendClientCmd(void) {
  ...
  if (gamestate == GS_WAITINGPLAYERS) {
    ...
    if (splitscreen ||
        botingame) // Send a special packet with 2 cmd for splitscreen
    {
  ...
  if (cl_mode == CL_CONNECTED || dedicated) {
    ...
    if (localtextcmd[0]) {
      ...
      if (HSendPacket(servernode, true, 0,
                      localtextcmd[0] + 1)) // Send can fail...
        localtextcmd[0] = 0;
    }
    ...
    if (localtextcmd2[0]) {
      ...
      // All extra data have been sent
      if (HSendPacket(servernode, true, 0,
                      localtextcmd2[0] + 1)) // Send can fail...
        localtextcmd2[0] = 0;
    }
    ...
    if (localtextcmd3[0]) {
      ...
      // All extra data have been sent
      if (HSendPacket(servernode, true, 0,
                      localtextcmd3[0] + 1)) // Send can fail...
        localtextcmd3[0] = 0;
    }
    ...
    if (localtextcmd4[0]) {
      ...
      // All extra data have been sent
      if (HSendPacket(servernode, true, 0,
                      localtextcmd4[0] + 1)) // Send can fail...
        localtextcmd4[0] = 0;
    }
  }
}
  ...
  for (n = 1; n < MAXNETNODES; n++)
    if (nodeingame[n]) {
      ...
      if (realfirsttic >= lasttictosend) {
        ...
        // packet detection work when we have received packet with firsttic >
        // neededtic (getpacket servertics case)
        DEBFILE(va("Nothing to send node %u mak=%u sup=%u net=%u \n", n,
                   lasttictosend, supposedtics[n], nettics[n]));
      ...
      for (i = realfirsttic; i < lasttictosend; i++) {
        ...
        if (packsize > software_MAXPACKETLENGTH) {
          DEBFILE(
              va("packet too large (%s) at tic %d (should be from %d to %d)\n",
          ...
          // textcmd case) but when numplayer changes the computation can be
          // different
          if (lasttictosend == realfirsttic) {
            if (packsize > MAXPACKETLENGTH)
              I_Error("Too many players: can't send %s data for %d players to "
                      "node %d\n"
      ...
      for (i = realfirsttic; i < lasttictosend; i++) {
        bufpos = G_DcpyTiccmd(bufpos, netcmds[i % TICQUEUE],
                              doomcom->numslots * sizeof(ticcmd_t));
      }
      ...
      if (supposedtics[n] < nettics[n])
        supposedtics[n] = nettics[n];
    }
  // node 0 is me!
...
static void Local_Maketic(INT32 realtics) {
  ...
  D_ProcessEvents(); // menu responder, cons responder,
                     ...
                     // F_Responder, and G_MapEventsToControls
  if (!dedicated)
  ...
  for (tic = server ? maketic : (neededtic - 1); tic >= gametic; tic--) {
    if (numadjust++ == TICQUEUE) {
      DEBFILE(va("SV_SpawnPlayer: All netcmds for player %d adjusted!\n",
                 playernum));
      // We already adjusted them all, waste of time doing the same thing over
      // and over This shouldn't happen normally though, either gametic was 0
      // (which is handled now anyway) or maketic >= gametic + TICQUEUE
      // -- Monster Iestyn 16/01/18
      break;
    }
    netcmds[tic % TICQUEUE][playernum].angleturn =
        (INT16)((angle >> 16) | TICCMD_RECEIVED);
...
static void SV_Maketic(void) {
  ...
  for (j = 0; j < MAXNETNODES; j++)
    if (playerpernode[j]) {
      ...
      if ((netcmds[maketic % TICQUEUE][player].angleturn & TICCMD_RECEIVED) ==
          0) { // we didn't receive this tic
        INT32 i;
        ...
        for (i = 0; i < playerpernode[j]; i++) {
          if (i == 0)
            player = nodetoplayer[j];
          else if (i == 1)
            player = nodetoplayer2[j];
          else if (i == 2)
            player = nodetoplayer3[j];
          else if (i == 3)
            player = nodetoplayer4[j];
          netcmds[maketic % TICQUEUE][player] =
              netcmds[(maketic - 1) % TICQUEUE][player];
...
boolean TryRunTics(tic_t realtics) {
  ...
  if (realtics >= 1) {
    COM_BufTicker();
    if (mapchangepending)
      D_MapChange(-1, 0, encoremode, false, 2, false,
                  fromlevelselect); // finish the map change
  }
  ...
  if (demo.playback) {
    neededtic = gametic +
                realtics * (gamestate == GS_LEVEL ? cv_playbackspeed.value : 1);
  ...
  if (ticking) {
    ...
    while (neededtic > gametic) {
      ...
      // Leave a certain amount of tics present in the net buffer as long as
      // we've ran at least one tic this frame.
      if (client && gamestate == GS_LEVEL && leveltime > 3 &&
          neededtic <= gametic + cv_netticbuffer.value)
        break;
    }
  } else {
    hu_stopped = true;
...
/* 	Ping Update except better:
        We call this once per second and check for people's pings. If their ping
   happens to be too high, we increment some timer and kick them out. If they're
   not lagging, decrement the timer by 1. Of course, reset all of this if they
   leave.

        Why do we do that? Well, I'm a person with unfortunately sometimes
   unstable internet and happen to keep getting kicked very unconveniently for
   very short high spikes. (700+ ms) Because my spikes are so high, the average
   ping is exponentially higher too (700s really add up...!) which leads me to
   getting kicked for a short burst of spiking. With this change here, this
   doesn't happen anymore as it checks if my ping has been CONSISTENTLY bad for
   long enough before killing me.
*/
...
static inline void PingUpdate(void) {
  ...
  // if (cv_maxping.value) -- always check for TICQUEUE overrun
  {
    for (i = 0; i < MAXPLAYERS; i++) {
      if (!playeringame[i] ||
          P_IsLocalPlayer(
              &players[i])) // should be P_IsMachineLocalPlayer for DRRR
      {
        pingtimeout[i] = 0;
        continue;
      }
      ...
      if ((maketic + 5) >=
          nettics[playernode[i]] + (TICQUEUE - (2 * TICRATE))) {
        // Anyone who's gobbled most of the TICQUEUE and is likely to halt the
        // server the next few times this runs has to die *right now*. (See also
        // NetUpdate)
        pingkick[i] = PINGKICK_TICQUEUE;
      } else if ((cv_maxping.value) && (realpingtable[i] / pingmeasurecount >
                                        (unsigned)cv_maxping.value)) {
        ...
        // you aren't lagging, but you aren't free yet. In case you'll keep
        // spiking, we just make the timer go back down. (Very unstable net must
        // still get kicked).
        if (pingtimeout[i] > 0)
          pingtimeout[i]--;
      }
    ...
    {
      UINT8 minimumkicklevel =
          (nonlaggers > 0) ? PINGKICK_LIMIT : PINGKICK_TICQUEUE;
      for (i = 0; i < MAXPLAYERS; i++) {
        XBOXSTATIC char buf[2];
  ...
  // send the server's maxping as last element of our ping table. This is useful
  // to let us know when we're about to get kicked.
...
// If a tree falls in the forest but nobody is around to hear it, does it make a
// tic?
  ...
  if (server && dedicated && gamestate == GS_LEVEL) {
    ...
    if (i == MAXNETNODES) {
      if (leveltime == 2) {
        ...
        if (D_GetExistingTextcmd(gametic, 0) ||
            D_GetExistingTextcmd(gametic + 1, 0)) {
  ...
  if (client) {
    ...
    if (!demo.playback && realtics > 0) {
      ...
      if (counts != -666) {
        ...
        for (; tictoclear < firstticstosend;
             tictoclear++)           // Clear only when acknowledged
...
tic_t GetLag(INT32 node) {
  // If the client has caught up to the server -- say, during a wipe -- lag is
  // meaningless.
