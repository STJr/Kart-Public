...
struct demovars_s {
  ...
  tic_t savebutton; // Used to determine when the local player can choose to
                    // save the replay while the race is still going
  enum {
    DSM_NOTSAVING,
    DSM_WILLAUTOSAVE,
    DSM_TITLEENTRY,
...
extern tic_t
    timeinmap; // Ticker for time spent in level (used for levelcard display)
extern INT16 rw_maximums[NUM_WEAPONS];

// used in game menu
extern consvar_t cv_chatwidth, cv_chatnotifications, cv_chatheight, cv_chattime,
    cv_consolechat, cv_chatbacktint,
    cv_chatspamprotection /*, cv_compactscoreboard*/;
extern consvar_t cv_songcredits;
extern consvar_t cv_pauseifunfocused;
// extern consvar_t cv_crosshair, cv_crosshair2, cv_crosshair3, cv_crosshair4;
extern consvar_t
    cv_invertmouse /*, cv_alwaysfreelook, cv_chasefreelook, cv_mousemove*/;
extern consvar_t
    cv_invertmouse2 /*, cv_alwaysfreelook2, cv_chasefreelook2, cv_mousemove2*/;
...
extern consvar_t cv_turnaxis, cv_moveaxis, cv_brakeaxis, cv_aimaxis,
    cv_lookaxis, cv_fireaxis, cv_driftaxis, cv_lookbackaxis, cv_xdeadzone,
    cv_ydeadzone;
extern consvar_t cv_turnaxis2, cv_moveaxis2, cv_brakeaxis2, cv_aimaxis2,
    cv_lookaxis2, cv_fireaxis2, cv_driftaxis2, cv_lookbackaxis2, cv_xdeadzone2,
    cv_ydeadzone2;
extern consvar_t cv_turnaxis3, cv_moveaxis3, cv_brakeaxis3, cv_aimaxis3,
    cv_lookaxis3, cv_fireaxis3, cv_driftaxis3, cv_lookbackaxis3, cv_xdeadzone3,
    cv_ydeadzone3;
extern consvar_t cv_turnaxis4, cv_moveaxis4, cv_brakeaxis4, cv_aimaxis4,
    cv_lookaxis4, cv_fireaxis4, cv_driftaxis4, cv_lookbackaxis4, cv_xdeadzone4,
    cv_ydeadzone4;
extern consvar_t cv_ghost_besttime, cv_ghost_bestlap, cv_ghost_last,
    cv_ghost_guest, cv_ghost_staff;
...
extern INT32
    localaiming[MAXSPLITSCREENPLAYERS]; // should be an angle_t but signed
...
INT32
G_FindMap(const char *query, char **foundmapnamep, mapsearchfreq_t **freqp,
          INT32 *freqc);
void G_FreeMapSearch(mapsearchfreq_t *freq, INT32 freqc);

/* Match map name by search + 2 digit map code or map number. */
INT32
G_FindMapByNameOrCode(const char *query, char **foundmapnamep);
...
#define DXD_PLAYSTATE                                                          \
  0x10 // state changed between playing, spectating, or not in-game
...
#define DFILE_ERROR_INCOMPLETEOUTOFORDER                                       \
  0x03 // Some files are loaded out of order, but others are not.
#define DFILE_ERROR_CANNOTLOAD 0x04 // Files are missing and cannot be loaded.
#define DFILE_ERROR_EXTRAFILES                                                 \
  0x05 // Extra files outside of the replay's file list are loaded.
...
INT32
G_FindView(INT32 startview, UINT8 viewnum, boolean onlyactive, boolean reverse);
INT32
