...
boolean majormods =
    false; // Set if Lua/Gameplay SOC/replacement map has been added.
...
// menu demo things
UINT8 numDemos = 0; // 3; -- i'm FED UP of losing my skincolour to a broken
                    // demo. change this back when we make new ones
...
UINT8 gamespeed; // Game's current speed (or difficulty, or cc, or etc); 0 for
                 // easy, 1 for normal, 2 for hard
...
tic_t indirectitemcooldown; // Cooldown before any more Shrink, SPB, or any
                            // other item that works indirectly is awarded
tic_t hyubgone;             // Cooldown before hyudoro is allowed to be rerolled
tic_t mapreset; // Map reset delay when enough players have joined an empty game
UINT8 nospectategrief; // How many players need to be in-game to eliminate last;
                       // for preventing spectate griefing
boolean thwompsactive; // Thwomps activate on lap 2
SINT8 spbplace;        // SPB exists, give the person behind better items
boolean startedInFreePlay; // Map was started in free play

// Client-sided, unsynched variables (NEVER use in anything that needs to be
// synced with other players)
boolean legitimateexit; // Did this client actually finish the match?
boolean
    comebackshowninfo; // Have you already seen the "ATTACK OR PROTECT" message?
tic_t curlap;          // Current lap time
tic_t bestlap;         // Best lap time
static INT16
    randmapbuffer[NUMMAPS + 1]; // Buffer for maps RandMap is allowed to roll
...
static struct {
  ...
  UINT8 desyncframes; // Don't try to resync unless we've been off for two
                      // frames, to monkeypatch a few trouble spots
...
static CV_PossibleValue_t recordmultiplayerdemos_cons_t[] = {
    {0, "Disabled"}, {1, "Manual Save"}, {2, "Auto Save"}, {0, NULL}};
consvar_t cv_recordmultiplayerdemos = {"netdemo_record",
                                       "Manual Save",
                                       CV_SAVE,
                                       recordmultiplayerdemos_cons_t,
                                       NULL,
                                       0,
                                       NULL,
                                       NULL,
...
static CV_PossibleValue_t netdemosyncquality_cons_t[] = {
    {1, "MIN"}, {35, "MAX"}, {0, NULL}};
consvar_t cv_netdemosyncquality = {"netdemo_syncquality",
                                   "1",
                                   CV_SAVE,
                                   netdemosyncquality_cons_t,
                                   NULL,
                                   0,
                                   NULL,
                                   NULL,
...
// static CV_PossibleValue_t crosshair_cons_t[] = {{0, "Off"}, {1, "Cross"}, {2,
// "Angle"}, {3, "Point"}, {0, NULL}};
...
"More Axis Sets"
    ...
    static CV_PossibleValue_t deadzone_cons_t[] = {
        {FRACUNIT / 16, "MIN"}, {FRACUNIT, "MAX"}, {0, NULL}};

// don't mind me putting these here, I was lazy to figure out where else I could
// put those without blowing up the compiler.

// it automatically becomes compact with 20+ players, but if you like it, I
// guess you can turn that on! SRB2Kart: irrelevant for us.
// consvar_t cv_compactscoreboard= {"compactscoreboard", "Off", CV_SAVE,
...
consvar_t cv_chattime = {
    "chattime", "8", CV_SAVE, chattime_cons_t, NULL, 0, NULL, NULL, 0, 0, NULL};

// chatwidth
static CV_PossibleValue_t chatwidth_cons_t[] = {
    {64, "MIN"}, {150, "MAX"}, {0, NULL}};
consvar_t cv_chatwidth = {"chatwidth", "150", CV_SAVE, chatwidth_cons_t,
                          NULL,        0,     NULL,    NULL,
                          0,           0,     NULL};
...
consvar_t cv_chatheight = {
    "chatheight", "8", CV_SAVE, chatheight_cons_t, NULL, 0, NULL, NULL, 0, 0,
    NULL};

// chat notifications (do you want to hear beeps? I'd understand if you didn't.)
consvar_t cv_chatnotifications = {"chatnotifications",
                                  "On",
                                  CV_SAVE,
                                  CV_OnOff,
                                  NULL,
...
consvar_t cv_chatspamprotection = {"chatspamprotection",
                                   "On",
                                   CV_SAVE,
                                   CV_OnOff,
                                   NULL,
...
consvar_t cv_chatbacktint = {"chatbacktint", "On", CV_SAVE, CV_OnOff, NULL, 0,
                             NULL,           NULL, 0,       0,        NULL};
...
consvar_t cv_consolechat = {"chatmode", "Window", CV_SAVE, consolechat_cons_t,
                            NULL,       0,        NULL,    NULL,
                            0,          0,        NULL};

// Pause game upon window losing focus
consvar_t cv_pauseifunfocused = {"pauseifunfocused",
                                 "Yes",
                                 CV_SAVE,
                                 CV_YesNo,
                                 NULL,
...
consvar_t cv_songcredits = {"songcredits", "On", CV_SAVE, CV_OnOff, NULL, 0,
                            NULL,          NULL, 0,       0,        NULL};

/*consvar_t cv_crosshair = {"crosshair", "Off", CV_SAVE, crosshair_cons_t, NULL,
0, NULL, NULL, 0, 0, NULL}; consvar_t cv_crosshair2 = {"crosshair2", "Off",
CV_SAVE, crosshair_cons_t, NULL, 0, NULL, NULL, 0, 0, NULL}; consvar_t
cv_crosshair3 = {"crosshair3", "Off", CV_SAVE, crosshair_cons_t, NULL, 0, NULL,
NULL, 0, 0, NULL}; consvar_t cv_crosshair4 = {"crosshair4", "Off", CV_SAVE,
crosshair_cons_t, NULL, 0, NULL, NULL, 0, 0, NULL};*/
consvar_t cv_invertmouse = {"invertmouse", "Off", CV_SAVE, CV_OnOff, NULL, 0,
                            NULL,          NULL,  0,       0,        NULL};
consvar_t cv_invertmouse2 = {"invertmouse2", "Off", CV_SAVE, CV_OnOff, NULL, 0,
                             NULL,           NULL,  0,       0,        NULL};
/*consvar_t cv_alwaysfreelook = {"alwaysmlook", "Off", CV_SAVE, CV_OnOff, NULL,
0, NULL, NULL, 0, 0, NULL}; consvar_t cv_alwaysfreelook2 = {"alwaysmlook2",
"Off", CV_SAVE, CV_OnOff, NULL, 0, NULL, NULL, 0, 0, NULL}; consvar_t
cv_chasefreelook = {"chasemlook", "Off", CV_SAVE, CV_OnOff, NULL, 0, NULL, NULL,
0, 0, NULL}; consvar_t cv_chasefreelook2 = {"chasemlook2", "Off", CV_SAVE,
CV_OnOff, NULL, 0, NULL, NULL, 0, 0, NULL}; consvar_t cv_mousemove =
{"mousemove", "Off", CV_SAVE, CV_OnOff, NULL, 0, NULL, NULL, 0, 0, NULL};
consvar_t cv_mousemove2 = {"mousemove2", "Off", CV_SAVE, CV_OnOff, NULL, 0,
NULL, NULL, 0, 0, NULL};*/
consvar_t cv_analog = {"analog", "Off", CV_CALL, CV_OnOff, Analog_OnChange,
                       0,        NULL,  NULL,    0,        0,
                       NULL};
consvar_t cv_analog2 = {"analog2", "Off", CV_CALL, CV_OnOff, Analog2_OnChange,
                        0,         NULL,  NULL,    0,        0,
                        NULL};
consvar_t cv_analog3 = {"analog3", "Off", CV_CALL, CV_OnOff, Analog3_OnChange,
                        0,         NULL,  NULL,    0,        0,
                        NULL};
consvar_t cv_analog4 = {"analog4", "Off", CV_CALL, CV_OnOff, Analog4_OnChange,
                        0,         NULL,  NULL,    0,        0,
                        NULL};
consvar_t cv_useranalog = {"useranalog",
                           "Off",
                           CV_SAVE | CV_CALL,
                           CV_OnOff,
                           UserAnalog_OnChange,
                           ...
                           0,
                           NULL};
consvar_t cv_useranalog2 = {"useranalog2",
                            "Off",
                            CV_SAVE | CV_CALL,
                            CV_OnOff,
                            UserAnalog2_OnChange,
                            0,
                            NULL,
                            NULL,
                            0,
                            0,
                            NULL};
consvar_t cv_useranalog3 = {"useranalog3",
                            "Off",
                            CV_SAVE | CV_CALL,
                            CV_OnOff,
                            UserAnalog3_OnChange,
                            0,
                            NULL,
                            NULL,
                            0,
                            0,
                            NULL};
consvar_t cv_useranalog4 = {"useranalog4",
                            "Off",
                            CV_SAVE | CV_CALL,
                            CV_OnOff,
                            UserAnalog4_OnChange,
                            0,
                            NULL,
                            NULL,
                            0,
...
consvar_t cv_turnaxis = {"joyaxis_turn",
                         "Left X",
                         CV_SAVE,
                         joyaxis_cons_t,
                         NULL,
                         ...
                         0,
                         NULL};
consvar_t cv_moveaxis = {"joyaxis_move",
                         "None",
                         CV_SAVE,
                         joyaxis_cons_t,
                         NULL,
                         ...
                         0,
                         NULL};
consvar_t cv_brakeaxis = {"joyaxis_brake",
                          "None",
                          CV_SAVE,
                          joyaxis_cons_t,
                          NULL,
                          ...
                          0,
                          NULL};
consvar_t cv_aimaxis = {
    "joyaxis_aim", "Left Y", CV_SAVE, joyaxis_cons_t, NULL, 0, NULL, NULL, 0, 0,
    NULL};
consvar_t cv_lookaxis = {"joyaxis_look",
                         "None",
                         CV_SAVE,
                         joyaxis_cons_t,
                         NULL,
                         ...
                         0,
                         NULL};
consvar_t cv_fireaxis = {"joyaxis_fire",
                         "L Trigger",
                         CV_SAVE,
                         joyaxis_cons_t,
                         NULL,
                         ...
                         0,
                         NULL};
consvar_t cv_driftaxis = {"joyaxis_drift",
                          "R Trigger",
                          CV_SAVE,
                          joyaxis_cons_t,
                          NULL,
                          ...
                          0,
                          NULL};
consvar_t cv_lookbackaxis = {"joyaxis_lookback",
                             "None",
                             CV_SAVE,
                             joyaxis_cons_t,
                             NULL,
                             ...
                             0,
                             NULL};
consvar_t cv_xdeadzone = {"joy_xdeadzone",
                          "0.3",
                          CV_FLOAT | CV_SAVE,
                          deadzone_cons_t,
                          NULL,
                          ...
                          0,
                          NULL};
consvar_t cv_ydeadzone = {"joy_ydeadzone",
                          "0.5",
                          CV_FLOAT | CV_SAVE,
                          deadzone_cons_t,
                          NULL,
...
consvar_t cv_turnaxis2 = {"joyaxis2_turn",
                          "Left X",
                          CV_SAVE,
                          joyaxis_cons_t,
                          NULL,
                          ...
                          0,
                          NULL};
consvar_t cv_moveaxis2 = {"joyaxis2_move",
                          "None",
                          CV_SAVE,
                          joyaxis_cons_t,
                          NULL,
                          ...
                          0,
                          NULL};
consvar_t cv_brakeaxis2 = {"joyaxis2_brake",
                           "None",
                           CV_SAVE,
                           joyaxis_cons_t,
                           NULL,
                           ...
                           0,
                           NULL};
consvar_t cv_aimaxis2 = {"joyaxis2_aim",
                         "Left Y",
                         CV_SAVE,
                         joyaxis_cons_t,
                         NULL,
                         ...
                         0,
                         NULL};
consvar_t cv_lookaxis2 = {"joyaxis2_look",
                          "None",
                          CV_SAVE,
                          joyaxis_cons_t,
                          NULL,
                          ...
                          0,
                          NULL};
consvar_t cv_fireaxis2 = {"joyaxis2_fire",
                          "L Trigger",
                          CV_SAVE,
                          joyaxis_cons_t,
                          NULL,
                          ...
                          0,
                          NULL};
consvar_t cv_driftaxis2 = {"joyaxis2_drift",
                           "R Trigger",
                           CV_SAVE,
                           joyaxis_cons_t,
                           NULL,
                           ...
                           0,
                           NULL};
consvar_t cv_lookbackaxis2 = {"joyaxis2_lookback",
                              "None",
                              CV_SAVE,
                              joyaxis_cons_t,
                              NULL,
                              ...
                              0,
                              NULL};
consvar_t cv_xdeadzone2 = {"joy2_xdeadzone",
                           "0.3",
                           CV_FLOAT | CV_SAVE,
                           deadzone_cons_t,
                           NULL,
                           ...
                           0,
                           NULL};
consvar_t cv_ydeadzone2 = {"joy2_ydeadzone",
                           "0.5",
                           CV_FLOAT | CV_SAVE,
                           deadzone_cons_t,
                           NULL,
...
consvar_t cv_turnaxis3 = {"joyaxis3_turn",
                          "Left X",
                          CV_SAVE,
                          joyaxis_cons_t,
                          NULL,
                          ...
                          0,
                          NULL};
consvar_t cv_moveaxis3 = {"joyaxis3_move",
                          "None",
                          CV_SAVE,
                          joyaxis_cons_t,
                          NULL,
                          ...
                          0,
                          NULL};
consvar_t cv_brakeaxis3 = {"joyaxis3_brake",
                           "None",
                           CV_SAVE,
                           joyaxis_cons_t,
                           NULL,
                           ...
                           0,
                           NULL};
consvar_t cv_aimaxis3 = {"joyaxis3_aim",
                         "Left Y",
                         CV_SAVE,
                         joyaxis_cons_t,
                         NULL,
                         ...
                         0,
                         NULL};
consvar_t cv_lookaxis3 = {"joyaxis3_look",
                          "None",
                          CV_SAVE,
                          joyaxis_cons_t,
                          NULL,
                          ...
                          0,
                          NULL};
consvar_t cv_fireaxis3 = {"joyaxis3_fire",
                          "L Trigger",
                          CV_SAVE,
                          joyaxis_cons_t,
                          NULL,
                          ...
                          0,
                          NULL};
consvar_t cv_driftaxis3 = {"joyaxis3_drift",
                           "R Trigger",
                           CV_SAVE,
                           joyaxis_cons_t,
                           NULL,
                           ...
                           0,
                           NULL};
consvar_t cv_lookbackaxis3 = {"joyaxis3_lookback",
                              "None",
                              CV_SAVE,
                              joyaxis_cons_t,
                              NULL,
                              ...
                              0,
                              NULL};
consvar_t cv_xdeadzone3 = {"joy3_xdeadzone",
                           "0.3",
                           CV_FLOAT | CV_SAVE,
                           deadzone_cons_t,
                           NULL,
                           ...
                           0,
                           NULL};
consvar_t cv_ydeadzone3 = {"joy3_ydeadzone",
                           "0.5",
                           CV_FLOAT | CV_SAVE,
                           deadzone_cons_t,
                           NULL,
...
consvar_t cv_turnaxis4 = {"joyaxis4_turn",
                          "Left X",
                          CV_SAVE,
                          joyaxis_cons_t,
                          NULL,
                          ...
                          0,
                          NULL};
consvar_t cv_moveaxis4 = {"joyaxis4_move",
                          "None",
                          CV_SAVE,
                          joyaxis_cons_t,
                          NULL,
                          ...
                          0,
                          NULL};
consvar_t cv_brakeaxis4 = {"joyaxis4_brake",
                           "None",
                           CV_SAVE,
                           joyaxis_cons_t,
                           NULL,
                           ...
                           0,
                           NULL};
consvar_t cv_aimaxis4 = {"joyaxis4_aim",
                         "Left Y",
                         CV_SAVE,
                         joyaxis_cons_t,
                         NULL,
                         ...
                         0,
                         NULL};
consvar_t cv_lookaxis4 = {"joyaxis4_look",
                          "None",
                          CV_SAVE,
                          joyaxis_cons_t,
                          NULL,
                          ...
                          0,
                          NULL};
consvar_t cv_fireaxis4 = {"joyaxis4_fire",
                          "L Trigger",
                          CV_SAVE,
                          joyaxis_cons_t,
                          NULL,
                          ...
                          0,
                          NULL};
consvar_t cv_driftaxis4 = {"joyaxis4_drift",
                           "R Trigger",
                           CV_SAVE,
                           joyaxis_cons_t,
                           NULL,
                           ...
                           0,
                           NULL};
consvar_t cv_lookbackaxis4 = {"joyaxis4_lookback",
                              "None",
                              CV_SAVE,
                              joyaxis_cons_t,
                              NULL,
                              ...
                              0,
                              NULL};
consvar_t cv_xdeadzone4 = {"joy4_xdeadzone",
                           "0.3",
                           CV_FLOAT | CV_SAVE,
                           deadzone_cons_t,
                           NULL,
                           ...
                           0,
                           NULL};
consvar_t cv_ydeadzone4 = {"joy4_ydeadzone",
                           "0.5",
                           CV_FLOAT | CV_SAVE,
                           deadzone_cons_t,
                           NULL,
...
char player_names[MAXPLAYERS][MAXPLAYERNAME + 1] = {
    ...
    "Player 16"}; // SRB2kart - removed Players 17 through 32
...
{
        if (!nightsrecords[i])
                nightsrecords[i] = Z_Malloc(sizeof(nightsdata_t), PU_STATIC,
NULL); memset(nightsrecords[i], 0, sizeof(nightsdata_t));
}*/
...
void G_SetNightsRecords(void)
{
        ...
        const size_t glen =
strlen(srb2home)+1+strlen("replay")+1+strlen(timeattackfolder)+1+strlen("MAPXX")+1;
        char *gpath;
...
ntemprecords.nummares << FRACBITS) + (FRACUNIT/2)) >> FRACBITS);

                // You need ALL rainbow As to get a rainbow A overall
                if (realrank == GRADE_S && (totalrank / ntemprecords.nummares)
!= GRADE_S) realrank = GRADE_A;
        ...
        // Now take all temp records and put them in the actual records
        {
                ...
                for (i = 0; i < ntemprecords.nummares + 1; ++i)
                {
                        ...
                        if (!maprecords->time[i] || maprecords->time[i] >
ntemprecords.time[i]) maprecords->time[i] = ntemprecords.time[i];
                }
        }
...
timeattackfolder, G_BuildMapName(gamemap)); snprintf(lastdemo, 255,
"%s-last.lmp", gpath);
        ...
        if (FIL_FileExists(lastdemo))
        {
                ...
                if (!FIL_FileExists(bestdemo) || G_CmpDemoTime(bestdemo,
lastdemo) & 1) { // Better time, save this demo. if (FIL_FileExists(bestdemo))
                                remove(bestdemo);
                        FIL_WriteFile(bestdemo, buf, len);
                        CONS_Printf("\x83%s\x80 %s '%s'\n", M_GetText("NEW
RECORD TIME!"), M_GetText("Saved replay as"), bestdemo);
                }

                snprintf(bestdemo, 255, "%s-score-best.lmp", gpath);
                if (!FIL_FileExists(bestdemo) || (G_CmpDemoTime(bestdemo,
lastdemo) & (1<<1))) { // Better score, save this demo. if
(FIL_FileExists(bestdemo)) remove(bestdemo); FIL_WriteFile(bestdemo, buf, len);
                        CONS_Printf("\x83%s\x80 %s '%s'\n", M_GetText("NEW HIGH
SCORE!"), M_GetText("Saved replay as"), bestdemo);
                }

                //CONS_Printf("%s '%s'\n", M_GetText("Saved replay as"),
lastdemo);
...
void G_SetGameModified(boolean silent, boolean major) {
  if ((majormods && modifiedgame) || !mainwads ||
      (refreshdirmenu & REFRESHDIR_GAMEDATA)) // new gamedata amnesty?
    return;
...
const char *G_BuildMapName(INT32 map) {
  ...
  if (map == 0) // hack???
  {
    if (gamestate == GS_TITLESCREEN)
      ...
      map = prevmap;
    map =
        G_RandMap(G_TOLFlag(cv_newgametype.value), map, false, 0, false, NULL) +
        1;
  }
 ...
 */
INT16
G_ClipAimingPitch(INT32 *aiming) {
  INT32 limitangle;
...
INT16
G_SoftwareClipAimingPitch(INT32 *aiming) {
  INT32 limitangle;
  ...
  if (axisval % 2) {
    ...
    if (!Joystick.bGamepadStyle && axissel < AXISDEAD) {
      const INT32 jdeadzone =
          ((JOYAXISRANGE - 1) * cv_xdeadzone.value) >> FRACBITS;
    ...
    if (!Joystick.bGamepadStyle && axissel < AXISDEAD) {
      const INT32 jdeadzone =
          ((JOYAXISRANGE - 1) * cv_ydeadzone.value) >> FRACBITS;
      if (abs(retaxis) <= jdeadzone)
        return 0;
    }
    if (flp)
...
static INT32 Joy2Axis(axis_input_e axissel) {
  ...
  if (axisval % 2) {
    ...
    if (!Joystick2.bGamepadStyle && axissel < AXISDEAD) {
      const INT32 jdeadzone =
          ((JOYAXISRANGE - 1) * cv_xdeadzone2.value) >> FRACBITS;
    ...
    if (!Joystick2.bGamepadStyle && axissel < AXISDEAD) {
      const INT32 jdeadzone =
          ((JOYAXISRANGE - 1) * cv_ydeadzone2.value) >> FRACBITS;
      if (-jdeadzone < retaxis && retaxis < jdeadzone)
        return 0;
    }
    if (flp)
...
static INT32 Joy3Axis(axis_input_e axissel) {
  ...
  if (axisval % 2) {
    ...
    if (!Joystick3.bGamepadStyle && axissel < AXISDEAD) {
      const INT32 jdeadzone =
          ((JOYAXISRANGE - 1) * cv_xdeadzone3.value) >> FRACBITS;
      if (-jdeadzone < retaxis && retaxis < jdeadzone)
        return 0;
    }
    if (flp)
    ...
    if (!Joystick3.bGamepadStyle && axissel < AXISDEAD) {
      const INT32 jdeadzone =
          ((JOYAXISRANGE - 1) * cv_ydeadzone3.value) >> FRACBITS;
      if (-jdeadzone < retaxis && retaxis < jdeadzone)
        return 0;
    }
    if (flp)
...
static INT32 Joy4Axis(axis_input_e axissel) {
  ...
  if (axisval % 2) {
    ...
    if (!Joystick4.bGamepadStyle && axissel < AXISDEAD) {
      const INT32 jdeadzone =
          ((JOYAXISRANGE - 1) * cv_xdeadzone4.value) >> FRACBITS;
      if (-jdeadzone < retaxis && retaxis < jdeadzone)
        return 0;
    }
    if (flp)
    ...
    if (!Joystick4.bGamepadStyle && axissel < AXISDEAD) {
      const INT32 jdeadzone =
          ((JOYAXISRANGE - 1) * cv_ydeadzone4.value) >> FRACBITS;
      if (-jdeadzone < retaxis && retaxis < jdeadzone)
        return 0;
    }
    if (flp)
...
INT32
JoyAxis(axis_input_e axissel, UINT8 p) {
  switch (p) {
  case 2:
...
void G_BuildTiccmd(ticcmd_t *cmd, INT32 realtics, UINT8 ssplayer) {
  ...
  boolean turnleft, turnright, mouseaiming, analogjoystickmove,
      gamepadjoystickmove;
  ...
  static boolean keyboard_look[MAXSPLITSCREENPLAYERS]; // true if lookup/down
                                                       // using keyboard
  static boolean
      resetdown[MAXSPLITSCREENPLAYERS]; // don't cam reset every frame
  ...
  // Kart, don't build a ticcmd if someone is resynching or the server is
  // stopped too so we don't fly off course in bad conditions
  if (paused || P_AutoPause() ||
      (gamestate == GS_LEVEL && player->playerstate == PST_REBORN) ||
      hu_resynching) {
  ...
  switch (ssplayer) {
  case 2:
    mouseaiming = player->spectator; //(PLAYER2INPUTDOWN(gc_mouseaiming)) ^
  ...
  case 1:
  default:
    mouseaiming = player->spectator; //(PLAYER1INPUTDOWN(gc_mouseaiming)) ^
  ...
  if (analogjoystickmove && axis != 0) {
    ...
    cmd->angleturn =
        (INT16)(cmd->angleturn - (((axis * angleturn[1]) >> 10))); // ANALOG!
  ...
  if (player->spectator) {
    ...
    cmd->driftturn =
        (INT16)(cmd->driftturn - ((mousex * (encoremode ? -1 : 1) * 8)));
  }

  if (player->spectator ||
      objectplacing) // SRB2Kart: spectators need special controls
  {
    ...
    // forward with key or button // SRB2kart - we use an accel/brake instead of
    // forward/backward.
    axis = JoyAxis(AXISMOVE, ssplayer);
  ...
  // spectator aiming shit, ahhhh...
  {
    ...
    if (player->spectator) {
      if (InputDown(gc_lookup, ssplayer) || (gamepadjoystickmove && axis < 0)) {
        ...
        kbl = true;
      } else if (InputDown(gc_lookdown, ssplayer) ||
                 (gamepadjoystickmove && axis > 0)) {
        laim -= KB_LOOKSPEED * screen_invert;
        kbl = true;
      }
    ...
    if (InputDown(
            gc_centerview,
            ssplayer)) // No need to put a spectator limit on this one though :V
      laim = 0;
  ...
  // Not grouped with the rest of turn stuff because it needs to know what
  // buttons you're pressing for rubber-burn turn limit turning to angleturn[1]
  // to stop mouselook letting you look too fast
  ...
  if (((player->mo && player->speed > 0) // Moving
       ||
       (leveltime > starttime && (cmd->buttons & BT_ACCELERATE &&
  ...
  cmd->latency =
      modeattacking
          ? 0
          : (leveltime & 0xFF); // Send leveltime when this tic was generated to
                                // the server for control lag calculations
  ...
  if (!hu_stopped) {
    ...
    camspin[ssplayer - 1] =
        (InputDown(gc_lookback, ssplayer) || (usejoystick && axis > 0));
  }

  /* 	Lua: Allow this hook to overwrite ticcmd.
          We check if we're actually in a level because for some reason this
     Hook would run in menus and on the titlescreen otherwise. Be aware that
     within this hook, nothing but this player's cmd can be edited (otherwise
     we'd run in some pretty bad synching problems since this is clientsided, or
     something)

          Possible usages for this are:
                  -Forcing the player to perform an action, which could
     otherwise require terrible, terrible hacking to replicate. -Preventing the
     player to perform an action, which would ALSO require some weirdo hacks.
                  -Making some galaxy brain autopilot Lua if you're a masochist
                  -Making a Mario Kart 8 Deluxe tier baby mode that steers you
     away from walls and whatnot. You know what, do what you want!
  */
...
static INT32 camtoggledelay, camtoggledelay2, camtoggledelay3,
    camtoggledelay4 = 0;
...
boolean G_Responder(event_t *ev) {
  ...
  if (gameaction == ga_nothing && !demo.quitafterplaying &&
      ((demo.playback && !modeattacking && !demo.title && !multiplayer) ||
  ...
  // Demo End
  else if (gamestate == GS_GAMEEND || gamestate == GS_EVALUATION ||
  ...
  if (gamestate == GS_LEVEL && ev->type == ev_keydown &&
      (ev->data1 == KEY_F12 || ev->data1 == gamecontrol[gc_viewpoint][0] ||
       ev->data1 == gamecontrol[gc_viewpoint][1])) {
  ...
  if (gamestate == GS_LEVEL && ev->type == ev_keydown && multiplayer &&
      demo.playback && !demo.freecam) {
    if (ev->data1 == gamecontrolbis[gc_viewpoint][0] ||
        ev->data1 == gamecontrolbis[gc_viewpoint][1]) {
      G_AdjustView(2, 1, true);

      return true;
    } else if (ev->data1 == gamecontrol3[gc_viewpoint][0] ||
               ev->data1 == gamecontrol3[gc_viewpoint][1]) {
      G_AdjustView(3, 1, true);

      return true;
    } else if (ev->data1 == gamecontrol4[gc_viewpoint][0] ||
               ev->data1 == gamecontrol4[gc_viewpoint][1]) {
    ...
    if (ev->data1 == gamecontrol[gc_pause][0] ||
        ev->data1 == gamecontrol[gc_pause][1] || ev->data1 == KEY_PAUSE) {
      paused = !paused;
...
boolean G_CouldView(INT32 playernum) {
  ...
  else if (G_TagGametype())
  {
          if (!players[consoleplayer].spectator
           && (players[consoleplayer].pflags & PF_TAGIT) != (player->pflags &
  PF_TAGIT)) return false;
  }
...
boolean G_CanView(INT32 playernum, UINT8 viewnum, boolean onlyactive) {
  ...
  if (!(onlyactive
            ? G_CouldView(playernum)
            : (playeringame[playernum] && !players[playernum].spectator)))
    return false;
...
//
INT32
G_FindView(INT32 startview, UINT8 viewnum, boolean onlyactive,
           boolean reverse) {
...
INT32
G_CountPlayersPotentiallyViewable(boolean active) {
  INT32 total = 0;
  INT32 i;
  for (i = 0; i < MAXPLAYERS; ++i) {
...
void G_ResetView(UINT8 viewnum, INT32 playernum, boolean onlyactive) {
  ...
  if ((playernum = G_FindView(playernum, viewnum, onlyactive,
                              playernum < olddisplayplayer)) == -1)
    return;
...
void G_Ticker(boolean run) {
  ...
  ticcmd_t *cmd;
  UINT32 ra_timeskip =
      (modeattacking && !demo.playback && leveltime < starttime - TICRATE * 4)
          ? 0
          : (starttime - TICRATE * 4 - 1);
  // starttime - TICRATE*4 is where we want RA to start when we PLAY IT, so we
  // will loop the main thinker on RA start to get it to this point, the reason
  // this is done is to ensure that ghosts won't look out of synch with other
  // map elements (objects, moving platforms...) when we REPLAY, don't skip, let
  // the camera spin, do its thing etc~
  ...
  if (gamestate == GS_LEVEL) {
    ...
    if (!(netgame || multiplayer) && G_GetRetryFlag()) {
      ...
      // already.
      /*if (G_GametypeUsesLives() && players[consoleplayer].playerstate ==
      PST_LIVE) players[consoleplayer].lives -= 1;

      G_DoReborn(consoleplayer);*/

      D_MapChange(gamemap, gametype, cv_kartencore.value, true, 1, false,
                  false);
    }
  ...
  if (!demo.playback)
    ...
    for (i = 0; i < MAXPLAYERS; i++) {
      ...
      if (playeringame[i]) {
        //@TODO all this throwdir stuff shouldn't be here! But it stays for now
        // to maintain 1.0.4 compat...
        // Remove for 1.1!
        ...
        // Use the leveltime sent in the player's ticcmd to determine control
        // lag
        cmd->latency =
            modeattacking
                ? 0
                : min(((leveltime & 0xFF) - cmd->latency) & 0xFF,
                      MAXPREDICTTICS -
                          1); //@TODO add a cvar to allow setting this max
      }
    }
  ...
  case GS_LEVEL:
    ...
    for (; ra_timeskip < starttime - TICRATE * 4;
         ra_timeskip++) // this looks weird but this is done to not break
                        // compability with older demos for now.
    {
      if (demo.title)
        F_TitleDemoTicker();
  ...
  if (run) {
    if (G_GametypeHasSpectators() &&
        (gamestate == GS_LEVEL || gamestate == GS_INTERMISSION ||
         gamestate == GS_VOTING              // definitely good
         || gamestate == GS_WAITINGPLAYERS)) // definitely a problem if we don't
                                             // do it at all in this gamestate,
                                             // but might need more protection?
    {
...
static inline void G_PlayerFinishLevel(INT32 player) {
  ...
  if (player == consoleplayer) {
    if (legitimateexit && !demo.playback &&
        !mapreset) // (yes you're allowed to unlock stuff this way when the game
                   // is modified)
    {
      matchesplayed++;
...
void G_PlayerReborn(INT32 player) {
  ...
  pflags =
      (players[player].pflags & (PF_TIMEOVER | PF_FLIPCAM | PF_TAGIT |
                                 PF_TAGGED | PF_ANALOGMODE | PF_WANTSTOJOIN));
  ...
  // SRB2kart
  p->kartstuff[k_starpostwp] =
      starpostwp; // TODO: get these out of kartstuff, it causes desync
...
void G_SpawnPlayer(INT32 playernum, boolean starpost) {
  ...
  // Order: DM->CTF->Coop
  else if (gametype == GT_MATCH || gametype == GT_TEAMMATCH ||
           gametype == GT_CTF ||
  ...
  if (numcoopstarts) {
    ...
    for (i = 0; i < MAXPLAYERS; i++) {
      ...
      if (players[i].score < players[playernum].score) {
        ...
        for (j = 0; j < MAXPLAYERS;
             j++) // I hate similar loops inside loops... :<
        {
    ...
    if (playernum == consoleplayer ||
        ...
        (splitscreen > 2 && playernum == displayplayers[3]))
      CONS_Alert(CONS_WARNING,
                 M_GetText("Could not spawn at any Race starts!\n"));
    return NULL;
  }
...
void G_DoReborn(INT32 playernum) {
  ...
  if (player->bot &&
      playernum != consoleplayer) { // Bots respawn next to their master.
    mobj_t *oldmo = NULL;
  ...
  /*else if (countdowntimeup || (!multiplayer && !modeattacking))
  {
          ...
          if (!countdowntimeup && (mapheaderinfo[gamemap-1]->levelflags &
LF_NORELOAD))
          {
                  INT32 i;
                  ...
                  {
                          if (camera[i].chase)
                                  P_ResetCamera(&players[displayplayers[i]],
&camera[i]);
                  }
...
void G_AddPlayer(INT32 playernum) {
  ...
  demo_extradata[playernum] |=
      DXD_PLAYSTATE | DXD_COLOR | DXD_NAME | DXD_SKIN; // Set everything
}
...
//
INT32
G_GetGametypeByName(const char *gametypestr) {
  INT32 i;
...
boolean G_GametypeHasSpectators(void) {
  // SRB2Kart: We don't have any exceptions to not being able to spectate yet.
  // Maybe when SP & bots roll around.
#if 0
...
//
boolean G_BattleGametype(void) { return (gametype == GT_MATCH); }
...
//
UINT8
G_SometimesGetDifferentGametype(UINT8 prefgametype) {
  // Most of the gametype references in this condition are intentionally not
  // prefgametype. This is so a server CAN continue playing a gametype if they
  // like the taste of it. The encore check needs prefgametype so can't use
  // G_RaceGametype...
  ...
  if (randmapbuffer[NUMMAPS] > 0 &&
      (cv_kartvoterulechanges.value !=
       3)) // used to be (encorepossible || rule changes != 3)
  {
    randmapbuffer[NUMMAPS]--;
    return (gametype | encoremodifier);
  }

  switch (cv_kartvoterulechanges.value) // okay, we're having a gametype change!
                                        // when's the next one, luv?
  {
  case 1:                       // sometimes
    randmapbuffer[NUMMAPS] = 5; // per "cup"
    break;
  default:
    // fallthrough - happens when clearing buffer, but needs a reasonable
    // countdown if cvar is modified
  case 2:                       // frequent
  ...
  // This might appear wrong HERE, but the game will display the Encore
  // possibility on the second voting choice instead.
...
//
boolean G_RaceGametype(void) { return (gametype == GT_RACE); }
 ...
 */
INT16
G_TOLFlag(INT32 pgametype) {
  if (!multiplayer)
    return TOL_SP;
  if (pgametype == GT_COOP)
    return TOL_RACE; // SRB2kart
  if (pgametype == GT_COMPETITION)
    return TOL_COMPETITION;
  if (pgametype == GT_RACE)
    return TOL_RACE;
  if (pgametype == GT_MATCH)
    return TOL_MATCH;
  if (pgametype == GT_TEAMMATCH)
    return TOL_MATCH;
  if (pgametype == GT_TAG)
    return TOL_TAG;
  if (pgametype == GT_HIDEANDSEEK)
    return TOL_TAG;
  if (pgametype == GT_CTF)
    return TOL_CTF;
...
INT16
G_RandMap(INT16 tolflags, INT16 pprevmap, boolean ignorebuffer, UINT8 maphell,
          boolean callagainsoon, INT16 *extbuffer) {
  INT32 numokmaps = 0;
  INT16 ix, bufx;
  UINT16 extbufsize = 0;
...
tryagain:
  ...
  usehellmaps =
      (maphell == 0 ? false
                    : (maphell == 2 ||
                       M_RandomChance(FRACUNIT / 100))); // 1% chance of Hell
  ...
  for (ix = 0; ix < NUMMAPS; ix++) {
    ...
    if ((mapheaderinfo[ix]->typeoflevel & tolflags) != tolflags ||
        ix == pprevmap || (!dedicated && M_MapLocked(ix + 1)) ||
        (usehellmaps !=
    ...
    {
      lumpnum_t l;
      if ((l = W_CheckNumForName(va("%sS01", G_BuildMapName(ix + 1)))) ==
          LUMPERROR)
        continue;
    }
  ...
  if (numokmaps == 0) // If there's no matches... (Goodbye, incredibly silly
                      // function chains :V)
  {
    if (!ignorebuffer) {
      ...
      {
        ignorebuffer =
            true; // This will probably only help in situations where there's
                  // very few maps, but it's folly not to at least try it
        goto tryagain;
      }

      for (bufx = 3; bufx < NUMMAPS;
           bufx++) // Let's clear all but the three most recent maps...
    ...
    for (bufx = 0; bufx < NUMMAPS + 1; bufx++)
      randmapbuffer[bufx] = -1; // if we're having trouble finding a map we
...
void G_AddMapToBuffer(INT16 map) {
  ...
  if (randmapbuffer[refreshnum] != -1) {
    ...
    for (bufx = 5; bufx < NUMMAPS;
         bufx++) // bufx < refreshnum? Might not handle everything for gametype
                 // switches, though.
      randmapbuffer[bufx] = -1;
...
static void G_DoCompleted(void) {
  ...
  // play some generic music if there's no win/cool/lose music going on (for
  // exitlevel commands)
  if (G_RaceGametype() &&
      ((multiplayer && demo.playback) || j == splitscreen + 1) &&
      (cv_inttime.value > 0))
  ...
  if (demo.playback)
    goto demointermission;

  // go to next level
  // nextmap is 0-based, unlike gamemap
  if (nextmapoverride != 0)
    nextmap = (INT16)(nextmapoverride - 1);
  else if (mapheaderinfo[gamemap - 1]->nextlevel ==
           1101) // SRB2Kart: !!! WHENEVER WE GET GRAND PRIX, GO TO AWARDS MAP
                 // INSTEAD !!!
    nextmap = (INT16)(mapheaderinfo[gamemap]
                          ? gamemap
                          : (spstage_start - 1)); // (gamemap-1)+1 == gamemap :V
  else
  ...
  if (!token && !G_IsSpecialStage(gamemap) && !modeattacking &&
    ...
    while (!mapheaderinfo[cm] || !(mapheaderinfo[cm]->typeoflevel & tolflag)) {
      ...
      {
        cm = nextmap; // Start the loop again so that the error checking below
                      // is executed.
      ...
      if (visitedmap[cm / 8] & (1 << (cm & 7))) // smells familiar
      {
        ...
        CONS_Alert(CONS_WARNING,
                   M_GetText("Can't find a compatible map after map %d; using "
                             "map %d anyway\n"),
                   prevmap + 1, cm + 1);
        break;
      }
...
void G_AfterIntermission(void) {
  ...
  if (demo.playback) {
    ...
    return;
             ...
             (modeattacking || demo.savemode != DSM_NOTSAVING))
    G_SaveDemo();
...
void G_NextLevel(void) {
  if (gamestate != GS_VOTING) {
    if ((cv_advancemap.value == 3) && !modeattacking && !skipstats &&
        (multiplayer || netgame)) {
      UINT8 i;
      for (i = 0; i < MAXPLAYERS; i++) {
        if (playeringame[i] && !players[i].spectator) {
...
static void G_DoWorldDone(void) {
  if (server) {
    // SRB2Kart
    D_MapChange(nextmap + 1, gametype, deferencoremode, forceresetplayers, 0,
                false, false);
  }
...
void G_SaveGameData(boolean force) {
  ...
  btemp =
      (UINT8)(savemoddata); // what used to be here was profoundly dunderheaded
  ...
  /*for (i = 0; i < NUMMAPS; i++)
  {
          ...
          for (curmare = 0; curmare < (nightsrecords[i]->nummares + 1);
  ++curmare)
          {
...
//
void G_DeferedInitNew(boolean pencoremode, const char *mapname,
                      INT32 pickedchar, UINT8 ssplayers, boolean FLS) {
  INT32 i;
  UINT8 color = 0;
  paused = false;
  ...
  if (mapname)
    D_MapChange(M_MapNumber(mapname[3], mapname[4]), gametype, pencoremode,
                true, 1, false, FLS);
}
...
void G_InitNew(UINT8 pencoremode, const char *mapname, boolean resetplayer,
  ...
  if (!demo.playback && !netgame) // Netgame sets random seed elsewhere, demo
                                  // playback sets seed just before us!
    P_SetRandSeed(M_RandomizedSeed()); // Use a more "Random" random seed

  // SRB2Kart - Score is literally the only thing you SHOULDN'T reset at all
  // times if (resetplayer)
  {
    ...
    for (i = 0; i < MAXPLAYERS; i++) {
      players[i].playerstate = PST_REBORN;
      players[i].starpostangle = players[i].starpostnum =
  ...
  if (!skipprecutscene && mapheaderinfo[gamemap - 1]->precutscenenum &&
      !modeattacking) // Start a custom cutscene.
...
INT32
G_FindMap(const char *mapname, char **foundmapnamep, mapsearchfreq_t **freqp,
          INT32 *freqcp) {
  INT32 newmapnum = 0;
  INT32 mapnum;
  INT32 apromapnum = 0;
...
INT32
G_FindMapByNameOrCode(const char *mapname, char **realmapnamep) {
...
#define DF_RECORDATTACK                                                        \
  0x02 // This demo is from record attack and contains its final completion
       // time!
#define DF_NIGHTSATTACK                                                        \
  0x04 // This demo is from NiGHTS attack and contains its time left, score, and
       // mares!
#define DF_ATTACKMASK 0x06 // This demo is from ??? attack and contains ???

#define DF_LUAVARS                                                             \
  0x20 // this demo contains extra lua vars; this is mostly used for backwards
       // compability
...
#define DW_EXTRASTUFF                                                          \
  0xFE // Numbers below this are reserved for writing player slot data
...
#define EZT_SPIN                                                               \
  0x02               // Because one type of thok object apparently wasn't enough
#define EZT_REV 0x03 // And two types wasn't enough either yet
#define EZT_THOKMASK 0x03
#define EZT_COLOR                                                              \
  0x04                 // Changed color (Super transformation, Mario
                       // fireflowers/invulnerability, etc.)
#define EZT_FLIP 0x08  // Reversed gravity
#define EZT_SCALE 0x10 // Changed size
#define EZT_HIT 0x20   // Damaged a mobj
#define EZT_SPRITE                                                             \
  0x40 // Changed sprite set completely out of PLAY (NiGHTS, SOCs, whatever)
#define EZT_KART                                                               \
  0x80 // SRB2Kart: Changed current held item/quantity and bumpers for battle
...
static INT32 GetSkinNumClosestToStats(UINT8 kartspeed, UINT8 kartweight) {
  ...
  for (i = 0; i < numskins; i++) {
    stat_diff = abs(skins[i].kartspeed - kartspeed) +
                abs(skins[i].kartweight - kartweight);
...
static void FindClosestSkinForStats(UINT32 p, UINT8 kartspeed,
                                    UINT8 kartweight) {
...
void G_ReadDemoExtraData(void) {
  ...
  while (p < DW_EXTRASTUFF) {
    ...
    if (extradata & DXD_RESPAWN) {
      if (players[p].mo)
        P_DamageMobj(players[p].mo, NULL, NULL,
                     10000); // Is this how this should work..?
    }
    ...
    if (extradata & DXD_PLAYSTATE) {
      ...
      case DXD_PST_SPECTATING:
        ...
        if (!playeringame[p]) {
          ...
          // There's likely an off-by-one error in timing recording or
          // playback of joins. This hacks around it so I don't have to find
          // out where that is. \o/
      ...
      if (demo_extradata[i] & DXD_PLAYSTATE) {
        ...
        if (!playeringame[i])
          WRITEUINT8(demo_p, DXD_PST_LEFT);
                 ...
                 !(players[i].pflags &
                   PF_WANTSTOJOIN) // <= fuck you specifically
        )
...
void G_GhostAddThok(INT32 playernum) {
  if (!demo.recording || !(demoflags & DF_GHOST))
    return;
  ghostext[playernum].flags =
      (ghostext[playernum].flags & ~EZT_THOKMASK) | EZT_THOK;
}
...
void G_GhostAddSpin(INT32 playernum) {
  if (!demo.recording || !(demoflags & DF_GHOST))
    return;
  ghostext[playernum].flags =
      (ghostext[playernum].flags & ~EZT_THOKMASK) | EZT_SPIN;
}
...
void G_GhostAddRev(INT32 playernum) {
  if (!demo.recording || !(demoflags & DF_GHOST))
    return;
  ghostext[playernum].flags =
      (ghostext[playernum].flags & ~EZT_THOKMASK) | EZT_REV;
}
...
void G_GhostAddHit(INT32 playernum, mobj_t *victim) {
  ...
  ghostext[playernum].hitlist =
      Z_Realloc(ghostext[playernum].hitlist,
                ghostext[playernum].hits * sizeof(mobj_t *), PU_LEVEL, NULL);
...
void G_WriteAllGhostTics(void) {
  ...
  for (i = 0; i < MAXPLAYERS; i++) {
    ...
    if (counter % cv_netdemosyncquality.value !=
        0) // Only write 1 in this many ghost datas per tic to cut down on
           // multiplayer replay size.
      continue;
...
#define MAXMOM (0x7FFF << 8)
  ...
  // GZT_XYZ is only useful if you've moved 256 FRACUNITS or more in a single
  // tic.
  if (abs(ghost->x - oldghost[playernum].x) > MAXMOM ||
      abs(ghost->y - oldghost[playernum].y) > MAXMOM ||
      abs(ghost->z - oldghost[playernum].z) > MAXMOM ||
      ((UINT8)(leveltime & 255) > 0 &&
       (UINT8)(leveltime & 255) <=
           (UINT8)cv_netdemosyncquality
               .value)) // Hack to enable slightly nicer resyncing
  {
    ...
    if (momx != oldghost[playernum].momx || momy != oldghost[playernum].momy) {
...
#undef MAXMOM
  ...
  // because exact values aren't too easy to discern to begin with when only 8
  // angles have different sprites and it does not affect this mode of movement
  // at all anyway.
  ...
  if (ghost->player) {
    if (ghostext[playernum].kartitem != ghost->player->kartstuff[k_itemtype] ||
        ghostext[playernum].kartamount !=
            ghost->player->kartstuff[k_itemamount] ||
  ...
  if (ghostext[playernum].flags) {
    ...
    if (ghostext[playernum].flags & EZT_HIT) {
      ...
      for (i = 0; i < ghostext[playernum].hits; i++) {
        ...
        WRITEUINT32(
            demo_p,
            UINT32_MAX); // reserved for some method of determining exactly
                         // which mobj this is. (mobjnum doesn't work here.)
...
void G_ConsGhostTic(INT32 playernum) {
  ...
  if (ziptic & GZT_NIGHTS) {
    if (!testmo || !testmo->player ||
        !(testmo->player->pflags & PF_NIGHTSMODE) || !testmo->tracer)
      nightsfail = true;
    else
      testmo = testmo->tracer;
  }

  if (ziptic & GZT_EXTRA) { // But wait, there's more!
    ziptic = READUINT8(demo_p);
    if (ziptic & EZT_COLOR)
      demo_p++;
    if (ziptic & EZT_SCALE)
      demo_p += sizeof(fixed_t);
    if (ziptic & EZT_HIT) { // Resync mob damage.
      ...
      for (i = 0; i < count; i++) {
        ...
        for (th = thinkercap.next; th != &thinkercap; th = th->next) {
          if (th->function.acp1 != (actionf_p1)P_MobjThinker)
            continue;
          mobj = (mobj_t *)th;
          if (mobj->type == (mobjtype_t)type && mobj->x == x && mobj->y == y &&
              mobj->z == z)
            break;
          mobj = NULL; // wasn't this one, keep searching.
        }
        if (mobj &&
            mobj->health != health) // Wasn't damaged?! This is desync! Fix it!
        {
          if (demosynced)
            CONS_Alert(CONS_WARNING,
                       M_GetText("Demo playback has desynced!\n"));
          demosynced = false;
  ...
  if (testmo) {
    ...
    if (nightsfail || abs(px - gx) > syncleeway || abs(py - gy) > syncleeway ||
        abs(pz - gz) > syncleeway) {
      ...
      if (ghostext[playernum].desyncframes >= 2) {
        ...
        if (abs(testmo->z - testmo->floorz) < 4 * FRACUNIT)
          testmo->z = testmo->floorz; // Sync players to the ground when they're
                                      // likely supposed to be there...
    ...
    if (
        ...
        demo.version != 0x0001 &&
#endif
        (players[playernum].kartstuff[k_itemtype] !=
             ghostext[playernum].kartitem ||
         players[playernum].kartstuff[k_itemamount] !=
             ghostext[playernum].kartamount ||
         players[playernum].kartstuff[k_bumper] !=
             ghostext[playernum].kartbumpers)) {
      if (demosynced)
        CONS_Alert(CONS_WARNING, M_GetText("Demo playback has desynced!\n"));
      demosynced = false;
...
void G_GhostTicker(void) {
  ...
  for (g = ghosts, p = NULL; g; g = g->next) {
    ...
    if (g->version != 0x0001) {
      ...
      while (ziptic != DW_END) // Get rid of extradata stuff
      {
        if (ziptic == 0) // Only support player 0 info for now
        {
          ...
          if (ziptic & DXD_SKIN)
            g->p += 18; // We _could_ read this info, but it shouldn't change
                        // anything in record attack...
          if (ziptic & DXD_COLOR)
            g->p += 16; // Same tbh
          if (ziptic & DXD_NAME)
            g->p += 16; // yea
          if (ziptic & DXD_PLAYSTATE && READUINT8(g->p) != DXD_PST_PLAYING)
            I_Error("Ghost is not a record attack ghost"); //@TODO lmao don't
                                                           // blow up like this
        } else if (ziptic == DW_RNG)
          g->p += 4; // RNG seed
        else
          I_Error("Ghost is not a record attack ghost"); //@TODO lmao don't blow
                                                         // up like this
    ...
    if (ziptic & GZT_EXTRA) { // But wait, there's more!
      ...
      if (ziptic &
          EZT_THOKMASK) { // Let's only spawn ONE of these per frame, thanks.
        mobj_t *mobj;
        INT32 type = -1;
        if (g->mo->skin) {
        ...
        if (type == MT_GHOST) {
          mobj = P_SpawnGhostMobj(
              g->mo); // does a large portion of the work for us
          mobj->frame = (mobj->frame & ~FF_FRAMEMASK) |
                        tr_trans60 << FF_TRANSSHIFT; // P_SpawnGhostMobj sets
                                                     // trans50, we want trans60
        } else {
          mobj = P_SpawnMobj(
              g->mo->x, g->mo->y,
              g->mo->z - FixedDiv(FixedMul(g->mo->info->height, g->mo->scale) -
                                      g->mo->height,
                                  3 * FRACUNIT),
              MT_THOK);
          mobj->sprite = states[mobjinfo[type].spawnstate].sprite;
          mobj->frame =
              (states[mobjinfo[type].spawnstate].frame & FF_FRAMEMASK) |
              tr_trans60 << FF_TRANSSHIFT;
          mobj->tics = -1; // nope.
        ...
        mobj->flags = MF_NOBLOCKMAP | MF_NOCLIP | MF_NOCLIPHEIGHT |
                      MF_NOGRAVITY; // make an ATTEMPT to curb crazy SOCs
                                    // fucking stuff up...
        P_SetThingPosition(mobj);
        mobj->fuse = 8;
        P_SetTarget(&mobj->target, g->mo);
      }
      if (ziptic & EZT_HIT) { // Spawn hit poofs for killing things!
        ...
        for (i = 0; i < count; i++) {
          ...
          angle = READANGLE(g->p);
          if (health != 0 ||
              i >= 4) // only spawn for the first 4 hits per frame, to prevent
                      // ghosts from splode-spamming too bad.
            continue;
          poof = P_SpawnMobj(x, y, z, MT_GHOST);
          poof->angle = angle;
          poof->flags = MF_NOBLOCKMAP | MF_NOCLIP | MF_NOCLIPHEIGHT |
                        MF_NOGRAVITY; // make an ATTEMPT to curb crazy SOCs
                                      // fucking stuff up...
          poof->health = 0;
...
#endif
      if (READUINT8(g->p) !=
          0xFF) // Make sure there isn't other ghost data here.
        I_Error("Ghost is not a record attack ghost"); //@TODO lmao don't blow
                                                       // up like this
#ifdef DEMO_COMPAT_100
    }
...
void G_PreviewRewind(tic_t previewtime) {
  ...
  if (info != next_info)
    tweenvalue = FixedDiv(previewtime - info->leveltime,
                          next_info->leveltime - info->leveltime);
...
#define TWEEN(pr)                                                              \
  info->playerinfo[i].mobj.pr +                                                \
      FixedMul((INT32)(next_info->playerinfo[i].mobj.pr -                      \
                       info->playerinfo[i].mobj.pr),                           \
               tweenvalue)
...
#undef TWEEN
    ...
    players[i].frameangle =
        info->playerinfo[i].player.frameangle +
        FixedMul((INT32)(next_info->playerinfo[i].player.frameangle -
                         info->playerinfo[i].player.frameangle),
                 tweenvalue);
...
void G_ConfirmRewind(tic_t rewindtime) {
  ...
  INT32 olddp1 = displayplayers[0], olddp2 = displayplayers[1],
        olddp3 = displayplayers[2], olddp4 = displayplayers[3];
...
void G_ReadMetalTic(mobj_t *metal) {
  ...
  if (ziptic & GZT_XYZ) {
    P_MoveOrigin(metal, READFIXED(metal_p), READFIXED(metal_p),
                 READFIXED(metal_p));
    oldmetal.x = metal->x;
    oldmetal.y = metal->y;
    oldmetal.z = metal->z;
  ...
  if (ziptic & GZT_SPRITE)
    metal_p++; // Currently unused. (Metal Sonic figures out what he's doing his
               // own damn self.)
  ...
  speed =
      FixedDiv(P_AproxDistance(oldmetal.momx, oldmetal.momy), metal->scale) >>
      FRACBITS;
...
void G_WriteMetalTic(mobj_t *metal) {
  ...
  if (!demo_p) // demo_p will be NULL until the race start linedef executor is
               // triggered!
    return;
...
#undef MAXMOM
  ...
  // Only store the 8 most relevant bits of angle
  // because exact values aren't too easy to discern to begin with when only 8
  // angles have different sprites and it does not affect movement at all
  // anyway.
...

  {
    UINT8 *exttic_p = NULL;
    UINT8 exttic = 0;
    if ((metal->eflags & MFE_VERTICALFLIP) !=
...
void G_BeginRecording(void) {
  ...
  demo_p = demobuffer;
  demoflags = DF_GHOST | (multiplayer ? DF_MULTIPLAYER
                                      : (modeattacking << DF_ATTACKSHIFT));

  if (encoremode)
    demoflags |= DF_ENCORE;

  if (!modeattacking &&
      gL) // Ghosts don't read luavars, and you shouldn't ever need to save Lua
          // in replays, you doof! SERIOUSLY THOUGH WHY WOULD YOU LOAD HOSTMOD
          // AND RECORD A GHOST WITH IT !????
    demoflags |= DF_LUAVARS;

  // Setup header.
  M_Memcpy(demo_p, DEMOHEADER, 12);
  demo_p += 12;
  ...
  {
    char *title = G_BuildMapTitle(gamemap);
    snprintf(demo.titlename, 64, "%s - %s", title,
             modeattacking ? "Time Attack" : connectedservername);
    Z_Free(title);
  }
  ...
  // game data
  M_Memcpy(demo_p, "PLAY", 4);
  demo_p += 4;
  WRITEINT16(demo_p, gamemap);
  M_Memcpy(demo_p, mapmd5, 16);
  demo_p += 16;
...
void G_WriteStanding(UINT8 ranking, char *name, INT32 skinnum, UINT8 color,
                     UINT32 val) {
  char temp[16];
    ...
    if (!toomany) {
      ...
      if (toomany) {
        CONS_Alert(
            CONS_WARNING,
            M_GetText(
                "Too many files loaded to add anymore for demo playback\n"));
        if (!CON_Ready())
          M_StartMessage(
              M_GetText("There are too many files loaded to add this demo's "
        ...
        if (!CON_Ready())
          M_StartMessage(
              M_GetText("There were errors trying to add this demo's addons. "
...
// Enabling quick prevents filesystem checks to see if needed files are
// available to load.
...
UINT8
G_CmpDemoTime(char *oldname, char *newname) {
  UINT8 *buffer, *p;
  UINT8 flags;
  ...
  if (memcmp(p, DEMOHEADER, 12)) {
    CONS_Alert(CONS_NOTICE,
               M_GetText("File '%s' invalid format. It will be overwritten.\n"),
               oldname);
    Z_Free(buffer);
    return UINT8_MAX;
  }
  ...
  case 0x0001:
    ...
    CONS_Alert(
        CONS_NOTICE,
        M_GetText(
            "File '%s' outdated version. It will be overwritten. Nyeheheh.\n"),
        oldname);
    Z_Free(buffer);
    return UINT8_MAX;
#endif
  ...
  if (!(flags & aflags)) {
    CONS_Alert(
        CONS_NOTICE,
        M_GetText(
            "File '%s' not from same game mode. It will be overwritten.\n"),
        oldname);
    Z_Free(buffer);
    return UINT8_MAX;
  }
  ...
  c = 0;
  if (newtime < oldtime || (newtime == oldtime && (newlap < oldlap)))
    c |= 1; // Better time
  if (newlap < oldlap || (newlap == oldlap && newtime < oldtime))
...
void G_LoadDemoInfo(menudemo_t *pdemo) {
  ...
  default:
    CONS_Alert(
        CONS_ERROR,
        M_GetText(
            "%s is an incompatible replay format and cannot be played.\n"),
        pdemo->filepath);
    pdemo->type = MD_INVALID;
    sprintf(pdemo->title, "INVALID REPLAY");
    Z_Free(infobuffer);
  ...
  if (memcmp(info_p, "PLAY", 4)) {
    CONS_Alert(
        CONS_ERROR,
        M_GetText("%s is the wrong type of recording and cannot be played.\n"),
        pdemo->filepath);
    pdemo->type = MD_INVALID;
    sprintf(pdemo->title, "INVALID REPLAY");
    Z_Free(infobuffer);
  ...
  if (!(pdemoflags & DF_MULTIPLAYER)) {
    CONS_Alert(CONS_ERROR,
               M_GetText("%s is not a multiplayer replay and can't be listed "
                         "on this menu fully yet.\n"),
               pdemo->filepath);
    Z_Free(infobuffer);
    return;
  }
    ...
    CONS_Alert(
        CONS_ERROR,
        M_GetText("%s is a legacy multiplayer replay and cannot be played.\n"),
        pdemo->filepath);
    pdemo->type = MD_INVALID;
    sprintf(pdemo->title, "INVALID REPLAY");
    Z_Free(infobuffer);
  ...
  while (READUINT8(extrainfo_p) ==
         DW_STANDING) // Assume standings are always first in the extrainfo
  {
    INT32 i;
    char temp[16];
    ...
    if (count >= MAXPLAYERS)
      break; //@TODO still cycle through the rest of these if extra demo data is
             // ever used
  }
...
void G_DoPlayDemo(char *defdemoname) {
...
#endif
  boolean spectator;
  UINT8 slots[MAXPLAYERS], kartspeed[MAXPLAYERS], kartweight[MAXPLAYERS],
      numslots = 0;
  ...
  if (defdemoname == NULL) {
    demo_p = demobuffer;
    pdemoname =
        ZZ_Alloc(1); // Easier than adding checks for this everywhere it's freed
  } else {
...
#if defined(SKIPERRORS) && !defined(DEVELOP)
      skiperrors =
          true; // SRB2Kart: Don't print warnings for staff ghosts, since
                // they'll inevitably happen when we make bugfixes/changes...
#endif
    }
  }
  ...
  if (demo.version == 0x0001) {
    if (demoflags & DF_MULTIPLAYER) {
      snprintf(msg, 1024,
               M_GetText(
                   "%s is an alpha multiplayer replay and cannot be played.\n"),
               pdemoname);
      CONS_Alert(CONS_ERROR, "%s", msg);
      M_StartMessage(msg, NULL, MM_NOTHING);
      Z_Free(pdemoname);
    ...
    if (demo.title) // Titledemos should always play and ought to always be
                    // compatible with whatever wadlist is running.
      ...
      if (error) {
        ...
        case DFILE_ERROR_NOTLOADED:
          snprintf(msg, 1024,
                   M_GetText("Required files for this demo are not "
                             "loaded.\n\nUse\n\"playdemo %s -addfiles\"\nto "
                             "load them and play the demo.\n"),
                   pdemoname);
          break;

        case DFILE_ERROR_OUTOFORDER:
          snprintf(msg, 1024,
                   M_GetText("Required files for this demo are loaded out of "
                             "order.\n\nUse\n\"playdemo %s -force\"\nto play "
                             "the demo anyway.\n"),
                   pdemoname);
          break;
        ...
        case DFILE_ERROR_INCOMPLETEOUTOFORDER:
          snprintf(
              msg, 1024,
              M_GetText(
                  "Required files for this demo are not loaded, and some are "
                  "out "
                  "of order.\n\nUse\n\"playdemo %s -addfiles\"\nto load needed "
                  "files and play the demo.\n"),
              pdemoname);
          break;

        case DFILE_ERROR_CANNOTLOAD:
          snprintf(msg, 1024,
                   M_GetText("Required files for this demo cannot be "
                             "loaded.\n\nUse\n\"playdemo %s -force\"\nto "
                             "play the demo anyway.\n"),
                   pdemoname);
          break;

        case DFILE_ERROR_EXTRAFILES:
          snprintf(msg, 1024,
                   M_GetText("You have additional files loaded beyond the "
                             "demo's file list.\n\nUse\n\"playdemo %s "
        ...
        CONS_Alert(CONS_ERROR, "%s", msg);
        if (!CON_Ready()) // In the console they'll just see the notice there!
                          // No point pulling them out.
          M_StartMessage(msg, NULL, MM_NOTHING);
        Z_Free(pdemoname);
        Z_Free(demobuffer);
        demobuffer = NULL;
  ...
  if (demo.version == 0x0001) {
    ...
    if (!SetPlayerSkin(0, skin)) {
      snprintf(
          msg, 1024,
          M_GetText("%s features a character that is not currently loaded.\n"),
          pdemoname);
      CONS_Alert(CONS_ERROR, "%s", msg);
      M_StartMessage(msg, NULL, MM_NOTHING);
      Z_Free(pdemoname);
    ...
    if (!gamemap || (gamemap > NUMMAPS) || !mapheaderinfo[gamemap - 1] ||
        !(mapheaderinfo[gamemap - 1]->menuflags & LF2_EXISTSHACK)) {
      snprintf(
          msg, 1024,
          M_GetText("%s features a course that is not currently loaded.\n"),
          pdemoname);
      CONS_Alert(CONS_ERROR, "%s", msg);
      M_StartMessage(msg, NULL, MM_NOTHING);
      Z_Free(pdemoname);
    ...
    CONS_Alert(
        CONS_WARNING,
        M_GetText(
            "Demo version does not match game version. Desyncs may occur.\n"));
  ...
  while (p != 0xFF) {
    ...
    if (p & DEMO_SPECTATOR) {
      ...
      if (modeattacking) {
        snprintf(msg, 1024,
                 M_GetText("%s is a Record Attack replay with spectators, and "
                           "is thus invalid.\n"),
                 pdemoname);
        CONS_Alert(CONS_ERROR, "%s", msg);
        M_StartMessage(msg, NULL, MM_NOTHING);
    ...
    if (modeattacking && numslots > 1) {
      snprintf(msg, 1024,
               M_GetText("%s is a Record Attack replay with multiple players, "
                         "and is thus invalid.\n"),
               pdemoname);
      CONS_Alert(CONS_ERROR, "%s", msg);
      M_StartMessage(msg, NULL, MM_NOTHING);
  ...
  if (demoflags &
      DF_LUAVARS) // again, used for compability, lua shit will be saved to
                  // replays regardless of if it's even been loaded
  {
  ...
  if (demo.title) {
    ...
    splitscreen =
        min(min(3, numslots - 1), splitscreen); // Bias toward 1p and 4p views
...
post_compat:
  ...
  G_InitNew(demoflags & DF_ENCORE, G_BuildMapName(gamemap), true,
            true); // Doesn't matter whether you reset or not here, given
                   // changes to resetplayer.
...
void G_AddGhost(char *defdemoname) {
  ...
  for (gh = ghosts; gh; gh = gh->next)
    if (!memcmp(md5, gh->checksum,
                16)) // another ghost in the game already has this checksum?
  ...
  if (!(flags & DF_GHOST)) {
    CONS_Alert(CONS_NOTICE,
               M_GetText("Ghost %s: No ghost data in this demo.\n"), pdemoname);
    Z_Free(pdemoname);
    Z_Free(buffer);
    return;
  }
...
#ifdef DEMO_COMPAT_100
  if (ghostversion == 0x0001) {
    // Player name (TODO: Display this somehow if it doesn't match
    // cv_playername!)
    M_Memcpy(name, p, 16);
    p += 16;
  ...
  if (*p == DEMOMARKER) {
    CONS_Alert(CONS_NOTICE,
               M_GetText("Failed to add ghost %s: Replay is empty.\n"),
               pdemoname);
    Z_Free(pdemoname);
    Z_Free(buffer);
    return;
  ...
  if (ghostversion != 0x0001) {
    ...
    if (READUINT8(p) != 0) {
      CONS_Alert(CONS_NOTICE,
                 M_GetText("Failed to add ghost %s: Invalid player slot.\n"),
                 pdemoname);
      Z_Free(pdemoname);
      Z_Free(buffer);
      return;
  ...
  if (i == numskins) {
    ...
    CONS_Alert(CONS_NOTICE,
               M_GetText("Ghost %s: Invalid character. Falling back to %s.\n"),
  ...
  I_Assert(mthing);
  { // A bit more complex than P_SpawnPlayer because ghosts aren't solid and
    // won't just push themselves out of the ceiling.
    fixed_t z, f, c;
    gh->mo =
        P_SpawnMobj(mthing->x << FRACBITS, mthing->y << FRACBITS, 0, MT_GHOST);
  ...
  gh->mo->frame = (gh->mo->state->frame & FF_FRAMEMASK) | tr_trans20
                                                              << FF_TRANSSHIFT;
  gh->mo->tics = -1;
...
void G_DoPlayMetal(void) {
  ...
  if ((l = W_CheckNumForName(va("%sMS", G_BuildMapName(gamemap)))) ==
      LUMPERROR) {
...
#ifdef DEMO_COMPAT_100
  case 0x0001:
    I_Error("You need to implement demo compat here, doofus! %s:%d", __FILE__,
            __LINE__);
#endif
  // too old, cannot support.
  default:
    CONS_Alert(CONS_WARNING,
               M_GetText("Failed to load bot recording for this map, format "
  ...
  if (memcmp(metal_p, "METL", 4)) {
    CONS_Alert(CONS_WARNING,
               M_GetText("Failed to load bot recording for this map, wasn't "
                         "recorded in Metal format.\n"));
    Z_Free(metalbuffer);
    return;
  }
  metal_p += 4; // "METL"
...
// Stops metal sonic's demo. Separate from other functions because metal +
// replays can coexist
...
ATTRNORETURN void FUNCNORETURN G_StopMetalRecording(void) {
  ...
  if (demo_p) {
    ...
    WRITEUINT8(demo_p, DEMOMARKER); // add the demo end marker
    for (i = 0; i < 16; i++, p++)
      *p = P_RandomByte(); // This MD5 was chosen by fair dice roll and most
                           // likely < 50% correct.
#else
    WRITEUINT8(demo_p, DEMOMARKER); // add the demo end marker
    md5_buffer(
        (char *)p + 16, demo_p - (p + 16),
        (void *)
            p); // make a checksum of everything after the checksum in the file.
#endif
    saved = FIL_WriteFile(va("%sMS.LMP", G_BuildMapName(gamemap)), demobuffer,
                          demo_p - demobuffer); // finally output the file.
  }
  free(demobuffer);
  demobuffer = NULL;
...
#ifdef NOMD5
  for (i = 0; i < 16; i++, p++)
    *p = M_RandomByte(); // This MD5 was chosen by fair dice roll and most
                         // likely < 50% correct.
#else
  // Make a checksum of everything after the checksum in the file up to the end
  // of the standard data. Extrainfo is freely modifiable.
  md5_buffer((char *)p + 16, (demobuffer + length) - (p + 16), p);
#endif

  if (FIL_WriteFile(va(pandf, srb2home, demoname), demobuffer,
                    demo_p - demobuffer)) // finally output the file.
...
boolean G_DemoTitleResponder(event_t *ev) {
  ...
  if (ch == KEY_ESCAPE) {
    demo.savemode = (cv_recordmultiplayerdemos.value == 2) ? DSM_WILLAUTOSAVE
                                                           : DSM_NOTSAVING;
    return true;
  }
...
INT32
G_TicsToHours(tic_t tics) { return tics / (3600 * TICRATE); }

INT32
G_TicsToMinutes(tic_t tics, boolean full) {
  if (full)
...
INT32
G_TicsToSeconds(tic_t tics) { return (tics / TICRATE) % 60; }

INT32
G_TicsToCentiseconds(tic_t tics) {
  return (INT32)((tics % TICRATE) * (100.00f / TICRATE));
}

INT32
G_TicsToMilliseconds(tic_t tics) {
