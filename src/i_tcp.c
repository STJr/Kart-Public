...
///        This is not really OS-dependent because all OSes have the same socket
///        API. Just use ifdef for OS-dependent parts.
...
#define errno                                                                  \
  h_errno // some very strange things happen when not using h_error?!?
#ifdef EWOULDBLOCK
#undef EWOULDBLOCK
#endif
...
#else
#if (defined(__unix__) && !defined(MSDOS)) || defined(__APPLE__) ||            \
    defined(__HAIKU__) || defined(_PS3)
...
#if (defined(WATTCP) && !defined(__libsocket_socklen_t)) ||                    \
    defined(USE_WINSOCK1)
...
static char *get_WSAErrorStr(int e) {
  ...
  if (!buf[0]) // provide a fallback error message if no message is available
               // for some reason
...
#define HAVE_NTOP
static const char *inet_ntopA(short af, const void *cp, char *buf,
                              socklen_t len) {
...
static inline void I_UPnP_add(const char *addr, const char *port,
                              const char *servicetype) {
  if (addr == NULL)
    addr = lanaddr;
  if (!urls.controlURL || urls.controlURL[0] == '\0')
    return;
  UPNP_AddPortMapping(urls.controlURL, data.first.servicetype, port, port, addr,
                      "SRB2", servicetype, NULL, NULL);
}
...
static boolean SOCK_cmpaddr(mysockaddr_t *a, mysockaddr_t *b, UINT8 mask) {
  ...
  if (b->any.sa_family == AF_INET)
    return (a->ip4.sin_addr.s_addr & bitmask) ==
               (b->ip4.sin_addr.s_addr & bitmask) &&
           (b->ip4.sin_port == 0 || (a->ip4.sin_port == b->ip4.sin_port));
#ifdef HAVE_IPV6
  else if (b->any.sa_family == AF_INET6)
    return memcmp(&a->ip6.sin6_addr, &b->ip6.sin6_addr,
                  sizeof(b->ip6.sin6_addr)) &&
...
static SINT8 getfreenode(void) {
  ...
  /** \warning No free node? Just in case a node might not have been freed
   * properly, look if there are connected nodes that aren't in game, and forget
   * them. It's dirty, and might result in a poor guy having to restart
   *          downloading a needed wad, but it's better than not letting anyone
   * join...
   */
...
static boolean hole_punch(ssize_t c) {
  /* See ../doc/Holepunch-Protocol.txt */
  if (cv_rendezvousserver.string[0] && c == 10 &&
    ...
    CONS_Debug(DBG_NETPLAY, "hole punching request from %s\n",
               SOCK_AddrToStr(&addr));
...
// Returns true if a packet was received from a new node, false in all other
// cases
...
static boolean FD_CPY(fd_set *src, fd_set *dst, SOCKET_TYPE *fd, size_t len) {
  ...
  FD_ZERO(dst);
  for (i = 0; i < len; i++) {
    if (fd[i] != (SOCKET_TYPE)ERRSOCKET && FD_ISSET(fd[i], src) &&
        !FD_ISSET(fd[i], dst)) // no checking for dups
    {
      FD_SET(fd[i], dst);
      testset = true;
    }
...
#ifndef NONET
static inline ssize_t SOCK_SendToAddr(SOCKET_TYPE socket,
  ...
  return sendto(socket, (char *)&doomcom->data, doomcom->datalength, 0,
                &sockaddr->any, d);
}
...
static void SOCK_Send(void) {
  ...
  if (doomcom->remotenode == BROADCASTADDR) {
    ...
    c = SOCK_SendToAddr(nodesocket[doomcom->remotenode],
                        &clientaddress[doomcom->remotenode]);
  }

  if (c == ERRSOCKET) {
    int e = errno; // save error code so it can't be modified later
    if (e != ECONNREFUSED && e != EWOULDBLOCK)
      I_Error("SOCK_Send, error sending to node %d (%s) #%u: %s",
              doomcom->remotenode, SOCK_GetNodeAddress(doomcom->remotenode), e,
              strerror(e));
  }
}
#endif
...
// allocate a socket
static SOCKET_TYPE UDP_Bind(int family, struct sockaddr *addr,
                            socklen_t addrlen) {
  ...
  { // Alam_GBC: disable the new UDP connection reset behavior for Win2k and up
    ...
    BOOL bfalse = FALSE;
    WSAIoctl(s, SIO_UDP_CONNRESET, &bfalse, sizeof(bfalse), NULL, 0,
  ...
  if (family == AF_INET) {
    ...
    if (setsockopt(s, SOL_SOCKET, SO_BROADCAST, (char *)&opt, opts)) {
      CONS_Alert(
          CONS_WARNING,
          M_GetText(
              "Could not get broadcast rights\n")); // I do not care anymore
    }
  }
#ifdef HAVE_IPV6
  else if (family == AF_INET6) {
    if (memcmp(addr, &in6addr_any, sizeof(in6addr_any)) ==
        0) // IN6_ARE_ADDR_EQUAL
    {
      opt = true;
      opts = (socklen_t)sizeof(opt);
      setsockopt(s, SOL_SOCKET, SO_REUSEADDR, (char *)&opt, opts);
    ...
    if (setsockopt(s, SOL_SOCKET, IPV6_V6ONLY, (char *)&opt, opts)) {
      CONS_Alert(
          CONS_WARNING,
          M_GetText("Could not limit IPv6 bind\n")); // I do not care anymore
    }
#endif
  }
#endif
  ...
  if (opt < 64 << 10) // 64k
  {
    ...
    if (opt < 64 << 10)
      CONS_Alert(
          CONS_WARNING,
          M_GetText(
              "Can't set buffer length to 64k, file transfer will be bad\n"));
    else
...
static boolean UDP_Socket(void) {
    ...
    while (M_IsNextParm()) {
      ...
      if (gaie == 0) {
        ...
        while (runp != NULL && s < MAXNETNODES + 1) {
          mysockets[s] = UDP_Bind(runp->ai_family, runp->ai_addr,
                                  (socklen_t)runp->ai_addrlen);
  ...
  if (gaie == 0) {
    ...
    clientaddress[s].ip4.sin_addr.s_addr =
        htonl(INADDR_LOOPBACK); // GetLocalAddress(); // my own ip
    s++;
  }
...
boolean I_InitTcpDriver(void) {
  ...
  if (!init_tcp_driver) {
    ...
    if (WSAresult != 0) {
      ...
      switch (WSAresult) {
      case WSASYSNOTREADY:
        WSError = "The underlying network subsystem is not ready for network "
                  "communication";
        break;
      case WSAEINPROGRESS:
        ...
        break;
      case WSAEPROCLIM:
        WSError = "Limit on the number of tasks supported by the Windows "
                  "Sockets implementation has been reached";
        break;
      case WSAEFAULT:
        WSError =
            "WSAData is not a valid pointer? What kind of setup do you have?";
        break;
      default:
...
#endif
#ifdef __DJGPP__
#ifdef WATTCP // Alam_GBC: survive bootp, dhcp, rarp and wattcp/pktdrv from
              // failing to load
...
#ifndef NONET
static boolean SOCK_GetAddr(struct sockaddr_in *sin, const char *address,
                            const char *port, boolean test) {
  ...
  if (test) {
    while (runp != NULL) {
      if (sendto(mysockets[0], NULL, 0, 0, runp->ai_addr, runp->ai_addrlen) ==
          0)
        break;
...
static void SOCK_RegisterHolePunch(void) { rendezvous(4); }
#endif
...
static void AddBannedIndex(void) {
  if (numbans >= banned_size) {
    ...
    banned = Z_ReallocAlign((void *)banned, sizeof(banned_t) * banned_size,
                            PU_STATIC, NULL, sizeof(banned_t) * 8);
  }
...
boolean I_InitTcpNetwork(void) {
  ...
  // As it was really redundant having two seperate parms that does the same
  // thing
  ...
  if (M_CheckParm("-server") || dedicated) {
    ...
    // If a number of clients (i.e. nodes) is specified, the server will wait
    // for the clients to connect before starting. If no number is specified
    // here, the server starts with 1 client, and others can join in-game. Since
    // Boris has implemented join in-game, there is no actual need for
    // specifying a particular number here.
    ...
    if (M_IsNextParm())
      ...
      serverhostname[0] =
          0; // assuming server in the LAN, use broadcast to detect it
