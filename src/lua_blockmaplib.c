...
static const char *const search_opt[] = {"objects", "lines", NULL};
...
static boolean blockfuncerror =
    false; // errors should only print once per search blockmap call

// Helper function for "objects" search
static UINT8 lib_searchBlockmap_Objects(lua_State *L, INT32 x, INT32 y,
                                        mobj_t *thing) {
  ...
  for (mobj = blocklinks[y * bmapwidth + x]; mobj; mobj = bnext) {
    ...
    if (!lua_isnil(gL, -1)) { // if nil, continue
...
static UINT8 lib_searchBlockmap_Lines(lua_State *L, INT32 x, INT32 y,
                                      mobj_t *thing) {
  INT32 offset;
...
//   false = searching of at least one block stopped mid-way (including if the
//   whole search was stopped)
static int lib_searchBlockmap(lua_State *L) {
  ...
  lua_remove(
      L, 1); // remove searchtype, stack is now function, mobj, [x1, x2, y1, y2]
  ...
  if (n > 2) // specific x/y ranges have been supplied
  {
    if (n < 6)
      return luaL_error(
          L,
          "arguments 4 to 6 not all given (expected 4 fixed-point integers)");
  ...
  for (bx = xl; bx <= xh; bx++)
    for (by = yl; by <= yh; by++) {
      ...
      // return value of searchFunc determines searchFunc's return value and/or
      // when to stop
      if (funcret == 2) {          // stop whole search
        ...
        retval = false; // this changes the return value, but doesn't stop the
                        // whole search
      // else don't do anything, continue as normal
      if (P_MobjWasRemoved(mobj)) { // ...unless the original object was removed
        lua_pushboolean(L,
                        false); // in which case we have to stop now regardless
        return 1;
      }
    }
