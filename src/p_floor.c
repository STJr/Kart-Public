...
//
static inline boolean P_MobjReadyToMove(mobj_t *mo, sector_t *sec,
                                        boolean sectorisffloor,
                                        boolean sectorisquicksand) {
  if (sectorisquicksand)
    ...
    return ((sectorisffloor) ? (mo->z + mo->height != sec->floorheight)
                             : (mo->z + mo->height != sec->ceilingheight));
  else
    return ((sectorisffloor) ? (mo->z != sec->ceilingheight)
...
result_e T_MovePlane(sector_t *sector, fixed_t speed, fixed_t dest,
                     boolean crush, INT32 floorOrCeiling, INT32 direction) {
  boolean flag;
  fixed_t lastpos;
  fixed_t
      destheight; // used to keep floors/ceilings from moving through each other
  ...
  // Optimization condition. If the sector is not an FOF, declare sec as the
  // main sector outside of the loop.
  if (!sectorisffloor)
    sec = sector;

  // Optimization condition. Only run the logic if there is any Things in the
  // sector.
  if (sectorisffloor || sec->thinglist) {
    // If this is an FOF being checked, check all the affected sectors for
    // moving mobjs.
    while ((sectorisffloor
                ? (secnum = P_FindSectorFromLineTag(sector->tagline, secnum))
                : (secnum = 1)) >= 0) {
      if (sectorisffloor) {
...
void T_MoveFloor(floormove_t *movefloor) {
  ...
  res = T_MovePlane(movefloor->sector, movefloor->speed,
                    movefloor->floordestheight, movefloor->crush, 0,
                    movefloor->direction);

  if (movefloor->type == bounceFloor) {
    const fixed_t origspeed =
        FixedDiv(movefloor->origspeed, (ELEVATORSPEED / 2));
    const fixed_t fs = abs(movefloor->sector->floorheight -
                           lines[movefloor->texture].frontsector->floorheight);
    const fixed_t bs = abs(movefloor->sector->floorheight -
                           lines[movefloor->texture].backsector->floorheight);
    if (fs < bs)
    ...
    if (movefloor->direction == 1) {
      ...
      case moveFloorByFrontSector:
        if (movefloor->texture < -1) // chained linedef executing
          P_LinedefExecute((INT16)(movefloor->texture + INT16_MAX + 2), NULL,
                           NULL);
        /* FALLTHRU */
      ...
      case bounceFloor: // Graue 03-12-2004
        if (movefloor->floordestheight ==
          ...
          movefloor->floordestheight =
              lines[movefloor->texture].frontsector->floorheight;
        movefloor->direction =
            (movefloor->floordestheight < movefloor->sector->floorheight) ? -1
                                                                          : 1;
      ...
      case bounceFloorCrush: // Graue 03-27-2004
        if (movefloor->floordestheight ==
          ...
          movefloor->speed = movefloor->origspeed =
              FixedDiv(abs(lines[movefloor->texture].dy),
                       4 * FRACUNIT); // return trip, use dy
        } else {
          movefloor->floordestheight =
              lines[movefloor->texture].frontsector->floorheight;
          movefloor->speed = movefloor->origspeed =
              FixedDiv(abs(lines[movefloor->texture].dx),
                       4 * FRACUNIT); // forward again, use dx
        }
        movefloor->direction =
            (movefloor->floordestheight < movefloor->sector->floorheight) ? -1
                                                                          : 1;
      ...
      case bounceFloorCrush: // Graue 03-27-2004
        ...
        return; // not break, why did this work? Graue 04-03-2004
      case crushFloorOnce:
        movefloor->sector->floordata =
            NULL; // Clear up the thinker so others can use it
...
void T_MoveElevator(elevator_t *elevator) {
  ...
  if (elevator->direction < 0) // moving down
  {
    if (elevator->type == elevateContinuous) {
      const fixed_t origspeed =
          FixedDiv(elevator->origspeed, (ELEVATORSPEED / 2));
      const fixed_t wh =
          abs(elevator->sector->floorheight - elevator->floorwasheight);
      const fixed_t dh =
          abs(elevator->sector->floorheight - elevator->floordestheight);
    ...
    res2 = T_MovePlane(elevator->sector, elevator->speed,
                       elevator->floordestheight, elevator->distance,
                       0, // move ceiling
                       elevator->direction);
  ...
  } else // moving up
  {
    if (elevator->type == elevateContinuous) {
      const fixed_t origspeed =
          FixedDiv(elevator->origspeed, (ELEVATORSPEED / 2));
      const fixed_t wc =
          abs(elevator->sector->ceilingheight - elevator->ceilingwasheight);
      const fixed_t dc =
          abs(elevator->sector->ceilingheight - elevator->ceilingdestheight);
      // Slow down when reaching destination Tails 12-06-2000
      if (wc < dc)
    ...
    res1 = T_MovePlane // jff 4/7/98 reverse order of ceiling/floor
        (elevator->sector, elevator->speed, elevator->floordestheight,
         elevator->distance,
         0, // move ceiling
         elevator->direction);
    ...
    if (res1 != crushed) {
      res2 = T_MovePlane(elevator->sector, elevator->speed,
                         elevator->ceilingdestheight, elevator->distance,
                         1, // move floor
                         elevator->direction);
    }
  ...
  if (res == pastdest || res == crushed) // if destination height acheived
  {
    if (elevator->type == elevateContinuous) {
      if (elevator->direction > 0) {
        ...
        if (elevator->low) {
          elevator->floordestheight = P_FindNextHighestFloor(
              elevator->sector, elevator->sector->floorheight);
          elevator->ceilingdestheight = elevator->floordestheight +
                                        elevator->sector->ceilingheight -
                                        elevator->sector->floorheight;
        } else {
          elevator->floordestheight = P_FindNextLowestFloor(
              elevator->sector, elevator->sector->floorheight);
          elevator->ceilingdestheight = elevator->floordestheight +
                                        elevator->sector->ceilingheight -
                                        elevator->sector->floorheight;
...
#define ceilingdestheight vars[5]
  ...
  {
    if (faller->sector->ceilingheight <=
        faller->ceilingdestheight) // if destination height acheived
    {
  ...
  {
    if (faller->sector->floorheight >=
        faller->floordestheight) // if destination height acheived
    {
      faller->sector->ceilingheight = faller->ceilingwasheight;
      faller->sector->floorheight = faller->floorwasheight;
      R_ClearLevelInterpolatorState(&faller->thinker);
...
static fixed_t P_SectorCheckWater(sector_t *analyzesector,
  ...
  if (analyzesector->ffloors) {
    ...
    for (rover = analyzesector->ffloors; rover; rover = rover->next) {
      ...
      if ((elevatorsec->ceilingheight + elevatorsec->floorheight) >> 1 <
          *rover->bottomheight)
        continue;
      ...
      if (*rover->topheight <
          analyzesector->floorheight +
              abs((elevatorsec->ceilingheight - elevatorsec->floorheight) >> 1))
        continue;

      if (*rover->topheight >
          watertop) // highest water block is the one to go for
...
#define floorwasheight vars[4]
  ...
  if (bouncer->sector->crumblestate == 4 ||
      bouncer->sector->crumblestate == 1 ||
      bouncer->sector->crumblestate ==
          2) // Oops! Crumbler says to remove yourself!
  {
    bouncer->sector->crumblestate = 1;
  ...
  for (i = -1; (i = P_FindSectorFromTag(bouncer->sourceline->tag, i)) >= 0;) {
    ...
    halfheight =
        abs(bouncer->sector->ceilingheight - bouncer->sector->floorheight) >> 1;

    waterheight = P_SectorCheckWater(
        actionsector, bouncer->sector); // sorts itself out if there's no
                                        // suitable water in the sector

    floorheight =
        P_FloorzAtPos(actionsector->soundorg.x, actionsector->soundorg.y,
                      bouncer->sector->floorheight, halfheight << 1);

    // Water level is up to the ceiling.
    if (waterheight > bouncer->sector->ceilingheight - halfheight &&
        bouncer->sector->ceilingheight >=
            actionsector->ceilingheight) // Tails 01-08-2004
    {
      bouncer->sector->ceilingheight = actionsector->ceilingheight;
      bouncer->sector->floorheight =
          bouncer->sector->ceilingheight - (halfheight * 2);
      T_MovePlane(bouncer->sector, 0, bouncer->sector->ceilingheight, 0, 1,
                  -1); // update things on ceiling
      T_MovePlane(bouncer->sector, 0, bouncer->sector->floorheight, 0, 0,
                  -1); // update things on floor
    ...
    T_MovePlane(bouncer->sector, bouncer->speed / 2,
                bouncer->sector->ceilingheight - 70 * FRACUNIT, 0, 1,
                -1); // move ceiling
    T_MovePlane(bouncer->sector, bouncer->speed / 2,
                bouncer->sector->floorheight - 70 * FRACUNIT, 0, 0,
                -1); // move floor
...
void T_StartCrumble(elevator_t *elevator) {
  ...
  if (elevator->distance != 0) {
    if (elevator->distance > 0) // Count down the timer
    {
      ...
      if (elevator->distance <= 0)
        elevator->distance =
            -15 * TICRATE; // Timer until platform returns to original position.
      else {
    ...
    {
      for (i = -1;
           (i = P_FindSectorFromTag(elevator->sourceline->tag, i)) >= 0;) {
        sector = &sectors[i];
    ...
    if (elevator->distance > -224 &&
        (leveltime % ((abs(elevator->distance) / 8) + 1) == 0)) {
      for (i = -1;
           (i = P_FindSectorFromTag(elevator->sourceline->tag, i)) >= 0;) {
        sector = &sectors[i];

        for (rover = sector->ffloors; rover; rover = rover->next) {
          if (!(rover->flags & FF_NORETURN) && rover->flags & FF_CRUMBLE &&
      ...
      (elevator->floordestheight == 1 && elevator->direction == 1)) // Down
  {
    ...
    if (elevator->high != 42) {
      ...
      if ((elevator->floordestheight == 0 &&
           !(elevator->sector->ceilingheight < -16384 * FRACUNIT)) ||
        ...
        T_MovePlane // jff 4/7/98 reverse order of ceiling/floor
            (elevator->sector, elevator->speed, dest, 0,
             1, // move floor
             elevator->direction);
        ...
        T_MovePlane(elevator->sector, elevator->speed, dest, 0,
                    0, // move ceiling
                    elevator->direction);
...
#define low vars[6]
  ...
  T_MovePlane(block->sector, block->speed,
  ...
  T_MovePlane(block->sector, block->speed,
              block->sector->floorheight + 70 * FRACUNIT * block->direction, 0,
              0, // move ceiling
              block->direction);

  if (block->sector->ceilingheight >=
      block->ceilingwasheight + 32 * FRACUNIT) // Go back down now..
...
void T_SpikeSector(levelspecthink_t *spikes) {
  ...
  for (; node; node = node->m_thinglist_next) {
    ...
    if (affectsec == spikes->sector) // Applied to an actual sector
    {
      ...
      fixed_t affectfloor =
          P_GetSpecialBottomZ(thing, affectsec, spikes->sector);
...
void T_FloatSector(levelspecthink_t *floater) {
  ...
  cheeseheight =
      (floater->sector->ceilingheight + floater->sector->floorheight) >> 1;

  // Just find the first sector with the tag.
  // Doesn't work with multiple sectors that have different floor/ceiling
  // heights.
  ...
  if (actionsector) {
    ...
    fixed_t waterheight = P_SectorCheckWater(
        actionsector,
        floater->sector); // find the highest suitable water block around

    if (waterheight == cheeseheight) // same height, no floating needed
      ;
    else if (floater->sector->floorheight == actionsector->floorheight &&
             waterheight < cheeseheight) // too low
      ;
    else if (floater->sector->ceilingheight == actionsector->ceilingheight &&
             waterheight > cheeseheight) // too high
      ;
    // we have something to float in! Or we're for some reason above the ground,
    // let's fall anyway
    else if (floater->sector->crumblestate == 0 ||
             floater->sector->crumblestate >= 3 /* || floatanyway*/)
...
#define ENDCONTROLTAG (ENDTAG + (ENDTAG - STARTTAG) + 1)
  ...
  for (j = STARTTAG; j <= ENDTAG; j++) {
    for (i = -1; (i = P_FindSectorFromTag(j, i)) >= 0;) {
      ...
      for (node = sector->touching_thinglist; node;
...
wegotit:
  ...
  if (playeronme && controlsec) {
...

    {
      ...
      INT32 midpoint =
          STARTCONTROLTAG + ((ENDCONTROLTAG - STARTCONTROLTAG) + 1) / 2;
      //			INT32 tagstart = STARTTAG - midpoint;
      //			INT32 tagend = ENDTAG - midpoint;

      //			CONS_Debug(DBG_GAMELOGIC, "tagstart is %d,
      // tagend is %d\n", tagstart, tagend);

      // Sag is adjusted by how close you are to the center
      dist = ((ENDCONTROLTAG - STARTCONTROLTAG)) / 2 -
             abs(bridge->sector->tag - midpoint);
    ...
    if (bridge->sector->ceilingheight <= LOWCEILINGHEIGHT) {
      bridge->sector->floorheight =
          LOWCEILINGHEIGHT -
          (bridge->sector->ceilingheight - bridge->sector->floorheight);
    ...
    if ((bridge->sector->ceilingheight - LOWCEILINGHEIGHT) <
      ...
      CURSPEED = FixedMul(
          CURSPEED, FixedDiv(bridge->sector->ceilingheight - LOWCEILINGHEIGHT,
                             (ORIGCEILINGHEIGHT - LOWCEILINGHEIGHT) >> 5));
      ...
      CURSPEED = FixedMul(
          CURSPEED, FixedDiv(ORIGCEILINGHEIGHT - bridge->sector->ceilingheight,
                             (ORIGCEILINGHEIGHT - LOWCEILINGHEIGHT) >> 5));
    ...
    res = T_MovePlane(bridge->sector,     // sector
                      CURSPEED,           // speed
    ...
    if (res == ok || res == pastdest)
      T_MovePlane(bridge->sector,   // sector
                  CURSPEED,         // speed
  ...
  dorest:
    // Adjust joined sector heights
    {
      ...
      if (divisor > 0) {
        ...
        //				CONS_Debug(DBG_GAMELOGIC, "interval is
        //%d\n", interval>>FRACBITS);
          ...
          for (i = -1; (i = P_FindSectorFromTag(j, i)) >= 0;) {
            if (sectors[i].ceilingheight >= sourcesec->ceilingheight) {
              sectors[i].ceilingheight =
                  ORIGCEILINGHEIGHT - (interval * plusplusme);
              sectors[i].floorheight =
                  ORIGFLOORHEIGHT - (interval * plusplusme);
            } else // Do the regular rise
            {
              ...
              //"ceildest: %d, floordest: %d\n", ceilingdestination>>FRACBITS,
              // floordestination>>FRACBITS);
              ...
              if ((bridge->sector->ceilingheight - LOWCEILINGHEIGHT) <
                ...
                CURSPEED = FixedMul(
                    CURSPEED,
                    FixedDiv(bridge->sector->ceilingheight - LOWCEILINGHEIGHT,
                             (ORIGCEILINGHEIGHT - LOWCEILINGHEIGHT) >> 5));
                ...
                CURSPEED = FixedMul(
                    CURSPEED,
                    FixedDiv(ORIGCEILINGHEIGHT - bridge->sector->ceilingheight,
                             (ORIGCEILINGHEIGHT - LOWCEILINGHEIGHT) >> 5));
      ...
      if (divisor > 0) {
        ...
        //				CONS_Debug(DBG_GAMELOGIC, "interval2 is
        //%d\n", interval>>FRACBITS);

        for (j = (INT16)(sourcesec->tag + 1);
             j <= ENDTAG + (ENDTAG - STARTTAG) + 1; j++, plusplusme++) {
          for (i = -1; (i = P_FindSectorFromTag(j, i)) >= 0;) {
            if (sectors[i].ceilingheight >= sourcesec->ceilingheight) {
              sectors[i].ceilingheight =
                  sourcesec->ceilingheight + (interval * plusplusme);
              sectors[i].floorheight =
                  sourcesec->floorheight + (interval * plusplusme);
            } else // Do the regular rise
            {
              bridge->sector = &sectors[i];
    ...
    for (j = (INT16)(ENDTAG + 1); j <= (ENDTAG + (ENDTAG - STARTTAG) + 1);
         j++) {
      for (i = -1; (i = P_FindSectorFromTag(j, i)) >= 0;) {
        bridge->sector = &sectors[i];
        ...
        //				CONS_Debug(DBG_GAMELOGIC, "ceildest: %d,
        // floordest: %d\n", ceilingdestination>>FRACBITS,
        // floordestination>>FRACBITS);
...
static mobj_t *SearchMarioNode(msecnode_t *node) {
  ...
  for (; node; node = node->m_thinglist_next) {
    ...
    if (!thing                          // take either the first thing
        || node->m_thing->x < thing->x  // or the thing with the lowest x value
                                        // (left to right item order)
        || node->m_thing->y < thing->y) // or the thing with the lowest y value
                                        // (top to bottom item order)
      thing = node->m_thing;
  }
  return thing;
}
...
void T_MarioBlockChecker(levelspecthink_t *block) {
  ...
  if (SearchMarioNode(block->sector->touching_thinglist))
    sides[masterline->sidenum[0]].midtexture =
        sides[masterline->sidenum[0]].bottomtexture;
  else
    sides[masterline->sidenum[0]].midtexture =
        sides[masterline->sidenum[0]].toptexture;
}
...
#define ceilingwasheight vars[5]
  ...
  if (thwomp->sourceline->flags & ML_EFFECT1 &&
      leveltime <
          (unsigned)(sides[thwomp->sourceline->sidenum[0]].textureoffset >>
                     FRACBITS))
    thwomp->direction = 0;

  // If you just crashed down, wait a second before coming back up.
  if (--thwomp->distance > 0) {
    sides[thwomp->sourceline->sidenum[0]].midtexture =
        sides[thwomp->sourceline->sidenum[0]].toptexture;
    // sides[thwomp->sourceline->sidenum[0]].midtexture =
    // sides[thwomp->sourceline->sidenum[0]].bottomtexture;
    return;
  }
  ...
  if (thwomp->direction > 0) // Moving back up..
  {
    ...
    sides[thwomp->sourceline->sidenum[0]].midtexture =
        sides[thwomp->sourceline->sidenum[0]].bottomtexture;
    /// \note this should only have to be done once, but is already done
    /// repeatedly, above
    ...
    res = T_MovePlane(thwomp->sector,         // sector
                      thwomp->speed,          // speed
    ...
    if (res == ok || res == pastdest)
      T_MovePlane(thwomp->sector,           // sector
                  thwomp->speed,            // speed
        ...
        T_MovePlane(thwomp->sector, // sector
                    ...
                    P_FloorzAtPos(thwompx, thwompy, thwomp->sector->floorheight,
                                  thwomp->sector->ceilingheight -
                                      thwomp->sector->floorheight), // dest
                    0,                                              // crush
                    0,                // floor or ceiling (0 for floor)
                    thwomp->direction // direction
        );
    ...
    if (res == pastdest) {
      ...
      if (!rover || (rover->flags & FF_EXISTS)) {
        if (thwomp->sourceline->flags & ML_EFFECT4)
          S_StartSound(mp,
                       sides[thwomp->sourceline->sidenum[0]].textureoffset >>
                           FRACBITS);
        else
  ...
  } else // Not going anywhere, so look for players.
  {
    ...
    if (!rover || (rover->flags & FF_EXISTS))
    {
            ...
            for (th = thinkercap.next; th != &thinkercap; th = th->next)
            {
                    ...
                    mo = (mobj_t *)th;
                    if (mo->type == MT_PLAYER && mo->health && mo->player &&
    !mo->player->spectator
                        && mo->z <= thwomp->sector->ceilingheight
                            && P_AproxDistance(thwompx - mo->x, thwompy - mo->y)
    <= 96*FRACUNIT)
                    {
                            thwomp->direction = -1;
                            break;
...
// T_NoEnemiesThinker
//
// Runs a linedef exec when no more MF_ENEMY/MF_BOSS objects with health are in
// the area \sa P_AddNoEnemiesThinker
//
void T_NoEnemiesSector(levelspecthink_t *nobaddies) {
  ...
  for (i = 0; i < nobaddies->sector->linecount; i++) {
    if (nobaddies->sector->lines[i]->special == 223) {
      ...
      for (s = -1; (s = P_FindSectorFromLineTag(nobaddies->sector->lines[i],
                                                s)) >= 0;) {
...
foundenemy:
  ...
  s = P_AproxDistance(nobaddies->sourceline->dx, nobaddies->sourceline->dy) >>
      FRACBITS;
...
//
static INT32 P_HavePlayersEnteredArea(boolean *curPlayers, boolean *oldPlayers,
                                      boolean inAndOut) {
  INT32 i;
...
void T_EachTimeThinker(levelspecthink_t *eachtime) {
  ...
  while ((secnum = P_FindSectorFromLineTag(eachtime->sourceline, secnum)) >=
         0) {
    sec = &sectors[secnum];

    FOFsector = false;

    if (GETSECSPECIAL(sec->special, 2) == 3 ||
        GETSECSPECIAL(sec->special, 2) == 5)
      floortouch = true;
    else if (GETSECSPECIAL(sec->special, 2) >= 1 &&
             GETSECSPECIAL(sec->special, 2) <= 8)
      floortouch = false;
    else
      continue;
    ...
    for (i = 0; i < sec->linecount; i++) {
      ...
      while ((targetsecnum =
                  P_FindSectorFromLineTag(sec->lines[i], targetsecnum)) >= 0) {
        ...
        for (j = 0; j < MAXPLAYERS; j++) {
          ...
          if (players[j].mo->subsector->sector == targetsec)
            ...
            for (node = players[j].mo->touching_sectorlist; node;
          ...
          if (floortouch == true &&
              P_IsObjectOnGroundIn(players[j].mo, targetsec)) {
            if (j & 1)
      ...
      for (i = 0; i < MAXPLAYERS; i++) {
        ...
        if (!(players[i].mo->subsector->sector == sec ||
              P_PlayerTouchingSectorSpecial(
                  &players[i], 2, (GETSECSPECIAL(sec->special, 2))) == sec))
          continue;
  ...
  while ((affectPlayer = P_HavePlayersEnteredArea(playersArea, oldPlayersArea,
                                                  inAndOut)) != -1) {
    if (GETSECSPECIAL(sec->special, 2) == 2 ||
        GETSECSPECIAL(sec->special, 2) == 3) {
      for (i = 0; i < MAXPLAYERS; i++) {
        if (!playeringame[i])
          continue;
    ...
    CONS_Debug(DBG_GAMELOGIC,
               "Trying to activate each time executor with tag %d\n",
               eachtime->sourceline->tag);

    // 03/08/14 -Monster Iestyn
    // No more stupid hacks involving changing eachtime->sourceline's tag or
    // special or whatever! This should now run ONLY the stuff for
    // eachtime->sourceline itself, instead of all trigger linedefs sharing the
    // same tag. Makes much more sense doing it this way, honestly.
    P_RunTriggerLinedef(eachtime->sourceline, players[affectPlayer].mo, sec);

    if (!eachtime->sourceline
             ->special) // this happens only for "Trigger on X calls" linedefs
...
void T_RaiseSector(levelspecthink_t *raise) {
  ...
  if ((raise->sector->ceilingheight - raise->vars[7]) <
    ...
    // Slow down as you get closer to the bottom
    raise->vars[3] = FixedMul(
        raise->vars[3], FixedDiv(raise->sector->ceilingheight - raise->vars[7],
                                 (raise->vars[5] - raise->vars[7]) >> 5));
    ...
    // Slow down as you get closer to the top
    raise->vars[3] = FixedMul(
        raise->vars[3], FixedDiv(raise->vars[5] - raise->sector->ceilingheight,
                                 (raise->vars[5] - raise->vars[7]) >> 5));
  ...
  res = T_MovePlane(raise->sector,      // sector
                    raise->vars[3],     // speed
                    ceilingdestination, // dest
                    0,                  // crush
                    1,                  // floor or ceiling (1 for ceiling)
  ...
  if (res == ok || res == pastdest)
    T_MovePlane(raise->sector,    // sector
                raise->vars[3],   // speed
                floordestination, // dest
                0,                // crush
                0,                // floor or ceiling (0 for floor)
...
void T_CameraScanner(elevator_t *elevator) {
  // leveltime is compared to make multiple scanners in one map function
  // correctly.
  ...
  if (players[displayplayers[0]].mo) {
    if (players[displayplayers[0]].mo->subsector->sector ==
  ...
  if (splitscreen && players[displayplayers[1]].mo) {
    if (players[displayplayers[1]].mo->subsector->sector ==
        elevator->actionsector) {
      if (t_cam2_rotate == -42)
      ...
      if (t_cam2_height != -42 && cv_cam2_height.value != t_cam2_height)
        CV_Set(&cv_cam2_height,
               va("%f", (double)FIXED_TO_FLOAT(t_cam2_height)));
  ...
  if (splitscreen > 1 && players[displayplayers[2]].mo) {
    if (players[displayplayers[2]].mo->subsector->sector ==
        elevator->actionsector) {
      if (t_cam3_rotate == -42)
      ...
      if (t_cam3_height != -42 && cv_cam3_height.value != t_cam3_height)
        CV_Set(&cv_cam3_height,
               va("%f", (double)FIXED_TO_FLOAT(t_cam3_height)));
  ...
  if (splitscreen > 2 && players[displayplayers[3]].mo) {
    if (players[displayplayers[3]].mo->subsector->sector ==
        elevator->actionsector) {
      if (t_cam4_rotate == -42)
      ...
      if (t_cam4_height != -42 && cv_cam4_height.value != t_cam4_height)
        CV_Set(&cv_cam4_height,
               va("%f", (double)FIXED_TO_FLOAT(t_cam4_height)));
  ...
  while ((secnum = P_FindSectorFromLineTag(line, secnum)) >= 0) {
    ...
    case moveFloorByFrontSector:
      ...
      if (line->flags & ML_BLOCKMONSTERS) {
        ...
        if (firstone && sides[line->sidenum[0]].textureoffset > 0)
          dofloor->texture =
              (sides[line->sidenum[0]].textureoffset >> FRACBITS) - 32769;
        else
          dofloor->texture = -1;
      }
    ...
    case moveFloorByFrontTexture:
      if (line->flags & ML_NOCLIMB)
        ...
        dofloor->speed = FixedDiv(sides[line->sidenum[0]].textureoffset,
                                  8 * FRACUNIT); // texture x offset
      dofloor->floordestheight =
          sec->floorheight +
          sides[line->sidenum[0]].rowoffset; // texture y offset
      if (dofloor->floordestheight > sec->floorheight)
        dofloor->direction = 1; // up
      else
        dofloor->direction = -1; // down
      ...
      /*
                              ...
                              case lowerFloorByLine:
                                      dofloor->direction = -1; // down
                                      dofloor->speed =
         FixedDiv(abs(line->dx),8*FRACUNIT); dofloor->floordestheight =
         sec->floorheight - abs(line->dy); if (dofloor->floordestheight >
         sec->floorheight) // wrapped around I_Error("Can't lower sector
         %d\n", secnum); break;
         ...
         FixedDiv(abs(line->dx),8*FRACUNIT); dofloor->floordestheight =
         sec->floorheight + abs(line->dy); if (dofloor->floordestheight <
         sec->floorheight) // wrapped around I_Error("Can't raise sector
         %d\n", secnum); break;
      */
    ...
    case bounceFloor:
      dofloor->speed = P_AproxDistance(
          line->dx, line->dy); // same speed as elevateContinuous
      dofloor->speed = FixedDiv(dofloor->speed, 4 * FRACUNIT);
      dofloor->origspeed =
          dofloor->speed; // it gets slowed down at the top and bottom
      dofloor->floordestheight = line->frontsector->floorheight;
      ...
      dofloor->delaytimer =
          sides[line->sidenum[0]].rowoffset >> FRACBITS; // Initial delay

      dofloor->texture =
          (fixed_t)(line - lines); // hack: store source line number
      break;
...
//
INT32
EV_DoElevator(line_t *line, elevator_e elevtype, boolean customspeed) {
  INT32 secnum = -1;
  INT32 rtn = 0;
  sector_t *sec;
  elevator_t *elevator;
  ...
  while ((secnum = P_FindSectorFromLineTag(line, secnum)) >= 0) {
    ...
    case elevateContinuous:
      ...
      elevator->sector = sec;
      elevator->low =
          !(line->flags & ML_NOCLIMB); // go down first unless noclimb is on
      if (elevator->low) {
        elevator->direction = 1;
...
void EV_CrumbleChain(sector_t *sec, ffloor_t *rover) {
  ...
  if (GETSECSPECIAL(rover->master->frontsector->special, 3) >= 8)
    type = MT_ROCKCRUMBLE1 +
           (GETSECSPECIAL(rover->master->frontsector->special, 3) - 7);
...
INT32
EV_BounceSector(sector_t *sec, fixed_t momz, line_t *sourceline) {
#define speed vars[0]
#define distance vars[1]
#define low vars[2]
...
INT32
EV_DoContinuousFall(sector_t *sec, sector_t *backsector, fixed_t spd,
                    boolean backwards) {
#define speed vars[0]
#define direction vars[1]
#define floorwasheight vars[2]
...
// Some other 3dfloor special things Tails 03-11-2002 (Search p_mobj.c for
// description)
INT32
EV_StartCrumble(sector_t *sec, ffloor_t *rover, boolean floating,
  ...
  for (i = -1; (i = P_FindSectorFromTag(elevator->sourceline->tag, i)) >= 0;) {
    ...
    P_SpawnMobj(foundsec->soundorg.x, foundsec->soundorg.y,
                elevator->direction == 1 ? elevator->sector->floorheight
                                         : elevator->sector->ceilingheight,
                MT_CRUMBLEOBJ);
  }
...
EV_MarioBlock(sector_t *sec, sector_t *roversector, fixed_t topheight,
  ...
  if (thing) {
    ...
    if (thing->flags & MF_SHOOTABLE)
      ...
      if (thing->type == MT_EMMY && thing->spawnpoint &&
          (thing->spawnpoint->options & MTF_OBJECTSPECIAL)) {
        mobj_t *tokenobj =
            P_SpawnMobj(roversector->soundorg.x, roversector->soundorg.y,
                        topheight, MT_TOKEN);
    ...
    S_StartSound(puncher,
                 sfx_mario1); // "Thunk!" sound - puncher is "close enough".
