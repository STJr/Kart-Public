...
consvar_t cv_gif_optimize = {"gif_optimize", "On", CV_SAVE, CV_OnOff, NULL, 0,
                             NULL,           NULL, 0,       0,        NULL};
consvar_t cv_gif_downscale = {
    "gif_downscale", "On", CV_SAVE, CV_OnOff, NULL, 0, NULL, NULL, 0, 0, NULL};
...
//
static void GIF_optimizeregion(const UINT8 *dst, const UINT8 *src, INT32 *x,
...
const UINT8 gifhead_nsid[19] = {
    0x21, 0xFF, 0x0B, // extension block + size
    0x4E, 0x45, 0x54, 0x53, 0x43, 0x41,
    0x50, 0x45, 0x32, 0x2E, 0x30,  // NETSCAPE2.0
...
// GIF FRAME (surprise!)
// ---
const UINT8 gifframe_gchead[4] = {0x21, 0xF9, 0x04,
                                  0x04}; // GCE, bytes, packed byte (no trans =
                                         // 0 | no input = 0 | don't remove = 4)
...
static void hwrconvert(void) {
  ...
  for (y = 0; y < vid.height; y++) {
    for (x = 0; x < vid.width; x++, i += 3) {
      ...
      b = (UINT8)linear[i + 2];
      dest[(y * vid.width) + x] =
          colorlookup[r >> SHIFTCOLORBITS][g >> SHIFTCOLORBITS]
                     [b >> SHIFTCOLORBITS];
    }
  }
...
static void GIF_framewrite(void) {
  ...
  if (gif_optimize && gif_frames > 0) {
    // before blit movie_screen points to last frame, cur_screen points to this
    // frame
    UINT8 *cur_screen = screens[0];
    GIF_optimizeregion(cur_screen, movie_screen, &blitx, &blity, &blitw,
                       &blith);
    ...
    if (gif_frames == 0) {
      if (rendermode == render_soft)
        ...
        hwrconvert();
        VID_BlitLinearScreen(screens[2], screens[0], vid.width * vid.bpp,
  ...
  // screen regions are handled in GIF_lzw
  {
    ...
    if (scrbuf_downscaleamt > 1) {
      ...
      blitw = ((blitw + (scrbuf_downscaleamt - 1)) / scrbuf_downscaleamt) *
              scrbuf_downscaleamt;
      blith = ((blith + (scrbuf_downscaleamt - 1)) / scrbuf_downscaleamt) *
              scrbuf_downscaleamt;
    }
    ...
    while (!gif_writeover) {
      ...
      if ((size_t)(p - gifframe_data) + gifbwr_bufsize + 1 >= gifframe_size) {
        ...
        gifframe_data =
            Z_Realloc(gifframe_data, (gifframe_size *= 2), PU_STATIC, NULL);
        p = gifframe_data +
            temppos; // realloc moves gifframe_data, so p is now invalid
      }
