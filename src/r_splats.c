...
void R_AddWallSplat(line_t *wallline, INT16 sectorside, const char *patchname,
                    fixed_t top, fixed_t wallfrac, INT32 flags) {
  ...
  splat->offset =
      FixedMul(wallfrac, linelength) - (SHORT(patch->width) << (FRACBITS - 1));
...
// --------------------------------------------------------------------------
void R_AddFloorSplat(subsector_t *subsec, mobj_t *mobj, const char *picname,
                     fixed_t x, fixed_t y, fixed_t z, INT32 flags) {
...
void R_ClearVisibleFloorSplats(void) { visfloorsplats = NULL; }
...
void R_AddVisibleFloorSplats(subsector_t *subsec) {
  ...
  // FIXME: depending on some flag in pSplat->flags, some splats may be visible
  // from 2 sides (above/below)
  if (pSplat->z < viewz) {
...
void ASMCALL rasterize_segment_tex(INT32 x1, INT32 y1, INT32 x2, INT32 y2,
                                   INT32 tv1, INT32 tv2, INT32 tc, INT32 dir);
#endif
...
// scan line
// --------------------------------------------------------------------------
static void R_RenderFloorSplat(floorsplat_t *pSplat, vertex_t *verts,
                               UINT8 *pTex) {
  // rasterizing
  ...
  if (y2 >= vid.height)
    y2 = vid.height - 1;
  rasterize_segment_tex(x1, ry1, x2, y2, 0, pSplat->size - 1, pSplat->size - 1,
                        1);
  if (ry1 < miny)
    miny = ry1;
  if (ry1 > maxy)
  ...
  if (y2 >= vid.height)
    y2 = vid.height - 1;
  rasterize_segment_tex(x1, ry1, x2, y2, pSplat->size - 1, 0, pSplat->size - 1,
                        0);
  if (ry1 < miny)
    miny = ry1;
  if (ry1 > maxy)
  ...
  for (y = miny; y <= maxy; y++) {
    ...
    angle =
        (currentplane->viewangle + currentplane->plangle) >> ANGLETOFINESHIFT;
