...
static int lib_iteratePlayers(lua_State *L) {
  INT32 i = -1;
  if (lua_gettop(L) < 2) {
    // return luaL_error(L, "Don't call players.iterate() directly, use it as
    // 'for player in players.iterate do <block> end'.");
...
static int lib_getPlayer(lua_State *L) {
  ...
  if (lua_type(L, 2) == LUA_TNUMBER) {
    ...
    if (i < 0 || i >= MAXPLAYERS)
      return luaL_error(L, "players[] index %d out of range (0 - %d)", i,
                        MAXPLAYERS - 1);
    if (!playeringame[i])
      return 0;
    if (!players[i].mo)
...
static int lib_iterateDisplayplayers(lua_State *L) {
  ...
  if (lua_gettop(L) < 2) {
    // return luaL_error(L, "Don't call displayplayers.iterate() directly, use
    // it as 'for player in displayplayers.iterate do <block> end'.");
  ...
  lua_remove(L, 1); // state is unused.
  if (!lua_isnil(L, 1)) {
    temp = (INT32)(*((player_t **)luaL_checkudata(L, 1, META_PLAYER)) -
                   players); // get the player # of the last iterated player.

    // @FIXME:
    // I didn't quite find a better way for this; Here, we go back to which
    // player in displayplayers we last iterated to resume the for loop below
    // for this new function call I don't understand enough about how the Lua
    // stacks work to get this to work in possibly a single line. So anyone feel
  ...
  for (i++; i < MAXSPLITSCREENPLAYERS; i++) {
    if (i > splitscreen || !playeringame[displayplayers[i]])
      return 0; // Stop! There are no more players for us to go through. There
                // will never be a player gap in displayplayers.
    ...
    lua_pushinteger(
        L,
        i); // push this to recall what number we were on for the next function
            // call. I suppose this also means you can retrieve the splitscreen
            // player number with 'for p, n in displayplayers.iterate'!
    return 2;
  }
  return 0;
}
...
static int lib_getDisplayplayers(lua_State *L) {
  ...
  if (lua_type(L, 2) == LUA_TNUMBER) {
    ...
    if (i < 0 || i >= MAXSPLITSCREENPLAYERS)
      return luaL_error(L, "displayplayers[] index %d out of range (0 - %d)", i,
...
static int player_get(lua_State *L) {
  ...
  //
    ...
    if (!lua_istable(L, -1)) { // no extra values table
      CONS_Debug(DBG_LUA,
                 M_GetText("'%s' has no extvars table or field named '%s'; "
                           "returning nil.\n"),
                 "player_t", field);
      return 0;
    }
    ...
    if (lua_isnil(L, -1)) // no value for this field
      CONS_Debug(DBG_LUA,
                 M_GetText("'%s' has no field named '%s'; returning nil.\n"),
                 "player_t", field);
  }
...
#define NOSET                                                                  \
  luaL_error(                                                                  \
      L, LUA_QL("player_t") " field " LUA_QS " should not be set directly.",   \
      field)
static int player_set(lua_State *L) {
  ...
  if (fastcmp(field, "mo")) {
    ...
    (newmo->player = plr)->mo = newmo; // set player pointer for new mobj, and
                                       // set new mobj as the player's mobj
  } else if (fastcmp(field, "cmd"))
    return NOSET;
  else if (fastcmp(field, "playerstate"))
  ...
  /*else if (fastcmp(field,"deltaviewheight"))
    ...
    if (newcolor >= MAXSKINCOLORS)
      return luaL_error(L, "player.skincolor %d out of range (0 - %d).",
                        newcolor, MAXSKINCOLORS - 1);
  ...
  //
    ...
    if (plr->awayviewtics && !plr->awayviewmobj) // awayviewtics must ALWAYS
                                                 // have an awayviewmobj set!!
      P_SetTarget(&plr->awayviewmobj,
                  plr->mo); // but since the script might set awayviewmobj
                            // immediately AFTER setting awayviewtics, use
                            // player mobj as filler for now.
  } else if (fastcmp(field, "awayviewaiming"))
    ...
    if (lua_isnil(L, -1)) {
      ...
      CONS_Debug(
          DBG_LUA,
          M_GetText("'%s' has no field named '%s'; adding it as Lua data.\n"),
          "player_t", field);
      lua_newtable(L);
      lua_pushlightuserdata(L, plr);
...
#define NOFIELD                                                                \
  luaL_error(L, LUA_QL("ticcmd_t") " has no field named " LUA_QS, field)
