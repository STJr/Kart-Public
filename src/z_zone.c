...
///        tag, and this file keeps track of all the allocations made. Later,
///        you can purge everything with a given tag.
///
///        Some tags (PU_CACHE, for example) may be automatically purged
///        whenever the space is needed, so memory allocated with these tags is
///        no longer guaranteed to be valid after another call to Z_Malloc().
///
...
static memblock_t *Ptr2Memblock2(void *ptr, const char *func, const char *file,
                                 INT32 line)
#else
...
#endif
{
  ...
  // properly.
  given =
      (void *)((size_t)((UINT8 *)ptr + extrabytes + sizeof *hdr + padsize / 2) &
               ~extrabytes);
...
#ifdef PARANOIA
  if (hdr->id != ZONEID)
    I_Error("Z_CT at %s:%d: wrong id", file, line);
#endif
...
size_t Z_TagUsage(INT32 tagnum) { return Z_TagsUsage(tagnum, tagnum); }
  ...
  for (block = head.next; block != &head; block = block->next)
    if (block->tag >= mintag && block->tag <= maxtag) {
      ...
      CONS_Printf("[%3d] %s (%s) bytes @ %s:%d\n", block->tag,
                  sizeu1(block->size), sizeu2(block->realsize),
                  filename ? filename + 1 : block->ownerfile, block->ownerline);
    }
}
#endif

// Creates a copy of a string.
char *Z_StrDup(const char *s) { return strcpy(ZZ_Alloc(strlen(s) + 1), s); }
