...
///        Fire flicker, light flash, strobe flash, lightning flash, glow, and
///        fade.
...
glow_t *P_SpawnAdjustableGlowingLight(sector_t *minsector, sector_t *maxsector,
  ...
  g->speed = length / 4;
  if (g->speed >
      (g->maxlight - g->minlight) / 2) // don't make it ridiculous speed
  ...
  if (ll->sector->lightlevel < ll->destlevel) {
    ...
    if (ll->sector->lightlevel + ll->speed >= ll->destlevel) {
      ...
      ll->sector->lightlevel =
          (INT16)(ll->sector->lightlevel + (INT16)ll->speed); // move lightlevel
  } else {
    // decrease lightlevel
    if (ll->sector->lightlevel - ll->speed <= ll->destlevel) {
      // stop changing light level
