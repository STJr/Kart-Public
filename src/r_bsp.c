...
boolean R_NoEncore(sector_t *sector, boolean ceiling) {
  ...
  if ((!(sector->special & (1 << 8)) ||
       (sector->special & ((4 | 8) << 8))) // spring panel
#endif
  ...
  if (sec->midmap != -1)
    ...
    for (i = 0; i <= splitscreen; i++) {
      if (viewplayer == &players[displayplayers[i]] && camera[i].chase) {
        heightsec =
            R_PointInSubsector(camera[i].x, camera[i].y)->sector->heightsec;
        break;
      }
    }
    ...
    if (i > splitscreen && viewmobj)
      heightsec =
          R_PointInSubsector(viewmobj->x, viewmobj->y)->sector->heightsec;
    else
      return sec;
    ...
    *tempsec = *sec;
    ...
    if ((underwater && (tempsec->floorheight = sec->floorheight,
                        ...
                        !back)) ||
        viewz <= s->floorheight) { // head-below-floor hack
...
boolean R_IsEmptyLine(seg_t *line, sector_t *front, sector_t *back) {
  return (!line->polyseg && back->ceilingpic == front->ceilingpic &&
          back->floorpic == front->floorpic &&
          back->f_slope == front->f_slope && back->c_slope == front->c_slope &&
...
static void R_AddLine(seg_t *line) {
  ...
  if (line->linedef->special == 40 && line->side == 0) {
    if (portalrender < cv_maxportals.value) {
      ...
      if (line2 >= 0) // found it!
      {
        R_AddPortal(line->linedef - lines, line2, x1,
                    x2); // Remember the lines for later rendering
  ...
  // Closed door.
  if (frontsector->f_slope || frontsector->c_slope || backsector->f_slope ||
      backsector->c_slope) {
...
#define SLOPEPARAMS(slope, end1, end2, normalheight)                           \
  if (slope) {                                                                 \
    ...
    SLOPEPARAMS(frontsector->f_slope, frontf1, frontf2,
                frontsector->floorheight)
    SLOPEPARAMS(frontsector->c_slope, frontc1, frontc2,
...
#undef SLOPEPARAMS
  ...
  // Identical floor and ceiling on both sides, identical light levels on both
  // sides, and no middle texture.
...
// 1 | 4 | 5 | 6
// 2 | 8 | 9 | A
INT32 checkcoord[12][4] = {{3, 0, 2, 1}, {3, 0, 2, 0}, {3, 1, 2, 0},
                           {0},               // UNUSED
                           {2, 0, 2, 1}, {0}, // UNUSED
                           {3, 1, 3, 0}, {0}, // UNUSED
...
static boolean R_CheckBBox(const fixed_t *bspcoord) {
  ...
  if ((boxpos = (viewx <= bspcoord[BOXLEFT]   ? 0
  ...
  // big room fix
  angle1 =
      R_PointToAngleEx(viewx, viewy, bspcoord[check[0]], bspcoord[check[1]]) -
      viewangle;
  angle2 =
      R_PointToAngleEx(viewx, viewy, bspcoord[check[2]], bspcoord[check[3]]) -
      viewangle;
...
#define pdist(x, y)                                                            \
  (FixedMul(                                                                   \
       R_PointToDist(x, y),                                                    \
       FINECOSINE((R_PointToAngle(x, y) - viewangle) >> ANGLETOFINESHIFT)) +   \
   0xFFFFFFF)
...
static void R_AddPolyObjects(subsector_t *sub) {
  ...
  for (i = 0; i < numpolys; ++i) {
    qsort(po_ptrs[i]->segs, po_ptrs[i]->segCount, sizeof(seg_t *),
          R_PolysegCompare);
...
static void R_Subsector(size_t num) {
  ...
  floorcenterz = frontsector->f_slope
                     ? P_GetZAt(frontsector->f_slope, frontsector->soundorg.x,
                                frontsector->soundorg.y)
                     : frontsector->floorheight;

  ceilingcenterz = frontsector->c_slope
                       ? P_GetZAt(frontsector->c_slope, frontsector->soundorg.x,
                                  frontsector->soundorg.y)
                       : frontsector->ceilingheight;

  // Check and prep all 3D floors. Set the sector floor/ceiling light levels and
  // colormaps.
  ...
  if (((frontsector->f_slope ? P_GetZAt(frontsector->f_slope, viewx, viewy)
    ...
    floorplane = R_FindPlane(
        ...
        frontsector->floor_xoffs, frontsector->floor_yoffs,
        frontsector->floorpic_angle, floorcolormap, NULL, NULL,
        frontsector->f_slope, R_NoEncore(frontsector, false));
  } else
    floorplane = NULL;
  ...
  if (((frontsector->c_slope ? P_GetZAt(frontsector->c_slope, viewx, viewy)
    ...
    ceilingplane = R_FindPlane(
        ...
        frontsector->ceiling_xoffs, frontsector->ceiling_yoffs,
        frontsector->ceilingpic_angle, ceilingcolormap, NULL, NULL,
        frontsector->c_slope, R_NoEncore(frontsector, true));
  } else
    ceilingplane = NULL;
  ...
  if (frontsector->ffloors) {
    ...
    for (rover = frontsector->ffloors; rover && numffloors < MAXFFLOORS;
         rover = rover->next) {
      if (!(rover->flags & FF_EXISTS) || !(rover->flags & FF_RENDERPLANES))
        continue;

      if (frontsector->cullheight) {
        if (R_DoCulling(frontsector->cullheight, viewsector->cullheight, viewz,
                        *rover->bottomheight, *rover->topheight)) {
      ...
      heightcheck = *rover->b_slope ? P_GetZAt(*rover->b_slope, viewx, viewy)
                                    : *rover->bottomheight;

      planecenterz = *rover->b_slope
                         ? P_GetZAt(*rover->b_slope, frontsector->soundorg.x,
                                    frontsector->soundorg.y)
                         : *rover->bottomheight;
      if (planecenterz <= ceilingcenterz && planecenterz >= floorcenterz &&
        ...
        ffloor[numffloors].plane = R_FindPlane(
            ...
            *rover->bottomyoffs, *rover->bottomangle,
            frontsector->lightlist[light].extra_colormap, rover, NULL,
            *rover->b_slope, R_NoEncore(rover->master->frontsector, true));
      ...
      heightcheck = *rover->t_slope ? P_GetZAt(*rover->t_slope, viewx, viewy)
                                    : *rover->topheight;

      planecenterz = *rover->t_slope
                         ? P_GetZAt(*rover->t_slope, frontsector->soundorg.x,
                                    frontsector->soundorg.y)
                         : *rover->topheight;
      if (planecenterz >= floorcenterz && planecenterz <= ceilingcenterz &&
        ...
        ffloor[numffloors].plane = R_FindPlane(
            ...
            *frontsector->lightlist[light].lightlevel, *rover->topxoffs,
            *rover->topyoffs, *rover->topangle,
            frontsector->lightlist[light].extra_colormap, rover, NULL,
            *rover->t_slope, R_NoEncore(rover->master->frontsector, false));
  ...
  if (sub->polyList) {
    ...
    while (po) {
      ...
      if (polysec->floorheight <= ceilingcenterz &&
        ...
        light = R_GetPlaneLight(frontsector, polysec->floorheight,
                                viewz < polysec->floorheight);
        ffloor[numffloors].plane = R_FindPlane(
            polysec->floorheight, polysec->floorpic, polysec->lightlevel,
            polysec->floor_xoffs, polysec->floor_yoffs,
            polysec->floorpic_angle - po->angle, NULL, NULL, po,
      ...
      if (polysec->ceilingheight >= floorcenterz &&
        ...
        light = R_GetPlaneLight(frontsector, polysec->ceilingheight,
                                viewz < polysec->ceilingheight);
        ffloor[numffloors].plane = R_FindPlane(
            polysec->ceilingheight, polysec->ceilingpic, polysec->lightlevel,
            polysec->ceiling_xoffs, polysec->ceiling_yoffs,
            polysec->ceilingpic_angle - po->angle, NULL, NULL, po,
            NULL // will ffloors be slopable eventually?
            ,
  ...
  while (count--) {
    ...
    if (!line->polyseg) // ignore segs that belong to polyobjects
      R_AddLine(line);
    line++;
    curline = NULL; /* cph 2001/11/18 - must clear curline now we're done with
                       it, so stuff doesn't try using it for other things */
  }
}
...
void R_Prep3DFloors(sector_t *sector) {
  ...
  fixed_t heighttest; // I think it's better to check the Z height at the
                      // sector's center than assume unsloped heights are
                      // accurate indicators of order in sloped sectors. -Red
  ...
  if (count != sector->numlights) {
    ...
    sector->lightlist =
        Z_Calloc(sizeof(*sector->lightlist) * count, PU_LEVEL, NULL);
    sector->numlights = count;
  } else
    memset(sector->lightlist, 0, sizeof(lightlist_t) * count);

  heighttest = sector->c_slope ? P_GetZAt(sector->c_slope, sector->soundorg.x,
                                          sector->soundorg.y)
  ...
  for (i = 1; i < count; i++) {
    ...
    for (rover = sector->ffloors; rover; rover = rover->next) {
      ...
      heighttest = *rover->t_slope
                       ? P_GetZAt(*rover->t_slope, sector->soundorg.x,
                                  sector->soundorg.y)
                       : *rover->topheight;
      ...
      if (rover->flags & FF_DOUBLESHADOW) {
        heighttest = *rover->b_slope
                         ? P_GetZAt(*rover->b_slope, sector->soundorg.x,
                                    sector->soundorg.y)
                         : *rover->bottomheight;
    ...
    if (best->flags & FF_NOSHADE) {
      ...
      sector->lightlist[i].extra_colormap =
          sector->lightlist[i - 1].extra_colormap;
    ...
    if (best->flags & FF_DOUBLESHADOW) {
      heighttest = *best->b_slope ? P_GetZAt(*best->b_slope, sector->soundorg.x,
                                             sector->soundorg.y)
                                  : *best->bottomheight;
      if (bestheight ==
          heighttest) /// TODO: do this in a more efficient way -Red
      {
        sector->lightlist[i].lightlevel =
            sector->lightlist[best->lastlight].lightlevel;
        sector->lightlist[i].extra_colormap =
