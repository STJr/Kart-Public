...
#if defined(_PSP) || defined(_arch_dreamcast) || defined(DJGPP) ||             \
    defined(_WII) || defined(_NDS) || defined(_PS3)
#define PRIdS "u"
#else
#define PRIdS "zu"
#endif
...
static CV_PossibleValue_t screenshot_cons_t[] = {
    {0, "Default"}, {1, "HOME"}, {2, "SRB2"}, {3, "CUSTOM"}, {0, NULL}};
consvar_t cv_screenshot_option = {"screenshot_option",
                                  "Default",
                                  CV_SAVE | CV_CALL,
                                  screenshot_cons_t,
                                  ...
                                  0,
                                  NULL};
consvar_t cv_screenshot_folder = {
    "screenshot_folder", "", CV_SAVE, NULL, NULL, 0, NULL, NULL, 0, 0, NULL};

static CV_PossibleValue_t moviemode_cons_t[] = {{MM_GIF, "GIF"},
                                                {MM_APNG, "aPNG"},
                                                {MM_SCREENSHOT, "Screenshots"},
                                                {0, NULL}};
consvar_t cv_moviemode = {"moviemode_mode",
                          "GIF",
                          CV_SAVE | CV_CALL,
                          moviemode_cons_t,
...
static CV_PossibleValue_t zlib_window_bits_t[] = {
    ...
    {8, "256"},
#endif
    {9, "512"}, {10, "1k"},  {11, "2k"},  {12, "4k"},
    {13, "8k"}, {14, "16k"}, {15, "32k"}, {0, NULL}};

static CV_PossibleValue_t apng_delay_t[] = {
    {1, "1x"}, {2, "1/2x"}, {3, "1/3x"}, {4, "1/4x"}, {0, NULL}};

// zlib memory usage is as follows:
// (1 << (zlib_window_bits+2)) +  (1 << (zlib_level+9))
consvar_t cv_zlib_memory = {"png_memory_level",
                            "7",
                            CV_SAVE,
                            zlib_mem_level_t,
                            NULL,
                            ...
                            0,
                            NULL};
consvar_t cv_zlib_level = {"png_compress_level",
                           "(Optimal) 6",
                           CV_SAVE,
                           zlib_level_t,
                           NULL,
                           ...
                           0,
                           NULL};
consvar_t cv_zlib_strategy = {"png_strategy",
                              "Normal",
                              CV_SAVE,
                              zlib_strategy_t,
                              NULL,
                              ...
                              0,
                              NULL};
consvar_t cv_zlib_window_bits = {"png_window_size",
                                 "32k",
                                 CV_SAVE,
                                 zlib_window_bits_t,
                                 NULL,
...
consvar_t cv_zlib_memorya = {"apng_memory_level",
                             "(Max Memory) 9",
                             CV_SAVE,
                             zlib_mem_level_t,
                             NULL,
                             ...
                             0,
                             NULL};
consvar_t cv_zlib_levela = {"apng_compress_level",
                            "4",
                            CV_SAVE,
                            zlib_level_t,
                            NULL,
                            ...
                            0,
                            NULL};
consvar_t cv_zlib_strategya = {"apng_strategy",
                               "RLE",
                               CV_SAVE,
                               zlib_strategy_t,
                               NULL,
                               ...
                               0,
                               NULL};
consvar_t cv_zlib_window_bitsa = {"apng_window_size",
                                  "32k",
                                  CV_SAVE,
                                  zlib_window_bits_t,
                                  NULL,
                                  ...
                                  0,
                                  NULL};
consvar_t cv_apng_delay = {
    "apng_speed", "1/2x", CV_SAVE, apng_delay_t, NULL, 0, NULL,
    NULL,         0,      0,       NULL};
...
M_MapNumber(char first, char second) {
  ...
  return 100 + ((INT32)tolower(first) - 'a') * 36 +
         (isdigit(second) ? ((INT32)second - '0')
...
//                          CONFIGURATION
// ==========================================================================
static boolean gameconfig_loaded =
    false; // true once config.cfg loaded AND executed
...
void Command_SaveConfig_f(void) {
  ...
  if (COM_Argc() < 2) {
    CONS_Printf(M_GetText(
        "saveconfig <filename[.cfg]> [-silent] : save config to a file\n"));
    return;
  }
...
void Command_LoadConfig_f(void) {
  if (COM_Argc() != 2) {
    CONS_Printf(
        M_GetText("loadconfig <filename[.cfg]> : load config from a file\n"));
    return;
  }
...
void Command_ChangeConfig_f(void) {
  if (COM_Argc() != 2) {
    CONS_Printf(M_GetText("changeconfig <filename[.cfg]> : save current config "
                          "and load another\n"));
    return;
  }
...
static const char *Newsnapshotfile(const char *pathname, const char *ext) {
  ...
  int add =
      i; // how much to add or subtract if wrong; gets divided by 2 each time
...
static void M_PNGhdr(png_structp png_ptr, png_infop png_info_ptr,
                     PNG_CONST png_uint_32 width, PNG_CONST png_uint_32 height,
                     PNG_CONST png_byte *palette) {
  const png_byte png_interlace = PNG_INTERLACE_NONE; // PNG_INTERLACE_ADAM7
  if (palette) {
    png_colorp png_PLTE =
        png_malloc(png_ptr, sizeof(png_color) * 256); // palette
    ...
    png_set_IHDR(png_ptr, png_info_ptr, width, height, 8,
                 PNG_COLOR_TYPE_PALETTE, png_interlace,
    ...
    png_set_IHDR(png_ptr, png_info_ptr, width, height, 8, PNG_COLOR_TYPE_RGB,
                 png_interlace, PNG_COMPRESSION_TYPE_BASE,
...
static void M_PNGText(png_structp png_ptr, png_infop png_info_ptr,
                      PNG_CONST png_byte movie) {
#ifdef PNG_TEXT_SUPPORTED
#define SRB2PNGTXT 11 // PNG_KEYWORD_MAX_LENGTH(79) is the max
  png_text png_infotext[SRB2PNGTXT];
  char keytxt[SRB2PNGTXT][12] = {"Title",      "Description", "Playername",
                                 "Mapnum",     "Mapname",     "Location",
                                 "Interface",  "Render Mode", "Revision",
  ...
  if (gamestate == GS_LEVEL && mapheaderinfo[gamemap - 1]->lvlttl[0] != '\0')
    snprintf(lvlttltext, 48, "%s%s%s", mapheaderinfo[gamemap - 1]->lvlttl,
             (strlen(mapheaderinfo[gamemap - 1]->zonttl) > 0)
                 ? va(" %s", mapheaderinfo[gamemap - 1]->zonttl)
                 : // SRB2kart
                 ((mapheaderinfo[gamemap - 1]->levelflags & LF_NOZONE)
                      ? ""
                      : " ZONE"),
             (strlen(mapheaderinfo[gamemap - 1]->actnum) > 0)
                 ? va(" %s", mapheaderinfo[gamemap - 1]->actnum)
                 : "");
  else
...
static inline void M_PNGImage(png_structp png_ptr, png_infop png_info_ptr,
                              PNG_CONST png_uint_32 height, png_bytep png_buf) {
...
#endif
typedef png_uint_32 (*P_png_set_acTL)(png_structp png_ptr, png_infop info_ptr,
                                      png_uint_32 num_frames,
                                      png_uint_32 num_plays);
typedef void (*P_png_write_frame_head)(
...
typedef void (*P_png_write_frame_tail)(png_structp png_ptr, png_infop info_ptr);
...
static void M_PNGFrame(png_structp png_ptr, png_infop png_info_ptr,
                       png_bytep png_buf) {
  png_uint_32 pitch = png_get_rowbytes(png_ptr, png_info_ptr);
  ...
  if (aPNG_write_frame_head)
    ...
    aPNG_write_frame_head(
        apng_ptr, apng_info_ptr, row_pointers, vid.width, /* width */
        height,                                           /* height */
        0,                                                /* x offset */
        0,                                                /* y offset */
...
static boolean M_SetupaPNG(png_const_charp filename, png_bytep pal) {
  ...
  apng_ptr =
      png_create_write_struct(PNG_LIBPNG_VER_STRING, NULL, PNG_error, PNG_warn);
  if (!apng_ptr) {
...
static inline moviemode_t M_StartMovieAPNG(const char *pathname) {
  ...
  CONS_Alert(CONS_ERROR,
             "Couldn't create aPNG: this build lacks aPNG support\n");
  return MM_OFF;
#endif
}
...
void M_StartMovie(void) {
  ...
  if (cv_screenshot_option.value == 0)
    ...
    pathname = srb2path;
  else if (cv_screenshot_option.value == 3 &&
           *cv_screenshot_folder.string != '\0')
 ...
 */
#if NUMSCREENS > 2
static boolean WritePCXfile(const char *filename, const UINT8 *data, int width,
                            int height, const UINT8 *palette) {
  int i;
  size_t length;
  pcx_t *pcx;
  UINT8 *pack;
...
void M_ScreenShot(void) { takescreenshot = true; }
...
void M_DoScreenShot(void) {
  ...
  if (rendermode == render_opengl)
    ...
    M_CreateScreenShotPalette();
#ifdef USE_PNG
    ret = M_SavePNG(va(pandf, pathname, freename), linear, vid.width,
                    vid.height, screenshot_palette);
#else
    ret = WritePCXfile(va(pandf, pathname, freename), linear, vid.width,
                       vid.height, screenshot_palette);
#endif
  }
...
failure:
  if (ret) {
    ...
    if (freename)
      CONS_Alert(CONS_ERROR,
                 M_GetText("Couldn't create screen shot %s in %s\n"), freename,
                 pathname);
    else
      CONS_Alert(
          CONS_ERROR,
          M_GetText(
              "Couldn't create screen shot in %s (all 10000 slots used!)\n"),
          pathname);
...
boolean M_ScreenshotResponder(event_t *ev) {
  ...
  if (ch >= KEY_MOUSE1 && menuactive) // If it's not a keyboard key, then don't
                                      // allow it in the menus!
    return false;

  if (ch == KEY_F8 || ch == gamecontrol[gc_screenshot][0] ||
      ch == gamecontrol[gc_screenshot][1]) // remappable F8
    M_ScreenShot();
  else if (ch == KEY_F9 || ch == gamecontrol[gc_recordgif][0] ||
           ch == gamecontrol[gc_recordgif][1]) // remappable F9
...
/** Token parser for TEXTURES, ANIMDEFS, and potentially other lumps later down
 * the line. Was originally R_GetTexturesToken when I was coding up the TEXTURES
 * parser, until I realized I needed it for ANIMDEFS too. Parses up to the next
 * whitespace character or comma. When finding the start of the next token,
 * whitespace is skipped. Commas are not; if a comma is encountered, then THAT'S
 * returned as the token. -Shadow Hog
 *
 * \param inputString The string to be parsed. If NULL is supplied instead of a
 * string, it will continue parsing the last supplied one. The pointer to the
 * last string supplied is stored as a static variable, so be careful not to
 * free it while this function is still using it! \return A pointer to a string,
 * containing the fetched token. This is in freshly allocated memory, so be sure
 * to Z_Free() it as appropriate.
 */
char *M_GetToken(const char *inputString) {
  static const char *stringToUse =
      NULL; // Populated if inputString != NULL; used otherwise
  static UINT32 startPos = 0;
  static UINT32 endPos = 0;
  static UINT32 stringLength = 0;
  static UINT8 inComment =
      0; // 0 = not in comment, 1 = // Single-line, 2 = /* Multi-line */
  ...
  if (startPos < stringLength - 1 && inComment == 0) {
    if (stringToUse[startPos] == '/' && stringToUse[startPos + 1] == '/') {
  ...
  while (
      (stringToUse[startPos] == ' ' || stringToUse[startPos] == '\t' ||
       stringToUse[startPos] == '\r' || stringToUse[startPos] == '\n' ||
       stringToUse[startPos] == '\0' ||
       stringToUse[startPos] == '"' // we're treating this as whitespace because
                                    // SLADE likes adding it for no good reason
       || inComment != 0) &&
      startPos < stringLength) {
    // Try to detect comment endings now
    if (inComment == 1 && stringToUse[startPos] == '\n') {
      // End of line for a single-line comment
      inComment = 0;
    } else if (inComment == 2 && startPos < stringLength - 1 &&
               stringToUse[startPos] == '*' &&
               stringToUse[startPos + 1] == '/') {
      // End of multi-line comment
      inComment = 0;
      startPos++; // Make damn well sure we're out of the comment ending at the
                  // end of it all
    }
  ...
  else if (stringToUse[startPos] == ',' || stringToUse[startPos] == '{' ||
           stringToUse[startPos] == '}') {
    endPos = startPos + 1;
  ...
  // Now find the end of the token. This includes several additional characters
  // that are okay to capture as one character, but not trailing at the end of
  // another token.
  endPos = startPos + 1;
  while ((stringToUse[endPos] != ' ' && stringToUse[endPos] != '\t' &&
          stringToUse[endPos] != '\r' && stringToUse[endPos] != '\n' &&
          stringToUse[endPos] != ',' && stringToUse[endPos] != '{' &&
          stringToUse[endPos] != '}' && stringToUse[endPos] != '"' // see above
          && inComment == 0) &&
         endPos < stringLength) {
    endPos++;
    // Try to detect comment starts now; if it's in a comment, we don't want it
    // in this token
    if (endPos < stringLength - 1 && inComment == 0) {
      if (stringToUse[endPos] == '/' && stringToUse[endPos + 1] == '/') {
        // Single-line comment start
        inComment = 1;
      } else if (stringToUse[endPos] == '/' && stringToUse[endPos + 1] == '*') {
        // Multi-line comment start
        inComment = 2;
      }
    }
  ...
  texturesToken = (char *)Z_Malloc((texturesTokenLength + 1) * sizeof(char),
                                   PU_STATIC, NULL);
  // Copy the string.
 ...
 */
UINT8
M_CountBits(UINT32 num, UINT8 size) {
  UINT8 i, sum = 0;
...
TVector *VectorMatrixMultiply(TVector v, TMatrix m) {
  ...
  ret[0] = FixedMul(v[0], m[0][0]) + FixedMul(v[1], m[1][0]) +
           FixedMul(v[2], m[2][0]) + FixedMul(v[3], m[3][0]);
  ret[1] = FixedMul(v[0], m[0][1]) + FixedMul(v[1], m[1][1]) +
           FixedMul(v[2], m[2][1]) + FixedMul(v[3], m[3][1]);
  ret[2] = FixedMul(v[0], m[0][2]) + FixedMul(v[1], m[1][2]) +
           FixedMul(v[2], m[2][2]) + FixedMul(v[3], m[3][2]);
  ret[3] = FixedMul(v[0], m[0][3]) + FixedMul(v[1], m[1][3]) +
           FixedMul(v[2], m[2][3]) + FixedMul(v[3], m[3][3]);
...
// Alam: note libkwave memcpy code comes from mplayer's libvo/aclib_template.c,
// r699
...
ATTRINLINE static FUNCINLINE void *__memcpy(void *dest, const void *src,
                                            size_t n) {
  int d0, d1, d2;
...
simplicity. [MF] */
static /*FUNCTARGET("sse2")*/ void *sse_cpy(void *dest, const void *src,
                                            size_t n) {
  void *retval = dest;
  size_t i;
...
static FUNCTARGET("mmx") void *mmx1_cpy(void *dest, const void *src,
                                        size_t n) // 3DNOW
{
  void *retval = dest;
  size_t i;
  ...
  if (n >= MMX1_MIN_LEN) {
    ...
    __asm__ __volatile__("femms" ::
                             : "memory"); // same as mmx_cpy() but with a femms
  }
  /*
   *	Now do the tail of the block
   */
...
// Alam: why? memcpy may be __cdecl/_System and our code may be not the same
// type
...
static /*FUNCTARGET("mmx")*/ void *mmx_cpy(void *dest, const void *src,
                                           size_t n) {
