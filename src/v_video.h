...
#define V_6WIDTHSPACE                                                          \
  0x00000400 // early 2.1 style spacing, variable widths, 6 character space
#define V_OLDSPACING                                                           \
  0x00000800 // Old style spacing, 8 per character 4 per space
#define V_MONOSPACE                                                            \
  0x00000C00 // Don't do width checks on characters, all characters 8 width
...
#define V_AUTOFADEOUT                                                          \
  0x000F0000 // used by CECHOs, automatic fade out when almost over
#define V_RETURN8 0x00100000 // 8 pixel return instead of 12
#define V_OFFSET 0x00200000  // account for offsets in patches
#define V_ALLOWLOWERCASE                                                       \
  0x00400000 // (strings only) allow fonts that have lowercase letters to use
             // them
...
#define V_DrawPatch(x, y, s, p)                                                \
  V_DrawFixedPatch((x) << FRACBITS, (y) << FRACBITS, FRACUNIT,                 \
                   s | V_NOSCALESTART | V_NOSCALEPATCH, p, NULL)
#define V_DrawTranslucentMappedPatch(x, y, s, p, c)                            \
  V_DrawFixedPatch((x) << FRACBITS, (y) << FRACBITS, FRACUNIT, s, p, c)
#define V_DrawSmallTranslucentMappedPatch(x, y, s, p, c)                       \
  V_DrawFixedPatch((x) << FRACBITS, (y) << FRACBITS, FRACUNIT / 2, s, p, c)
#define V_DrawTinyTranslucentMappedPatch(x, y, s, p, c)                        \
  V_DrawFixedPatch((x) << FRACBITS, (y) << FRACBITS, FRACUNIT / 4, s, p, c)
#define V_DrawMappedPatch(x, y, s, p, c)                                       \
  V_DrawFixedPatch((x) << FRACBITS, (y) << FRACBITS, FRACUNIT, s, p, c)
#define V_DrawSmallMappedPatch(x, y, s, p, c)                                  \
  V_DrawFixedPatch((x) << FRACBITS, (y) << FRACBITS, FRACUNIT / 2, s, p, c)
#define V_DrawTinyMappedPatch(x, y, s, p, c)                                   \
  V_DrawFixedPatch((x) << FRACBITS, (y) << FRACBITS, FRACUNIT / 4, s, p, c)
#define V_DrawScaledPatch(x, y, s, p)                                          \
  V_DrawFixedPatch((x) << FRACBITS, (y) << FRACBITS, FRACUNIT, s, p, NULL)
#define V_DrawSmallScaledPatch(x, y, s, p)                                     \
  V_DrawFixedPatch((x) << FRACBITS, (y) << FRACBITS, FRACUNIT / 2, s, p, NULL)
#define V_DrawTinyScaledPatch(x, y, s, p)                                      \
  V_DrawFixedPatch((x) << FRACBITS, (y) << FRACBITS, FRACUNIT / 4, s, p, NULL)
#define V_DrawTranslucentPatch(x, y, s, p)                                     \
  V_DrawFixedPatch((x) << FRACBITS, (y) << FRACBITS, FRACUNIT, s, p, NULL)
#define V_DrawSmallTranslucentPatch(x, y, s, p)                                \
  V_DrawFixedPatch((x) << FRACBITS, (y) << FRACBITS, FRACUNIT / 2, s, p, NULL)
#define V_DrawTinyTranslucentPatch(x, y, s, p)                                 \
  V_DrawFixedPatch((x) << FRACBITS, (y) << FRACBITS, FRACUNIT / 4, s, p, NULL)
#define V_DrawSciencePatch(x, y, s, p, sc)                                     \
...
void V_DrawKartString(INT32 x, INT32 y, INT32 option,
                      const char *string); // SRB2kart
