...
extern UINT8
    window_notinfocus; // are we in focus? (backend independant -- handles auto
                       // pausing and display of "focus lost" message)
...
// CTF colors.
extern UINT8 skincolor_redteam, skincolor_blueteam, skincolor_redring,
    skincolor_bluering;
...
typedef struct {
  ...
  UINT8
  fadeinid; // ID of the first fade, to a color -- ignored if fadecolor is 0
  UINT8 fadeoutid; // ID of the second fade, to the new screen
} scene_t; // TODO: It would probably behoove us to implement subsong/track
           // selection here, too, but I'm lazy -SH
...
extern mapthing_t *rflagpoint,
    *bflagpoint; // Pointers to the flag spawn locations
#define GF_REDFLAG 1
...
// Custom Lua values
// (This is not ifdeffed so the map header structure can stay identical, just in
// case.)
typedef struct {
  char option[32]; // 31 usable characters
  char value[256]; // 255 usable characters. If this seriously isn't enough then
                   // wtf.
} customoption_t;
...
typedef struct {
  ...
  char lvlttl[22]; ///< Level name without "Zone". (21 character limit instead
                   ///< of 32, 21 characters can display on screen max anyway)
  ...
  UINT16 mustrack; ///< Subsong to play. Only really relevant for music modules
                   ///< and specific formats supported by GME. 0 to ignore.
  UINT32 muspos;   ///< Music position to jump to.
  char forcecharacter[17]; ///< (SKINNAMESIZE+1) Skin to switch to or "" to
                           ///< disable.
  UINT8 weather; ///< 0 = sunny day, 1 = storm, 2 = snow, 3 = rain, 4 = blank, 5
                 ///< = thunder w/o rain, 6 = rain w/o lightning, 7 = heat wave.
  INT16 skynum;  ///< Sky number to use.
  INT16
  skybox_scalex; ///< Skybox X axis scale. (0 = no movement, 1 = 1:1 movement,
                 ///< 16 = 16:1 slow movement, -4 = 1:4 fast movement, etc.)
  ...
  char runsoc[33];     ///< SOC to execute at start of level (32 character limit
                       ///< instead of 63)
  char scriptname[33]; ///< Script to use when the map is switched to. (32
                       ///< character limit instead of 191)
  ...
  SINT8
  unlockrequired; ///< Is an unlockable required to play this level? -1 if no.
  UINT8 levelselect; ///< Is this map available in the level select? If so,
                     ///< which map list is it available in?
  SINT8 bonustype; ///< What type of bonus does this level have? (-1 for null.)
  SINT8 saveoverride; ///< Set how the game is allowed to save (1 for always, -1
                      ///< for never, 0 is 2.1 default)

  UINT8 levelflags; ///< LF_flags:  merged eight booleans into one UINT8 for
                    ///< space, see below
  UINT8 menuflags;  ///< LF2_flags: options that affect record attack / nights
                    ///< mode menus

  // NiGHTS stuff.
  UINT8 numGradedMares;   ///< Internal. For grade support.
  nightsgrades_t *grades; ///< NiGHTS grades. Allocated dynamically for space
                          ///< reasons. Be careful.

  // SRB2kart
  // boolean automap;    ///< Displays a level's white map outline in modified
  // games
  fixed_t mobj_scale; ///< Replacement for TOL_ERZ3

  // Music stuff.
  UINT32 musinterfadeout; ///< Fade out level music on intermission screen in
                          ///< milliseconds
  char musintername[7];   ///< Intermission screen music.

  // Lua stuff.
  // (This is not ifdeffed so the map header structure can stay identical, just
  // in case.)
  UINT8 numCustomOptions;     ///< Internal. For Lua custom value support.
  customoption_t *customopts; ///< Custom options. Allocated dynamically for
                              ///< space reasons. Be careful.
} mapheader_t;
...
#define LF2_NOVISITNEEDED                                                      \
  16 ///< Available in time attack/nights mode without visiting the level

#define LF2_EXISTSHACK                                                         \
  128 ///< Map lump exists; as noted, a single-bit hack that can be freely
      ///< movable to other variables without concern.
...
enum GameType // SRB2Kart
{
  ...
  // the following have been left in on account of just not wanting to deal with
  // removing all the checks for them
  GT_COOP,
  GT_COMPETITION,
  GT_TEAMMATCH,
  GT_TAG,
...
#define EMERALD6 32
#define EMERALD7 64
#define ALL7EMERALDS(v)                                                        \
  ((v & (EMERALD1 | EMERALD2 | EMERALD3 | EMERALD4 | EMERALD5 | EMERALD6 |     \
         EMERALD7)) == (EMERALD1 | EMERALD2 | EMERALD3 | EMERALD4 | EMERALD5 | \
                        EMERALD6 | EMERALD7))

extern INT32 nummaprings, nummapboxes,
    numgotboxes; // keep track of spawned rings/coins/battle mode items
