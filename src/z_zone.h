...
#include <stdio.h>
    ...
    (__GNUC__ == 4 && (__GNUC_MINOR__ >= 3 ||                                  \
                       (__GNUC_MINOR__ == 2 && __GNUC_PATCHLEVEL__ >= 5)))
...
#define PU_LUA                                                                 \
  2 // static entire execution time -- used by lua so it doesn't get caught in
    // loops forever
...
#define PU_HWRPATCHCOLMIPMAP                                                   \
  22 // Hardware GLMipmap_t struct colromap variation of patch
...
#define PU_HWRCACHE_UNLOCKED                                                   \
  102 // 'second-level' cache for graphics
...
void *Z_Malloc2(size_t size, INT32 tag, void *user, INT32 alignbits,
                const char *file, INT32 line) FUNCALLOC(1);
