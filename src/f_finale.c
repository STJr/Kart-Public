...
static void F_SkyScroll(INT32 scrollspeed) {
  ...
  anim2 = SHORT(pat2->width) -
          (((finalecount * scrollspeed) / 16) % SHORT(pat2->width));

  // SRB2Kart: F_DrawPatchCol is over-engineered; recoded to be less shitty and
  // error-prone
  if (rendermode != render_none) {
    ...
    y = 0;
    while (x < w) {
      V_DrawFixedPatch(x * FRACUNIT, y * FRACUNIT, FRACUNIT,
                       V_SNAPTOTOP | V_SNAPTOLEFT, pat, NULL);
      x += SHORT(pat->width);
    }
    ...
    while (x < w) {
      V_DrawFixedPatch(x * FRACUNIT, y * FRACUNIT, FRACUNIT,
                       V_SNAPTOBOTTOM | V_SNAPTOLEFT, pat2, NULL);
      x += SHORT(pat2->width);
    }
  }
  ...
  if (intro_scenenum == 0) {
    if (timetonext <= 0) {
      ...
      // Stay on black for a bit. =)
      {
        ...
        quittime =
            I_GetTime() +
            NEWTICRATE * 2; // Shortened the quit time, used to be 2 seconds
...
static const char *credits[] = {
    ...
    "Karol \"Fooruman\" D"
    "\x1E"
    "browski", // Dąbrowski, <Sryder> accents in srb2 :ytho:
    "\"Virt\"",
    "",
    "\1Special Thanks",
    "SEGA",
...
void F_CreditDrawer(void) {
  ...
  // Draw background
  V_DrawSciencePatch(
      0, 0 - FixedMul(32 << FRACBITS, FixedDiv(credbgtimer % TICRATE, TICRATE)),
      V_SNAPTOTOP, W_CachePatchName("CREDTILE", PU_CACHE), FRACUNIT);

  V_DrawSciencePatch(
      0,
      0 - FixedMul(40 << FRACBITS,
                   FixedDiv(credbgtimer % (TICRATE / 2), (TICRATE / 2))),
      V_SNAPTOTOP, W_CachePatchName("CREDZIGZ", PU_CACHE), FRACUNIT);
  V_DrawSciencePatch(
      320 << FRACBITS,
      0 - FixedMul(40 << FRACBITS,
                   FixedDiv(credbgtimer % (TICRATE / 2), (TICRATE / 2))),
      V_SNAPTOTOP | V_FLIP, W_CachePatchName("CREDZIGZ", PU_CACHE), FRACUNIT);
  ...
  for (i = 0; credits_pics[i].patch; i++) {
    ...
    if (credits_pics[i].colorize != SKINCOLOR_NONE) {
      colormap = R_GetTranslationColormap(TC_RAINBOW, credits_pics[i].colorize,
                                          GTC_MENUCACHE);
      sc = FRACUNIT; // quick hack so I don't have to add another field to
                     // credits_pics
    }

    V_DrawFixedPatch(
        credits_pics[i].x << FRACBITS,
        (credits_pics[i].y << FRACBITS) - 4 * (animtimer << FRACBITS) / 5, sc,
        0, W_CachePatchName(credits_pics[i].patch, PU_CACHE), colormap);
  }
  ...
  for (i = 0; credits[i]; i++) {
    ...
    case 1:
      if (y >> FRACBITS > -20)
        V_DrawCreditString((160 - (V_CreditStringWidth(&credits[i][1]) >> 1))
                               << FRACBITS,
                           y, 0, &credits[i][1]);
      y += 30 << FRACBITS;
      break;
  ...
  // RR isn't any time soon as of writing, but v1.4 is expected to be the last
  // v1 release. Let's give it a proper send off.
  if (finalecount) {
    const char *goodbyefornow = "See you in "
                                "\x82"
    ...
    fixed_t x = FixedMul(((BASEVIDWIDTH << FRACBITS) + w + lpad),
                         ((finalecount - 1) << FRACBITS) / (5 * TICRATE)) -
                w - (lpad / 2);
    V_DrawString(x >> FRACBITS, y >> FRACBITS, V_ALLOWLOWERCASE,
                 goodbyefornow); // for some reason DrawStringAtFixed can't
                                 // tolerate colour codes
  }
}

void F_CreditTicker(void) {
  // "Simulate" the drawing of the credits so that dedicated mode doesn't get
  // stuck
  UINT16 i;
  fixed_t y = (80 << FRACBITS) - 5 * (animtimer << FRACBITS) / 8;
  ...
  // Do this here rather than in the drawer you doofus! (this is why dedicated
  // mode broke at credits)
...
void F_GameEvaluationDrawer(void) {
  ...
  for (i = 0; i < 7; ++i) {
    ...
    if (emeralds & (1 << i))
      ...
      V_DrawTranslucentPatch(x, y, TRANSLEVEL,
                             W_CachePatchName(patchname, PU_CACHE));
...
void F_StartTitleScreen(void) {
  ...
  if (!demo.fromtitle) // SRB2Kart: Don't reset music if the right track is
                       // already playing
    S_StopMusic();
...
void F_TitleScreenDrawer(void) {
  ...
  if (!ttbanner ||
      (gamestate != GS_TITLESCREEN && gamestate != GS_WAITINGPLAYERS)) {
  ...
  if (finalecount < 50) {
    ...
    if (finalecount >= 20)
      ...
      V_DrawSciencePatch((84 << FRACBITS) -
                             FixedDiv(180 << FRACBITS, 10 << FRACBITS) *
                                 (20 - finalecount),
                         (87 << FRACBITS), 0, ttkart, FRACUNIT / 2);
    ...
    V_DrawSciencePatch(
        0, 0 - FixedMul(40 << FRACBITS, FixedDiv(finalecount % 70, 70)),
        V_SNAPTOTOP | V_SNAPTOLEFT, ttcheckers, FRACUNIT);
    V_DrawSciencePatch(
        280 << FRACBITS,
        -(40 << FRACBITS) +
            FixedMul(40 << FRACBITS, FixedDiv(finalecount % 70, 70)),
        V_SNAPTOTOP | V_SNAPTORIGHT, ttcheckers, FRACUNIT);
...
void F_TitleScreenTicker(boolean run) {
  ...
  if (!(--demoIdleLeft)) {
    ...
    /*if (curDemo == numDemos) -- uuuh... we have a LOT of maps AND a big
    devteam... probably not gonna see a repeat unless you're super unlucky :V
    {
            curDemo = 0;
            F_StartIntro();
            return;
    ...
    mapname =
        G_BuildMapName(G_RandMap(TOL_RACE, -2, false, 0, false, NULL) + 1);

    numstaff = 1;
    while (numstaff < 99 &&
           (l = W_CheckNumForName(va("%sS%02u", mapname, numstaff + 1))) !=
               LUMPERROR)
      numstaff++;
    ...
    {
            CONS_Alert(CONS_ERROR, M_GetText("Demo lump \"%s\" doesn't
    exist\n"), dname); F_StartIntro(); return;
    }*/
...
void F_TitleDemoTicker(void) { keypressed = false; }
...
void F_StartWaitingPlayers(void) {
  ...
  waitcolormap =
      R_GetTranslationColormap(randskin, skins[randskin].prefcolor, 0);

  for (i = 0; i < 2; i++) {
    sprframe =
        &skins[randskin].spritedef.spriteframes[(6 + (i * 3)) & FF_FRAMEMASK];
...
void F_WaitingPlayersTicker(void) {
  ...
  // dumb hack, only start the music on the 1st tick so if you instantly go into
  // the map you aren't hearing a tic of music
  if (finalecount == 2)
    S_ChangeMusicInternal("WAIT2J", true);
}

void F_WaitingPlayersDrawer(void) {
  UINT32 frame = (finalecount % 8) /
                 4; // The game only tics every other frame while waitingplayers
  INT32 flags = V_FLIP;
  const char *waittext1 = "You will join";
  const char *waittext2 = "the next race...";
  V_DrawFill(0, 0, BASEVIDWIDTH, BASEVIDHEIGHT, 31);
  V_DrawCreditString((160 - (V_CreditStringWidth(waittext1) >> 1)) << FRACBITS,
                     48 << FRACBITS, 0, waittext1);
  V_DrawCreditString((160 - (V_CreditStringWidth(waittext2) >> 1)) << FRACBITS,
                     64 << FRACBITS, 0, waittext2);
  V_DrawFixedPatch((160 << FRACBITS) - driver[frame]->width / 2,
                   150 << FRACBITS, 1 << FRACBITS, flags, driver[frame],
                   waitcolormap);
}
...
void F_ContinueDrawer(void) {
  patch_t *contsonic;
  INT32 i, x = (BASEVIDWIDTH / 2) + 4,
           ncontinues = players[consoleplayer].continues;
  if (ncontinues > 20)
    ncontinues = 20;
  ...
  // Draw a Sonic!
  V_DrawScaledPatch((BASEVIDWIDTH - SHORT(contsonic->width)) / 2, 32, 0,
                    contsonic);

  // Draw the continue markers! Show continues minus one.
  x -= ncontinues * 6;
  for (i = 0; i < ncontinues; ++i)
    V_DrawContinueIcon(x + (i * 12), 140, 0, players[consoleplayer].skin,
                       players[consoleplayer].skincolor);

  V_DrawCenteredString(BASEVIDWIDTH / 2, 168, 0,
                       va("\x82*\x80"
                          " %02d "
                          "\x82*\x80",
                          timetonext / TICRATE));
...
static void F_AdvanceToNextScene(void) {
  if (rendermode != render_none) {
    ...
    if (cutscenes[cutnum]->scene[scenenum].fadecolor) {
      V_DrawFill(0, 0, BASEVIDWIDTH, BASEVIDHEIGHT,
                 cutscenes[cutnum]->scene[scenenum].fadecolor);
  ...
  // (possible overflow / bad patch names from the one tic drawn before the
  // fade)
...
void F_EndCutScene(void) {
  cutsceneover = true; // do this first, just in case G_EndGame or something
                       // wants to turn it back false later
  ...
  if (cutscenes[cutnum]->scene[scenenum].picname[picnum][0] != '\0') {
    if (cutscenes[cutnum]->scene[scenenum].pichires[picnum])
      V_DrawSmallScaledPatch(
          picxpos, picypos, 0,
          W_CachePatchName(cutscenes[cutnum]->scene[scenenum].picname[picnum],
                           PU_CACHE));
    else
      V_DrawScaledPatch(
          picxpos, picypos, 0,
...
void F_CutsceneTicker(void) {
  ...
  for (i = 0; i < MAXPLAYERS; i++) {
    ...
    if (players[i].cmd.buttons & BT_BRAKE ||
        players[i].cmd.buttons & BT_ACCELERATE) // SRB2kart
    {
      keypressed = false;
