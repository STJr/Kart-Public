...
//-----------------------------------------------------------------------------
/// \file  r_data.c
/// \brief Preparation of data for rendering,generation of lookups, caching,
/// retrieval by name
...
static UINT8 *
    *texturecache; // graphics data for each generated full-size texture
...
//
static inline void R_DrawColumnInCache(column_t *patch, UINT8 *cache,
                                       INT32 originy, INT32 cacheheight) {
...
static UINT8 *R_GenerateTexture(size_t texnum) {
  ...
  for (i = 0, patch = texture->patches; i < texture->patchcount; i++, patch++) {
    ...
    for (; x < x2; x++) {
      patchcol =
          (column_t *)((UINT8 *)realpatch + LONG(realpatch->columnofs[x - x1]));

      // generate column ofset lookup
      *(UINT32 *)&colofs[x << 2] =
          LONG((x * texture->height) + (texture->width * 4));
      R_DrawColumnInCache(patchcol, block + LONG(*(UINT32 *)&colofs[x << 2]),
...
// R_CheckTextureCache
//
// Use this if you need to make sure the texture is cached before R_GetColumn
// calls e.g.: midtextures and FOF walls
//
...
// Need these prototypes for later; defining them here instead of r_data.h so
// they're "private"
...
void R_LoadTextures(void) {
  ...
  // the markers.
  // This system will allocate memory for all duplicate/patched textures even if
  // it never uses them, but the alternative is to spend a ton of time checking
  // and re-checking all previous entries just to skip any potentially patched
  // textures.
  for (w = 0, numtextures = 0; w < numwadfiles; w++) {
    ...
    while (texturesLumpPos != INT16_MAX) {
      numtextures +=
          R_CountTexturesInTEXTURESLump((UINT16)w, (UINT16)texturesLumpPos);
      texturesLumpPos =
          W_CheckNumForNamePwad("TEXTURES", (UINT16)w, texturesLumpPos + 1);
    }
    ...
    if (texstart != INT16_MAX && texend != INT16_MAX) {
      ...
      if (W_FileHasFolders(wadfiles[w])) {
        for (j = texstart; j < texend; j++) {
          if (!W_IsLumpFolder(
                  (UINT16)w,
                  j)) // Check if lump is a folder; if not, then count it
            numtextures++;
        }
      } else {
  ...
  // Allocate memory and initialize to 0 for all the textures we are
  // initialising. There are actually 5 buffers allocated in one for
  // convenience.
  textures = Z_Calloc((numtextures * sizeof(void *)) * 5, PU_STATIC, NULL);

  // Allocate texture column offset table.
  texturecolumnofs =
      (void *)((UINT8 *)textures + (numtextures * sizeof(void *)));
  // Allocate texture referencing cache.
  texturecache =
      (void *)((UINT8 *)textures + ((numtextures * sizeof(void *)) * 2));
  // Allocate texture width mask table.
  texturewidthmask =
      (void *)((UINT8 *)textures + ((numtextures * sizeof(void *)) * 3));
  // Allocate texture height mask table.
  textureheight =
      (void *)((UINT8 *)textures + ((numtextures * sizeof(void *)) * 4));
  ...
  for (i = 0, w = 0; w < numwadfiles; w++) {
    ...
    for (j = 0; j < (texend - texstart); j++) {
      ...
      // CONS_Printf("\n\"%s\" is a single patch, dimensions %d x
      // %d",W_CheckNameForNumPwad((UINT16)w,texstart+j),patchlump->width,
      // patchlump->height);
      texture = textures[i] =
          Z_Calloc(sizeof(texture_t) + sizeof(texpatch_t), PU_STATIC, NULL);

      // Set texture properties.
      M_Memcpy(texture->name, W_CheckNameForNumPwad((UINT16)w, texstart + j),
               sizeof(texture->name));
...
static texpatch_t *R_ParsePatch(boolean actuallyLoadPatch) {
  ...
  if (texturesToken == NULL) {
    I_Error("Error parsing TEXTURES lump: Unexpected end of file where patch "
            "name should be");
  }
  texturesTokenLength = strlen(texturesToken);
  if (texturesTokenLength > 8) {
    I_Error(
        "Error parsing TEXTURES lump: Patch name \"%s\" exceeds 8 characters",
        texturesToken);
  } else {
    if (patchName != NULL) {
      Z_Free(patchName);
    }
    patchName = (char *)Z_Malloc((texturesTokenLength + 1) * sizeof(char),
                                 PU_STATIC, NULL);
  ...
  texturesToken = M_GetToken(NULL);
  if (texturesToken == NULL) {
    I_Error("Error parsing TEXTURES lump: Unexpected end of file where comma "
            "after \"%s\"'s patch name should be",
            patchName);
  }
  if (strcmp(texturesToken, ",") != 0) {
    I_Error("Error parsing TEXTURES lump: Expected \",\" after %s's patch "
            "name, got \"%s\"",
  ...
  if (endPos == texturesToken // Empty string
    ...
    I_Error("Error parsing TEXTURES lump: Expected an integer for patch "
            "\"%s\"'s x coordinate, got \"%s\"",
            patchName, texturesToken);
  }
  ...
  if (texturesToken == NULL) {
    I_Error("Error parsing TEXTURES lump: Unexpected end of file where comma "
            "after patch \"%s\"'s x coordinate should be",
            patchName);
  }
  if (strcmp(texturesToken, ",") != 0) {
    I_Error("Error parsing TEXTURES lump: Expected \",\" after patch \"%s\"'s "
            "x coordinate, got \"%s\"",
            patchName, texturesToken);
  }
...
static texture_t *R_ParseTexture(boolean actuallyLoadTexture) {
  ...
  texturesToken = M_GetToken(NULL);
  if (texturesToken == NULL) {
    I_Error("Error parsing TEXTURES lump: Unexpected end of file where texture "
            "name should be");
  }
  texturesTokenLength = strlen(texturesToken);
  if (texturesTokenLength > 8) {
    I_Error(
        "Error parsing TEXTURES lump: Texture name \"%s\" exceeds 8 characters",
        texturesToken);
  } else {
    memset(&newTextureName, 0, 9);
    M_Memcpy(newTextureName, texturesToken, texturesTokenLength);
    // ^^ we've confirmed that the token is <= 8 characters so it will never
    // overflow a 9 byte char buffer
    strupr(
        newTextureName); // Just do this now so we don't have to worry about it
  }
  Z_Free(texturesToken);
  ...
  if (texturesToken == NULL) {
    I_Error("Error parsing TEXTURES lump: Unexpected end of file where comma "
            "after texture \"%s\"'s name should be",
            newTextureName);
  } else if (strcmp(texturesToken, ",") != 0) {
    I_Error("Error parsing TEXTURES lump: Expected \",\" after texture "
            "\"%s\"'s name, got \"%s\"",
  ...
  {
    I_Error("Error parsing TEXTURES lump: Expected a positive integer for "
            "texture \"%s\"'s width, got \"%s\"",
            newTextureName, texturesToken);
  }
  Z_Free(texturesToken);
  ...
  if (texturesToken == NULL) {
    I_Error("Error parsing TEXTURES lump: Unexpected end of file where comma "
            "after texture \"%s\"'s width should be",
            newTextureName);
  }
  if (strcmp(texturesToken, ",") != 0) {
    I_Error("Error parsing TEXTURES lump: Expected \",\" after texture "
  ...
  texturesToken = M_GetToken(NULL);
  if (texturesToken == NULL) {
    I_Error("Error parsing TEXTURES lump: Unexpected end of file where open "
            "curly brace for texture \"%s\" should be",
            newTextureName);
  }
  if (strcmp(texturesToken, "{") == 0) {
    if (actuallyLoadTexture) {
      // Allocate memory for a zero-patch texture. Obviously, we'll be adding
      // patches momentarily.
    ...
    if (texturesToken == NULL) {
      I_Error("Error parsing TEXTURES lump: Unexpected end of file where patch "
              "definition for texture \"%s\" should be",
              newTextureName);
    }
    while (strcmp(texturesToken, "}") != 0) {
      if (stricmp(texturesToken, "PATCH") == 0) {
        ...
        if (resultTexture) {
          ...
          resultTexture =
              Z_Realloc(resultTexture,
                        sizeof(texture_t) + (resultTexture->patchcount + 1) *
                                                sizeof(texpatch_t),
                        PU_STATIC, NULL);
          // Populate the uninitialized values in the new patch entry of our
          // array
          M_Memcpy(&resultTexture->patches[resultTexture->patchcount], newPatch,
                   sizeof(texpatch_t));
          // Account for the new number of patches in the texture
          resultTexture->patchcount++;
          // Then free up the memory assigned to R_ParsePatch, as it's unneeded
          // now
          Z_Free(newPatch);
        } else {
          R_ParsePatch(false);
        ...
        I_Error("Error parsing TEXTURES lump: Expected \"PATCH\" in texture "
                "\"%s\", got \"%s\"",
                newTextureName, texturesToken);
      }
      ...
      if (texturesToken == NULL) {
        I_Error(
            "Error parsing TEXTURES lump: Unexpected end of file where patch "
            "declaration or right curly brace for texture \"%s\" should be",
            newTextureName);
      }
    }
    if (resultTexture && resultTexture->patchcount == 0) {
      I_Error("Error parsing TEXTURES lump: Texture \"%s\" must have at least "
              "one patch",
              newTextureName);
    }
  } else {
    I_Error("Error parsing TEXTURES lump: Expected \"{\" for texture \"%s\", "
            "got \"%s\"",
            newTextureName, texturesToken);
  }
  Z_Free(texturesToken);
...
int R_CountTexturesInTEXTURESLump(UINT16 wadNum, UINT16 lumpNum) {
  ...
  texturesText = (char *)Z_Malloc((texturesLumpLength + 1) * sizeof(char),
                                  PU_STATIC, NULL);
  ...
  while (texturesToken != NULL) {
    if (stricmp(texturesToken, "WALLTEXTURE") == 0) {
      ...
      I_Error(
          "Error parsing TEXTURES lump: Expected \"WALLTEXTURE\", got \"%s\"",
          texturesToken);
    }
    texturesToken = M_GetToken(NULL);
  }
...
/*
static inline lumpnum_t R_CheckNumForNameList(const char *name, lumplist_t
*list, size_t listsize) // SRB2kart - unused.
{
        size_t i;
        UINT16 lump;
        ...
        {
                lump = W_CheckNumForNamePwad(name, list[i].wadfile,
list[i].firstlump); if (lump == INT16_MAX || lump > (list[i].firstlump +
list[i].numlumps)) continue; else return (list[i].wadfile<<16)+lump;
        }
        return LUMPERROR;
}
*/
...
static void R_InitExtraColormaps(void)
{
        ...
        for (cfile = clump = 0; cfile < numwadfiles; cfile++, clump = 0)
        {
                ...
                if (endnum == INT16_MAX)
                        I_Error("R_InitExtraColormaps: C_START without
C_END\n");

                // This shouldn't be possible when you use the Pwad function,
silly
                //if (WADFILENUM(startnum) != WADFILENUM(endnum))
                        //I_Error("R_InitExtraColormaps: C_START and C_END in
different wad files!\n");

                if (numcolormaplumps >= maxcolormaplumps)
                        maxcolormaplumps *= 2;
                colormaplumps = Z_Realloc(colormaplumps,
                        sizeof (*colormaplumps) * maxcolormaplumps, PU_STATIC,
NULL); colormaplumps[numcolormaplumps].wadfile = cfile;
                colormaplumps[numcolormaplumps].firstlump = startnum+1;
                colormaplumps[numcolormaplumps].numlumps = endnum - (startnum +
1); numcolormaplumps++;
        }
...
lumpnum_t R_GetFlatNumForName(const char *name) {
  ...
  for (i = numwadfiles - 1; i >= 0; i--) {
    ...
    case RET_WAD:
      if ((start = W_CheckNumForNamePwad("F_START", (UINT16)i, 0)) ==
          INT16_MAX) {
        if ((start = W_CheckNumForNamePwad("FF_START", (UINT16)i, 0)) ==
            INT16_MAX)
          continue;
        else if ((end = W_CheckNumForNamePwad("FF_END", (UINT16)i, start)) ==
                 INT16_MAX)
          continue;
      } else if ((end = W_CheckNumForNamePwad("F_END", (UINT16)i, start)) ==
                 INT16_MAX)
        continue;
      break;
    case RET_PK3:
...
//
// R_InitSpriteLumps
// Finds the width and hoffset of all sprites in the wad, so the sprite does not
// need to be cached completely, just for having the header info ready during
// rendering.
//
...
static void R_InitSpriteLumps(void) {
  ...
  Z_Malloc(max_spritelumps * sizeof(*spritecachedinfo), PU_STATIC,
           &spritecachedinfo);
}
...
#ifdef HASINVERT
  R_MakeInvertmap(); // this isn't the BEST place to do it the first time, but
                     // whatever
#endif
}
...
/*INT32 R_ColormapNumForName(char *name)
{
        ...
        if (num_extra_colormaps == MAXCOLORMAPS)
                I_Error("R_ColormapNumForName: Too many colormaps! the limit is
%d\n", MAXCOLORMAPS);

        lump = R_CheckNumForNameList(name, colormaplumps, numcolormaplumps);
        if (lump == LUMPERROR)
                I_Error("R_ColormapNumForName: Cannot find colormap lump
%.8s\n", name);
...
Z_MallocAlign(W_LumpLength(lump), PU_LEVEL, NULL, 16); W_ReadLump(lump,
extra_colormaps[num_extra_colormaps].colormap);
...
void R_MakeInvertmap(void) {
  ...
  for (i = 0; i < 256; i++)
    invertmap[i] = NearestColor(256 - pLocalPalette[i].s.red,
                                256 - pLocalPalette[i].s.green,
                                256 - pLocalPalette[i].s.blue);
}
#endif
...
#define HEX2INT(x)                                                             \
  ...
  if (p1[0] == '#') {
    ...
    if (encoremap) {
      ...
      // -- moved encoremap upwards for optimisation
...
#undef HEX2INT
  ...
  if (rendermode == render_soft) {
    ...
    // deltas[i] stores a corresponding fade delta between the RGB color and the
    // final fade color;
    ...
    colormap_p =
        Z_MallocAlign((256 * (encoremap ? 64 : 32)) + 10, PU_LEVEL, NULL, 8);
    ...
    for (p = 0; p < 32; p++) {
      for (i = 0; i < 256; i++) {
        *colormap_p =
            NearestColor((UINT8)RoundUp(map[i][0]), (UINT8)RoundUp(map[i][1]),
...
R_CheckTextureNumForName(const char *name) {
  ...
  // Need to parse the list backwards, so textures loaded more recently are used
  // in lieu of ones loaded earlier
  // for (i = 0; i < numtextures; i++) <- old
  for (i = (numtextures - 1); i >= 0; i--) // <- new
    if (!strncasecmp(textures[i]->name, name, 8)) {
      tidcachelen++;
      Z_Realloc(tidcache, tidcachelen * sizeof(*tidcache), PU_STATIC,
                &tidcache);
      strncpy(tidcache[tidcachelen - 1].name, name, 8);
      tidcache[tidcachelen - 1].name[8] = '\0';
#ifndef ZDEBUG
      CONS_Debug(DBG_SETUP, "texture #%s: %s\n", sizeu1(tidcachelen),
                 tidcache[tidcachelen - 1].name);
#endif
...
void R_PrecacheLevel(void) {
  ...
  // do not flush the memory, Z_Malloc twice with same user will cause error in
  // Z_CheckHeap()
  if (rendermode != render_soft)
    return;
  ...
  if (texturepresent == NULL)
    I_Error("%s: Out of memory looking up textures", "R_PrecacheLevel");
  ...
  // flat, while the sky texture is stored like a wall texture, with a skynum
  // dependent name.
  ...
  if (spritepresent == NULL)
    I_Error("%s: Out of memory looking up sprites", "R_PrecacheLevel");
