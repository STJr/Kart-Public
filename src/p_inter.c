...
boolean P_CanPickupItem(player_t *player, UINT8 weapon) {
  ...
  /*if (G_BattleGametype() && player->kartstuff[k_bumper] <= 0) // No bumpers in
  Match return false;*/
...
void P_DoNightsScore(player_t *player) {
  ...
  dummymo = P_SpawnMobj(player->mo->x, player->mo->y,
                        player->mo->z + player->mo->height / 2, MT_NIGHTSCORE);
  if (player->bot)
  ...
  if (player->linkcount < 10) {
    if (player->bonustime) {
      ...
      P_SetMobjState(dummymo,
                     dummymo->info->xdeathstate + player->linkcount - 1);
    } else {
      P_AddPlayerScore(player, player->linkcount * 10);
      P_SetMobjState(dummymo,
                     dummymo->info->spawnstate + player->linkcount - 1);
    }
  } else {
    if (player->bonustime) {
  ...
  // P_InstaThrust(dummymo, R_PointToAngle2(dummymo->x, dummymo->y, camera[0].x,
  // camera[0].y), 3*FRACUNIT);
...
void P_TouchSpecialThing(mobj_t *special, mobj_t *toucher,
  ...
  if (heightcheck) {
    ...
    { // flingemerald sprites are low to the ground, so extend collision radius
    down some. if (toucher->z > (special->z + special->height)) return; if
    (special->z - special->height > (toucher->z + toucher->height)) return;
    }
    else*/
    {
      if (toucher->momz < 0) {
  ...
  if (special->flags & MF_BOSS) {
    ...
    || player->powers[pw_invulnerability] || player->powers[pw_super]) // Do you
    possess the ability to subdue the object?
    {
    ...
    || (toucher->z + toucher->height > special->z + special->height &&
    (toucher->eflags & MFE_VERTICALFLIP)))
    && player->charability == CA_FLY
    && (player->powers[pw_tailsfly]
    || (toucher->state >= &states[S_PLAY_SPC1] && toucher->state <=
    &states[S_PLAY_SPC4]))) // Tails can shred stuff with his propeller.
    {
    ...
    return;
             ...
             (special->type !=
              MT_SPRINGSHELL)) // Kart: prevent random hits from these things
  {
    P_DamageMobj(toucher, special, special, 1);
    return;
    ...
    case MT_MEMENTOSTP: // Mementos teleport
      // Teleport player to the other teleporter (special->target). We'll
      // assume there's always only ever 2.
      if (!special->target)
        return; // foolproof crash prevention check!!!!!

      P_SetOrigin(player->mo, special->target->x, special->target->y,
                  special->target->z + (48 << FRACBITS));
    ...
    case MT_EGGMANITEM:
      if ((special->target == toucher || special->target == toucher->target) &&
          (special->threshold > 0))
        return;
      ...
      {
        mobj_t *poof =
            P_SpawnMobj(special->x, special->y, special->z, MT_EXPLODE);
        S_StartSound(poof, special->info->deathsound);
      }

      if (special->target && special->target->player) {
        if (G_RaceGametype() ||
            special->target->player->kartstuff[k_bumper] > 0)
    ...
    case MT_KARMAHITBOX:
      ...
      if (!special->target->player->kartstuff[k_comebackmode]) {
        if (player->kartstuff[k_growshrinktimer] ||
          ...
          mobj_t *boom = P_SpawnMobj(special->target->x, special->target->y,
                                     special->target->z, MT_BOOMEXPLODE);
          ...
          if (player->kartstuff[k_bumper] ==
              1) // If you have only one bumper left, and see if it's a 1v1
          {
            INT32 numingame = 0;

            for (i = 0; i < MAXPLAYERS; i++) {
              if (!playeringame[i] || players[i].spectator ||
                  players[i].kartstuff[k_bumper] <= 0)
                continue;
              numingame++;
            }

            if (numingame <= 2) // If so, then an extra karma point so they
                                // are 100% certain to switch places; it's
                                // annoying to end matches with a bomb kill
              ptadd++;
          }
      ...
      } else if (special->target->player->kartstuff[k_comebackmode] == 1 &&
        ...
        for (i = 0; i < 5; i++) {
          mobj_t *firework =
              P_SpawnMobj(special->x, special->y, special->z, MT_KARMAFIREWORK);
          ...
          P_Thrust(firework, FixedAngle((72 * i) << FRACBITS),
                   P_RandomRange(1, 8) * special->scale);
          P_SetObjectMomZ(firework, P_RandomRange(1, 8) * special->scale,
                          false);
      ...
      } else if (special->target->player->kartstuff[k_comebackmode] == 2 &&
                 P_CanPickupItem(player, 2)) {
        mobj_t *poof =
            P_SpawnMobj(special->x, special->y, special->z, MT_EXPLODE);
            ...
            1) // If you have only one bumper left, and see if it's a 1v1
        {
          ...
          if (numingame <= 2) // If so, then an extra karma point so they are
                              // 100% certain to switch places; it's annoying
                              // to end matches with a fake kill
            ptadd++;
        }
        ...
        if (special->target->player->kartstuff[k_eggmanblame] >= 0 &&
            ...
            !players[special->target->player->kartstuff[k_eggmanblame]]
                 .spectator)
          player->kartstuff[k_eggmanblame] =
              special->target->player->kartstuff[k_eggmanblame];
        else
    ...
    case MT_SPB:
      ...
      if (special->tracer && !P_MobjWasRemoved(special->tracer) &&
        ...
        spbexplode =
            P_SpawnMobj(toucher->x, toucher->y, toucher->z, MT_SPBEXPLOSION);
        spbexplode->extravalue1 =
            1; // Tell K_ExplodePlayer to use extra knockback
    ...
    case MT_CDUFO: // SRB2kart
      ...
      S_StartSound(
          toucher,
          sfx_cdfm73); // they don't make this sound in the original game but
                       // it's nice to have a "reward" for good play
    ...
    case MT_TUMBLECOIN: {
      ...
      if ((toucher->momx || toucher->momy) && (flip * special->momz <= 0)) {
        ...
        special->momz =
            flip * max(P_AproxDistance(toucher->momx, toucher->momy) / 4,
                       FixedMul(special->info->speed, special->scale));
    ...
    case MT_BLUEBALL:
      ...
      if (states[special->info->deathstate].tics > 0)
        special->scalespeed =
            FixedDiv(FixedDiv(special->destscale, special->scale),
                     states[special->info->deathstate].tics << FRACBITS);
      else
    ...
    case MT_RAILPICKUP:
      ...
      if (special->info->mass >= pw_infinityring &&
          special->info->mass <= pw_railring) {
        INT32 pindex = special->info->mass - (INT32)pw_infinityring;

        player->powers[special->info->mass] +=
            (UINT16)special->info->reactiontime;
    ...
    case MT_BLUEFLAG:
      ...
      //				return;
      {
        ...
        snprintf(plname, sizeof(plname), "%s%s%s", CTFTEAMCODE(player),
                 player_names[player - players], CTFTEAMENDCODE(player));

        if (player->ctfteam ==
            flagteam) // Player is on the same team as the flag
        {
        ...
        } else if (player->ctfteam) // Player is on the other team (and not a
                                    // spectator)
        {
          UINT16 flagflag =
              (special->type == MT_REDFLAG) ? GF_REDFLAG : GF_BLUEFLAG;
          mobj_t **flagmobj =
              (special->type == MT_REDFLAG) ? &redflag : &blueflag;
    ...
    /*case MT_NIGHTSDRONE:
            ...
            if (player->bonustime)
            {
                    if (G_IsSpecialStage(gamemap)) //After-mare bonus
    time/emerald reward in special stages.
                    {
                            // only allow the player with the emerald in-hand
    to leave. if (toucher->tracer && toucher->tracer->target
                            && toucher->tracer->target->type == MT_GOTEMERALD)
                            {
                            }
                            else // Make sure that SOMEONE has the emerald, at
    least!
                            {
                                    for (i = 0; i < MAXPLAYERS; i++)
                                            if (playeringame[i] &&
    players[i].playerstate == PST_LIVE
                                            && players[i].mo->tracer &&
    players[i].mo->tracer->target
                                            &&
    players[i].mo->tracer->target->type == MT_GOTEMERALD) return;
                                    // Well no one has an emerald, so exit
    anyway!
                            }
                            P_GiveEmerald(false);
            ...
            else //Initial transformation. Don't allow second chances in
    special stages!
            {
            ...
            if (!(netgame || multiplayer) && !(player->pflags &
    PF_NIGHTSMODE)) P_SetTarget(&special->tracer, toucher);
    ...
    case MT_NIGHTSLOOPHELPER:
            ...
            if (special->fuse < toucher->fuse - TICRATE)
            {
                    ...
                    if (special->target != toucher) // These ain't your
    helpers, pal! return;
    ...
    th->next)
                    {
                            if (th->function.acp1 !=
    (actionf_p1)P_MobjThinker) continue;
                    ...
                    y = (y/count)<<FRACBITS;
                    z = (z/count)<<FRACBITS;
                    gatherradius = P_AproxDistance(P_AproxDistance(special->x
    - x, special->y - y), special->z - z); P_RemoveMobj(special);
                    ...
                    if (gatherradius < 30*FRACUNIT) // Player is probably just
    sitting there. return;

                    for (d = 0; d < 16; d++)
                            P_SpawnParaloop(x, y, z, gatherradius, 16,
    MT_NIGHTSPARKLE, sparklestate, d*ANGLE_22h, false);

                    S_StartSound(toucher, sfx_prloop);

                    // Now we RE-scan all the thinkers to find close objects
    to pull
    ...
    (actionf_p1)P_MobjThinker) continue;
                            ...
                            if (P_AproxDistance(P_AproxDistance(mo2->x - x,
    mo2->y - y), mo2->z - z) > gatherradius) continue;
                            ...
                            {
                                    // Requires Bonus Time
                                    if ((mo2->flags2 & MF2_STRONGBOX) &&
    !player->bonustime) continue;

                                    if (!(mo2->flags & MF_SPECIAL) &&
    mo2->health)
                                    {
                                            P_SetMobjState(mo2,
    mo2->info->seestate); mo2->flags |= MF_SPECIAL; mo2->flags &=
    ~MF_NIGHTSITEM; S_StartSound(toucher, sfx_hidden); continue;
                                    }
                            }

                            if (!(mo2->type == MT_NIGHTSWING || mo2->type ==
    MT_RING || mo2->type == MT_COIN
    ...
    case MT_EGGCAPSULE:
            ...
            // make sure everything is as it should be, THEN take rings from
    players in special stages if (player->pflags & PF_NIGHTSMODE &&
    !toucher->target) return;

            if (player->mare != special->threshold) // wrong mare
                    return;

            if (special->reactiontime > 0) // capsule already has a player
    attacking it, ignore return;

            if (G_IsSpecialStage(gamemap) && !player->exiting)
            { // In special stages, share rings. Everyone gives up theirs to
    the player who touched the capsule for (i = 0; i < MAXPLAYERS; i++) if
    (playeringame[i] && (&players[i] != player) && players[i].mo->health > 1)
                            {
                                    toucher->health +=
    players[i].mo->health-1; player->health = toucher->health;
    ...
    case MT_NIGHTSBUMPER:
      ...
      if (player->bumpertime < TICRATE / 4) {
        ...
        } else // More like a spring
        {
          ...
          const fixed_t speed =
              FixedMul(FixedDiv(special->info->speed * FRACUNIT, 75 * FRACUNIT),
          ...
          if (special->threshold > 0)
            fa = (FixedAngle(((special->threshold * 30) - 1) * FRACUNIT) >>
                  ANGLETOFINESHIFT) &
                 FINEMASK;
          else
            fa = 0;
          ...
          P_SetPlayerMobjState(toucher,
                               S_KART_STND1); // SRB2kart - was S_PLAY_FALL1
        }
      }
      return;
    ...
    (UINT16)special->info->speed; else
            {
                    for (i = 0; i < MAXPLAYERS; i++)
                            if (playeringame[i] && players[i].pflags &
    PF_NIGHTSMODE) players[i].powers[pw_nights_superloop] =
    (UINT16)special->info->speed; if (special->info->deathsound != sfx_None)
    ...
    case MT_NIGHTSDRILLREFILL:
            ...
            else
            {
                    for (i = 0; i < MAXPLAYERS; i++)
                            if (playeringame[i] && players[i].pflags &
    PF_NIGHTSMODE) players[i].drillmeter = special->info->speed; if
    (special->info->deathsound != sfx_None) S_StartSound(NULL,
    ...
    case MT_NIGHTSHELPER:
            ...
            if (!G_IsSpecialStage(gamemap))
            {
                    ...
                    mobj_t *flickyobj = P_SpawnMobj(toucher->x, toucher->y,
    toucher->z + toucher->info->height, MT_NIGHTOPIANHELPER);
                    P_SetTarget(&flickyobj->target, toucher);

                    player->powers[pw_nights_helper] =
    (UINT16)special->info->speed;
            }
            else
            {
                    mobj_t *flickyobj;
    ...
    players[i].pflags & PF_NIGHTSMODE) { players[i].powers[pw_nights_helper] =
    (UINT16)special->info->speed; flickyobj = P_SpawnMobj(players[i].mo->x,
    players[i].mo->y, players[i].mo->z + players[i].mo->info->height,
    MT_NIGHTOPIANHELPER); P_SetTarget(&flickyobj->target, players[i].mo);
                            }
    ...
    G_IsSpecialStage(gamemap))
            {
                    HU_SetCEchoFlags(V_AUTOFADEOUT);
                    HU_SetCEchoDuration(4);
                    HU_DoCEcho(M_GetText("\\\\\\\\\\\\\\\\Nightopian
    Helper"));
            }
            break;
    ...
    PF_NIGHTSMODE)
                            {
                                    players[i].nightstime +=
    special->info->speed; players[i].startedtime += special->info->speed;
    ...
    (UINT16)special->info->speed; player->linktimer = 2*TICRATE;
            }
            else
            {
                    for (i = 0; i < MAXPLAYERS; i++)
    ...
    PF_NIGHTSMODE)
                            {
                                    players[i].powers[pw_nights_linkfreeze] +=
    (UINT16)special->info->speed; players[i].linktimer = 2*TICRATE;
                            }
                    if (special->info->deathsound != sfx_None)
                            S_StartSound(NULL, special->info->deathsound);
            }
    ...
    G_IsSpecialStage(gamemap))
            ...
            break;*/
    case MT_NIGHTSWING:
      if (G_IsSpecialStage(gamemap) && useNightsSS) { // Pseudo-ring.
    ...
    case MT_SHELL:
      if (special->state == &states[S_SHELL]) // Resting anim
      {
        ...
        P_InstaThrust(special, special->angle,
                      FixedMul(special->info->speed, special->scale));
    ...
    case MT_STARPOST:
      ...
      player->starposttime =
          player->realtime; // this makes race mode's timers work correctly
                            // whilst not affecting sp -x
      ...
      player->kartstuff[k_starpostflip] =
          special->spawnpoint->options & MTF_OBJECTFLIP; // store flipping
      player->grieftime = 0;
    ...
    case MT_FAKEMOBILE: {
      ...
      if (P_AproxDistance(toucher->x - special->x, toucher->y - special->y) >
          P_AproxDistance((toucher->x - toucher->momx) - special->x,
    ...
    case MT_BLACKEGGMAN_GOOPFIRE:
      if (!player->powers[pw_flashing]) // SRB2kart
      {
        ...
        if (special->target &&
            special->target->state == &states[S_BLACKEGG_SHOOT1]) {
          if (special->target->health <= 2 && P_RandomChance(FRACUNIT / 2))
            P_SetMobjState(special->target,
    ...
    case MT_EGGSHIELD: {
      ...
      if (!(angle > ANGLE_90 && angle < ANGLE_270)) {
        ...
        return;
                 ...
                 player->powers[pw_super]) // Do you possess the ability to
                                           // subdue the object?
      {
        // Shatter the shield!
    ...
    case MT_BIGMACECHAIN:
      ...
      if (special->target && (special->target->type == MT_SPINMACEPOINT ||
                              special->target->type == MT_HIDDEN_SLING)) {
        player->pflags |= PF_MACESPIN;
        S_StartSound(toucher, sfx_spin);
        P_SetPlayerMobjState(toucher,
                             S_KART_STND1); // SRB2kart - was S_PLAY_ATK1
      } else
        player->pflags |= PF_ITEMHANG;
    ...
    case MT_SPECIALSPIKEBALL:
      if (!(!useNightsSS &&
            G_IsSpecialStage(gamemap))) // Only for old special stages
      {
        P_DamageMobj(toucher, special, special, 1);
        return;
      }
    ...
    case MT_WATERDROP:
      if (special->state == &states[special->info->spawnstate]) {
        special->z = toucher->z + toucher->height -
                     FixedMul(8 * FRACUNIT, special->scale);
        special->momz = 0;
        special->flags |= MF_NOGRAVITY;
        P_SetMobjState(special, special->info->deathstate);
        S_StartSound(special, special->info->deathsound +
                                  (P_RandomKey(special->info->mass)));
      }
      return;
  ...
  if (!P_MobjWasRemoved(special)) {
    S_StartSound(
        toucher,
        special->info->deathsound); // was NULL, but changed to player so you
                                    // could hear others pick up rings
...
void P_CheckTimeLimit(void) {
  ...
  (players[i].pflags & PF_TAGIT)) continue;

                          CONS_Printf(M_GetText("%s received double points for
  surviving the round.\n"), player_names[i]); P_AddPlayerScore(&players[i],
  players[i].score);
                  }
          }
  }
      ...
      if (!G_GametypeHasTeams()) {
        ...
        for (i = 1; i < playercount; i++) {
          for (k = i; k < playercount; k++) {
            if (players[playerarray[i - 1]].marescore <
                players[playerarray[k]].marescore) {
        ...
        if (players[playerarray[0]].marescore ==
            players[playerarray[1]].marescore)
          return;
      } else {
...
void P_CheckPointLimit(void) {
  ...
  /*if (G_GametypeHasTeams())
  {
          ...
          if ((UINT32)cv_pointlimit.value <= redscore ||
  (UINT32)cv_pointlimit.value <= bluescore)
          {
                  if (server)
                          SendNetXCmd(XD_EXITLEVEL, NULL, 0);
          }
    ...
    for (i = 0; i < MAXPLAYERS; i++) {
      ...
      if ((UINT32)cv_pointlimit.value <= players[i].marescore) {
        for (i = 0; i < MAXPLAYERS;
             i++) // AAAAA nested loop using the same iteration variable ;;
        {
          if (!playeringame[i] || players[i].spectator)
            continue;
          if (players[i].exiting)
        ...
        /*if (server)
                SendNetXCmd(XD_EXITLEVEL, NULL, 0);*/
        return; // good thing we're leaving the function immediately instead of
                // letting the loop get mangled!
      }
    }
  }
}
...
/*void P_CheckSurvivors(void)
{
        ...
        for (i=0; i < MAXPLAYERS; i++) //figure out counts of taggers, survivors
and spectators.
        {
                if (playeringame[i])
                {
        ...
        if (!taggers) //If there are no taggers, pick a survivor at random to be
it.
        {
                // Exception for hide and seek. If a round has started and the
IT player leaves, end the round. if (gametype == GT_HIDEANDSEEK && (leveltime >=
(hidetime * TICRATE)))
                {
                        CONS_Printf(M_GetText("The IT player has left the
game.\n")); if (server) SendNetXCmd(XD_EXITLEVEL, NULL, 0);
...
player_names[newtagger]); // Tell everyone who is it! players[newtagger].pflags
|= PF_TAGIT;

                        survivors--; //Get rid of the guy we just made IT.

                        //Yeah, we have an eligible tagger, but we may not have
anybody for him to tag!
                        //If there is only one guy waiting on the game to fill
or spectators to enter game, don't bother. if (!survivors && (D_NumPlayers() -
spectators) > 1)
                        {
                                CONS_Printf(M_GetText("All players have been
tagged!\n")); if (server) SendNetXCmd(XD_EXITLEVEL, NULL, 0);
                        }
                ...
                //If we reach this point, no player can replace the one that was
IT.
                //Unless it is one player waiting on a game, end the round.
                if ((D_NumPlayers() - spectators) > 1)
                {
                        CONS_Printf(M_GetText("There are no players able to
become IT.\n")); if (server) SendNetXCmd(XD_EXITLEVEL, NULL, 0);
                }
        ...
        //Except when the tagger is by himself and the rest of the game are
spectators. if (!survivors && (D_NumPlayers() - spectators) > 1)
        {
...
boolean P_CheckRacers(void) {
  ...
  if (cv_karteliminatelast.value) {
    ...
    if (numplayersingame > 1 && nospectategrief > 0 &&
        numplayersingame >= nospectategrief) // prevent spectate griefing
    {
      // check if we just got unlucky and there was only one guy who was a
      // problem
      for (j = i + 1; j < MAXPLAYERS; j++) {
        if (!playeringame[j] || players[j].spectator || players[j].exiting ||
            !players[j].lives)
          continue;
  ...
  if (!racecountdown) // Check to see if the winners have finished, to set
                      // countdown.
  {
    ...
    if (numexiting >= winningpos)
      racecountdown =
          (((netgame || multiplayer) ? cv_countdowntime.value : 30) * TICRATE) +
          1; // 30 seconds to finish, get going!
  }
...
void P_KillMobj(mobj_t *target, mobj_t *inflictor, mobj_t *source) {
  ...
  // if (inflictor && (inflictor->type == MT_SHELL || inflictor->type ==
  // MT_FIREBALL)) 	P_SetTarget(&target->tracer, inflictor);

  if (!useNightsSS && G_IsSpecialStage(gamemap) && target->player &&
      sstimer > 6)
  ...
  if (target->type != MT_PLAYER && !(target->flags & MF_MONITOR) &&
      !(target->type == MT_ORBINAUT || target->type == MT_ORBINAUT_SHIELD ||
        target->type == MT_JAWZ || target->type == MT_JAWZ_DUD ||
        target->type == MT_JAWZ_SHIELD || target->type == MT_BANANA ||
        target->type == MT_BANANA_SHIELD || target->type == MT_EGGMANITEM ||
        target->type == MT_EGGMANITEM_SHIELD || target->type == MT_BALLHOG ||
        target->type == MT_SPB))   // kart dead items
    target->flags |= MF_NOGRAVITY; // Don't drop Tails 03-08-2000
  else
    target->flags &= ~MF_NOGRAVITY; // lose it if you for whatever reason have
                                    // it, I'm looking at you shields
  //
  ...
  target->health =
      0; // This makes it easy to check if something's dead elsewhere.
  ...
  if (target->target && target->target->player && target->target->player->mo) {
    if (target->target->player->kartstuff[k_eggmanheld] &&
        target->type == MT_EGGMANITEM_SHIELD)
      target->target->player->kartstuff[k_eggmanheld] = 0;

    if (target->target->player->kartstuff[k_itemheld]) {
      if ((target->type == MT_BANANA_SHIELD &&
           target->target->player->kartstuff[k_itemtype] ==
               KITEM_BANANA) // trail items
          || (target->type == MT_SSMINE_SHIELD &&
              target->target->player->kartstuff[k_itemtype] == KITEM_MINE) ||
          (target->type == MT_SINK_SHIELD &&
           target->target->player->kartstuff[k_itemtype] ==
               KITEM_KITCHENSINK)) {
        if (target->movedir != 0 &&
            target->movedir <
                (UINT16)target->target->player->kartstuff[k_itemamount]) {
      ...
      } else if ((target->type == MT_ORBINAUT_SHIELD &&
                  target->target->player->kartstuff[k_itemtype] ==
                      KITEM_ORBINAUT) // orbit items
                 || (target->type == MT_JAWZ_SHIELD &&
                     target->target->player->kartstuff[k_itemtype] ==
                         KITEM_JAWZ)) {
        target->target->player->kartstuff[k_itemamount]--;
  ...
  // Above block does not clean up rocket sneakers when a player dies, so we
  // need to do it here target->target is null when using rocket sneakers
  if (target->player)
    K_DropRocketSneaker(target->player);

  // Let EVERYONE know what happened to a player! 01-29-2002 Tails
  if (target->player && !target->player->spectator) {
    if (metalrecording) // Ack! Metal Sonic shouldn't die! Cut the tape, end
                        // recording!
      G_StopMetalRecording();
    /*if (gametype == GT_MATCH && cv_match_scoring.value == 0 // note, no team
    match suicide penalty
            && ((target == source) || (source == NULL && inflictor == NULL) ||
    (source && !source->player))) { // Suicide penalty - Not in Kart if
    (target->player->score >= 50) target->player->score -= 50; else
  ...
  if (source && source->player) {
    if (target->flags & MF_MONITOR || target->type == MT_RANDOMITEM) {
      ...
      if (cv_itemrespawn.value && (netgame || multiplayer)) {
        target->fuse =
            cv_itemrespawntime.value * TICRATE + 2; // Random box generation
      }
    }
    ...
    /*{ // Enemies shouldn't award points in Kart
            ...
            if (maptol & TOL_NIGHTS) // Enemies always worth 200, bosses don't
    do anything.
            {
                    if ((target->flags & MF_ENEMY) && !(target->flags &
    (MF_MISSILE|MF_BOSS)))
                    {
                            score = 200;
                            ...
                            // Sonic Time Attacked did and it seems like a good
    enough incentive
                            // to make people want to actually dash
    towards/paraloop enemies if (++source->player->linkcount >
    source->player->maxlink) source->player->maxlink =
    source->player->linkcount; source->player->linktimer = 2*TICRATE;
                    }
            }
            else
            {
                    if (target->flags & MF_BOSS)
                            score = 1000;
                    else if ((target->flags & MF_ENEMY) && !(target->flags &
    MF_MISSILE))
                    {
                            mobj_t *scoremobj;
                            UINT32 scorestate = mobjinfo[MT_SCORE].spawnstate;

                            scoremobj = P_SpawnMobj(target->x, target->y,
    target->z + (target->height / 2), MT_SCORE);

                            // On ground? No chain starts.
                            if (!source->player->powers[pw_invulnerability] &&
    P_IsObjectOnGround(source))
                            {
                                    source->player->scoreadd = 0;
                                    score = 100;
                            }
                            // Mario Mode has Mario-like chain point values
                            else if (mariomode) switch
    (++source->player->scoreadd)
                            {
                                    case 1: score = 100;  break;
                                    case 2: score = 200;  scorestate += 1;
    break; case 3: score = 400;  scorestate += 5; break; case 4: score = 800;
    scorestate += 6; break; case 5: score = 1000; scorestate += 3; break; case
    6: score = 2000; scorestate += 7; break; case 7: score = 4000; scorestate +=
    8; break; case 8: score = 8000; scorestate += 9; break; default: // 1up for
    a chain this long if (modeattacking) // but 1ups don't exist in record
    attack! { // So we just go back to 10k points. score = 10000; scorestate +=
    4; break;
                                            }
                                            P_GivePlayerLives(source->player,
    1); P_PlayLivesJingle(source->player); scorestate += 10; break;
                            }
                            ...
                            else switch (++source->player->scoreadd)
                            {
                                    case 1:  score = 100;   break;
                                    case 2:  score = 200;   scorestate += 1;
    break; case 3:  score = 500;   scorestate += 2; break; case 4: case 5: case
    6: case 7: case 8: case 9: case 10: case 11: case 12: case 13: case 14:
                                             score = 1000;  scorestate += 3;
    break; default: score = 10000; scorestate += 4; break;
                            }
  ...
  if (target->player) {
    ...
    if (!target->player->bot && !G_IsSpecialStage(gamemap) &&
      ...
      if (target->player->lives <= 0) // Tails 03-14-2000
      {
        if (P_IsLocalPlayer(
              target
                ->player) /* && target->player == &players[consoleplayer] */) {
          S_StopMusic(); // Stop the Music! Tails 03-14-2000
          S_ChangeMusicInternal(
              "gmover", false); // Yousa dead now, Okieday? Tails 03-14-2000
        }
      }
    }
    ...
    /*if (G_TagGametype()
     && !(target->player->pflags & PF_TAGIT) && (!source || !source->player) &&
    !(target->player->spectator))
    {
            // if you accidentally die before you run out of time to hide,
    ignore it.
    ...
    player_names[target->player-players]); // Tell everyone who is it!
                    ...
                    else
                    {
                            if (!(target->player->pflags & PF_TAGGED))
                            {
                                    ...
                                    //in hide and seek, suiciding players are
    counted as found. INT32 w;

                                    for (w=0; w < MAXPLAYERS; w++)
                                    {
                                            if (players[w].pflags & PF_TAGIT)
                                                    P_AddPlayerScore(&players[w],
    1);
                                    }

                                    target->player->pflags |= PF_TAGGED;
                                    CONS_Printf(M_GetText("%s was found!\n"),
    player_names[target->player-players]); P_CheckSurvivors();
                            }
                    }
            }
    }
  ...
  if (!mariomode // Don't show birds, etc. in Mario Mode Tails 12-23-2001
    ...
    if (cv_soniccd.value)
      ...
      default:
        if (target->info->doomednum)
          prandom =
              target->info->doomednum % 5; // "Random" animal for new enemies.
        else
          prandom =
              P_RandomKey(5); // No placable object, just use a random number.
    ...
    mo = P_SpawnMobj(target->x, target->y,
                     target->z + (target->height / 2) -
                         FixedMul(mobjinfo[item].height / 2, target->scale),
                     item);
  ...
  // Other death animation effects
    ...
    case MT_PLAYER:
      ...
      target->fuse = TICRATE * 3; // timer before mobj disappears from view
                                  // (even if not an actual player)
      if (!(source && source->type == MT_NULL &&
            source->threshold == 42)) // Don't jump up when drowning
        P_SetObjectMomZ(target, 14 * FRACUNIT, false);

      if (source && source->type == MT_NULL &&
          source->threshold == 42) // drowned
        S_StartSound(target, sfx_drown);
      else if (source &&
               (source->type == MT_SPIKE ||
                (source->type == MT_NULL && source->threshold == 43))) // Spikes
  ...
  if (target->type ==
      MT_AQUABUZZ) // Additionally spawns breathable bubble for players to get
  {
    if (inflictor && inflictor->player // did a player kill you? Spawn relative
                                       // to the player so he's bound to get it
        &&
        P_AproxDistance(inflictor->x - target->x, inflictor->y - target->y) <=
            inflictor->radius + target->radius +
                FixedMul(8 * FRACUNIT, inflictor->scale) // close enough?
        && inflictor->z <= target->z + target->height +
                               FixedMul(8 * FRACUNIT, inflictor->scale) &&
        inflictor->z + inflictor->height >=
            target->z - FixedMul(8 * FRACUNIT, inflictor->scale))
      mo = P_SpawnMobj(inflictor->x + inflictor->momx,
                       inflictor->y + inflictor->momy,
                       inflictor->z + (inflictor->height / 2) + inflictor->momz,
                       MT_EXTRALARGEBUBBLE);
    else
    ...
    P_SetScale(mo, mo->destscale);
             ...
             MT_YELLOWSHELL) // Spawns a spring that falls to the ground
  {
  ...
  if (target->type == MT_EGGMOBILE3) {
    ...
    // scan the thinkers to make sure all the old pinch dummies are gone on
    // death this can happen if the boss was hurt earlier than expected
    for (th = thinkercap.next; th != &thinkercap; th = th->next) {
      if (th->function.acp1 != (actionf_p1)P_MobjThinker)
        continue;
  ...
  if ((target->type == MT_JAWZ || target->type == MT_JAWZ_DUD ||
       target->type == MT_JAWZ_SHIELD) &&
      !(target->flags2 & MF2_AMBUSH)) {
  ...
  if (target->type == MT_FROGGER || target->type == MT_ROBRA_HEAD ||
      target->type == MT_BLUEROBRA_HEAD) // clean hnext list
  {
    mobj_t *cur = target->hnext;
    while (cur && !P_MobjWasRemoved(cur)) {
  ...
  // Bounce up on death
  if (target->type == MT_SMK_PIPE || target->type == MT_SMK_MOLE ||
      target->type == MT_SMK_THWOMP) {
  ...
  if (target->type == MT_SPIKE && inflictor &&
      target->info->deathstate != S_NULL) {
    const fixed_t x = target->x, y = target->y, z = target->z;
    const fixed_t scale = target->scale;
    const boolean flip =
        (target->eflags & MFE_VERTICALFLIP) == MFE_VERTICALFLIP;
    S_StartSound(target, target->info->deathsound);
    ...
    target->x += P_ReturnThrustX(target, target->angle,
                                 FixedMul(8 * FRACUNIT, target->scale));
    target->y += P_ReturnThrustY(target, target->angle,
                                 FixedMul(8 * FRACUNIT, target->scale));
    if (flip)
    ...
    if (flip) {
      target = P_SpawnMobj(x, y, z - FixedMul(12 * FRACUNIT, target->scale),
                           MT_SPIKE);
    ...
    if (target->info->xdeathstate != S_NULL) {
      ...
      target->y += P_ReturnThrustY(target, target->angle,
                                   FixedMul(8 * FRACUNIT, target->scale));
      P_SetThingPosition(target);
      P_InstaThrust(target, target->angle,
                    FixedMul(4 * FRACUNIT, target->scale));
  ...
  /** \note For player, the above is redundant because of P_SetMobjState
     (target, S_PLAY_DIE1) in P_DamageMobj() Graue 12-22-2003 */
}
...
static inline void P_NiGHTSDamage(mobj_t *target, mobj_t *source) {
  ...
  if (!player->powers[pw_flashing] && !(player->pflags & PF_GODMODE)) {
    angle_t fa;
    ...
    if (oldnightstime > 10 * TICRATE && player->nightstime < 10 * TICRATE) {
      // S_StartSound(NULL, sfx_timeup); // that creepy "out of time" music from
      // NiGHTS. Dummied out, as some on the dev team thought it wasn't Sonic-y
      // enough (Mystic, notably). Uncomment to restore. -SH
      S_ChangeMusicInternal("drown", false);
    }
  }
}

static inline boolean P_TagDamage(mobj_t *target, mobj_t *inflictor,
                                  mobj_t *source, INT32 damage) {
  player_t *player = target->player;
  ...
  if ((player->pflags & PF_TAGIT &&
       !(cv_friendlyfire.value && source && source->player &&
  ...
  // The tag occurs so long as you aren't shooting another tagger with
  // friendlyfire on.
  /*if (source->player->pflags & PF_TAGIT && !(player->pflags & PF_TAGIT))
  {
          ...
          {
                  player->pflags |= PF_TAGIT; //in survivor, the player becomes
  IT and helps hunt down the survivors. CONS_Printf(M_GetText("%s is now
  IT!\n"), player_names[player-players]); // Tell everyone who is it!
          }
          else
          {
                  player->pflags |= PF_TAGGED; //in hide and seek, the player is
  tagged and stays stationary. CONS_Printf(M_GetText("%s was found!\n"),
  player_names[player-players]); // Tell everyone who is it!
          }
...
static inline boolean P_PlayerHitsPlayer(mobj_t *target, mobj_t *inflictor,
  ...
  // You can't kill yourself, idiot... // Unless it's Mario kart. Which it is.
  // In this mod. All the time.
  // if (source == target)
  //	return false;

  // In COOP/RACE/CHAOS, you can't hurt other players unless cv_friendlyfire is
  // on
  ...
  // Add pity.
  if (!player->powers[pw_flashing] && !player->powers[pw_invulnerability] &&
      !player->powers[pw_super] && source->player->score > player->score)
    player->pity++;
...
static void P_KillPlayer(player_t *player, mobj_t *source, INT32 damage) {
  player->pflags &= ~(PF_CARRIED | PF_SLIDING | PF_ITEMHANG | PF_MACESPIN |
                      PF_ROPEHANG | PF_NIGHTSMODE);
  ...
  /*if (gametype == GT_CTF && (player->gotflag & (GF_REDFLAG|GF_BLUEFLAG)))
  {
          ...
          if (source && source->player)
          {
                  // Award no points when players shoot each other when
  cv_friendlyfire is on. if (!G_GametypeHasTeams() || !(source->player->ctfteam
  == player->ctfteam && source != player->mo)) P_AddPlayerScore(source->player,
  1);
          }
  }
  if (source && source->player && !player->powers[pw_super]) //don't score
  points against super players
  {
          // Award no points when players shoot each other when cv_friendlyfire
  is on. if (!G_GametypeHasTeams() || !(source->player->ctfteam ==
  player->ctfteam && source != player->mo)) P_AddPlayerScore(source->player, 1);
  }
  ...
  if (gametype != GT_COOP && player->powers[pw_super])
  {
          ...
          HU_DoCEcho(va("%s\\is no longer super.\\\\\\\\",
  ...
  if (player->pflags & PF_TIMEOVER) {
    ...
    boom =
        P_SpawnMobj(player->mo->x, player->mo->y, player->mo->z, MT_FZEROBOOM);
  ...
  if (player->powers[pw_shield] & SH_FORCE) { // Multi-hit
    ...
    if (!player->powers[pw_super]) {
      ...
      G_GhostAddColor((INT32)(player - players), GHC_NORMAL);
             ...
             SH_BOMB) // Give them what's coming to them!
  {
...
/*
static void P_ShieldDamage(player_t *player, mobj_t *inflictor, mobj_t *source,
INT32 damage) // SRB2kart - unused.
{
        ...
        if (source && (source->type == MT_SPIKE || (source->type == MT_NULL &&
source->threshold == 43))) // spikes S_StartSound(player->mo, sfx_spkdth); else
        ...
        if (gametype == GT_CTF && (player->gotflag & (GF_REDFLAG|GF_BLUEFLAG)))
        {
                ...
                if (source && source->player)
                {
                        // Award no points when players shoot each other when
cv_friendlyfire is on. if (!G_GametypeHasTeams() || !(source->player->ctfteam ==
player->ctfteam && source != player->mo)) P_AddPlayerScore(source->player, 25);
                }
        }
        if (source && source->player && !player->powers[pw_super]) //don't score
points against super players
...
cv_friendlyfire is on. if (!G_GametypeHasTeams() || !(source->player->ctfteam ==
player->ctfteam && source != player->mo)) P_AddPlayerScore(source->player,
cv_match_scoring.value == 1 ? 25 : 50);
        }
}
*/
...
static void P_RingDamage(player_t *player, mobj_t *inflictor, mobj_t *source,
                         INT32 damage) {
  // const UINT8 scoremultiply = ((K_IsWantedPlayer(player) && !trapitem) : 2 ?
  // 1);
...
boolean P_DamageMobj(mobj_t *target, mobj_t *inflictor, mobj_t *source,
  ...
  if (!force) {
    ...
    if (target->flags & MF_MONITOR &&
        ((!source || !source->player || source->player->bot) ||
  ...
  if (target->type == MT_CRAWLACOMMANDER) {
    ...
    P_InstaThrust(target, target->angle - ANGLE_180,
                  FixedMul(5 * FRACUNIT, target->scale));
  } else if (target->flags & MF_BOSS) {
    if (!force &&
        target->flags2 & MF2_FRET) // Currently flashing from being hit
      return false;
  ...
  if (player) // Player is the target
  {
    if (!force) {
      ...
      if (!(target->player->pflags & (PF_NIGHTSMODE | PF_NIGHTSFALL)) &&
          (maptol & TOL_NIGHTS))
        return false;
    }
      ...
      if (!force) {
        ...
        if (source && source->player && !cv_friendlyfire.value &&
            (gametype == GT_COOP ||
             (G_GametypeHasTeams() &&
              target->player->ctfteam == source->player->ctfteam)))
          return false; // Don't run eachother over in special stages and team
                        // games and such
      }
      if (LUAh_MobjDamage(target, inflictor, source, damage))
        return true;
    ...
    if (damage == 10000)
      ...
      if (inflictor &&
          (inflictor->type == MT_ORBINAUT ||
           inflictor->type == MT_ORBINAUT_SHIELD ||
           inflictor->type == MT_JAWZ || inflictor->type == MT_JAWZ_SHIELD ||
           inflictor->type == MT_JAWZ_DUD || inflictor->type == MT_SMK_THWOMP ||
           inflictor->player)) {
...
// ignore bouncing & such in invulnerability
            || (player->powers[pw_super] &&
!(ALL7EMERALDS(player->powers[pw_emeralds]) && inflictor && ((inflictor->flags &
MF_MISSILE) || inflictor->player))))
    {
...
player->bot))  //If One-Hit Shield
    ...
    else // No shield, no rings, no invincibility.
    {
            ...
            if (force || !(G_GametypeHasTeams()
                    && source && source->player && (source->player->ctfteam ==
player->ctfteam)
    ...
    */
    ...
    if (inflictor && ((inflictor->flags & MF_MISSILE) || inflictor->player) &&
      ...
      if (player->powers[pw_shield]) {
        ...
        player->health -=
            (10 * (1 << (INT32)(player->powers[pw_super] / 10500)));
        if (player->health < 2)
          player->health = 2;
      }
  ...
  if (cv_killingdead.value && (source && source->player) &&
      (inflictor && inflictor->player) && P_RandomChance(5 * FRACUNIT / 16))
    P_DamageMobj(source, target, target, 1);

  // do the damage
  if (player && player->powers[pw_super] &&
      ALL7EMERALDS(player->powers[pw_emeralds]) && inflictor &&
      ((inflictor->flags & MF_MISSILE) || inflictor->player)) {
    ...
    case MT_EGGMOBILE2:                          // egg slimer
      if (target->health < target->info->damage) // in pinch phase
      {
        P_SetMobjState(target,
                       target->info->meleestate); // go to pinch pain state
        break;
      }
      /* FALLTHRU */
    default:
...
void P_PlayerRingBurst(player_t *player, INT32 num_rings) {
  ...
  // There's no ring spilling in kart, so I'm hijacking this for the same thing
  // as TD
  for (i = 0; i < num_rings; i++) {
    ...
    fa = ((i * FINEANGLES / 16) + (player->mo->angle >> ANGLETOFINESHIFT) -
          ((num_rings - 1) * FINEANGLES / 32)) &
         FINEMASK;

    // Make rings spill out around the player in 16 directions like SA, but
    // spill like Sonic 2. Technically a non-SA way of spilling rings. They just
    // so happen to be a little similar.
...
void P_PlayerFlagBurst(player_t *player, boolean toss) {
  ...
  if (toss)
    P_InstaThrust(flag, player->mo->angle,
                  FixedMul(6 * FRACUNIT, player->mo->scale));
  else {
    angle_t fa = P_RandomByte() * FINEANGLES / 256;
    flag->momx =
        FixedMul(FINECOSINE(fa), FixedMul(6 * FRACUNIT, player->mo->scale));
    if (!(twodlevel || (player->mo->flags2 & MF2_TWOD)))
      flag->momy =
          FixedMul(FINESINE(fa), FixedMul(6 * FRACUNIT, player->mo->scale));
  }
