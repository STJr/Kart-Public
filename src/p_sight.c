...
static boolean P_CrossSubsecPolyObj(polyobj_t *po, register los_t *los) {
  ...
  for (i = 0; i < po->numLines; ++i) {
    ...
    // TODO: figure out if it's worth considering partially blocked cases or
    // not? maybe to adjust los's top/bottom slopes if needed
...
static boolean P_CrossSubsector(size_t num, register los_t *los) {
  ...
  for (count = subsectors[num].numlines; --count >= 0; seg++) // check lines
  {
    ...
    // calculate fractional intercept (how far along we are divided by how far
    // we are from t2)
    frac = P_InterceptVector2(&los->strace, &divl);
...
static boolean P_CrossBSPNode(INT32 bspnum, register los_t *los) {
  while (!(bspnum & NF_SUBSECTOR)) {
    ...
    INT32 side =
        P_DivlineSide(los->strace.x, los->strace.y, (divline_t *)bsp) & 1;
...
boolean P_CheckSight(mobj_t *t1, mobj_t *t2) {
  ...
  if (!t1->subsector || !t2->subsector || !t1->subsector->sector ||
      !t2->subsector->sector)
    return false;
  ...
  if (!t1->subsector->polyList && t1->subsector == t2->subsector)
    return true;
