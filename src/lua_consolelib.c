...
void Got_Luacmd(UINT8 **cp, INT32 playernum) {
  ...
  LUA_Call(gL,
           (int)argc); // argc is 1-based, so this will cover the player we
                       // passed too.
  return;

deny:
  // must be hacked/buggy client
  if (gL) // check if Lua is actually turned on first, you dummmy -- Monster
          // Iestyn 04/07/18
...
void COM_Lua_f(void) {
  ...
  if (flags &
      COM_SPLITSCREEN) // flag 2: splitscreen player command. TODO: support 4P
  {
    if (!splitscreen) {
  ...
  {                                    // Send the command through the network
    ...
    if (flags & COM_ADMIN && !server &&
        !IsPlayerAdmin(
            playernum)) // flag 1: only server/admin can use this command.
    {
      CONS_Printf(
  ...
  // Do the command locally, NetXCmds don't go through outside of GS_LEVEL ||
  // GS_INTERMISSION || GS_VOTING
  lua_rawgeti(gL, -1, 1); // push function from command info table
  ...
  LUA_Call(gL, (int)COM_Argc()); // COM_Argc is 1-based, so this will cover the
                                 // player we passed too.
}
...
static int lib_comAddCommand(lua_State *L) {
  ...
  NOHUD
  if (lua_gettop(L) >=
      3) { // For the third argument, only take a boolean or a number.
    lua_settop(L, 3);
    if (lua_type(L, 3) == LUA_TBOOLEAN) {
      CONS_Alert(CONS_WARNING, "Using a boolean for admin commands is "
                               "deprecated and will be removed.\n"
                               "Use \"COM_ADMIN\" instead.\n");
    } else
      luaL_checktype(L, 3, LUA_TNUMBER);
  } else { // No third argument? Default to 0.
    lua_settop(L, 2);
    lua_pushinteger(L, 0);
  }
  ...
  if (com_return <
      0) { // failed to add -- free the lowercased name and return error
    Z_Free(name);
    return luaL_error(L, "Couldn't add a new console command \"%s\"", luaname);
  } else if (com_return == 1) { // command existed already -- free our name as
                                // the old string will continue to be used
    CONS_Printf("Replaced command \"%s\"\n", name);
    Z_Free(name);
  } else { // new command was added -- do NOT free the string as it will forever
...
static int lib_comBufAddText(lua_State *L) {
  ...
  if (n < 2)
    return luaL_error(
        L, "COM_BufAddText requires two arguments: player and text.");
  NOHUD
  lua_settop(L, 2);
...
static int lib_comBufInsertText(lua_State *L) {
  ...
  if (n < 2)
    return luaL_error(
        L, "COM_BufInsertText requires two arguments: player and text.");
  NOHUD
  lua_settop(L, 2);
  plr = *((player_t **)luaL_checkudata(L, 1, META_PLAYER));
  if (!plr)
...
void LUA_CVarChanged(const char *name) { cvname = name; }
...
static int lib_cvRegisterVar(lua_State *L) {
  ...
  lua_settop(
      L,
      1); // Clear out all other possible arguments, leaving only the first one.
  NOHUD
  cvar = lua_newuserdata(L, sizeof(consvar_t));
  luaL_getmetatable(L, META_CVAR);
  lua_setmetatable(L, -2);

#define FIELDERROR(f, e)                                                       \
  luaL_error(L,                                                                \
             "bad value for " LUA_QL(f) " in table passed to " LUA_QL(         \
                 "CV_RegisterVar") " (%s)",                                    \
             e);
#define TYPEERROR(f, t)                                                        \
  FIELDERROR(                                                                  \
      f, va("%s expected, got %s", lua_typename(L, t), luaL_typename(L, -1)))
  ...
  while (lua_next(L, 1)) {
    ...
    if (i == 1 || (k && fasticmp(k, "name"))) {
      ...
      if (lua_islightuserdata(L, 4)) {
        ...
        if (pv == CV_OnOff || pv == CV_YesNo || pv == CV_Unsigned ||
            pv == CV_Natural)
          cvar->PossibleValue = pv;
        else
          FIELDERROR("PossibleValue",
                     "CV_PossibleValue_t expected, got unrecognised pointer")
      } else if (lua_istable(L, 4)) {
        // Accepts tables in the form of {MIN=0, MAX=9999} or {Red=0, Green=1,
        // Blue=2} and converts them to CV_PossibleValue_t
        // {{0,"MIN"},{9999,"MAX"}} or {{0,"Red"},{1,"Green"},{2,"Blue"}}
        //
        // I don't really like the way this does it because a single
        // PossibleValue table being used for multiple cvars will be converted
        // and stored multiple times. So maybe instead it should be a seperate
        // function which must be run beforehand or something.
        size_t count = 0;
        ...
        while (lua_next(L, 4)) {
          ...
          //                       4             5      6
          if (lua_type(L, 5) != LUA_TSTRING || lua_type(L, 6) != LUA_TNUMBER)
            FIELDERROR("PossibleValue",
                       "custom PossibleValue table requires a format of "
                       "string=integer, i.e. {MIN=0, MAX=9999}");
        ...
        FIELDERROR("PossibleValue",
                   va("%s or CV_PossibleValue_t expected, got %s",
                      lua_typename(L, LUA_TTABLE), luaL_typename(L, -1)))
    } else if (cvar->flags & CV_CALL &&
...
#undef TYPEERROR
  ...
  if (cvar->flags & CV_MODIFIED)
    return luaL_error(L, "failed to register cvar (probable conflict with "
                         "internal variable/command names)");
...
static int CVarSetFunction(lua_State *L, void (*Set)(consvar_t *, const char *),
...
static int lib_consPrintf(lua_State *L) {
  ...
  if (n < 2)
    return luaL_error(
        L, "CONS_Printf requires at least two arguments: player and text.");
  // HUDSAFE
  plr = *((player_t **)luaL_checkudata(L, 1, META_PLAYER));
  if (!plr)
    return LUA_ErrInvalid(L, "player_t");
  ...
  for (i = 2; i <= n; i++) {
    ...
    if (s == NULL)
      return luaL_error(
          L,
          LUA_QL("tostring") " must return a string to " LUA_QL("CONS_Printf"));
    if (i > 2)
      CONS_Printf("\n");
    CONS_Printf("%s", s);
...
static luaL_Reg lib[] = {{"COM_AddCommand", lib_comAddCommand},
...
static int cvar_get(lua_State *L) {
  ...
  if (fastcmp(field, "name"))
    ...
    return luaL_error(L, LUA_QL("consvar_t") " has no field named " LUA_QS,
                      field);
  else
    return 0;
  return 1;
