...
void P_MixUp(mobj_t *thing, fixed_t x, fixed_t y, fixed_t z, angle_t angle,
  ...
  if (thing->player) {
    if (thing->eflags & MFE_VERTICALFLIP)
      thing->player->viewz =
          thing->z + thing->height - thing->player->viewheight;
    else
...
boolean P_Teleport(mobj_t *thing, fixed_t x, fixed_t y, fixed_t z,
  ...
  if (!dontstopmove)
    ...
    P_InstaThrust(thing, thing->angle,
                  P_AproxDistance(thing->momx, thing->momy));
  ...
  if (thing->player) {
    ...
    if (!dontstopmove) {
      INT32 p;
      // Search for any players you might be carrying, so you can get them off
      // before they end up being taken with you!
      for (p = 0; p < MAXPLAYERS; p++)
        if (playeringame[p] && players[p].mo &&
            players[p].pflags & PF_CARRIED && players[p].mo->tracer == thing) {
