...
static struct color_hash *create_colorhash(int maxnum) {
  ...
  hash->entries =
      (struct hash_entry *)SDL_malloc(maxnum * sizeof(struct hash_entry));
  if (!hash->entries) {
    SDL_free(hash->table);
    SDL_free(hash);
    return NULL;
...
static int add_colorhash(struct color_hash *hash, char *key, int cpp,
                         Uint32 color) {
...
static int color_to_rgb(const char *spec, int speclen, Uint32 *rgb) {
  ...
  static struct {
    const char *name;
    Uint32 rgb;
  } known[] = {{"none", 0xFFFFFFFF},
               {"black", 0x000000},
               {"white", 0xFFFFFF},
               {"red", 0xFF0000},
               {"green", 0x00FF00},
               {"blue", 0x0000FF},
/* This table increases the size of the library by 40K, so it's disabled by
 * default */
...
static SDL_Surface *load_xpm(const char **xpm, SDL_RWops *src) {
   ...
   * one day.
   */
  if (SDL_sscanf(line, "%d %d %d %d", &w, &h, &ncolors, &cpp) != 4 || w <= 0 ||
      h <= 0 || ncolors <= 0 || cpp <= 0) {
  ...
  if (ncolors <= 256) {
    ...
    indexed = 0;
    image = SDL_CreateRGBSurface(SDL_SWSURFACE, w, h, 32, 0xff0000, 0x00ff00,
                                 0x0000ff, 0);
  }
  if (!image) {
...
/* See if an image is contained in a data source */
int IMG_isXPM(SDL_RWops *src) { return (0); }

/* Load a XPM type image from an SDL datasource */
SDL_Surface *IMG_LoadXPM_RW(SDL_RWops *src) { return (NULL); }
