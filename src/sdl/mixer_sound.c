...
static void var_cleanup(void) {
  loop_point = song_length = music_bytes = fading_source = fading_target =
...
static Mix_Chunk *ds2chunk(void *stream) {
  ...
  default:
    ...
    else // strange and unusual fractional frequency steps, plus anything
         // higher than 44100hz.
      newsamples = FixedMul(FixedDiv(samples, freq), 44100) +
                   1; // add 1 to counter truncation.
    if (newsamples >= UINT32_MAX >> 2)
      return NULL; // would and/or did wrap, can't store.
    break;
  }
  sound =
      Z_Malloc(newsamples << 2, PU_SOUND,
               NULL); // samples * frequency shift * bytes per sample * channels
  ...
  case 44100: // already at the same rate? well that makes it simple.
    while (i++ < samples) {
      o = ((INT16)(*s++) + 0x80)
          << 8; // changed signedness and shift up to 16 bits
      *d++ = o; // left channel
      *d++ = o; // right channel
    }
    break;
  ...
  default: // convert arbitrary hz to 44100.
    step = 0;
    frac =
        ((UINT32)freq << FRACBITS) / 44100 + 1; // Add 1 to counter truncation.
    while (i < samples) {
...
#ifdef HAVE_LIBGME
  // VGZ format
  if (((UINT8 *)lump)[0] == 0x1F && ((UINT8 *)lump)[1] == 0x8B) {
#ifdef HAVE_ZLIB
    UINT8 *inflatedData;
    size_t inflatedLen;
    z_stream stream;
    ...
    inflatedLen =
        *(UINT32 *)lump +
        (sfx->length - 4); // Last 4 bytes are the decompressed size, typically
    inflatedData = (UINT8 *)Z_Malloc(
        inflatedLen, PU_SOUND, NULL); // Make room for the decompressed data
    ...
    if (zErr == Z_OK) // We're good to go
    {
      ...
      if (zErr == Z_STREAM_END) {
        ...
        if (!gme_open_data(inflatedData, inflatedLen, &emu, 44100)) {
          ...
          Z_Free(inflatedData); // GME supposedly makes a copy for itself, so we
                                // don't need this lying around
      ...
      CONS_Alert(CONS_ERROR, "Encountered %s when running inflateInit: %s\n",
                 get_zlib_error(zErr), stream.msg);
    Z_Free(inflatedData); // GME didn't open jack, but don't let that stop us
                          // from freeing this up
#else
...
void I_FreeSfx(sfxinfo_t *sfx) {
  if (sfx->data) {
    ...
    if (chunk->allocated == 0) {
      // We allocated the data in this chunk, so get the abuf from mixer, then
      // let it free the chunk, THEN we free the data I believe this should
      // ensure the sound is not playing when we free it
      abufdata = chunk->abuf;
    }
...
I_StartSound(sfxenum_t id, UINT8 vol, UINT8 sep, UINT8 pitch, UINT8 priority,
             INT32 channel) {
  UINT8 volume =
      (((UINT16)vol + 1) * (UINT16)sfx_volume) / 62; // (256 * 31) / 62 == 127
  INT32 handle = Mix_PlayChannel(channel, S_sfx[id].data, 0);
  Mix_Volume(handle, volume);
  Mix_SetPanning(handle, min((UINT16)(0xff - sep) << 1, 0xff),
                 min((UINT16)(sep) << 1, 0xff));
  (void)pitch;    // Mixer can't handle pitch
  (void)priority; // priority and channel management is handled by SRB2...
  return handle;
}

void I_StopSound(INT32 handle) { Mix_HaltChannel(handle); }

boolean I_SoundIsPlaying(INT32 handle) { return Mix_Playing(handle); }
...
static void music_loop(void) {
  if (is_looping) {
    ...
    music_bytes = loop_point * 44100.0L *
                  4; // assume 44.1khz, 4-byte length (see I_GetSongPosition)
  } else
    I_StopSong();
}
...
static UINT32 music_fade(UINT32 interval, void *param) {
  ...
  if (!is_fading || internal_volume == fading_target || fading_duration == 0) {
    ...
    if (fading_target < fading_source)
      internal_volume =
          max(min(internal_volume, fading_source - FixedMul(delta, factor)),
              fading_target);
    else if (fading_target > fading_source)
      internal_volume =
          min(max(internal_volume, fading_source + FixedMul(delta, factor)),
              fading_target);
    Mix_VolumeMusic(get_real_volume(music_volume));
    return interval;
  }
...
musictype_t I_SongType(void) {
  ...
  if (gme)
    ...
    return MU_MOD;
           ...
           Mix_GetMusicType(music) == MUS_MP3_MAD)
    return MU_MP3;
  else
  ...
  if (gme) {
    ...
    if (!length)
      CONS_Debug(DBG_DETAILED,
                 "Getting music length: music is missing LENGTHMS= tag. Needed "
                 "for seeking.\n");
    return length;
  }
}
...
boolean I_SetSongPosition(UINT32 position) {
  ...
  if (gme) {
    ...
    if (Mix_SetMusicPosition((float)(position / 1000.0L)) == 0)
      music_bytes = position / 1000.0L * 44100.0L *
                    4; // assume 44.1khz, 4-byte length (see I_GetSongPosition)
    else
  ...
  if (gme) {
    ...
    if (gme_e != NULL) {
      ...
      if (info->length > 0)
        ...
        position = position >= (info->intro_length + info->loop_length)
                       ? (position % info->loop_length)
                       : position;
      else
...
#ifdef HAVE_LIBGME
  if ((UINT8)data[0] == 0x1F && (UINT8)data[1] == 0x8B) {
#ifdef HAVE_ZLIB
    UINT8 *inflatedData;
    size_t inflatedLen;
    z_stream stream;
    ...
    inflatedLen =
        *(UINT32 *)(data +
                    (len -
                     4)); // Last 4 bytes are the decompressed size, typically
    inflatedData = (UINT8 *)Z_Calloc(
        inflatedLen, PU_MUSIC, NULL); // Make room for the decompressed data
  ...
  while ((UINT32)(p - data) < len) {
    if (fpclassify(loop_point) == FP_ZERO && !strncmp(p, key1, key1len)) {
      ...
      if (!strncmp(p, key2, key2len)) // is it LOOPPOINT=?
      {
        ...
        loop_point = (float)((44.1L + atoi(p)) /
                             44100.0L); // LOOPPOINT works by sample count.
        ...
        // This means a lot of tracks that are only 22050hz for a reasonable
        // downloadable file size will loop VERY badly.
      } else if (!strncmp(p, key3, key3len)) // is it LOOPMS=?
      {
        p += key3len; // skip MS=
        loop_point =
            (float)(atoi(p) /
                    1000.0L); // LOOPMS works by real time, as miliseconds.
...
boolean I_PlaySong(boolean looping) {
  ...
  if (fpclassify(song_length) == FP_ZERO &&
      (I_SongType() == MU_OGG || I_SongType() == MU_MP3 ||
       I_SongType() == MU_FLAC))
    CONS_Debug(DBG_DETAILED,
               "This song is missing a LENGTHMS= tag! Required to make seeking "
               "work properly.\n");
  ...
  if (I_SongType() != MU_MOD && I_SongType() != MU_MID &&
    ...
    return false;
  } else if ((I_SongType() == MU_MOD || I_SongType() == MU_MID) &&
             Mix_PlayMusic(music, looping ? -1 : 0) ==
                 -1) // if MOD, loop forever
  {
    CONS_Alert(CONS_ERROR, "Mix_PlayMusic: %s\n", Mix_GetError());
    return false;
  }
  ...
  if (I_SongType() != MU_MOD && I_SongType() != MU_MID &&
      !Mix_RegisterEffect(MIX_CHANNEL_POST, count_music_bytes, NULL, NULL))
    CONS_Alert(CONS_WARNING,
               "Error registering SDL music position counter: %s\n",
               Mix_GetError());
...
boolean I_SetSongTrack(int track) {
  ...
  // If the specified track is within the number of tracks playing, then change
  // it
  if (gme) {
    SDL_LockAudio();
    if (track >= 0 && track < gme_track_count(gme)) {
...
void I_StopFadingSong(void) {
  ...
  is_fading = false;
  fading_source = fading_target = fading_timer = fading_duration = fading_id =
      0;
}
