...
//-----------------------------------------------------------------------------
/// \file
/// \brief SDL_ttf interface code. Necessary for platforms with no framebuffer
/// console systems.
...
INT32
I_TTFLoadFont(const char *file, UINT32 ptsize) {
...
static void I_TTFRendSurface(const char *textmsg, TTF_Font *font,
                             TextQuality quality, SDL_Color fontfgcolor,
                             SDL_Color fontbgcolor) {
  // Print text in the buffer.
  // SDL_ttf has three modes to draw text.
  // Solid rendering is quick, but dirty. Use it if you need speed more than
  // quality.
  switch (quality) {
  case solid:
    TTFRendSurface = TTF_RenderText_Solid(font, textmsg, fontfgcolor);
    break;
  // Shaded rendering adds a background to the rendered text. Because of this,
  // I_TTFDrawText takes an extra color more than the other styles to be a
  // background color. Shaded is supposedly as fast as solid rendering and
  // about as good quality as blended.
  case shaded:
    TTFRendSurface =
        TTF_RenderText_Shaded(font, textmsg, fontfgcolor, fontbgcolor);
    break;
...
// Draw text to screen. It will accept four colour vales (red, green, blue and
// alpha) with foreground for draw modes Solid and Blended, and an extra four
// values for background colour with draw type Shaded.
void I_TTFDrawText(TTF_Font *font, TextQuality quality, INT32 fgR, INT32 fgG,
                   INT32 fgB, INT32 fgA, INT32 bgR, INT32 bgG, INT32 bgB,
                   INT32 bgA, const char *textmsg) {
  ...
  while (*textmsg != '\0') {
    ...
    // Otherwise...
      ...
      // If the previous character was a newline, actually move to the next
      // line.
      if (prevc == '\n') {
        if (textbuffer != NULL) {
      ...
      // If we have reached the end of the screen, move to the next available
      // line.
    ...
    textmsg++;
    // Copy contents of the now-old buffer to somewhere else, so it can be
    // referenced in next loop.
    prevc = c;
  }
...
void I_StartupTTF(UINT32 fontpointsize, Uint32 initflags, Uint32 vidmodeflags) {
  ...
  TTFSurface =
      SDL_SetVideoMode(res.width, res.height, bitsperpixel, vidmodeflags);
  if (!TTFSurface)
  ...
  if (fontpath)
    fontstatus = I_TTFLoadFont(fontpath, fontpointsize);
#endif
  ...
  if (fontstatus < 0) {
    ...
    CONS_Printf("Compiled for SDL_ttf version: %d.%d.%d\n", TTFcompiled.major,
                TTFcompiled.minor, TTFcompiled.patch);
    CONS_Printf("Linked with SDL_ttf version: %d.%d.%d\n", TTFlinked->major,
                TTFlinked->minor, TTFlinked->patch);
  }
}
