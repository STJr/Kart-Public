...
consvar_t cv_vidwait = {
    "vid_wait", "Off",         CV_SAVE | CV_CALL | CV_NOINIT,
    CV_OnOff,   Impl_SetVsync, 0,
    NULL,       NULL,          0,
    0,          NULL};
static consvar_t cv_stretch = {
    "stretch", "Off", CV_SAVE | CV_NOSHOWHELP, CV_OnOff, NULL, 0, NULL, NULL, 0,
    0,         NULL};
...
static INT32 SDLJoyAxis(const Sint16 axis, evtype_t which) {
  ...
  if (which == ev_joystick) {
    ...
    if (Joystick3.bGamepadStyle) {
      ...
      raxis = JoyInfo3.scale != 1 ? ((raxis / JoyInfo3.scale) * JoyInfo3.scale)
                                  : raxis;
    ...
    if (Joystick4.bGamepadStyle) {
      ...
      raxis = JoyInfo4.scale != 1 ? ((raxis / JoyInfo4.scale) * JoyInfo4.scale)
                                  : raxis;
...
static void Impl_HandleControllerAxisEvent(SDL_ControllerAxisEvent evt) {
  ...
  joyid[1] =
      SDL_JoystickInstanceID(SDL_GameControllerGetJoystick(JoyInfo2.dev));
  joyid[2] =
      SDL_JoystickInstanceID(SDL_GameControllerGetJoystick(JoyInfo3.dev));
  joyid[3] =
      SDL_JoystickInstanceID(SDL_GameControllerGetJoystick(JoyInfo4.dev));
...
static void Impl_HandleControllerButtonEvent(SDL_ControllerButtonEvent evt,
                                             Uint32 type) {
  event_t event;
  SDL_JoystickID joyid[4];
...
void I_GetEvent(void) {
  ...
  while (SDL_PollEvent(&evt)) {
    ...
    case SDL_CONTROLLERDEVICEADDED: {
      ...
      SDL_GameController *newcontroller =
      ...
      //
      // 2. Set OTHERS' cv_usejoystickX.value to THEIR new device index,
      // because it likely changed
      ...
      if (newcontroller &&
          (!JoyInfo.dev || !SDL_GameControllerGetAttached(JoyInfo.dev)) &&
          JoyInfo2.dev != newcontroller && JoyInfo3.dev != newcontroller &&
          JoyInfo4.dev !=
              newcontroller) // don't override a currently active device
      {
      ...
      //////////////////////////////
               ...
               (!JoyInfo2.dev ||
                !SDL_GameControllerGetAttached(JoyInfo2.dev)) &&
      ...
      //////////////////////////////
               ...
               (!JoyInfo3.dev ||
                !SDL_GameControllerGetAttached(JoyInfo3.dev)) &&
      ...
      //////////////////////////////
               ...
               (!JoyInfo4.dev ||
                !SDL_GameControllerGetAttached(JoyInfo4.dev)) &&
               JoyInfo.dev != newcontroller && JoyInfo2.dev != newcontroller &&
               JoyInfo3.dev !=
                   newcontroller) // don't override a currently active device
      {
      ...
      else if (atoi(cv_usejoystick3.string) <=
                   I_NumJoys() // don't mess if we intentionally set higher than
                               // NumJoys
               &&
      ...
      else if (atoi(cv_usejoystick4.string) <=
                   I_NumJoys() // don't mess if we intentionally set higher than
                               // NumJoys
               &&
      ...
      // This is a little wasteful since cv_usejoystick already calls this,
      // but we need to do this in case CV_SetValue did nothing because the
      // string was already same. if the device is already active, this should
    ...
    case SDL_CONTROLLERDEVICEREMOVED:
      ...
      if (JoyInfo3.dev)
        cv_usejoystick3.value = JoyInfo3.oldjoy =
            I_GetJoystickDeviceIndex(JoyInfo3.dev) + 1;
      else if (atoi(cv_usejoystick.string) != JoyInfo.oldjoy &&
      ...
      if (JoyInfo4.dev)
        cv_usejoystick4.value = JoyInfo4.oldjoy =
            I_GetJoystickDeviceIndex(JoyInfo4.dev) + 1;
      else if (atoi(cv_usejoystick.string) != JoyInfo.oldjoy &&
               atoi(cv_usejoystick.string) != JoyInfo2.oldjoy &&
               atoi(cv_usejoystick.string) != JoyInfo3.oldjoy)
  ...
  if (mousemovex || mousemovey) {
    ...
    event.data1 = 0;
    event.data2 =
        (INT32)lround(mousemovex * ((float)wwidth / (float)realwidth));
    event.data3 =
        (INT32)lround(mousemovey * ((float)wheight / (float)realheight));
    D_PostEvent(&event);
  }
...
INT32
VID_GetModeForSize(INT32 w, INT32 h) {
  int i;
  for (i = 0; i < MAXWINMODES; i++) {
...
static SDL_bool Impl_CreateWindow(SDL_bool fullscreen) {
  ...
  // Create a window
  window =
      SDL_CreateWindow("SRB2Kart " VERSIONSTRING, SDL_WINDOWPOS_CENTERED,
                       SDL_WINDOWPOS_CENTERED, realwidth, realheight, flags);
