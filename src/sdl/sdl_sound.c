...
static Uint16 samplecount =
    1024; // Alam: 1KB samplecount at 22050hz
          // is 46.439909297052154195011337868481ms of buffer
...
typedef struct chan_struct {
  ...
  Sint16 *rightvol_lookup; // static INT32
...
static void
Snd_LockAudio(void) // Alam: Lock audio data and uninstall audio callback
{
  if (Snd_Mutex)
...
static void
Snd_UnlockAudio(void) // Alam: Unlock audio data and reinstall audio callback
{
  if (Snd_Mutex)
    ...
    Mix_SetPostMix(audio.callback, audio.userdata);
}
...
static void *getsfx(lumpnum_t sfxlump, size_t *len) {
  ...
  if (sr > csr &&
      SDL_BuildAudioCVT(&sfxcvt, AUDIO_U8, 1, sr, AUDIO_U8, 1,
                        csr)) { // Alam: Setup the AudioCVT for the SFX

    sfxcvt.len = (INT32)size - 8; // Alam: Chop off the header
    sfxcvt.buf = malloc(sfxcvt.len * sfxcvt.len_mult); // Alam: make room
    if (sfxcvt.buf)
      M_Memcpy(sfxcvt.buf, &(sfx->data),
               sfxcvt.len); // Alam: copy the sfx sample
    ...
    } else // Alam: the convert failed, not needed or I couldn't malloc the buf
    {
      ...
      *len = size - 8;
      ...
      paddedsfx =
          (dssfx_t *)M_Memcpy(Z_Malloc(size, PU_SOUND, NULL), sfx, size);
    }
  } else {
...
// Returns a handle.
//
static INT32 addsfx(sfxenum_t sfxid, INT32 volume, INT32 step,
                    INT32 seperation) {
...
#else
      (sfx_litng1 <= sfxid && sfxid >= sfx_litng4) || sfxid == sfx_trfire ||
      sfxid == sfx_alarm || sfxid == sfx_spin || sfxid == sfx_athun1 ||
      sfxid == sfx_athun2 || sfxid == sfx_rainin
#endif
  ) {
  ...
  channels[slot].samplerate =
      (channels[slot].data[3] << 8) + channels[slot].data[2];
...
void I_SetSfxVolume(UINT8 volume) {
  ...
  for (i = 0; i < NUM_CHANNELS; i++)
    if (channels[i].end)
      I_SetChannelParams(&channels[i], channels[i].vol, channels[i].sep,
                         channels[i].realstep);
  ...
  while (len--) {
    ...
    for (chan = 0; chan < NUM_CHANNELS; chan++) {
      ...
      if (channels[chan].end) {
        ...
        if (channels[chan].data + (channels[chan].stepremainder >> 16) >=
            channels[chan].end)
  ...
  while (len--) {
    ...
    for (chan = 0; chan < NUM_CHANNELS; chan++) {
      ...
      if (channels[chan].end) {
        ...
        //  to the current data.
        // Adjust volume accordingly.
        d = (Sint16)(d + ((channels[chan].leftvol_lookup[sample] +
                           channels[chan].rightvol_lookup[sample]) >>
                          9));
        // Increment stepage
        channels[chan].stepremainder += channels[chan].step;
        // Check whether we are done.
  ...
  while (len--) {
    ...
    for (chan = 0; chan < NUM_CHANNELS; chan++) {
      ...
      if (channels[chan].end) {
        ...
        //  to the current data.
        // Adjust volume accordingly.
        d += (channels[chan].leftvol_lookup[sample] +
              channels[chan].rightvol_lookup[sample]) >>
             1;
        // Increment stepage
        channels[chan].stepremainder += channels[chan].step;
...
static void SDLCALL I_UpdateStream(void *userdata, Uint8 *stream, int len) {
  ...
  if ((audio.channels != 1 && audio.channels != 2) ||
    ...
    // 				I_UpdateSteamGME(sa_userdata->gme_emu, (INT16
    // *)stream, len/4, sa_userdata->gme_loop);
    // 		}
    // #endif
  }
}
...
void I_StartupSound(void) {
  ...
  if (M_CheckParm("-freq") && M_IsNextParm()) {
    ...
    audio.samples =
        (Uint16)((samplecount / 2) *
                 (INT32)(audio.freq /
                         11025)); // Alam: to keep it around the same XX ms
  ...
  CONS_Printf(M_GetText(" configured audio device with %d samples/slice at "
                        "%ikhz(%dms buffer)\n"),
              samplecount, audio.freq / 1000,
              (INT32)(((float)audio.samples * 1000.0f) / audio.freq));
...
UINT32
I_GetSongLoopPoint(void) { return 0; }
...
UINT32
I_GetSongPosition(void) { return 0; }
