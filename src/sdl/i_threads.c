...
static Link Insert_link(Link *head, Link link) {
  link->prev = NULL;
  link->next = (*head);
  if ((*head))
    (*head)->prev = link;
...
static void Free_link(Link *head, Link link) {
  if (link->prev)
...
static Link New_link(void *data) {
  Link link;
...
static void *Identity(Link *pool_anchor, I_mutex pool_mutex,
...
void I_spawn_thread(const char *name, I_thread_fn entry, void *userdata) {
  ...
  I_lock_mutex(&i_thread_pool_mutex);
  {
    ...
    if (SDL_AtomicGet(&i_threads_running)) {
      th->thread = SDL_CreateThread((SDL_ThreadFunction)Worker, name, link);
...
int I_thread_is_stopped(void) { return (!SDL_AtomicGet(&i_threads_running)); }
...
void I_start_threads(void) {
  ...
  if (!(i_thread_pool_mutex && i_mutex_pool_mutex && i_cond_pool_mutex)) {
    abort();
  }
}
...
void I_stop_threads(void) {
  ...
  if (i_threads_running.value) {
    ...
    I_lock_mutex(&i_thread_pool_mutex);
    {
      for (link = i_thread_pool; link; link = next) {
        next = link->next;
        th = link->data;
    ...
    for (link = i_mutex_pool; link; link = next) {
      next = link->next;
      mutex = link->data;
    ...
    for (link = i_cond_pool; link; link = next) {
      next = link->next;
      cond = link->data;
...
void I_lock_mutex(I_mutex *anchor) {
  ...
  mutex = Identity(&i_mutex_pool, i_mutex_pool_mutex, anchor,
...
void I_hold_cond(I_cond *cond_anchor, I_mutex mutex_id) {
  ...
  cond = Identity(&i_cond_pool, i_cond_pool_mutex, cond_anchor,
...
void I_wake_one_cond(I_cond *anchor) {
  ...
  cond = Identity(&i_cond_pool, i_cond_pool_mutex, anchor,
                  (Create_fn)SDL_CreateCond);
