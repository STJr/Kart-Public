...
#define HAVE_SDLCPUINFO
    ...
    (defined(UNIXCOMMON) && !defined(__HAIKU__))
...
static void I_ReportSignal(int num, int coredumped) {
  ...
  case SIGSEGV:
    ...
    break;
    //	case SIGTERM:
    //		sigmsg = "SIGTERM - Software termination signal from
    // kill"; 		break; 	case SIGBREAK: 		sigmsg =
    // "SIGBREAK - Ctrl-Break sequence"; 		break;
  case SIGABRT:
  ...
  SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Process killed by signal",
                           sigmsg, NULL);
}
...
void I_GetConsoleEvents(void) {
  ...
  // we have something
  // backspace?
  // NOTE TTimo testing a lot of values .. seems it's the only way to get it to
  // work everywhere
...
void I_OutputMsg(const char *fmt, ...) {
...
#ifdef HAVE_TTF
  if (TTF_WasInit())
    I_TTFDrawText(currentfont, solid, DEFAULTFONTFGR, DEFAULTFONTFGG,
                  DEFAULTFONTFGB, DEFAULTFONTFGA, DEFAULTFONTBGR,
                  DEFAULTFONTBGG, DEFAULTFONTBGB, DEFAULTFONTBGA, txt);
#endif
  ...
  // 2004-03-03 AJR Since not all messages end in newline, some were getting
  // displayed late.
  if (!framebuffer)
    fflush(stderr);
}
...
INT32
I_GetJoystickDeviceIndex(SDL_GameController *dev) {
...
// Misleading function: updates device indices for all players BUT the one
// specified. Necessary for SDL_JOYDEVICEADDED events
void I_UpdateJoystickDeviceIndices(INT32 player) {
  if (player != 1) // This is a fucking mess.
  {
    ...
    else if ( // don't check JoyInfo or cv_usejoystick; we're currently
              // operating on those
  ...
  if (player != 2) {
    ...
    // is cv_usejoystick2 used?
             ...
             // don't check JoyInfo2 or cv_usejoystick2; we're currently
             // operating on those
             && atoi(cv_usejoystick2.string) != JoyInfo3.oldjoy &&
             atoi(cv_usejoystick2.string) != cv_usejoystick3.value &&
             atoi(cv_usejoystick2.string) != JoyInfo4.oldjoy &&
  ...
  if (player != 3) {
    ...
    // is cv_usejoystick3 used?
             ...
             atoi(cv_usejoystick3.string) != JoyInfo2.oldjoy &&
             atoi(cv_usejoystick3.string) != cv_usejoystick2.value
             // don't check JoyInfo3 or cv_usejoystick3; we're currently
             // operating on those
             && atoi(cv_usejoystick3.string) != JoyInfo4.oldjoy &&
             atoi(cv_usejoystick3.string) != cv_usejoystick4.value)
  ...
  if (player != 4) {
    ...
    // is cv_usejoystick4 used?
             ...
             atoi(cv_usejoystick4.string) != JoyInfo3.oldjoy &&
             atoi(cv_usejoystick4.string) != cv_usejoystick3.value)
      // don't check JoyInfo4 or cv_usejoystick4; we're currently operating on
      // those
...
void I_GetJoystickEvents(void) {
  ...
  joyhats |=
      SDL_GameControllerGetButton(JoyInfo.dev, SDL_CONTROLLER_BUTTON_DPAD_UP);
  joyhats |=
      SDL_GameControllerGetButton(JoyInfo.dev, SDL_CONTROLLER_BUTTON_DPAD_DOWN)
      << 1;
  joyhats |=
      SDL_GameControllerGetButton(JoyInfo.dev, SDL_CONTROLLER_BUTTON_DPAD_LEFT)
      << 2;
  joyhats |=
      SDL_GameControllerGetButton(JoyInfo.dev, SDL_CONTROLLER_BUTTON_DPAD_RIGHT)
      << 3;
...
static int joy_open(int joyindex) {
  ...
  // Handle the edge case where the device <-> joystick index assignment can
  // change due to hotplugging This indexing is SDL's responsibility and there's
  ...
  // 6. Plug Controller A   -> Index 0 REPLACED, opened as Controller A; Index 1
  // is now Controller B
  if (JoyInfo.dev) {
    if (JoyInfo.dev == newdev // same device, nothing to do
        || (newdev == NULL &&
            SDL_GameControllerGetAttached(
                JoyInfo.dev))) // we failed, but already have a working device
      return SDL_CONTROLLER_AXIS_MAX;
    // Else, we're changing devices, so send neutral joy events
    CONS_Debug(DBG_GAMELOGIC,
               "Joystick1 device is changing; resetting events...\n");
    I_ShutdownJoystick();
  }
  ...
  if (JoyInfo.dev == NULL) {
    CONS_Debug(DBG_GAMELOGIC,
               M_GetText("Joystick1: Couldn't open device - %s\n"),
               SDL_GetError());
    return -1;
  } else {
...
void I_GetJoystick2Events(void) {
  ...
  joyhats |=
      SDL_GameControllerGetButton(JoyInfo2.dev, SDL_CONTROLLER_BUTTON_DPAD_UP);
  joyhats |=
      SDL_GameControllerGetButton(JoyInfo2.dev, SDL_CONTROLLER_BUTTON_DPAD_DOWN)
      << 1;
  joyhats |=
      SDL_GameControllerGetButton(JoyInfo2.dev, SDL_CONTROLLER_BUTTON_DPAD_LEFT)
      << 2;
  joyhats |= SDL_GameControllerGetButton(JoyInfo2.dev,
...
static int joy_open2(int joyindex) {
  ...
  if (JoyInfo2.dev) {
    if (JoyInfo2.dev == newdev // same device, nothing to do
        || (newdev == NULL &&
            SDL_GameControllerGetAttached(
                JoyInfo2.dev))) // we failed, but already have a working device
      return SDL_CONTROLLER_AXIS_MAX;
    // Else, we're changing devices, so send neutral joy events
    CONS_Debug(DBG_GAMELOGIC,
               "Joystick2 device is changing; resetting events...\n");
    I_ShutdownJoystick2();
  }
  ...
  if (JoyInfo2.dev == NULL) {
    CONS_Debug(DBG_GAMELOGIC,
               M_GetText("Joystick2: couldn't open device - %s\n"),
               SDL_GetError());
    return -1;
  } else {
...
void I_GetJoystick3Events(void) {
  ...
  joyhats |=
      SDL_GameControllerGetButton(JoyInfo3.dev, SDL_CONTROLLER_BUTTON_DPAD_UP);
  joyhats |=
      SDL_GameControllerGetButton(JoyInfo3.dev, SDL_CONTROLLER_BUTTON_DPAD_DOWN)
      << 1;
  joyhats |=
      SDL_GameControllerGetButton(JoyInfo3.dev, SDL_CONTROLLER_BUTTON_DPAD_LEFT)
      << 2;
  joyhats |= SDL_GameControllerGetButton(JoyInfo3.dev,
                                         SDL_CONTROLLER_BUTTON_DPAD_RIGHT)
             << 3;
...
static int joy_open3(int joyindex) {
  ...
  if (JoyInfo3.dev) {
    if (JoyInfo3.dev == newdev // same device, nothing to do
        || (newdev == NULL &&
            SDL_GameControllerGetAttached(
                JoyInfo3.dev))) // we failed, but already have a working device
      return SDL_CONTROLLER_AXIS_MAX;
    // Else, we're changing devices, so send neutral joy events
    CONS_Debug(DBG_GAMELOGIC,
               "Joystick3 device is changing; resetting events...\n");
    I_ShutdownJoystick3();
  }
  ...
  if (JoyInfo3.dev == NULL) {
    CONS_Debug(DBG_GAMELOGIC,
               M_GetText("Joystick3: couldn't open device - %s\n"),
               SDL_GetError());
    return -1;
  } else {
...
void I_GetJoystick4Events(void) {
  ...
  joyhats |=
      SDL_GameControllerGetButton(JoyInfo4.dev, SDL_CONTROLLER_BUTTON_DPAD_UP);
  joyhats |=
      SDL_GameControllerGetButton(JoyInfo4.dev, SDL_CONTROLLER_BUTTON_DPAD_DOWN)
      << 1;
  joyhats |=
      SDL_GameControllerGetButton(JoyInfo4.dev, SDL_CONTROLLER_BUTTON_DPAD_LEFT)
      << 2;
  joyhats |= SDL_GameControllerGetButton(JoyInfo4.dev,
                                         SDL_CONTROLLER_BUTTON_DPAD_RIGHT)
             << 3;
...
static int joy_open4(int joyindex) {
  ...
  if (JoyInfo4.dev) {
    if (JoyInfo4.dev == newdev // same device, nothing to do
        || (newdev == NULL &&
            SDL_GameControllerGetAttached(
                JoyInfo4.dev))) // we failed, but already have a working device
      return SDL_CONTROLLER_AXIS_MAX;
    // Else, we're changing devices, so send neutral joy events
    CONS_Debug(DBG_GAMELOGIC,
               "Joystick4 device is changing; resetting events...\n");
    I_ShutdownJoystick4();
  }
  ...
  if (JoyInfo4.dev == NULL) {
    CONS_Debug(DBG_GAMELOGIC,
               M_GetText("Joystick4: couldn't open device - %s\n"),
               SDL_GetError());
    return -1;
  } else {
...
void I_InitJoystick(void) {
  ...
  if (newcontroller &&
      (JoyInfo2.dev == newcontroller || JoyInfo3.dev == newcontroller ||
       JoyInfo4.dev == newcontroller)) // don't override an active device
    cv_usejoystick.value = I_GetJoystickDeviceIndex(JoyInfo.dev) + 1;
  else if (newcontroller && joy_open(cv_usejoystick.value) != -1) {
    // SDL's device indexes are unstable, so cv_usejoystick may not match
    // the actual device index. So let's cheat a bit and find the device's
    // current index.
  ...
  if (JoyInfo.dev != newcontroller && JoyInfo2.dev != newcontroller &&
      JoyInfo3.dev != newcontroller && JoyInfo4.dev != newcontroller)
...
void I_InitJoystick2(void) {
  ...
  if (newcontroller &&
      (JoyInfo.dev == newcontroller || JoyInfo3.dev == newcontroller ||
       JoyInfo4.dev == newcontroller)) // don't override an active device
    cv_usejoystick2.value = I_GetJoystickDeviceIndex(JoyInfo2.dev) + 1;
...
void I_InitJoystick3(void) {
  ...
  if (newcontroller &&
      (JoyInfo.dev == newcontroller || JoyInfo2.dev == newcontroller ||
       JoyInfo4.dev == newcontroller)) // don't override an active device
    cv_usejoystick3.value = I_GetJoystickDeviceIndex(JoyInfo3.dev) + 1;
...
static void I_SetupMumble(void) {
  ...
  mumble =
      mmap(NULL, sizeof(*mumble), PROT_READ | PROT_WRITE, MAP_SHARED, shmfd, 0);
...
void I_UpdateMumble(const mobj_t *mobj, const listener_t listener) {
  ...
  if (mumble->uiVersion != 2) {
    ...
    wcsncpy(mumble->description,
            L"Sonic Robo Blast 2 Kart with integrated Mumble Link support.",
            2048);
    mumble->uiVersion = 2;
  }
  mumble->uiTick++;
  ...
  if (!netgame || gamestate != GS_LEVEL) { // Zero out, but never delink.
    mumble->fAvatarPosition[0] = mumble->fAvatarPosition[1] =
        mumble->fAvatarPosition[2] = 0.0f;
    mumble->fAvatarFront[0] = 1.0f;
    mumble->fAvatarFront[1] = mumble->fAvatarFront[2] = 0.0f;
    mumble->fCameraPosition[0] = mumble->fCameraPosition[1] =
        mumble->fCameraPosition[2] = 0.0f;
...
void I_StartupMouse2(void) {
  ...
  if (cv_usemouse2.value == 0)
    return;
  if ((fdmouse2 = open(cv_mouse2port.string,
...
ticcmd_t *I_BaseTiccmd2(void) { return &emptycmd2; }

/**	\brief empty ticcmd for player 3
 */
static ticcmd_t emptycmd3;

ticcmd_t *I_BaseTiccmd3(void) { return &emptycmd3; }

/**	\brief empty ticcmd for player 4
 */
static ticcmd_t emptycmd4;

ticcmd_t *I_BaseTiccmd4(void) { return &emptycmd4; }

static Uint64 timer_frequency;

precise_t I_GetPreciseTime(void) { return SDL_GetPerformanceCounter(); }

UINT64
I_GetPrecisePrecision(void) { return SDL_GetPerformanceFrequency(); }
...
void I_Sleep(UINT32 ms) { SDL_Delay(ms); }

#ifdef NEWSIGNALHANDLER
static void newsignalhandler_Warn(const char *pr) {
  char text[128];

  snprintf(text, sizeof text, "Error while setting up signal reporting: %s: %s",
           pr, strerror(errno));

  I_OutputMsg("%s\n", text);

  SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Startup error", text, NULL);
...
I_StartupSystem(void) {
  ...
  I_OutputMsg("Compiled for SDL version: %d.%d.%d\n", SDLcompiled.major,
              SDLcompiled.minor, SDLcompiled.patch);
  I_OutputMsg("Linked with SDL version: %d.%d.%d\n", SDLlinked.major,
              SDLlinked.minor, SDLlinked.patch);
  if (SDL_Init(0) < 0)
...
void I_Error(const char *error, ...) {
  ...
  // on the target system
  SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR,
                           "SRB2Kart " VERSIONSTRING " Error", buffer, NULL);
...
INT32
I_mkdir(const char *dirname, INT32 unixright) {
//[segabor]
#if defined(__unix__) || defined(__APPLE__) || defined(UNIXCOMMON) ||          \
...
INT32
I_ClipboardCopy(const char *data, size_t size) {
  char storage[256];
  if (size > 255)
    size = 255;
...
#define X_SUM 0
  ...
  if (total)
    DosQuerySysInfo(QSV_TOTPHYSMEM, QSV_TOTPHYSMEM, (PVOID)total,
                    sizeof(UINT32));
  DosQuerySysInfo(QSV_MAXPRMEM, QSV_MAXPRMEM, (PVOID)&pr_arena, sizeof(UINT32));
