  ...
  while (ls->current != del) {
    ...
    case '\\': {
      ...
      case 'x': {
        ...
        case 5:
        default:
          luaX_lexerror(ls, "escape sequence wrong size (2 or 4 digits)",
                        TK_STRING);
        }
      }
        continue;
        ...
        //
        // Note that although codes are entered like this (must have min.
        // four digit, just to tease you!) the actual outcome in the string
        // will vary between 1..4 bytes, depending on the value range.
        //
      case 'u': {
        int i;
        c = 0;
        ...
        if ((i != 4) && (i != 6)) {
          luaX_lexerror(ls, "escape sequence wrong size (4 or 6 digits)",
                        TK_STRING);
        }
        ...
        /* http://www.cl.cam.ac.uk/~mgk25/unicode.html#utf-8
           ...
           10xxxxxx (U-00200000 - U-03FFFFFF:	111110xx 10xxxxxx
           10xxxxxx 10xxxxxx 10xxxxxx) (U-04000000 - U-7FFFFFFF: 1111110x
...
static int llex(LexState *ls, SemInfo *seminfo) {
  ...
  for (;;) {
    ...
    case '/': {
      ...
      /* else is a comment */
      if (ls->current == '*') {
        char last = '\n'; /* don't remember the first asterisk, we need a
                             different one. */
        next(ls);
