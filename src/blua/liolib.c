...
static const char *whitelist[] = { // Allow scripters to write files of these
                                   // types to SRB2's folder
    ".txt", ".sav2", ".cfg", ".png", ".bmp"};
...
static int StartsWith(const char *a,
                      const char *b) // this is wolfs being lazy yet again
{
...
static int io_open(lua_State *L) {
  ...
  if (strstr(filename, "..") || strchr(filename, ':') ||
      StartsWith(filename, "\\") || StartsWith(filename, "/") || !pass) {
  ...
  while ((splitter = (forward && backward) ? min(forward, backward)
                                           : (forward ?: backward))) {
    *splitter = 0;
  ...
  for (; nargs--; arg++) {
    if (lua_type(L, arg) == LUA_TNUMBER) {
      ...
      if (ftell(f) + l > FILELIMIT) {
        luaL_error(
            L, "write limit bypassed in file. Changes have been discarded.");
        break;
      }
...
static const luaL_Reg iolib[] = {{"close", io_close}, {"flush", io_flush},
                                 ...
                                 {"read", io_read},   {"tmpfile", io_tmpfile},
                                 {"type", io_type},   {"write", io_write},
                                 {NULL, NULL}};
