  ...
  case '$': {
    ...
    if (i <= 0 || i > ls->fs->nlhs)
      ...
      while (i--)
        lhs = lhs->prev;
      *v = lhs->v;
      /* If this is a VINDEXED, we need to stash the result in a temporary
       * without freereg'ing the index locals. Really, it would be better to
       * keep the value in case we repeat the $, but there's no way to do that
       * without shifting all the temps up one slot to create room for the
       * value. It would be semantically correct to also do that for globals,
       * in case I ever get around to importing the shift mechanism from
       * comprehensions.
       */
      if (v->k == VINDEXED) {
...
static void assignment(LexState *ls) {
  ...
  if (testnext(ls, ',')) { /* assignment -> `,' primaryexp assignment */
    ...
    if (ls->fs->nrhs != ls->fs->nlhs) {
      ...
      if (ls->fs->nrhs > ls->fs->nlhs)
        ls->fs->freereg -=
            ls->fs->nrhs - ls->fs->nlhs; /* remove extra values */
    } else {
...
static void continuestat(LexState *ls) {
  ...
  FuncState *fs = ls->fs;
  BlockCnt *bl = fs->bl;
  // TBD: Not sure if also continue should have the upvalue check? Original
  // patch had not.
  // int upval = 0;
...
static void whilestat(LexState *ls, int line) {
  ...
  testnext(ls, TK_DO);
  block(ls);
  //  luaK_patchlist(fs, luaK_jump(fs), bl.continuelist); -- bug rendering
  //  'continue' useless in 'while', asko?
