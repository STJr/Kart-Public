...
void luaV_settable(lua_State *L, TValue *t, TValue *key, StkId val) {
  ...
  for (loop = 0; loop < MAXTAGLOOP; loop++) {
    ...
    if (ttistable(t)) { /* `t' is a table? */
      ...
      /* oldval is nil=> look for newindex, oldval is not nil => look for
       * usedindex */
      if (!((ttisnil(oldval) &&
             ((tm = fasttm(L, h->metatable, TM_NEWINDEX)) != NULL)) ||
            ((!ttisnil(oldval)) &&
             ((tm = fasttm(L, h->metatable, TM_USEDINDEX)) != NULL)))) {
...
static int call_binTM(lua_State *L, TValue *p1, TValue *p2, StkId res,
                      TMS event) {
...
static int call_orderTM(lua_State *L, TValue *p1, TValue *p2, TMS event) {
...
static void Arith(lua_State *L, StkId ra, TValue *rb, TValue *rc, TMS op) {
  TValue tempb, tempc;
  const TValue *b, *c;
  if ((b = luaV_tonumber(rb, &tempb)) != NULL &&
    ...
    case TM_UNM:
      ...
      break;
    case TM_AND:
      setnvalue(ra, luai_numand(nb, nc));
      break;
    case TM_OR:
      setnvalue(ra, luai_numor(nb, nc));
      break;
    case TM_XOR:
      setnvalue(ra, luai_numxor(nb, nc));
      break;
    case TM_SHL:
