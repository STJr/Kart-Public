...
static const char *const sector_opt[] = {
    "valid",      "floorheight", "ceilingheight", "floorpic",
    "ceilingpic", "lightlevel",  "special",       "tag",
    "thinglist",  "heightsec",   "camsec",        "lines",
    "ffloors",    "f_slope",     "c_slope",       NULL};
...
static const char *const subsector_opt[] = {"valid", "sector", "numlines",
                                            "firstline", NULL};
...
static const char *const line_opt[] = {
    "valid",    "v1",        "v2",          "dx",         "dy",
    "flags",    "special",   "tag",         "sidenum",    "frontside",
    "backside", "slopetype", "frontsector", "backsector", "firsttag",
    "nexttag",  "text",      "callcount",   NULL};
...
static const char *const side_opt[] = {
    "valid",         "textureoffset", "rowoffset", "toptexture",
    "bottomtexture", "midtexture",    "sector",    "special",
    "repeatcnt",     "text",          NULL};

enum vertex_e { vertex_valid = 0, vertex_x, vertex_y, vertex_z };

static const char *const vertex_opt[] = {"valid", "x", "y", "z", NULL};
...
static const char *const ffloor_opt[] = {
    "valid",        "topheight", "toppic",  "toplightlevel",
    "bottomheight", "bottompic", "t_slope", "b_slope",
    "sector", // secnum pushed as control sector userdata
    "flags",
    "master", // control linedef
    "target", // target sector
    "next",         "prev",      "alpha",   NULL};
...
static const char *const slope_opt[] = {
    "valid",       "o",          "d",      "zdelta", "normal", "zangle",
    "xydirection", "sourceline", "refpos", "flags",  NULL};

// shared by both vector2_t and vector3_t
enum vector_e { vector_x = 0, vector_y, vector_z };

static const char *const vector_opt[] = {"x", "y", "z", NULL};
...
static int lib_iterateSectorThinglist(lua_State *L) {
  ...
  if (lua_gettop(L) < 2)
    return luaL_error(L,
                      "Don't call sector.thinglist() directly, use it as 'for "
                      "rover in sector.thinglist do <block> end'.");
...
static int lib_iterateSectorFFloors(lua_State *L) {
  ...
  if (lua_gettop(L) < 2)
    return luaL_error(L, "Don't call sector.ffloors() directly, use it as 'for "
                         "rover in sector.ffloors do <block> end'.");
...
static int sector_iterate(lua_State *L) {
  lua_pushvalue(L,
                lua_upvalueindex(1)); // iterator function, or the "generator"
  lua_pushvalue(
      L,
      lua_upvalueindex(
          2));    // state (used as the "start" of the list for our purposes
...
static int sectorlines_get(lua_State *L) {
  ...
  // check first linedef to figure which of its sectors owns this sector->lines
  // pointer then check that sector's linecount to get a maximum index
  // if (!seclines[0])
  ...
  else if (seclines[0]->backsector &&
           seclines[0]->backsector->lines ==
               seclines) // check backsector exists first
...
static int sector_get(lua_State *L) {
  ...
  case sector_thinglist: // thinglist
    ...
    lua_pushcclosure(L, sector_iterate,
                     2); // push lib_iterateSectorThinglist and
                         // sector->thinglist as upvalues for the function
    return 1;
  ...
  case sector_ffloors: // ffloors
    ...
    lua_pushcclosure(L, sector_iterate,
                     2); // push lib_iterateFFloors and sector->ffloors as
                         // upvalues for the function
    return 1;
...
static int sector_set(lua_State *L) {
  ...
  case sector_cslope:    // c_slope
  default:
    return luaL_error(L, "sector_t field " LUA_QS " cannot be set.",
                      sector_opt[field]);
  ...
  case sector_special:
    ...
    break;
  case sector_tag:
    P_ChangeSectorTag((UINT32)(sector - sectors),
                      (INT16)luaL_checkinteger(L, 3));
    break;
  }
  return 0;
}
...
static int subsector_get(lua_State *L) {
  subsector_t *subsector =
      *((subsector_t **)luaL_checkudata(L, 1, META_SUBSECTOR));
  enum subsector_e field =
      luaL_checkoption(L, 2, subsector_opt[0], subsector_opt);
...
static int side_set(lua_State *L) {
  ...
  case side_text:
  default:
    return luaL_error(L, "side_t field " LUA_QS " cannot be set.",
                      side_opt[field]);
  case side_textureoffset:
...
static int lib_iterateSectors(lua_State *L) {
  ...
  if (lua_gettop(L) < 2)
    return luaL_error(L,
                      "Don't call sectors.iterate() directly, use it as 'for "
                      "sector in sectors.iterate do <block> end'.");
  lua_settop(L, 2);
  lua_remove(L, 1); // state is unused.
  if (!lua_isnil(L, 1))
    i = (size_t)(*((sector_t **)luaL_checkudata(L, 1, META_SECTOR)) - sectors) +
        1;
  if (i < numsectors) {
...
static int lib_iterateSubsectors(lua_State *L) {
  ...
  if (lua_gettop(L) < 2)
    return luaL_error(L,
                      "Don't call subsectors.iterate() directly, use it as "
                      "'for subsector in subsectors.iterate do <block> end'.");
  lua_settop(L, 2);
  lua_remove(L, 1); // state is unused.
  if (!lua_isnil(L, 1))
    i = (size_t)(*((subsector_t **)luaL_checkudata(L, 1, META_SUBSECTOR)) -
                 subsectors) +
        1;
  if (i < numsubsectors) {
...
static int lib_iterateLines(lua_State *L) {
  size_t i = 0;
  if (lua_gettop(L) < 2)
    return luaL_error(L, "Don't call lines.iterate() directly, use it as 'for "
                         "line in lines.iterate do <block> end'.");
  lua_settop(L, 2);
  lua_remove(L, 1); // state is unused.
  if (!lua_isnil(L, 1))
...
static int lib_iterateSides(lua_State *L) {
  size_t i = 0;
  if (lua_gettop(L) < 2)
    return luaL_error(L, "Don't call sides.iterate() directly, use it as 'for "
                         "side in sides.iterate do <block> end'.");
  lua_settop(L, 2);
  lua_remove(L, 1); // state is unused.
  if (!lua_isnil(L, 1))
...
static int lib_iterateVertexes(lua_State *L) {
  ...
  if (lua_gettop(L) < 2)
    return luaL_error(L,
                      "Don't call vertexes.iterate() directly, use it as 'for "
                      "vertex in vertexes.iterate do <block> end'.");
  lua_settop(L, 2);
  lua_remove(L, 1); // state is unused.
  if (!lua_isnil(L, 1))
    i = (size_t)(*((vertex_t **)luaL_checkudata(L, 1, META_VERTEX)) -
                 vertexes) +
        1;
  if (i < numvertexes) {
...
static int ffloor_set(lua_State *L) {
  ...
  default:
    return luaL_error(L, "ffloor_t field " LUA_QS " cannot be set.",
                      ffloor_opt[field]);
...
static int slope_set(lua_State *L) {
  ...
  case slope_refpos:     // refpos
  default:
    return luaL_error(L, "pslope_t field " LUA_QS " cannot be set.",
                      slope_opt[field]);
  case slope_o: { // o
  ...
  case slope_zangle: { // zangle
    ...
    slope->zangle = zangle;
    slope->zdelta =
        -FINETANGENT(((slope->zangle + ANGLE_90) >> ANGLETOFINESHIFT) & 4095);
    P_CalculateSlopeNormal(slope);
    break;
  }
  case slope_xydirection: // xydirection
    ...
    slope->d.x =
        -FINECOSINE((slope->xydirection >> ANGLETOFINESHIFT) & FINEMASK);
...
static int vector2_get(lua_State *L) {
  ...
  switch (field) {
  case vector_x:
    lua_pushfixed(L, vec->x);
    return 1;
  case vector_y:
    lua_pushfixed(L, vec->y);
    return 1;
  default:
    break;
  }
...
static int mapheaderinfo_get(lua_State *L) {
  mapheader_t *header =
      *((mapheader_t **)luaL_checkudata(L, 1, META_MAPHEADER));
  ...
  // TODO add support for reading numGradedMares and grades
    ...
    UINT8 j = 0;
    for (; j < header->numCustomOptions &&
           !fastcmp(field, header->customopts[j].option);
         ++j)
      ;
