...
static lua_CFunction liblist[] = {
    ...
    LUA_BaseLib,     // string concatination by +, CONS_Printf, p_local.h stuff
                     // (P_InstaThrust, P_Move), etc.
...
FUNCNORETURN static int LUA_Panic(lua_State *L) {
  ...
  I_Error("An unfortunate Lua processing error occurred in the exe itself. "
          "This is not a scripting error on your part.");
#ifndef __GNUC__
  return -1;
#endif
}
...
static int noglobals(lua_State *L) {
  ...
  Z_Free(name);
  return luaL_error(L,
                    "Implicit global " LUA_QS
                    " prevented. Create a local variable instead.",
                    csname);
}
...
void LUA_LoadLump(UINT16 wad, UINT16 lump) {
  ...
  } else // If it's not a .lua file, copy the lump name in too.
  {
    ...
    len +=
        1 + strlen(lump_p->fullname); // length of file name, '|', and lump name
    name = malloc(len + 1);
...
static int dumpWriter(lua_State *L, const void *p, size_t sz, void *ud) {
  ...
  (void)L;
  if (!sz)
    return 0; // nothing to write? can't fail that! :D
  ...
  if (lua_isnil(L, -1)) { // no userdata? deary me, we'll have to make one.
    ...
    *userdata = data;
    ...
    lua_pushlightuserdata(
        L, data);         // k (store the userdata via the data's pointer)
    lua_pushvalue(L, -2); // v (copy of the userdata)
    lua_rawset(L, -4);

    // stack is left with the userdata on top, as if getting it had originally
    // succeeded.
  }
...
static const struct {
  const char *meta;
  UINT8 arch;
} meta2arch[] = {{META_MOBJINFO, ARCH_MOBJINFO},
...
static UINT8 ArchiveValue(int TABLESINDEX, int myindex) {
  ...
  case LUA_TSTRING: {
    UINT16 len = (UINT16)lua_objlen(
        gL, myindex); // get length of string, including embedded zeros
    ...
    // Saving the size of the string also allows us to get the size of the
    // string on the other end, fixing the awful crashes previously
    // encountered for reading strings longer than 1024 (yes I know that's
    // kind of a stupid thing to care about, but it'd be evil to trim or
    // ignore them?)
    ...
    while (i < len)
      WRITECHAR(
          save_p,
          s[i++]); // write chars individually, including the embedded zeros
    break;
  }
  case LUA_TTABLE: {
    boolean found = false;
...
// because of how this function works it has to be pasted allll over again just
// to save to demo_p since it returns an uint8 to begin with
...
static void ArchiveExtVars(void *pointer, const char *ptype) {
  ...
  if (!gL) {
    if (fastcmp(ptype,
                "player")) // players must always be included, even if no vars
  ...
  if (!lua_istable(gL, -1)) { // no extra values table
    lua_pop(gL, 1);
    if (fastcmp(ptype,
                "player")) // players must always be included, even if no vars
      WRITEUINT16(save_p, 0);
  ...
  if (i == 0) {
    if (fastcmp(ptype, "player")) // always include players even if they have no
                                  // extra variables
      WRITEUINT16(save_p, 0);
    lua_pop(gL, 1);
    return;
  ...
  while (lua_next(gL, -2)) {
    ...
    if (ArchiveValue(TABLESINDEX, -1) == 2)
      CONS_Alert(
          CONS_ERROR,
          "Type of value for %s entry '%s' (%s) could not be archived!\n",
...
static void ArchiveTables(void) {
  ...
  for (i = 1; i <= n; i++) {
    ...
    while (lua_next(gL, -2)) {
      // Write key
      e = ArchiveValue(TABLESINDEX,
                       -2); // key should be either a number or a
                            // string, ArchiveValue can handle this.
      if (e == 2) // invalid key type (function, thread, lightuserdata, or
                  // anything we don't recognise)
      {
        lua_pushvalue(gL, -2);
        CONS_Alert(CONS_ERROR,
                   "Index '%s' (%s) of table %d could not be archived!\n",
      ...
      else if (e == 2) // invalid value type
      {
        lua_pushvalue(gL, -2);
        CONS_Alert(CONS_ERROR,
                   "Type of value for table %d entry '%s' (%s) could not be "
                   "archived!\n",
...
static void ArchiveTablesDemo(void) {
  ...
  for (i = 1; i <= n; i++) {
    ...
    while (lua_next(gL, -2)) {
      // Write key
      e = ArchiveValueDemo(TABLESINDEX,
                           -2); // key should be either a number or a string,
                                // ArchiveValue can handle this.
      if (e == 2) // invalid key type (function, thread, lightuserdata, or
                  // anything we don't recognise)
      {
        lua_pushvalue(gL, -2);
...
static UINT8 UnArchiveValue(int TABLESINDEX) {
  ...
  case ARCH_STRING: {
    UINT16 len =
        READUINT16(save_p); // length of string, including embedded zeros
    char *value;
    UINT16 i = 0;
    ...
    while (i < len)
      value[i++] = READCHAR(
          save_p); // read chars individually, including the embedded zeros
    lua_pushlstring(
        gL, value,
        len); // push the string (note: this function supports embedded zeros)
...
static UINT8 UnArchiveValueDemo(int TABLESINDEX, char field[1024]) {
  ...
  case ARCH_STRING: {
    UINT16 len =
        READUINT16(demo_p); // length of string, including embedded zeros
    char *value;
    UINT16 i = 0;
    // See my comments in the ArchiveValue function;
    // it's much the same for reading strings as writing them!
    ...
    while (i < len)
      value[i++] = READCHAR(
          demo_p); // read chars individually, including the embedded zeros
    lua_pushlstring(
        gL, value,
        len); // push the string (note: this function supports embedded zeros)
    free(value); // free the buffer
  ...
  case ARCH_STATE:
    ...
    break;
  case ARCH_MOBJ:
    demo_p += sizeof(UINT32); // Skip this data, we can't read a mobj here,
                              // it'd point to garbage and crash the game.
    if (field)
      CONS_Alert(CONS_WARNING,
                 "Cannot read mobj_t stored in player variable \'%s\'. "
                 "Desyncs may occur.\n",
                 field);
    else
...
static void UnArchiveExtVarsDemo(void *pointer) {
  ...
  for (i = 0; i < field_count; i++) {
    ...
    if (UnArchiveValueDemo(TABLESINDEX, field) !=
        3) // This will return 3 if we shouldn't set this field.
      lua_setfield(gL, -2, field);
  }
...
static void UnArchiveTables(void) {
  ...
  for (i = 1; i <= n; i++) {
    ...
    while (true) {
      ...
      if (lua_isnil(
              gL,
              -2)) // if key is nil (if a function etc was accidentally saved)
      {
        CONS_Alert(CONS_ERROR,
                   "A nil key in table %d was found! (Invalid key type or "
                   "corrupted save?)\n",
                   i);
        lua_pop(gL, 2); // pop key and value instead of setting them in the
                        // table, to prevent Lua panic errors
      } else
        lua_rawset(gL, -3);
    }
    lua_pop(gL, 1);
...
void LUA_Archive(void) {
  ...
  for (i = 0; i < MAXPLAYERS; i++) {
    if (!playeringame[i] &&
        i > 0) // NEVER skip player 0, this is for dedi servs.
      continue;
...
void LUA_UnArchive(void) {
  ...
  for (i = 0; i < MAXPLAYERS; i++) {
    if (!playeringame[i] &&
        i > 0) // same here, this is to synch dediservs properly.
      continue;
  ...
  LUAh_NetArchiveHook(
      NetUnArchive); // call the NetArchive hook in unarchive mode
  UnArchiveTables();
...
int Lua_optoption(lua_State *L, int narg, const char *def,
