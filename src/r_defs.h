...
// Indeed, true color support is possible precalculating 24bpp lightmap/colormap
// LUT from darkening PLAYPAL to all black. Could even use more than 32 levels.
...
typedef enum {
  ...
  FF_CRUMBLE =
      0x100000, ///< Falls 2 seconds after being stepped on, and randomly brings
                ///< all touching crumbling 3dfloors down with it, providing
                ///< their master sectors share the same tag (allows crumble
                ///< platforms above or below, to also exist).
  FF_SHATTERBOTTOM =
      0x200000, ///< Used with ::FF_BUSTUP. Like FF_SHATTER, but only breaks
                ///< from the bottom. Good for springing up through rubble.
  FF_MARIO =
      0x400000, ///< Acts like a question block when hit from underneath. Goodie
                ///< spawned at top is determined by master sector.
  FF_BUSTUP =
      0x800000, ///< You can spin through/punch this block and it will crumble!
  FF_QUICKSAND = 0x1000000,       ///< Quicksand!
  FF_PLATFORM = 0x2000000,        ///< You can jump up through this to the top.
  FF_REVERSEPLATFORM = 0x4000000, ///< A fall-through floor in normal gravity, a
                                  ///< platform in reverse gravity.
  FF_INTANGABLEFLATS =
      0x6000000, ///< Both flats are intangable, but the sides are still solid.
  FF_SHATTER =
      0x8000000, ///< Used with ::FF_BUSTUP. Thinks everyone's Knuckles.
  FF_SPINBUST = 0x10000000, ///< Used with ::FF_BUSTUP. Jump or fall onto it
                            ///< while curled in a ball.
  FF_ONLYKNUX =
      0x20000000, ///< Used with ::FF_BUSTUP. Only Knuckles can break this rock.
  FF_RIPPLE = 0x40000000,       ///< Ripple the flats
  FF_COLORMAPONLY = 0x80000000, ///< Only copy the colormap, not the lightlevel
  FF_GOOWATER =
      FF_SHATTERBOTTOM, ///< Used with ::FF_SWIMMABLE. Makes thick bouncey goop.
} ffloortype_e;
...
typedef struct lightlist_s {
  ...
  INT32 flags;
  ffloor_t *caster;
  struct pslope_s *slope; // FF_DOUBLESHADOW makes me have to store this pointer
                          // here. Bluh bluh.
} lightlist_t;
...
typedef enum {
  ...
  SL_NODYNAMIC = 1 << 1, // Slope will never need to move during the level, so
                         // don't fuss with recalculating it
  SL_ANCHORVERTEX =
      1 << 2, // Slope is using a Slope Vertex Thing to anchor its position
  SL_VERTEXSLOPE = 1 << 3, // Slope is built from three Slope Vertex Things
} slopeflags_t;

typedef struct pslope_s {
  UINT16
  id; // The number of the slope, mostly used for netgame syncing purposes
  ...
  // the origin in 2d mapspace. (Basically a thrust of FRACUNIT in xydirection
  // angle)
  vector2_t d;
  ...
  // The normal of the slope; will always point upward, and thus be inverted on
  // ceilings. I think it's only needed for physics? -Red
  vector3_t normal;
  ...
  angle_t zangle; // Angle of the plane going up from the ground (not mesured in
                  // degrees)
  angle_t xydirection; // The direction the slope is facing (north, west, south,
                       // etc.)

  struct line_s *sourceline; // The line that generated the slope
  fixed_t extent;            // Distance value used for recalculating zdelta
  UINT8 refpos; // 1=front floor 2=front ceiling 3=back floor 4=back ceiling
                // (used for dynamic sloping)

  UINT8 flags; // Slope options
  mapthing_t *
      *vertices; // List should be three long for slopes made by vertex things,
                 // or one long for slopes using one vertex thing to anchor

  struct pslope_s
      *next; // Make a linked list of dynamic slopes, for easy reference later
} pslope_t;
...
typedef struct sector_s {
  ...
  // these are saved for netgames, so do not let Lua touch these!
  INT32 spawn_nexttag, spawn_firsttag; // the actual nexttag/firsttag values may
                                       // differ if the sector's tag was changed
...
typedef struct line_s {
  ...
  char *text;      // a concatination of all front and back texture names, for
                   // linedef specials that require a string.
  INT16 callcount; // no. of calls left before triggering, for the "X calls"
                   // linedef specials, defaults to 0
} line_t;
...
typedef struct {
  ...
  char *text; // a concatination of all top, bottom, and mid texture names, for
              // linedef specials that require a string.
} side_t;
...
// starts at touching_thinglist in a sector_t and flows through the
// m_thinglist_next links to find all mobjs that are entirely or partially in
// the sector. The second thread starts at touching_sectorlist in an mobj_t and
// flows through the m_sectorlist_next links to find all sectors a thing
// touches. This is useful when applying friction or push effects to sectors.
// These effects can be done as thinkers that act upon all objects touching
// their sectors. As an mobj moves through the world, these nodes are created
// and destroyed, with the links changed appropriately.
//
...
typedef struct seg_s {
  ...
  // Sector references.
  // Could be retrieved from linedef, too. backsector is NULL for one sided
  // lines
  sector_t *frontsector;
  sector_t *backsector;
...
// Horizontal flipping is used to save space, thus NNNNF2F5 defines a mirrored
// patch. Some sprites will only have one picture used for all views: NNNNF0
