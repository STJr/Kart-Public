...
// NOTE: This sits here and not in w_wad.c because p_setup.c makes use of it to
// load map WADs inside PK3s.
#if defined(_MSC_VER)
#pragma pack(1)
#endif
typedef struct {
...
typedef struct {
  ...
  char
      *fullname; //                   e.g.
                 //                   "Folder/Subfolder/LongEntryName.extension"
...
// Replay code relies on it being an UINT8. There are no SINT8s handling WAD
// indices, though. Can be set all the way up to 255 but not 256, because an
// UINT8 will never be >= 256, probably breaking some conditionals.
...
// Resource type of the WAD. Yeah, I know this sounds dumb, but I'll leave it
// like this until I clean up the code further.
...
// Opens a WAD file. Returns the FILE * handle for the file, or NULL if not
// found or could not be opened
FILE *W_OpenWadFile(const char **filename, boolean useerrors);
// Load and add a wadfile to the active wad files, returns numbers of lumps,
// INT16_MAX on error
UINT16
...
// so that it stops with a message if a file was not found, but not if all is
// okay.
INT32
...
UINT16
W_CheckNumForNamePwad(const char *name, UINT16 wad,
                      UINT16 startlump); // checks only in one pwad
UINT16
...
lumpnum_t W_CheckNumForNameInBlock(const char *name, const char *blockstart,
                                   const char *blockend);
UINT8
W_LumpExists(const char *name); // Lua uses this.
...
size_t W_ReadLumpHeaderPwad(UINT16 wad, UINT16 lump, void *dest, size_t size,
                            size_t offset);
size_t W_ReadLumpHeader(lumpnum_t lump, void *dest, size_t size,
                        size_t offest); // read all or a part of a lump
...
#ifdef HWRENDER
// void *W_CachePatchNumPwad(UINT16 wad, UINT16 lump, INT32 tag); // return a
// patch_t
