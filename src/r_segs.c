...
static INT32 worldtopslope, worldbottomslope, worldhighslope,
    worldlowslope; // worldtop/bottom at end of slope
static fixed_t rw_toptextureslide, rw_midtextureslide,
    rw_bottomtextureslide; // Defines how to adjust Y offsets along the wall for
                           // slopes
static fixed_t rw_midtextureback,
    rw_midtexturebackslide; // Values for masked midtexture height calculation
...
static void R_DrawWallSplats(void) {
  ...
  for (; splat; splat = splat->next) {
    ...
    dc_texturemid =
        splat->top + (SHORT(patch->height) << (FRACBITS - 1)) - viewz;
    if (splat->yoffset)
    ...
    for (dc_x = x1; dc_x <= x2; dc_x++, spryscale += rw_scalestep) {
      ...
      if (frontsector->extra_colormap)
        dc_colormap =
            frontsector->extra_colormap->colormap + (dc_colormap - colormaps);
      ...
      // draw the texture
      col =
          (column_t *)((UINT8 *)patch + LONG(patch->columnofs[texturecolumn]));
...
static INT32 column2s_length; // column->length : for multi-patch on 2sided wall
                              // = texture->height
...
static void R_Render2sidedMultiPatchColumn(column_t *column) {
  ...
  topscreen =
      sprtopscreen; // + spryscale*column->topdelta;  topdelta is 0 for the wall
...
void R_RenderMaskedSegRange(drawseg_t *ds, INT32 x1, INT32 x2) {
  ...
  if (curline->polyseg && curline->polyseg->translucency > 0) {
    ...
    dc_transmap =
        transtables + ((curline->polyseg->translucency - 1) << FF_TRANSSHIFT);
    colfunc = fuzzcolfunc;
  }
  ...
  // handle case where multipatch texture is drawn on a 2sided wall, multi-patch
  // textures are not stored per-column with post info in SRB2
  if (textures[texnum]->holes)
    colfunc_2s = R_DrawMaskedColumn; // render the usual 2sided single-patch
                                     // packed texture
  else {
    colfunc_2s = R_Render2sidedMultiPatchColumn; // render multipatch with no
                                                 // holes (no post_t info)
  ...
  if (frontsector->numlights) {
    ...
    for (i = 0; i < dc_numlights; i++) {
      ...
      rlight->startheight =
          rlight->height; // keep starting value here to reset for each repeat
  ...
  for (times = 0; times < repeats; times++) {
    if (times > 0) {
      rw_scalestep = ds->scalestep;
      spryscale = ds->scale1 + (x1 - ds->x1) * rw_scalestep;
      if (dc_numlights) { // reset all lights to their starting heights
        for (i = 0; i < dc_numlights; i++) {
          rlight = &dc_lightlist[i];
    ...
    for (dc_x = x1; dc_x <= x2; dc_x++) {
      ...
      if (!!(curline->linedef->flags & ML_DONTPEGBOTTOM) ^
          !!(curline->linedef->flags & ML_EFFECT3))
        dc_texturemid +=
            (textureheight[texnum]) * times + textureheight[texnum];
      else
        dc_texturemid -= (textureheight[texnum]) * times;
      // calculate lighting
      if (maskedtexturecol[dc_x] != INT16_MAX) {
        // Check for overflows first
        overflow_test = (INT64)centeryfrac -
                        (((INT64)dc_texturemid * spryscale) >> FRACBITS);
        if (overflow_test < 0)
        ...
        if (dc_numlights) {
          ...
          realbot = windowbottom =
              FixedMul(textureheight[texnum], spryscale) + sprtopscreen;
          dc_iscale = 0xffffffffu / (unsigned)spryscale;

          // draw the texture
          col = (column_t *)((UINT8 *)R_GetColumn(texnum,
          ...
          for (i = 0; i < dc_numlights; i++) {
            ...
            pindex = FixedMul(spryscale, FixedDiv(640, vid.width)) >>
                     LIGHTSCALESHIFT;
            ...
            if (rlight->extra_colormap)
              rlight->rcolormap = rlight->extra_colormap->colormap +
                                  (xwalllights[pindex] - colormaps);
            else
        ...
        // draw the texture
        col =
            (column_t *)((UINT8 *)R_GetColumn(texnum, maskedtexturecol[dc_x]) -
                         3);

//#ifdef POLYOBJECTS_PLANES
#if 0 // Disabling this allows inside edges to render below the planes, for
      // until the clipping is fixed to work right when POs are near the camera.
      // -Red
...
static void R_DrawRepeatMaskedColumn(column_t *col) {
  while (sprtopscreen < sprbotscreen) {
    ...
    if ((INT64)sprtopscreen + dc_texheight * spryscale >
        (INT64)INT32_MAX) // prevent overflow
...
void R_RenderThickSideRange(drawseg_t *ds, INT32 x1, INT32 x2,
                            ffloor_t *pfloor) {
  size_t pindex;
  column_t *col;
  INT32 lightnum;
  ...
  INT32 range;
  line_t *newline = NULL;
  // Render FOF sides kinda like normal sides, with the frac and step and
  // everything NOTE: INT64 instead of fixed_t because overflow concerns
  ...
  frontsector = curline->frontsector == pfloor->target ? curline->backsector
                                                       : curline->frontsector;
  ...
  range = max(ds->x2 - ds->x1, 1);
  // SoM: Moved these up here so they are available for my lightlist
  // calculations
  rw_scalestep = ds->scalestep;
  spryscale = ds->scale1 + (x1 - ds->x1) * rw_scalestep;
...
#define SLOPEPARAMS(slope, end1, end2, normalheight)                           \
  if (slope) {                                                                 \
    ...
    end1 = end2 = normalheight;
      ...
      SLOPEPARAMS(*pfloor->b_slope, pfloorleft, pfloorright,
                  *pfloor->bottomheight)
      ...
      if (leftheight > pfloorleft && rightheight > pfloorright &&
        ...
        if ((nextlight->slope
                 ? P_GetZAt(nextlight->slope, ds->leftpos.x, ds->leftpos.y)
                 : nextlight->height) > pfloorleft &&
            (nextlight->slope
                 ? P_GetZAt(nextlight->slope, ds->rightpos.x, ds->rightpos.y)
                 : nextlight->height) > pfloorright)
          continue;
      }
...
#define CLAMPMIN                                                               \
  (-INT32_MAX) // This is not INT32_MIN on purpose! INT32_MIN makes the drawers
               // freak out.
      // Monster Iestyn (25/03/18): do not skip these lights if they fail
      // overflow test, just clamp them instead so they behave.
      overflow_test =
          (INT64)centeryfrac - (((INT64)leftheight * ds->scale1) >> FRACBITS);
      if (overflow_test > (INT64)CLAMPMAX)
        rlight->height = CLAMPMAX;
      else if (overflow_test > (INT64)CLAMPMIN)
        rlight->height = (fixed_t)overflow_test;
      else
        rlight->height = CLAMPMIN;

      overflow_test =
          (INT64)centeryfrac - (((INT64)rightheight * ds->scale2) >> FRACBITS);
      if (overflow_test > (INT64)CLAMPMAX)
        rlight->heightstep = CLAMPMAX;
      else if (overflow_test > (INT64)CLAMPMIN)
        rlight->heightstep = (fixed_t)overflow_test;
      else
        rlight->heightstep = CLAMPMIN;
      rlight->heightstep = (rlight->heightstep - rlight->height) / (range);
      rlight->flags = light->flags;
      if (light->flags & FF_CUTLEVEL) {
        SLOPEPARAMS(*light->caster->b_slope, leftheight, rightheight,
...
#undef SLOPEPARAMS
        ...
        overflow_test =
            (INT64)centeryfrac - (((INT64)leftheight * ds->scale1) >> FRACBITS);
        if (overflow_test > (INT64)CLAMPMAX)
          rlight->botheight = CLAMPMAX;
        else if (overflow_test > (INT64)CLAMPMIN)
          rlight->botheight = (fixed_t)overflow_test;
        else
          rlight->botheight = CLAMPMIN;

        overflow_test = (INT64)centeryfrac -
                        (((INT64)rightheight * ds->scale2) >> FRACBITS);
        if (overflow_test > (INT64)CLAMPMAX)
          rlight->botheightstep = CLAMPMAX;
        else if (overflow_test > (INT64)CLAMPMIN)
          rlight->botheightstep = (fixed_t)overflow_test;
        else
          rlight->botheightstep = CLAMPMIN;
        rlight->botheightstep =
            (rlight->botheightstep - rlight->botheight) / (range);
      }
      ...
      if (pfloor->flags & FF_FOG)
        rlight->lightnum =
            (pfloor->master->frontsector->lightlevel >> LIGHTSEGSHIFT);
      else
        rlight->lightnum = (rlight->lightlevel >> LIGHTSEGSHIFT);

      if (pfloor->flags & FF_FOG || rlight->flags & FF_FOG ||
          (rlight->extra_colormap && rlight->extra_colormap->fog))
        ;
      else if (curline->v1->y == curline->v2->y)
        rlight->lightnum--;
    ...
    if ((frontsector->extra_colormap && frontsector->extra_colormap->fog))
      ...
      lightnum =
          R_FakeFlat(frontsector, &tempsec, &templight, &templight, false)
              ->lightlevel >>
          LIGHTSEGSHIFT;
  ...
  if (*pfloor->b_slope)
    left_bottom =
        P_GetZAt(*pfloor->b_slope, ds->leftpos.x, ds->leftpos.y) - viewz;
  else
  ...
  if (slopeskew) {
    angle_t lineangle = R_PointToAngle2(curline->v1->x, curline->v1->y,
                                        curline->v2->x, curline->v2->y);

    if (skewslope)
      ffloortextureslide = FixedMul(
          skewslope->zdelta,
          FINECOSINE((lineangle - skewslope->xydirection) >> ANGLETOFINESHIFT));
  }
  ...
  // otherwise texture[texnum]->holes may be false when it shouldn't be
  R_CheckTextureCache(texnum);
  // faB: handle case where multipatch texture is drawn on a 2sided wall,
  // multi-patch textures
  //      are not stored per-column with post info anymore in Doom Legacy
  if (textures[texnum]->holes)
    colfunc_2s = R_DrawRepeatMaskedColumn; // render the usual 2sided
                                           // single-patch packed texture
  else {
    colfunc_2s = R_Render2sidedMultiPatchColumn; // render multipatch with no
                                                 // holes (no post_t info)
    column2s_length = textures[texnum]->height;
  ...
  // Set heights according to plane, or slope, whichever
  {
    ...
    if (*pfloor->t_slope)
      right_top =
          P_GetZAt(*pfloor->t_slope, ds->rightpos.x, ds->rightpos.y) - viewz;
    else
      right_top = *pfloor->topheight - viewz;

    if (*pfloor->b_slope)
      right_bottom =
          P_GetZAt(*pfloor->b_slope, ds->rightpos.x, ds->rightpos.y) - viewz;
    else
      right_bottom = *pfloor->bottomheight - viewz;

    // using INT64 to avoid 32bit overflow
    top_frac =
        (INT64)centeryfrac - (((INT64)left_top * ds->scale1) >> FRACBITS);
    bottom_frac =
        (INT64)centeryfrac - (((INT64)left_bottom * ds->scale1) >> FRACBITS);
    top_step =
        (INT64)centeryfrac - (((INT64)right_top * ds->scale2) >> FRACBITS);
    bottom_step =
        (INT64)centeryfrac - (((INT64)right_bottom * ds->scale2) >> FRACBITS);
  ...
  for (dc_x = x1; dc_x <= x2; dc_x++) {
    if (maskedtexturecol[dc_x] != INT16_MAX) {
      if (ffloortextureslide) { // skew FOF walls
        if (oldx != -1)
          dc_texturemid += FixedMul(
              ffloortextureslide,
              (maskedtexturecol[oldx] - maskedtexturecol[dc_x]) << FRACBITS);
        oldx = dc_x;
      }
      // Calculate bounds
      // clamp the values if necessary to avoid overflows and rendering glitches
      // caused by them

      if (top_frac > (INT64)CLAMPMAX)
        sprtopscreen = windowtop = CLAMPMAX;
      else if (top_frac > (INT64)CLAMPMIN)
        sprtopscreen = windowtop = (fixed_t)top_frac;
      else
        sprtopscreen = windowtop = CLAMPMIN;
      if (bottom_frac > (INT64)CLAMPMAX)
        sprbotscreen = windowbottom = CLAMPMAX;
      else if (bottom_frac > (INT64)CLAMPMIN)
        sprbotscreen = windowbottom = (fixed_t)bottom_frac;
      else
        sprbotscreen = windowbottom = CLAMPMIN;
      ...
      col = (column_t *)((UINT8 *)R_GetColumn(texnum, maskedtexturecol[dc_x]) -
                         3);
      ...
      if (dc_numlights) {
        ...
        for (i = 0; i < dc_numlights; i++) {
          ...
          if (lighteffect) {
            ...
            if (pfloor->flags & FF_FOG) {
              if (pfloor->master->frontsector->extra_colormap)
                rlight->rcolormap =
                    pfloor->master->frontsector->extra_colormap->colormap +
                    (xwalllights[pindex] - colormaps);
              else
                rlight->rcolormap = xwalllights[pindex];
            } else {
          ...
          solid =
              0; // don't carry over solid-cutting flag from the previous light
          ...
          if (rlight->flags & FF_CUTSOLIDS && !(pfloor->flags & FF_EXTRA))
            ...
            if (rlight->flags & FF_EXTRA) {
              ...
              if ((rlight->flags & (FF_FOG | FF_SWIMMABLE)) ==
                  (pfloor->flags & (FF_FOG | FF_SWIMMABLE)))
                solid = 1;
            } else
              solid = 1;
          } else
      ...
      if (pfloor->flags & FF_FOG && pfloor->master->frontsector->extra_colormap)
        dc_colormap = pfloor->master->frontsector->extra_colormap->colormap +
...
static void R_RenderSegLoop(void) {
  ...
  for (; rw_x < rw_stopx; rw_x++) {
    ...
    if (numffloors) {
      ...
      for (i = 0; i < numffloors; i++) {
        if (ffloor[i].polyobj &&
            (!curline->polyseg || ffloor[i].polyobj != curline->polyseg))
          continue;

        // FIXME hack to fix planes disappearing when a seg goes behind the
        // camera. This NEEDS to be changed to be done properly. -Red
        if (curline->polyseg) {
        ...
        if (ffloor[i].height < viewz) {
          ...
          if (curline->polyseg && ffloor[i].polyobj &&
              ffloor[i].polyobj == curline->polyseg && top_w >= bottom_w) {
    ...
    if (dc_numlights) {
      ...
      for (i = 0; i < dc_numlights; i++) {
        ...
        if (dc_lightlist[i].extra_colormap)
          dc_lightlist[i].rcolormap = dc_lightlist[i].extra_colormap->colormap +
                                      (xwalllights[pindex] - colormaps);
        else
    ...
    if (midtexture) {
      ...
      if (yl <= yh && yh >= 0 && yl < viewheight) {
        ...
        // note: don't use min/max macros, since casting from INT32 to INT16 is
        // involved here
        if (markceiling)
          ceilingclip[rw_x] = (yl >= 0)
                                  ? ((yl > viewheight) ? (INT16)viewheight
                                                       : (INT16)((INT16)yl - 1))
                                  : -1;
        if (markfloor)
          floorclip[rw_x] = (yh < viewheight)
                                ? ((yh < -1) ? -1 : (INT16)((INT16)yh + 1))
                                : (INT16)viewheight;
      }
    } else {
      // two sided line
    ...
    for (i = 0; i < numbackffloors; i++) {
      ffloor[i].f_clip[rw_x] = ffloor[i].c_clip[rw_x] =
          (INT16)((ffloor[i].b_frac >> HEIGHTBITS) & 0xFFFF);
...
void R_StoreWallRange(INT32 start, INT32 stop) {
  ...
  // SoM: Code to remove limits on openings.
  {
    ...
    if (need > maxopenings) {
      ...
      openings =
          Z_Realloc(openings, maxopenings * sizeof(*openings), PU_STATIC, NULL);
...
#undef ADJUST
  ...
  // Figure out map coordinates of where start and end are mapping to on seg, so
  // we can clip right for slope bullshit
  if (frontsector->hasslope ||
      (backsector &&
       backsector->hasslope)) // Commenting this out for FOFslop. -Red
  {
    angle_t temp;
    ...
    {
      // Both lines can be written in slope-intercept form, so figure out line
      // intersection
      float a1, b1, c1, a2, b2, c2,
          det; // 1 is the seg, 2 is the view angle vector...
      /// TODO: convert to FPU

      a1 = FIXED_TO_FLOAT(curline->v2->y - curline->v1->y);
      b1 = FIXED_TO_FLOAT(curline->v1->x - curline->v2->x);
      c1 = a1 * FIXED_TO_FLOAT(curline->v1->x) +
           b1 * FIXED_TO_FLOAT(curline->v1->y);
  ...
  if (frontsector->c_slope) {
    ...
    worldtopslope =
        P_GetZAt(frontsector->c_slope, segright.x, segright.y) - viewz;
  } else {
    worldtopslope = worldtop = frontsector->ceilingheight - viewz;
  }

  if (frontsector->f_slope) {
    worldbottom = P_GetZAt(frontsector->f_slope, segleft.x, segleft.y) - viewz;
    worldbottomslope =
        P_GetZAt(frontsector->f_slope, segright.x, segright.y) - viewz;
  } else {
    worldbottomslope = worldbottom = frontsector->floorheight - viewz;
  }
  ...
  if (numffloors) {
    for (i = 0; i < numffloors; i++) {
      if (ffloor[i].polyobj && (!ds_p->curline->polyseg ||
                                ffloor[i].polyobj != ds_p->curline->polyseg))
        continue;

      if (ffloor[i].slope) {
        ffloor[i].f_pos =
            P_GetZAt(ffloor[i].slope, segleft.x, segleft.y) - viewz;
        ffloor[i].f_pos_slope =
            P_GetZAt(ffloor[i].slope, segright.x, segright.y) - viewz;
      } else
...

  {
    ...
    if (frontsector->f_slope)
      floorfrontslide =
          FixedMul(frontsector->f_slope->zdelta,
                   FINECOSINE((lineangle - frontsector->f_slope->xydirection) >>
                              ANGLETOFINESHIFT));

    if (frontsector->c_slope)
      ceilingfrontslide =
          FixedMul(frontsector->c_slope->zdelta,
                   FINECOSINE((lineangle - frontsector->c_slope->xydirection) >>
                              ANGLETOFINESHIFT));

    if (backsector && backsector->f_slope)
      floorbackslide =
          FixedMul(backsector->f_slope->zdelta,
                   FINECOSINE((lineangle - backsector->f_slope->xydirection) >>
                              ANGLETOFINESHIFT));

    if (backsector && backsector->c_slope)
      ceilingbackslide =
          FixedMul(backsector->c_slope->zdelta,
                   FINECOSINE((lineangle - backsector->c_slope->xydirection) >>
                              ANGLETOFINESHIFT));
  }
  ...
  if (!backsector) {
    ...
    if (backsector->c_slope) {
      ...
      worldhighslope =
          P_GetZAt(backsector->c_slope, segright.x, segright.y) - viewz;
    } else {
      worldhighslope = worldhigh = backsector->ceilingheight - viewz;
    }

    if (backsector->f_slope) {
      worldlow = P_GetZAt(backsector->f_slope, segleft.x, segleft.y) - viewz;
      worldlowslope =
          P_GetZAt(backsector->f_slope, segright.x, segright.y) - viewz;
    } else {
    ...
    if (worldbottomslope > worldlowslope || worldbottom > worldlow) {
      ...
      if ((backsector->f_slope ? P_GetZAt(backsector->f_slope, viewx, viewy)
                               : backsector->floorheight) > viewz)
        ds_p->bsilheight = INT32_MAX;
      else
        ds_p->bsilheight =
            (frontsector->f_slope ? INT32_MAX : frontsector->floorheight);
    } else if ((backsector->f_slope
                    ? P_GetZAt(backsector->f_slope, viewx, viewy)
                    : backsector->floorheight) > viewz) {
      ds_p->silhouette = SIL_BOTTOM;
      ds_p->bsilheight = INT32_MAX;
      // ds_p->sprbottomclip = negonearray;
    ...
    if (worldtopslope < worldhighslope || worldtop < worldhigh) {
      ...
      if ((backsector->c_slope ? P_GetZAt(backsector->c_slope, viewx, viewy)
                               : backsector->ceilingheight) < viewz)
        ds_p->tsilheight = INT32_MIN;
      else
        ds_p->tsilheight =
            (frontsector->c_slope ? INT32_MIN : frontsector->ceilingheight);
    } else if ((backsector->c_slope
                    ? P_GetZAt(backsector->c_slope, viewx, viewy)
                    : backsector->ceilingheight) < viewz) {
      ds_p->silhouette |= SIL_TOP;
      ds_p->tsilheight = INT32_MIN;
      // ds_p->sprtopclip = screenheightarray;
    ...
    if (viewsector != frontsector && viewsector != backsector) {
      ...
      //  frontsector->ceiling and backsector->floor to see if a door was
      //  closed. Without the following code, sprites get displayed behind
      //  closed doors.
      {
        if (doorclosed ||
        ...
        if (doorclosed ||
            (worldlow >= worldtop &&
             worldlowslope >= worldtopslope)) { // killough 1/17/98, 2/8/98
          ds_p->sprtopclip = screenheightarray;
          ds_p->tsilheight = INT32_MIN;
          ds_p->silhouette |= SIL_TOP;
        }
    ...
    if (worldhigh < worldtop || worldhighslope < worldtopslope) {
      ...
      if ((linedef->flags & (ML_DONTPEGTOP) &&
        ...
        // Special case... use offsets from 2nd side but only if it has a
        // texture.
        side_t *def = &sides[linedef->sidenum[1]];
        toptexture = R_GetTextureNum(def->toptexture);

        if (!toptexture) // Second side has no texture, use the first side's
                         // instead.
      ...
      if (!(linedef->flags & ML_EFFECT1)) { // Ignore slopes for lower/upper
                                            // textures unless flag is checked
        if (linedef->flags & ML_DONTPEGTOP)
    ...
    if (worldlow > worldbottom ||
        worldlowslope > worldbottomslope) // seulement si VISIBLE!!!
    {
      // bottom texture
    ...
    if (frontsector && backsector && frontsector->tag != backsector->tag &&
      ...
      fixed_t high1, highslope1, low1, lowslope1, high2, highslope2, low2,
          lowslope2;
      ...
      if (frontsector->ffloors && backsector->ffloors) {
        i = 0;
        for (rover = backsector->ffloors; rover && i < MAXFFLOORS;
             rover = rover->next) {
        ...
        for (rover = frontsector->ffloors; rover && i < MAXFFLOORS;
             rover = rover->next) {
          if (!(rover->flags & FF_RENDERSIDES) || !(rover->flags & FF_EXISTS))
            continue;
        ...
        for (rover = backsector->ffloors, i = 0; rover && i < MAXFFLOORS;
             rover = rover->next) {
          if (!(rover->flags & FF_RENDERSIDES) || !(rover->flags & FF_EXISTS) ||
          ...
          // Oy vey.
          if (((*rover->t_slope
                    ? P_GetZAt(*rover->t_slope, segleft.x, segleft.y)
                    : *rover->topheight) <= worldbottom + viewz &&
               (*rover->t_slope
                    ? P_GetZAt(*rover->t_slope, segright.x, segright.y)
                    : *rover->topheight) <= worldbottomslope + viewz) ||
              ((*rover->b_slope
                    ? P_GetZAt(*rover->b_slope, segleft.x, segleft.y)
                    : *rover->bottomheight) >= worldtop + viewz &&
               (*rover->b_slope
                    ? P_GetZAt(*rover->b_slope, segright.x, segright.y)
                    : *rover->bottomheight) >= worldtopslope + viewz))
            continue;
        ...
        for (rover = frontsector->ffloors, i = 0; rover && i < MAXFFLOORS;
             rover = rover->next) {
          if (!(rover->flags & FF_RENDERSIDES) || !(rover->flags & FF_EXISTS) ||
          ...
          if (((*rover->t_slope
                    ? P_GetZAt(*rover->t_slope, segleft.x, segleft.y)
                    : *rover->topheight) <= worldlow + viewz &&
               (*rover->t_slope
                    ? P_GetZAt(*rover->t_slope, segright.x, segright.y)
                    : *rover->topheight) <= worldlowslope + viewz) ||
              ((*rover->b_slope
                    ? P_GetZAt(*rover->b_slope, segleft.x, segleft.y)
                    : *rover->bottomheight) >= worldhigh + viewz &&
               (*rover->b_slope
                    ? P_GetZAt(*rover->b_slope, segright.x, segright.y)
                    : *rover->bottomheight) >= worldhighslope + viewz))
            continue;
    ...
    if (sidedef->midtexture > 0 && sidedef->midtexture < numtextures) {
      ...
      maskedtextureheight =
          ds_p->maskedtextureheight; // note to red, this ==
                                     // &(ds_p->maskedtextureheight[0])

      if (curline->polyseg) { // use REAL front and back floors please, so
                              // midtexture rendering isn't mucked up
        rw_midtextureslide = rw_midtexturebackslide = 0;
        if (!!(linedef->flags & ML_DONTPEGBOTTOM) ^
            !!(linedef->flags & ML_EFFECT3))
          rw_midtexturemid = rw_midtextureback =
              max(curline->frontsector->floorheight,
                  curline->backsector->floorheight) -
              viewz;
        else
          rw_midtexturemid = rw_midtextureback =
              min(curline->frontsector->ceilingheight,
        ...
        if (linedef->flags & ML_EFFECT2) { // Ignore slopes when texturing
          ...
          if (!!(linedef->flags & ML_DONTPEGBOTTOM) ^
              !!(linedef->flags & ML_EFFECT3))
            rw_midtexturemid = rw_midtextureback =
                max(frontsector->floorheight, backsector->floorheight) - viewz;
          else
            rw_midtexturemid = rw_midtextureback =
                min(frontsector->ceilingheight, backsector->ceilingheight) -
                viewz;
  ...
  if (frontsector->heightsec == -1) {
    if ((frontsector->f_slope ? P_GetZAt(frontsector->f_slope, viewx, viewy)
                              : frontsector->floorheight) >= viewz) {
      // above view plane
      markfloor = false;
    }

    if ((frontsector->c_slope ? P_GetZAt(frontsector->c_slope, viewx, viewy)
  ...
  if (linedef->special == 41) { // HORIZON LINES
    ...
    if (frontsector->c_slope) {
      fixed_t topfracend =
          (centeryfrac >> 4) - FixedMul(worldtopslope, ds_p->scale2);
      topstep = (topfracend - topfrac) / (range);
    }
    if (frontsector->f_slope) {
      fixed_t bottomfracend =
          (centeryfrac >> 4) - FixedMul(worldbottomslope, ds_p->scale2);
  ...
  if (frontsector->numlights) {
    ...
    for (i = p = 0; i < dc_numlights; i++) {
      ...
      rlight->heightstep =
          (centeryfrac >> 4) - FixedMul(rightheight, ds_p->scale2);
      rlight->heightstep = (rlight->heightstep - rlight->height) / (range);
      rlight->flags = light->flags;
      ...
      if (light->caster && light->caster->flags & FF_CUTSOLIDS) {
        if (*light->caster->b_slope) {
          ...
          rightheight =
              P_GetZAt(*light->caster->b_slope, segright.x, segright.y);
  ...
  if (numffloors) {
    for (i = 0; i < numffloors; i++) {
      ...
      ffloor[i].f_frac =
          (centeryfrac >> 4) - FixedMul(ffloor[i].f_pos, rw_scale);
      ffloor[i].f_step =
          ((centeryfrac >> 4) - FixedMul(ffloor[i].f_pos_slope, ds_p->scale2) -
           ffloor[i].f_frac) /
          (range);
    }
  }
  ...
  if (backsector) {
    ...
    if (toptexture) {
      ...
      if (backsector->c_slope) {
        fixed_t topfracend =
            (centeryfrac >> 4) - FixedMul(worldhighslope, ds_p->scale2);
    ...
    if (bottomtexture) {
      ...
      if (backsector->f_slope) {
        fixed_t bottomfracend =
            (centeryfrac >> 4) - FixedMul(worldlowslope, ds_p->scale2);
...

    {
      ...
      if (backsector->ffloors) {
        for (rover = backsector->ffloors; rover && i < MAXFFLOORS;
          ...
          roverleft =
              (*rover->b_slope ? P_GetZAt(*rover->b_slope, segleft.x, segleft.y)
                               : *rover->bottomheight) -
              viewz;
          roverright = (*rover->b_slope
                            ? P_GetZAt(*rover->b_slope, segright.x, segright.y)
                            : *rover->bottomheight) -
                       viewz;
          planevistest =
          ...
          if ((roverleft >> 4 <= worldhigh ||
               roverright >> 4 <= worldhighslope) &&
              (roverleft >> 4 >= worldlow ||
               roverright >> 4 >= worldlowslope) &&
            ...
            ffloor[i].b_frac =
                (centeryfrac >> 4) - FixedMul(ffloor[i].b_pos, rw_scale);
            ffloor[i].b_step = (centeryfrac >> 4) -
                               FixedMul(ffloor[i].b_pos_slope, ds_p->scale2);
          ...
          roverleft =
              (*rover->t_slope ? P_GetZAt(*rover->t_slope, segleft.x, segleft.y)
                               : *rover->topheight) -
              viewz;
          roverright = (*rover->t_slope
                            ? P_GetZAt(*rover->t_slope, segright.x, segright.y)
                            : *rover->topheight) -
                       viewz;
          planevistest =
      ...
      if (curline->polyseg && frontsector &&
          (curline->polyseg->flags & POF_RENDERPLANES)) {
  ...
  if (markceiling) {
    ...
    // Don't mark ceiling flat lines for polys unless this line has an upper
    // texture, otherwise we get flat leakage pulling downward (If it DOES have
    // an upper texture and we do this, the ceiling won't render at all)
  ...
  if (markfloor) {
    ...
    // Don't mark floor flat lines for polys unless this line has a lower
    // texture, otherwise we get flat leakage pulling upward (If it DOES have a
    // lower texture and we do this, the floor won't render at all)
  ...
  if (maskedtexture && !(ds_p->silhouette & SIL_TOP)) {
    ds_p->silhouette |= SIL_TOP;
    ds_p->tsilheight =
        (sidedef->midtexture > 0 && sidedef->midtexture < numtextures)
            ? INT32_MIN
            : INT32_MAX;
  }
