...
// SOME numpty changed all the gametype constants and it fell out of sync with
// vanilla and now we have to pretend to be vanilla when talking to the master
// server...
...
typedef enum {
  ...
  PT_RESYNCHEND, // Player is now resynched and is being requested to remake the
                 // gametic
  ...
  PT_CLIENT4CMD, // 4P
  PT_CLIENT4MIS,
  PT_BASICKEEPALIVE, // Keep the network alive during wipes, as tics aren't
                     // advanced and NetUpdate isn't called
  ...
  PT_TELLFILESNEEDED, // Client, to server: "what other files do I need starting
                      // from this number?"
  PT_MOREFILESNEEDED, // Server, to client: "you need these (+ more on top of
                      // those)"
...
typedef struct {
  ...
  INT32 ctfteam[MAXPLAYERS]; // Which team? (can't be 1 bit, since in regular
                             // Match there are no teams)
...
typedef struct {
  ...
  */
  ...
  UINT8 modifiedgame;
  UINT8 cheatsenabled;
  UINT8 kartvars; // Previously isdedicated, now appropriated for our own
                  // nefarious purposes
  UINT8 fileneedednum;
  tic_t time;
  tic_t leveltime;
  ...
  UINT8 actnum;
  UINT8 iszone;
  char httpsource[MAX_MIRROR_LENGTH]; // HTTP URL to download from, always
...
typedef struct {
  ...
  UINT8 address[4]; // sending another string would run us up against
                    // MAXPACKETLENGTH
  UINT8 team;
  UINT8 skin;
  UINT8 data; // Color is first four bits, hasflag, isit and issuper have one
              // bit each, the last is unused.
  UINT32 score;
...
typedef struct {
  UINT32 checksum;
  UINT8 ack; // If not zero the node asks for acknowledgement, the receiver must
             // resend the ack
  ...
  union {
    ...
    UINT8 resynchgot;              //
    UINT8 textcmd[MAXTEXTCMD + 1]; //       66049 bytes (wut??? 64k??? More like
                                   //       257 bytes...)
    filetx_pak filetxpak;          //         139 bytes
    clientconfig_pak clientcfg;    //         153 bytes
    serverinfo_pak serverinfo;     //        1024 bytes
    serverrefuse_pak serverrefuse; //       65025 bytes (somehow I feel like
                                   //       those values are garbage...)
...
extern consvar_t
    ...
    cv_joinnextround,
#endif
    cv_netticbuffer, cv_allownewplayer, cv_maxplayers, cv_resynchattempts,
...
void SendNetXCmd2(netxcmd_t id, const void *param,
                  size_t nparam); // splitsreen player
void SendNetXCmd3(netxcmd_t id, const void *param,
                  size_t nparam); // splitsreen3 player
void SendNetXCmd4(netxcmd_t id, const void *param,
                  size_t nparam); // splitsreen4 player
...
extern UINT8 hu_stopped; // kart, true when the game is stopped for players due
                         // to a disconnecting or connecting player
