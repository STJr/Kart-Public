...
mobj_t *tmfloorthing; // the thing corresponding to tmfloorz or NULL if tmfloorz
                      // is from a sector
...
boolean P_DoSpring(mobj_t *spring, mobj_t *object) {
  ...
  object->standingslope = NULL; // Okay, now we can't return - no launching off
                                // at silly angles for you.
  ...
  if (vertispeed > 0)
    ...
    offx = P_ReturnThrustX(spring, spring->angle,
                           (spring->radius + object->radius + 1) * 2);
    offy = P_ReturnThrustY(spring, spring->angle,
                           (spring->radius + object->radius + 1) * 2);
  ...
  if (vertispeed)
    object->momz =
        FixedMul(vertispeed, FixedSqrt(FixedMul(vscale, spring->scale)));

  if (horizspeed) {
    if (!object->player)
      P_InstaThrustEvenIn2D(
          object, spring->angle,
          FixedMul(horizspeed, FixedSqrt(FixedMul(hscale, spring->scale))));
    else {
      ...
      P_InstaThrustEvenIn2D(
          object, spring->angle,
          FixedMul(finalSpeed, FixedSqrt(FixedMul(hscale, spring->scale))));
    }
  }
  ...
  if (object->player) {
    ...
    if (horizspeed && object->player->cmd.forwardmove == 0 &&
        object->player->cmd.sidemove == 0) {
    ...
    // pflags = object->player->pflags & (PF_JUMPED|PF_SPINNING|PF_THOKKED); //
    // I still need these.
    ...
    {
            if (pflags & (PF_JUMPED|PF_SPINNING) && object->player->panim ==
    PA_ROLL) object->player->pflags = pflags; else P_SetPlayerMobjState(object,
    S_PLAY_RUN1);
    }
...
static void P_DoFanAndGasJet(mobj_t *spring, mobj_t *object) {
  ...
  fixed_t speed = spring->info->mass; // conveniently, both fans and gas jets
                                      // use this for the vertical thrust
  SINT8 flipval =
      P_MobjFlip(spring); // virtually everything here centers around the
                          // thruster's gravity, not the object's!

  if (p && object->state ==
               &states[object->info->painstate]) // can't use fans and gas jets
                                                 // when player is in pain!
    return;

  // is object's top below thruster's position? if not, calculate distance
  ...
  switch (spring->type) {
  case MT_FAN:                                // fan
    if (zdist > (spring->health << FRACBITS)) // max z distance determined by
                                              // health (set by map thing angle)
      break;
    if (flipval * object->momz >=
        FixedMul(speed, spring->scale)) // if object's already moving faster
                                        // than your best, don't bother
      break;
    if (p &&
        (p->climbing ||
         p->pflags &
             PF_GLIDING)) // doesn't affect Knux when he's using his abilities!
      break;
    ...
    if (spring->spawnpoint && spring->spawnpoint->options & MTF_OBJECTSPECIAL) {
      ...
      object->momz =
          ...
          FixedMul(speed,
                   FixedSqrt(FixedMul(
                       spring->scale,
                       object->scale))); // scale the speed with both objects'
                                         // scales, just like with springs!
...
static boolean PIT_CheckThing(mobj_t *thing) {
  ...
  if (tmthing->type == MT_NAMECHECK) {
    ...
    // (also don't bother to check that tmthing->target->player is non-NULL
    // because we're not actually using it here.)
    if (!thing->player || thing->player->spectator ||
        (tmthing->target && thing->player == tmthing->target->player))
      return true;
  ...
  !(thing->flags & MF_SHOOTABLE))) return true; blockdist = thing->radius +
  tmthing->radius; if (abs(thing->x - tmx) >= blockdist || abs(thing->y - tmy)
  >= blockdist) return true; // didn't hit it
          // see if it went over / under
          if (tmthing->z > thing->z + thing->height)
                  return true; // overhead
          if (tmthing->z + tmthing->height < thing->z)
          ...
          if (thing->type == MT_SPIKE)
          {
                  ...
                  for (thing = thing->subsector->sector->thinglist; thing; thing
  = thing->snext) if (thing->type == MT_SPIKE && thing->health > 0 &&
  thing->flags & MF_SOLID && P_AproxDistance(thing->x - tmthing->x, thing->y -
  tmthing->y) < FixedMul(56*FRACUNIT, thing->scale)) P_KillMobj(thing, tmthing,
  tmthing);
          }
          else
          {
  ...
  if (!(thing->flags & (MF_SOLID | MF_SPECIAL | MF_PAIN | MF_SHOOTABLE)) ||
      (thing->flags & MF_NOCLIPTHING))
    return true;

  // Don't collide with your buddies while NiGHTS-flying.
  if (tmthing->player && thing->player && (maptol & TOL_NIGHTS) &&
      ((tmthing->player->pflags & PF_NIGHTSMODE) ||
  ...
  if (thing->flags &
      MF_PAPERCOLLISION) // CAUTION! Very easy to get stuck inside MF_SOLID
                         // objects. Giving the player MF_PAPERCOLLISION is a
                         // bad idea unless you know what you're doing.
  {
    ...
    cosradius =
        FixedMul(thing->radius, FINECOSINE(thing->angle >> ANGLETOFINESHIFT));
    sinradius =
        FixedMul(thing->radius, FINESINE(thing->angle >> ANGLETOFINESHIFT));
    ...
    if (tmthing->flags &
        MF_PAPERCOLLISION) // more strenuous checking to prevent clipping issues
    {
      INT32 check1, check2, check3, check4;
      cosradius = FixedMul(tmthing->radius,
                           FINECOSINE(tmthing->angle >> ANGLETOFINESHIFT));
      sinradius = FixedMul(tmthing->radius,
                           FINESINE(tmthing->angle >> ANGLETOFINESHIFT));
      check1 = P_PointOnLineSide(tmx - cosradius, tmy - sinradius, &junk);
      check2 = P_PointOnLineSide(tmx + cosradius, tmy + sinradius, &junk);
      check3 = P_PointOnLineSide(tmx + tmthing->momx - cosradius,
                                 tmy + tmthing->momy - sinradius, &junk);
      check4 = P_PointOnLineSide(tmx + tmthing->momx + cosradius,
                                 tmy + tmthing->momy + sinradius, &junk);
      if ((check1 == check2) && (check2 == check3) && (check3 == check4))
        return true; // the line doesn't cross between collider's start or end
    } else {
      if ((P_PointOnLineSide(tmx - tmthing->radius, tmy - tmthing->radius,
                             &junk) == P_PointOnLineSide(tmx + tmthing->radius,
                                                         tmy + tmthing->radius,
                                                         &junk)) &&
          (P_PointOnLineSide(tmx + tmthing->radius, tmy - tmthing->radius,
                             &junk) == P_PointOnLineSide(tmx - tmthing->radius,
                                                         tmy + tmthing->radius,
                                                         &junk)))
        return true; // the line doesn't cross between either pair of opposite
                     // corners
    }
    ...
    if ((P_PointOnLineSide(thing->x - thing->radius, thing->y - thing->radius,
                           &junk) == P_PointOnLineSide(thing->x + thing->radius,
                                                       thing->y + thing->radius,
                                                       &junk)) &&
        (P_PointOnLineSide(thing->x + thing->radius, thing->y - thing->radius,
                           &junk) == P_PointOnLineSide(thing->x - thing->radius,
                                                       thing->y + thing->radius,
                                                       &junk)))
      return true; // the line doesn't cross between either pair of opposite
  ...
  {
    UINT8 shouldCollide =
        LUAh_MobjCollide(thing, tmthing); // checks hook for thing's type
  ...
  {
    UINT8 shouldCollide =
        LUAh_MobjMoveCollide(tmthing, thing); // checks hook for tmthing's type
    if (P_MobjWasRemoved(tmthing) || P_MobjWasRemoved(thing))
      return true; // one of them was removed???
    if (shouldCollide == 1)
      return false; // force collide
  ...
  // When solid spikes move, assume they just popped up and teleport things on
  // top of them to hurt.
  if (tmthing->type == MT_SPIKE && tmthing->flags & MF_SOLID) {
    ...
    if (tmthing->eflags & MFE_VERTICALFLIP)
      thing->z = thing->old_z =
          tmthing->z - thing->height - FixedMul(FRACUNIT, tmthing->scale);
    else
      thing->z = thing->old_z =
          tmthing->z + tmthing->height + FixedMul(FRACUNIT, tmthing->scale);
  ...
  if (thing->flags &
      MF_PAIN) { // Player touches painful thing sitting on the floor
    // see if it went over / under
    if (thing->z > tmthing->z + tmthing->height)
      return true; // overhead
    if (thing->z + thing->height < tmthing->z)
    ...
    return true;
  } else if (tmthing->flags &
             MF_PAIN) { // Painful thing splats player in the face
    // see if it went over / under
    if (tmthing->z > thing->z + thing->height)
      return true; // overhead
    if (tmthing->z + tmthing->height < thing->z)
  ...
  if (thing->type == MT_HOOPCOLLIDE && thing->flags & MF_SPECIAL &&
      tmthing->player) {
  ...
  if (thing->type == MT_SMK_ICEBLOCK) {
    ...
    if (!(tmthing->flags & MF_SOLID || tmthing->flags & MF_SHOOTABLE ||
          tmthing->flags & MF_BOUNCE))
      return true;
    ...
    if (tmthing->type == MT_BANANA || tmthing->type == MT_BANANA_SHIELD ||
        tmthing->type == MT_EGGMANITEM ||
        tmthing->type == MT_EGGMANITEM_SHIELD || tmthing->type == MT_SSMINE ||
        tmthing->type == MT_SSMINE_SHIELD ||
    ...
    /*if (tmthing->player && (tmthing->player->kartstuff[k_invincibilitytimer] >
       0
  ...
  if (tmthing->type == MT_EGGMANITEM &&
      (thing->type == MT_RANDOMITEM || thing->type == MT_EGGMANITEM)) {
    // see if it went over / under
    if (tmthing->z > thing->z + thing->height)
      return true; // overhead
    if (tmthing->z + tmthing->height < thing->z)
    ...
    P_InstaThrust(tmthing,
                  R_PointToAngle2(thing->x, thing->y, tmthing->x, tmthing->y),
                  thing->radius / 4);
    return true;
  } else if (thing->type == MT_EGGMANITEM && (tmthing->type == MT_RANDOMITEM ||
                                              tmthing->type == MT_EGGMANITEM)) {
    // see if it went over / under
    if (tmthing->z > thing->z + thing->height)
      return true; // overhead
    ...
    P_InstaThrust(thing,
                  R_PointToAngle2(tmthing->x, tmthing->y, thing->x, thing->y),
                  tmthing->radius / 4);
    return true;
  }
  ...
  if (tmthing->type == MT_ORBINAUT || tmthing->type == MT_JAWZ ||
      tmthing->type == MT_JAWZ_DUD || tmthing->type == MT_ORBINAUT_SHIELD ||
      tmthing->type == MT_JAWZ_SHIELD) {
    // see if it went over / under
    if (tmthing->z > thing->z + thing->height)
      return true; // overhead
    if (tmthing->z + tmthing->height < thing->z)
    ...
    if (((tmthing->target == thing) || (tmthing->target == thing->target)) &&
        ((tmthing->threshold > 0 && thing->type == MT_PLAYER) ||
         (thing->type != MT_PLAYER && thing->threshold > 0)))
      return true;

    if (tmthing->health <= 0 || thing->health <= 0)
      return true;

    if ((tmthing->type == MT_ORBINAUT_SHIELD ||
         tmthing->type == MT_JAWZ_SHIELD) &&
        tmthing->lastlook &&
        (thing->type == MT_ORBINAUT_SHIELD || thing->type == MT_JAWZ_SHIELD) &&
        thing->lastlook &&
        (tmthing->target ==
         thing->target)) // Don't hit each other if you have the same target
      return true;

    if (thing->player && thing->player->powers[pw_flashing] &&
        !(tmthing->type == MT_ORBINAUT || tmthing->type == MT_JAWZ ||
          tmthing->type == MT_JAWZ_DUD))
      return true;
    ...
    if (thing->type == MT_PLAYER) {
      ...
      P_InstaThrust(
          ...
          16 * FRACUNIT);
               ...
               thing->type == MT_JAWZ_DUD ||
               thing->type == MT_ORBINAUT_SHIELD ||
               thing->type == MT_JAWZ_SHIELD || thing->type == MT_BANANA ||
               thing->type == MT_BANANA_SHIELD || thing->type == MT_BALLHOG) {
      // Other Item Damage
      ...
      P_SpawnMobj(thing->x / 2 + tmthing->x / 2, thing->y / 2 + tmthing->y / 2,
                  thing->z / 2 + tmthing->z / 2, MT_ITEMCLASH);
      ...
      P_KillMobj(thing, tmthing, tmthing);
               ...
               (tmthing->type == MT_JAWZ || tmthing->type == MT_JAWZ_DUD ||
                tmthing->type == MT_ORBINAUT))
      P_DoSpring(thing, tmthing);

    return true;
  } else if (tmthing->flags & MF_SPRING &&
             (thing->type == MT_JAWZ || thing->type == MT_JAWZ_DUD ||
              thing->type == MT_ORBINAUT)) {
    // see if it went over / under
    if (tmthing->z > thing->z + thing->height)
      return true; // overhead
    if (tmthing->z + tmthing->height < thing->z)
    ...
    if (thing->type == MT_PLAYER) {
      ...
      HU_DoCEcho(va("%s\\was hit by a kitchen sink.\\\\\\\\",
    ...
    return true; // This doesn't collide with anything, but we want it to effect
                 // the player anyway.
    ...
    if (thing->type == MT_PLAYER) {
      ...
      // Player Damage
      K_SpinPlayer(
          thing->player, tmthing->target, 0, tmthing,
          (tmthing->type == MT_BANANA || tmthing->type == MT_BANANA_SHIELD));
      ...
      P_InstaThrust(
          ...
          16 * FRACUNIT);
               ...
               thing->type == MT_ORBINAUT ||
               thing->type == MT_ORBINAUT_SHIELD || thing->type == MT_JAWZ ||
               thing->type == MT_JAWZ_DUD || thing->type == MT_JAWZ_SHIELD ||
    ...
    if (thing->type == MT_PLAYER) {
      // Bomb punting
      if ((tmthing->state >= &states[S_SSMINE1] &&
           tmthing->state <= &states[S_SSMINE4]) ||
          (tmthing->state >= &states[S_SSMINE_DEPLOY8] &&
           tmthing->state <= &states[S_SSMINE_DEPLOY13]))
        P_KillMobj(tmthing, thing, thing);
      else
    ...
    return true;
             ...
             (thing->type == MT_ORBINAUT_SHIELD || thing->type == MT_ORBINAUT ||
              thing->type == MT_JAWZ_SHIELD || thing->type == MT_JAWZ ||
              thing->type == MT_JAWZ_DUD || thing->type == MT_BANANA_SHIELD ||
              thing->type == MT_BANANA || thing->type == MT_SSMINE_SHIELD ||
              thing->type == MT_SSMINE || thing->type == MT_MINEEXPLOSION ||
    ...
    if (tmthing->player &&
        tmthing->player
            ->kartstuff[k_hyudorotimer]) // I thought about doing this for just
                                         // the objects below but figured it
    ...
    if (thing->type == MT_ORBINAUT_SHIELD || thing->type == MT_JAWZ_SHIELD ||
      ...
      // Player Damage
      K_SpinPlayer(
          tmthing->player, thing->target, 0, thing,
      ...
      // Bomb punting
      if ((thing->state >= &states[S_SSMINE1] &&
           thing->state <= &states[S_SSMINE4]) ||
          (thing->state >= &states[S_SSMINE_DEPLOY8] &&
           thing->state <= &states[S_SSMINE_DEPLOY13]))
        P_KillMobj(thing, tmthing, tmthing);
      else
  ...
  if ((thing->type == MT_FALLINGROCK &&
       (tmthing->player || tmthing->type == MT_FALLINGROCK)) ||
      (tmthing->type == MT_FALLINGROCK &&
  ...
  if ((thing->type == MT_SPRINGSHELL || thing->type == MT_YELLOWSHELL) &&
      thing->health > 0 &&
      (tmthing->player || (tmthing->flags & MF_PUSHABLE)) &&
      tmthing->health > 0) {
    // Multiplying by -1 inherently flips "less than" and "greater than"
    fixed_t tmz =
        ((thing->eflags & MFE_VERTICALFLIP) ? -(tmthing->z + tmthing->height)
                                            : tmthing->z);
    fixed_t tmznext =
        ((thing->eflags & MFE_VERTICALFLIP) ? -tmthing->momz : tmthing->momz) +
        tmz;
    fixed_t thzh =
        ((thing->eflags & MFE_VERTICALFLIP) ? -thing->z
                                            : thing->z + thing->height);
    // fixed_t sprarea = FixedMul(8*FRACUNIT, thing->scale) * P_MobjFlip(thing);

    // if ((tmznext <= thzh && tmz > thzh) || (tmznext > thzh - sprarea &&
    // tmznext < thzh))
    if (tmznext <= thzh) {
      P_DoSpring(thing, tmthing);
      //	return true;
    }
    // else if (tmz > thzh - sprarea && tmz < thzh) // Don't damage people
    // springing up / down
    return true;
  }
  ...
  if (tmthing->flags & MF_MISSILE || tmthing->type == MT_SHELL) {
    ...
    // Special case for bounce rings so they don't get caught behind solid
    // objects.
    if ((tmthing->type == MT_THROWNBOUNCE && tmthing->fuse > 8 * TICRATE) &&
        ...
        && thing->type == MT_BLACKEGGMAN && tmthing->target != thing)
    {
            ...
            if (!(thing->state >= &states[S_BLACKEGG_PAIN1] && thing->state <=
    &states[S_BLACKEGG_PAIN35])) P_SetMobjState(thing, thing->info->painstate);
            return false;
    }*/
    ...
    if (tmthing->flags & MF_MISSILE && thing->player && tmthing->target &&
    ...
    /*if (thing->type == MT_EGGSHIELD)
    {
            ...
            if (P_AproxDistance(tmthing->x-thing->x, tmthing->y-thing->y) >
                    P_AproxDistance((tmthing->x-tmthing->momx)-thing->x,
    (tmthing->y-tmthing->momy)-thing->y))
            {
            ...
            angle = R_PointToAngle2(thing->x, thing->y, touchx, touchy) -
    thing->angle;

            if (!(angle > ANGLE_90 && angle < ANGLE_270)) // hit front of
    shield, didn't destroy it return false; else // hit shield from behind,
    shield is destroyed!
            {
                    P_KillMobj(thing, tmthing, tmthing);
                    return false;
    ...
    // damage / explode
    if (tmthing->flags &
        MF_ENEMY) // An actual ENEMY! (Like the deton, for example)
      P_DamageMobj(thing, tmthing, tmthing, 1);
    ...
    ((thing->player->pflags & PF_ITEMHANG) || (thing->player->pflags &
    PF_JUMPED)))
    {
            // Ignore
    }
  ...
  if (thing->flags & MF_PUSHABLE &&
      ...
      !(netgame && tmthing->player &&
        tmthing->player->spectator)) // Push thing!
  {
  ...
  {
          mobj_t *droneobj = (tmthing->type == MT_NIGHTSDRONE) ? tmthing :
  thing; player_t *pl = (droneobj == thing) ? tmthing->player : thing->player;

          // Must be in bonus time, and must be NiGHTS, must wait about a second
          // must be flying in the SAME DIRECTION as the last time you came
  through.
          // not (your direction) xor (stored direction)
          // In other words, you can't u-turn and respawn rings near the drone.
          if (pl->bonustime && (pl->pflags & PF_NIGHTSMODE) && (INT32)leveltime
  > droneobj->extravalue2 && (
  ...
  if (tmthing->type == MT_SPIKE && tmthing->flags & MF_SOLID &&
      thing->player) // moving spike rams into player?!
  {
    if (tmthing->eflags & MFE_VERTICALFLIP) {
      if (thing->z + thing->height <=
              tmthing->z + FixedMul(FRACUNIT, tmthing->scale) &&
          thing->z + thing->height + thing->momz >=
              tmthing->z + FixedMul(FRACUNIT, tmthing->scale) + tmthing->momz)
        P_DamageMobj(thing, tmthing, tmthing, 1);
    } else if (thing->z >= tmthing->z + tmthing->height -
                               FixedMul(FRACUNIT, tmthing->scale) &&
               thing->z + thing->momz <=
                   tmthing->z + tmthing->height -
                       FixedMul(FRACUNIT, tmthing->scale) + tmthing->momz)
      P_DamageMobj(thing, tmthing, tmthing, 1);
  } else if (thing->type == MT_SPIKE && thing->flags & MF_SOLID &&
             tmthing->player) // unfortunate player falls into spike?!
  {
    if (thing->eflags & MFE_VERTICALFLIP) {
      if (tmthing->z + tmthing->height <=
              thing->z - FixedMul(FRACUNIT, thing->scale) &&
          tmthing->z + tmthing->height + tmthing->momz >=
              thing->z - FixedMul(FRACUNIT, thing->scale))
        P_DamageMobj(tmthing, thing, thing, 1);
    } else if (tmthing->z >= thing->z + thing->height +
                                 FixedMul(FRACUNIT, thing->scale) &&
               tmthing->z + tmthing->momz <=
                   thing->z + thing->height + FixedMul(FRACUNIT, thing->scale))
      P_DamageMobj(tmthing, thing, thing, 1);
  }
  ...
  if (tmthing->player &&
      thing->player
      // Make sure they aren't able to damage you ANYWHERE along the Z axis, you
    ...
    if (tmthing->scale >
        thing->scale +
            (mapobjectscale / 8)) // SRB2kart - Handle squishes first!
    ...
    else if (tmthing->player->kartstuff[k_invincibilitytimer] &&
             !thing->player->kartstuff[k_invincibilitytimer]) // SRB2kart - Then
                                                              // invincibility!
      P_DamageMobj(thing, tmthing, tmthing, 1);
    else if (thing->player->kartstuff[k_invincibilitytimer] &&
             !tmthing->player->kartstuff[k_invincibilitytimer])
      P_DamageMobj(tmthing, thing, thing, 1);

    /*if (G_BattleGametype() && (!G_GametypeHasTeams() ||
    tmthing->player->ctfteam != thing->player->ctfteam))
    {
            if ((tmthing->player->powers[pw_invulnerability] ||
    tmthing->player->powers[pw_super])
                    && !thing->player->powers[pw_super])
                    P_DamageMobj(thing, tmthing, tmthing, 1);
            else if ((thing->player->powers[pw_invulnerability] ||
    thing->player->powers[pw_super])
                    && !tmthing->player->powers[pw_super])
                    P_DamageMobj(tmthing, thing, thing, 1);
    }
    ...
    if (G_TagGametype() && cv_touchtag.value &&
            ((thing->player->pflags & PF_TAGIT) != (tmthing->player->pflags &
    PF_TAGIT)))
    {
            if ((tmthing->player->pflags & PF_TAGIT) && !(thing->player->pflags
    & PF_TAGIT)) P_DamageMobj(thing, tmthing, tmthing, 1); else if
    ((thing->player->pflags & PF_TAGIT) && !(tmthing->player->pflags &
    PF_TAGIT)) P_DamageMobj(tmthing, thing, tmthing, 1);
    }*/
  }
  ...
  if (thing->player) {
    // Doesn't matter what gravity player's following! Just do your stuff in
    // YOUR direction only
    /*if (tmthing->eflags & MFE_VERTICALFLIP
    ...
    && (tmthing->z + tmthing->momz > thing->z + thing->height
     || tmthing->z + tmthing->momz <= thing->z))
            ...
            && !P_IsObjectOnGround(tmthing) // Don't crush if the monitor is on
    the ground...
  ...
  if (tmthing->player) // Is the moving/interacting object the player?
  {
    ...
    } else if (thing->player) // bounce when players collide
    {
      ...
      if (thing->player->kartstuff[k_squishedtimer] ||
          thing->player->kartstuff[k_hyudorotimer] ||
          thing->player->kartstuff[k_justbumped] ||
          thing->scale > tmthing->scale + (mapobjectscale / 8) ||
          tmthing->player->kartstuff[k_squishedtimer] ||
          tmthing->player->kartstuff[k_hyudorotimer] ||
          tmthing->player->kartstuff[k_justbumped] ||
          tmthing->scale > thing->scale + (mapobjectscale / 8)) {
        return true;
      }

      if (G_BattleGametype() && ((thing->player->kartstuff[k_bumper] &&
                                  !tmthing->player->kartstuff[k_bumper]) ||
                                 (tmthing->player->kartstuff[k_bumper] &&
                                  !thing->player->kartstuff[k_bumper]))) {
        return true;
      }
      ...
      if (G_BattleGametype()) {
        if (thing->player->kartstuff[k_sneakertimer] &&
            !(tmthing->player->kartstuff[k_sneakertimer]) &&
            !(thing->player->powers[pw_flashing])) // Don't steal bumpers while
                                                   // intangible
        {
          K_StealBumper(thing->player, tmthing->player, false);
          K_SpinPlayer(tmthing->player, thing, 0, tmthing, false);
        } else if (tmthing->player->kartstuff[k_sneakertimer] &&
                   !(thing->player->kartstuff[k_sneakertimer]) &&
      ...
      return true;
    } else if (thing->type == MT_BLUEROBRA_HEAD ||
               thing->type == MT_BLUEROBRA_JOINT) {
      // see if it went over / under
      if (tmthing->z > thing->z + thing->height)
        return true; // overhead
      if (tmthing->z + tmthing->height < thing->z)
      ...
      if ((tmthing->z < thing->z) &&
          (thing->z >= thing->movefactor - (256 << FRACBITS))) {
      ...
      if (tmthing->player->kartstuff[k_squishedtimer]) {
        ...
        tmthing->player->powers[pw_flashing] =
            K_GetKartFlashing(tmthing->player);
        return true;
      }

      // no interaction
      if (tmthing->player->powers[pw_flashing] > 0 ||
          tmthing->player->kartstuff[k_hyudorotimer] > 0 ||
          tmthing->player->kartstuff[k_spinouttimer] >
              0) //|| tmthing->player->kartstuff[k_squishedtimer] > 0
        return true;
    ...
    // Are you touching the side of the object you're interacting with?
    else if (thing->z - FixedMul(FRACUNIT, thing->scale) <=
                 tmthing->z + tmthing->height &&
             thing->z + thing->height + FixedMul(FRACUNIT, thing->scale) >=
                 tmthing->z) {
      if (thing->flags & MF_MONITOR &&
        ...
        SINT8 flipval =
            P_MobjFlip(thing); // Save this value in case monitor gets removed.
        fixed_t *momz = &tmthing->momz; // tmthing gets changed by P_DamageMobj,
                                        // so we need a new pointer?! X_x;;
        ...
        if ((P_MobjWasRemoved(thing) // Monitor was removed
            ...
            && (flipval * (*momz) <
                0)) // monitor is on the floor and you're going down, or on the
                    // ceiling and you're going up
          *momz = -*momz; // Therefore, you should be thrust in the opposite
  ...
  if (thing->flags & MF_SPRING &&
    ...
    if (iwassprung) // this spring caused you to gain MFE_SPRUNG just now...
      return false; // "cancel" P_TryMove via blocking so you keep your current
                    // position
  } else if (tmthing->flags & MF_SPRING &&
             (thing->player || thing->flags & MF_PUSHABLE))
    ; // Fix a few nasty spring-jumping bugs that happen sometimes.
  // Monitors are not treated as solid to players who are jumping, spinning or
  // gliding, unless it's a CTF team monitor and you're on the wrong team
  /*else if (thing->flags & MF_MONITOR && tmthing->player &&
  tmthing->player->pflags & (PF_JUMPED|PF_SPINNING|PF_GLIDING)
  && !((thing->type == MT_REDRINGBOX && tmthing->player->ctfteam != 1) ||
  (thing->type == MT_BLUERINGBOX && tmthing->player->ctfteam != 2)))
          ;*/
  // z checking at last
  // Treat noclip things as non-solid!
    ...
    if (tmthing->eflags & MFE_VERTICALFLIP) {
      ...
      topz = thing->z -
             thing->scale; // FixedMul(FRACUNIT, thing->scale), but thing->scale
                           // == FRACUNIT in base scale anyways
...
static boolean PIT_CheckCameraLine(line_t *ld) {
  ...
  // this line is out of the if so upper and lower textures can be hit by a
  // splat
  blockingline = ld;
...
static boolean PIT_CheckLine(line_t *ld) {
  ...
  if (tmthing->flags &
      MF_PAPERCOLLISION) // Caution! Turning whilst up against a wall will get
                         // you stuck. You probably shouldn't give the player
                         // this flag.
  {
    fixed_t cosradius, sinradius;
    cosradius = FixedMul(tmthing->radius,
  ...
  if (!(tmthing->flags & MF_MISSILE)) {
    if (ld->flags &
        ML_IMPASSIBLE) // block objects from moving through this linedef.
      return false;
...
boolean P_CheckPosition(mobj_t *thing, fixed_t x, fixed_t y) {
...
#ifdef PARANOIA
  if (P_MobjWasRemoved(thing))
    I_Error("Previously-removed Thing of type %u crashes P_CheckPosition!",
            thing->type);
#endif
  ...
  // will adjust them.
  tmfloorz = tmdropoffz = P_GetFloorZ(thing, newsubsec->sector, x, y,
                                      NULL); // newsubsec->sector->floorheight;
  tmceilingz = P_GetCeilingZ(thing, newsubsec->sector, x, y,
                             NULL); // newsubsec->sector->ceilingheight;
  tmfloorslope = newsubsec->sector->f_slope;
  tmceilingslope = newsubsec->sector->c_slope;

  // Check list of fake floors and see if tmfloorz/tmceilingz need to be
  // altered.
  if (newsubsec->sector->ffloors) {
    ...
    for (rover = newsubsec->sector->ffloors; rover; rover = rover->next) {
      ...
      bottomheight =
          P_GetFOFBottomZ(thing, newsubsec->sector, rover, x, y, NULL);
      ...
      if ((rover->flags & (FF_SWIMMABLE | FF_GOOWATER)) ==
        ...
        if (thing->z < topheight && bottomheight < thingtop &&
          ...
          // Oh no! The object is stick in between the surface of the goo and
          // sinklevel! help them out!
          if (!(thing->eflags & MFE_VERTICALFLIP) &&
              thing->z > topheight - sinklevel && thing->momz >= 0 &&
              thing->momz < (minspeed >> 2))
            thing->momz += minspeed >> 2;
          else if (thing->eflags & MFE_VERTICALFLIP &&
                   thingtop < bottomheight + sinklevel && thing->momz <= 0 &&
                   thing->momz > -(minspeed >> 2))
            thing->momz -= minspeed >> 2;
      ...
      delta1 = thing->z - (bottomheight + ((topheight - bottomheight) / 2));
      ...
      for (bx = xl; bx <= xh; bx++) {
        ...
        while (plink) {
          ...
          if (po->validcount != validcount) // if polyobj hasn't been checked
          {
            ...
            if (!P_BBoxInsidePolyobj(po, tmbbox) || !(po->flags & POF_SOLID)) {
...
// P_CheckPosition optimized for the MT_HOOPCOLLIDE object. This needs to be as
// fast as possible!
...
boolean P_CheckCameraPosition(fixed_t x, fixed_t y, camera_t *thiscam) {
  ...
  if (GETSECSPECIAL(newsubsec->sector->special, 4) ==
      12) { // Camera noclip on entire sector.
  ...
  // will adjust them.
  tmfloorz = tmdropoffz =
      P_CameraGetFloorZ(thiscam, newsubsec->sector, x, y, NULL);
  ...
  if (newsubsec->sector->heightsec >= 0) {
    ...
    tmceilingz = tmdrpoffceilz =
        sectors[newsubsec->sector->heightsec].ceilingheight;
  }

  // Use preset camera clipping heights if set with Sector Special Parameters
  // whose control sector has Camera Intangible special -Red
  if (newsubsec->sector->camsec >= 0) {
    tmfloorz = tmdropoffz = sectors[newsubsec->sector->camsec].floorheight;
    tmceilingz = tmdrpoffceilz =
        sectors[newsubsec->sector->camsec].ceilingheight;
  }
  ...
  if (newsubsec->sector->ffloors) {
    ...
    for (rover = newsubsec->sector->ffloors; rover; rover = rover->next) {
      ...
      topheight =
          P_CameraGetFOFTopZ(thiscam, newsubsec->sector, rover, x, y, NULL);
      bottomheight =
          P_CameraGetFOFBottomZ(thiscam, newsubsec->sector, rover, x, y, NULL);

      delta1 = thiscam->z - (bottomheight + ((topheight - bottomheight) / 2));
      delta2 = thingtop - (bottomheight + ((topheight - bottomheight) / 2));
      if (topheight > tmfloorz && abs(delta1) < abs(delta2)) {
        tmfloorz = tmdropoffz = topheight;
      }
...
boolean P_TryCameraMove(fixed_t x, fixed_t y, camera_t *thiscam) {
  ...
  if (!itsatwodlevel && players[displayplayers[0]].mo) {
...
#ifndef NOCLIPCAM
      if ((thiscam == &camera[i] &&
           (players[displayplayers[i]].pflags & PF_NOCLIP)) ||
          (leveltime < introtime)) // Noclipping player camera noclips too!!
#else
      if (thiscam == &camera[i] &&
          !(players[displayplayers[i]].pflags &
            PF_TIMEOVER)) // Time Over should not clip through walls
#endif
      {
        floatok = true;
    ...
    tmfloorz =
        P_CameraGetFloorZ(thiscam, thiscam->subsector->sector, x, y, NULL);
    tmceilingz =
        P_CameraGetCeilingZ(thiscam, thiscam->subsector->sector, x, y, NULL);
  }
...
boolean PIT_PushableMoved(mobj_t *thing) {
  ...
  if (!(thing->flags & MF_SOLID) || (thing->flags & MF_NOGRAVITY))
  ...
  if ((!(stand->eflags & MFE_VERTICALFLIP) &&
       thing->z !=
           stand->z + stand->height + FixedMul(FRACUNIT, stand->scale)) ||
      ((stand->eflags & MFE_VERTICALFLIP) &&
       thing->z + thing->height != stand->z - FixedMul(FRACUNIT, stand->scale)))
  ...
  if (thing->player) {
    ...
    // Ridiculous amount of newly declared stuff so players can't get stuck in
    // walls AND so gargoyles don't break themselves at the same time either
    // These are all non-static map variables that are changed for each and
    // every single mobj See, changing player's momx/y would possibly trigger
    // stuff as if the player were running somehow, so this must be done to keep
    // the player standing All this so players can ride gargoyles!
    ...
    // Now restore EVERYTHING so the gargoyle doesn't keep the player's tmstuff
    // and break
    floatok = oldfltok;
    tmfloorz = oldflrz;
    tmceilingz = oldceilz;
    ...
    if (!(thing->flags & MF_NOCLIP)) {
      ...
      if (thing->player) {
        ...
        if (P_PlayerTouchingSectorSpecial(thing->player, 1, 13) ||
          ...
          maxstep <<= 1;
        // If using type Section1:12, no maxstep. For ledges you don't want the
        // player to climb! (see: Egg Zeppelin & SMK port walls)
        else if (P_PlayerTouchingSectorSpecial(thing->player, 1, 12) ||
                 GETSECSPECIAL(R_PointInSubsector(x, y)->sector->special, 1) ==
                     12)
          maxstep = 0;
          ...
          return false; // mobj must lower itself to fit
                 ...
                 !(thing->player &&
                   (P_PlayerTouchingSectorSpecial(thing->player, 1, 14) ||
                    GETSECSPECIAL(R_PointInSubsector(x, y)->sector->special,
                                  1) == 14))) // Step down
      {
        // If the floor difference is MAXSTEPMOVE or less, and the sector isn't
        // Section1:14, ALWAYS step down! Formerly required a Section1:13 sector
        // for the full MAXSTEPMOVE, but no more.
      ...
      if (!allowdropoff && !(thing->flags & MF_FLOAT) &&
          thing->type != MT_SKIM && !tmfloorthing) {
        if (thing->eflags & MFE_VERTICALFLIP) {
...
static boolean P_ThingHeightClip(mobj_t *thing) {
  ...
  floormoved =
      (thing->eflags & MFE_VERTICALFLIP && tmceilingz != thing->ceilingz) ||
...
static boolean PTR_SlideCameraTraverse(intercept_t *in) {
  ...
  if (openbottom - mapcampointer->z >
      0)             // We don't want to make the camera step up.
...
isblocking:
  if (li->polyobj && slidemo->player) {
    if ((li->polyobj->lines[0]->backsector->flags & SF_TRIGGERSPECIAL_TOUCH) &&
        !(li->polyobj->flags & POF_NOSPECIALS))
      P_ProcessSpecialSector(slidemo->player, slidemo->subsector->sector,
                             li->polyobj->lines[0]->backsector);
  }
...
void P_SlideMove(mobj_t *mo, boolean forceslide) {
  ...
  if (tmhitthing && mo->z + mo->height > tmhitthing->z &&
      mo->z < tmhitthing->z + tmhitthing->height) {
    // Don't mess with your momentum if it's a pushable object. Pushables do
    // their own crazy things already.
    if (tmhitthing->flags & MF_PUSHABLE)
      return;

    // Thankfully box collisions are a lot simpler than arbitrary lines. There's
    // only four possible cases.
    if (mo->y + mo->radius <= tmhitthing->y - tmhitthing->radius) {
      mo->momy = 0;
      P_TryMove(mo, mo->x + mo->momx,
                tmhitthing->y - tmhitthing->radius - mo->radius, true);
      ...
      mo->momx = 0;
      P_TryMove(mo, tmhitthing->x - tmhitthing->radius - mo->radius,
                mo->y + mo->momy, true);
    } else if (mo->x - mo->radius >= tmhitthing->x + tmhitthing->radius) {
      mo->momx = 0;
      P_TryMove(mo, tmhitthing->x + tmhitthing->radius + mo->radius,
...
retry:
  ...
  if (!forceslide && bestslideline &&
      !(bestslideline->flags & ML_BOUNCY)) // SRB2kart - All walls are bouncy
...
void P_BouncePlayerMove(mobj_t *mo) {
  ...
  P_PathTraverse(leadx, leady, leadx + mmomx, leady + mmomy, PT_ADDLINES,
                 PTR_SlideTraverse);
  P_PathTraverse(trailx, leady, trailx + mmomx, leady + mmomy, PT_ADDLINES,
                 PTR_SlideTraverse);
  P_PathTraverse(leadx, traily, leadx + mmomx, traily + mmomy, PT_ADDLINES,
                 PTR_SlideTraverse);
...
retry:
  ...
  bounceback:
    if (P_TryMove(mo, mo->x - mmomx, mo->y - mmomy, true)) {
      ...
      mo->momy *= -1;
      mo->momx =
          FixedMul(mo->momx, (FRACUNIT - (FRACUNIT >> 2) - (FRACUNIT >> 3)));
      mo->momy =
          FixedMul(mo->momy, (FRACUNIT - (FRACUNIT >> 2) - (FRACUNIT >> 3)));
    }
    return;
  }
  ...
  if (mo->type == MT_SHELL) {
    ...
    tmymove = FixedMul(mmomy, (FRACUNIT - (FRACUNIT >> 6) - (FRACUNIT >> 5)));
             ...
             mo->type == MT_CYBRAKDEMON_NAPALM_BOMB_LARGE) {
...
static boolean PIT_RadiusAttack(mobj_t *thing) {
  ...
  if (thing == bombspot // ignore the bomb itself (Deton fix)
      || (bombsource &&
          thing->type ==
              bombsource->type)) // ignore the type of guys who dropped the bomb
                                 // (Jetty-Syn Bomber or Skim can bomb
                                 // eachother, but not themselves.)
    return true;
...
static boolean PIT_ChangeSector(mobj_t *thing, boolean realcrush) {
  ...
  // Thing doesn't fit. If thing is vulnerable, that means it's being crushed.
  // If thing is pushable, it might just be blocked by another object - check if
  // it's really a ceiling!
  if (thing->z + thing->height > thing->ceilingz &&
      thing->z <= thing->ceilingz) {
    if (thing->flags & MF_PUSHABLE &&
        thing->z + thing->height > thing->subsector->sector->ceilingheight) {
    ...
    if (thing->subsector->sector->ffloors &&
        (realcrush || thing->flags & MF_PUSHABLE)) {
      ffloor_t *rover;
      fixed_t topheight, bottomheight;
      fixed_t delta1, delta2;
      INT32 thingtop = thing->z + thing->height;

      for (rover = thing->subsector->sector->ffloors; rover;
           rover = rover->next) {
        ...
        /*#ifdef ESLOPE
                                        if (rover->t_slope)
                                                topheight =
        P_GetZAt(rover->t_slope, thing->x, thing->y); if (rover->b_slope)
                                                bottomheight =
        P_GetZAt(rover->b_slope, thing->x, thing->y); #endif*/
        ...
        if (bottomheight <= thing->ceilingz && abs(delta1) >= abs(delta2)) {
          if (thing->flags & MF_PUSHABLE) {
            ...
            // If the thing was crushed by a crumbling FOF, reward the player
            // who made it crumble!
            thinker_t *think;
            elevator_t *crumbler;
    ...
    if (realcrush) {
      ...
      if (netgame && thing->player && thing->player->spectator)
        ...
        if (!killer) {
          // Nobody is responsible for crushing the object, so give a generic
          // crush message
        ...
        if (!thing->player)
          ...
          K_SquishPlayer(thing->player, killer,
                         killer); // SRB2kart - Squish instead of kill
      }
    }
  }
...
boolean P_CheckSector(sector_t *sector, boolean crunch) {
  ...
  do {
    for (n = sector->touching_thinglist; n;
         n = n->m_thinglist_next) // go through list
...
static mprecipsecnode_t *P_AddPrecipSecnode(sector_t *s, precipmobj_t *thing,
                                            mprecipsecnode_t *nextnode) {
  mprecipsecnode_t *node;
...
static inline boolean PIT_GetPrecipSectors(line_t *ld) {
  ...
  precipsector_list =
      P_AddPrecipSecnode(ld->frontsector, tmprecipthing, precipsector_list);
  ...
  if (ld->backsector)
    precipsector_list =
        P_AddPrecipSecnode(ld->backsector, tmprecipthing, precipsector_list);
...
void P_CreatePrecipSecNodeList(precipmobj_t *thing, fixed_t x, fixed_t y) {
  ...
  precipmobj_t *saved_tmthing =
      tmprecipthing; /* cph - see comment at func end */
  ...
  // Add the sector of the (x, y) point to sector_list.
  precipsector_list =
      P_AddPrecipSecnode(thing->subsector->sector, thing, precipsector_list);
...
void P_MapEnd(void) { P_SetTarget(&tmthing, NULL); }
