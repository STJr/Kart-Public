...
// Called by main loop, runs for demo playback. If this returns true, nullify
...
#define IT_ARROWS                                                              \
  2 // call function with 0 for left arrow and 1 for right arrow in param
...
#define IT_MSGHANDLER                                                          \
  12 // same as key but with event and sometime can handle y/n key (special for
     // message
...
#define IT_HEADERTEXT                                                          \
  (48 + 128) // Non-selectable header option, displays in yellow offset to the
             // left a little
#define IT_QUESTIONMARKS                                                       \
  (64 + 128)          // Displays as question marks, used for secrets
...
//  A lot of them became redundant with the advent of the Pause menu, so they
//  were removed
...
typedef struct menu_s {
  ...
  boolean (*quitroutine)(
      void); // called before quit a menu return true if we can
} menu_t;
...
#define DEFAULTMENUSTYLE(header, source, prev, x, y)                           \
  {                                                                            \
    header, sizeof(source) / sizeof(menuitem_t), prev, source,                 \
        M_DrawGenericMenu, x, y, 0, NULL                                       \
  }

#define PAUSEMENUSTYLE(source, x, y)                                           \
  {                                                                            \
    NULL, sizeof(source) / sizeof(menuitem_t), NULL, source, M_DrawPauseMenu,  \
        x, y, 0, NULL                                                          \
  }
...
#define CENTERMENUSTYLE(header, source, prev, y)                               \
  {                                                                            \
    header, sizeof(source) / sizeof(menuitem_t), prev, source,                 \
        M_DrawCenteredMenu, BASEVIDWIDTH / 2, y, 0, NULL                       \
  }

#define MAPICONMENUSTYLE(header, source, prev)                                 \
  {                                                                            \
    header, sizeof(source) / sizeof(menuitem_t), prev, source,                 \
        M_DrawServerMenu, 24, 40, 0, NULL                                      \
  }

#define CONTROLMENUSTYLE(source, prev)                                         \
  {                                                                            \
    "M_CONTRO", sizeof(source) / sizeof(menuitem_t), prev, source,             \
        M_DrawControl, 26, 40, 0, NULL                                         \
  }

#define IMAGEDEF(source)                                                       \
  {                                                                            \
    NULL, sizeof(source) / sizeof(menuitem_t), NULL, source, M_DrawImageDef,   \
        0, 0, 0, NULL                                                          \
  }
