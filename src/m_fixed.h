...
FUNCMATH FUNCINLINE static inline fixed_t FixedMul(fixed_t a, fixed_t b) // asm
{
  ...
  asm("imull %2;"          // a*b
      ...
      : "0"(a),
        "r"(b) // and %2 is what we use imull on with what in %1
        ,
...
FUNCMATH FUNCINLINE static inline fixed_t FixedDiv2(fixed_t a, fixed_t b) {
  fixed_t ret;
  asm("movl  %1,%%edx;" // these two instructions allow the next two to pair, on
  ...
  return ret;
    ...
    !defined(NOASM) // ARMv4 ASM
FUNCMATH FUNCINLINE static inline fixed_t FixedMul(fixed_t a,
                                                   fixed_t b) // let abuse smull
{
  fixed_t ret;
  asm("smull %[lo], r1, %[a], %[b];"
      "mov %[lo], %[lo], lsr %3;"
      ...
      : [a] "r"(a), [b] "r"(b), "i"(FRACBITS)
      : "r1");
  return ret;
}

#define __USE_C_FIXEDDIV__ // no double or asm div in ARM land
#elif defined(__GNUC__) && defined(__ppc__) && !defined(NOASM) &&              \
    0 // WII: PPC CPU
FUNCMATH FUNCINLINE static inline fixed_t FixedMul(fixed_t a, fixed_t b) // asm
{
  fixed_t ret, hi, lo;
...
#elif defined(__GNUC__) && defined(__mips__) && !defined(NOASM) &&             \
    0 // PSP: MIPS CPU
FUNCMATH FUNCINLINE static inline fixed_t FixedMul(fixed_t a, fixed_t b) // asm
{
  fixed_t ret;
  asm("mult %3, %4;"                // a*b=h<32+l
...
FUNCMATH FUNCINLINE static ATTRINLINE fixed_t FixedFloor(fixed_t x) {
  ...
  if (f == 0)
    return x;
  if (x != INT32_MIN) { // return rounded down to nearest whole number
    if (x > 0)
      return x - f;
    else
      return x - (FRACUNIT - f);
...
FUNCMATH FUNCINLINE static ATTRINLINE fixed_t FixedTrunc(fixed_t x) {
  ...
  const fixed_t i = (a >> FRACBITS) << FRACBITS; // cut out the fractional part
  const fixed_t f = a - i;                       // cut out the integral part
  if (x != INT32_MIN) { // return rounded to nearest whole number, towards zero
    if (x > 0)
      return x - f;
    else
      return x + f;
...
FUNCMATH FUNCINLINE static ATTRINLINE fixed_t FixedCeil(fixed_t x) {
  ...
  if (x == INT32_MIN)
    return INT32_MIN;
           ...
           FixedFloor(INT32_MAX)) { // return rounded up to nearest whole number
    if (x > 0)
      return x + (FRACUNIT - f);
    else
      return x + f;
...
FUNCMATH FUNCINLINE static ATTRINLINE fixed_t FixedRound(fixed_t x) {
  ...
  if (x == INT32_MIN)
    return INT32_MIN;
  else if (x < FixedFloor(INT32_MAX)) { // return rounded to nearest whole
