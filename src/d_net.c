...
///        This protocol uses a mix of "goback n" and "selective repeat"
///        implementation The NOTHING packet is sent when connection is idle to
///        acknowledge packets
...
static boolean GetFreeAcknum(UINT8 *freeack, boolean lowtimer) {
  ...
  if (cmpack((UINT8)((node->remotefirstack + MAXACKTOSEND) % 256),
             node->nextacknum) < 0) {
  ...
  for (i = 0; i < MAXACKPACKETS; i++)
    if (!ackpak[i].acknum) {
      // For low priority packets, make sure to let freeslots so urgent packets
      // can be sent
...
static boolean Processackpak(void) {
  ...
  if (netbuffer->ackreturn &&
      cmpack(node->remotefirstack, netbuffer->ackreturn) < 0) {
      ...
      if (goodpacket) {
        ...
        if (ack == nextfirstack) {
          ...
          if (!nextfirstack)
            nextfirstack = 1;
          hm1 =
              (UINT8)((node->acktosend_head - 1 + MAXACKTOSEND) % MAXACKTOSEND);
          while (change) {
            change = false;
            for (i = node->acktosend_tail; i != node->acktosend_head;
                 i = (i + 1) % MAXACKTOSEND) {
        ...
        } else // Out of order packet
        {
          ...
          { // We can admit the packet but we will not detect the duplication
            // after :(
  ...
  for (i = 0; i < MAXACKPACKETS; i++) {
    ...
    if (ackpak[i].acknum && ackpak[i].senttime + NODETIMEOUT < I_GetTime()) {
      if (ackpak[i].resentnum > 10 && (node->flags & NF_CLOSE)) {
        DEBFILE(
            va("ack %d sent 10 times so connection is supposed lost: node %d\n",
               i, nodei));
      ...
      DEBFILE(va("Resend ack %d, %u<%d at %u\n", ackpak[i].acknum,
                 ackpak[i].senttime, NODETIMEOUT, I_GetTime()));
...
void Net_UnAcknowledgePacket(INT32 node) {
  ...
  if (nodes[node].acktosend[hm1] == netbuffer->ack) {
    ...
    while (nodes[node].firstacktosend != netbuffer->ack) {
      nodes[node].acktosend_tail =
          (UINT8)((nodes[node].acktosend_tail - 1 + MAXACKTOSEND) %
                  MAXACKTOSEND);
      nodes[node].acktosend[nodes[node].acktosend_tail] =
          nodes[node].firstacktosend;
...
void Net_CloseConnection(INT32 node) {
  ...
  if (node < 0 ||
      node >= MAXNETNODES) // prevent invalid nodes from crashing the game
  {
...
static const char *packettypename[NUMPACKETTYPE] = {
    "NOTHING",       "SERVERCFG",        "CLIENTCMD",
    "CLIENTMIS",     "CLIENT2CMD",       "CLIENT2MIS",
    "NODEKEEPALIVE", "NODEKEEPALIVEMIS", "SERVERTICS",
    "SERVERREFUSE",  "SERVERSHUTDOWN",   "CLIENTQUIT",

    "ASKINFO",       "SERVERINFO",       "PLAYERINFO",
    "REQUESTFILE",   "ASKINFOVIAMS",

    "RESYNCHEND",    "RESYNCHGET",

    "CLIENT3CMD",    "CLIENT3MIS",       "CLIENT4CMD",
    "CLIENT4MIS",    "BASICKEEPALIVE",

    "FILEFRAGMENT",  "TEXTCMD",          "TEXTCMD2",
    "TEXTCMD3",      "TEXTCMD4",         "CLIENTJOIN",
    "NODETIMEOUT",   "RESYNCHING",       "PING"};

static void DebugPrintpacket(const char *header) {
  fprintf(debugfile, "%-12s (node %d,ack %d,ackret %d,size %d) type(%d) : %s\n",
          header, doomcom->remotenode, netbuffer->ack, netbuffer->ackreturn,
          doomcom->datalength, netbuffer->packettype,
  ...
  case PT_CLIENTJOIN:
    fprintf(debugfile, "    number %d mode %d\n",
            netbuffer->u.clientcfg.localplayers, netbuffer->u.clientcfg.mode);
    break;
  case PT_SERVERTICS: {
    servertics_pak *serverpak = &netbuffer->u.serverpak;
    UINT8 *cmd =
        (UINT8 *)(&serverpak->cmds[serverpak->numslots * serverpak->numtics]);
    size_t ntxtcmd = &((UINT8 *)netbuffer)[doomcom->datalength] - cmd;

    fprintf(debugfile, "    firsttic %u ply %d tics %d ntxtcmd %s\n    ",
            (UINT32)serverpak->starttic, serverpak->numslots,
            serverpak->numtics, sizeu1(ntxtcmd));
  ...
  case PT_TEXTCMD4:
    ...
    fprintfstringnewline((char *)netbuffer->u.textcmd + 2,
                         netbuffer->u.textcmd[0] - 1);
    break;
  case PT_SERVERCFG:
    fprintf(debugfile,
            "    playerslots %d clientnode %d serverplayer %d "
            "gametic %u gamestate %d gametype %d modifiedgame %d\n",
            netbuffer->u.servercfg.totalslotnum,
            netbuffer->u.servercfg.clientnode,
            netbuffer->u.servercfg.serverplayer,
            (UINT32)LONG(netbuffer->u.servercfg.gametic),
  ...
  case PT_FILEFRAGMENT:
    fprintf(debugfile, "    fileid %d datasize %d position %u\n",
            ...
            (UINT16)SHORT(netbuffer->u.filetxpak.size),
...
void Command_Drop(void) {
  ...
  if (!(stricmp(COM_Argv(1), "reset") && stricmp(COM_Argv(1), "cancel") &&
        stricmp(COM_Argv(1), "stop"))) {
...
static boolean ShouldDropPacket(void) {
  return (packetdropquantity[netbuffer->packettype]) ||
         (packetdroprate != 0 &&
          rand() < (RAND_MAX * (packetdroprate / 100.f))) ||
         packetdroprate == 100;
}
#endif
#endif
...
boolean HSendPacket(INT32 node, boolean reliable, UINT8 acknum,
  ...
  if (node == 0) // Packet is to go back to us
  {
    ...
    netbuffer->ack = netbuffer->ackreturn =
        0; // don't hold over values from last packet sent/received
    M_Memcpy(&reboundstore[rebound_head], netbuffer, doomcom->datalength);
...
boolean HGetPacket(void) {
  ...
  while (true) {
    ...
    if (netbuffer->checksum != NetbufferChecksum()) {
      ...
      // Net_CloseConnection(nodejustjoined ? (doomcom->remotenode | FORCECLOSE)
      // : doomcom->remotenode);
...
static void Internal_FreeNodenum(INT32 nodenum) { (void)nodenum; }

SINT8
I_NetMakeNode(const char *hostname) {
  SINT8 newnode = -1;
...
void Command_Ping_f(void) {
  ...
  for (i = 0; i < pingc; ++i) {
    CONS_Printf("%02d : %-*s %*d frames (%*d ms)\n", pingv[i].num, name_width,
                player_names[pingv[i].num], f_width, pingv[i].f, ms_width,
                pingv[i].ms);
  }
  ...
  if (!server && playeringame[consoleplayer]) {
    CONS_Printf("\nYour ping is %d frames (%d ms)\n",
                ...
                (INT32)(playerpingtable[i] * (1000.00f / TICRATE)));
  }
}
