...
{
        if (menuactive && (currentMenu != &MainDef && currentMenu !=
&SP_LoadDef)) return 0; // Only on the main menu, or the save select!
...
/*static cheatseq_t cheat_ultimate = {
        0, cheatf_ultimate,
        { SCRAMBLE('u'), SCRAMBLE('l'), SCRAMBLE('t'), SCRAMBLE('i'),
SCRAMBLE('m'), SCRAMBLE('a'), SCRAMBLE('t'), SCRAMBLE('e'), 0xff }
};*/

/*static cheatseq_t cheat_ultimate_joy = {
        0, cheatf_ultimate,
        { SCRAMBLE(KEY_UPARROW), SCRAMBLE(KEY_UPARROW), SCRAMBLE(KEY_DOWNARROW),
SCRAMBLE(KEY_DOWNARROW), SCRAMBLE(KEY_LEFTARROW), SCRAMBLE(KEY_RIGHTARROW),
SCRAMBLE(KEY_LEFTARROW), SCRAMBLE(KEY_RIGHTARROW), SCRAMBLE(KEY_ENTER), 0xff }
};*/

static cheatseq_t cheat_warp = {0,
                                cheatf_warp,
                                //{ SCRAMBLE('r'), SCRAMBLE('e'), SCRAMBLE('d'),
                                //SCRAMBLE('x'), SCRAMBLE('v'),
                                // SCRAMBLE('i'), 0xff }
                                {SCRAMBLE('b'), SCRAMBLE('a'), SCRAMBLE('n'),
                                 SCRAMBLE('a'), SCRAMBLE('n'), SCRAMBLE('a'),
                                 0xff}};
...
static cheatseq_t cheat_warp_joy = {
    ...
    {SCRAMBLE(KEY_LEFTARROW), SCRAMBLE(KEY_UPARROW), SCRAMBLE(KEY_RIGHTARROW),
     ...
     SCRAMBLE(KEY_DOWNARROW), SCRAMBLE(KEY_RIGHTARROW), SCRAMBLE(KEY_ENTER),
     0xff}};

#ifdef DEVELOP
static cheatseq_t cheat_devmode = {0,
                                   cheatf_devmode,
                                   {SCRAMBLE('d'), SCRAMBLE('e'), SCRAMBLE('v'),
                                    SCRAMBLE('m'), SCRAMBLE('o'), SCRAMBLE('d'),
                                    SCRAMBLE('e'), 0xff}};
#endif
...
#define REQUIRE_PANDORA                                                        \
  if (!M_SecretUnlocked(SECRET_PANDORA) && !cv_debug) {                        \
    CONS_Printf(M_GetText("You haven't earned this yet.\n"));                  \
    return;                                                                    \
  }

#define REQUIRE_DEVMODE                                                        \
  if (!cv_debug) {                                                             \
    CONS_Printf(M_GetText("DEVMODE must be enabled.\n"));                      \
    return;                                                                    \
  }

#define REQUIRE_OBJECTPLACE                                                    \
  if (!objectplacing) {                                                        \
    CONS_Printf(M_GetText("OBJECTPLACE must be enabled.\n"));                  \
    return;                                                                    \
  }

#define REQUIRE_INLEVEL                                                        \
  if (gamestate != GS_LEVEL || demo.playback) {                                \
    CONS_Printf(M_GetText("You must be in a level to use this.\n"));           \
    return;                                                                    \
  }

#define REQUIRE_SINGLEPLAYER                                                   \
  if (netgame || multiplayer) {                                                \
    CONS_Printf(M_GetText("This only works in single player.\n"));             \
    return;                                                                    \
  }

#define REQUIRE_NOULTIMATE                                                     \
  if (ultimatemode) {                                                          \
...
void Command_Scale_f(void) {
  ...
  if (scale < FRACUNIT / 100 ||
      scale > 100 * FRACUNIT) // COM_Argv(1) will return a null string if they
                              // did not give a paramater, so...
  {
    CONS_Printf(
        M_GetText("scale <value> (0.01-100.0): set player scale size\n"));
    return;
  }
...
void Command_Hurtme_f(void) {
  ...
  if (COM_Argc() < 2) {
    CONS_Printf(
        M_GetText("hurtme <damage>: Damage yourself by a specific amount\n"));
    return;
  }
...
/*void Command_JumpToAxis_f(void)
{
        ...
        if (COM_Argc() != 2)
        {
                CONS_Printf(M_GetText("jumptoaxis <axisnum>: Jump to axis within
current mare.\n")); return;
        }
...
void Command_Charability_f(void)
{
        ...
        if (COM_Argc() < 3)
        {
                CONS_Printf(M_GetText("charability <1/2> <value>: change
character abilities\n")); return;
        }
...
void Command_Charspeed_f(void)
{
        ...
        if (COM_Argc() < 3)
        {
                CONS_Printf(M_GetText("charspeed
<normalspeed/runspeed/thrustfactor/accelstart/acceleration/actionspd> <value>:
set character speed\n")); return;
        }

        if (!strcasecmp(COM_Argv(1), "normalspeed"))
                players[consoleplayer].normalspeed =
atoi(COM_Argv(2))<<FRACBITS; else if (!strcasecmp(COM_Argv(1), "runspeed"))
...
void Command_RTeleport_f(void) {
  ...
  if (COM_Argc() < 3 || COM_Argc() > 7) {
    CONS_Printf(M_GetText("rteleport -x <value> -y <value> -z <value>: "
                          "relative teleport to a location\n"));
    return;
  }
  ...
  ss = R_IsPointInSubsector(p->mo->x + intx * FRACUNIT,
                            p->mo->y + inty * FRACUNIT);
  if (!ss ||
      ss->sector->ceilingheight - ss->sector->floorheight < p->mo->height) {
  ...
  P_MapStart();
  if (!P_SetOrigin(p->mo, p->mo->x + intx * FRACUNIT,
                   p->mo->y + inty * FRACUNIT, intz))
...
void Command_Dumplua_f(void) {
  if (modifiedgame) {
    CONS_Printf(M_GetText("This command has been disabled in modified games, "
                          "to prevent scripted attacks.\n"));
    return;
  }

  if (COM_Argc() < 2) {
    CONS_Printf(M_GetText("dumplua <filename>: Compile a plain text lua script "
                          "(not a wad!) into bytecode.\n"));
    CONS_Alert(CONS_WARNING,
               M_GetText("This command makes irreversible file changes, please "
                         "use with caution!\n"));
    return;
  }
...
void Command_Savecheckpoint_f(void) {
  ...
  players[consoleplayer].starpostz =
  ...
  CONS_Printf(M_GetText("Temporary checkpoint created at %d, %d, %d\n"),
              players[consoleplayer].starpostx,
              players[consoleplayer].starposty,
              players[consoleplayer].starpostz);
}
...
void Command_Devmode_f(void) {
  ...
  if (COM_Argc() > 1) {
    ...
    if (arg[0] && arg[0] == '0' && arg[1] && arg[1] == 'x') // Use hexadecimal!
      cv_debug = axtoi(arg + 2);
    else
      cv_debug = atoi(arg);
  } else {
    CONS_Printf(M_GetText("devmode <flags>: enable debugging tools and info, "
                          "prepend with 0x to use hexadecimal\n"));
    return;
  }
...
players[consoleplayer].mo->health = 1;
                P_GivePlayerRings(&players[consoleplayer], atoi(COM_Argv(1)));
                if (!G_IsSpecialStage(gamemap) || !useNightsSS)
                        players[consoleplayer].totalring -= atoi(COM_Argv(1));
//undo totalring addition done in P_GivePlayerRings
...
consvar_t cv_mapthingnum = {"op_mapthingnum",
                            ...
                            0,
                            NULL};
consvar_t cv_speed = {"op_speed", "16", CV_NOTINNET, op_speed_t, NULL, 0,
                      NULL,       NULL, 0,           0,          NULL};
consvar_t cv_opflags = {"op_flags", "0",  CV_NOTINNET, op_flags_t, NULL, 0,
                        NULL,       NULL, 0,           0,          NULL};
...
static void OP_CycleThings(INT32 amt) {
  ...
  while (amt) {
    do {
      ...
      if (op_currentthing >= NUMMOBJTYPES)
        op_currentthing = 0;
    } while (mobjinfo[op_currentthing].doomednum == -1 ||
  ...
  if (states[mobjinfo[op_currentthing].spawnstate].sprite == SPR_NULL) {
    states[S_OBJPLACE_DUMMY].sprite =
        states[mobjinfo[op_currentthing].seestate].sprite;
    states[S_OBJPLACE_DUMMY].frame =
        states[mobjinfo[op_currentthing].seestate].frame;
  } else {
    states[S_OBJPLACE_DUMMY].sprite =
        states[mobjinfo[op_currentthing].spawnstate].sprite;
    states[S_OBJPLACE_DUMMY].frame =
        states[mobjinfo[op_currentthing].spawnstate].frame;
  }
...
static boolean OP_HeightOkay(player_t *player, UINT8 ceiling) {
  ...
  if (ceiling) {
    ...
    fixed_t cheight = sec->c_slope
                          ? P_GetZAt(sec->c_slope, player->mo->x & 0xFFFF0000,
                                     player->mo->y & 0xFFFF0000)
                          : sec->ceilingheight;

    if (((cheight - player->mo->z - player->mo->height) >> FRACBITS) >=
        (1 << (16 - ZSHIFT))) {
      CONS_Printf(
          M_GetText(
              "Sorry, you're too %s to place this object (max: %d %s).\n"),
          M_GetText("low"), (1 << (16 - ZSHIFT)),
          M_GetText("below top ceiling"));
      return false;
    }
  } else {
    fixed_t fheight = sec->f_slope
                          ? P_GetZAt(sec->f_slope, player->mo->x & 0xFFFF0000,
                                     player->mo->y & 0xFFFF0000)
                          : sec->floorheight;
    if (((player->mo->z - fheight) >> FRACBITS) >= (1 << (16 - ZSHIFT))) {
      CONS_Printf(
          M_GetText(
              "Sorry, you're too %s to place this object (max: %d %s).\n"),
          M_GetText("high"), (1 << (16 - ZSHIFT)),
          M_GetText("above bottom floor"));
      return false;
    }
  }
  return true;
...
static mapthing_t *OP_CreateNewMapThing(player_t *player, UINT16 type,
                                        boolean ceiling) {
  ...
  mapthings =
      Z_Realloc(mapthings, ++nummapthings * sizeof(*mapthings), PU_LEVEL, NULL);

  // as Z_Realloc can relocate mapthings, quickly go through thinker list and
  // correct the spawnpoints of any objects that have them to the new location
  if (mt != mapthings) {
    thinker_t *th;
    mobj_t *mo;
    ...
    for (th = thinkercap.next; th != &thinkercap; th = th->next) {
      ...
      mo = (mobj_t *)th;
      // get offset from mt, which points to old mapthings, then add new
      // location
      if (mo->spawnpoint)
  ...
  if (ceiling) {
    fixed_t cheight = sec->c_slope ? P_GetZAt(sec->c_slope, mt->x << FRACBITS,
                                              mt->y << FRACBITS)
                                   : sec->ceilingheight;
    mt->options =
        (UINT16)((cheight - player->mo->z - player->mo->height) >> FRACBITS);
  } else {
    fixed_t fheight = sec->f_slope ? P_GetZAt(sec->f_slope, mt->x << FRACBITS,
                                              mt->y << FRACBITS)
                                   : sec->floorheight;
...
void OP_NightsObjectplace(player_t *player) {
  ...
  if (player->pflags & PF_ATTACKDOWN) {
    ...
    if (!(cmd->buttons &
          (BT_ATTACK | BT_ACCELERATE | BT_BRAKE | BT_FORWARD | BT_BACKWARD)))
  ...
  if (cmd->buttons & BT_ATTACK) {
    ...
    INT16 temp =
        (INT16)FixedInt(AngleFixed(player->mo->angle)); // Traditional 2D Angle
    sector_t *sec = player->mo->subsector->sector;
    fixed_t fheight = sec->f_slope
                          ? P_GetZAt(sec->f_slope, player->mo->x & 0xFFFF0000,
                                     player->mo->y & 0xFFFF0000)
    ...
    // Tilt
    mt->angle =
        (INT16)FixedInt(FixedDiv(angle * FRACUNIT, 360 * (FRACUNIT / 256)));
    ...
    mt->options = (UINT16)((player->mo->z - fheight) >> FRACBITS);
    mt->angle =
        (INT16)(mt->angle + (INT16)((FixedInt(FixedDiv(temp * FRACUNIT,
                                                       360 * (FRACUNIT / 256))))
                                    << 8));
  ...
  /*if (cmd->buttons & BT_SPECTATE)
  {
          ...
          mt = OP_CreateNewMapThing(player,
  (UINT16)mobjinfo[MT_NIGHTSBUMPER].doomednum, false); P_SpawnMapThing(mt);
  }*/
  ...
  if (cmd->buttons & BT_BACKWARD) {
    ...
    mt = OP_CreateNewMapThing(player, (UINT16)mobjinfo[MT_RING].doomednum,
                              false);
    P_SpawnHoopsAndRings(mt);
  }
  ...
  if (cmd->buttons & BT_FORWARD) {
    ...
    mt = OP_CreateNewMapThing(player, (UINT16)mobjinfo[MT_NIGHTSWING].doomednum,
                              false);
    P_SpawnHoopsAndRings(mt);
  }
...
void OP_ObjectplaceMovement(player_t *player) {
  ...
  if (!player->climbing &&
      (netgame || !cv_analog.value || (player->pflags & PF_SPINNING)))
  ...
  if (cmd->forwardmove != 0) {
    P_Thrust(player->mo, player->mo->angle,
             (cmd->forwardmove * FRACUNIT / MAXPLMOVE) * cv_speed.value);
    P_MoveOrigin(player->mo, player->mo->x + player->mo->momx,
                 player->mo->y + player->mo->momy, player->mo->z);
  ...
  (cmd->sidemove*FRACUNIT/MAXPLMOVE)*cv_speed.value); P_MoveOrigin(player->mo,
  player->mo->x+player->mo->momx, player->mo->y+player->mo->momy,
  player->mo->z); player->mo->momx = player->mo->momy = 0;
  }*/
  ...
  // Moved up so it always updates.
  {
    ...
    if (!!(mobjinfo[op_currentthing].flags & MF_SPAWNCEILING) ^
        !!(cv_opflags.value & MTF_OBJECTFLIP)) {
      fixed_t cheight = sec->c_slope
                            ? P_GetZAt(sec->c_slope, player->mo->x & 0xFFFF0000,
                                       player->mo->y & 0xFFFF0000)
                            : sec->ceilingheight;
      op_displayflags = (UINT16)((cheight - player->mo->z -
                                  mobjinfo[op_currentthing].height) >>
                                 FRACBITS);
    } else {
      fixed_t fheight = sec->f_slope
                            ? P_GetZAt(sec->f_slope, player->mo->x & 0xFFFF0000,
  ...
  if (cmd->buttons & BT_ATTACK) {
    ...
    if (cv_mapthingnum.value > 0 && cv_mapthingnum.value < 4096) {
      ...
      if (spawnmid == NUMMOBJTYPES) {
        CONS_Alert(CONS_ERROR,
                   M_GetText("Can't place an object with mapthingnum %d.\n"),
    ...
    ceiling = !!(mobjinfo[spawnmid].flags & MF_SPAWNCEILING) ^
              !!(cv_opflags.value & MTF_OBJECTFLIP);
...
void Command_ObjectPlace_f(void) {
  ...
  if (!objectplacing) {
    ...
    op_oldcolor =
        players[0].mo->color; // save color too in case of super/fireflower
