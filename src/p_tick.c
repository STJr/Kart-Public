...
void Command_CountMobjs_f(void) {
  ...
  if (COM_Argc() >= 2) {
    ...
    for (j = 1; j < COM_Argc(); j++) {
      ...
      CONS_Printf(
          M_GetText(
              "There are %d objects of type %d currently in the level.\n"),
          count, i);
    }
    return;
  }
...
mobj_t *P_SetTarget(mobj_t **mop, mobj_t *targ) {
  ...
  if ((*mop = targ) !=
      NULL) // Set new target and if non-NULL, increase its counter
...
//
// Determine if the teams are unbalanced, and if so, move a player to the other
// team.
//
        ...
        for (i = 0; i < MAXPLAYERS; i++)
        {
                if (playeringame[i] && players[i].ctfteam)
                {
                        if (players[i].ctfteam == 1)
                        {
                                ...
                                {
                                        redarray[red] = i; //store the player's
node. red++;
                                }
                                else
                                        redflagcarrier++;
...
void P_DoTeamscrambling(void)
{
        ...
        {
                if (players[scrambleplayers[scramblecount]].ctfteam !=
scrambleteams[scramblecount])
                {
                        NetPacket.packet.newteam = scrambleteams[scramblecount];
                        NetPacket.packet.playernum =
scrambleplayers[scramblecount]; NetPacket.packet.verification = true;
                ...
                scramblecount++; //Increment, and get to the next player when we
...
players[i].powers[pw_spacetime] = 0;
        ...
        if (sstimer < 15*TICRATE+6 && sstimer > 7 &&
(mapheaderinfo[gamemap-1]->levelflags & LF_SPEEDMUSIC)) S_SpeedMusic(1.4f);
...
MFE_UNDERWATER)) inwater = true;
                ...
                if (ssrings >= totalrings && totalrings > 0)
                {
                        ...
                        for (i = 0; i < MAXPLAYERS; i++)
                                ...
                                {
                                        players[i].mo->momx =
players[i].mo->momy = 0; players[i].exiting = raceexittime+1;
                                }
...
player_names[i]); // Tell everyone who is it! break;
                        }
                }
        }
...
players[i].playerstate == PST_LIVE
                        && !(players[i].pflags & (PF_TAGIT|PF_TAGGED)))
                                //points given is the number of participating
players divided by two. P_AddPlayerScore(&players[i], participants/2);
                }
        }
}
...
static inline void P_DoCTFStuff(void)
{
        ...
        if (leveltime % (TICRATE * 5) == 0) //only check once per five seconds
for the sake of CPU conservation.
        {
                // Do not attempt to autobalance and scramble teams at the same
time.
                // Only the server should execute this. No verified admins,
please. if ((cv_autobalance.value && !cv_teamscramble.value) &&
cv_allowteamchange.value && server) P_DoAutobalanceTeams();
        }
...
void P_Ticker(boolean run) {
  ...
  if (paused || P_AutoPause()) {
    ...
    } else if (demo.freecam &&
               democam.cam) // special case: allow freecam to MOVE during pause!
  ...
  if (run) {
    ...
    if (demo.playback) {
      ...
      if (demo.version == 0x0001) {
        ...
        for (i = 0; i < MAXPLAYERS; i++)
          if (playeringame[i]) {
            //@TODO all this throwdir stuff shouldn't be here! But it's added to
            // maintain 1.0.4 compat for now...
            // Remove for 1.1!
  ...
  // as this is mostly used for HUD stuff, add the record attack specific hack
  // to it as well!
  if (!(modeattacking && !demo.playback) ||
      leveltime >= starttime - TICRATE * 4)
    timeinmap++;
  ...
  if (run) {
    ...
    if (quake.time) {
      ...
      /// \todo Calculate distance from epicenter if set and modulate the
      /// intensity accordingly based on radius.
    ...
    if (demo.recording) {
      ...
      if (cv_recordmultiplayerdemos.value &&
          (demo.savemode == DSM_NOTSAVING || demo.savemode == DSM_WILLAUTOSAVE))
    ...
    if (mapreset > 1 && --mapreset <= 1 &&
        server) // Remember: server uses it for mapchange, but EVERYONE ticks
                // down for the animation
