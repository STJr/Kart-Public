...
void P_CalculateSlopeNormal(pslope_t *slope) {
  ...
  slope->normal.x =
      -FixedMul(FINESINE(slope->zangle >> ANGLETOFINESHIFT), -slope->d.x);
  slope->normal.y =
      -FixedMul(FINESINE(slope->zangle >> ANGLETOFINESHIFT), -slope->d.y);
}
...
static void P_ReconfigureVertexSlope(pslope_t *slope) {
  ...
  slope->extent = max(max(max(abs(vec1.x), abs(vec1.y)), abs(vec1.z)),
  ...
  slope->extent = R_PointToDist2(
      0, 0, R_PointToDist2(0, 0, slope->normal.x, slope->normal.y),
      slope->normal.z);
  if (slope->normal.z < 0)
  ...
  if (slope->normal.x == 0 &&
      slope->normal.y == 0) { // Set some defaults for a non-sloped "slope"
    ...
    // Get angles
    slope->xydirection =
        R_PointToAngle2(0, 0, slope->d.x, slope->d.y) + ANGLE_180;
...
void P_RunDynamicSlopes(void) {
  ...
  for (slope = slopelist; slope; slope = slope->next) {
    ...
    switch (slope->refpos) {
    case 1: // front floor
      zdelta = slope->sourceline->backsector->floorheight -
               slope->sourceline->frontsector->floorheight;
      slope->o.z = slope->sourceline->frontsector->floorheight;
      break;
    case 2: // front ceiling
      zdelta = slope->sourceline->backsector->ceilingheight -
               slope->sourceline->frontsector->ceilingheight;
      slope->o.z = slope->sourceline->frontsector->ceilingheight;
      break;
    case 3: // back floor
      zdelta = slope->sourceline->frontsector->floorheight -
               slope->sourceline->backsector->floorheight;
      slope->o.z = slope->sourceline->backsector->floorheight;
      break;
    case 4: // back ceiling
      zdelta = slope->sourceline->frontsector->ceilingheight -
               slope->sourceline->backsector->ceilingheight;
...
void P_SpawnSlope_Line(int linenum) {
  ...
  if (!line->frontsector || !line->backsector) {
    CONS_Debug(DBG_SETUP,
               "P_SpawnSlope_Line used on a line without two sides. (line "
               "number %i)\n",
               linenum);
    return;
  }
  ...
  // For FOF slopes, make a special function to copy to the xy origin &
  // direction relative to the position of the FOF on the map!
  if (frontfloor || frontceil) {
    line->frontsector->hasslope =
        true; // Tell the software renderer that we're sloped
    ...
    if (extent < 0) {
      CONS_Printf("P_SpawnSlope_Line failed to get frontsector extent on line "
                  "number %i\n",
                  linenum);
      return;
    }
    ...
    // TODO: We take origin and point 's xy values and translate them to the
    // center of an FOF!
    ...
    if (frontfloor) {
      ...
      // To find the real highz/lowz of a slope, you need to check all the
      // vertexes in the slope's sector with P_GetZAt to get the REAL lowz &
      // highz Although these slopes are set by floorheights the ANGLE is what a
      // slope is, so technically any slope can extend on forever (they are just
      // bound by sectors) *You can use sourceline as a reference to see if two
      // slopes really are the same

      // Default points for high and low
      highest = point.z > origin.z ? point.z : origin.z;
      lowest = point.z < origin.z ? point.z : origin.z;

      // Now check to see what the REAL high and low points of the slope inside
      // the sector
      // TODO: Is this really needed outside of FOFs? -Red

      for (l = 0; l < line->frontsector->linecount; l++) {
        fixed_t height = P_GetZAt(line->frontsector->f_slope,
                                  line->frontsector->lines[l]->v1->x,
                                  line->frontsector->lines[l]->v1->y);
      ...
      fslope->xydirection =
          R_PointToAngle2(origin.x, origin.y, point.x, point.y);
    ...
    if (frontceil) {
      ...
      for (l = 0; l < line->frontsector->linecount; l++) {
        fixed_t height = P_GetZAt(line->frontsector->c_slope,
                                  line->frontsector->lines[l]->v1->x,
                                  line->frontsector->lines[l]->v1->y);
  ...
  if (backfloor || backceil) {
    ...
    if (extent < 0) {
      CONS_Printf("P_SpawnSlope_Line failed to get backsector extent on line "
                  "number %i\n",
                  linenum);
      return;
    }
    ...
    if (backfloor) {
      ...
      for (l = 0; l < line->backsector->linecount; l++) {
        fixed_t height = P_GetZAt(line->backsector->f_slope,
                                  line->backsector->lines[l]->v1->x,
                                  line->backsector->lines[l]->v1->y);
    ...
    if (backceil) {
      ...
      for (l = 0; l < line->backsector->linecount; l++) {
        fixed_t height = P_GetZAt(line->backsector->c_slope,
                                  line->backsector->lines[l]->v1->x,
                                  line->backsector->lines[l]->v1->y);
...
//
static pslope_t *P_NewVertexSlope(INT16 tag1, INT16 tag2, INT16 tag3,
                                  UINT8 flags) {
  size_t i;
  ...
  for (i = 0; i < nummapthings; i++, mt++) {
    if (mt->type !=
        750) // Haha, I'm hijacking the old Chaos Spawn thingtype for something!
      continue;
  ...
  for (i = 0; i < 3; i++) {
    mt = ret->vertices[i];
    if (!mt) // If a vertex wasn't found, it's game over. There's nothing you
             // can do to recover (except maybe try and kill the slope instead -
             // TODO?)
      I_Error(
          "P_NewVertexSlope: Slope vertex %s (for linedef tag %d) not found!",
          sizeu1(i), tag1);
    if (mt->extrainfo)
      mt->z = mt->options;
    else
      mt->z = (R_PointInSubsector(mt->x << FRACBITS, mt->y << FRACBITS)
                   ->sector->floorheight >>
               FRACBITS) +
              (mt->options >> ZSHIFT);
  }
...
void P_CopySectorSlope(line_t *line) {
  ...
  line->special = 0; // Linedef was use to set slopes, it finished its job, so
                     // now make it a normal linedef
}

//
// P_SlopeById
//
// Looks in the slope list for a slope with a specified ID. Mostly useful for
// netgame sync
//
...
void P_ResetDynamicSlopes(void) {
  size_t i;
#ifdef ESLOPE_TYPESHIM // Rewrite old specials to new ones, and give a console
                       // warning
  ...
  // Yes, this means copied slopes won't affect things' spawning heights. Too
  // bad for you.
...
#define WARNME                                                                 \
  if (!warned) {                                                               \
    ...
    CONS_Alert(CONS_WARNING,                                                   \
               "This level uses old slope specials.\nA conversion will be "    \
               "needed before 2.2's release.\n");                              \
  }
    case 386:
    case 387:
    case 388:
    ...
    case 715: {
      ...
      if (lines[i].flags & ML_NOKNUX)
        *slopetoset = P_NewVertexSlope(
            lines[i].tag,
            sides[lines[i].sidenum[which]].textureoffset >> FRACBITS,
            sides[lines[i].sidenum[which]].rowoffset >> FRACBITS, flags);
      else
        *slopetoset =
            P_NewVertexSlope(lines[i].tag, lines[i].tag, lines[i].tag, flags);
...
//
// When given a vector, rotates and aligns it to a flat surface (from being
// relative to a given slope)
...
// SRB2Kart: This fixes all slope-based jumps for different scales in Kart
// automatically without map tweaking. However, they will always feel off every
// single time... see for yourself:
// https://cdn.discordapp.com/attachments/270211093761097728/484924392128774165/kart0181.gif
...
void P_SlopeLaunch(mobj_t *mo) {
  if (!(mo->standingslope->flags &
        SL_NOPHYSICS)) // If there's physics, time for launching.
  {
    // Double the pre-rotation Z, then halve the post-rotation Z. This reduces
    // the vertical launch given from slopes while increasing the horizontal
    // launch given. Good for SRB2's gravity and horizontal speeds.
    vector3_t slopemom;
    slopemom.x = mo->momx;
    slopemom.y = mo->momy;
    slopemom.z = mo->momz;
...
void P_HandleSlopeLanding(mobj_t *thing, pslope_t *slope) {
  ...
  if (slope->flags &
      SL_NOPHYSICS) { // No physics, no need to make anything complicated.
...
void P_ButteredSlope(mobj_t *mo) {
  ...
  if (mo->flags & (MF_NOCLIPHEIGHT | MF_NOGRAVITY))
    return; // don't slide down slopes if you can't touch them or you're not
            // affected by gravity
  ...
  if (mo->player) {
    ...
    if (abs(mo->standingslope->zdelta) < FRACUNIT / 2 &&
      ...
      return; // Allow the player to stand still on slopes below a certain
              // steepness
  }

  thrust = FINESINE(mo->standingslope->zangle >> ANGLETOFINESHIFT) * 15 / 16 *
           (mo->eflags & MFE_VERTICALFLIP ? 1 : -1);

  if (mo->player && (mo->player->pflags & PF_SPINNING)) {
    fixed_t mult = 0;
    if (mo->momx || mo->momy) {
      angle_t angle = R_PointToAngle2(0, 0, mo->momx, mo->momy) -
                      mo->standingslope->xydirection;
  ...
  if (mo->momx ||
      mo->momy) // Slightly increase thrust based on the object's speed
    thrust =
        FixedMul(thrust, FRACUNIT + P_AproxDistance(mo->momx, mo->momy) / 16);
  // This makes it harder to zigzag up steep slopes, as well as allows greater
  // top speed when rolling down

  // Let's get the gravity strength for the object...
  thrust = FixedMul(thrust, abs(P_GetMobjGravity(mo)));

  // ... and its friction against the ground for good measure (divided by
  // original friction to keep behaviour for normal slopes the same).
