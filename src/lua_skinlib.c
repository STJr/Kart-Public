...
static const char *const skin_opt[] = {
    "valid", "name", "spritedef", "wadnum", "flags", "realname", "hudname",
    "facerank", "facewant", "facemmap",
    // SRB2kart
    "kartspeed", "kartweight",
    //
    "starttranscolor", "prefcolor", "highresscale", "soundsid", NULL};

#define UNIMPLEMENTED                                                          \
  luaL_error(                                                                  \
      L,                                                                       \
      LUA_QL("skin_t") " field " LUA_QS                                        \
...
static int skin_get(lua_State *L) {
  ...
  case skin_wadnum:
    // !!WARNING!! May differ between clients due to music wads, therefore NOT
    // NETWORK SAFE
    return UNIMPLEMENTED;
  case skin_flags:
...
static int lib_iterateSkins(lua_State *L) {
  ...
  if (lua_gettop(L) < 2) {
    // return luaL_error(L, "Don't call skins.iterate() directly, use it as 'for
    // skin in skins.iterate do <block> end'.");
...
static int lib_getSkin(lua_State *L) {
  ...
  if (lua_type(L, 2) == LUA_TNUMBER) {
    ...
    if (i < 0 || i >= MAXSKINS)
      return luaL_error(L, "skins[] index %d out of range (0 - %d)", i,
                        MAXSKINS - 1);
    if (i >= numskins)
      return 0;
    LUA_PushUserdata(L, &skins[i], META_SKIN);
