...
typedef enum {
  ...
  MF_NOBLOCKMAP = 1 << 4,
  // Thin, paper-like collision bound (for visual equivalent, see
  // FF_PAPERSPRITE)
  MF_PAPERCOLLISION = 1 << 5,
  // You can push this object. It can activate switches and things by pushing it
  // on top.
  MF_PUSHABLE = 1 << 6,
  // Object is a boss.
  MF_BOSS = 1 << 7,
...
typedef enum {
  ...
  MF2_BEYONDTHEGRAVE =
      1 << 9, // Source of this missile has died and has since respawned.
  ...
  MF2_INFLOAT = 1 << 14, // Floating to a height for a move, don't auto float to
                         // target's height.
  ...
  MF2_SUPERFIRE =
      1 << 19, // Firing something with Super Sonic-stopping properties. Or, if
               // mobj has MF_MISSILE, this is the actual fire from it.
  MF2_SHADOW = 1 << 20,    // Fuzzy draw, makes targeting harder.
  MF2_STRONGBOX = 1 << 21, // Flag used for "strong" random monitors.
  MF2_OBJECTFLIP =
      1 << 22,            // Flag for objects that always have flipped gravity.
  ...
  MF2_BOSSDEAD = 1 << 27,   // Boss is dead! (Not necessarily fleeing, if a
                            // fleeing point doesn't exist.)
...
typedef enum {
  ...
  MFE_TOUCHWATER = 1 << 2,
  // The mobj stands in a sector with water, and his waist is BELOW the water
  // surface (for player, allows swimming up/down)
  MFE_UNDERWATER = 1 << 3,
  ...
  // Goo water
  MFE_GOOWATER = 1 << 6,
  // SRB2Kart: The mobj just hit & bounced off a wall, this is cleared on next
  // frame
  ...
  // Platform movement
  MFE_APPLYPMOMZ = 1 << 9,
  // SRB2Kart: Splitscreen sprite display; very wasteful but I couldn't think of
...
typedef struct mobj_s {
  ...
  struct msecnode_s *
      touching_sectorlist; // a linked list of sectors where this object appears
  ...
  void *skin; // overrides 'sprite' when non-NULL (for player bodies to
              // 'remember' the skin)
  // Player and mobj sprites in multiplayer modes are modified
  //  using an internal color lookup table for re-indexing.
  UINT8
  color; // This replaces MF_TRANSLATION. Use 0 for default (no translation).
  ...
  struct mobj_s *target; // Thing being chased/attacked (or NULL), and
  ...
  mapthing_t *spawnpoint; // Used for CTF flags, objectplace, and a handful
                          // other applications.
  ...
  UINT32 mobjnum; // A unique number for this mobj. Used for restoring pointers
                  // on save games.
  ...
  struct pslope_s
      *standingslope; // The slope that the object is standing on (shouldn't
                      // need synced in savegames, right?)

  boolean resetinterp; // if true, some fields should not be interpolated (see
                       // R_InterpolateMobjState implementation)
...
typedef struct precipmobj_s {
  ...
  // Momentums, used to update position.
  fixed_t momx, momy, momz;
  fixed_t precipflags; // fixed_t so it uses the same spot as "pmomz" even as we
                       // use precipflags_t for it
