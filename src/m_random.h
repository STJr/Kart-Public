...
// M_Random functions pull random numbers of various types that aren't network
// synced. P_Random functions pulls random bytes from a PRNG that is network
// synced.
...
UINT8
P_RandomByteD(const char *rfile, INT32 rline);
INT32
P_RandomKeyD(const char *rfile, INT32 rline, INT32 a);
INT32
P_RandomRangeD(const char *rfile, INT32 rline, INT32 a, INT32 b);
#else
...
P_RandomKey(INT32 a);
INT32
P_RandomRange(INT32 a, INT32 b);
#endif

// Macros for other functions
#define M_SignedRandom()                                                       \
  ((INT32)M_RandomByte() - 128) // [-128, 127] signed byte, originally a
#define P_SignedRandom()                                                       \
  ((INT32)P_RandomByte() - 128) // function of its own, moved to a macro

#define M_RandomChance(p)                                                      \
  (M_RandomFixed() < p) // given fixed point probability, p, between 0 (0%)
#define P_RandomChance(p)                                                      \
  (P_RandomFixed() < p) // and FRACUNIT (100%), returns true p% of the time
...
UINT32
P_GetRandSeedD(const char *rfile, INT32 rline);
UINT32
P_GetInitSeedD(const char *rfile, INT32 rline);
