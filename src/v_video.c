...
consvar_t cv_ticrate = {"showfps", "No", CV_SAVE, CV_YesNo, NULL, 0,
                        NULL,      NULL, 0,       0,        NULL};
consvar_t cv_usegamma = {"gamma",
                         "0",
                         CV_SAVE | CV_CALL,
                         gamma_cons_t,
...
consvar_t cv_allcaps = {"allcaps", "Off", 0, CV_OnOff, NULL, 0,
                        NULL,      NULL,  0, 0,        NULL};
...
consvar_t cv_constextsize = {"con_textsize",
                             "Medium",
                             CV_SAVE | CV_CALL,
                             constextsize_cons_t,
...
static CV_PossibleValue_t grgamma_cons_t[] = {
    {1, "MIN"}, {255, "MAX"}, {0, NULL}};
static CV_PossibleValue_t grfakecontrast_cons_t[] = {
    {0, "Off"}, {1, "Standard"}, {2, "Smooth"}, {0, NULL}};

consvar_t cv_grshaders = {"gr_shaders", "On", CV_SAVE, CV_OnOff, NULL, 0,
                          NULL,         NULL, 0,       0,        NULL};
consvar_t cv_grfovchange = {"gr_fovchange", "Off", CV_SAVE, CV_OnOff, NULL, 0,
                            NULL,           NULL,  0,       0,        NULL};
consvar_t cv_grgammared = {"gr_gammared",
                           "127",
                           CV_SAVE | CV_CALL,
                           grgamma_cons_t,
                           CV_Gammaxxx_ONChange,
                           0,
                           NULL,
                           NULL,
                           0,
                           0,
                           NULL};
consvar_t cv_grgammagreen = {"gr_gammagreen",
                             "127",
                             CV_SAVE | CV_CALL,
                             grgamma_cons_t,
                             CV_Gammaxxx_ONChange,
                             ...
                             0,
                             NULL};
consvar_t cv_grgammablue = {"gr_gammablue",
                            "127",
                            CV_SAVE | CV_CALL,
                            grgamma_cons_t,
                            CV_Gammaxxx_ONChange,
...
// static CV_PossibleValue_t CV_MD2[] = {{0, "Off"}, {1, "On"}, {2, "Old"}, {0,
// NULL}};
//  console variables in development
consvar_t cv_grmdls = {"gr_mdls", "On", CV_SAVE, CV_OnOff, NULL, 0,
                       NULL,      NULL, 0,       0,        NULL};
consvar_t cv_grfallbackplayermodel = {"gr_fallbackplayermodel",
                                      "Off",
                                      CV_SAVE,
                                      CV_OnOff,
                                      NULL,
...
consvar_t cv_grshearing = {"gr_shearing", "Off", CV_SAVE, CV_OnOff, NULL, 0,
                           NULL,          NULL,  0,       0,        NULL};
consvar_t cv_grspritebillboarding = {"gr_spritebillboarding",
                                     "On",
                                     CV_SAVE,
                                     CV_OnOff,
                                     NULL,
                                     ...
                                     0,
                                     NULL};
consvar_t cv_grfakecontrast = {"gr_fakecontrast",
                               "Standard",
                               CV_SAVE,
                               grfakecontrast_cons_t,
                               NULL,
                               0,
                               NULL,
                               NULL,
...
const char *GetPalette(void) {
  if (gamestate == GS_LEVEL)
    return R_GetPalname((encoremode ? mapheaderinfo[gamemap - 1]->encorepal
...
static void CV_constextsize_OnChange(void) { con_recalc = true; }
...
void VID_BlitLinearScreen(const UINT8 *srcptr, UINT8 *destptr, INT32 width,
                          ...
                          size_t destrowbytes) {
#ifdef HAVE_VIDCOPY
  VID_BlitLinearScreen_ASM(srcptr, destptr, width, height, srcrowbytes,
                           destrowbytes);
#else
...
void V_DrawFixedPatch(fixed_t x, fixed_t y, fixed_t pscale, INT32 scrn,
                      patch_t *patch, const UINT8 *colormap) {
  ...
  // So it turns out offsets aren't scaled in V_NOSCALESTART unless V_OFFSET is
  // applied ...poo, that's terrible For now let's just at least give V_OFFSET
  // the ability to support V_FLIP I'll probably make a better fix for 2.2 where
  // I don't have to worry about breaking existing support for stuff
  // -- Monster Iestyn 29/10/18
  {
    ...
    if (scrn & V_FLIP)
      offsetx =
          FixedMul((SHORT(patch->width) - SHORT(patch->leftoffset)) << FRACBITS,
                   pscale) +
          1;
    else
    ...
    // top offset
    // TODO: make some kind of vertical version of V_FLIP, maybe by deprecating
    // V_OFFSET in future?!?
    offsety = FixedMul(SHORT(patch->topoffset) << FRACBITS, pscale);

    if ((scrn & (V_NOSCALESTART | V_OFFSET)) ==
        (V_NOSCALESTART | V_OFFSET)) // Multiply by dupx/dupy for crosshairs
    {
  ...
  if (scrn & V_NOSCALESTART) {
    ...
    if (!(scrn & V_SCALEPATCHMASK)) {
      ...
      /*if (x == 0 && FixedMul(SHORT(patch->width)<<FRACBITS, pscale)>>FRACBITS
      == BASEVIDWIDTH
              && y == 0 && FixedMul(SHORT(patch->height)<<FRACBITS,
      pscale)>>FRACBITS == BASEVIDHEIGHT)
      {
              column = (const column_t *)((const UINT8 *)(patch) +
      LONG(patch->columnofs[0])); source = (const UINT8 *)(column) + 3;
              V_DrawFill(0, 0, BASEVIDWIDTH, BASEVIDHEIGHT, (column->topdelta ==
      0xff ? 31 : source[0]));
      }*/
      ...
      if (vid.width != BASEVIDWIDTH * dupx) {
        ...
        if ((scrn & (V_HORZSCREEN | V_SNAPTOLEFT)) ==
            (V_HORZSCREEN | V_SNAPTOLEFT))
      ...
      if (vid.height != BASEVIDHEIGHT * dupy) {
        // same thing here
        if ((scrn & (V_SPLITSCREEN | V_SNAPTOTOP)) ==
            (V_SPLITSCREEN | V_SNAPTOTOP))
  ...
  for (col = 0; (col >> FRACBITS) < SHORT(patch->width);
    ...
    while (column->topdelta != 0xff) {
      ...
      for (ofs = 0; dest < deststop && (ofs >> FRACBITS) < column->length;
           ofs += rowfrac) {
        if (dest >=
...
void V_DrawCroppedPatch(fixed_t x, fixed_t y, fixed_t pscale, INT32 scrn,
                        patch_t *patch, fixed_t sx, fixed_t sy, fixed_t w,
                        fixed_t h) {
  fixed_t col, ofs, colfrac, rowfrac, fdup;
  INT32 dupx, dupy;
  const column_t *column;
  ...
  if (scrn & V_NOSCALESTART) {
    ...
    if (!(scrn & V_SCALEPATCHMASK)) {
      ...
      // BAD, BAD, BAD, FUCK OFF, STOP, EW, AAAAAAA
      // This does NOT account for transparent pixels
      /*if (x == 0 && SHORT(patch->width) == BASEVIDWIDTH && y == 0 &&
      SHORT(patch->height) == BASEVIDHEIGHT)
      {
              column = (const column_t *)((const UINT8 *)(patch) +
      LONG(patch->columnofs[0])); source = (const UINT8 *)(column) + 3;
              V_DrawFill(0, 0, BASEVIDWIDTH, BASEVIDHEIGHT, (column->topdelta ==
  ...
  for (col = sx << FRACBITS;
       (col >> FRACBITS) < SHORT(patch->width) && (col >> FRACBITS) < w;
    ...
    while (column->topdelta != 0xff) {
      ...
      for (ofs = sy << FRACBITS;
           dest < deststop && (ofs >> FRACBITS) < column->length &&
           (ofs >> FRACBITS) < h;
           ofs += rowfrac) {
        if (dest >=
            screens[scrn & V_PARAMMASK]) // don't draw off the top of the screen
...
void V_DrawContinueIcon(INT32 x, INT32 y, INT32 flags, INT32 skinnum,
  ...
  if (skinnum < 0 || skinnum >= numskins || (skins[skinnum].flags & SF_HIRES))
    V_DrawScaledPatch(x - 10, y - 14, flags,
                      W_CachePatchName("CONTINS", PU_CACHE)); // Draw a star
  else { // Find front angle of the first waiting frame of the character's
         // actual sprites
    spriteframe_t *sprframe =
        &skins[skinnum].spritedef.spriteframes[2 & FF_FRAMEMASK];
...
void V_DrawFillConsoleMap(INT32 x, INT32 y, INT32 w, INT32 h, INT32 c) {
  ...
  if (rendermode == render_opengl) {
    ...
    HWR_DrawConsoleFill(x, y, w, h, hwcolor,
                        c); // we still use the regular color stuff but only for
                            // flags. actual draw color is "hwcolor" for this.
    return;
  }
  ...
  if (!alphalevel) {
    ...
    const UINT8 *fadetable = ((UINT8 *)transtables +
                              ((alphalevel - 1) << FF_TRANSSHIFT) + (c * 256));
...
void V_DrawVhsEffect(boolean rewind) {
  ...
  UINT8 *normalmapstart =
      ((UINT8 *)transtables + (8 << FF_TRANSSHIFT | (19 << 8)));
#ifdef HQ_VHS
  ...
  for (y = 0; y < vid.height; y += 2) {
    ...
    if (y >= upbary && y < upbary + barsize) {
      ...
      offs +=
          updistort * 2.0f * min(y - upbary, upbary + barsize - y) / barsize;
    }
    if (y >= downbary && y < downbary + barsize) {
      thismapstart -= (2 << FF_TRANSSHIFT) - (5 << 8);
      offs -= downdistort * 2.0f * min(y - downbary, downbary + barsize - y) /
              barsize;
    }
    ...
    const UINT8 *fadetable =
        (color > 0xFFF0) // Grab a specific colormap palette?
            ...
            : ((color & 0xFF00) // Color is not palette index?
                   ...
                   : ((UINT8 *)transtables + ((9 - strength) << FF_TRANSSHIFT) +
...
UINT8 *V_GetStringColormap(INT32 colorflags) {
  ...
  if (!colorflags || colorflags > 15) // INT32 is signed, but V_CHARCOLORMASK is
                                      // a very restrictive mask.
    return NULL;
...
// Writes a single character for the chat (half scaled). (draw WHITE if bit 7
// set) 16/02/19: Scratch the scaling thing, chat doesn't work anymore under 2x
// res -Lat'
//
void V_DrawChatCharacter(INT32 x, INT32 y, INT32 c, boolean lowercaseallowed,
                         UINT8 *colormap) {
  INT32 w, flags;
  // const UINT8 *colormap = V_GetStringColormap(c);
  ...
  V_DrawFixedPatch(x * FRACUNIT, y * FRACUNIT, FRACUNIT / 2, flags, hu_font[c],
                   colormap);
}
...
void V_DrawString(INT32 x, INT32 y, INT32 option, const char *string) {
  ...
  for (;; ch++) {
    ...
    V_DrawFixedPatch((cx + center) << FRACBITS, cy << FRACBITS, FRACUNIT,
                     option, hu_font[c], colormap);
...
void V_DrawRightAlignedString(INT32 x, INT32 y, INT32 option,
                              const char *string) {
...
void V_DrawSmallString(INT32 x, INT32 y, INT32 option, const char *string) {
  ...
  for (;; ch++) {
    ...
    V_DrawFixedPatch((cx + center) << FRACBITS, cy << FRACBITS, FRACUNIT / 2,
                     option, hu_font[c], colormap);
...
void V_DrawRightAlignedSmallString(INT32 x, INT32 y, INT32 option,
                                   const char *string) {
...
void V_DrawThinString(INT32 x, INT32 y, INT32 option, const char *string) {
  ...
  for (;; ch++) {
    ...
    if (charwidth)
      ...
      w = ((option & V_6WIDTHSPACE ? max(1,
                                         SHORT(tny_font[c]->width) -
                                             1) // Reuse this flag for the
                                                // alternate bunched-up spacing
                                   : SHORT(tny_font[c]->width)) *
           dupx);
    ...
    V_DrawFixedPatch(cx << FRACBITS, cy << FRACBITS, FRACUNIT, option,
                     tny_font[c], colormap);
...
void V_DrawCenteredThinString(INT32 x, INT32 y, INT32 option,
                              const char *string) {
  x -= V_ThinStringWidth(string, option) / 2;
  V_DrawThinString(x, y, option, string);
}

void V_DrawRightAlignedThinString(INT32 x, INT32 y, INT32 option,
                                  const char *string) {
...
void V_DrawStringAtFixed(fixed_t x, fixed_t y, INT32 option,
                         const char *string) {
  fixed_t cx = x, cy = y;
  ...
  for (;; ch++) {
    ...
    V_DrawSciencePatch(cx + (center << FRACBITS), cy, option, hu_font[c],
                       FRACUNIT);
...
INT32
V_DrawPingNum(INT32 x, INT32 y, INT32 flags, INT32 num, const UINT8 *colormap) {
  INT32 w = SHORT(pingnum[0]->width); // this SHOULD always be 5 but I guess
                                      // custom graphics exist.
  ...
  do {
    ...
    V_DrawFixedPatch(x << FRACBITS, y << FRACBITS, FRACUNIT, flags,
                     pingnum[num % 10], colormap);
    num /= 10;
  } while (num);
...
// NOTE: the text is centered for screens larger than the base width
//
void V_DrawCreditString(fixed_t x, fixed_t y, INT32 option,
                        const char *string) {
...
//
INT32
V_CreditStringWidth(const char *string) {
  INT32 c, w = 0;
  size_t i;
...
//
INT32
V_LevelNameWidth(const char *string) {
  INT32 c, w = 0;
  size_t i;
...
//
INT32
V_LevelNameHeight(const char *string) {
  INT32 c, w = 0;
  size_t i;
...
//
INT32
V_StringWidth(const char *string, INT32 option) {
  INT32 c, w = 0;
  INT32 spacewidth = 4, charwidth = 0;
  size_t i;
...
//
INT32
V_SmallStringWidth(const char *string, INT32 option) {
  INT32 c, w = 0;
  INT32 spacewidth = 2, charwidth = 0;
  size_t i;
...
//
INT32
V_ThinStringWidth(const char *string, INT32 option) {
  INT32 c, w = 0;
  INT32 spacewidth = 2, charwidth = 0;
  ...
  for (i = 0; i < strlen(string); i++) {
    ...
    if (c < 0 || c >= HU_FONTSIZE || !tny_font[c])
      ...
      w += (charwidth ? charwidth
                      : ((option & V_6WIDTHSPACE && i < strlen(string) - 1)
                             ? max(1,
                                   SHORT(tny_font[c]->width) -
                                       1) // Reuse this flag for the alternate
                                          // bunched-up spacing
                             : SHORT(tny_font[c]->width)));
    }
  }
...
void V_DoPostProcessor(INT32 view, postimg_t type, INT32 param) {
  ...
  if (type == postimg_water) {
    ...
    for (y = yoffset; y < yoffset + viewheight; y++) {
      ...
      if (sine < 0) {
        M_Memcpy(&tmpscr[(y * vid.width) + xoffset + newpix],
                 &srcscr[(y * vid.width) + xoffset], viewwidth - newpix);

        // Cleanup edge
        while (newpix) {
          tmpscr[(y * vid.width) + xoffset + newpix] =
              srcscr[(y * vid.width) + xoffset];
          newpix--;
        }
      } else {
        M_Memcpy(&tmpscr[(y * vid.width) + xoffset + 0],
                 &srcscr[(y * vid.width) + xoffset + sine], viewwidth - newpix);

        // Cleanup edge
        while (newpix) {
          tmpscr[(y * vid.width) + xoffset + viewwidth - newpix] =
              srcscr[(y * vid.width) + xoffset + (viewwidth - 1)];
          newpix--;
        }
      }
    ...
    VID_BlitLinearScreen(tmpscr + vid.width * vid.bpp * yoffset + xoffset,
                         screens[0] + vid.width * vid.bpp * yoffset + xoffset,
                         viewwidth * vid.bpp, viewheight, vid.width * vid.bpp,
                         vid.width);
  ...
  } else if (type == postimg_flip) // Flip the screen upside-down
  {
    ...
    for (y = yoffset, y2 = yoffset + viewheight - 1; y < yoffset + viewheight;
         y++, y2--)
      M_Memcpy(&tmpscr[(y2 * vid.width) + xoffset],
               &srcscr[(y * vid.width) + xoffset], viewwidth);
  ...
  } else if (type == postimg_heat) // Heat wave
  {
    ...
    for (y = yoffset; y < yoffset + viewheight; y++) {
      if (heatshifter[heatindex[view]++]) {
        ...
        M_Memcpy(&tmpscr[(y * vid.width) + xoffset],
                 &srcscr[(y * vid.width) + xoffset + vid.dupx],
                 viewwidth - vid.dupx);
      } else
        M_Memcpy(&tmpscr[(y * vid.width) + xoffset],
                 &srcscr[(y * vid.width) + xoffset], viewwidth);
  ...
  } else if (type == postimg_mirror) // Flip the screen on the x axis
  {
    ...
    for (y = yoffset; y < yoffset + viewheight; y++) {
      for (x = xoffset, x2 = xoffset + ((viewwidth * vid.bpp) - 1);
           x < xoffset + (viewwidth * vid.bpp); x++, x2--)
    ...
    for (r = 0; r < CLUTSIZE; r++)
      for (g = 0; g < CLUTSIZE; g++)
        for (b = 0; b < CLUTSIZE; b++)
          colorlookup[r][g][b] = NearestColor(
              r << SHIFTCOLORBITS, g << SHIFTCOLORBITS, b << SHIFTCOLORBITS);
    clutinit = true;
