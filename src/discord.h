...
typedef struct discordRequest_s {
  ...
  char *discriminator; // Discord discriminator (The little hashtag thing after
                       // the username). Separated for a "hide discriminators"
                       // cvar.
  char *userID; // The ID of the Discord user, gets used with Discord_Respond()

  // HAHAHA, no.
  // *Maybe* if it was only PNG I would boot up curl just to get AND convert
  // this to Doom GFX, but it can *also* be a JEPG, WebP, or GIF :) Hey, wanna
  // add ImageMagick as a dependency? :dying:
  // patch_t *avatar;

  struct discordRequest_s *next; // Next request in the list.
  struct discordRequest_s
      *prev; // Previous request in the list. Not used normally, but just in
             // case something funky happens, this should repair the list.
} discordRequest_t;
...
/*--------------------------------------------------
        void DRPC_UpdatePresence(void);
                ...
                Updates what is displayed by Rich Presence on the user's
profile. Should be called whenever something that is displayed is changed
in-game.
--------------------------------------------------*/
