...
const char *const sfxinfo_ropt[] = {"name",  "singular",  "priority",
                                    "flags", "skinsound", NULL};
...
const char *const sfxinfo_wopt[] = {"singular", "priority", "flags", NULL};
...
static int lib_getState(lua_State *L) {
  ...
  if (i >= NUMSTATES)
    return luaL_error(L, "states[] index %d out of range (0 - %d)", i,
                      NUMSTATES - 1);
...
static int lib_setState(lua_State *L) {
  ...
  lua_settop(L, 1); // cut the stack here. the only thing left now is the table
                    // of data we're assigning to the state.
  ...
  while (lua_next(L, 1)) {
    ...
    if (i == 1 || (str && fastcmp(str, "sprite"))) {
      ...
      case LUA_TSTRING: // It's a string, expect the name of a built-in action
        ...
        break;
      case LUA_TFUNCTION: // It's a function (a Lua function or a C function?
                          // either way!)
        lua_getfield(L, LUA_REGISTRYINDEX, LREG_STATEACTION);
        I_Assert(lua_istable(L, -1));
        lua_pushlightuserdata(L, state); // We'll store this function by the
                                         // state's pointer in the registry.
        lua_pushvalue(L, 3);             // Bring it to the top of the stack
        lua_rawset(L, -3);               // Set it in the registry
        lua_pop(L, 1);                   // pop LREG_STATEACTION
        state->action.acp1 =
            (actionf_p1)A_Lua; // Set the action for the userdata.
        break;
      default: // ?!
...
boolean LUA_SetLuaAction(void *stv, const char *action) {
  ...
  I_Assert(st != NULL);
  // I_Assert(st >= states && st < states+NUMSTATES); // if you REALLY want to
  // be paranoid...
  ...
  lua_pushlightuserdata(
      gL,
      stv); // We'll store this function by the state's pointer in the registry.
...
boolean LUA_CallAction(const char *csaction, mobj_t *actor) {
  ...
  if (superstack &&
      fasticmp(csaction,
               superactions[superstack - 1])) // the action is calling itself,
  ...
  if (superstack == MAXRECURSION) {
    CONS_Alert(CONS_WARNING,
               "Max Lua Action recursion reached! Cool it on the calling "
               "A_Action functions from inside A_Action functions!\n");
    return true;
  }
...
static int state_get(lua_State *L) {
  ...
  if (fastcmp(field, "sprite"))
    ...
    if (!name)  // If it's not a hardcoded function and it's not a Lua
                // function...
      return 0; // Just what is this??
    // get the function from the global
    // because the metatable will trigger.
    lua_getglobal(L, name);  // actually gets from LREG_ACTIONS if applicable,
                             // and pushes a new C closure if not.
    lua_pushstring(L, name); // push the name we found.
    return 2; // return both the function and its name, in case somebody wanted
              // to do a comparison by name or something?
    ...
    return luaL_error(L, LUA_QL("state_t") " has no field named " LUA_QS,
                      field);
  else
    return 0;
...
static int state_set(lua_State *L) {
  ...
  if (fastcmp(field, "sprite")) {
    ...
    case LUA_TFUNCTION: // It's a function (a Lua function or a C function?
      ...
      lua_getfield(L, LUA_REGISTRYINDEX, LREG_STATEACTION);
      I_Assert(lua_istable(L, -1));
      lua_pushlightuserdata(L, st);        // We'll store this function by the
                                           // state's pointer in the registry.
      lua_pushvalue(L, 3);                 // Bring it to the top of the stack
      lua_rawset(L, -3);                   // Set it in the registry
      lua_pop(L, 1);                       // pop LREG_STATEACTION
...
static int lib_getMobjInfo(lua_State *L) {
  ...
  i = luaL_checkinteger(L, 1);
  if (i >= NUMMOBJTYPES)
    return luaL_error(L, "mobjinfo[] index %d out of range (0 - %d)", i,
                      NUMMOBJTYPES - 1);
...
static int lib_setMobjInfo(lua_State *L) {
  ...
  lua_settop(L, 1); // cut the stack here. the only thing left now is the table
                    // of data we're assigning to the mobjinfo.
...
static int mobjinfo_get(lua_State *L) {
  ...
  if (fastcmp(field, "doomednum"))
    ...
    lua_pushinteger(
        L, info->speed); // sometimes it's fixed_t, sometimes it's not...
...
static int mobjinfo_set(lua_State *L) {
  ...
  // else
  // return luaL_error(L, LUA_QL("mobjinfo_t") " has no field named " LUA_QS,
  // field);
  return 0;
}
...
static int lib_getSfxInfo(lua_State *L) {
  ...
  i = luaL_checkinteger(L, 1);
  if (i >= NUMSFX)
    return luaL_error(L, "sfxinfo[] index %d out of range (0 - %d)", i,
                      NUMSFX - 1);
