...
/**	\brief	Returns precise time value for performance measurement. The
   precise time should be a monotonically increasing counter, and will wrap.
                        precise_t is internally represented as an unsigned
   integer and integer arithmetic may be used directly between values of
   precise_t.
  */
...
/**	\brief	Sleeps for the given duration in milliseconds. Depending on the
            operating system's scheduler, the calling thread may give up its
                        time slice for a longer duration. The implementation
   should give a best effort to sleep for the given duration, without
   spin-locking. Calling code should check the current precise time after
   sleeping and not assume the thread has slept for the expected duration.
...
typedef struct JoyFF_s {
  ...
  // RAMPFORCE
  INT32 Start; ///< Magnitude at the start of the effect, in the range from
               ///< -10,000 through 10,000.
  INT32 End; ///< Magnitude at the end of the effect, in the range from -10,000
             ///< through 10,000.
  // PERIODIC
  INT32 Offset;  ///< Offset of the effect.
  UINT32 Phase;  ///< Position in the cycle of the periodic effect at which
                 ///< playback begins, in the range from 0 through 35,999
...
/**	\brief Setup signal handler, plus stuff for trapping errors and cleanly
 * exit.
 */
INT32
