...
//
static dirtype_t opposite[] = {DI_WEST,      DI_SOUTHWEST, DI_SOUTH,
                               DI_SOUTHEAST, DI_EAST,      DI_NORTHEAST,
                               DI_NORTH,     DI_NORTHWEST, DI_NODIR};

static dirtype_t diags[] = {DI_NORTHWEST, DI_NORTHEAST, DI_SOUTHWEST,
                            DI_SOUTHEAST};
...
boolean P_JetbCheckMeleeRange(mobj_t *actor) {
  ...
  if (actor->eflags & MFE_VERTICALFLIP) {
    if (pl->z <
        actor->z + actor->height + FixedMul(40 << FRACBITS, actor->scale))
      return false;
  } else {
...
boolean P_SkimCheckMeleeRange(mobj_t *actor) {
  ...
  if (actor->eflags & MFE_VERTICALFLIP) {
    if (pl->z <
        actor->z + actor->height + FixedMul(24 << FRACBITS, actor->scale))
      return false;
  } else {
...
boolean P_CheckMissileRange(mobj_t *actor) {
  ...
  dist = P_AproxDistance(actor->x - actor->target->x,
         ...
         FixedMul(64 * FRACUNIT, actor->scale);

  if (!actor->info->meleestate)
    dist -=
        FixedMul(128 * FRACUNIT, actor->scale); // no melee attack, so fire more
...
static const fixed_t xspeed[NUMDIRS] = {
    FRACUNIT,  46341 >> (16 - FRACBITS),    0, -(46341 >> (16 - FRACBITS)),
    -FRACUNIT, -(46341 >> (16 - FRACBITS)), 0, 46341 >> (16 - FRACBITS)};
static const fixed_t yspeed[NUMDIRS] = {
    0, 46341 >> (16 - FRACBITS),    FRACUNIT,  46341 >> (16 - FRACBITS),
    0, -(46341 >> (16 - FRACBITS)), -FRACUNIT, -(46341 >> (16 - FRACBITS))};
...
boolean P_Move(mobj_t *actor, fixed_t speed) {
  ...
  if (actor->type == MT_SKIM &&
      !P_WaterInSector(actor, tryx, tryy)) // bail out if sector lacks water
    return false;
...
boolean P_LookForPlayers(mobj_t *actor, boolean allaround, boolean tracer,
  ...
  for (;; actor->lastlook = (actor->lastlook + 1) & PLAYERSMASK) {
    ...
    if (dist > 0 && P_AproxDistance(P_AproxDistance(player->mo->x - actor->x,
                                                    player->mo->y - actor->y),
                                    player->mo->z - actor->z) > dist)
...
static boolean P_LookForShield(mobj_t *actor) {
  ...
  for (;; actor->lastlook = ((actor->lastlook + 1) & PLAYERSMASK)) {
    ...
    if ((player->powers[pw_shield] & SH_NOSTACK) == SH_ATTRACT &&
        (P_AproxDistance(P_AproxDistance(actor->x - player->mo->x,
                                         actor->y - player->mo->y),
                         actor->z - player->mo->z) <
         FixedMul(RING_DIST / 4, player->mo->scale))) {
      P_SetTarget(&actor->tracer, player->mo);
      return true;
    }
  }
...
static int P_RecycleCompare(const void *p1, const void *p2) {
  ...
  if (!G_RaceGametype()) {
    ...
    if (player1->powers[pw_invulnerability] >
        player2->powers[pw_invulnerability])
      return -1;
    else if (player2->powers[pw_invulnerability] >
             player1->powers[pw_invulnerability])
      return 1;

    // One has a shield, the other doesn't.
    if (player1->powers[pw_shield] && !player2->powers[pw_shield])
      return -1;
    else if (player2->powers[pw_shield] && !player1->powers[pw_shield])
      return 1;

    // Sneakers.
    if (player1->powers[pw_sneakers] > player2->powers[pw_sneakers])
      return -1;
    else if (player2->powers[pw_sneakers] > player1->powers[pw_sneakers])
      return 1;
  } else // Match, Team Match, CTF, Tag, Etc.
  {
    ...
    UINT8 player1_rw =
        M_CountBits((UINT32)player1->ringweapons, NUM_WEAPONS - 1);
    UINT8 player2_rw =
        M_CountBits((UINT32)player2->ringweapons, NUM_WEAPONS - 1);
    ...
    if (player1->powers[pw_super] && !player2->powers[pw_super])
      return -1;
    else if (player2->powers[pw_super] && !player1->powers[pw_super])
      return 1;

    // Emerald count if neither player is Super.
    if (player1_em > player2_em)
      return -1;
    else if (player1_em < player2_em)
      return 1;

    // One has a shield, the other doesn't.
    // (the likelihood of a shielded player being worse off than one without one
    // is low.)
    if (player1->powers[pw_shield] && !player2->powers[pw_shield])
      return -1;
    else if (player2->powers[pw_shield] && !player1->powers[pw_shield])
    ...
    // Ring weapons count
    if (player1_rw > player2_rw)
      return -1;
    else if (player1_rw < player2_rw)
      return 1;

    // Ring ammo if they have the same number of weapons
    if (player1_am > player2_am)
      return -1;
    else if (player1_am < player2_am)
      return 1;
  }
...
// var2 = If 1, only change to seestate. If 2, only play seesound. If 0, do
// both.
//
void A_Look(mobj_t *actor) {
  INT32 locvar1 = var1;
  ...
  if (!P_LookForPlayers(actor, locvar1 & 65535, false,
                        FixedMul((locvar1 >> 16) * FRACUNIT, actor->scale)))
    return;
...
nomissile:
  ...
  if (multiplayer && !actor->threshold &&
...
void A_JetJawRoam(mobj_t *actor) {
  ...
  if (actor->reactiontime) {
    actor->reactiontime--;
    P_InstaThrust(actor, actor->angle,
                  FixedMul(actor->info->speed * FRACUNIT / 4, actor->scale));
  } else {
...
void A_PointyThink(mobj_t *actor) {
  ...
  angle_t fa;
  fixed_t radius =
      FixedMul(actor->info->radius * actor->info->reactiontime, actor->scale);
  ...
  if (P_AproxDistance(player->mo->x - actor->x, player->mo->y - actor->y) <
      P_AproxDistance(player->mo->x + player->mo->momx - actor->x,
  ...
  while (ball) {
    ...
    res = VectorMatrixMultiply(v,
                               *RotateXMatrix(FixedAngle(actor->lastlook + i)));
...
void A_HoodThink(mobj_t *actor) {
  ...
  if ((P_AproxDistance(actor->x - actor->target->x,
                       actor->y - actor->target->y) >
       FixedMul(192 * FRACUNIT, actor->scale)) ||
      (actor->spawnpoint && (actor->spawnpoint->options &
                             MTF_AMBUSH))) // If you can't jump, might as well
                                           // shoot regardless of distance!
    P_SetMobjState(actor, actor->info->missilestate);
  else if (!(actor->spawnpoint &&
             (actor->spawnpoint->options & MTF_AMBUSH))) // But we WILL jump!
...
void A_SnailerThink(mobj_t *actor) {
  ...
  if (actor->target->z + actor->target->height >
          actor->z - FixedMul(32 * FRACUNIT, actor->scale) &&
      actor->target->z <
          actor->z + actor->height + FixedMul(32 * FRACUNIT, actor->scale) &&
      !(leveltime % (TICRATE * 2))) {
    angle_t an;
    fixed_t z;
    ...
    if (an > ANGLE_45 &&
        an < ANGLE_315) // fire as close as you can to the target, even if too
                        // sharp an angle from your front
    {
      fixed_t dist;
      fixed_t dx, dy;
      ...
      } else if (an >= ANGLE_270 &&
                 an < ANGLE_315) // fire at 45 degrees to the right
      {
        dx = actor->x + P_ReturnThrustX(actor, actor->angle - ANGLE_45, dist);
        dy = actor->y + P_ReturnThrustY(actor, actor->angle - ANGLE_45, dist);
      ...
      P_SpawnPointMissile(actor, dx, dy, actor->target->z, MT_ROCKET, actor->x,
                          actor->y, z);
    } else
  ...
  if ((!(actor->eflags & MFE_VERTICALFLIP) && actor->target->z > actor->z) ||
      (actor->eflags & MFE_VERTICALFLIP &&
       (actor->target->z + actor->target->height) > (actor->z + actor->height)))
...
void A_SharpSpin(mobj_t *actor) {
  ...
  if (actor->threshold && actor->target) {
    ...
    P_Thrust(
        ...
        FixedMul(actor->info->speed * FRACUNIT, actor->scale));
    actor->threshold--;
  } else {
    actor->reactiontime = actor->info->reactiontime;
    P_SetMobjState(actor, actor->info->spawnstate);
...
void A_VultureVtol(mobj_t *actor) {
  ...
  if (actor->z < actor->target->z + (actor->target->height / 4) &&
      actor->z + actor->height < actor->ceilingz)
    actor->momz = FixedMul(2 * FRACUNIT, actor->scale);
  else if (actor->z > (actor->target->z + (actor->target->height / 4) * 3) &&
...
void A_SkimChase(mobj_t *actor) {
  ...
  if (!actor->target || !(actor->target->flags & MF_SHOOTABLE)) {
    ...
    // the spawnstate for skims already calls this function so just return
    // either way without changing state
    return;
  }
...
void A_FaceTracer(mobj_t *actor) {
  ...
  actor->angle =
      R_PointToAngle2(actor->x, actor->y, actor->tracer->x, actor->tracer->y);
}
...
void A_LobShot(mobj_t *actor) {
  ...
  hitspot = P_SpawnMobj(actor->target->x & (64 * FRACUNIT - 1),
                        actor->target->y & (64 * FRACUNIT - 1),
                        actor->target->subsector->sector->floorheight, MT_NULL);
  ...
  dist =
      P_AproxDistance(actor->target->x - shot->x, actor->target->y - shot->y);
...
void A_FireShot(mobj_t *actor) {
  ...
  if (actor->eflags & MFE_VERTICALFLIP)
    z = actor->z + actor->height -
        FixedMul(48 * FRACUNIT + locvar2 * FRACUNIT, actor->scale);
  else
...
void A_BossFireShot(mobj_t *actor) {
  ...
  switch (locvar2) {
  case 0:
    x = actor->x + P_ReturnThrustX(actor, actor->angle + ANGLE_90,
                                   FixedMul(43 * FRACUNIT, actor->scale));
    y = actor->y + P_ReturnThrustY(actor, actor->angle + ANGLE_90,
                                   FixedMul(43 * FRACUNIT, actor->scale));
    if (actor->eflags & MFE_VERTICALFLIP)
  ...
  case 2:
    x = actor->x + P_ReturnThrustX(actor, actor->angle - ANGLE_90,
                                   FixedMul(56 * FRACUNIT, actor->scale));
    y = actor->y + P_ReturnThrustY(actor, actor->angle - ANGLE_90,
                                   FixedMul(56 * FRACUNIT, actor->scale));
    if (actor->eflags & MFE_VERTICALFLIP)
  ...
  case 3:
    x = actor->x + P_ReturnThrustX(actor, actor->angle - ANGLE_90,
                                   FixedMul(58 * FRACUNIT, actor->scale));
    y = actor->y + P_ReturnThrustY(actor, actor->angle - ANGLE_90,
                                   FixedMul(58 * FRACUNIT, actor->scale));
    if (actor->eflags & MFE_VERTICALFLIP)
...
//
// Description: Shoot 4 missiles of a specific object type at your target ala
// Black Eggman
//
// var1 = object # to shoot
// var2 = firing sound
...
void A_Boss7FireMissiles(mobj_t *actor) {
  ...
  dummymo.z =
      actor->target->z + FixedMul(16 * FRACUNIT, actor->scale); // raised height

  P_SpawnXYZMissile(
      actor, &dummymo, locvar1,
      actor->x + P_ReturnThrustX(actor, actor->angle - ANGLE_90,
                                 FixedDiv(actor->radius, 3 * FRACUNIT / 2) +
                                     FixedMul(4 * FRACUNIT, actor->scale)),
      actor->y + P_ReturnThrustY(actor, actor->angle - ANGLE_90,
                                 FixedDiv(actor->radius, 3 * FRACUNIT / 2) +
                                     FixedMul(4 * FRACUNIT, actor->scale)),
...
void A_Boss1Laser(mobj_t *actor) {
  ...
  case 0:
    ...
    y = actor->y + P_ReturnThrustY(actor, actor->angle + ANGLE_90,
                                   FixedMul(43 * FRACUNIT, actor->scale));
    if (actor->eflags & MFE_VERTICALFLIP)
      z = actor->z + actor->height - FixedMul(56 * FRACUNIT, actor->scale) -
          mobjinfo[locvar1].height;
    else
  ...
  if (!(actor->flags2 & MF2_FIRING)) {
    ...
    if (!(actor->spawnpoint && actor->spawnpoint->options & MTF_AMBUSH)) {
      point =
          P_SpawnMobj(x + P_ReturnThrustX(actor, actor->angle, actor->radius),
                      y + P_ReturnThrustY(actor, actor->angle, actor->radius),
                      actor->z - actor->height / 2, MT_EGGMOBILE_TARGET);
      point->fuse = actor->tics + 1;
      P_SetTarget(&point->target, actor->target);
      P_SetTarget(&actor->target, point);
    }
  } else if (actor->target &&
             !(actor->spawnpoint && actor->spawnpoint->options & MTF_AMBUSH))
    actor->angle = R_PointToAngle2(x, y, actor->target->x, actor->target->y);

  if (actor->spawnpoint && actor->spawnpoint->options & MTF_AMBUSH)
    angle = FixedAngle(
        FixedDiv(actor->tics * 160 * FRACUNIT, actor->state->tics * FRACUNIT) +
        10 * FRACUNIT);
  else
    angle = R_PointToAngle2(
        z + (mobjinfo[locvar1].height >> 1), 0, actor->target->z,
  ...
  point->momx =
      FixedMul(FINESINE(angle >> ANGLETOFINESHIFT),
               FixedMul(FINECOSINE(point->angle >> ANGLETOFINESHIFT), speed));
  point->momy =
      FixedMul(FINESINE(angle >> ANGLETOFINESHIFT),
               FixedMul(FINESINE(point->angle >> ANGLETOFINESHIFT), speed));
...
void A_FocusTarget(mobj_t *actor) {
  ...
  if (actor->target) {
    ...
    fixed_t dist =
        R_PointToDist2(actor->x, actor->y, actor->target->x, actor->target->y);
    angle_t vangle = R_PointToAngle2(
        actor->z, 0, actor->target->z + (actor->target->height >> 1), dist);
    angle_t hangle =
        R_PointToAngle2(actor->x, actor->y, actor->target->x, actor->target->y);
    switch (locvar1) {
    case 0: {
      actor->momx -= actor->momx >> 4, actor->momy -= actor->momy >> 4,
          actor->momz -= actor->momz >> 4;
      actor->momz += FixedMul(FINECOSINE(vangle >> ANGLETOFINESHIFT), speed);
      actor->momx +=
          FixedMul(FINESINE(vangle >> ANGLETOFINESHIFT),
                   FixedMul(FINECOSINE(hangle >> ANGLETOFINESHIFT), speed));
      actor->momy +=
          FixedMul(FINESINE(vangle >> ANGLETOFINESHIFT),
                   FixedMul(FINESINE(hangle >> ANGLETOFINESHIFT), speed));
    } break;
    case 1:
      if (dist > speed) {
        actor->momz = FixedMul(FINECOSINE(vangle >> ANGLETOFINESHIFT), speed);
...
void A_BossZoom(mobj_t *actor) {
  ...
  an = actor->angle >> ANGLETOFINESHIFT;
  actor->momx =
      FixedMul(FixedMul(actor->info->speed * 5 * FRACUNIT, actor->scale),
               FINECOSINE(an));
  actor->momy = FixedMul(
      FixedMul(actor->info->speed * 5 * FRACUNIT, actor->scale), FINESINE(an));
  dist = P_AproxDistance(dest->x - actor->x, dest->y - actor->y);
...
//
// Description: Spawns explosions and plays appropriate sounds around the
// defeated boss.
//
// var1:
...
void A_BossScream(mobj_t *actor) {
  ...
  case 0:
    ...
    actor->movecount %= 360;
    fa = (FixedAngle(actor->movecount * FRACUNIT) >> ANGLETOFINESHIFT) &
         FINEMASK;
    break;
  case 1:
    fa = (FixedAngle(P_RandomKey(360) * FRACUNIT) >> ANGLETOFINESHIFT) &
         FINEMASK;
    break;
  }
  ...
  // Determine what mobj to spawn. If undefined or invalid, use MT_BOSSEXPLODE
  // as default.
  ...
  if (actor->eflags & MFE_VERTICALFLIP)
    z = actor->z + actor->height - mobjinfo[explodetype].height -
        FixedMul((P_RandomByte() << (FRACBITS - 2)) - 8 * FRACUNIT,
                 actor->scale);
  else
    z = actor->z + FixedMul((P_RandomByte() << (FRACBITS - 2)) - 8 * FRACUNIT,
                            actor->scale);
...
void A_Scream(mobj_t *actor) {
  if (LUA_CallAction("A_Scream", actor))
    return;
  if (actor->tracer &&
      (actor->tracer->type == MT_SHELL || actor->tracer->type == MT_FIREBALL))
...
// Function: A_Fall
//
// Description: Changes a dying object's flags to reflect its having fallen to
// the ground.
//
// var1 = unused
// var2 = unused
...
void A_1upThinker(mobj_t *actor) {
  ...
  for (i = 0; i < MAXPLAYERS; i++) {
    ...
    temp = P_AproxDistance(players[i].mo->x - actor->x,
                           players[i].mo->y - actor->y);
  ...
  if (closestplayer == -1 ||
      skins[players[closestplayer].skin].spritedef.numframes <=
          states[S_PLAY_BOX1]
              .frame) { // Closest player not found (no players in
                        // game?? may be empty dedicated server!),
                        // or does not have correct sprite.
    actor->frame = 0;
    if (actor->tracer) {
...
void A_MonitorPop(mobj_t *actor) {
  ...
  remains =
      P_SpawnMobj(actor->x, actor->y,
                  ((actor->eflags & MFE_VERTICALFLIP)
                       ? (actor->z + 3 * (actor->height / 4) -
                          FixedMul(mobjinfo[explode].height, actor->scale))
  ...
  remains = P_SpawnMobj(
      ...
      ((actor->eflags & MFE_VERTICALFLIP)
           ? (actor->z + actor->height -
              FixedMul(mobjinfo[actor->info->speed].height, actor->scale))
           : actor->z),
      actor->info->speed);
...
#undef QUESTIONBOXCHANCES
          ...
          if (numchoices == 0)
          {
                  CONS_Alert(CONS_WARNING, M_GetText("All monitors turned
off.\n")); return;
          }
  ...
  if (item != 0) {
    ...
    if (actor->eflags & MFE_VERTICALFLIP) {
      newmobj = P_SpawnMobj(
          actor->x, actor->y,
          actor->z + actor->height -
              FixedMul(13 * FRACUNIT + mobjinfo[item].height, actor->scale),
          item);
      newmobj->eflags |= MFE_VERTICALFLIP;
    } else
      newmobj =
          P_SpawnMobj(actor->x, actor->y,
                      actor->z + FixedMul(13 * FRACUNIT, actor->scale), item);
    ...
    if (item == MT_1UPICO && newmobj->target->player) {
      ...
      if (!newmobj->target->skin ||
          ((skin_t *)newmobj->target->skin)->spritedef.numframes <=
              states[S_PLAY_BOX1].frame)
    ...
    CONS_Debug(DBG_GAMELOGIC,
               "Powerup item not defined in 'damage' field for A_MonitorPop\n");
...
// Function: A_Explode
//
// Description: Explodes an object, doing damage to any objects nearby. The
// target is used as the cause of the explosion. Damage value is used as
// explosion range.
//
// var1 = unused
// var2 = unused
...
void A_BossDeath(mobj_t *mo) {
  ...
  for (i = 0; i < MAXPLAYERS; i++)
    if (playeringame[i] &&
        (players[i].health > 0 ||
         ((netgame || multiplayer) &&
          (players[i].lives > 0 || players[i].continues > 0))))
      break;
...
bossjustdie:
  ...
  } else // eggmobiles
  {
    ...
    for (th = thinkercap.next; th != &thinkercap; th = th->next) {
      ...
      if (mo2->type == MT_BOSSFLYPOINT) {
        ...
        if (!mo->target ||
            P_AproxDistance(P_AproxDistance(mo->x - mo2->x, mo->y - mo2->y),
                            mo->z - mo2->z) <
                P_AproxDistance(P_AproxDistance(mo->x - mo->target->x,
                                                mo->y - mo->target->y),
                                mo->z - mo->target->z))
    ...
    if (mo->target) {
      ...
      mo->momz = FixedMul(FixedDiv(mo->target->z - mo->z,
                                   P_AproxDistance(mo->x - mo->target->x,
                                                   mo->y - mo->target->y)),
  ...
  if (mo->type == MT_EGGMOBILE2) {
    mo2 =
        P_SpawnMobj(mo->x + P_ReturnThrustX(mo, mo->angle - ANGLE_90,
                                            FixedMul(32 * FRACUNIT, mo->scale)),
                    mo->y + P_ReturnThrustY(mo, mo->angle - ANGLE_90,
                                            FixedMul(32 * FRACUNIT, mo->scale)),
                    mo->z + mo->height / 2 +
                        ((mo->eflags & MFE_VERTICALFLIP)
                             ? FixedMul(8 * FRACUNIT, mo->scale) -
                                   mobjinfo[MT_BOSSTANK1].height
                             : -FixedMul(8 * FRACUNIT, mo->scale)),
                    MT_BOSSTANK1); // Right tank
    mo2->angle = mo->angle;
    ...
    P_InstaThrust(mo2, mo2->angle - ANGLE_90,
                  FixedMul(4 * FRACUNIT, mo2->scale));
    ...
    mo2 =
        P_SpawnMobj(mo->x + P_ReturnThrustX(mo, mo->angle + ANGLE_90,
                    ...
                    mo->z + mo->height / 2 +
                        ((mo->eflags & MFE_VERTICALFLIP)
                             ? FixedMul(8 * FRACUNIT, mo->scale) -
                                   mobjinfo[MT_BOSSTANK2].height
                             : -FixedMul(8 * FRACUNIT, mo->scale)),
                    MT_BOSSTANK2); // Left tank
    mo2->angle = mo->angle;
    mo2->destscale = mo->scale;
    P_SetScale(mo2, mo2->destscale);
    if (mo->eflags & MFE_VERTICALFLIP) {
    ...
    mo2 = P_SpawnMobj(
        mo->x, mo->y,
        mo->z + ((mo->eflags & MFE_VERTICALFLIP)
                     ? mobjinfo[MT_BOSSSPIGOT].height -
                           FixedMul(32 * FRACUNIT, mo->scale)
                     : mo->height + FixedMul(32 * FRACUNIT, mo->scale)),
        MT_BOSSSPIGOT);
    mo2->angle = mo->angle;
    mo2->destscale = mo->scale;
    P_SetScale(mo2, mo2->destscale);
...
//
// Description: Provides a custom powerup. Target (must be a player) is awarded
// the powerup. Reactiontime of the object is used as an index to the powers
// array.
//
// var1 = Power index #
...
void A_JumpShield(mobj_t *actor) {
  ...
  if ((player->powers[pw_shield] & SH_NOSTACK) != SH_JUMP) {
    player->powers[pw_shield] =
        SH_JUMP | (player->powers[pw_shield] & SH_STACK);
    P_SpawnShieldOrb(player);
  }
...
void A_RingShield(mobj_t *actor) {
  ...
  if ((player->powers[pw_shield] & SH_NOSTACK) != SH_ATTRACT) {
    player->powers[pw_shield] =
        SH_ATTRACT | (player->powers[pw_shield] & SH_STACK);
    P_SpawnShieldOrb(player);
  }
...
void A_SuperSneakers(mobj_t *actor) {
  ...
  if (P_IsLocalPlayer(player) && !player->powers[pw_super]) {
    if (S_SpeedMusic(0.0f) &&
        (mapheaderinfo[gamemap - 1]->levelflags & LF_SPEEDMUSIC))
      S_SpeedMusic(1.4f);
    else {
      S_StopMusic();
...
void A_BombShield(mobj_t *actor) {
  ...
  if ((player->powers[pw_shield] & SH_NOSTACK) != SH_BOMB) {
    player->powers[pw_shield] =
        SH_BOMB | (player->powers[pw_shield] & SH_STACK);
    P_SpawnShieldOrb(player);
  }
...
void A_WaterShield(mobj_t *actor) {
  ...
  if ((player->powers[pw_shield] & SH_NOSTACK) != SH_ELEMENTAL) {
    player->powers[pw_shield] =
        SH_ELEMENTAL | (player->powers[pw_shield] & SH_STACK);
    P_SpawnShieldOrb(player);
  }

  /* // SRB2kart - Can't drown.
  if (player->powers[pw_underwater] && player->powers[pw_underwater] <=
  12*TICRATE + 1) P_RestoreMusic(player);
  */
...
void A_ForceShield(mobj_t *actor) {
  ...
  if (!(player->powers[pw_shield] & SH_FORCE)) {
    player->powers[pw_shield] =
        SH_FORCE | (player->powers[pw_shield] & SH_STACK) | 0x01;
    P_SpawnShieldOrb(player);
  } else
    player->powers[pw_shield] =
        SH_FORCE | (player->powers[pw_shield] & SH_STACK) | 0x01;
...
void A_PityShield(mobj_t *actor) {
  ...
  if ((player->powers[pw_shield] & SH_NOSTACK) != SH_PITY) {
    player->powers[pw_shield] =
        SH_PITY + (player->powers[pw_shield] & SH_STACK);
    P_SpawnShieldOrb(player);
  }
...
void A_ParticleSpawn(mobj_t *actor) {
  ...
  spawn->scalespeed = FixedDiv(
      ((actor->spawnpoint->angle >> 8) & 63) * actor->scale, 100 << FRACBITS);
...
void A_BunnyHop(mobj_t *actor) {
  ...
  if (((actor->eflags & MFE_VERTICALFLIP) &&
    ...
    P_InstaThrust(actor, actor->angle,
                  FixedMul(locvar2 * FRACUNIT,
                           actor->scale)); // Launch the hopping action! PHOOM!!
  }
}

// Function: A_BubbleSpawn
//
// Description: Spawns a randomly sized bubble from the object's location. Only
// works underwater.
//
// var1 = Distance to look for players.  If no player is in this distance,
// bubbles aren't spawned. (Ambush overrides) var2 = unused
//
void A_BubbleSpawn(mobj_t *actor) {
  ...
  if (!(actor->flags2 & MF2_AMBUSH)) {
    ...
    for (i = 0; i < MAXPLAYERS; ++i)
      if (playeringame[i] && players[i].mo &&
          P_AproxDistance(actor->x - players[i].mo->x,
                          actor->y - players[i].mo->y) < (locvar1 << FRACBITS))
        break; // Stop looking.
    if (i == MAXPLAYERS)
  ...
  if (leveltime % (3 * TICRATE) < 8)
    bubble = P_SpawnMobj(actor->x, actor->y, actor->z + (actor->height / 2),
                         MT_EXTRALARGEBUBBLE);
  else if (prandom > 128)
    bubble = P_SpawnMobj(actor->x, actor->y, actor->z + (actor->height / 2),
                         MT_SMALLBUBBLE);
...
void A_BubbleRise(mobj_t *actor) {
  ...
  if (actor->type == MT_EXTRALARGEBUBBLE)
    P_SetObjectMomZ(actor, FixedDiv(6 * FRACUNIT, 5 * FRACUNIT),
                    false); // make bubbles rise!
  else {
    ...
    if (!locvar1) {
      ...
      {
        P_InstaThrust(actor,
                      prandom & 0x70 ? actor->angle + ANGLE_90 : actor->angle,
                      FixedMul(prandom & 0xF0 ? FRACUNIT / 2 : -FRACUNIT / 2,
                               actor->scale));
      } else if (!(prandom & 0x38)) // **000***
      {
        P_InstaThrust(actor,
                      prandom & 0x70 ? actor->angle - ANGLE_90
                                     : actor->angle - ANGLE_180,
                      FixedMul(prandom & 0xF0 ? FRACUNIT / 2 : -FRACUNIT / 2,
                               actor->scale));
      }
...
//
// Description: Checks if a bubble should be drawn or not. Bubbles are not drawn
// above water.
//
// var1 = unused
// var2 = unused
...
//
// Description: Makes a ring chase after a player with a ring shield and also
// causes spilled rings to flicker.
//
// var1 = unused
// var2 = unused
//
void A_AttractChase(mobj_t *actor) {
  ...
  if (leveltime & 1 && actor->type == (mobjtype_t)actor->info->reactiontime &&
      actor->fuse && actor->fuse < 2 * TICRATE)
    actor->flags2 |= MF2_DONTDRAW;
  else
    actor->flags2 &= ~MF2_DONTDRAW;
  ...
  if (actor->tracer && actor->tracer->player &&
      ...
      actor->type != (mobjtype_t)actor->info->reactiontime) {
    mobj_t *newring;
    newring =
        P_SpawnMobj(actor->x, actor->y, actor->z, actor->info->reactiontime);
  ...
  if (!actor->tracer || !actor->tracer->player || !actor->tracer->health ||
      !P_CheckSight(actor,
                    actor->tracer)) // You have to be able to SEE it...sorta
  {
...
// Function: A_DropMine
//
// Description: Drops a mine. Raisestate specifies the object # to use for the
// mine.
//
// var1 = height offset
// var2:
...
void A_DropMine(mobj_t *actor) {
  ...
  if (locvar2 & 65535) {
    ...
    dist =
        P_AproxDistance(actor->x - target->x, actor->y - target->y) >> FRACBITS;
  ...
  if (actor->eflags & MFE_VERTICALFLIP)
    z = actor->z + actor->height - mobjinfo[actor->info->raisestate].height -
        FixedMul((locvar1 * FRACUNIT) - 12 * FRACUNIT, actor->scale);
  else
    z = actor->z + FixedMul((locvar1 * FRACUNIT) - 12 * FRACUNIT, actor->scale);

  // Use raisestate instead of MT_MINE
  mine =
      P_SpawnMobj(actor->x, actor->y, z, (mobjtype_t)actor->info->raisestate);
  if (actor->eflags & MFE_VERTICALFLIP)
...
//
// var1 = Jump strength (in FRACBITS), if specified. Otherwise, uses the angle
// value. var2 = unused
//
void A_FishJump(mobj_t *actor) {
  INT32 locvar1 = var1;
  ...
  if ((actor->z <= actor->floorz) ||
      (actor->z <=
       actor->watertop - FixedMul((64 << FRACBITS), actor->scale))) {
    fixed_t jumpval;
    ...
    if (!jumpval)
      jumpval = FixedMul(44 * (FRACUNIT / 4), actor->scale);
    actor->momz = jumpval;
    P_SetMobjStateNF(actor, actor->info->seestate);
  }

  if (actor->momz < 0 && (actor->state < &states[actor->info->meleestate] ||
                          actor->state > &states[actor->info->xdeathstate]))
...
void A_ThrownRing(mobj_t *actor) {
  ...
  if (leveltime % (TICRATE / 7) == 0) {
    ...
    if (actor->flags2 & MF2_EXPLOSION) {
      ...
      // Else spawn nothing because it's totally stationary and constantly
      // smoking would be weird -SH
  ...
  if (actor->tracer) {
    ...
    if ((!actor->tracer->health) // || (actor->tracer->player &&
                                 // (actor->tracer->player->powers[pw_shield] &
                                 // SH_NOSTACK) == SH_ATTRACT
        || P_AproxDistance(P_AproxDistance(actor->tracer->x - actor->x,
                                           actor->tracer->y - actor->y),
                           actor->tracer->z - actor->z) >
               FixedMul(RING_DIST, actor->tracer->scale)) // SRB2kart
    {
      P_SetTarget(&actor->tracer, NULL);
    }

    if (actor->tracer &&
        (actor->tracer->health)) // SRB2kart - jawz always follow
                                 //&& (actor->tracer->player->powers[pw_shield]
                                 //& SH_NOSTACK) == SH_ATTRACT)// Already found
                                 // someone to follow.
    {
  ...
  for (;; actor->lastlook = (actor->lastlook + 1) & PLAYERSMASK) {
    ...
    if (actor->target && actor->target->player) {
      ...
      if (actor->target->player->kartstuff[k_position] <
          player->kartstuff[k_position]) // SRB2kart - Jawz only go after people
                                         // ahead of you
        continue;
    }
    ...
    if (dist <
        FixedMul(2048 * FRACUNIT,
                 player->mo->scale)) // SRB2kart  // (player->powers[pw_shield]
                                     // & SH_NOSTACK) == SH_ATTRACT &&
      P_SetTarget(&actor->tracer, player->mo);
    return;
  }
...
static inline boolean PIT_GrenadeRing(mobj_t *thing) {
  ...
  if (thing->type !=
      MT_PLAYER) // Don't explode for anything but an actual player.
    return true;
  ...
  if (thing == grenade->target &&
      grenade->threshold != 0) // Don't blow up at your owner.
    return true;

  if (thing->player && (thing->player->kartstuff[k_hyudorotimer] ||
                        (G_BattleGametype() && thing->player &&
                         thing->player->kartstuff[k_bumper] <= 0 &&
  ...
  if ((gametype == GT_CTF || gametype == GT_TEAMMATCH) &&
      ...
      grenade->target->player->ctfteam ==
          thing->player->ctfteam) // Don't blow up at your teammates, unless
                                  // friendlyfire is on
    return true;
  ...
  if (P_AproxDistance(
          P_AproxDistance(thing->x - grenade->x, thing->y - grenade->y),
...
static inline boolean PIT_MineExplode(mobj_t *thing) {
  if (!grenade || P_MobjWasRemoved(grenade))
    return false; // There's the possibility these can chain react onto
                  // themselves after they've already died if there are enough
                  // all in one spot
  ...
  if (thing == grenade ||
      thing->type ==
          MT_MINEEXPLOSIONSOUND) // Don't explode yourself! Endless loop!
    return true;
  ...
  if (G_BattleGametype() && grenade->target && grenade->target->player &&
      grenade->target->player->kartstuff[k_bumper] <= 0 &&
      thing == grenade->target)
    return true;
  ...
  if (thing->player) // Looks like we're going to have to need a seperate
...
void A_MineExplode(mobj_t *actor) {
  ...
  for (d = 0; d < 16; d++)
    K_SpawnKartExplosion(actor->x, actor->y, actor->z,
                         explodedist + 32 * mapobjectscale, 32, type,
                         d * (ANGLE_45 / 4), true, false,
                         actor->target); // 32 <-> 64
  ...
  actor->flags2 |= MF2_DEBRIS; // Set this flag to ensure that the explosion
                               // won't be effective more than 1 frame.
}
//}
...
//
// Description: Changes the state of a level end sign to reflect the player that
// hit it.
//
// var1 = unused
// var2 = unused
...
void A_SignPlayer(mobj_t *actor) {
  ...
  // Set the sign to be an appropriate background color for this player's
  // skincolor.
...
void A_OverlayThink(mobj_t *actor) {
  ...
  if (!splitscreen && rendermode != render_soft) {
    ...
    if (players[displayplayers[0]].awayviewtics)
      viewingangle =
          R_PointToAngle2(actor->target->x, actor->target->y,
      ...
      viewingangle = R_PointToAngle2(actor->target->x, actor->target->y,
    ...
    destx =
        actor->target->x + P_ReturnThrustX(actor->target, viewingangle,
                                           FixedMul(FRACUNIT, actor->scale));
    desty =
        actor->target->y + P_ReturnThrustY(actor->target, viewingangle,
                                           FixedMul(FRACUNIT, actor->scale));
  } else {
  ...
  P_SetThingPosition(actor);
  if (actor->eflags & MFE_VERTICALFLIP)
    actor->z = actor->target->z + actor->target->height -
               mobjinfo[actor->type].height -
               ((var2 >> 16) ? -1 : 1) * (var2 & 0xFFFF) * FRACUNIT;
  else
    actor->z =
        actor->target->z + ((var2 >> 16) ? -1 : 1) * (var2 & 0xFFFF) * FRACUNIT;
  ...
  actor->momz =
      actor->target
          ->momz; // assume target has correct momz! Do not use P_SetObjectMomZ!
}
...
void A_JetChase(mobj_t *actor) {
  ...
  if (actor->momz &&
      ((actor->z - FixedMul((32 << FRACBITS), actor->scale)) < thefloor) &&
      !((thefloor + FixedMul(32 * FRACUNIT, actor->scale) + actor->height) >
        actor->ceilingz))
    actor->momz = -actor->momz / 2;
  ...
  if ((multiplayer || netgame) && !actor->threshold &&
      (actor->target->health <= 0 || !P_CheckSight(actor, actor->target)))
    if (P_LookForPlayers(actor, true, false, 0))
      return; // got a new target

  // If the player is over 3072 fracunits away, then look for another player
  if (P_AproxDistance(P_AproxDistance(actor->target->x - actor->x,
                                      actor->target->y - actor->y),
                      actor->target->z - actor->z) >
          FixedMul(3072 * FRACUNIT, actor->scale) &&
  ...
  // chase towards player
  if (ultimatemode)
    P_Thrust(actor, actor->angle,
             FixedMul(actor->info->speed / 2, actor->scale));
  else
    P_Thrust(actor, actor->angle,
             FixedMul(actor->info->speed / 4, actor->scale));

  // must adjust height
  if (ultimatemode) {
    if (actor->z < (actor->target->z + actor->target->height +
                    FixedMul((64 << FRACBITS), actor->scale)))
      actor->momz += FixedMul(FRACUNIT / 2, actor->scale);
    else
      actor->momz -= FixedMul(FRACUNIT / 2, actor->scale);
  } else {
    if (actor->z < (actor->target->z + actor->target->height +
                    FixedMul((32 << FRACBITS), actor->scale)))
      actor->momz += FixedMul(FRACUNIT / 2, actor->scale);
    else
      actor->momz -= FixedMul(FRACUNIT / 2, actor->scale);
  }
...
void A_JetbThink(mobj_t *actor) {
  ...
  if (actor->target) {
    ...
    // check for melee attack
    if ((actor->z >
         (actor->floorz + FixedMul((32 << FRACBITS), actor->scale))) &&
      ...
      // use raisestate instead of MT_MINE
      bomb = P_SpawnMobj(actor->x, actor->y,
                         actor->z - FixedMul((32 << FRACBITS), actor->scale),
                         (mobjtype_t)actor->info->raisestate);
  ...
  } else if (((actor->z - FixedMul((32 << FRACBITS), actor->scale)) <
              thefloor) &&
             !((thefloor + FixedMul((32 << FRACBITS), actor->scale) +
  ...
  nextsector =
      R_PointInSubsector(actor->x + actor->momx, actor->y + actor->momy)
          ->sector;
...
//
// Description: Shoots a bullet. Raisestate defines object # to use as
// projectile.
//
// var1 = unused
// var2 = unused
...
void A_MinusDigging(mobj_t *actor) {
  ...
  if (P_AproxDistance(actor->target->x - actor->x,
                      actor->target->y - actor->y) < actor->radius &&
...
void A_MinusCheck(mobj_t *actor) {
  ...
  if (P_MobjFlip(actor) * actor->momz < 0 &&
      actor->state < &states[actor->info->meleestate])
    P_SetMobjState(actor, actor->info->meleestate);
}
...
void A_JetgThink(mobj_t *actor) {
  ...
  if (actor->target) {
    if (P_RandomChance(FRACUNIT / 8) && !actor->reactiontime)
      ...
      A_JetChase(actor);
  } else if (actor->z - FixedMul((32 << FRACBITS), actor->scale) < thefloor &&
             !(thefloor + FixedMul((32 << FRACBITS), actor->scale) +
                   actor->height >
               actor->ceilingz)) {
    actor->z = thefloor + FixedMul((32 << FRACBITS), actor->scale);
...
void A_DetonChase(mobj_t *actor) {
  ...
  if (multiplayer && !actor->threshold &&
      P_LookForPlayers(actor, true, true, 0))
    return; // got a new target
  ...
  // movedir is up/down angle: how much it has to go up as it goes over to the
  // player
  xydist =
      P_AproxDistance(actor->tracer->x - actor->x, actor->tracer->y - actor->y);
  ...
  // check for melee attack
  if (actor->tracer) {
    if (P_AproxDistance(actor->tracer->x - actor->x,
                        actor->tracer->y - actor->y) <
  ...
  if (actor->reactiontime == -42) {
    ...
    xyspeed = FixedMul(FixedMul(K_GetKartSpeed(actor->tracer->player, false),
                                3 * FRACUNIT / 4),
                       FINECOSINE(exact));
    actor->momz =
        FixedMul(FixedMul(K_GetKartSpeed(actor->tracer->player, false),
                          3 * FRACUNIT / 4),
                 FINESINE(exact));
    ...
    xyspeed = (P_AproxDistance(actor->tracer->x - actor->x,
                               P_AproxDistance(actor->tracer->y - actor->y,
                                               actor->tracer->z - actor->z)) >>
               (FRACBITS + 6));
...
void A_CapeChase(mobj_t *actor) {
  ...
  CONS_Debug(DBG_GAMELOGIC,
             "A_CapeChase called from object type %d, var1: %d, var2: %d\n",
  ...
  if (!chaser || (chaser->health <= 0)) {
    if (chaser)
      CONS_Debug(DBG_GAMELOGIC,
                 "Hmm, the guy I'm chasing (object type %d) has no health.. so "
                 "I'll die too!\n",
                 chaser->type);
  ...
  foffsetx =
      P_ReturnThrustX(chaser, chaser->angle,
                      FixedMul((locvar2 >> 16) * FRACUNIT, actor->scale));
  foffsety =
      P_ReturnThrustY(chaser, chaser->angle,
                      FixedMul((locvar2 >> 16) * FRACUNIT, actor->scale));

  boffsetx =
      P_ReturnThrustX(chaser, chaser->angle - ANGLE_90,
                      FixedMul((locvar2 & 65535) * FRACUNIT, actor->scale));
  boffsety =
      P_ReturnThrustY(chaser, chaser->angle - ANGLE_90,
                      FixedMul((locvar2 & 65535) * FRACUNIT, actor->scale));
  ...
  if (chaser->eflags & MFE_VERTICALFLIP) {
    ...
    actor->z = chaser->z + chaser->height - actor->height -
               FixedMul((locvar1 >> 16) * FRACUNIT, actor->scale);
  } else {
    actor->eflags &= ~MFE_VERTICALFLIP;
    actor->flags2 &= ~MF2_OBJECTFLIP;
...
void A_RotateSpikeBall(mobj_t *actor) {
  ...
  if (actor->type ==
      MT_SPECIALSPIKEBALL) // don't remove this, these spikeballs share the same
                           // states as the rotating spikeballs
    return;

  if (!((!locvar1 && (actor->target)) ||
        (locvar1 && (actor->tracer)))) // This should NEVER happen.
  {
...
void A_UnidusBall(mobj_t *actor) {
  ...
  if (actor->movecount) {
    if (P_AproxDistance(actor->momx, actor->momy) <
        FixedMul(actor->info->damage / 2, actor->scale))
      P_ExplodeMissile(actor);
    return;
  }
  ...
  if (!actor->target || !actor->target->health) {
    CONS_Debug(
        DBG_GAMELOGIC,
        "A_UnidusBall: Removing unthrown spikeball from nonexistant Unidus\n");
    P_RemoveMobj(actor);
    return;
  }
  ...
  {
    const angle_t angle =
        actor->movedir + FixedAngle(actor->info->speed * (leveltime % 360));
    ...
    if (locvar1 == 1 && actor->target->target) {
      const angle_t tang =
          R_PointToAngle2(actor->target->x, actor->target->y,
                          actor->target->target->x, actor->target->target->y);
  ...
  if (locvar1 == 1 && canthrow) {
    if (P_AproxDistance(actor->target->target->x - actor->target->x,
                        actor->target->target->y - actor->target->y) >
            FixedMul(MISSILERANGE >> 1, actor->scale) ||
    ...
    P_InstaThrust(actor,
                  R_PointToAngle2(actor->x, actor->y, actor->target->target->x,
...
void A_RockSpawn(mobj_t *actor) {
  ...
  if (i == -1) {
    CONS_Debug(DBG_GAMELOGIC,
               "A_RockSpawn: Unable to find parameter line 12 (tag %d)!\n",
               actor->threshold);
    return;
  }
  ...
  mo->angle =
      R_PointToAngle2(line->v2->x, line->v2->y, line->v1->x, line->v1->y);
...
void A_MaceRotate(mobj_t *actor) {
  ...
  radius =
      FixedMul(actor->info->speed * actor->reactiontime, actor->target->scale);

  // Double the radius if the chain links are made up of maces.
  if (actor->target->type == MT_AXIS &&
      (actor->type == MT_SMALLMACE || actor->type == MT_BIGMACE))
    radius *= 2;
  ...
  // Swinging Chain.
  if (actor->target->type == MT_HANGMACEPOINT ||
      actor->target->type == MT_SWINGMACEPOINT) {
    actor->movecount += actor->target->lastlook;
    actor->movecount &= FINEMASK;

    actor->threshold = FixedMul(FINECOSINE(actor->movecount),
                                actor->target->lastlook << FRACBITS);
    ...
    res = VectorMatrixMultiply(
        v, *RotateZMatrix(actor->target->health << ANGLETOFINESHIFT));
    M_Memcpy(&v, res, sizeof(v));
  }
  // Rotating Chain.
  else {
    ...
    // Calculate the angle matrixes for the link.
    res = VectorMatrixMultiply(
        v, *RotateXMatrix(actor->target->threshold << ANGLETOFINESHIFT));
    M_Memcpy(&v, res, sizeof(v));
    res = VectorMatrixMultiply(
        v, *RotateZMatrix(actor->target->health << ANGLETOFINESHIFT));
    M_Memcpy(&v, res, sizeof(v));
  ...
  if (!(actor->target->flags2 &
        MF2_BOSSNOTRAP) // flag that makes maces shut up on request
      && !(leveltime & 63) &&
      (actor->type == MT_BIGMACE || actor->type == MT_SMALLMACE) &&
      actor->target->type == MT_MACEPOINT)
    S_StartSound(actor, actor->info->activesound);
}

// Function: A_SetFuse
//
// Description: Sets the actor's fuse timer if not set already. May also change
// state when fuse reaches the last tic, otherwise by default the actor will die
// or disappear. (Replaces A_SnowBall)
//
// var1 = fuse timer duration (in tics).
// var2:
//		lower 16 bits = if > 0, state to change to when fuse = 1
//		upper 16 bits: 0 = (default) don't set fuse unless 0, 1 = force
// change, 2 = force no change
//
void A_SetFuse(mobj_t *actor) {
  INT32 locvar1 = var1;
  INT32 locvar2 = var2;
  ...
  if ((!actor->fuse || (locvar2 >> 16)) &&
      (locvar2 >> 16) != 2) // set the actor's fuse value
    actor->fuse = locvar1;

  if (actor->fuse == 1 &&
      (locvar2 & 65535)) // change state on the very last tic (fuse is handled
                         // before actions in P_MobjThinker)
  {
...
void A_CrawlaCommanderThink(mobj_t *actor) {
  ...
  if (actor->target->player && actor->health > 1) {
    if (dist < FixedMul(128 * FRACUNIT, actor->scale) &&
        ((actor->target->player->pflags & PF_JUMPED) ||
         (actor->target->player->pflags & PF_SPINNING))) {
      // Auugh! He's trying to kill you! Strafe! STRAAAAFFEEE!!
      if (actor->target->momx || actor->target->momy)
        P_InstaThrust(actor, actor->angle - ANGLE_180,
                      FixedMul(20 * FRACUNIT, actor->scale));
      return;
    }
  }
  ...
  if (actor->reactiontime && actor->reactiontime <= 2 * TICRATE &&
      dist > actor->target->radius - FixedMul(FRACUNIT, actor->scale)) {
    actor->threshold = 0;
    ...
    if (actor->health > 1) // Hover Mode
    {
      if (dist < FixedMul(512 * FRACUNIT, actor->scale)) {
        actor->angle = R_PointToAngle2(actor->x, actor->y, actor->target->x,
                                       actor->target->y);
        P_InstaThrust(actor, actor->angle,
                      FixedMul(60 * FRACUNIT, actor->scale));
        actor->threshold = 1;
      }
    }
  ...
  if (!actor->fuse && actor->health == 1 && actor->z <= actor->floorz) {
    if (dist < FixedMul(256 * FRACUNIT, actor->scale)) {
      ...
      actor->angle = R_PointToAngle2(actor->x, actor->y, actor->target->x,
                                     actor->target->y) +
                     (P_RandomChance(FRACUNIT / 2) ? -prandom : +prandom);
      P_InstaThrust(
          actor, actor->angle,
          FixedDiv(FixedMul(locvar2, actor->scale), 3 * FRACUNIT / 2));
  ...
  if (nextsector->floorheight > actor->z &&
      nextsector->floorheight - actor->z <
          FixedMul(128 * FRACUNIT, actor->scale))
...
void A_RingExplode(mobj_t *actor) {
  ...
  for (d = 0; d < 16; d++)
    P_SpawnParaloop(actor->x, actor->y, actor->z + actor->height,
                    ...
                    MT_NIGHTSPARKLE, S_NULL, d * (ANGLE_22h), true);
  ...
  for (th = thinkercap.next; th != &thinkercap; th = th->next) {
    ...
    if (P_AproxDistance(P_AproxDistance(mo2->x - actor->x, mo2->y - actor->y),
                        mo2->z - actor->z) >
...
void A_OldRingExplode(mobj_t *actor) {
  ...
  for (i = 0; i < 32; i++) {
    ...
    P_SetTarget(&mo->target,
                actor->target); // Transfer target so player gets the points
...
void A_MixUp(mobj_t *actor) {
  ...
  // The random factor is okay for other game modes, but in these, it is
  // cripplingly unfair.
  ...
  for (i = 0; i < MAXPLAYERS; i++)
    if (playeringame[i] && players[i].mo && players[i].mo->health > 0 &&
        players[i].playerstate == PST_LIVE && !players[i].exiting &&
        !players[i].powers[pw_super]) {
  ...
  else if (numplayers == 2) // Special case -- simple swap
  {
    ...
    for (i = 0; i < MAXPLAYERS; i++)
      if (playeringame[i] && players[i].mo && players[i].mo->health > 0 &&
        ...
        if ((netgame || multiplayer) &&
            players[i].spectator) // Ignore spectators
          continue;
    ...
    P_MixUp(players[one].mo, players[two].mo->x, players[two].mo->y,
            players[two].mo->z, players[two].mo->angle, players[two].starpostx,
            players[two].starposty, players[two].starpostz,
            players[two].starpostnum, players[two].starposttime,
            players[two].starpostangle, players[two].mo->flags2);

    P_MixUp(players[two].mo, x, y, z, angle, starpostx, starposty, starpostz,
            starpostnum, starposttime, starpostangle, mflags2);

    // flags set after mixup.  Stupid P_ResetPlayer() takes away some of the
    // flags we look for... but not all of them!  So we need to make sure they
    // aren't set wrong or anything.
    ...
    for (i = 0; i < MAXPLAYERS; i++) {
      position[i][0] = position[i][1] = position[i][2] = anglepos[i] =
          pindex[i] = -1;
      teleported[i] = false;
    }
    ...
    for (i = 0; i < MAXPLAYERS; i++) {
      if (playeringame[i] && players[i].playerstate == PST_LIVE &&
        ...
        P_MixUp(players[i].mo, position[teleportfrom][0],
                position[teleportfrom][1], position[teleportfrom][2],
                anglepos[teleportfrom], spposition[teleportfrom][0],
                spposition[teleportfrom][1], spposition[teleportfrom][2],
                starpostnum[teleportfrom], starposttime[teleportfrom],
                starpostangle[teleportfrom], flags2[teleportfrom]);
  ...
  for (i = 0; i < MAXPLAYERS; i++) {
    if (teleported[i]) {
      if (playeringame[i] && players[i].playerstate == PST_LIVE &&
        ...
        players[i].mo->floorz =
            P_GetFloorZ(players[i].mo, players[i].mo->subsector->sector,
                        players[i].mo->x, players[i].mo->y, NULL);
        players[i].mo->ceilingz =
            P_GetCeilingZ(players[i].mo, players[i].mo->subsector->sector,
                          players[i].mo->x, players[i].mo->y, NULL);
...
void A_RecyclePowers(mobj_t *actor) {
  ...
  for (i = 0, j = 0; i < MAXPLAYERS; i++) {
    if (playeringame[i] && players[i].mo && players[i].mo->health > 0 &&
      ...
      if (actor && actor->target && actor->target->player &&
          &players[i] == actor->target->player)
        beneficiary = (UINT8)i;
#endif
  ...
  for (i = 0; i < numplayers; i++) {
    ...
    for (j = 0; j < NUMPOWERS; j++) {
      if (j == pw_flashing || j == pw_underwater || j == pw_spacetime ||
          j == pw_tailsfly || j == pw_extralife || j == pw_nocontrol ||
          j == pw_super)
        continue;
...
nomissile:
  ...
  if (actor->flags & MF_FLOAT &&
      !(actor->flags2 &
        MF2_SKULLFLY)) { // Float up/down to your target's position. Stay above
                         // them, but not out of jump range.
    fixed_t target_min =
        actor->target->floorz + FixedMul(64 * FRACUNIT, actor->scale);
    ...
    if (actor->z > target_min + FixedMul(16 * FRACUNIT, actor->scale))
      actor->momz =
          FixedMul((-actor->info->speed << (FRACBITS - 1)), actor->scale);
...
void A_Boss2Chase(mobj_t *actor) {
  ...
  if (actor->extravalue1 > 0) {
    ...
    // Set base angle
    {
      const angle_t fa = (actor->target->angle + FixedAngle(actor->watertop)) >>
                         ANGLETOFINESHIFT;
      const fixed_t fc = FixedMul(FINECOSINE(fa), radius);
      const fixed_t fs = FixedMul(FINESINE(fa), radius);
      actor->angle = R_PointToAngle2(actor->x, actor->y, actor->target->x + fc,
                                     actor->target->y + fs);
    }

    // Now turn you around!
    // Note that the start position is the final position, we move it back
    // around to intermediary positions...
    actor->angle -= FixedAngle(
        FixedMul(FixedDiv(180 << FRACBITS, actor->extravalue2 << FRACBITS),
                 actor->extravalue1 << FRACBITS));
  } else {
    ...
    actor->target->angle += // Don't use FixedAngleC!
        FixedAngle(
            FixedDiv(FixedMul(actor->watertop,
    ...
    if (leveltime % (speedvar * 15 / 10) - 1 == 0) {
      ...
      mobj_t *goop;
      fixed_t fz =
          actor->z + actor->height + FixedMul(24 * FRACUNIT, actor->scale);
      angle_t fa;
...
void A_Boss2Pogo(mobj_t *actor) {
  if (LUA_CallAction("A_Boss2Pogo", actor))
    return;
  if (actor->z <= actor->floorz + FixedMul(8 * FRACUNIT, actor->scale) &&
      actor->momz <= 0) {
...
//
// Description: Special function for Boss 2 so you can't just sit and destroy
// him.
//
...
void A_Boss7Chase(mobj_t *actor) {
  ...
  if (actor->tracer) {
    if (P_AproxDistance(actor->x - actor->tracer->x,
                        actor->y - actor->tracer->y) >
        128 * FRACUNIT - actor->radius)
      P_InstaThrust(actor,
                    R_PointToAngle2(actor->x, actor->y, actor->tracer->x,
                                    actor->tracer->y),
                    FRACUNIT);
  }
  ...
  for (i = 0; i < MAXPLAYERS; i++) {
    ...
    if (P_AproxDistance(players[i].mo->x - actor->x,
                        players[i].mo->y - actor->y) > actor->radius)
      continue;
  ...
  if (actor->health <= actor->info->damage && actor->target &&
      actor->target->player && (actor->target->player->pflags & PF_ITEMHANG)) {
    A_FaceTarget(actor);
...
void A_Boss2PogoSFX(mobj_t *actor) {
  ...
  if (P_AproxDistance(actor->x - actor->target->x,
                      actor->y - actor->target->y) <
      FixedMul(256 * FRACUNIT, actor->scale)) {
    actor->angle =
        R_PointToAngle2(actor->x, actor->y, actor->target->x, actor->target->y);
    P_InstaThrust(actor, actor->angle,
                  FixedMul(actor->info->speed, actor->scale));
...
//
// Description: Pogoing for Boss 2, tries to actually land on the player
// directly.
//
// var1 = pogo jump strength
// var2 = idle pogo speed
...
void A_Boss2PogoTarget(mobj_t *actor) {
  ...
  if (!actor->target || !(actor->target->flags & MF_SHOOTABLE) ||
      (actor->target->player && actor->target->player->powers[pw_flashing]) ||
      P_AproxDistance(actor->x - actor->target->x,
                      actor->y - actor->target->y) >=
          FixedMul(512 * FRACUNIT, actor->scale)) {
    // look for a new target
  ...
  // Target hit, retreat!
  if (actor->target->player->powers[pw_flashing] > TICRATE ||
      actor->flags2 & MF2_FRET) {
    UINT8 prandom = P_RandomByte();
    ...
    actor->angle = R_PointToAngle2(actor->x, actor->y, actor->target->x,
                   ...
                   (P_RandomChance(FRACUNIT / 2)
                        ? -prandom
                        : +prandom); // Pick a direction, and randomize it.
    P_InstaThrust(actor, actor->angle + ANGLE_180,
                  FixedMul(FixedMul(actor->info->speed, (locvar2)),
                           actor->scale)); // Move at wandering speed
  }
  // Try to land on top of the player.
  else if (P_AproxDistance(actor->x - actor->target->x,
                           actor->y - actor->target->y) <
           FixedMul(512 * FRACUNIT, actor->scale)) {
    ...
    actor->z++; // unstick from the floor
    actor->momz =
        FixedMul(locvar1 + (locvar1 >> 2), actor->scale); // Bounce up in air
    ...
    */
    ...
    // airtime = FixedDiv(-actor->momz<<1, gravityadd)<<1; // going from 0 to 0
    // is much simpler
    zoffs = (P_GetPlayerHeight(actor->target->player) >> 1) +
            (actor->target->floorz -
             actor->floorz); // offset by the difference in floor height plus
                             // half the player height,
    airtime = FixedDiv((-actor->momz -
                        FixedSqrt(FixedMul(actor->momz, actor->momz) + zoffs)),
                       gravityadd)
              << 1; // to try and land on their head rather than on their feet

    actor->angle =
        R_PointToAngle2(actor->x, actor->y, actor->target->x, actor->target->y);
    P_InstaThrust(actor, actor->angle,
                  FixedDiv(P_AproxDistance(actor->x - actor->target->x,
                                           actor->y - actor->target->y),
                           airtime));
  }
  ...
  if (actor->info->missilestate) // spawn the pogo stick collision box
  {
    mobj_t *pogo =
        P_SpawnMobj(actor->x, actor->y,
                    actor->z - mobjinfo[actor->info->missilestate].height,
                    (mobjtype_t)actor->info->missilestate);
...
void A_SparkFollow(mobj_t *actor) {
  ...
  P_UnsetThingPosition(actor);
  {
    ...
    actor->x =
        actor->target->x +
        FixedMul(FINECOSINE(fa), FixedMul(actor->info->speed, actor->scale));
    actor->y =
        actor->target->y +
        FixedMul(FINESINE(fa), FixedMul(actor->info->speed, actor->scale));
    if (actor->target->eflags & MFE_VERTICALFLIP)
      actor->z = actor->target->z + actor->target->height -
                 FixedDiv(actor->target->height, 3 * FRACUNIT);
    else
      actor->z = actor->target->z +
                 FixedDiv(actor->target->height, 3 * FRACUNIT) - actor->height;
  }
  P_SetThingPosition(actor);
}
...
// Function: A_BuzzFly
//
// Description: Makes an object slowly fly after a player, in the manner of a
// Buzz.
//
// var1 = sfx to play
...
void A_BuzzFly(mobj_t *actor) {
  ...
  if (actor->target->health <= 0 ||
      (!actor->threshold && !P_CheckSight(actor, actor->target))) {
  ...
  if (P_AproxDistance(P_AproxDistance(actor->target->x - actor->x,
    ...
    if (multiplayer || netgame)
      P_LookForPlayers(
          actor, true, false,
          FixedMul(3072 * FRACUNIT, actor->scale)); // maybe get a new target
  ...
  // chase towards player
  {
    ...
    actor->momx =
        FixedMul(FixedDiv(actor->target->x - actor->x, dist), realspeed);
    actor->momy =
        FixedMul(FixedDiv(actor->target->y - actor->y, dist), realspeed);
    actor->momz =
        FixedMul(FixedDiv(actor->target->z - actor->z, dist), realspeed);

    if (actor->z + actor->momz >= actor->waterbottom &&
        actor->watertop > actor->floorz &&
        actor->z + actor->momz >
            actor->watertop - FixedMul(256 * FRACUNIT, actor->scale) &&
...
void A_GuardChase(mobj_t *actor) {
  ...
  if (--actor->movecount < 0 ||
      !P_Move(actor, (actor->flags2 & MF2_AMBUSH) ? actor->info->speed * 2
                                                  : actor->info->speed)) {
    P_NewChaseDir(actor);
...
void A_EggShield(mobj_t *actor) {
  ...
  newx = actor->target->x + P_ReturnThrustX(actor, actor->target->angle,
                                            FixedMul(FRACUNIT, actor->scale));
  newy = actor->target->y + P_ReturnThrustY(actor, actor->target->angle,
                                            FixedMul(FRACUNIT, actor->scale));
  ...
  for (i = 0; i < MAXPLAYERS; i++) {
    ...
    angle = R_PointToAngle2(actor->x, actor->y, player->mo->x, player->mo->y) -
            actor->angle;
...
void A_Boss1Spikeballs(mobj_t *actor) {
  ...
  ball->movedir = FixedAngle(FixedMul(
      FixedDiv(locvar1 << FRACBITS, locvar2 << FRACBITS), 360 << FRACBITS));
  ...
  } else if (actor->threshold >= 0) // Traveling mode
  {
    ...
    for (th = thinkercap.next; th != &thinkercap; th = th->next) {
      ...
      if (mo2->type == MT_BOSS3WAYPOINT && mo2->spawnpoint &&
          mo2->spawnpoint->angle == actor->threshold) {
    ...
    if (!actor->target) // Should NEVER happen
    {
      CONS_Debug(
          DBG_GAMELOGIC,
          "Error: Boss 3 Dummy was unable to find specified waypoint: %d\n",
          actor->threshold);
      return;
    }
    ...
    dist2 = P_AproxDistance(
        P_AproxDistance(actor->target->x - (actor->x + actor->momx),
                        actor->target->y - (actor->y + actor->momy)),
        actor->target->z - (actor->z + actor->momz));
    ...
    if ((dist >> FRACBITS) <= (dist2 >> FRACBITS)) {
      ...
      if (actor->target->spawnpoint) {
        ...
        CONS_Debug(DBG_GAMELOGIC,
                   "Error: Boss 3 Dummy waypoint has no spawnpoint associated "
                   "with it.\n");
    }
  }
}
...
//
// Description: Object's location is used to set the calling sector. The tag
// used is var1. Optionally, if var2 is set, the actor's angle (multiplied by
// var2) is added to the tag number as well.
//
// var1 = tag
...
void A_LinedefExecute(mobj_t *actor) {
  ...
  tagnum = locvar1;
  // state numbers option is no more, custom states cannot be guaranteed to stay
  // the same number anymore, now that they can be defined by names instead

  if (locvar2)
    tagnum += locvar2 * (AngleFixed(actor->angle) >> FRACBITS);

  CONS_Debug(DBG_GAMELOGIC,
             "A_LinedefExecute: Running mobjtype %d's sector with tag %d\n",
             actor->type, tagnum);

  // tag 32768 displayed in map editors is actually tag -32768, tag 32769 is
  // -32767, 65535 is -1 etc.
...
void A_SmokeTrailer(mobj_t *actor) {
  ...
  if (actor->eflags & MFE_VERTICALFLIP) {
    th = P_SpawnMobj(actor->x - actor->momx, actor->y - actor->momy,
                     actor->z + actor->height -
                         FixedMul(mobjinfo[locvar1].height, actor->scale),
                     locvar1);
...
void A_SpawnObjectAbsolute(mobj_t *actor) {
  ...
  // Spawn objects with an angle matching the spawner's, rather than spawning
  // Eastwards - Monster Iestyn
...
void A_SpawnObjectRelative(mobj_t *actor) {
  ...
  CONS_Debug(
      DBG_GAMELOGIC,
      "A_SpawnObjectRelative called from object type %d, var1: %d, var2: %d\n",
      actor->type, locvar1, locvar2);
  ...
  // NOTE: Doing actor->z + actor->height is the bottom of the object while the
  // object has reverse gravity. - Flame
  mo = P_SpawnMobj(actor->x + FixedMul(x << FRACBITS, actor->scale),
                   actor->y + FixedMul(y << FRACBITS, actor->scale),
                   (actor->eflags & MFE_VERTICALFLIP)
                       ? ((actor->z + actor->height - mobjinfo[type].height) -
                          FixedMul(z << FRACBITS, actor->scale))
                       : (actor->z + FixedMul(z << FRACBITS, actor->scale)),
                   type);
...
//
// Description: Changes the angle to a random relative value between the min and
// max. Set min and max to the same value to eliminate randomness
//
// var1 = min
// var2 = max
//
void A_ChangeAngleRelative(mobj_t *actor) {
  // Oh god, the old code /sucked/. Changed this and the absolute version to get
  // a random range using amin and amax instead of
  //  getting a random angle from the _entire_ spectrum and then clipping. While
  //  we're at it, do the angle conversion to the result rather than the ranges,
  //  so <0 and >360 work as possible values. -Red
  INT32 locvar1 = var1;
  INT32 locvar2 = var2;
...
//
// Description: Changes the angle to a random absolute value between the min and
// max. Set min and max to the same value to eliminate randomness
//
// var1 = min
// var2 = max
...
//
// Description: Finds the nearest/furthest mobj of the specified type and sets
// actor->target to it.
//
// var1 = mobj type
...
void A_FindTarget(mobj_t *actor) {
  ...
  CONS_Debug(DBG_GAMELOGIC,
             "A_FindTarget called from object type %d, var1: %d, var2: %d\n",
             actor->type, locvar1, locvar2);
  ...
  for (th = thinkercap.next; th != &thinkercap; th = th->next) {
    ...
    if (mo2->type == (mobjtype_t)locvar1) {
      if (mo2->player &&
          (mo2->player->spectator || mo2->player->pflags & PF_INVIS))
  ...
  if (!targetedmobj) {
    CONS_Debug(
        DBG_GAMELOGIC,
        "A_FindTarget: Unable to find the specified object to target.\n");
...
void A_FindTracer(mobj_t *actor) {
  ...
  CONS_Debug(DBG_GAMELOGIC,
             "A_FindTracer called from object type %d, var1: %d, var2: %d\n",
             actor->type, locvar1, locvar2);
  ...
  if (!targetedmobj) {
    CONS_Debug(
        DBG_GAMELOGIC,
        "A_FindTracer: Unable to find the specified object to target.\n");
    return; // Oops, nothing found..
  }
...
//
// var1 = tics to set to
// var2 = if this is set, and no var1 is supplied, the mobj's threshold value
// will be used.
//
void A_SetTics(mobj_t *actor) {
  INT32 locvar1 = var1;
...
//
// Description: Changes the color of an object by an absolute value. Note: 0 is
// default colormap.
//
...
void A_MoveRelative(mobj_t *actor) {
  ...
  P_Thrust(actor, actor->angle + FixedAngle(locvar1 * FRACUNIT),
           FixedMul(locvar2 * FRACUNIT, actor->scale));
}
...
void A_MoveAbsolute(mobj_t *actor) {
  ...
  P_InstaThrust(actor, FixedAngle(locvar1 * FRACUNIT),
                FixedMul(locvar2 * FRACUNIT, actor->scale));
}
...
void A_Thrust(mobj_t *actor) {
  ...
  if (locvar2)
    P_InstaThrust(actor, actor->angle,
                  FixedMul(locvar1 * FRACUNIT, actor->scale));
  else
...
// var1 = amount of force
// var2:
//		lower 16 bits = If 1, xy momentum is lost. If 0, xy momentum is
// kept 		upper 16 bits = If 1, z momentum is lost. If 0, z momentum
// is kept
//
void A_ZThrust(mobj_t *actor) {
  INT32 locvar1 = var1;
...
//
// Description: Sets your target to the object who your target is targeting.
// Yikes! If it happens to be NULL, you're just out of luck.
//
// var1: (Your target)
//		0 = target
//		1 = tracer
...
void A_SetObjectFlags(mobj_t *actor) {
  ...
  if ((UINT32)(locvar1 & (MF_NOBLOCKMAP | MF_NOSECTOR)) !=
      (actor->flags &
       (MF_NOBLOCKMAP |
        MF_NOSECTOR))) // Blockmap/sector status has changed, so reset the links
    unlinkthings = true;
...
//
// Description: Spawns jet fumes/other attachment miscellany for the boss. To
// only be used when he is spawned.
//
// var1:
...
void A_BossJetFume(mobj_t *actor) {
  ...
  if (locvar1 == 0) // Boss1 jet fumes
  {
    ...
    jetx = actor->x + P_ReturnThrustX(actor, actor->angle,
                                      -FixedMul(64 * FRACUNIT, actor->scale));
    jety = actor->y + P_ReturnThrustY(actor, actor->angle,
                                      -FixedMul(64 * FRACUNIT, actor->scale));
    if (actor->eflags & MFE_VERTICALFLIP)
      jetz =
          actor->z + actor->height -
          FixedMul(38 * FRACUNIT + mobjinfo[MT_JETFUME1].height, actor->scale);
    else
    ...
    if (actor->eflags & MFE_VERTICALFLIP)
      jetz =
          actor->z + actor->height -
          FixedMul(12 * FRACUNIT + mobjinfo[MT_JETFUME1].height, actor->scale);
    else
      jetz = actor->z + FixedMul(12 * FRACUNIT, actor->scale);

    filler = P_SpawnMobj(
        jetx + P_ReturnThrustX(actor, actor->angle - ANGLE_90,
                               FixedMul(24 * FRACUNIT, actor->scale)),
        jety + P_ReturnThrustY(actor, actor->angle - ANGLE_90,
                               FixedMul(24 * FRACUNIT, actor->scale)),
        jetz, MT_JETFUME1);
    P_SetTarget(&filler->target, actor);
    filler->destscale = actor->scale;
  ...
  } else if (locvar1 == 1) // Boss 3 propeller
  {
    ...
    if (actor->eflags & MFE_VERTICALFLIP)
      jetz =
          actor->z + actor->height -
          FixedMul(17 * FRACUNIT + mobjinfo[MT_PROPELLER].height, actor->scale);
    else
  ...
  } else if (locvar1 == 3) // Boss 4 jet flame
  {
    ...
    if (actor->eflags & MFE_VERTICALFLIP)
      jetz =
          actor->z + actor->height +
          FixedMul(50 * FRACUNIT - mobjinfo[MT_JETFLAME].height, actor->scale);
    else
...
//
// Description: Calls two actions. Be careful, if you reference the same state
// this action is called from, you can create an infinite loop.
//
...
void A_DualAction(mobj_t *actor) {
  ...
  CONS_Debug(DBG_GAMELOGIC,
             "A_DualAction called from object type %d, var1: %d, var2: %d\n",
             actor->type, locvar1, locvar2);
  ...
  CONS_Debug(DBG_GAMELOGIC,
             "A_DualAction: Calling First Action (state %d)...\n", locvar1);
  ...
  CONS_Debug(DBG_GAMELOGIC,
             "A_DualAction: Calling Second Action (state %d)...\n", locvar2);
  states[locvar2].action.acp1(actor);
}

// Function: A_RemoteAction
//
// Description: var1 is the remote object. var2 is the state reference for
// calling the action called on var1. var1 becomes the actor's target, the
// action (var2) is called on var1. actor's target is restored
//
...
void A_RemoteAction(mobj_t *actor) {
  ...
  if (locvar1 >= 0) {
    ...
    if (!targetedmobj) {
      CONS_Debug(
          DBG_GAMELOGIC,
          "A_RemoteAction: Unable to find the specified object to target.\n");
      return; // Oops, nothing found..
    }
  ...
  if (actor->target) {
    ...
    CONS_Debug(DBG_GAMELOGIC,
               "A_RemoteAction: Calling action on %p\n"
               "var1 is %d\nvar2 is %d\n",
...
void A_JawzChase(mobj_t *actor) {
  ...
  if (actor->tracer) {
    ...
    if (actor->tracer->health) {
      ...
      ret = P_SpawnMobj(actor->tracer->x, actor->tracer->y, actor->tracer->z,
                        MT_PLAYERRETICULE);
...
void A_JawzExplode(mobj_t *actor) {
  ...
  while (shrapnel) {
    ...
    truc =
        P_SpawnMobj(actor->x + P_RandomRange(-8, 8) * FRACUNIT,
                    actor->y + P_RandomRange(-8, 8) * FRACUNIT,
...
void A_SPBChase(mobj_t *actor) {
  ...
  if (actor->extravalue1 == 1) // MODE: TARGETING
  {
    if (actor->tracer && actor->tracer->health &&
        !(actor->tracer->player && actor->tracer->player->spectator)) {

      fixed_t defspeed = wspeed;
      fixed_t range = (160 * actor->tracer->scale);
      fixed_t cx = 0, cy = 0;

      // we're tailing a player, now's a good time to regain our damage
      // properties
      actor->flags &= ~MF_NOCLIPTHING;
      ...
      if (actor->tracer->player) {
        UINT8 fracmax = 32;
        UINT8 spark = ((10 - actor->tracer->player->kartspeed) +
                       actor->tracer->player->kartweight) /
                      2;
        fixed_t easiness =
            ((actor->tracer->player->kartspeed + (10 - spark)) << FRACBITS) / 2;

        actor->lastlook = actor->tracer->player -
                          players; // Save the player num for death scumming...

        if (!P_IsObjectOnGround(actor->tracer) /*&& !actor->tracer->player->kartstuff[k_pogospring]*/)
				{
          // In the air you have no control; basically don't hit unless you make
          // a near complete stop
          defspeed = (7 * actor->tracer->player->speed) / 8;
        } else {
          // 7/8ths max speed for Knuckles, 3/4ths max speed for min accel,
          // exactly max speed for max accel
          defspeed = FixedMul(((fracmax + 1) << FRACBITS) - easiness,
                              K_GetKartSpeed(actor->tracer->player, false)) /
                     fracmax;
        }
      ...
      dist = P_AproxDistance(P_AproxDistance(actor->x - actor->tracer->x,
                                             actor->y - actor->tracer->y),
                             actor->z - actor->tracer->z);
      ...
      //  ^^^^ current section: These are annoying, and grand metropolis in
      //  particular needs this.

      hang = R_PointToAngle2(actor->x, actor->y, actor->tracer->x,
                             actor->tracer->y);
...

      {
        ...
        xyspeed = FixedMul(
            actor->cvmem,
            max(0, (((180 << FRACBITS) - AngleFixed(input)) / 90) - FRACUNIT));
        ...
        // Slow down when turning; might as well do it for momz, since we do it
        // above too
        zspeed = FixedMul(
            actor->cvmem,
            max(0, (((180 << FRACBITS) - AngleFixed(input)) / 90) - FRACUNIT));
      ...
      actor->momx =
          cx + FixedMul(FixedMul(xyspeed,
                                 FINECOSINE(actor->angle >> ANGLETOFINESHIFT)),
                        FINECOSINE(actor->movedir >> ANGLETOFINESHIFT));
      actor->momy =
          cy + FixedMul(FixedMul(xyspeed,
                                 FINESINE(actor->angle >> ANGLETOFINESHIFT)),
                        FINECOSINE(actor->movedir >> ANGLETOFINESHIFT));
      actor->momz =
          FixedMul(zspeed, FINESINE(actor->movedir >> ANGLETOFINESHIFT));

      // Red speed lines for when it's gaining on its target. A tell for when
      // you're starting to lose too much speed!
      if (R_PointToDist2(0, 0, actor->momx, actor->momy) >
              (actor->tracer->player
                   ? (16 * actor->tracer->player->speed) / 15
                   : (16 * R_PointToDist2(0, 0, actor->tracer->momx,
                                          actor->tracer->momy)) /
                         15) // Going faster than the target
          && xyspeed >
                 K_GetKartSpeed(actor->tracer->player, false) /
                     4) // Don't display speedup lines at pitifully low speeds
      {
        mobj_t *fast =
            P_SpawnMobj(actor->x + (P_RandomRange(-24, 24) * actor->scale),
                        actor->y + (P_RandomRange(-24, 24) * actor->scale),
                        actor->z + (actor->height / 2) +
                            (P_RandomRange(-24, 24) * actor->scale),
                        MT_FASTLINE);
  ...
  } else if (actor->extravalue1 == 2) // MODE: WAIT...
  {
    ...
    if (actor->lastlook != -1 && playeringame[actor->lastlook] &&
  ...
  } else // MODE: SEEKING
  {
    ...
    actor->momx = FixedMul(
        FixedMul(xyspeed, FINECOSINE(actor->angle >> ANGLETOFINESHIFT)),
        FINECOSINE(actor->movedir >> ANGLETOFINESHIFT));
    actor->momy =
        FixedMul(FixedMul(xyspeed, FINESINE(actor->angle >> ANGLETOFINESHIFT)),
                 FINECOSINE(actor->movedir >> ANGLETOFINESHIFT));
    actor->momz =
        FixedMul(zspeed, FINESINE(actor->movedir >> ANGLETOFINESHIFT));
    ...
    if (dist <= (3072 * actor->tracer->scale)) // Close enough to target?
    {
      S_StartSound(
          actor,
          actor->info
              ->attacksound); // Siren sound; might not need this anymore, but
                              // I'm keeping it for now just for debugging.
      actor->extravalue1 = 1; // TARGET ACQUIRED
      actor->extravalue2 = 7 * TICRATE;
      actor->cvmem = wspeed;
    }
...
// Dumb simple function that gives a mobj its target's momentums without
// updating its angle.
void A_LightningFollowPlayer(mobj_t *actor) {
  ...
  {
    if (actor->extravalue1) // Make the radius also follow the player somewhat
                            // accuratly
    {
      sx = actor->target->x +
           FixedMul((actor->target->scale * actor->extravalue1),
                    FINECOSINE((actor->angle) >> ANGLETOFINESHIFT));
      sy = actor->target->y +
           FixedMul((actor->target->scale * actor->extravalue1),
                    FINESINE((actor->angle) >> ANGLETOFINESHIFT));
    ...
    K_MatchGenericExtraFlags(actor,
                             actor->target); // copy our target for graviflip
    actor->momx = actor->target->momx;
    actor->momy = actor->target->momy;
    actor->momz = actor->target->momz; // Give momentum since we don't teleport
                                       // to our player literally every frame.
  }
}
...
void A_FZBoomFlash(mobj_t *actor) {
  ...
  for (i = 0; i < MAXPLAYERS; i++) {
    ...
    if (!playeringame[i] || !players[i].mo)
      continue;
    dist = P_AproxDistance(P_AproxDistance(actor->x - players[i].mo->x,
                                           actor->y - players[i].mo->y),
                           actor->z - players[i].mo->z);
...
void A_FZBoomSmoke(mobj_t *actor) {
  ...
  for (i = 0; i < 8 + (4 * var1); i++) {
    mobj_t *smoke =
        P_SpawnMobj(actor->x + (P_RandomRange(-rad, rad) * actor->scale),
                    actor->y + (P_RandomRange(-rad, rad) * actor->scale),
...
void A_RandomShadowFrame(mobj_t *actor) {
  ...
  if (!actor->extravalue1) // Hack that spawns thoks that look like random
                           // shadows. Otherwise the state would overwrite our
                           // frame and that's a pain.
  {
  ...
  // I have NO CLUE how to hardcode all of that fancy Linedef Executor shit so
  // the fire spinout will be done by these entities directly.
  if (P_LookForPlayers(actor, false, false, 380 << FRACBITS)) // got target
  {
    if (actor->target && !actor->target->player->powers[pw_flashing] &&
        ...
        P_IsObjectOnGround(actor->target) && actor->z == actor->target->z) {
      P_DamageMobj(actor->target, actor, actor, 1);
      P_InstaThrust(actor->target, actor->angle, 16 << FRACBITS);
      fire = P_SpawnMobj(actor->target->x, actor->target->y, actor->target->z,
                         MT_THOK);
  ...
  if (!actor->extravalue2) {
    ...
    actor->extravalue1 =
        ((actor->extravalue1)
             ? (actor->extravalue1 - 1)
             : (TICRATE * 5 +
                1)); // deplete timer if set, set to 5 ticrate otherwise.
    ...
    if (P_LookForPlayers(actor, false, false, 256 << FRACBITS)) // got target
    {
      if (actor->target && !actor->target->player->powers[pw_flashing] &&
        ...
        // Spawn the WIND:
        wind = P_SpawnMobj(actor->target->x, actor->target->y, actor->target->z,
                           MT_THOK); // Opaque layer:
        P_SetMobjState(wind, S_GARU1);
        P_SetScale(wind, FRACUNIT * 3 / 2);
        wind = P_SpawnMobj(actor->target->x, actor->target->y, actor->target->z,
                           MT_THOK); // Translucent layer:
...
// A_MayonakaArrow
// Used for the arrow sprite animations in Mayonaka. It's only extra visual
// bullshit to make em more random.
...
void A_MayonakaArrow(mobj_t *actor) {
  ...
  iswarning = actor->spawnpoint->options &
              MTF_OBJECTSPECIAL; // is our object a warning sign?
  // "animtimer" is replaced by "extravalue1" here.
  actor->extravalue1 =
      ((actor->extravalue1)
           ? (actor->extravalue1 + 1)
           : (P_RandomRange(0, (iswarning) ? (TICRATE / 2) : TICRATE * 3)));
  flip = ((actor->spawnpoint->options & 1)
              ? (3)
              : (0)); // flip adds 3 frames, which is the flipped version of the
                      // sign.
...
void A_MementosTPParticles(mobj_t *actor) {
  ...
  for (; i < 4; i++) {
    particle = P_SpawnMobj(actor->x + (P_RandomRange(-256, 256) << FRACBITS),
                           actor->y + (P_RandomRange(-256, 256) << FRACBITS),
                           actor->z + (P_RandomRange(48, 256) << FRACBITS),
                           MT_MEMENTOSPARTICLE);
    particle->frame = 0;
  ...
  // Although this is mostly used to spawn particles, we will also save the
  // OTHER teleport inside actor->target. That way teleporting doesn't require a
  // thinker iteration. Doesn't seem like much given the small amount of mobjs
  // this map has but heh.
  if (!actor->target) {
    for (th = thinkercap.next; th != &thinkercap; th = th->next) {
      if (th->function.acp1 != (actionf_p1)P_MobjThinker)
        continue;
...
// A_ReaperThinker
// Mementos's Reaper's thinker. A huge pain in the Derek Bum to translate from
// Lua to this shite if you ask me.
...
void A_ReaperThinker(mobj_t *actor) {
  ...
  UINT32 maxscore =
      0; // we target the player with the highest score so yeah there you go.
  player_t *player; // used as a shortcut in a loop.
  mobj_t *targetplayermo =
      NULL; // the player mo we can eventually target, or whatever.

  if (LUA_CallAction("A_ReaperThinker", (actor)))
    return;

  // We don't have custom variables or whatever so we'll do with whatever the
  // fuck we have left.

  if (actor->health ==
      1000) // if health is 1000, set it to a small scale and have it start
            // growing with destscale. Then set the health to uh, not 1000.
  {
    actor->scale = 1;
    actor->destscale = 2 << FRACBITS;
    actor->scalespeed =
        FRACUNIT / 24; // Should take a bit less than 2 seconds to fully grow.
    S_StartSound(NULL, sfx_chain);
    actor->health--; // now we have 999 health, so that above won't happen
                     // again. Awesome.
  }
  ...
  if (actor->scale < 2 << FRACBITS) // we haven't finished growing YET.
  {
    ...
    for (; i < 16; i++) {
      particle = P_SpawnMobj(actor->x + (P_RandomRange(-60, 60) << FRACBITS),
                             actor->y + (P_RandomRange(-60, 60) << FRACBITS),
                             actor->z, MT_THOK);
  ...
  // We finished growing and can now be a dangerous piece o' garbage scaring the
  // living heck outta players!

  actor->flags = MF_NOGRAVITY | MF_PAIN | MF_SPECIAL | MF_NOCLIP |
                 MF_NOCLIPHEIGHT; // set our flags to be a damaging thing.
  // Handle animation:
  if (!(leveltime % 5))
    actor->extravalue2 =
        (actor->extravalue2 < 9)
            ? (actor->extravalue2 + 1)
            : (0); // Ghetto animation, but hey it works for what it's worth
  ...
  if (!actor->target) {
    if (actor->hnext) {
      P_SetTarget(&actor->target,
                  actor->hnext); // Default back to last waypoint.
      return;
    }

    // We have no target and oughta find one, so let's scan through thinkers for
    // a waypoint of angle 0, or something.
    for (th = thinkercap.next; th != &thinkercap; th = th->next) {
      if (th->function.acp1 != (actionf_p1)P_MobjThinker)
        continue;
      ...
      P_SetTarget(&actor->target, mo2); // The main target we're pursing.
      P_SetTarget(&actor->hnext,
                  mo2); // The last waypoint we hit. We will default back to
                        // that if a player goes out of our range!
      actor->extravalue1 =
          0; // This will store the angle of the last waypoint we touched. This
             // will essentially be useful later on.
      if (!actor->tracer) // If we already have a tracer (Waypoint #0), don't do
                          // anything.
      {
        P_SetTarget(&actor->tracer,
                    mo2); // Because our target might be a player OR a waypoint,
                          // we need some sort of fallback option. This will
                          // always be waypoint 0.
        break;
      }
    }
  } else // Awesome, we now have a target.
  {
    ...
    for (i = 0; i < MAXPLAYERS; i++) {
      ...
      player = &players[i];
      if (player && player->mo && player->kartstuff[k_bumper] &&
    ...
    if (targetplayermo) {
      ...
      {
        if (!(actor->target == targetplayermo && actor->target &&
              !actor->target->player->powers[pw_flashing] &&
              !actor->target->player->kartstuff[k_invincibilitytimer] &&
              !actor->target->player->kartstuff[k_growshrinktimer] &&
              !actor->target->player->kartstuff[k_spinouttimer]))
          P_SetTarget(&actor->target, actor->hnext);
        // if the above isn't correct, then we should go back to targetting
    ...
    if (actor->target->type == MT_REAPERWAYPOINT) {
      ...
      if (R_PointToDist2(actor->x, actor->y, actor->target->x,
                         actor->target->y) < 22 << FRACBITS) {
        P_SetTarget(&actor->target,
                    NULL); // remove target so we can default back to first
                           // waypoint if things go ham.
        ...
        for (th = thinkercap.next; th != &thinkercap; th = th->next) {
          ...
          P_SetTarget(&actor->hnext,
                      mo2); // The last waypoint we hit. We will default back to
                            // that if a player goes out of our range!
          actor->extravalue1++; // This will store the angle of the last
                                // waypoint we touched. This will essentially be
                                // useful later on.
          break;
        }
      }
    ...
    {
      if (!P_CheckSight(actor, actor->target) ||
          R_PointToDist2(actor->x, actor->y, actor->target->x,
                         actor->target->y) > 1024 << FRACBITS)
        P_SetTarget(&actor->target, actor->hnext);
    }
  }
}
...
void A_FlameParticle(mobj_t *actor) {
  ...
  par = P_SpawnMobj(actor->x + (P_RandomRange(-rad, rad) << FRACBITS),
...
// var2 = Lower four bits: height offset, Upper 4 bits = set if object is
// Nightopian Helper
//
void A_OrbitNights(mobj_t *actor) {
  ...
  if (!actor->target || !actor->target->player || !actor->target->tracer ||
    ...
    P_UnsetThingPosition(actor);
    {
      ...
      const angle_t ofa =
          ((angle_t)actor->extravalue1 + (ofs * ANG1)) >> ANGLETOFINESHIFT;

      const fixed_t fc =
          FixedMul(FINECOSINE(fa), FixedMul(32 * FRACUNIT, actor->scale));
      const fixed_t fh =
          FixedMul(FINECOSINE(ofa), FixedMul(20 * FRACUNIT, actor->scale));
      const fixed_t fs =
          FixedMul(FINESINE(fa), FixedMul(32 * FRACUNIT, actor->scale));

      actor->x = actor->target->tracer->x + fc;
      actor->y = actor->target->tracer->y + fs;
      actor->z =
          actor->target->tracer->z + fh + FixedMul(16 * FRACUNIT, actor->scale);
...
// Function: A_GhostMe
//
// Description: Spawns a "ghost" mobj of this actor, ala spindash trails and the
// minus's digging "trails"
//
// var1 = unused
// var2 = unused
//
...
//
// Description: Changes the state of all active objects of a certain type in a
// certain range of the actor.
//
// var1 = state number
// var2:
//		lower 16 bits = type
...
// var2:
//		lower 16 bits = If 1, xy momentum is lost. If 0, xy momentum is
// kept 		upper 16 bits = 0 - target, 1 - tracer
//
void A_PushAway(mobj_t *actor) {
  INT32 locvar1 = var1;
  INT32 locvar2 = var2;
  ...
  if ((!(locvar2 >> 16) && !actor->target) ||
      ((locvar2 >> 16) && !actor->tracer))
    return;
...
void A_SplitShot(mobj_t *actor) {
  ...
  A_FaceTarget(actor);
  {
    ...
    P_SpawnPointMissile(actor, actor->target->x + xs, actor->target->y + ys,
                        actor->target->z, loc2lw, actor->x, actor->y, z);
    P_SpawnPointMissile(actor, actor->target->x - xs, actor->target->y - ys,
                        actor->target->z, loc2lw, actor->x, actor->y, z);
  }
}
...
//
// Description: If the object is a missile it will create a new missile with an
// alternate flight path owned by the one who shot the former missile.
//
...
void A_MissileSplit(mobj_t *actor) {
  ...
  if (LUA_CallAction("A_MissileSplit", actor))
    return;
  if (actor->eflags & MFE_VERTICALFLIP)
    P_SpawnAlteredDirectionMissile(actor, locvar1, actor->x, actor->y,
                                   actor->z + actor->height, locvar2);
  else
    P_SpawnAlteredDirectionMissile(actor, locvar1, actor->x, actor->y, actor->z,
                                   locvar2);
}

// Function: A_MultiShot
//
// Description: Shoots objects horizontally that spread evenly in all
// directions.
//
// var1:
...
void A_MultiShot(mobj_t *actor) {
  ...
  if (actor->eflags & MFE_VERTICALFLIP)
    ...
    z = actor->z + FixedMul(48 * FRACUNIT + locvar2 * FRACUNIT, actor->scale);
  xr = FixedMul((P_SignedRandom() / 3) << FRACBITS,
                actor->scale); // please note p_mobj.c's P_Rand() abuse
  yr = FixedMul((P_SignedRandom() / 3) << FRACBITS,
                actor->scale); // of rand(), RAND_MAX and signness mess

  while (count <= loc1lw && loc1lw >= 1) {
    const angle_t fa =
        FixedAngleC(count * FRACUNIT * 360, ad) >> ANGLETOFINESHIFT;
    ...
    P_SpawnPointMissile(actor, xrc - yrs + actor->x, xrs + yrc + actor->y, z,
                        loc1up, actor->x, actor->y, z);
    count++;
  }
...
void A_InstaLoop(mobj_t *actor) {
  ...
  const UINT16 loc1lw = (UINT16)(locvar1 & 65535);
  const UINT16 loc1up = (UINT16)(locvar1 >> 16);
  const angle_t fa = FixedAngleC(loc1lw * FRACUNIT * 360, loc1up * FRACUNIT) >>
                     ANGLETOFINESHIFT;
  ...
  P_InstaThrust(actor, actor->angle,
                FixedMul(ac, FixedMul(force, actor->scale)));
...
// var2:
//		lower 16 bits = vertical rotation speed in 1/10 fracunits per
// tic 		upper 16 bits = horizontal rotation speed in 1/10 fracunits per
// tic
//
void A_Custom3DRotate(mobj_t *actor) {
  INT32 locvar1 = var1;
  ...
  P_UnsetThingPosition(actor);
  {
    ...
    if (vspeed == 0 && hspeed != 0) {
      ...
      actor->x = actor->target->x +
                 FixedMul(FixedMul(FINESINE(md), FINECOSINE(fa)), radius);
      actor->y = actor->target->y +
                 FixedMul(FixedMul(FINESINE(md), FINESINE(fa)), radius);
      actor->z = actor->target->z + FixedMul(FINECOSINE(md), radius) +
                 actor->target->height / 2 - actor->height / 2 + hOff;
    }
  }
  P_SetThingPosition(actor);
}
...
//
// Description: Checks if the actor has targeted a vulnerable player. If not a
// new player will be searched all around. If no players are available the
// object can call a specific state. (Useful for not moving enemies)
//
// var1:
//		if var1 == 0, if necessary call state with same state number as
// var2 		else, do not call a specific state if no players are
// available
// var2 = state number
//
void A_SearchForPlayers(mobj_t *actor) {
...
//
// Description: Calls a state depending on the ammount of rings currently owned
// by targeted players.
//
...
//
// Description: Calls a state depending on the maximum ammount of rings owned by
// all players during this try.
//
...
void A_CheckRange(mobj_t *actor) {
  ...
  if ((!(locvar1 >> 16) && !actor->target) ||
      ((locvar1 >> 16) && !actor->tracer))
    return;
  ...
  if (!(locvar1 >> 16)) // target
    ...
    dist = P_AproxDistance(actor->tracer->x - actor->x,
                           actor->tracer->y - actor->y);
...
//
// Description: Calls a state if the object and it's target have a height offset
// <= var1 compared to each other.
//
// var1:
...
//
// Description: Calls a state if the object's target is in true range. (Checks
// height, too.)
//
// var1:
//		lower 16 bits = range
...
void A_RelayCustomValue(mobj_t *actor) {
  ...
  INT32 tempT; // target's value - changed to tracer if var1 upper 16 bits set,
               // then modified to become final value
...
//
// Description: Calls an action from a reference state applying custom value
// parameters.
//
...
//      else if var2 == 3, replace new action's var2 with custom value and var1
//      with memory value else if var2 == 2, replace new action's var1 with
//      custom value and var2 with memory value else if var2 == 1, only replace
//      new action's var2 with custom value else if var2 == 0, only replace new
//      action's var1 with custom value
//
void A_CusValAction(mobj_t *actor) {
  INT32 locvar1 = var1;
  INT32 locvar2 = var2;
...
// Function: A_Repeat
//
// Description: Returns to state var2 until animation has been used var1 times,
// then continues to nextstate.
//
// var1 = repeat count
...
void A_SetScale(mobj_t *actor) {
  ...
  if (!(locvar2 & 65535))
    P_SetScale(
        target,
        locvar1); // this instantly changes current scale to var1 if used,
                  // if not destscale will alter scale to var1 anyway
}

// Function: A_RemoteDamage
//
// Description: Damages, kills or even removes either the actor or its
// target/tracer. Actor acts as the inflictor/source unless harming itself
//
...
void A_RemoteDamage(mobj_t *actor) {
  ...
  {
    if (target
            ->player) // players die using P_DamageMobj instead for some reason
...
void A_HomingChase(mobj_t *actor) {
  ...
  dist =
      P_AproxDistance(P_AproxDistance(dest->x - actor->x, dest->y - actor->y),
                      dest->z - actor->z);
...
// Function: A_TrapShot
//
// Description: Fires a missile in a particular direction and angle rather than
// AT something, Trapgoyle-style!
//
// var1:
...
void A_TrapShot(mobj_t *actor) {
  ...
  x = actor->x + P_ReturnThrustX(actor, actor->angle,
                                 FixedMul(frontoff * FRACUNIT, actor->scale));
  y = actor->y + P_ReturnThrustY(actor, actor->angle,
                                 FixedMul(frontoff * FRACUNIT, actor->scale));

  if (actor->eflags & MFE_VERTICALFLIP) {
    z = actor->z + actor->height - FixedMul(vertoff * FRACUNIT, actor->scale) -
        FixedMul(mobjinfo[type].height, actor->scale);
    vertang = InvAngle(vertang); // flip firing angle
  } else
    z = actor->z + FixedMul(vertoff * FRACUNIT, actor->scale);

  CONS_Debug(DBG_GAMELOGIC,
             "A_TrapShot: missile no. = %d, front offset = %d, vertical angle "
             "= %d, z offset = %d\n",
  ...
  missile->momx =
      FixedMul(FINECOSINE(vertang >> ANGLETOFINESHIFT),
               FixedMul(FINECOSINE(missile->angle >> ANGLETOFINESHIFT), speed));
  missile->momy =
      FixedMul(FINECOSINE(vertang >> ANGLETOFINESHIFT),
               FixedMul(FINESINE(missile->angle >> ANGLETOFINESHIFT), speed));
  missile->momz = FixedMul(FINESINE(vertang >> ANGLETOFINESHIFT), speed);
}

// Function: A_VileTarget
//
// Description: Spawns an object directly on the target, and sets this object as
// the actor's tracer.
//              Originally used by Archviles to summon a pillar of hellfire,
//              hence the name.
//
// var1 = mobj to spawn
// var2 = If 0, target only the actor's target. Else, target every player,
// period.
//
void A_VileTarget(mobj_t *actor) {
  INT32 locvar1 = var1;
  ...
  if (!locvar2) {
    fog =
        P_SpawnMobj(actor->target->x, actor->target->y,
                    actor->target->z +
                        ((actor->target->eflags & MFE_VERTICALFLIP)
                             ? actor->target->height - mobjinfo[fogtype].height
                             : 0),
                    fogtype);
    if (actor->target->eflags & MFE_VERTICALFLIP) {
    ...
    // Our "Archvile" here is actually Oprah. "YOU GET A TARGET! YOU GET A
    // TARGET! YOU ALL GET A TARGET!"
    for (i = 0; i < MAXPLAYERS; i++) {
      if (!playeringame[i] || players[i].spectator)
        continue;
      ...
      fog = P_SpawnMobj(
          ...
          players[i].mo->z +
              ((players[i].mo->eflags & MFE_VERTICALFLIP)
                   ? players[i].mo->height - mobjinfo[fogtype].height
                   : 0),
          fogtype);
      ...
      if (players[i].mo ==
          actor->target) // We only care to track the fog targeting who we
                         // REALLY hate right now
        P_SetTarget(&actor->tracer, fog);
      P_SetTarget(&fog->target, actor);
...
//
// Description: Instantly hurts the actor's target, if it's in the actor's line
// of sight.
//              Originally used by Archviles to cause explosions where their
//              hellfire pillars were, hence the name.
//
// var1 = sound to play
// var2:
//		Lower 16 bits = optional explosion object
//		Upper 16 bits = If 0, attack only the actor's target. Else,
// attack all the players. All of them.
//
void A_VileAttack(mobj_t *actor) {
  INT32 locvar1 = var1;
  INT32 locvar2 = var2;
  ...
  if (!(locvar2 & 0xFFFF0000)) {
    ...
    P_DamageMobj(actor->target, actor, actor, 1);
    // actor->target->momz = 1000*FRACUNIT/actor->target->info->mass; // How id
    // did it
    actor->target->momz += FixedMul(10 * FRACUNIT, actor->scale) *
                           P_MobjFlip(actor->target); // How we're doing it
    if (explosionType != MT_NULL) {
      P_SpawnMobj(actor->target->x, actor->target->y, actor->target->z,
                  explosionType);
    }

    // Extra attack. This was for additional damage in Doom. Doesn't really
    // belong in SRB2, but the heck with it, it's here anyway.
    fire = actor->tracer;
    ...
    P_MoveOrigin(fire,
                 actor->target->x -
                     P_ReturnThrustX(fire, actor->angle,
                                     FixedMul(24 * FRACUNIT, fire->scale)),
                 actor->target->y -
                     P_ReturnThrustY(fire, actor->angle,
                                     FixedMul(24 * FRACUNIT, fire->scale)),
                 fire->z);
    ...
    for (i = 0; i < MAXPLAYERS; i++) {
      ...
      // actor->target->momz = 1000*FRACUNIT/actor->target->info->mass; // How
      // id did it
      players[i].mo->momz += FixedMul(10 * FRACUNIT, actor->scale) *
                             P_MobjFlip(players[i].mo); // How we're doing it
      if (explosionType != MT_NULL) {
        P_SpawnMobj(players[i].mo->x, players[i].mo->y, players[i].mo->z,
                    explosionType);
      }

      // Extra attack. This was for additional damage in Doom. Doesn't really
      // belong in SRB2, but the heck with it, it's here anyway. However, it
      // ONLY applies to the actor's target. Nobody else matters!
...
// Function: A_VileFire
//
// Description: Kind of like A_CapeChase; keeps this object in front of its
// tracer, unless its target can't see it.
//				Originally used by Archviles to keep their
// hellfire pillars on top of the player, hence the name (although it was just
// "A_Fire"
// there; added "Vile" to make it more specific). 				Added
// some functionality to optionally draw a line directly to the enemy doing the
// targetting. Y'know, to hammer things in a bit.
//
// var1 = sound to play
// var2:
...
void A_VileFire(mobj_t *actor) {
  ...
  P_UnsetThingPosition(actor);
  actor->x = dest->x + P_ReturnThrustX(actor, dest->angle,
                                       FixedMul(24 * FRACUNIT, actor->scale));
  actor->y = dest->y + P_ReturnThrustY(actor, dest->angle,
                                       FixedMul(24 * FRACUNIT, actor->scale));
  actor->z =
      dest->z +
      ((actor->eflags & MFE_VERTICALFLIP) ? dest->height - actor->height : 0);
  P_SetThingPosition(actor);
  ...
  if (locvar2 & 0xFFFF) {
    ...
    distZ = ((actor->target->z +
              FixedMul(actor->target->height / 2, actor->target->scale)) -
             (actor->z + FixedMul(actor->height / 2, actor->scale))) /
            numLineMobjs;

    for (i = 1; i <= numLineMobjs; i++) {
      P_SpawnMobj(actor->x + (distX * i), actor->y + (distY * i),
                  actor->z + (distZ * i) +
                      FixedMul(actor->height / 2, actor->scale),
                  lineMobj);
    }
  }
}
...
//
// Description: Chase after your target, but speed and attack are tied to
// health.
//
// Every time this is called, generate a random number from a 1/4 to 3/4 of
// mobj's spawn health. If health is above that value, use missilestate to
// attack. If health is at or below that value, use meleestate to attack
// (default to missile state if not available).
//
// Likewise, state will linearly speed up as health goes down.
// Upper bound will be the frame's normal length.
// Lower bound defaults to 1 tic (technically 0, but we round up), unless a
// lower bound is specified in var1.
//
...
void A_BrakChase(mobj_t *actor) {
  ...
  // Set new tics NOW, in case the state changes while we're doing this and we
  // try applying this to the painstate or something silly
  ...
  if (P_CheckMissileRange(actor) && !actor->movecount) {
    // Check if we should use "melee" attack first. (Yes, this still runs
    // outside of melee range. Quiet, you.)
    if (actor->info->meleestate &&
        actor->health <=
            P_RandomRange(actor->info->spawnhealth / 4,
                          (actor->info->spawnhealth * 3) /
                              4)) // Guaranteed true if <= 1/4 health,
                                  // guaranteed false if > 3/4 health
    {
      if (actor->info->attacksound)
        S_StartAttackSound(actor, actor->info->attacksound);
  ...
  // make active sound
  if (actor->type != MT_CYBRAKDEMON && actor->info->activesound &&
      P_RandomChance(3 * FRACUNIT / 256)) {
    S_StartSound(actor, actor->info->activesound);
  }
}
...
//
// Description: Shoot an object at your target, offset to match where Brak's gun
// is. Also, sets Brak's reaction time; behaves normally otherwise.
//
// var1 = object # to shoot
// var2 = unused
//
void A_BrakFireShot(mobj_t *actor) {
  ...
  x = actor->x +
      P_ReturnThrustX(actor, actor->angle,
                      FixedMul(64 * FRACUNIT, actor->scale)) +
      P_ReturnThrustX(actor, actor->angle + ANGLE_270,
                      FixedMul(32 * FRACUNIT, actor->scale));
  y = actor->y +
      P_ReturnThrustY(actor, actor->angle,
                      FixedMul(64 * FRACUNIT, actor->scale)) +
      P_ReturnThrustY(actor, actor->angle + ANGLE_270,
                      FixedMul(32 * FRACUNIT, actor->scale));
  if (actor->eflags & MFE_VERTICALFLIP)
...
//
// Description: Lobs an object at the floor about a third of the way toward your
// target.
//				Implication is it'll bounce the rest of the way.
//				(You can also just aim straight at the target,
// but whatever) 				Formula grabbed from
// http://en.wikipedia.org/wiki/Trajectory_of_a_projectile#Angle_required_to_hit_coordinate_.28x.2Cy.29
//
// var1 = object # to lob
// var2:
//		Lower 16 bits: height offset to shoot from, from the actor's
// bottom (none that "airtime" malarky) 		Upper 16 bits: if 0, aim
// 1/3 of the way. Else, aim directly at target.
//
...
void A_BrakLobShot(mobj_t *actor) {
  ...
  INT32 intHypotenuse; // x^2 + y^2. Frequently overflows fixed point, hence why
                       // we need integers proper.
  fixed_t fixedHypotenuse; // However, we can work around that and still get a
                           // fixed-point number.
  ...
  if (actor->subsector->sector->gravity)
    g = FixedMul(
        gravity,
        (FixedDiv(*actor->subsector->sector->gravity >> FRACBITS, 1000)));
  else
    g = gravity;
  ...
  if (!aimDirect) {
    ...
    // Look up height difference between actor and the ground 1/3 of the way to
    // its target
    y = P_FloorzAtPos(newTargetX, newTargetY, actor->target->z,
                      actor->target->height) -
        (actor->z + FixedMul(locvar2 * FRACUNIT, actor->scale));
  } else {
  ...
  // Get x^2 + y^2. Have to do it in a roundabout manner, because this overflows
  // fixed_t way too easily otherwise.
  x_int = x >> FRACBITS;
  y_int = y >> FRACBITS;
  ...
  // Determine which one isn't actually an imaginary number (or the smaller of
  // the two, if both are real), and use that for v.
  if (a1 < 0 || a2 < 0) {
    if (a1 < 0 && a2 < 0) {
      // Somehow, v^2 is negative in both cases. v is therefore imaginary and
      // something is horribly wrong. Abort!
      return;
    }
  ...
  // We can cut the "+/- sqrt" part out entirely, since v was calculated
  // specifically for it to equal zero. So:
  ...
  if (locvar1 <= 0 || locvar1 >= NUMMOBJTYPES)
    ...
    typeOfShot = (mobjtype_t)locvar1;
  shot = P_SpawnMobj(actor->x, actor->y,
                     actor->z + FixedMul(locvar2 * FRACUNIT, actor->scale),
                     typeOfShot);
  if (shot->info->seesound)
    S_StartSound(shot, shot->info->seesound);
  ...
  // Horizontal axes first. First parameter is initial horizontal impulse,
  // second is to correct its angle.
  shot->momx = FixedMul(FixedMul(v, FINECOSINE(theta >> ANGLETOFINESHIFT)),
                        FINECOSINE(shot->angle >> ANGLETOFINESHIFT));
  shot->momy = FixedMul(FixedMul(v, FINECOSINE(theta >> ANGLETOFINESHIFT)),
                        FINESINE(shot->angle >> ANGLETOFINESHIFT));
...
//				Intended for use with objects that are affected
// by gravity; would be kind of silly otherwise.
//
// var1:
//		Lower 16 bits: object # to lob (TODO: come up with a default)
//		Upper 16 bits: Number to lob (default 8)
// var2:
//		Lower 16 bits: distance to toss them (No default - 0 does just
// that - but negatives will revert to 128) 		Upper 16 bits: airtime
// in tics (default 16)
//
void A_NapalmScatter(mobj_t *actor) {
  ...
  fixed_t airtime =
      var2 & 0xFFFF0000; // How long until impact (assuming no obstacles)
  ...
  if (typeOfShot >= NUMMOBJTYPES) // I'd add a <0 check, too, but 0x0000FFFF
                                  // isn't negative in this case
    typeOfShot = MT_NULL;
  if (numToShoot <= 0) // Presumably you forgot to set var1 up; else, why are
                       // you calling this to shoot nothing?
    numToShoot = 8;
  else if (numToShoot >
           8192) // If you seriously need this many objects spawned, stop and
                 // ask yourself "Why am I doing this?"
    numToShoot = 8192;
  if (distance <
      0) // Presumably you thought this was an unsigned integer, you naive fool
  ...
  for (i = 0; i < numToShoot; i++) {
    ...
    P_SetTarget(&mo->target, actor->target); // Transfer target so Brak doesn't
                                             // hit himself like an idiot
...
//
// Description: Spawns a copy of the mobj. x, y, z, angle, scale, target and
// tracer carry over; everything else starts anew.
//				Mostly writing this because I want to do
// multiple actions to pass these along in a single frame instead of several.
//
// var1 = unused
// var2 = unused
//
