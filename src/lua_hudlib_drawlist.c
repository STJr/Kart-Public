...
huddrawlist_h LUA_HUD_CreateDrawList(void) {
  ...
  drawlist = (huddrawlist_h)Z_CallocAlign(sizeof(struct huddrawlist_s),
                                          PU_STATIC, NULL, 64);
  drawlist->items = NULL;
...
static size_t AllocateDrawItem(huddrawlist_h list) {
  if (!list)
    I_Error("can't allocate draw item: invalid list");
  if (list->items_capacity <= list->items_len + 1) {
    if (list->items_capacity == 0)
      list->items_capacity = 128;
    else
      list->items_capacity *= 2;
    list->items = (drawitem_t *)Z_ReallocAlign(
        list->items, sizeof(struct drawitem_s) * list->items_capacity,
        PU_STATIC, NULL, 64);
  }
...
static size_t CopyString(huddrawlist_h list, const char *str) {
  ...
  if (!list)
    I_Error("can't allocate string; invalid list");
  lenstr = strlen(str);
  if (list->strbuf_capacity <= list->strbuf_len + lenstr + 1) {
    if (list->strbuf_capacity == 0)
      list->strbuf_capacity = 256;
    else
      list->strbuf_capacity *= 2;
    list->strbuf = (char *)Z_ReallocAlign(
        list->strbuf, sizeof(char) * list->strbuf_capacity, PU_STATIC, NULL, 8);
  }
...
void LUA_HUD_DrawList(huddrawlist_h list) {
  ...
  if (!list)
    I_Error("HUD drawlist invalid");
  if (list->items_len <= 0)
    return;
  if (!list->items)
    I_Error("HUD drawlist->items invalid");
  ...
  for (i = 0; i < list->items_len; i++) {
    ...
    switch (item->type) {
    case DI_Draw:
      V_DrawFixedPatch(item->x << FRACBITS, item->y << FRACBITS, FRACUNIT,
                       item->flags, item->patch, item->colormap);
      break;
    case DI_DrawScaled:
      V_DrawFixedPatch(item->x, item->y, item->scale, item->flags, item->patch,
                       item->colormap);
      break;
    case DI_DrawNum:
    ...
    case DI_DrawPaddedNum:
      V_DrawPaddedTallNum(item->x, item->y, item->flags, item->num,
                          item->digits);
      break;
    case DI_DrawFill:
