...
//-----------------------------------------------------------------------------
/// \file  y_inter.c
/// \brief Tally screens, or "Intermissions" as they were formally called in
/// Doom
...
static void Y_CompareRace(INT32 i) {
  UINT32 val =
      ((players[i].pflags & PF_TIMEOVER || players[i].realtime == UINT32_MAX)
           ? (UINT32_MAX - 1)
           : players[i].realtime);
...
static void Y_CompareBattle(INT32 i) {
  UINT32 val = ((players[i].pflags & PF_TIMEOVER) ? (UINT32_MAX - 1)
                                                  : players[i].marescore);

  if (!(data.match.val[data.match.numplayers] == UINT32_MAX ||
        (!(players[i].pflags & PF_TIMEOVER) &&
         val > data.match.val[data.match.numplayers])))
    return;
...
static void Y_CompareRank(INT32 i) {
  UINT8 increase =
      ((data.match.increase[i] == UINT8_MAX) ? 0 : data.match.increase[i]);
  if (!(data.match.val[data.match.numplayers] == UINT32_MAX ||
        (players[i].score - increase) > data.match.val[data.match.numplayers]))
    return;
...
static void Y_CalculateMatchData(UINT8 rankingsmode,
                                 void (*comparison)(INT32)) {
  INT32 i, j;
  ...
  if (rankingsmode > 1)
    ...
    if (mapheaderinfo[prevmap]->levelflags & LF_NOZONE) {
      if (mapheaderinfo[prevmap]->actnum[0])
        snprintf(data.match.levelstring, sizeof data.match.levelstring,
                 "* %s %s *", mapheaderinfo[prevmap]->lvlttl,
                 mapheaderinfo[prevmap]->actnum);
      else
        snprintf(data.match.levelstring, sizeof data.match.levelstring,
                 "* %s *", mapheaderinfo[prevmap]->lvlttl);
    } else {
      const char *zonttl =
          (mapheaderinfo[prevmap]->zonttl[0] ? mapheaderinfo[prevmap]->zonttl
                                             : "Zone");
      if (mapheaderinfo[prevmap]->actnum[0])
        snprintf(data.match.levelstring, sizeof data.match.levelstring,
                 "* %s %s %s *", mapheaderinfo[prevmap]->lvlttl, zonttl,
                 mapheaderinfo[prevmap]->actnum);
      else
        snprintf(data.match.levelstring, sizeof data.match.levelstring,
                 "* %s %s *", mapheaderinfo[prevmap]->lvlttl, zonttl);
    }
  ...
  for (j = 0; j < numplayersingame; j++) {
    INT32 nump = ((G_RaceGametype() && nospectategrief > 0) ? nospectategrief
                                                            : numplayersingame);
...
// (SRB2Kart: er, about that...) Neat concept, huh?
//
void Y_IntermissionDrawer(void) {
  ...
  if (useinterpic)
    ...
    if (rendermode == render_soft && usebuffer)
      VID_BlitLinearScreen(screens[1], screens[0], vid.width * vid.bpp,
                           vid.height, vid.width * vid.bpp, vid.rowbytes);
    else if (rendermode != render_soft && usebuffer) {
      HWR_DrawIntermissionBG();
    } else {
      if (widebgpatch && rendermode == render_soft &&
          vid.width / vid.dupx == 400)
  ...
  /*if (intertype == int_timeattack)
  {
          ...
          else
          {
                  ...
                  // SRB2kart - pulled from old coop block, just in case we need
  it
                  // we should show centiseconds on the intermission screen too,
  if the conditions are right. if (modeattacking || cv_timetic.value == 2)
                  {
  ...
  Period ST_DrawPadNumFromHud(HUD_TICS, tictrn, 2); // Tics
          ...
          V_DrawLevelTitle(data.coop.passedx2,
  49+V_LevelNameHeight(data.coop.passed2)+2, 0, data.coop.passed2);

          if (strlen(mapheaderinfo[prevmap]->actnum) > 0)
                  V_DrawScaledPatch(244, 57, 0, data.coop.ttlnum);

          //if (gottimebonus && endtic != -1)
          //	V_DrawCenteredString(BASEVIDWIDTH/2, 172, V_YELLOWMAP, "TIME
  BONUS UNLOCKED!");
  }
  else*/
  if (intertype == int_race || intertype == int_match) {
#define NUMFORNEWCOLUMN 8
    INT32 y = 41, gutter = ((data.match.numplayers > NUMFORNEWCOLUMN)
                                ? 0
                                : (BASEVIDWIDTH / 2));
    ...
    // draw the level name
    V_DrawCenteredString(-4 + x + BASEVIDWIDTH / 2, 12, 0,
                         data.match.levelstring);
    V_DrawFill((x - 3) - duptweak, 34, dupadjust - 2, 1, 0);

    if (data.match.encore)
      V_DrawCenteredString(-4 + x + BASEVIDWIDTH / 2, 12 - 8, hilicol,
                           "ENCORE MODE");
    ...
    V_DrawRightAlignedString(x + (BASEVIDWIDTH / 2) + 152, 24, hilicol,
                             timeheader);
    ...
    for (i = 0; i < data.match.numplayers; i++) {
      ...
      if (data.match.num[i] != MAXPLAYERS && playeringame[data.match.num[i]] &&
          !players[data.match.num[i]].spectator) {
        ...
        if (data.match.color[i]) {
          UINT8 *colormap = R_GetTranslationColormap(
              *data.match.character[i], *data.match.color[i], GTC_CACHE);
          V_DrawMappedPatch(x + 16, y - 4, 0,
                            facerankprefix[*data.match.character[i]], colormap);
        }
        ...
        if (data.match.num[i] == whiteplayer) {
          ...
          V_DrawScaledPatch(
              x + 16, y - 4, 0,
              W_CachePatchName(va("K_CHILI%d", cursorframe + 1), PU_CACHE));
        }

        STRBUFCPY(strtime, data.match.name[i]);

        if (data.match.numplayers > NUMFORNEWCOLUMN)
          V_DrawThinString(x + 36, y - 1,
                           ((data.match.num[i] == whiteplayer) ? hilicol : 0) |
        ...
        if (data.match.rankingsmode) {
          if (data.match.increase[data.match.num[i]] != UINT8_MAX) {
            if (data.match.increase[data.match.num[i]] > 9)
              snprintf(strtime, sizeof strtime, "(+%02d)",
                       data.match.increase[data.match.num[i]]);
            else
              snprintf(strtime, sizeof strtime, "(+  %d)",
                       data.match.increase[data.match.num[i]]);

            if (data.match.numplayers > NUMFORNEWCOLUMN)
              V_DrawRightAlignedThinString(x + 135 + gutter, y - 1,
                                           V_6WIDTHSPACE, strtime);
            else
          ...
          if (data.match.numplayers > NUMFORNEWCOLUMN)
            V_DrawRightAlignedThinString(x + 152 + gutter, y - 1, V_6WIDTHSPACE,
                                         strtime);
          else
          ...
          if (data.match.val[i] == (UINT32_MAX - 1))
            V_DrawRightAlignedThinString(
                x + 152 + gutter, y - 1,
                (data.match.numplayers > NUMFORNEWCOLUMN ? V_6WIDTHSPACE : 0),
                "NO CONTEST.");
          else {
            if (intertype == int_race) {
              snprintf(strtime, sizeof strtime, "%i'%02i\"%02i",
                       G_TicsToMinutes(data.match.val[i], true),
                       G_TicsToSeconds(data.match.val[i]),
                       G_TicsToCentiseconds(data.match.val[i]));
              strtime[sizeof strtime - 1] = '\0';

              if (data.match.numplayers > NUMFORNEWCOLUMN)
                V_DrawRightAlignedThinString(x + 152 + gutter, y - 1,
                                             V_6WIDTHSPACE, strtime);
              else
                V_DrawRightAlignedString(x + 152 + gutter, y, 0, strtime);
            } else {
              if (data.match.numplayers > NUMFORNEWCOLUMN)
                V_DrawRightAlignedThinString(x + 152 + gutter, y - 1,
                                             V_6WIDTHSPACE,
                                             va("%i", data.match.val[i]));
              else
                V_DrawRightAlignedString(x + 152 + gutter, y, 0,
                                         va("%i", data.match.val[i]));
            }
          }
        }
        ...
        data.match.num[i] = MAXPLAYERS; // this should be the only field setting
                                        // in this function
...
dotimer:
  ...
  if ((demo.recording || demo.savemode == DSM_SAVED) && !demo.playback)
    ...
    case DSM_NOTSAVING:
      V_DrawRightAlignedThinString(BASEVIDWIDTH - 2, 2,
                                   V_SNAPTOTOP | V_SNAPTORIGHT |
                                       V_ALLOWLOWERCASE | hilicol,
  ...
  if (cv_scrambleonchange.value && cv_teamscramble.value &&
      (intertic / TICRATE % 2 == 0))
    V_DrawCenteredString(BASEVIDWIDTH / 2, BASEVIDHEIGHT / 2, hilicol,
                         M_GetText("Teams will be scrambled next round!"));
}

//
// Y_Ticker
//
// Manages fake score tally for single player end of act, and decides when
...
void Y_Ticker(void) {
  ...
  if (demo.recording) {
    ...
    if (demo.savemode == DSM_NOTSAVING &&
        (InputDown(gc_lookback, 1) || (cv_usejoystick.value && axis > 0)))
  ...
  /*if (G_GametypeHasTeams() && cv_teamscramble.value &&
  !cv_scrambleonchange.value && server)
  {
  ...
  if (intertype == int_race || intertype == int_match) {
    if (netgame || multiplayer) {
      if (sorttic == -1)
        sorttic = intertic + max((cv_inttime.value / 2) - 2, 2) *
                                 TICRATE; // 8 second pause after match results
      else if (!(multiplayer &&
                 demo.playback)) // Don't advance to rankings in replays
      {
        if (!data.match.rankingsmode && (intertic >= sorttic + 8))
          Y_CalculateMatchData(1, Y_CompareRank);

        if (data.match.rankingsmode &&
            intertic > sorttic + 16 + (2 * TICRATE)) {
          INT32 q = 0, r = 0;
          ...
          if (r) {
            ...
            endtic =
                intertic + 3 * TICRATE; // 3 second pause after end of tally
        }
      }
    } else
...
static void Y_UpdateRecordReplays(void) {
  ...
  if ((mainrecords[gamemap - 1]->time == 0) ||
      (players[consoleplayer].realtime < mainrecords[gamemap - 1]->time))
    mainrecords[gamemap - 1]->time = players[consoleplayer].realtime;

  if ((mainrecords[gamemap - 1]->lap == 0) ||
      (bestlap < mainrecords[gamemap - 1]->lap))
  ...
  if (FIL_FileExists(lastdemo)) {
    ...
    if (!FIL_FileExists(bestdemo) ||
        G_CmpDemoTime(bestdemo, lastdemo) & 1) { // Better time, save this demo.
      if (FIL_FileExists(bestdemo))
        remove(bestdemo);
    ...
    if (!FIL_FileExists(bestdemo) ||
        G_CmpDemoTime(bestdemo, lastdemo) &
            (1 << 1)) { // Better lap time, save this demo.
      if (FIL_FileExists(bestdemo))
        remove(bestdemo);
      FIL_WriteFile(bestdemo, buf, len);
      CONS_Printf("\x83%s\x80 %s '%s'\n", M_GetText("NEW RECORD LAP!"),
                  M_GetText("Saved replay as"), bestdemo);
    }
  ...
  if ((earnedEmblems = M_CheckLevelEmblems()))
    CONS_Printf(M_GetText("\x82"
                          "Earned %hu medal%s for Record Attack records.\n"),
...
void Y_StartIntermission(void) {
  ...
  if (!multiplayer) {
    ...
    */
    ...
    if (cv_inttime.value == 0 && gametype == GT_COOP)
      timer = 0;
    else if (demo.playback) // Override inttime (which is pulled from the replay
                            // anyway
      timer = 10 * TICRATE;
    else {
  ...
  {
    if (!majormods && !multiplayer &&
        !demo.playback) // remove this once we have a proper time attack screen
    {
...
//
static void Y_UnloadData(void) {
  // In hardware mode, don't Z_ChangeTag a pointer returned by
  // W_CachePatchName(). It doesn't work and is unnecessary.
...
void Y_VoteDrawer(void) {
  ...
  if (widebgpatch && rendermode == render_soft && vid.width / vid.dupx > 320)
    V_DrawScaledPatch(((vid.width / 2) / vid.dupx) -
                          (SHORT(widebgpatch->width) / 2),
                      (vid.height / vid.dupy) - SHORT(widebgpatch->height),
                      V_SNAPTOTOP | V_SNAPTOLEFT, widebgpatch);
  else
    V_DrawScaledPatch(((vid.width / 2) / vid.dupx) -
                          (SHORT(bgpatch->width) /
                           2), // Keep the width/height adjustments, for
                               // screens that are less wide than 320(?)
                      (vid.height / vid.dupy) - SHORT(bgpatch->height),
                      V_SNAPTOTOP | V_SNAPTOLEFT, bgpatch);

  for (i = 0; i < 4;
       i++) // First, we need to figure out the height of this thing...
  {
    UINT8 j;
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (selected[i]) {
      ...
      for (j = 0; j <= splitscreen;
           j++) // another loop for drawing the selection backgrounds in the
                // right order, grumble grumble..
      {
        INT32 handy = y;
        UINT8 p;
        UINT8 *colormap;
        ...
        if (!splitscreen) {
          ...
          colormap = R_GetTranslationColormap(TC_DEFAULT, players[p].skincolor,
                                              GTC_CACHE);
        }
        ...
        V_DrawMappedPatch(BASEVIDWIDTH - 124, handy, V_SNAPTORIGHT, thiscurs,
                          colormap);

        if (votetic % 10 < 4)
          V_DrawFill(BASEVIDWIDTH - 100 - sizeadd, y - sizeadd,
                     80 + (sizeadd * 2), 50 + (sizeadd * 2),
                     120 | V_SNAPTORIGHT);
        else
          V_DrawFill(BASEVIDWIDTH - 100 - sizeadd, y - sizeadd,
                     80 + (sizeadd * 2), 50 + (sizeadd * 2),
      ...
      if (!levelinfo[i].encore)
        ...
        V_DrawFixedPatch((BASEVIDWIDTH - 20) << FRACBITS, (y) << FRACBITS,
                         FRACUNIT / 2, V_FLIP | V_SNAPTORIGHT, pic, 0);
        V_DrawFixedPatch((BASEVIDWIDTH - 60) << FRACBITS,
                         ((y + 25) << FRACBITS) - (rubyheight << 1), FRACUNIT,
                         V_SNAPTORIGHT, rubyicon, NULL);
      }

      V_DrawRightAlignedThinString(BASEVIDWIDTH - 21, 40 + y,
                                   V_SNAPTORIGHT | V_6WIDTHSPACE, str);

      if (levelinfo[i].gts) {
        INT32 w = V_ThinStringWidth(levelinfo[i].gts, V_SNAPTORIGHT) + 1;
        V_DrawFill(BASEVIDWIDTH - 100, y + 10, w + 1, 2, V_SNAPTORIGHT | 31);
        V_DrawFill(BASEVIDWIDTH - 100, y, w, 11,
                   V_SNAPTORIGHT | levelinfo[i].gtc);
        V_DrawDiag(BASEVIDWIDTH - 100 + w + 1, y, 12, V_SNAPTORIGHT | 31);
        V_DrawDiag(BASEVIDWIDTH - 100 + w, y, 11,
                   V_SNAPTORIGHT | levelinfo[i].gtc);
        V_DrawThinString(BASEVIDWIDTH - 99, y + 1, V_SNAPTORIGHT,
                         levelinfo[i].gts);
      }
      ...
      if (!levelinfo[i].encore)
        ...
        V_DrawFixedPatch((BASEVIDWIDTH - 20) << FRACBITS, y << FRACBITS,
                         FRACUNIT / 4, V_FLIP | V_SNAPTORIGHT, pic, 0);
        V_DrawFixedPatch((BASEVIDWIDTH - 40) << FRACBITS,
                         (y << FRACBITS) + (25 << (FRACBITS - 1)) - rubyheight,
                         FRACUNIT / 2, V_SNAPTORIGHT, rubyicon, NULL);
      }
  ...
  for (i = 0; i < MAXPLAYERS; i++) {
    if (dedicated && i == 0) // While leaving blank spots for non-existent
                             // players is largely intentional, the first spot
                             // *always* being blank looks a tad silly :V
      continue;
    ...
    if ((playeringame[i] && !players[i].spectator) && votes[i] != -1) {
      ...
      if (!timer && i == voteclient.ranim) {
        ...
        if (voteendtic != -1 && !(votetic % 4))
          ...
          V_DrawFill(x - 1, y - 1, 42, 27,
                     levelinfo[votes[i]].gtc | V_SNAPTOLEFT);
      }

      if (!levelinfo[votes[i]].encore)
        V_DrawTinyScaledPatch(x, y, V_SNAPTOLEFT, pic);
      else {
        V_DrawFixedPatch((x + 40) << FRACBITS, (y) << FRACBITS, FRACUNIT / 4,
                         V_SNAPTOLEFT | V_FLIP, pic, 0);
        V_DrawFixedPatch((x + 20) << FRACBITS,
                         (y << FRACBITS) + (25 << (FRACBITS - 1)) - rubyheight,
                         FRACUNIT / 2, V_SNAPTOLEFT, rubyicon, NULL);
      }
      ...
      if (players[i].skincolor) {
        UINT8 *colormap = R_GetTranslationColormap(
            players[i].skin, players[i].skincolor, GTC_CACHE);
        V_DrawMappedPatch(x + 24, y + 9, V_SNAPTOLEFT,
                          facerankprefix[players[i].skin], colormap);
      }

      if (!splitscreen && i == consoleplayer) {
        UINT8 cursorframe = (votetic / 4) % 8;
        V_DrawScaledPatch(
            x + 24, y + 9, V_SNAPTOLEFT,
            W_CachePatchName(va("K_CHILI%d", cursorframe + 1), PU_CACHE));
      }
    }
...
static void Y_VoteStops(SINT8 pick, SINT8 level) {
  ...
  if (mapheaderinfo[nextmap] &&
      (mapheaderinfo[nextmap]->menuflags & LF2_HIDEINMENU))
...
void Y_VoteTicker(void) {
  ...
  for (i = 0; i < MAXPLAYERS; i++) // Correct votes as early as possible, before
                                   // they're processed by the game at all
  {
    if (!playeringame[i] || players[i].spectator)
      votes[i] = -1; // Spectators are the lower class, and have effectively no
                     // voice in the government. Democracy sucks.
    else if (pickedvote != -1 && votes[i] == -1)
      votes[i] = 3; // Slow people get random
  }

  if (server && pickedvote != -1 &&
      votes[pickedvote] ==
          -1) // Uh oh! The person who got picked left! Recalculate, quick!
    D_PickVote();
  ...
  if (pickedvote != -1) {
    ...
    if (voteendtic == -1) {
      ...
      if (voteclient.rendoff == 0 || voteclient.roffset < voteclient.rendoff)
        voteclient.ranim =
            tempvotes[((pickedvote + voteclient.roffset) % numvotes)];

      if (voteclient.roffset >= 20) {
        if (voteclient.rendoff == 0) {
          if (voteclient.rsynctime % 51 ==
              0) // Song is 1.45 seconds long (sorry @ whoever wants to replace
                 // it in a music wad :V)
          {
            for (i = 5; i >= 3; i--) // Find a suitable place to stop
            {
              if (tempvotes[((pickedvote + voteclient.roffset + i) %
                             numvotes)] == pickedvote) {
    ...
    if (votetic < 3 * (NEWTICRATE /
                       7)) // give it some time before letting you control it :V
      return;
    ...
    for (i = 0; i <= splitscreen; i++) {
      ...
      if ((playeringame[p] && !players[p].spectator) &&
          !voteclient.playerinfo[i].delay && pickedvote == -1 &&
          votes[p] == -1) {
        ...
        if ((InputDown(gc_aimbackward, i + 1) || JoyAxis(AXISAIM, i + 1) > 0) &&
            !pressed) {
          voteclient.playerinfo[i].selection++;
          pressed = true;
        }
        ...
        if ((InputDown(gc_accelerate, i + 1) || JoyAxis(AXISMOVE, i + 1) > 0) &&
            !pressed) {
...
void Y_StartVote(void) {
  ...
  widebgpatch = W_CachePatchName(
      ((prefgametype == GT_MATCH) ? "BATTLSCW" : "INTERSCW"), PU_STATIC);
  bgpatch = W_CachePatchName(
      ((prefgametype == GT_MATCH) ? "BATTLSCR" : "INTERSCR"), PU_STATIC);
  ...
  for (i = 0; i < 4; i++) {
    ...
    // set up the levelstring
    if (mapheaderinfo[votelevels[i][0]]->levelflags & LF_NOZONE ||
        !mapheaderinfo[votelevels[i][0]]->zonttl[0]) {
      if (mapheaderinfo[votelevels[i][0]]->actnum[0])
        snprintf(levelinfo[i].str, sizeof levelinfo[i].str, "%s %s",
                 mapheaderinfo[votelevels[i][0]]->lvlttl,
                 mapheaderinfo[votelevels[i][0]]->actnum);
      else
        snprintf(levelinfo[i].str, sizeof levelinfo[i].str, "%s",
                 mapheaderinfo[votelevels[i][0]]->lvlttl);
    } else {
      if (mapheaderinfo[votelevels[i][0]]->actnum[0])
        snprintf(levelinfo[i].str, sizeof levelinfo[i].str, "%s %s %s",
                 mapheaderinfo[votelevels[i][0]]->lvlttl,
                 mapheaderinfo[votelevels[i][0]]->zonttl,
                 mapheaderinfo[votelevels[i][0]]->actnum);
      else
        snprintf(levelinfo[i].str, sizeof levelinfo[i].str, "%s %s",
                 mapheaderinfo[votelevels[i][0]]->lvlttl,
    ...
    // set up the pic
    lumpnum =
        W_CheckNumForName(va("%sP", G_BuildMapName(votelevels[i][0] + 1)));
    if (lumpnum != LUMPERROR)
      levelinfo[i].pic = W_CachePatchName(
          va("%sP", G_BuildMapName(votelevels[i][0] + 1)), PU_STATIC);
    else
