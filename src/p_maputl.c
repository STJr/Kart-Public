...
///        Blockmap iterator functions, and some PIT_* functions to use for
///        iteration
...
//
void P_ClosestPointOnLine(fixed_t x, fixed_t y, line_t *line,
                          vertex_t *result) {
...
//
void P_ClosestPointOnLine3D(fixed_t x, fixed_t y, fixed_t z, line_t *line,
                            vertex_t *result) {
  fixed_t startx = line->v1->x;
  fixed_t starty = line->v1->y;
  ...
  // Normalize (&V, &V);
  magnitude = R_PointToDist2(
      0, line->v2->z, R_PointToDist2(line->v2->x, line->v2->y, startx, starty),
      startz);
  vx = FixedDiv(vx, magnitude);
  vy = FixedDiv(vy, magnitude);
...
void P_CameraLineOpening(line_t *linedef) {
  ...
  if (front->camsec >= 0) {
    ...
    if (sectors[front->camsec]
            .f_slope) // SRB2CBTODO: ESLOPE (sectors[front->heightsec].f_slope)
      frontfloor =
          P_GetZAt(sectors[front->camsec].f_slope, camera[0].x, camera[0].y);
    if (sectors[front->camsec].c_slope)
      frontceiling =
          P_GetZAt(sectors[front->camsec].c_slope, camera[0].x, camera[0].y);
    ...
    if (sectors[front->heightsec]
            .f_slope) // SRB2CBTODO: ESLOPE (sectors[front->heightsec].f_slope)
      frontfloor =
          P_GetZAt(sectors[front->heightsec].f_slope, camera[0].x, camera[0].y);
    if (sectors[front->heightsec].c_slope)
      frontceiling =
          P_GetZAt(sectors[front->heightsec].c_slope, camera[0].x, camera[0].y);
  } else {
  ...
  if (back->camsec >= 0) {
    ...
    if (sectors[back->camsec]
            .f_slope) // SRB2CBTODO: ESLOPE (sectors[front->heightsec].f_slope)
      frontfloor =
          P_GetZAt(sectors[back->camsec].f_slope, camera[0].x, camera[0].y);
    if (sectors[back->camsec].c_slope)
      frontceiling =
          P_GetZAt(sectors[back->camsec].c_slope, camera[0].x, camera[0].y);
  } else if (back->heightsec >= 0) {
    backfloor = sectors[back->heightsec].floorheight;
    backceiling = sectors[back->heightsec].ceilingheight;
    if (sectors[back->heightsec]
            .f_slope) // SRB2CBTODO: ESLOPE (sectors[front->heightsec].f_slope)
      frontfloor =
          P_GetZAt(sectors[back->heightsec].f_slope, camera[0].x, camera[0].y);
    if (sectors[back->heightsec].c_slope)
      frontceiling =
          P_GetZAt(sectors[back->heightsec].c_slope, camera[0].x, camera[0].y);
  } else {
    ...
    if (front->ffloors || back->ffloors) {
      ...
      if (front->ffloors)
        for (rover = front->ffloors; rover; rover = rover->next) {
          ...
          if (!(rover->flags & FF_BLOCKOTHERS) ||
              !(rover->flags & FF_RENDERALL) || !(rover->flags & FF_EXISTS) ||
              GETSECSPECIAL(rover->master->frontsector->special, 4) == 12)
            continue;

          topheight = P_CameraGetFOFTopZ(mapcampointer, front, rover, tmx, tmy,
                                         linedef);
          bottomheight = P_CameraGetFOFBottomZ(mapcampointer, front, rover, tmx,
                                               tmy, linedef);

          delta1 = abs(mapcampointer->z -
                       (bottomheight + ((topheight - bottomheight) / 2)));
          delta2 =
              abs(thingtop - (bottomheight + ((topheight - bottomheight) / 2)));
      ...
      if (back->ffloors)
        for (rover = back->ffloors; rover; rover = rover->next) {
          ...
          topheight =
              P_CameraGetFOFTopZ(mapcampointer, back, rover, tmx, tmy, linedef);
          bottomheight = P_CameraGetFOFBottomZ(mapcampointer, back, rover, tmx,
                                               tmy, linedef);
...
void P_LineOpening(line_t *linedef, mobj_t *mobj) {
  ...
  if (mobj) {
    ...
    if (linedef->flags & ML_EFFECT4 &&
      ...
      INT32 texnum = R_GetTextureNum(
          side->midtexture); // make sure the texture is actually valid

      if (texnum) {
        // Get the midtexture's height
        texheight = textures[texnum]->height << FRACBITS;

        // Set texbottom and textop to the Z coordinates of the texture's
        // boundaries
      ...
      // on non-solid polyobjects should NEVER happen in the future
...
#endif
        {
          if (linedef->flags & ML_EFFECT5 &&
              !side->repeatcnt) { // "infinite" repeat
    ...
    if (front->ffloors || back->ffloors || linedef->polyobj) {
      ...
      for (rover = front->ffloors; rover; rover = rover->next) {
        ...
        delta1 =
            abs(mobj->z - (bottomheight + ((topheight - bottomheight) / 2)));
        delta2 =
            abs(thingtop - (bottomheight + ((topheight - bottomheight) / 2)));

        if (delta1 >= delta2 &&
            !(rover->flags & FF_PLATFORM)) // thing is below FOF
        {
        ...
        if (delta1 < delta2 &&
            !(rover->flags & FF_REVERSEPLATFORM)) // thing is above FOF
        {
      ...
      if (linedef->polyobj && (linedef->polyobj->flags & POF_TESTHEIGHT)) {
        ...
        delta1 = abs(mobj->z -
                     (polysec->floorheight +
                      ((polysec->ceilingheight - polysec->floorheight) / 2)));
        delta2 = abs(thingtop -
                     (polysec->floorheight +
                      ((polysec->ceilingheight - polysec->floorheight) / 2)));
...
void P_UnsetThingPosition(mobj_t *thing) {
  ...
  if (!(thing->flags & MF_NOBLOCKMAP)) {
    ...
    if (bprev &&
        (*bprev = bnext = thing->bnext) != NULL) // unlink from block map
      bnext->bprev = bprev;
  }
}
...
void P_UnsetPrecipThingPosition(precipmobj_t *thing) {
  ...
  thing->touching_sectorlist =
      NULL; // to be restored by P_SetPrecipThingPosition
}
...
// Sets thing->subsector properly
//
void P_SetUnderlayPosition(mobj_t *thing) { // link into subsector
  subsector_t *ss;
  mobj_t **link, *lend;
  I_Assert(thing);
...
void P_SetPrecipitationThingPosition(precipmobj_t *thing) {
  ...
  *link = thing;
  ...
  thing->touching_sectorlist =
      precipsector_list;    // Attach to Thing's precipmobj_t
...
boolean P_BlockThingsIterator(INT32 x, INT32 y, boolean (*func)(mobj_t *)) {
  ...
  for (mobj = blocklinks[y * bmapwidth + x]; mobj; mobj = bnext) {
    P_SetTarget(&bnext,
                mobj->bnext); // We want to note our reference to bnext here
                              // incase it is MF_NOTHINK and gets removed!
    if (!func(mobj)) {
      P_SetTarget(&bnext, NULL);
      return false;
    }
    if (P_MobjWasRemoved(
            tmthing) // func just popped our tmthing, cannot continue.
        || (bnext &&
            P_MobjWasRemoved(
                bnext))) // func just broke blockmap chain, cannot continue.
    {
      P_SetTarget(&bnext, NULL);
      return true;
    }
...
static void P_CheckIntercepts(void) {
  ...
  if (max_intercepts <= count) {
    ...
    intercepts = Z_Realloc(intercepts, sizeof(*intercepts) * max_intercepts,
                           PU_STATIC, NULL);
