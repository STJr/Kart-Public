...
FUNCINLINE static ATTRINLINE void Polyobj_bboxAdd(fixed_t *bbox,
                                                  vertex_t *add) {
  bbox[BOXTOP] += add->y;
...
FUNCINLINE static ATTRINLINE void Polyobj_bboxSub(fixed_t *bbox,
                                                  vertex_t *sub) {
  bbox[BOXTOP] -= sub->y;
...
FUNCINLINE static ATTRINLINE void Polyobj_vecSub2(vertex_t *dst, vertex_t *v1,
                                                  vertex_t *v2) {
  dst->x = v1->x - v2->x;
  dst->y = v1->y - v2->y;
}
...
// Unlike P_AddThinker, this adds it to the front of the list instead of the
// back, so that carrying physics can work right. -Red
...
//
void Polyobj_GetInfo(INT16 tag, INT32 *polyID, INT32 *mirrorID,
                     UINT16 *exparg) {
...
static void Polyobj_addVertex(polyobj_t *po, vertex_t *v) {
  ...
  if (po->numVertices >= po->numVerticesAlloc) {
    ...
    po->vertices = (vertex_t **)Z_Realloc(
        po->vertices, po->numVerticesAlloc * sizeof(vertex_t *), PU_LEVEL,
        NULL);
    po->origVerts = (vertex_t *)Z_Realloc(
        po->origVerts, po->numVerticesAlloc * sizeof(vertex_t), PU_LEVEL, NULL);

    po->tmpVerts = (vertex_t *)Z_Realloc(
        po->tmpVerts, po->numVerticesAlloc * sizeof(vertex_t), PU_LEVEL, NULL);
  }
...
static void Polyobj_addLine(polyobj_t *po, line_t *l) {
  ...
  if (po->numLines >= po->numLinesAlloc) {
    ...
    po->lines = (line_t **)Z_Realloc(
        po->lines, po->numLinesAlloc * sizeof(line_t *), PU_LEVEL, NULL);
  }
  l->polyobj = po;
...
static void Polyobj_addSeg(polyobj_t *po, seg_t *seg) {
  if (po->segCount >= po->numSegsAlloc) {
    ...
    po->segs = (seg_t **)Z_Realloc(po->segs, po->numSegsAlloc * sizeof(seg_t *),
                                   PU_LEVEL, NULL);
  }
...
static void Polyobj_findSegs(polyobj_t *po, seg_t *seg) {
  ...
  if (!(po->flags & POF_ONESIDE)) {
    // Find backfacings
    for (s = 0; s < numsegs; s++) {
      if (segs[s].linedef == seg->linedef && segs[s].side == 1) {
        size_t r;
  ...
  for (i = 0; i < numsegs; ++i) {
    ...
    if (segs[i].v1->x == seg->v2->x && segs[i].v1->y == seg->v2->y) {
      ...
      if (!(po->flags & POF_ONESIDE)) {
        // Find backfacings
        for (q = 0; q < numsegs; q++) {
          if (segs[q].linedef == segs[i].linedef && segs[q].side == 1) {
            size_t r;
            for (r = 0; r < po->segCount; r++) {
              if (po->segs[r] == &segs[q])
                break;
  ...
  for (i = 0; i < numsegs; ++i) {
    ...
    if (polyID == po->id && parentID > 0) {
      if (numSegItems >= numSegItemsAlloc) {
        ...
        segitems = Z_Realloc(segitems, numSegItemsAlloc * sizeof(segitem_t),
                             PU_STATIC, NULL);
      }
...
static void Polyobj_spawnPolyObj(INT32 num, mobj_t *spawnSpot, INT32 id) {
  ...
  for (i = 0; i < numsegs; ++i) {
    ...
    if (polyID == po->id) {
      ...
      po->spawnflags =
          po->flags; // save original flags to reference later for netgames!
...
static void Polyobj_linkToBlockmap(polyobj_t *po) {
  ...
  // 2/26/06: start line box with values of first vertex, not
  // INT32_MIN/INT32_MAX
  ...
  blockbox[BOXRIGHT] =
      (unsigned)(blockbox[BOXRIGHT] - bmaporgx) >> MAPBLOCKSHIFT;
  ...
  blockbox[BOXBOTTOM] =
      (unsigned)(blockbox[BOXBOTTOM] - bmaporgy) >> MAPBLOCKSHIFT;
...
static inline boolean Polyobj_untouched(line_t *ld, mobj_t *mo) {
  ...
  return (ptmbbox[BOXRIGHT] = (x = mo->x) + mo->radius) <= ld->bbox[BOXLEFT] ||
...
// Polyobj_slideThing
//
// Moves an object resting on top of a polyobject by (x, y). Template function
// to make alteration easier.
//
...
#define SPINMULT                                                               \
  5184 // Consider this a substitute for properly calculating FRACUNIT-friction.
       // I'm tired. -Red
...
static void Polyobj_carryThings(polyobj_t *po, fixed_t dx, fixed_t dy) {
  ...
  for (y = po->blockbox[BOXBOTTOM]; y <= po->blockbox[BOXTOP]; ++y) {
    for (x = po->blockbox[BOXLEFT]; x <= po->blockbox[BOXRIGHT]; ++x) {
      ...
      for (; mo; mo = mo->bnext) {
        // lastlook is used by the SPB to determine targets, do not let it
        // affect it
        if (mo->type == MT_SPB)
          continue;
        ...
        if (mo->flags & MF_NOGRAVITY)
          continue;
        // (The above check used to only move MF_SOLID objects, but that's
        // inconsistent with conveyor behavior. -Red)

        if (mo->flags & MF_NOCLIP)
          continue;

        if ((mo->eflags & MFE_VERTICALFLIP) &&
            mo->z + mo->height != po->lines[0]->backsector->floorheight)
          continue;

        if (!(mo->eflags & MFE_VERTICALFLIP) &&
            mo->z != po->lines[0]->backsector->ceilingheight)
          continue;
...
static INT32 Polyobj_clipThings(polyobj_t *po, line_t *line) {
  ...
  linebox[BOXLEFT] =
      (unsigned)(line->bbox[BOXLEFT] - bmaporgx - MAXRADIUS) >> MAPBLOCKSHIFT;
  linebox[BOXRIGHT] =
      (unsigned)(line->bbox[BOXRIGHT] - bmaporgx + MAXRADIUS) >> MAPBLOCKSHIFT;
  linebox[BOXBOTTOM] =
      (unsigned)(line->bbox[BOXBOTTOM] - bmaporgy - MAXRADIUS) >> MAPBLOCKSHIFT;
  linebox[BOXTOP] =
      (unsigned)(line->bbox[BOXTOP] - bmaporgy + MAXRADIUS) >> MAPBLOCKSHIFT;
    ...
    for (x = linebox[BOXLEFT]; x <= linebox[BOXRIGHT]; ++x) {
      if (!(x < 0 || y < 0 || x >= bmapwidth || y >= bmapheight)) {
        ...
        for (; mo; mo = mo->bnext) {
          ...
          if (mo->player &&
              (po->lines[0]->backsector->flags & SF_TRIGGERSPECIAL_TOUCH) &&
              !(po->flags & POF_NOSPECIALS))
            P_ProcessSpecialSector(mo->player, mo->subsector->sector,
...
//
static inline void Polyobj_rotatePoint(vertex_t *v, const vertex_t *c,
                                       angle_t ang) {
  vertex_t tmp = *v;
...
//
// Causes objects resting on top of the rotating polyobject to 'ride' with its
// movement.
//
static void Polyobj_rotateThings(polyobj_t *po, vertex_t origin, angle_t delta,
                                 UINT8 turnthings) {
    ...
    for (x = po->blockbox[BOXLEFT]; x <= po->blockbox[BOXRIGHT]; ++x) {
      ...
      for (; mo; mo = mo->bnext) {
...

        {
          ...
          if (mo->player) // Hack to fix players sliding off of spinning polys
                          // -Red
          {
            fixed_t temp;
...
void Polyobj_InitLevel(void) {
  ...
  if (numPolyObjects) {
    ...
    PolyObjects = Z_Calloc(numPolyObjects * sizeof(polyobj_t), PU_LEVEL, NULL);
...
#else
  {
    CONS_Debug(DBG_POLYOBJ,
               "T_PolyObjRotate: thinker with invalid id %d removed.\n",
               th->polyObjNum);
...
//
FUNCINLINE static ATTRINLINE void Polyobj_componentSpeed(INT32 resVel,
                                                         INT32 angle,
                                                         fixed_t *xVel,
                                                         fixed_t *yVel) {
  if (angle == 0) {
...
void T_PolyObjMove(polymove_t *th) {
  ...
  {
    CONS_Debug(DBG_POLYOBJ,
               "T_PolyObjMove: thinker with invalid id %d removed.\n",
               th->polyObjNum);
    P_RemoveThinkerDelayed(&th->thinker);
    return;
  }
...
void T_PolyObjWaypoint(polywaypoint_t *th) {
  ...
  {
    CONS_Debug(DBG_POLYOBJ,
               "T_PolyObjWaypoint: thinker with invalid id %d removed.",
               th->polyObjNum);
    P_RemoveThinkerDelayed(&th->thinker);
    return;
  }
  ...
  if (!target) {
    CONS_Debug(DBG_POLYOBJ,
               "T_PolyObjWaypoint: Unable to find target waypoint!\n");
    return;
  }
  ...
  adjustz = po->lines[0]->backsector->floorheight +
            (po->lines[0]->backsector->ceilingheight -
             po->lines[0]->backsector->floorheight) /
                2 +
            th->diffz;

  dist =
      P_AproxDistance(P_AproxDistance(target->x - adjustx, target->y - adjusty),
                      target->z - adjustz);
  ...
  // its current coordinates, or closer? (shift down to fracunits to avoid
  // approximation errors)
  if (dist >> FRACBITS <=
      P_AproxDistance(P_AproxDistance(target->x - adjustx - momx,
                                      target->y - adjusty - momy),
                      target->z - adjustz - momz) >>
      FRACBITS) {
    ...
    amtz = (po->lines[0]->backsector->ceilingheight -
            po->lines[0]->backsector->floorheight) /
           2;
    ...
    while ((po = Polyobj_GetChild(oldpo, &start))) {
      ...
      // TODO: use T_MovePlane
      po->lines[0]->backsector->floorheight +=
          diffz; // move up/down by same amount as the parent did
    ...
    if (!th->stophere) {
      ...
      for (wp = thinkercap.next; wp != &thinkercap; wp = wp->next) {
        if (wp->function.acp1 !=
            (actionf_p1)P_MobjThinker) // Not a mobj thinker
          continue;
    ...
    if (waypoint) {
      ...
      dist = P_AproxDistance(
          P_AproxDistance(target->x - adjustx, target->y - adjusty),
          target->z - adjustz);
  ...
  // Sal: Remember to check your sectors!
  P_CheckSector(po->lines[0]->frontsector,
                (boolean)(po->damage)); // frontsector is NEEDED for crushing
  P_CheckSector(po->lines[0]->backsector,
                (boolean)(po->damage)); // backsector may not be necessary, but
                                        // just in case
...
void T_PolyDoorSlide(polyslidedoor_t *th) {
  ...
  {
    CONS_Debug(DBG_POLYOBJ,
               "T_PolyDoorSlide: thinker with invalid id %d removed.\n",
               th->polyObjNum);
    P_RemoveThinkerDelayed(&th->thinker);
    return;
  }
...
void T_PolyDoorSwing(polyswingdoor_t *th) {
  ...
  {
    CONS_Debug(DBG_POLYOBJ,
               "T_PolyDoorSwing: thinker with invalid id %d removed.\n",
               th->polyObjNum);
    P_RemoveThinkerDelayed(&th->thinker);
    return;
  }
...
void T_PolyObjDisplace(polydisplace_t *th) {
  ...
  newheights =
      th->controlSector->floorheight + th->controlSector->ceilingheight;
...
INT32
EV_DoPolyObjRotate(polyrotdata_t *prdata) {
  polyobj_t *po;
  polyobj_t *oldpo;
  polyrotate_t *th;
  INT32 start;
...
INT32
EV_DoPolyObjMove(polymovedata_t *pmdata) {
  polyobj_t *po;
  polyobj_t *oldpo;
  polymove_t *th;
  INT32 start;
...
INT32
EV_DoPolyObjWaypoint(polywaypointdata_t *pwdata) {
  polyobj_t *po;
  polyobj_t *oldpo;
  polywaypoint_t *th;
  mobj_t *mo2;
  ...
  if (!first) {
    CONS_Debug(DBG_POLYOBJ,
               "EV_DoPolyObjWaypoint: Missing starting waypoint!\n");
    po->thinker = NULL;
    P_RemoveThinker(&th->thinker);
    return 0;
  }
  ...
  th->diffz = 0; // first->z - (po->lines[0]->backsector->floorheight +
                 // (po->lines[0]->backsector->ceilingheight -
                 // po->lines[0]->backsector->floorheight)/2);

  if (last->x == po->centerPt.x && last->y == po->centerPt.y &&
      last->z == (po->lines[0]->backsector->floorheight +
                  (po->lines[0]->backsector->ceilingheight -
                   po->lines[0]->backsector->floorheight) /
                      2)) {
  ...
  /*for (wp = thinkercap.next; wp != &thinkercap; wp = wp->next)
  {
          if (wp->function.acp1 != (actionf_p1)P_MobjThinker) // Not a mobj
  thinker continue;
  ...
  R_CreateInterpolator_SectorPlane(&th->thinker, po->lines[0]->backsector,
                                   false);
  R_CreateInterpolator_SectorPlane(&th->thinker, po->lines[0]->backsector,
                                   true);
...
INT32
EV_DoPolyDoor(polydoordata_t *doordata) {
  polyobj_t *po;
  ...
  case POLY_DOOR_SWING:
    ...
    break;
  default:
    CONS_Debug(DBG_POLYOBJ, "EV_DoPolyDoor: unknown door type %d",
               doordata->doorType);
    return 0;
  }
...
INT32
EV_DoPolyObjDisplace(polydisplacedata_t *prdata) {
  polyobj_t *po;
  polyobj_t *oldpo;
  polydisplace_t *th;
  INT32 start;
...
void T_PolyObjFlag(polymove_t *th) {
  ...
  {
    CONS_Debug(DBG_POLYOBJ,
               "T_PolyObjFlag: thinker with invalid id %d removed.\n",
               th->polyObjNum);
    P_RemoveThinkerDelayed(&th->thinker);
    return;
  }
...
INT32
EV_DoPolyObjFlag(line_t *pfdata) {
  polyobj_t *po;
  polyobj_t *oldpo;
  polymove_t *th;
  size_t i;
  ...
  if (po->numVertices & 1) {
    CONS_Debug(DBG_POLYOBJ,
               "EV_DoPolyFlag: Polyobject has odd # of vertices!\n");
    return 0;
  }
  ...
  th->angle = R_PointToAngle2(pfdata->v1->x, pfdata->v1->y, pfdata->v2->x,
                              pfdata->v2->y) >>
              ANGLETOFINESHIFT;
