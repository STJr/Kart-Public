...
static const char *const mobj_opt[] = {"valid",
                                       "x",
                                       "y",
                                       "z",
                                       "snext",
...
#define UNIMPLEMENTED                                                          \
  luaL_error(                                                                  \
      L,                                                                       \
      LUA_QL("mobj_t") " field " LUA_QS                                        \
                       " is not implemented for Lua and cannot be accessed.",  \
      mobj_opt[field])
...
static int mobj_get(lua_State *L) {
  ...
  case mobj_sprev:
    ...
    // i.e. it will always ultimately point to THIS mobj -- so that's actually
    // not useful to Lua and won't be included.
  ...
  case mobj_target:
    if (mo->target &&
        P_MobjWasRemoved(mo->target)) { // don't put invalid mobj back into Lua.
  ...
  case mobj_tracer:
    if (mo->tracer &&
        P_MobjWasRemoved(mo->tracer)) { // don't put invalid mobj back into Lua.
...
#define NOSET                                                                  \
  luaL_error(L,                                                                \
             LUA_QL("mobj_t") " field " LUA_QS " should not be set directly.", \
             mobj_opt[field])
#define NOSETPOS                                                                            \
  luaL_error(                                                                               \
      L,                                                                                    \
      LUA_QL(                                                                               \
          "mobj_t") " field " LUA_QS " should not be set directly. "                        \
                    "Use " LUA_QL("P_Move") ", " LUA_QL(                                    \
                        "P_"                                                                \
                        "Tr"                                                                \
                        "yM"                                                                \
                        "ov"                                                                \
                        "e") ", or " LUA_QL("P_SetOrigin") ", or " LUA_QL("P_"              \
                                                                          "Mo"              \
                                                                          "ve"              \
                                                                          "Or"              \
                                                                          "ig"              \
...
static int mobj_set(lua_State *L) {
  ...
  case mobj_color: {
    ...
    if (newcolor >= MAXTRANSLATIONS)
      return luaL_error(L, "mobj.color %d out of range (0 - %d).", newcolor,
                        MAXTRANSLATIONS - 1);
    mo->color = newcolor;
    break;
  }
  ...
  case mobj_type: // yeah sure, we'll let you change the mobj's type.
  {
    ...
    if (newtype >= NUMMOBJTYPES)
      return luaL_error(L, "mobj.type %d out of range (0 - %d).", newtype,
                        NUMMOBJTYPES - 1);
    mo->type = newtype;
  ...
  case mobj_spawnpoint:
    if (lua_isnil(L, 3))
      ...
      mapthing_t *spawnpoint =
          *((mapthing_t **)luaL_checkudata(L, 3, META_MAPTHING));
...
static int mapthing_get(lua_State *L) {
  ...
  if (fastcmp(field, "valid")) {
    ...
    return luaL_error(L, LUA_QL("mapthing_t") " has no field named " LUA_QS,
                      field);
  else
    return 0;
...
static int lib_iterateMapthings(lua_State *L) {
  ...
  if (lua_gettop(L) < 2)
    return luaL_error(L,
                      "Don't call mapthings.iterate() directly, use it as 'for "
                      "mapthing in mapthings.iterate do <block> end'.");
  lua_settop(L, 2);
  lua_remove(L, 1); // state is unused.
  if (!lua_isnil(L, 1))
    i = (size_t)(*((mapthing_t **)luaL_checkudata(L, 1, META_MAPTHING)) -
                 mapthings) +
        1;
  if (i < nummapthings) {
