...
#define Blame(...) CONS_Printf("\x85" __VA_ARGS__)
...
static void Contact_error(void) {
  CONS_Alert(CONS_ERROR,
             "There was a problem contacting the master server...\n");
}

static void Printf_url(const char *url) {
  boolean startup;
  ...
  (startup ? I_OutputMsg : CONS_Printf)("HMS: connecting '%s'...\n", url);
}

static size_t HMS_on_read(char *s, size_t _1, size_t n, void *userdata) {
...
static struct HMS_buffer *HMS_connect(const char *format, ...) {
  ...
  va_start(ap, format);
  url = malloc(seek + vsnprintf(0, 0, format, ap) + sizeof HMS_QUERY_VERSION -
               1 + token_length + 1);
  va_end(ap);
...
static int HMS_do(struct HMS_buffer *buffer) {
  CURLcode cc;
  long status;
  ...
  if (status != 200) {
    ...
    Contact_error();
    Blame("Master server error %ld: %s%s\n", status, buffer->buffer,
          ((p) ? "" : " (malformed)"));
...
static void HMS_end(struct HMS_buffer *buffer) {
...
int HMS_register(void) {
  ...
  hms =
      HMS_connect("games/%s/%d/servers/register", SRB2APPLICATION, MODVERSION);
...
int HMS_update(void) {
  ...
  snprintf(post, sizeof post, "title=%s", title);
...
static char *Strip_trailing_slashes(char *api) {
